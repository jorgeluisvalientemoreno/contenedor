DECLARE
  ...
  CURSOR cu_audsid3 IS
    SELECT * FROM gv$session WHERE MODULE = 'EXECUTOR_PROCESS';

  audsid        gv$session%rowtype;
  ejecutor      NUMBER;
  hilo          NUMBER;
  posInicio     NUMBER;
  posFin        NUMBER;
...
BEGIN
  ...
  if nuThreadCount > 0 then
    FOR audsid IN cu_audsid3 LOOP
      ...
      GE_BOExec_Server_MSG.Put_ServerMSG(Ge_BOExec_Server_MSG.csbMSG_STOP_EXIT,
                                         null,
                                         ejecutor,
                                         hilo,
                                         audsid.audsid);
