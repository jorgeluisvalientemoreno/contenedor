...
DECLARE CURSOR cuGetSesion IS
  SELECT sid FROM gv$session WHERE module = 'PROCESS_SERVER';

CURSOR cuValidateSession(IDEN NUMBER) IS
  SELECT UPPER(q.sql_text) "texto"
    FROM gv$session s, gv$sqltext q
...
DECLARE CURSOR cuDifHilos IS
  WITH base as
   (select process_executor_id,
           'Executor=' || e.process_executor_id ejecutor,
           e.thread hilo,
           count(s.SID) cantidad
      from open.ge_process_executor e
      left join gv$session s
        on s.MODULE = 'EXECUTOR_PROCESS'
       and s.username = 'OPEN'
       and 'Executor=' || e.process_executor_id =
           substr(client_info, 0, instr(client_info, ' ') - 1)
     group by e.process_executor_id, e.thread)
  SELECT b.*, hilo - cantidad faltante
    FROM base b
...
begin
  ...
  FOR reg IN cuDifHilos LOOP
    ...
    WHILE nuContador <= reg.faltante LOOP
      ...
      DBMS_OUTPUT.PUT_LINE('Faltante Contador: ' || reg.process_executor_id);
