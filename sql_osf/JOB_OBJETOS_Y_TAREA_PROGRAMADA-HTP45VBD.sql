select distinct gps.process_schedule_id Tarea_Programada,
 ...
 inner join open.ge_object g
    ON 1 = 1
      --and upper(g.name_) like upper('%LDC_CAMBESTDLIC%')
...
select * from open.ge_object g where upper(g.name_) like upper('%BOASIG%'); --LDC_BO_GESTIONSUSPSEG.Ldc_job_suspensecleg

select gps.*
  from open.ge_process_schedule gps,
       (select g.object_id
          from open.ge_object g
         where upper(g.name_) like upper('%BOASIG%')) gobject
