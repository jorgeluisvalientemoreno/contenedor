SELECT cu.operating_unit_id,
       ...
       (select oou.name
         ...
         where oou.operating_unit_id = a.operating_unit_id) nombre,
       cu.items_id,
       --open.dage_items.fsbgetdescription(cu.items_id) 
       (select gi.description
          from open.ge_items gi
         where gi.items_id = cu.items_id) descripcion,
       a.balance,
       (select balance
  ...
  FROM (SELECT operating_unit_id,
          ...
          FROM (SELECT o.operating_unit_id,
                       items_id,
                       (CASE
                         WHEN o.movement_type = 'D' AND
                              o.item_moveme_caus_id = 4 THEN
                         ...
                         WHEN --open.daor_operating_unit.fnugetoper_unit_classif_id(o.target_oper_unit_id, null) 
                          (select oou.oper_unit_classif_id
                             from open.or_operating_unit oou
                            where oou.operating_unit_id = o.target_oper_unit_id) = 11 AND
                         --open.daor_operating_unit.fsbgetname(o.target_oper_unit_id, null) 
                          (select oou.name
                             from open.or_operating_unit oou
                            where oou.operating_unit_id = o.target_oper_unit_id) like
                          '%ACTIVO%' THEN
                          'A'
                         WHEN --open.daor_operating_unit.fnugetoper_unit_classif_id(o.target_oper_unit_id, null) 
                          (select oou.oper_unit_classif_id
                         ...
                         --open.daor_operating_unit.fsbgetname(o.target_oper_unit_id, null) 
                          ...
                          '%INVENTARIO%' THEN
                          'I'
                         WHEN d.causal_id = 3368 THEN
                          'A'
                         WHEN d.causal_id = 3369 THEN
                          'I'
                         WHEN ITEMS_ID = 10004070 THEN
                          'I'
                  ...
                  FROM open.or_uni_item_bala_mov o
                  LEFT JOIN open.ge_items_documento d
                    ON (o.id_items_documento = d.id_items_documento)
                 ...
                 GROUP BY o.operating_unit_id,
                          ...
                          o.movement_type,
                          document_type_id,
                          o.item_moveme_caus_id,
                          d.causal_id,
                          o.target_oper_unit_id,
                          d.documento_externo,
                          o.id_items_documento)
         GROUP BY OPERATING_UNIT_ID, ITEMS_ID
        UNION all
        SELECT B.CUADHOMO OPERATING_UNIT_ID,
               I.ITEMS_ID ITEMS_ID,
               SUM(DECODE(EIXBTIPO, 'A', EIXBDISU, 0)) CANTIDAD_ACTIVO,
               SUM(DECODE(EIXBTIPO, 'A', EIXBVLOR, 0)) VALOR_ACTIVO,
               SUM(DECODE(EIXBTIPO, 'I', EIXBDISU, 0)) CANTIDAD_INV,
               SUM(DECODE(EIXBTIPO, 'I', EIXBVLOR, 0)) VALOR_INVEN,
               SUM(DECODE(EIXBTIPO, 'X', EIXBDISU, 0)) CANTIDAD_X,
               SUM(DECODE(EIXBTIPO, 'X', EIXBVLOR, 0)) VALOR_X,
               SUM(DECODE(EIXBTIPO, 'AJUSTE', EIXBDISU, 0)) CANTIDAD_AJUSTE,
               SUM(DECODE(EIXBTIPO, 'X', EIXBVLOR, 0)) VALOR_AJUSTE
          FROM MIGRA.LDC_TEMP_EXITEBOD_SGE A,
               MIGRA.LDC_MIG_CUADCONT      B,
               OPEN.GE_ITEMS               I
        ...
        --and nvl(MIGRA.FNUGETITEMOSF_ROLLOUT(eixbitem, a.BASEDATO), 0) = I.ITEMS_ID
         GROUP BY B.CUADHOMO, I.ITEMS_ID) cu,
       OPEN.OR_OPE_UNI_ITEM_BALA a
 where cu.operating_unit_id = a.operating_unit_id
   ...
   AND (select balance
          from open.ldc_act_ouib b
         where a.items_id = b.items_id
           and a.operating_unit_id = b.operating_unit_id) <> 0 having
 SUM(CANTIDAD_X) = 0
   and SUM(VALOR_X) = 0
