select a.* from ge_entity a where a.name_ = 'SUSCRIPC';
--BLACK_LIST_PRODUCT
select (select b.attribute_type_id || ' - ' || gat.description
...
select (select b.attribute_type_id || ' - ' || gat.description
       ...
       b.tag_element Nombre_TAG
  from ge_entity_attributes b
 where b.technical_name like upper('%Identificador%cliente%Solicitud%')
    or b.display_name like upper('%Identificador%cliente%Solicitud%')
    or b.comment_ like upper('%Identificador%cliente%Solicitud%');
