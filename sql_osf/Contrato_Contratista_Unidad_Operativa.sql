...
select distinct oo.order_id Orden,
                ...
                (select oos.description
                  ...
                  where oos.order_status_id = oo.order_status_id) Estado,
                oo.is_pending_liq Pendiente_Acta,
                oo.legalization_date Legalizacion,
                oo.defined_contract_id || ' - ' || gcontrato.descripcion Contrato_Orden,
                (select h.name
                   from open.or_operating_unit h
                  where h.operating_unit_id = oo.operating_unit_id) Unidad_Operativa,
                gcontrato.fecha_inicial Fecha_Inicial,
                gcontrato.fecha_final Fecha_Final,
                gcontrato.fecha_cierre Fecha_Cierre,
                gcontrato.id_contratista || ' - ' ||
                gcontratista.descripcion Contratista,
                decode(gcontrato.status,
                       'RG',
                       'RG - Registrado',
                       'AB',
                       'AB - Abierto',
                       ...
                       'CE',
                       'CE - Cerrado') Estado,
                gcontrato.id_tipo_contrato Tipo_Contrato,
                oou.admin_base_id Base_Admi_Unidad_Operativa
/*nvl(ooa.subscription_id, mm.subscription_id) Contrato_Usuario,
nvl(ooa.product_id, mm.product_id) Producto_Usuario,
...
oo.operating_unit_id || ' - ' || oo.created_date Fecha_Creacion_Orden,
...
ga.extern_invoice_num Factura_Pago_Acta*/
  ...
  left join open.ge_contrato gcontrato
    on gcontrato.id_contrato = oo.defined_contract_id
  left join open.or_operating_unit oou
    on oou.operating_unit_id = oo.operating_unit_id
  left join open.ge_contratista gcontratista
    on gcontratista.id_contratista = gcontrato.id_contratista
 where
---Orden
-- 
 ooa.order_id in (355023927,
                  355023928,
                  355024207,
                  355024208,
                  355024348,
...
-- and mp.package_id in (222578325)
