...
/*+ Ubicaci¿n : OR_boSearchDataServices.GetOrdersByNTL */
 WHERE AB_ADDRESS.address_id(+) = OR_ORDER.external_address_id
   ...
   AND fm_possible_ntl.possible_ntl_id = 17014;

-----------------------------------------
--proyecto - prodcuto - ordenes
select distinct fm_possible_ntl.possible_ntl_id, OR_ORDER.ORDER_ID ORDER_ID, or_order_activity.product_id 
  FROM OR_ORDER,
       AB_ADDRESS,
       fm_possible_ntl,
       mo_packages,
...
/*+ Ubicaci¿n : OR_boSearchDataServices.GetOrdersByNTL */
 WHERE AB_ADDRESS.address_id = OR_ORDER.external_address_id
   ...
   AND (fm_possible_ntl.order_id = or_order.order_id OR        
        or_order_activity.package_id = mo_packages.package_id)
   AND fm_possible_ntl.possible_ntl_id = 17014--:inuPossibleNtl
