...
select distinct oo.order_id Orden,
                ...
                (select decode(count(1),
                               0,
                               'No existe configurado',
                               'Existe configuracion')
                   from open.ldc_anexolegaliza lal
                  where lal.order_id = oo.order_id) LEGO
...
--and 
 oo.order_status_id in (0, 5, 7)
-- and oo.causal_id = 9944--in (8, 12)
 and oo.task_type_id in (11259) -- VISITA PARA INSTALACION DE MEDIDOR PREPAGO  
 and ooa.activity_id in (100009441) -- VISITA PARA INSTALACION DE MEDIDOR PREPAGO  )
