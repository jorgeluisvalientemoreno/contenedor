...
SELECT b.operating_unit_id Unidad,
       b.items_id,
       UO.NAME DESC_UNI,
       uo.es_externa Externa,
       b.items_id Item,
       it.description DESC_ITEM,
       it.item_classif_id Clas,
       b.quota,
       b.balance,
       (select LAOUIB.Balance
          from OPEN.LDC_ACT_OUIB LAOUIB
         where LAOUIB.Items_Id = b.items_id
           and LAOUIB.Operating_Unit_Id = b.operating_unit_id) Cant_act,
       (select LIOUIB.Balance
          from OPEN.LDC_INV_OUIB LIOUIB
         where LIOUIB.Items_Id = b.items_id
           and LIOUIB.Operating_Unit_Id = b.operating_unit_id) Cant_inv,
       b.total_costs Costo_total,
       (select LAOUIB.Total_Costs
          from OPEN.LDC_ACT_OUIB LAOUIB
         where LAOUIB.Items_Id = b.items_id
           and LAOUIB.Operating_Unit_Id = b.operating_unit_id) Cost_act,
       (select LIOUIB.Total_Costs
          from OPEN.LDC_INV_OUIB LIOUIB
         where LIOUIB.Items_Id = b.items_id
           and LIOUIB.Operating_Unit_Id = b.operating_unit_id) Cost_inv,
       b.transit_in,
       b.transit_out,
 ...
 WHERE b.items_id = it.items_id
   ...
   AND b.operating_unit_id  in (3979, 1773)
   AND b.items_id IN (10011386);
...
--Actualizado
declare osbMesjErr varchar2(3000);
ol_Payload CLOB;
sesion number;
begin
  DBMS_OUTPUT.PUT_LINE('inicia el proceso pl_movimiento_material.sql');
...
--Se valida el despacho de la requision de SAP a OSF, el campo MMITDSAP debe tener un codigo que proviene de SAP
  select a.*, rowid
    from OPEN.LDCI_INTEMMIT a
   where a.mmitfecr > trunc(sysdate)
    ...
    SELECT /*+  ORDERED 
           ...
           (select b.SERIE
             ...
             WHERE a.SERIAL_ITEMS_ID = b.id_items_seriado
               AND a.asso_serial_items_id =
                   GE_ITEMS_SERIADO.id_items_seriado) SERIAL_ITEMS,
...
---Se validan los seriales devueltos y sin unidad.
  select gis.*
          from OPEN.LDCI_SERIDMIT a, open.ge_items_seriado gis
         where gis.serie = a.serinume
           and a.serimmit = 101257;
