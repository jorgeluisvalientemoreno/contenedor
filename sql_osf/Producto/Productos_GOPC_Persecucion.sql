...
DISTINCT LP.proceso_id CODIGO_PROCESO,
         ...
         (SELECT G.GEO_LOCA_FATHER_ID
           ...
           WHERE G.GEOGRAP_LOCATION_ID =
                 open.DAAB_ADDRESS.FNUGETGEOGRAP_LOCATION_ID(pr.ADDRESS_ID)) Cod_departamento,
         (SELECT G2.DESCRIPTION
            FROM GE_GEOGRA_LOCATION G, GE_GEOGRA_LOCATION G2
           WHERE G2.GEOGRAP_LOCATION_ID = G.GEO_LOCA_FATHER_ID
             AND G.GEOGRAP_LOCATION_ID =
                 open.DAAB_ADDRESS.FNUGETGEOGRAP_LOCATION_ID(pr.ADDRESS_ID)) Departamento,
         open.DAAB_ADDRESS.FNUGETGEOGRAP_LOCATION_ID(pr.ADDRESS_ID) Cod_localidad,
         (SELECT G.DESCRIPTION
            FROM GE_GEOGRA_LOCATION G
           WHERE G.GEOGRAP_LOCATION_ID =
                 open.DAAB_ADDRESS.FNUGETGEOGRAP_LOCATION_ID(pr.ADDRESS_ID)) Localidad
 ...
 where a.ITEMS_ID = SUSP_PERSEC_ACTIVID
   ...
   --and LP.proceso_id = 3
   and susp_persec_depa = 4
   and susp_persec_loca = 192
   --and susp_persec_ciclcodi in (4204)
   --and lpa.activity_id = 4000849
   and ldc_getedadrp(SESUNUSE) <= 54
   AND (SELECT COUNT(p.prod_suspension_id)
          FROM pr_prod_suspension p
         WHERE p.product_id = SUSP_PERSEC_PRODUCTO
           ...
           AND p.active = 'Y') > 0
          -- and rownum = 1
          -- and SUSP_PERSEC_PRODUCTO = 6634386
 order by SUSP_PERSEC_PRODUCTO desc
