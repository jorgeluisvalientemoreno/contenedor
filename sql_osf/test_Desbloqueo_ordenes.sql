...
begin
  ...
  for rfcuOrden in cuOrden loop
    BEGIN
      DBMS_SCHEDULER.CREATE_JOB(job_name   => 'DESBLOQUEO_ORDEN_' || contador,
                                job_type   => 'PLSQL_BLOCK',
                                job_action => 'DECLARE
                                               onuerrorcode numeric;
                                               osberrormessage varchar2(4000);                                               
                                               BEGIN os_unlockorder(inuorderid      => ' ||
                                              rfcuOrden.order_id || ',
                                                                    inucommenttype  => 1296,
                                                                    inucomment      => ''DesBloqueo Orden'',
                                ...
                                auto_drop  => TRUE,
                                enabled    => TRUE,
                                comments   => 'Job DesBloqueo Orden ' ||
                                              rfcuOrden.order_id);
      dbms_output.put_line('Job DESBLOQUEO_ORDEN_' || contador);
    exception
      when others then
