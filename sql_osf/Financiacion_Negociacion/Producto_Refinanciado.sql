 ...
 where 1 = 1
   ...
   and not exists (select null
         ...
         where cc.cuconuse = pr.product_id
           and cc.cucovare > 0)
   and exists (select d2.difenuse, count(distinct(d2.difecofi))
          from open.diferido d2
         where d2.difenuse = pr.product_id
           and d2.difeprog = 'GCNED' having
         count(distinct(d2.difecofi)) > 0
         group by d2.difenuse)
      /*and exists (select null
       from open.diferido d
      where d.difenuse = pr.product_id
        and d.difenucu = 120
        and d.difesape > 0)*/ --con filtro de 120 coutas y saldo pendiente mayor a 0
