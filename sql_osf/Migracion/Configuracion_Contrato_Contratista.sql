...
select t.*
 ...
 where 1 = 1
   and t.status = 'AB'
   and t.id_contrato = nvl(&Contrato, t.id_contrato)
   and t.id_contratista = nvl(&Contratista, t.id_contratista)
   and 1 = 1;
...
--CONTRATO POR FECHA MAXIMA DE ASIGANCION
select *
  from open.LDC_CONTFEMA a
 where a.id_contrato = nvl(&Contrato, a.id_contrato);

--Alcemena la condicion de pago del contrato
select *
  from open.LDC_CONT_PLAN_COND a
 where a.contract_id = nvl(&Contrato, a.contract_id);

--Area relacionada al contrato
select *
  from open.LDC_CONTRATO_AREA a
 where a.id_contrato = nvl(&Contrato, a.id_contrato);

--TIPO DE INCREMENTO POR CONTRATO
select *
  from open.ldc_tipoinc_bycon a
 where a.id_contrato = nvl(&Contrato, a.id_contrato);
...
select *
 ...
 where t.contract_type_id in
       (select tc.id_tipo_contrato
         ...
         where tc.id_contrato = nvl(&Contrato, tc.id_contrato));
...
minus
select t1.unidad_operativa from ldc_const_unoprl t1;

--Tipo de trabajo
select ott.task_type_id
  from open.or_task_type@osfpl ott
minus
select ott.task_type_id from open.or_task_type ott;

--Tipo de trabajo x items
select *
  from open.or_task_types_items@osfpl pltt0
minus
select * from open.or_task_types_items pltt0;
...
minus
select l.task_type_id, l.items_id from LDC_CMMITEMSXTT l;
