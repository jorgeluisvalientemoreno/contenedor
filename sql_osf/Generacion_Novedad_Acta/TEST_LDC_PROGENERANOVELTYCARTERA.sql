DECLARE
  ...
  TYPE LDC_UNI_ACT_OT2_0 IS RECORD(
    unidad_operativa            NUMBER(15),
    actividad                   NUMBER(15),
    nussesion                   NUMBER(15),
    item                        NUMBER(15),
    liquidacion                 VARCHAR2(1),
    cantidad_item_legalizada    NUMBER(10, 4),
    nro_acta                    NUMBER(15),
    actividad_novedad_ofertados NUMBER(15),
    zona_ofertados              NUMBER(15));
  
  TYPE tbLDC_UNI_ACT_OT2_0 IS TABLE OF LDC_UNI_ACT_OT2_0 INDEX BY BINARY_INTEGER;

  vtbtbLDC_UNI_ACT_OT2_0 tbLDC_UNI_ACT_OT2_0;

  nupaacta       open.ge_acta.id_acta%TYPE := 257297;
  dtfechainiacta DATE;
  dtfechafinacta DATE;
  nucantlegaliz  number;
  nuactiv        open.or_order_activity.activity_id%type;
  ...
  CURSOR cuordernesacta(nuparcussion open.ldc_uni_act_ot.nussesion%TYPE,
                        nucurpacta   open.ldc_uni_act_ot.nro_acta%TYPE) IS
  -- se modifica cursor (Cambio 242)
    select t_unidad_operativa unidad_operativa,
           t_actividad actividad,
           t_item item,
           t_liquidacion liquidacion,
           t_zona_ofertados zona_ofertados,
           t_cantidad cantidad,
           (select t2.actividad_novedad_ofertados
              from open.ldc_uni_act_ot2 t2
      ...
      from (select unidad_operativa t_unidad_operativa,
                   decode(actividad_padre, null, actividad, actividad_padre) t_actividad,
                   item t_item,
                   liquidacion t_liquidacion,
                   zona_ofertados t_zona_ofertados,
                   sum(cantidad) t_cantidad
              from (SELECT t.unidad_operativa,
                           t.actividad,
                           (select actividad_padre
                              from open.ldc_act_father_act_hija
                             where actividad_hija = t.actividad) actividad_padre,
                           t.item,
                           t.liquidacion,
                           t.zona_ofertados,
                           t.actividad_novedad_ofertados actividad_novedad_ofertada,
                           nvl(t.cantidad_item_legalizada, 0) cantidad
                      FROM open.ldc_uni_act_ot2 t
                     WHERE t.nussesion = nuparcussion
                       AND t.nro_acta = nucurpacta
                       and sbEntrega554 = 'N')
             group by unidad_operativa,
                      decode(actividad_padre,
                             null,
                             actividad,
                             actividad_padre),
                      item,
    ...
    select unidad_operativa,
           decode(actividad_padre, null, actividad, actividad_padre) actividad,
           item,
           liquidacion,
           zona_ofertados,
           sum(cantidad) cantidad,
           actividad_novedad_ofertada
      from (SELECT t.unidad_operativa,
                   t.actividad,
                   (select actividad_padre
                      from open.ldc_act_father_act_hija
             ...
             WHERE t.nussesion = nuparcussion
               ...
               and sbEntrega554 = 'S')
     group by unidad_operativa,
              decode(actividad_padre, null, actividad, actividad_padre),
              item,
              liquidacion,
              zona_ofertados,
              actividad_novedad_ofertada;
  ...
  CURSOR curangos(nucuunidadoper open.or_operating_unit.operating_unit_id%TYPE,
                  ...
                  nucuzonaofer   open.ldc_zona_ofer_cart.id_zona_oper%TYPE) IS
    SELECT lq.cantidad_inicial, lq.cantidad_final, lq.valor_liquidar
     ...
     WHERE lq.unidad_operativa = nucuunidadoper
       ...
       AND trunc(dtfechafinacta) BETWEEN trunc(lq.fecha_ini_vigen) AND
           trunc(lq.fecha_fin_vige)
     ORDER BY cantidad_inicial;

  cursor cuZonaOfertados(nuUnidOper open.or_operating_unit.operating_unit_id%type) is
    SELECT COUNT(1)
  ...
  nuvaanoper        NUMBER(4);
  nuvamesper        NUMBER(2);
  nucontratoacta    open.ge_acta.id_contrato%TYPE;
  nudireccgene      open.ab_address.address_id%TYPE;
  nulocalidaddirgen open.ab_address.geograp_location_id%TYPE;

  nuZonaOfertados open.ldc_const_liqtarran.zona_ofertados%type;

  -----------------------------------------------------------------------------------------------
  -- Cambio 242
  FUNCTION ldc_getactividadPadre(nuActHija in open.ldc_act_father_act_hija.actividad_hija%type)
    return number is
    ...
    nuActPadre open.ldc_act_father_act_hija.actividad_padre%type;
  ...
  -----------------------------------------------------------------------------------------------
  FUNCTION ldc_getUnidPadre(nuUnidHija in open.ldc_unid_oper_hija_mod_tar.unidad_operativa_hija%type)
    return number is
    -- Cambio 242
    cursor cuGetUnidPadre is
      select uh.unidad_operativa_padre
        from open.ldc_unid_oper_hija_mod_tar uh
       where uh.unidad_operativa_hija = nuUnidHija;
  
    nuUnidPadre open.ldc_unid_oper_hija_mod_tar.unidad_operativa_padre%type;
  
  begin
    open cuGetUnidPadre;
    fetch cuGetUnidPadre
      into nuUnidPadre;
  ...
  PROCEDURE ldcprollenaldcuniactot(nuparacta      open.ge_acta.id_acta%TYPE,
    ...
    CURSOR cuordenesgenerarnovedadact(nucurtacta open.ge_acta.id_acta%TYPE) IS
      select operating_unit_id unidad_operativa,
             actividad,
             nuitemss,
             task_type_id,
             ...
             liquidacion,
             id_zona_oper,
             sum(cantidad_legalizada) cantidad_legalizada
        from (SELECT ot.operating_unit_id,
                     ot.order_id orden,
                     ot.task_type_id,
                     (select cx.actividad_novedad_ofertados
                        from open.ldc_tipo_trab_x_nov_ofertados cx
                       where cx.tipo_trabajo = ot.task_type_id) actividad_novedad_ofertados,
                     ...
                     CASE
                       WHEN (SELECT COUNT(1)
                              ...
                              WHERE bv.unidad_operativa = ot.operating_unit_id
                                AND bv.zona_ofertados = -1
                                AND trunc(dtpasfechaacta) BETWEEN
                                    bv.fecha_ini_vigen AND bv.fecha_fin_vige) >= 1 THEN
                        -1
                       ELSE
                        zo.id_zona_oper
                     END id_zona_oper
              ...
              SELECT ot.operating_unit_id,
               ...
               WHERE oc.certificate_id = nuparacta
                 ...
                 AND ot.operating_unit_id = iu.unidad_operativa
                 AND oa.activity_id =
                     decode(iu.actividad, -1, oa.activity_id, iu.actividad)
                ...
                FROM (SELECT orden,
                        ...
                        FROM (SELECT sca.orden,
                                     ...
                                     CASE
                                       WHEN (SELECT COUNT(1)
                                               FROM open.ldc_const_liqtarran bv
                                              WHERE bv.unidad_operativa =
                                                    sca.unidad_operativa
                                                AND bv.zona_ofertados = -1
                                                AND trunc(dtpasfechaacta) BETWEEN
                                                    bv.fecha_ini_vigen AND
                                                    bv.fecha_fin_vige) >= 1 THEN
                                        -1
                                       ELSE
                                        zona_ofertado
                                     END zona_ofertado,
                                ...
                                FROM (SELECT r.orden_nov_generada           orden,
                                             r.tipo_rabajo,
                                             r.unidad_operativa,
                                       ...
                                       WHERE r.nuano = nuparano
                                         ...
                                         AND r.unidad_operativa in
                                             (select distinct od.operating_unit_id
                                                from open.ct_order_certifica oct,
                                                     open.or_order           od
                                               where oct.order_id = od.order_id
                                                 and oct.certificate_id =
                                                     nuparacta)
                                         AND r.localidad = zo.localidad
                                         AND r.tipo_rabajo = cx.tipo_trabajo) sca,
                                     open.ldc_item_uo_lr iu
                               WHERE sca.unidad_operativa =
                                     iu.unidad_operativa
  ...
  BEGIN
    ...
    FOR i IN cuordenesgenerarnovedadact(nuparacta) LOOP
      ...
      IF i.actividad_novedad_ofertados is not null THEN
        BEGIN
          dbms_output.put_line(' INSERT INTO ldc_uni_act_ot2
            (nussesion,
             unidad_operativa,
             actividad,
             item,
...
BEGIN
  IF TRUE THEN
    --fblaplicaentregaxcaso('0000554') THEN
    sbEntrega554 := 'S';
  else
    sbEntrega554 := 'N';
  End if;
  ...
  IF fblaplicaentrega(sbEntrega) THEN
    ...
    dbms_output.put_line(nuccano || ',' || nuccmes || ',' ||
                         'LDC_PROGENERANOVELTYCARTERA' || ',' ||
                         'En ejecucion..' || ',' || nupaacta,
                         sbuser);
    -- Obtenemos la fecha inicio y fin del acta
    BEGIN
      SELECT k.fecha_inicio, k.fecha_fin, k.id_contrato
        INTO dtfechainiacta, dtfechafinacta, nucontratoacta
        FROM open.ge_acta k
       WHERE k.id_acta = nupaacta;
    EXCEPTION
      WHEN no_data_found THEN
        sbmensaje := 'No existe un acta con el id : ' || nupaacta;
        dbms_output.put_line(nupaacta,
                             nvl(sbmensaje, 'Ok'),
                             'LDC_PROGENERANOVELTYCARTERA',
                             'Termino con errores.');
        RETURN;
    ...
    END IF;
    /*dtfechainiacta*/
    dtfechafinacta := to_date(to_char(dtfechafinacta, 'DD/MM/YYYY') ||
                              ' 00:00:00',
                              'DD/MM/YYYY HH24:MI:SS');
    -- Obtenemos el periodo contable
    BEGIN
      SELECT cc.cicoano, cc.cicomes
        INTO nuvaanoper, nuvamesper
        FROM open.ldc_ciercome cc
       WHERE /*dtfechainiacta*/
       dtfechafinacta BETWEEN cc.cicofein AND cc.cicofech;
    EXCEPTION
      WHEN no_data_found THEN
    ...
    ldcprollenaldcuniactot(nupaacta,
                           nuvaanoper,
                           nuvamesper,
                           dtfechafinacta,
                           nusession);
    ...
    FOR i IN cuordernesacta(nusession, nupaacta) LOOP
      ...
      fetch cuZonaOfertados
        into nuZonaOfertados;
      ...
      IF nuZonaOfertados <> -1 THEN
        ...
        BEGIN
          SELECT nvl(SUM(cantidad_asignada), 0)
            INTO nucantidad
            FROM (SELECT nvl(SUM(sa.cantidad_asignada), 0) cantidad_asignada
                   ...
                   WHERE sa.nuano = nuvaanoper
                     ...
                     AND sa.actividad in
                         (select i.actividad
                            from dual
                          union
                          select af.actividad_hija
                            from ldc_act_father_act_hija af
                           where af.actividad_padre = i.actividad)
        ...
        IF nvl(nuvalordescontar, 0) >= 1 THEN
          ...
          IF nunovedadgenera IS NOT NULL THEN
            ...
            IF swencontro = 0 THEN
              ...
              dbms_output.put_line('or_boorder.closeorderwithproduct(nunovedadgenera' || ',' ||
                                   i.unidad_operativa || ',' || 1,
                                   nupersona || ',' || 'NULL' || ',' ||
                                   SYSDATE || ',' || 1 || ',' ||
                                   nvl(nuvalordescontarxcant, 0) || ',' || 1400,
                                   'ACTA_OFERTADOS : ' ||
                                   to_char(nupaacta || ',' ||
                                           '000000000000000') ||
              ...
              IF nuorderid IS NOT NULL THEN
                ...
                BEGIN
                  SELECT xc.direccion_gen_nov, df.geograp_location_id
                    INTO nudireccgene, nulocalidaddirgen
                ...
                IF nudireccgene IS NULL THEN
                  sbmensaje := 'La unidad operativa : ' ||
                               to_char(i.unidad_operativa) ||
                               ' no tiene o no tiene direccion asociada.';
                  dbms_output.put_line(nupaacta,
                                       nvl(sbmensaje, 'Ok'),
                                       'LDC_PROGENERANOVELTYCARTERA',
                                       'Termino con errores.');
                ...
                UPDATE open.or_order_activity gh
                   SET gh.comment_   = 'Orden de novedad generada ACTIVIDAD : ' ||
                                       ...
                                       to_char(i.item) ||
                                       ' ZONA_OFERTADOS : ' ||
                                       to_char(i.zona_ofertados) ||
                                       ' CANTIDAD : ' || to_char(i.cantidad),
                       gh.address_id = nudireccgene
                 WHERE gh.order_id = nuorderid;
                INSERT INTO open.ct_order_certifica
    ...
    END IF;
    DELETE ldc_uni_act_ot2 lk
     WHERE \*lk.nussesion = nusession AND*\
     lk.nro_acta = nupaacta;
    ...
    UPDATE ldc_cant_asig_ofer_cart sa
     ...
     WHERE sa.nuano = nuvaanoper
       ...
       and sa.reg_activo = 'Y'
       AND sa.nro_acta = -1
       AND sa.unidad_operatva_cart in
           (select distinct od.operating_unit_id
    ...
    dbms_output.put_line(nupaacta || ',' || nvl(sbmensaje, 'Ok') || ',' ||
                         'LDC_PROGENERANOVELTYCARTERA' || ',' ||
                         'Termino Ok.');
  END IF;
EXCEPTION
