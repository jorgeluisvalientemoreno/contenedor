...
nupaso number:=0;
  ...
  inuOrden or_order.order_id%TYPE DEFAULT NULL;
  ...
  --/*
    SELECT LPTOU.OPERATING_UNIT_ID,
           ...
           decode(sbBLBSS_FACT_LJLB_0000158,
                  'S',
                  LDC_BOASIGAUTO.FNUGETCATE(ooa.subscription_id,
                                            ooa.product_id,
                                            ooa.task_type_id,
                                            ooa.address_id),
    ...
    SELECT NVL(p.product_type_id, 0) TIPO_PRODUCTO
     ...
     WHERE p.product_id = NUPRODUCT_ID
       AND p.product_type_id =
           dald_parameter.fnuGetNumeric_Value('COD_PRO_GEN', NULL);
  ...
  --00000158
  sbPost    ld_parameter.value_chain%type := nvl(dald_parameter.fsbGetValue_Chain('EJECUTA_PROCESO_POST',
                                                                                  null),
                                                 'N');
  ...
  PROCEDURE PRRESEINTEOR IS
    ...
    ***************************************************************************/
    ...
    CURSOR cuOrdenes IS
      SELECT lo.ORDER_ID orden, lo.package_id paquete
        FROM LDC_ORDER LO, or_order o
       WHERE LO.ASIGNADO = 'N'
         AND o.order_id = lo.order_id
         AND o.ORDER_STATUS_ID IN (0, 1)
         AND nvl(asignacion, 0) >
             dald_parameter.fnuGetNumeric_Value('CANTIDAD_INTENTOS_ASIGNACION')
         AND ROUND((SYSDATE - ORDEFEBL) * 1440, 0) >
             dald_parameter.fnuGetNumeric_Value('LDC_TIEMPO_RESTINTE');
    --TICKET 2001377 LJLB -- registro de ordenes
    TYPE regDatos IS RECORD(
      nuOrden     LDC_ORDER.ORDER_ID%TYPE,
  ...
  BEGIN
    ...
    LOOP
      FETCH cuOrdenes BULK COLLECT
        INTO vtbOrdenes LIMIT inLimit;
      FOR idx IN 1 .. vtbOrdenes.COUNT LOOP
        UPDATE LDC_ORDER
           SET asignacion = 0, ORDEFEBL = NULL
         WHERE order_id = vtbOrdenes(idx).nuOrden
           AND NVL(package_id, 0) = NVL(vtbOrdenes(idx).nuSolicitud, 0);
      END LOOP;
  ...
  BEGIN
    ...
    IF sbError IS NOT NULL and sbDato is null THEN
      UPDATE LDC_ORDER
         SET ORDEOBSE = sbError, ORDEFERE = SYSDATE
       WHERE ORDER_ID = nuOrden
         AND NVL(PACKAGE_ID, 0) = NVL(nuSolicitud, 0);
    ELSE
      UPDATE LDC_ORDER
         SET ORDEOBSE = sbError, ORDEFERE = NULL
       WHERE ORDER_ID = nuOrden
         AND NVL(PACKAGE_ID, 0) = NVL(nuSolicitud, 0);
    END IF;
  ...
  EXCEPTION
    WHEN OTHERS THEN
      ...
      UPDATE LDC_ORDER
         SET ORDEOBSE = sbErrorInt, ORDEFERE = SYSDATE
       WHERE ORDER_ID = nuOrden
  ...
  begin
    UPDATE or_order
       SET OPERATING_SECTOR_ID = inuSector
     WHERE order_id = inuOrden;
    --commit;
...
BEGIN
  ...
  --PROINIT;
  pkg_traza.trace('Paso 1', csbNivelTraza);
  ...
  pkg_traza.trace('Cantidad Intentos: ' || nuCantidadIntentos,
                  csbNivelTraza);

  pkg_traza.trace('Paso 2', csbNivelTraza);
  ...
  FOR TEMPCULDC_ORDER IN CULDC_ORDER(nuCantidadIntentos) LOOP
    ...
    BEGIN
      -- CASO 272: Se agrega bloque de control
      pkg_traza.trace('Paso 3', csbNivelTraza);
    
      NUSOLICITUD := TEMPCULDC_ORDER.PACKAGE_ID;
      NUORDEN     := TEMPCULDC_ORDER.ORDER_ID;
    
      nuestadoOt := DAOR_ORDER.FNUGETORDER_STATUS_ID(TEMPCULDC_ORDER.ORDER_ID,
                                                     NULL);
      ...
      ELSE
        ...
        FOR TEMPCUUOBYSOL IN CUUOBYSOL(TEMPCULDC_ORDER.ORDER_ID,
                                       TEMPCULDC_ORDER.PACKAGE_ID) LOOP
          ...
          BEGIN
            ...
            IF sbFlag = 'S' THEN
              ...
              FETCH cuSectorOperativo
                INTO nuSectorOperativo;
              IF cuSectorOperativo%NOTFOUND OR nuSectorOperativo IS NULL THEN
              
                sbError := sbError || v_salto || ' orden [' ||
                           TEMPCULDC_ORDER.ORDER_ID || '], solicitud [' ||
                           TEMPCULDC_ORDER.PACKAGE_ID || ']: el producto [' ||
                           TEMPCUUOBYSOL.PRODUCT_ID ||
                           ']  no tiene sector operativo definido';
            ...
            END IF;
      pkg_traza.trace('Paso 4', csbNivelTraza);
        
            IF NUCONTROLCICLOUOBYSOL = 0 THEN
            
              SBUOBYSOL := 'DATOS CURSOR CUUOBYSOL[ Unidad Opertaiva: ' ||
                           ...
                           ' - Actividad UOBYSOL: ' ||
                           TEMPCUUOBYSOL.ITEMS_ID || ' - Producto Orden: ' ||
                           TEMPCUUOBYSOL.PRODUCT_ID ||
                           ' - Direccion Orden: ' ||
                           TEMPCUUOBYSOL.ADDRESS_ID ||
                           ' - Actividad Orden: ' ||
                           TEMPCUUOBYSOL.ACTIVITY_ID ||
                           ' - Contrato Orden: ' ||
                           TEMPCUUOBYSOL.SUBSCRIPTION_ID ||
                           ' - Zona Unidad Operativa: ' ||
                           TEMPCUUOBYSOL.ZONA_UNIDAD_OPERATIVA ||
                           ' - Categoria Produto Orden: ' ||
            ...
            IF CULDC_CARUNIOPE%NOTFOUND THEN
              ...
              SBDATAIN := NVL(TEMPCULDC_ORDER.ORDER_ID, -1) || '|' ||
                          ...
                          NVL(TEMPCUUOBYSOL.SUBSCRIPTION_ID, -1) || '|' ||
                          'PRASIGNACION' || '|' ||
                          TEMPCUUOBYSOL.CATEGORY_ID;
              ...
              nuPackType := damo_packages.fnugetpackage_type_id(TEMPCULDC_ORDER.PACKAGE_ID,
                                                                NULL);
            pkg_traza.trace('Paso 5', csbNivelTraza);
              ...
              --SBUNIDADOPERATIVA := NULL;
              IF TEMPCUUOBYSOL.PROCESOPRE IS NOT NULL AND
                 SBUNIDADOPERATIVA IS NULL THEN
              ...
              IF SBUNIDADOPERATIVA IS NULL THEN
                ...
                END;
              pkg_traza.trace('Paso 6', csbNivelTraza);
                ...
                pkg_traza.trace('Paso 6.1', csbNivelTraza);
                  ...
                  IF CUTIPOPRODUCTO%FOUND THEN
                    ...
                    pkg_Traza.trace('RTCUSEGMENTOCATEGORIA.CATEGORIA: ' ||
                  ...
                  CLOSE CUTIPOPRODUCTO;
                  --FIN NC 2493
                pkg_traza.trace('Paso 6.2', csbNivelTraza);
    
                  --CASO 200-2067 comentariado
                  --IF CUCATEGORIA%FOUND OR TEMPCUUOBYSOL.CATECODI = -1 OR
                  IF TEMPCUUOBYSOL.CATEGORY_ID IS NOT NULL OR
                     TEMPCUUOBYSOL.CATECODI = -1 OR
                    --NC 2493
                     RTCUSEGMENTOCATEGORIA.CATEGORIA =
                     TEMPCUUOBYSOL.CATECODI THEN
                    --FIN NC 2493
              ...
              pkg_traza.trace('Paso 7', csbNivelTraza);
                      ...
                      BEGIN
                        ...
                        IF sbFlag = 'S' THEN
                          ...
                          SELECT COUNT(1)
                            INTO nuContrato
                            FROM ge_contrato C, ge_contratista g
                           WHERE C.ID_CONTRATISTA = G.ID_CONTRATISTA
                             AND C.status = ct_boconstants.fsbGetOpenStatus
                             AND C.ID_CONTRATISTA IN
                                 (SELECT contractor_id
                                    FROM or_operating_unit ou
                                   WHERE ou.operating_unit_id =
                                         TEMPCUUOBYSOL.OPERATING_UNIT_ID)
                             AND (SYSDATE BETWEEN C.FECHA_INICIAL AND
                                 C.FECHA_FINAL);
                        ELSE
                          nuContrato := 1;
                        END IF;
                        ...
                        IF nuContrato > 0 THEN
                          ...
                          pkg_traza.trace('Asignacion: '|| TEMPCULDC_ORDER.Order_Id||','|| TEMPCUUOBYSOL.OPERATING_UNIT_ID, csbNivelTraza, pkg_traza.csbINICIO);
                          ...
                          ELSE
                            ...
                            IF sbFlag = 'S' THEN
                              --TICKET 2001377 LJLB -- se registra log en la tabla LDC_ORDER
                              sbError := substr(sbError || v_salto ||
                                                'ORDEN [' ||
                                                TEMPCULDC_ORDER.Order_Id ||
                                                '] UNIDAD OPERATIVA [' ||
                                                TEMPCUUOBYSOL.OPERATING_UNIT_ID ||
                    ...
                    ELSE
                      ...
                      IF sbFlag = 'S' THEN
                        ...
                        sbError := substr(sbError || v_salto || 'ORDEN [' ||
                                          TEMPCULDC_ORDER.Order_Id ||
                                          '] UNIDAD OPERATIVA [' ||
                                          TEMPCUUOBYSOL.OPERATING_UNIT_ID ||
                                          '] LA CATEGORIA [' ||
                                          TEMPCUUOBYSOL.CATECODI ||
                                          '] CONFIGURADA (UOBYSOL) NO ES IGUAL A LA CATERORIA [' ||
                                          TEMPCUUOBYSOL.CATEGORY_ID ||
                                          '] DEL CONTRATO ' ||
                                          TEMPCUUOBYSOL.SUBSCRIPTION_ID,
                                          1,
                  ...
                  ELSE
                    ...
                    IF sbFlag = 'S' THEN
                      ...
                      ACTUALIZA_MENSAJE(TEMPCULDC_ORDER.ORDER_ID,
                                        TEMPCULDC_ORDER.PACKAGE_ID,
                                        sbError);
                ...
                ELSE
                  ...
                  IF sbFlag = 'S' THEN
                    --TICKET 2001377 LJLB -- se registra log en la tabla LDC_ORDER
                    sbError := substr(sbError || v_salto || 'LA ORDEN [' ||
                                      TEMPCULDC_ORDER.ORDER_ID ||
                                      '] Y LA UNIDAD OPERATIVA [' ||
                                      TEMPCUUOBYSOL.OPERATING_UNIT_ID ||
                                      '] NO ESTAN EN LA MISMA ZONA OPERATIVA. SECTOR ORDEN[' ||
                                      nuSectorOperativo ||
                                      ']  ZONA OPERATIVA  DE LA UNIDAD OPERATIVA [' ||
                                      NUOPERATING_ZONE_ID || ']',
                                      1,
                                      3999);
