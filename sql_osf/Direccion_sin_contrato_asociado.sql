select pr_product.product_id "Producto",
       ...
       ab_premise.category_ || ' - ' || c.catedesc Categoria,
       ab_premise.subcategory_ || ' - ' || sc.sucadesc Subcategoria,
  ...
  left join open.categori c
    on c.catecodi = ab_premise.category_
  left join open.subcateg sc
    on sc.sucacate = ab_premise.category_
   and sc.sucacodi = ab_premise.subcategory_
 ...
 (select null
   ...
   and not exists (select null
         ...
         where s.susciddi = ab_address.address_id)
   ...
   and ab_premise.category_ = 1
      --and ab_premise.subcategory_ = 4
      --and ge_geogra_location.geograp_location_id = 55
   and ge_geogra_location.geo_loca_father_id = 8978
...
select a.address_id,
       ...
       ap.premise_type_id || '- ' || ap.category_ "Categoria",
