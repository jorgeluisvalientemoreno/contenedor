DECLARE
  nupaacta open.ge_acta.id_acta%TYPE := 257331;
  ...
  BEGIN
    DELETE ldc_uni_act_ot2 lk
     WHERE /*lk.nussesion = nuparsesion AND*/
     lk.nro_acta = nuparacta;
...
BEGIN
  ...
  IF fblaplicaentrega(sbEntrega) THEN
    ...
    ldc_proinsertaestaprog(nuccano,
                           nuccmes,
    ...
    EXCEPTION
      WHEN no_data_found THEN
        ...
        ldc_proactualizaestaprog(nupaacta,
                                 nvl(sbmensaje, 'Ok'),
                                 'LDC_PROGENERANOVELTYCARTERA',
                                 'Termino con errores.');
        RETURN;
