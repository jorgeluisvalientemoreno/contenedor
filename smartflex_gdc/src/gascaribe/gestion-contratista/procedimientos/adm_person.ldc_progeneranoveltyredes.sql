create or replace PROCEDURE ADM_PERSON.LDC_PROGENERANOVELTYREDES(nupaacta ge_acta.id_acta%TYPE) IS
 /****************************************************************************************
    Propiedad intelectual de JM GESTIONINFORMATICA S.A.

    Unidad         : ldc_progeneranoveltyredes
    ...
    ==========        ==================            ======================================
	01/04/2019		   dsaltarin			                  -ca200-2532 se modifica para recuperar los cambios de la entrega OSS_CON_ELAL_200_1858_4
 ...
 CURSOR cuordenesgenerarnovedadact(nucurtacta ge_acta.id_acta%TYPE) IS
  SELECT ot.operating_unit_id             unidad_operativa
   ...
   WHERE oc.certificate_id     = nucurtacta
     AND oi.legal_item_amount >  0 --= 1 --200-2532
     AND iu.liquidacion        = 'A'
	 AND xu.tipo_ofertado      = 3	 --200-2532
     AND ot.order_id           = oc.order_id
     AND ot.order_id           = oa.order_id
     AND ot.order_id           = fo.orden_nieta
...
BEGIN
 ...
 --REQ. 264. --Control de excepciones en loop 1
 EXCEPTION
   WHEN OTHERS THEN
   NULL;
   errors.geterror(nuerrorcode, sberrormessage);
...
BEGIN
   ...
   -- Registramos inicio del proceso
     ldc_proinsertaestaprog(nuccano,nuccmes,'LDC_PROGENERANOVELTYREDES','En ejecucion..',nupaacta,sbuser);
   EXCEPTION
     WHEN OTHERS THEN
     NULL;
     errors.geterror(nuerrorcode, sberrormessage);
         ...
         FOR j IN cuordernesactaot(i.unidad_operativa,i.actividad,i.item,nusession,nupaacta,i.orden) LOOP
            ...
            IF nvl(nuvalordescontar,0) >= 1 THEN
               ...
               EXCEPTION
                  ...
                  ldc_proactualizaestaprog(nupaacta,nvl(sbmensaje,'Ok'),'LDC_PROGENERANOVELTYREDES','Termino con errores.');
                  --
                  errors.geterror(nuerrorcode, sberrormessage);
                  sbcontrolerror := nuerrorcode || ' - ' || sberrormessage;
                  LDC_PROREG_CT_PROCESS_LOG(inuprocess_log_id  => SEQ_CT_PROCESS_LOG_109639.NEXTVAL
 ...
 EXCEPTION
    ...
    LDC_PROREG_CT_PROCESS_LOG(inuprocess_log_id  => SEQ_CT_PROCESS_LOG_109639.NEXTVAL
                              ...
                              ,inuerror_message  => 'Error insertando en ldc_actas_aplica_proc_ofert '
                                                    ||'Acta= '||nupaacta
                                                    ||'Procejec= '||'LDC_PROGENERANOVELTYREDES'
 ...
 ldc_proactualizaestaprog(nupaacta,nvl(sbmensaje,'Ok'),'LDC_PROGENERANOVELTYREDES','Termino Ok.');
 --
...
EXCEPTION
 WHEN OTHERS THEN
  ...
  ut_trace.trace('LDC_PROGENERANOVELTYREDES'||' '||SQLERRM, 11);
  --REQ.264.
  NULL;
  errors.geterror(nuerrorcode, sberrormessage);
  sbcontrolerror := nuerrorcode || ' - ' || sberrormessage;
  LDC_PROREG_CT_PROCESS_LOG(inuprocess_log_id  => SEQ_CT_PROCESS_LOG_109639.NEXTVAL
                            ...
                            ,inucontract_id    => nupaacta--Se envia el acta
                            --
                            ,inuerror_message  => 'LDC_PROGENERANOVELTYREDES - When others general - '||substr(SQLERRM || ' - ' || sbcontrolerror,1,1900)
                            );
END;
/
BEGIN
    pkg_utilidades.prAplicarPermisos('LDC_PROGENERANOVELTYREDES', 'ADM_PERSON');
END;
/
GRANT EXECUTE ON ADM_PERSON.LDC_PROGENERANOVELTYREDES TO REXEREPORTES;
/