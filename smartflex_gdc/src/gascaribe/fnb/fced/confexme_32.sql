...
BEGIN
--{
	UT_Trace.Trace( '********************** Creando paquete CONFEXME_32 ***********************', 5 );
	SA_BOCreatePackages.CreatePackage
	(
		'CONFEXME_32',
		'CREATE OR REPLACE PACKAGE CONFEXME_32 IS ' || chr(10) || chr(10) ||
...
--{
	...
	(
		'CONFEXME_32',
		'CREATE OR REPLACE PACKAGE BODY CONFEXME_32 IS ' || chr(10) || chr(10) ||
		chr(9) || '-- Procedimiento para ejecutar una sentencia en una transacción autónoma' || chr(10) ||
		chr(9) || 'PROCEDURE ExecuteSQLSentence' || chr(10) ||
		chr(9) || '(' || chr(10) ||
		chr(9) || chr(9) || 'isbSQLSentence    in   varchar2' || chr(10) ||
...
--{
	...
	if ( not CONFEXME_32.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	...
	open CONFEXME_32.cuFormat( 83 );
	fetch CONFEXME_32.cuFormat into nuFormatId;
	close CONFEXME_32.cuFormat;
...
EXCEPTION
	...
	when OTHERS then
		CONFEXME_32.boProcessStatusOK := FALSE;
		ROLLBACK;
...
--{
	...
	--{
		...
		CONFEXME_32.rcED_Formato.formcodi := nuFormatId;

		-- Se actualiza la información del formato
		UPDATE ED_Formato
		SET    formdesc = 'LDC_CUPON_VENTA_BRILLA',
		       formtido = 66,
		       formiden = '<83>',
		       formtico = 49,
		       formdein = '<LDC_CUPON_VENTA_BRILLA>',
		       formdefi = '</LDC_CUPON_VENTA_BRILLA>'
		WHERE  formcodi = CONFEXME_32.rcED_Formato.formcodi;
	--}
	else
	--{
		-- Se genera un identificador para el formato
		CONFEXME_32.rcED_Formato.formcodi := 83 ;
		...
		VALUES
		(
			CONFEXME_32.rcED_Formato.formcodi,
			'LDC_CUPON_VENTA_BRILLA',
			66,
			'<83>',
			49,
			'<LDC_CUPON_VENTA_BRILLA>',
...
--{
	...
	CONFEXME_32.tbrcGE_Statement( '120187720' ).statement_id := nuNextSeqValue;

	UT_Trace.Trace( 'Insertando Sentencia [LDC_VENTA_BRILLA_BARCODE]', 5 );
	...
	VALUES
	(
		CONFEXME_32.tbrcGE_Statement( '120187720' ).statement_id,
		1,
		'LDC_VENTA_BRILLA_BARCODE',
...
--{
	...
	-- Se genera un identificador para la sentencia
	nuNextSeqValue := GE_BOSequence.NextGe_Statement;
	CONFEXME_32.tbrcGE_Statement( '120187721' ).statement_id := nuNextSeqValue;

	UT_Trace.Trace( 'Insertando Sentencia [LDC_VENTA_BRILLA]', 5 );
	INSERT INTO GE_Statement
	(
		statement_id,
		module_id,
	...
	VALUES
	(
		CONFEXME_32.tbrcGE_Statement( '120187721' ).statement_id,
		1,
		'LDC_VENTA_BRILLA',
		'SELECT CLIENTE, IDENTIFICACION, PROYECTO, CONTRATO, DIRPROYECTO, SOLICITUD, TIPOCUPON, CUPON, VALOR, VALIDOHASTA, TIPO_SOL, CODIGODEBARRAS
FROM (select decode(B.subscriber_id, null, null,dage_subscriber.fsbgetsubscriber_name(B.subscriber_id))||'' ''||
       decode(B.subscriber_id, null, null,dage_subscriber.fsbgetsubs_last_name(B.subscriber_id))          CLIENTE ,
 decode(B.subscriber_id, null, null,dage_subscriber.fsbgetidentification(B.subscriber_id))          IDENTIFICACION,
 '''' PROYECTO,
 S.SUSCCODI CONTRATO,
 A.address  DIRPROYECTO,
 PA.package_id SOLICITUD,
 C.cupotipo   TIPOCUPON,
 decode(C.cuponume,null, ''No se ha generado Cupon'', C.cuponume) CUPON,
  decode(C.cupovalo,null, ''No se ha generado Cupon'',
  ''$''||to_char(C.cupovalo,''FM999,999,999'')) VALOR,
 CASE WHEN LDC_FBLAPLICAENTREGA(''CRM_HT_0000463_1'') = ''NO''  THEN
          to_char(PA.expect_atten_date,''DD-MON-YYYY'')       
       ELSE  CASE WHEN 
 INSTR('',''||dald_parameter.fsbgetvalue_chain(''LDC_TIPOSOPR'', NULL)||'','','',''||pa.package_type_id||'','' ) > 0 THEN
      to_char(TO_DATE(dald_parameter.fsbgetvalue_chain(''LDC_FECHVICU'', NULL), ''DD/MM/YYYY''),''DD-MON-YYYY'')  
 	       ELSE     to_char(PA.expect_atten_date,''DD-MON-YYYY'')  
 		END  END VALIDOHASTA ,
  ''CUPON DE PAGO DE VENTA DE BRILLA CON ANTICIPO''                 TIPO_SOL,
...
from suscripc S, ge_subscriber b
     ,ab_address a
     , cupon C, mo_packages PA
where  (PA.package_id = cc_bcCoupon.fnuGetPackageByCoupon(C.cuponume, C.cupodocu) or PA.package_id=C.cupodocu)
    and s.SUSCIDDI = a.address_id
    and s.suscclie = b.subscriber_id
    and s.susccodi = C.cuposusc
    and C.cuponume = to_number(obtenervalorinstancia(''CUPON'',''CUPONUME''))
    and C.cupotipo IN (''CC'',''PP'')
UNION
select decode(B.subscriber_id, null, null,dage_subscriber.fsbgetsubscriber_name(B.subscriber_id))||'' ''||
   decode(B.subscriber_id, null, null,dage_subscriber.fsbgetsubs_last_name(B.subscriber_id))          CLIENTE ,
   decode(B.subscriber_id, null, null,dage_subscriber.fsbgetidentification(B.subscriber_id))          IDENTIFICACION,
   PA.document_key  PROYECTO,
   S.SUSCCODI CONTRATO,
   A.address DIRPROYECTO,
   PA.package_id SOLICITUD,
   C.cupotipo  TIPOCUPON,
   decode(C.cuponume,null, ''No se ha generado Cupon'', C.cuponume)   CUPON,
    ...
    CASE WHEN LDC_FBLAPLICAENTREGA(''CRM_HT_0000463_1'') = ''NO''  THEN
	         to_char(PA.expect_atten_date,''DD-MON-YYYY'')       
         ELSE	CASE  WHEN INSTR('',''||dald_parameter.fsbgetvalue_chain(''LDC_TIPOSOPR'', NULL)||'','','',''||pa.package_type_id||'','' ) > 0 
		   THEN	 to_char(TO_DATE(dald_parameter.fsbgetvalue_chain(''LDC_FECHVICU'', NULL), ''DD/MM/YYYY''),''DD-MON-YYYY'')  
		 ELSE to_char(PA.expect_atten_date,''DD-MON-YYYY'')  
		eND  END VALIDOHASTA ,
	UPPER(''CUPON DE PAGO DE ''||
    daps_package_type.fsbgetdescription(pa.package_type_id))        TIPO_SOL,
    ''(415)''||dald_parameter.fsbGetValue_Chain(''COD_EAN_CODIGO_BARRAS'')
    ||''(8020)''||LPAD(to_char(CUPONUME),10,''0'')
    ||''(3900)''||LPAD(to_char(CUPOVALO),10,''0'')
    ||''(96)''||to_char(CUPOFECH,''yyyymmdd'') CODIGODEBARRAS
...
--{
	...
	CONFEXME_32.tbrcED_Franja( 5498 ).francodi := nuNextSeqValue;
	...
	VALUES
	(
		CONFEXME_32.tbrcED_Franja( 5498 ).francodi,
		'LDC_FRANJADEPOSITO',
		CONFEXME_32.tbrcED_Franja( 5498 ).frantifr,
		NULL,
		NULL
	);
...
--{
	...
	CONFEXME_32.tbrcED_FranForm( '5311' ).frfocodi := nuNextSeqValue;

	-- Se asigna el formato
	CONFEXME_32.tbrcED_FranForm( '5311' ).frfoform := CONFEXME_32.rcED_Formato.formcodi;

	-- Se asigna la franja
	if ( CONFEXME_32.tbrcED_Franja.exists( 5498 ) ) then
		CONFEXME_32.tbrcED_FranForm( '5311' ).frfofran := CONFEXME_32.tbrcED_Franja( 5498 ).francodi;
	end if;
	...
	VALUES
	(
		CONFEXME_32.tbrcED_FranForm( '5311' ).frfocodi,
		CONFEXME_32.tbrcED_FranForm( '5311' ).frfoform,
		CONFEXME_32.tbrcED_FranForm( '5311' ).frfofran,
		1,
		'N'
	);
...
--{
	...
	CONFEXME_32.tbrcED_FuenDato( '4004' ).fudacodi := nuNextSeqValue;

	-- Se asigna la sentencia asociada a la fuente de datos
	if ( CONFEXME_32.tbrcGE_Statement.exists( '120187720' ) ) then
		CONFEXME_32.tbrcED_FuenDato( '4004' ).fudasent := CONFEXME_32.tbrcGE_Statement( '120187720' ).statement_id;
	end if;

	UT_Trace.Trace( 'Insertando Fuente de Datos [LDC_VENTA_BRILLA_BARCODE]', 5 );
	...
	VALUES
	(
		CONFEXME_32.tbrcED_FuenDato( '4004' ).fudacodi,
		'LDC_VENTA_BRILLA_BARCODE',
		NULL,
		CONFEXME_32.tbrcED_FuenDato( '4004' ).fudasent
	);
...
--{
	...
	-- Se genera un identificador para la fuente de datos
	nuNextSeqValue := pkBOInsertMgr.NextED_FUENDATO;
	CONFEXME_32.tbrcED_FuenDato( '4005' ).fudacodi := nuNextSeqValue;

	-- Se asigna la sentencia asociada a la fuente de datos
	if ( CONFEXME_32.tbrcGE_Statement.exists( '120187721' ) ) then
		CONFEXME_32.tbrcED_FuenDato( '4005' ).fudasent := CONFEXME_32.tbrcGE_Statement( '120187721' ).statement_id;
	end if;

	UT_Trace.Trace( 'Insertando Fuente de Datos [LDC_VENTA_BRILLA]', 5 );
	INSERT INTO ED_FuenDato
	(
		fudacodi,
		fudadesc,
	...
	VALUES
	(
		CONFEXME_32.tbrcED_FuenDato( '4005' ).fudacodi,
		'LDC_VENTA_BRILLA',
		NULL,
		CONFEXME_32.tbrcED_FuenDato( '4005' ).fudasent
	);
...
--{
	...
	CONFEXME_32.tbrcED_AtriFuda( '32975' ).atfdcodi := nuNextSeqValue;

	-- Se asigna la fuente de datos
	if ( CONFEXME_32.tbrcED_FuenDato.exists( '4004' ) ) then
		CONFEXME_32.tbrcED_AtriFuda( '32975' ).atfdfuda := CONFEXME_32.tbrcED_FuenDato( '4004' ).fudacodi;
	end if;
	...
	VALUES
	(
		CONFEXME_32.tbrcED_AtriFuda( '32975' ).atfdcodi,
		'CODE',
		'CODE',
		1,
		'S',
		CONFEXME_32.tbrcED_AtriFuda( '32975' ).atfdfuda,
		1
	);
...
--{
	...
	-- Se genera un identificador para el atributo
	nuNextSeqValue := pkBOInsertMgr.NextED_ATRIFUDA;
	CONFEXME_32.tbrcED_AtriFuda( '32976' ).atfdcodi := nuNextSeqValue;

	-- Se asigna la fuente de datos
	if ( CONFEXME_32.tbrcED_FuenDato.exists( '4004' ) ) then
		CONFEXME_32.tbrcED_AtriFuda( '32976' ).atfdfuda := CONFEXME_32.tbrcED_FuenDato( '4004' ).fudacodi;
	end if;
	...
	VALUES
	(
		CONFEXME_32.tbrcED_AtriFuda( '32976' ).atfdcodi,
		'IMAGE',
		'IMAGE',
		2,
		'S',
		CONFEXME_32.tbrcED_AtriFuda( '32976' ).atfdfuda,
		1
	);
...
--{
	...
	-- Se genera un identificador para el atributo
	nuNextSeqValue := pkBOInsertMgr.NextED_ATRIFUDA;
	CONFEXME_32.tbrcED_AtriFuda( '32989' ).atfdcodi := nuNextSeqValue;

	-- Se asigna la fuente de datos
	if ( CONFEXME_32.tbrcED_FuenDato.exists( '4005' ) ) then
		CONFEXME_32.tbrcED_AtriFuda( '32989' ).atfdfuda := CONFEXME_32.tbrcED_FuenDato( '4005' ).fudacodi;
	end if;
	...
	VALUES
	(
		CONFEXME_32.tbrcED_AtriFuda( '32989' ).atfdcodi,
		'CLIENTE',
		'CLIENTE',
		1,
		'S',
		CONFEXME_32.tbrcED_AtriFuda( '32989' ).atfdfuda,
		1
	);
...
--{
	...
	-- Se genera un identificador para el atributo
	nuNextSeqValue := pkBOInsertMgr.NextED_ATRIFUDA;
	CONFEXME_32.tbrcED_AtriFuda( '32990' ).atfdcodi := nuNextSeqValue;

	-- Se asigna la fuente de datos
	if ( CONFEXME_32.tbrcED_FuenDato.exists( '4005' ) ) then
		CONFEXME_32.tbrcED_AtriFuda( '32990' ).atfdfuda := CONFEXME_32.tbrcED_FuenDato( '4005' ).fudacodi;
	end if;
	...
	VALUES
	(
		CONFEXME_32.tbrcED_AtriFuda( '32990' ).atfdcodi,
		'IDENTIFICACION',
		'IDENTIFICACION',
		2,
		'S',
		CONFEXME_32.tbrcED_AtriFuda( '32990' ).atfdfuda,
		1
	);
...
--{
	...
	-- Se genera un identificador para el atributo
	nuNextSeqValue := pkBOInsertMgr.NextED_ATRIFUDA;
	CONFEXME_32.tbrcED_AtriFuda( '32991' ).atfdcodi := nuNextSeqValue;

	-- Se asigna la fuente de datos
	if ( CONFEXME_32.tbrcED_FuenDato.exists( '4005' ) ) then
		CONFEXME_32.tbrcED_AtriFuda( '32991' ).atfdfuda := CONFEXME_32.tbrcED_FuenDato( '4005' ).fudacodi;
	end if;
	...
	VALUES
	(
		CONFEXME_32.tbrcED_AtriFuda( '32991' ).atfdcodi,
		'PROYECTO',
		'PROYECTO',
		3,
		'S',
		CONFEXME_32.tbrcED_AtriFuda( '32991' ).atfdfuda,
		1
	);
...
--{
	...
	-- Se genera un identificador para el atributo
	nuNextSeqValue := pkBOInsertMgr.NextED_ATRIFUDA;
	CONFEXME_32.tbrcED_AtriFuda( '32992' ).atfdcodi := nuNextSeqValue;

	-- Se asigna la fuente de datos
	if ( CONFEXME_32.tbrcED_FuenDato.exists( '4005' ) ) then
		CONFEXME_32.tbrcED_AtriFuda( '32992' ).atfdfuda := CONFEXME_32.tbrcED_FuenDato( '4005' ).fudacodi;
	end if;
	...
	VALUES
	(
		CONFEXME_32.tbrcED_AtriFuda( '32992' ).atfdcodi,
		'CONTRATO',
		'CONTRATO',
		4,
		'S',
		CONFEXME_32.tbrcED_AtriFuda( '32992' ).atfdfuda,
		2
	);
...
--{
	...
	-- Se genera un identificador para el atributo
	nuNextSeqValue := pkBOInsertMgr.NextED_ATRIFUDA;
	CONFEXME_32.tbrcED_AtriFuda( '32993' ).atfdcodi := nuNextSeqValue;

	-- Se asigna la fuente de datos
	if ( CONFEXME_32.tbrcED_FuenDato.exists( '4005' ) ) then
		CONFEXME_32.tbrcED_AtriFuda( '32993' ).atfdfuda := CONFEXME_32.tbrcED_FuenDato( '4005' ).fudacodi;
	end if;
	...
	VALUES
	(
		CONFEXME_32.tbrcED_AtriFuda( '32993' ).atfdcodi,
		'DIRPROYECTO',
		'DIRPROYECTO',
		5,
		'S',
		CONFEXME_32.tbrcED_AtriFuda( '32993' ).atfdfuda,
		1
	);
...
--{
	...
	-- Se genera un identificador para el atributo
	nuNextSeqValue := pkBOInsertMgr.NextED_ATRIFUDA;
	CONFEXME_32.tbrcED_AtriFuda( '32994' ).atfdcodi := nuNextSeqValue;

	-- Se asigna la fuente de datos
	if ( CONFEXME_32.tbrcED_FuenDato.exists( '4005' ) ) then
		CONFEXME_32.tbrcED_AtriFuda( '32994' ).atfdfuda := CONFEXME_32.tbrcED_FuenDato( '4005' ).fudacodi;
	end if;
	...
	VALUES
	(
		CONFEXME_32.tbrcED_AtriFuda( '32994' ).atfdcodi,
		'SOLICITUD',
		'SOLICITUD',
		6,
		'S',
		CONFEXME_32.tbrcED_AtriFuda( '32994' ).atfdfuda,
		2
	);
...
--{
	...
	-- Se genera un identificador para el atributo
	nuNextSeqValue := pkBOInsertMgr.NextED_ATRIFUDA;
	CONFEXME_32.tbrcED_AtriFuda( '32995' ).atfdcodi := nuNextSeqValue;

	-- Se asigna la fuente de datos
	if ( CONFEXME_32.tbrcED_FuenDato.exists( '4005' ) ) then
		CONFEXME_32.tbrcED_AtriFuda( '32995' ).atfdfuda := CONFEXME_32.tbrcED_FuenDato( '4005' ).fudacodi;
	end if;
	...
	VALUES
	(
		CONFEXME_32.tbrcED_AtriFuda( '32995' ).atfdcodi,
		'TIPOCUPON',
		'TIPOCUPON',
		7,
		'S',
		CONFEXME_32.tbrcED_AtriFuda( '32995' ).atfdfuda,
		1
	);
...
--{
	...
	-- Se genera un identificador para el atributo
	nuNextSeqValue := pkBOInsertMgr.NextED_ATRIFUDA;
	CONFEXME_32.tbrcED_AtriFuda( '32996' ).atfdcodi := nuNextSeqValue;

	-- Se asigna la fuente de datos
	if ( CONFEXME_32.tbrcED_FuenDato.exists( '4005' ) ) then
		CONFEXME_32.tbrcED_AtriFuda( '32996' ).atfdfuda := CONFEXME_32.tbrcED_FuenDato( '4005' ).fudacodi;
	end if;
	...
	VALUES
	(
		CONFEXME_32.tbrcED_AtriFuda( '32996' ).atfdcodi,
		'CUPON',
		'CUPON',
		8,
		'S',
		CONFEXME_32.tbrcED_AtriFuda( '32996' ).atfdfuda,
		1
	);
...
--{
	...
	-- Se genera un identificador para el atributo
	nuNextSeqValue := pkBOInsertMgr.NextED_ATRIFUDA;
	CONFEXME_32.tbrcED_AtriFuda( '32997' ).atfdcodi := nuNextSeqValue;

	-- Se asigna la fuente de datos
	if ( CONFEXME_32.tbrcED_FuenDato.exists( '4005' ) ) then
		CONFEXME_32.tbrcED_AtriFuda( '32997' ).atfdfuda := CONFEXME_32.tbrcED_FuenDato( '4005' ).fudacodi;
	end if;
	...
	VALUES
	(
		CONFEXME_32.tbrcED_AtriFuda( '32997' ).atfdcodi,
		'VALOR',
		'VALOR',
		9,
		'S',
		CONFEXME_32.tbrcED_AtriFuda( '32997' ).atfdfuda,
		1
	);
...
--{
	...
	-- Se genera un identificador para el atributo
	nuNextSeqValue := pkBOInsertMgr.NextED_ATRIFUDA;
	CONFEXME_32.tbrcED_AtriFuda( '32998' ).atfdcodi := nuNextSeqValue;

	-- Se asigna la fuente de datos
	if ( CONFEXME_32.tbrcED_FuenDato.exists( '4005' ) ) then
		CONFEXME_32.tbrcED_AtriFuda( '32998' ).atfdfuda := CONFEXME_32.tbrcED_FuenDato( '4005' ).fudacodi;
	end if;
	...
	VALUES
	(
		CONFEXME_32.tbrcED_AtriFuda( '32998' ).atfdcodi,
		'VALIDOHASTA',
		'VALIDOHASTA',
		10,
		'S',
		CONFEXME_32.tbrcED_AtriFuda( '32998' ).atfdfuda,
		1
	);
...
--{
	...
	-- Se genera un identificador para el atributo
	nuNextSeqValue := pkBOInsertMgr.NextED_ATRIFUDA;
	CONFEXME_32.tbrcED_AtriFuda( '32999' ).atfdcodi := nuNextSeqValue;

	-- Se asigna la fuente de datos
	if ( CONFEXME_32.tbrcED_FuenDato.exists( '4005' ) ) then
		CONFEXME_32.tbrcED_AtriFuda( '32999' ).atfdfuda := CONFEXME_32.tbrcED_FuenDato( '4005' ).fudacodi;
	end if;
	...
	VALUES
	(
		CONFEXME_32.tbrcED_AtriFuda( '32999' ).atfdcodi,
		'TIPO_SOL',
		'TIPO_SOL',
		11,
		'S',
		CONFEXME_32.tbrcED_AtriFuda( '32999' ).atfdfuda,
		1
	);
...
--{
	...
	-- Se genera un identificador para el atributo
	nuNextSeqValue := pkBOInsertMgr.NextED_ATRIFUDA;
	CONFEXME_32.tbrcED_AtriFuda( '33000' ).atfdcodi := nuNextSeqValue;

	-- Se asigna la fuente de datos
	if ( CONFEXME_32.tbrcED_FuenDato.exists( '4005' ) ) then
		CONFEXME_32.tbrcED_AtriFuda( '33000' ).atfdfuda := CONFEXME_32.tbrcED_FuenDato( '4005' ).fudacodi;
	end if;
	...
	VALUES
	(
		CONFEXME_32.tbrcED_AtriFuda( '33000' ).atfdcodi,
		'CODIGODEBARRAS',
		'CODIGODEBARRAS',
		12,
		'S',
		CONFEXME_32.tbrcED_AtriFuda( '33000' ).atfdfuda,
		1
	);
...
--{
	...
	CONFEXME_32.tbrcED_Bloque( 7544 ).bloqcodi := nuNextSeqValue;

	-- Se asigna la fuente de datos del bloque
	if ( CONFEXME_32.tbrcED_FuenDato.exists( '4004' ) ) then
		CONFEXME_32.tbrcED_Bloque( 7544 ).bloqfuda := CONFEXME_32.tbrcED_FuenDato( '4004' ).fudacodi;
	end if;
	...
	VALUES
	(
		CONFEXME_32.tbrcED_Bloque( 7544 ).bloqcodi,
		'LDC_EXTRA_BARCODE_',
		CONFEXME_32.tbrcED_Bloque( 7544 ).bloqtibl,
		CONFEXME_32.tbrcED_Bloque( 7544 ).bloqfuda,
		'<EXTRA_BARCODE_>',
		'</EXTRA_BARCODE_>'
	);
...
--{
	...
	-- Se genera un identificador para el bloque
	nuNextSeqValue := pkBOInsertMgr.NextED_BLOQUE;
	CONFEXME_32.tbrcED_Bloque( 7545 ).bloqcodi := nuNextSeqValue;

	-- Se asigna la fuente de datos del bloque
	if ( CONFEXME_32.tbrcED_FuenDato.exists( '4005' ) ) then
		CONFEXME_32.tbrcED_Bloque( 7545 ).bloqfuda := CONFEXME_32.tbrcED_FuenDato( '4005' ).fudacodi;
	end if;
	...
	VALUES
	(
		CONFEXME_32.tbrcED_Bloque( 7545 ).bloqcodi,
		'LDC_BLOQUEDEPOSITO',
		CONFEXME_32.tbrcED_Bloque( 7545 ).bloqtibl,
		CONFEXME_32.tbrcED_Bloque( 7545 ).bloqfuda,
...
--{
	...
	CONFEXME_32.tbrcED_BloqFran( '7694' ).blfrcodi := nuNextSeqValue;

	-- Se asigna la franja-formato
	CONFEXME_32.tbrcED_BloqFran( '7694' ).blfrfrfo := CONFEXME_32.tbrcED_FranForm( '5311' ).frfocodi;

	-- Se asigna el bloque
	if ( CONFEXME_32.tbrcED_Bloque.exists( 7544 ) ) then
		CONFEXME_32.tbrcED_BloqFran( '7694' ).blfrbloq := CONFEXME_32.tbrcED_Bloque( 7544 ).bloqcodi;
	end if;
	...
	VALUES
	(
		CONFEXME_32.tbrcED_BloqFran( '7694' ).blfrcodi,
		CONFEXME_32.tbrcED_BloqFran( '7694' ).blfrbloq,
		CONFEXME_32.tbrcED_BloqFran( '7694' ).blfrfrfo,
		1,
		'S',
		'I'
	);
...
--{
	...
	-- Se genera un identificador para el bloque-franja
	nuNextSeqValue := pkBOInsertMgr.NextED_BLOQFRAN;
	CONFEXME_32.tbrcED_BloqFran( '7695' ).blfrcodi := nuNextSeqValue;

	-- Se asigna la franja-formato
	CONFEXME_32.tbrcED_BloqFran( '7695' ).blfrfrfo := CONFEXME_32.tbrcED_FranForm( '5311' ).frfocodi;

	-- Se asigna el bloque
	if ( CONFEXME_32.tbrcED_Bloque.exists( 7545 ) ) then
		CONFEXME_32.tbrcED_BloqFran( '7695' ).blfrbloq := CONFEXME_32.tbrcED_Bloque( 7545 ).bloqcodi;
	end if;
	...
	VALUES
	(
		CONFEXME_32.tbrcED_BloqFran( '7695' ).blfrcodi,
		CONFEXME_32.tbrcED_BloqFran( '7695' ).blfrbloq,
		CONFEXME_32.tbrcED_BloqFran( '7695' ).blfrfrfo,
		0,
		'S',
		'I'
	);
...
--{
	...
	CONFEXME_32.tbrcED_Item( '39261' ).itemcodi := nuNextSeqValue;

	-- Se establece el nombre del objeto por defecto
	CONFEXME_32.tbrcED_Item( '39261' ).itemobna := NULL;

	-- Se asigna el grupo de entidad asociado al item
	if ( CONFEXME_32.tbrcED_AtriFuda.exists( '32975' ) ) then
		CONFEXME_32.tbrcED_Item( '39261' ).itematfd := CONFEXME_32.tbrcED_AtriFuda( '32975' ).atfdcodi;
	end if;
	...
	VALUES
	(
		CONFEXME_32.tbrcED_Item( '39261' ).itemcodi,
		'CODE',
		CONFEXME_32.tbrcED_Item( '39261' ).itemceid,
		NULL,
		CONFEXME_32.tbrcED_Item( '39261' ).itemobna,
		0,
		NULL,
		NULL,
		CONFEXME_32.tbrcED_Item( '39261' ).itemgren,
		NULL,
		1,
		'<CODE>',
		'</CODE>',
		CONFEXME_32.tbrcED_Item( '39261' ).itematfd
	);
...
--{
	...
	-- Se genera un identificador para el item
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEM;
	CONFEXME_32.tbrcED_Item( '39262' ).itemcodi := nuNextSeqValue;

	-- Se establece el nombre del objeto por defecto
	CONFEXME_32.tbrcED_Item( '39262' ).itemobna := NULL;

	-- Se asigna el grupo de entidad asociado al item
	if ( CONFEXME_32.tbrcED_AtriFuda.exists( '32976' ) ) then
		CONFEXME_32.tbrcED_Item( '39262' ).itematfd := CONFEXME_32.tbrcED_AtriFuda( '32976' ).atfdcodi;
	end if;
	...
	VALUES
	(
		CONFEXME_32.tbrcED_Item( '39262' ).itemcodi,
		'IMAGE',
		CONFEXME_32.tbrcED_Item( '39262' ).itemceid,
		NULL,
		CONFEXME_32.tbrcED_Item( '39262' ).itemobna,
		0,
		NULL,
		NULL,
		CONFEXME_32.tbrcED_Item( '39262' ).itemgren,
		NULL,
		1,
		'<IMAGE>',
		'</IMAGE>',
		CONFEXME_32.tbrcED_Item( '39262' ).itematfd
	);
...
--{
	...
	-- Se genera un identificador para el item
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEM;
	CONFEXME_32.tbrcED_Item( '39275' ).itemcodi := nuNextSeqValue;

	-- Se establece el nombre del objeto por defecto
	CONFEXME_32.tbrcED_Item( '39275' ).itemobna := NULL;

	-- Se asigna el grupo de entidad asociado al item
	if ( CONFEXME_32.tbrcED_AtriFuda.exists( '32989' ) ) then
		CONFEXME_32.tbrcED_Item( '39275' ).itematfd := CONFEXME_32.tbrcED_AtriFuda( '32989' ).atfdcodi;
	end if;
	...
	VALUES
	(
		CONFEXME_32.tbrcED_Item( '39275' ).itemcodi,
		'CLIENTE',
		CONFEXME_32.tbrcED_Item( '39275' ).itemceid,
		NULL,
		CONFEXME_32.tbrcED_Item( '39275' ).itemobna,
		0,
		NULL,
		NULL,
		CONFEXME_32.tbrcED_Item( '39275' ).itemgren,
		NULL,
		1,
		'<CLIENTE>',
		'</CLIENTE>',
		CONFEXME_32.tbrcED_Item( '39275' ).itematfd
	);
...
--{
	...
	-- Se genera un identificador para el item
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEM;
	CONFEXME_32.tbrcED_Item( '39276' ).itemcodi := nuNextSeqValue;

	-- Se establece el nombre del objeto por defecto
	CONFEXME_32.tbrcED_Item( '39276' ).itemobna := NULL;

	-- Se asigna el grupo de entidad asociado al item
	if ( CONFEXME_32.tbrcED_AtriFuda.exists( '32990' ) ) then
		CONFEXME_32.tbrcED_Item( '39276' ).itematfd := CONFEXME_32.tbrcED_AtriFuda( '32990' ).atfdcodi;
	end if;
	...
	VALUES
	(
		CONFEXME_32.tbrcED_Item( '39276' ).itemcodi,
		'IDENTIFICACION',
		CONFEXME_32.tbrcED_Item( '39276' ).itemceid,
		NULL,
		CONFEXME_32.tbrcED_Item( '39276' ).itemobna,
		0,
		NULL,
		NULL,
		CONFEXME_32.tbrcED_Item( '39276' ).itemgren,
		NULL,
		1,
		'<IDENTIFICACION>',
		'</IDENTIFICACION>',
		CONFEXME_32.tbrcED_Item( '39276' ).itematfd
	);
...
--{
	...
	-- Se genera un identificador para el item
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEM;
	CONFEXME_32.tbrcED_Item( '39277' ).itemcodi := nuNextSeqValue;

	-- Se establece el nombre del objeto por defecto
	CONFEXME_32.tbrcED_Item( '39277' ).itemobna := NULL;

	-- Se asigna el grupo de entidad asociado al item
	if ( CONFEXME_32.tbrcED_AtriFuda.exists( '32991' ) ) then
		CONFEXME_32.tbrcED_Item( '39277' ).itematfd := CONFEXME_32.tbrcED_AtriFuda( '32991' ).atfdcodi;
	end if;
	...
	VALUES
	(
		CONFEXME_32.tbrcED_Item( '39277' ).itemcodi,
		'PROYECTO',
		CONFEXME_32.tbrcED_Item( '39277' ).itemceid,
		NULL,
		CONFEXME_32.tbrcED_Item( '39277' ).itemobna,
		0,
		NULL,
		NULL,
		CONFEXME_32.tbrcED_Item( '39277' ).itemgren,
		NULL,
		1,
		'<PROYECTO>',
		'</PROYECTO>',
		CONFEXME_32.tbrcED_Item( '39277' ).itematfd
	);
...
--{
	...
	-- Se genera un identificador para el item
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEM;
	CONFEXME_32.tbrcED_Item( '39278' ).itemcodi := nuNextSeqValue;

	-- Se establece el nombre del objeto por defecto
	CONFEXME_32.tbrcED_Item( '39278' ).itemobna := NULL;

	-- Se asigna el grupo de entidad asociado al item
	if ( CONFEXME_32.tbrcED_AtriFuda.exists( '32992' ) ) then
		CONFEXME_32.tbrcED_Item( '39278' ).itematfd := CONFEXME_32.tbrcED_AtriFuda( '32992' ).atfdcodi;
	end if;
	...
	VALUES
	(
		CONFEXME_32.tbrcED_Item( '39278' ).itemcodi,
		'CONTRATO',
		CONFEXME_32.tbrcED_Item( '39278' ).itemceid,
		NULL,
		CONFEXME_32.tbrcED_Item( '39278' ).itemobna,
		0,
		NULL,
		NULL,
		CONFEXME_32.tbrcED_Item( '39278' ).itemgren,
		NULL,
		2,
		'<CONTRATO>',
		'</CONTRATO>',
		CONFEXME_32.tbrcED_Item( '39278' ).itematfd
	);
...
--{
	...
	-- Se genera un identificador para el item
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEM;
	CONFEXME_32.tbrcED_Item( '39279' ).itemcodi := nuNextSeqValue;

	-- Se establece el nombre del objeto por defecto
	CONFEXME_32.tbrcED_Item( '39279' ).itemobna := NULL;

	-- Se asigna el grupo de entidad asociado al item
	if ( CONFEXME_32.tbrcED_AtriFuda.exists( '32993' ) ) then
		CONFEXME_32.tbrcED_Item( '39279' ).itematfd := CONFEXME_32.tbrcED_AtriFuda( '32993' ).atfdcodi;
	end if;
	...
	VALUES
	(
		CONFEXME_32.tbrcED_Item( '39279' ).itemcodi,
		'DIRPROYECTO',
		CONFEXME_32.tbrcED_Item( '39279' ).itemceid,
		NULL,
		CONFEXME_32.tbrcED_Item( '39279' ).itemobna,
		0,
		NULL,
		NULL,
		CONFEXME_32.tbrcED_Item( '39279' ).itemgren,
		NULL,
		1,
		'<DIRPROYECTO>',
		'</DIRPROYECTO>',
		CONFEXME_32.tbrcED_Item( '39279' ).itematfd
	);
...
--{
	...
	-- Se genera un identificador para el item
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEM;
	CONFEXME_32.tbrcED_Item( '39280' ).itemcodi := nuNextSeqValue;

	-- Se establece el nombre del objeto por defecto
	CONFEXME_32.tbrcED_Item( '39280' ).itemobna := NULL;

	-- Se asigna el grupo de entidad asociado al item
	if ( CONFEXME_32.tbrcED_AtriFuda.exists( '32994' ) ) then
		CONFEXME_32.tbrcED_Item( '39280' ).itematfd := CONFEXME_32.tbrcED_AtriFuda( '32994' ).atfdcodi;
	end if;
	...
	VALUES
	(
		CONFEXME_32.tbrcED_Item( '39280' ).itemcodi,
		'SOLICITUD',
		CONFEXME_32.tbrcED_Item( '39280' ).itemceid,
		NULL,
		CONFEXME_32.tbrcED_Item( '39280' ).itemobna,
		0,
		NULL,
		NULL,
		CONFEXME_32.tbrcED_Item( '39280' ).itemgren,
		NULL,
		2,
		'<SOLICITUD>',
		'</SOLICITUD>',
		CONFEXME_32.tbrcED_Item( '39280' ).itematfd
	);
...
--{
	...
	-- Se genera un identificador para el item
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEM;
	CONFEXME_32.tbrcED_Item( '39281' ).itemcodi := nuNextSeqValue;

	-- Se establece el nombre del objeto por defecto
	CONFEXME_32.tbrcED_Item( '39281' ).itemobna := NULL;

	-- Se asigna el grupo de entidad asociado al item
	if ( CONFEXME_32.tbrcED_AtriFuda.exists( '32995' ) ) then
		CONFEXME_32.tbrcED_Item( '39281' ).itematfd := CONFEXME_32.tbrcED_AtriFuda( '32995' ).atfdcodi;
	end if;
	...
	VALUES
	(
		CONFEXME_32.tbrcED_Item( '39281' ).itemcodi,
		'TIPOCUPON',
		CONFEXME_32.tbrcED_Item( '39281' ).itemceid,
		NULL,
		CONFEXME_32.tbrcED_Item( '39281' ).itemobna,
		0,
		NULL,
		NULL,
		CONFEXME_32.tbrcED_Item( '39281' ).itemgren,
		NULL,
		1,
		'<TIPOCUPON>',
		'</TIPOCUPON>',
		CONFEXME_32.tbrcED_Item( '39281' ).itematfd
	);
...
--{
	...
	-- Se genera un identificador para el item
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEM;
	CONFEXME_32.tbrcED_Item( '39282' ).itemcodi := nuNextSeqValue;

	-- Se establece el nombre del objeto por defecto
	CONFEXME_32.tbrcED_Item( '39282' ).itemobna := NULL;

	-- Se asigna el grupo de entidad asociado al item
	if ( CONFEXME_32.tbrcED_AtriFuda.exists( '32996' ) ) then
		CONFEXME_32.tbrcED_Item( '39282' ).itematfd := CONFEXME_32.tbrcED_AtriFuda( '32996' ).atfdcodi;
	end if;
	...
	VALUES
	(
		CONFEXME_32.tbrcED_Item( '39282' ).itemcodi,
		'CUPON',
		CONFEXME_32.tbrcED_Item( '39282' ).itemceid,
		NULL,
		CONFEXME_32.tbrcED_Item( '39282' ).itemobna,
		0,
		NULL,
		NULL,
		CONFEXME_32.tbrcED_Item( '39282' ).itemgren,
		NULL,
		1,
		'<CUPON>',
		'</CUPON>',
		CONFEXME_32.tbrcED_Item( '39282' ).itematfd
	);
...
--{
	...
	-- Se genera un identificador para el item
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEM;
	CONFEXME_32.tbrcED_Item( '39283' ).itemcodi := nuNextSeqValue;

	-- Se establece el nombre del objeto por defecto
	CONFEXME_32.tbrcED_Item( '39283' ).itemobna := NULL;

	-- Se asigna el grupo de entidad asociado al item
	if ( CONFEXME_32.tbrcED_AtriFuda.exists( '32997' ) ) then
		CONFEXME_32.tbrcED_Item( '39283' ).itematfd := CONFEXME_32.tbrcED_AtriFuda( '32997' ).atfdcodi;
	end if;
	...
	VALUES
	(
		CONFEXME_32.tbrcED_Item( '39283' ).itemcodi,
		'VALOR',
		CONFEXME_32.tbrcED_Item( '39283' ).itemceid,
		NULL,
		CONFEXME_32.tbrcED_Item( '39283' ).itemobna,
		0,
		NULL,
		NULL,
		CONFEXME_32.tbrcED_Item( '39283' ).itemgren,
		NULL,
		1,
		'<VALOR>',
		'</VALOR>',
		CONFEXME_32.tbrcED_Item( '39283' ).itematfd
	);
...
--{
	...
	-- Se genera un identificador para el item
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEM;
	CONFEXME_32.tbrcED_Item( '39284' ).itemcodi := nuNextSeqValue;

	-- Se establece el nombre del objeto por defecto
	CONFEXME_32.tbrcED_Item( '39284' ).itemobna := NULL;

	-- Se asigna el grupo de entidad asociado al item
	if ( CONFEXME_32.tbrcED_AtriFuda.exists( '32998' ) ) then
		CONFEXME_32.tbrcED_Item( '39284' ).itematfd := CONFEXME_32.tbrcED_AtriFuda( '32998' ).atfdcodi;
	end if;
	...
	VALUES
	(
		CONFEXME_32.tbrcED_Item( '39284' ).itemcodi,
		'VALIDOHASTA',
		CONFEXME_32.tbrcED_Item( '39284' ).itemceid,
		NULL,
		CONFEXME_32.tbrcED_Item( '39284' ).itemobna,
		0,
		NULL,
		NULL,
		CONFEXME_32.tbrcED_Item( '39284' ).itemgren,
		NULL,
		1,
		'<VALIDOHASTA>',
		'</VALIDOHASTA>',
		CONFEXME_32.tbrcED_Item( '39284' ).itematfd
	);
...
--{
	...
	-- Se genera un identificador para el item
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEM;
	CONFEXME_32.tbrcED_Item( '39285' ).itemcodi := nuNextSeqValue;

	-- Se establece el nombre del objeto por defecto
	CONFEXME_32.tbrcED_Item( '39285' ).itemobna := NULL;

	-- Se asigna el grupo de entidad asociado al item
	if ( CONFEXME_32.tbrcED_AtriFuda.exists( '32999' ) ) then
		CONFEXME_32.tbrcED_Item( '39285' ).itematfd := CONFEXME_32.tbrcED_AtriFuda( '32999' ).atfdcodi;
	end if;
	...
	VALUES
	(
		CONFEXME_32.tbrcED_Item( '39285' ).itemcodi,
		'TIPO_SOL',
		CONFEXME_32.tbrcED_Item( '39285' ).itemceid,
		NULL,
		CONFEXME_32.tbrcED_Item( '39285' ).itemobna,
		0,
		NULL,
		NULL,
		CONFEXME_32.tbrcED_Item( '39285' ).itemgren,
		NULL,
		1,
		'<TIPO_SOL>',
		'</TIPO_SOL>',
		CONFEXME_32.tbrcED_Item( '39285' ).itematfd
	);
...
--{
	...
	-- Se genera un identificador para el item
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEM;
	CONFEXME_32.tbrcED_Item( '39286' ).itemcodi := nuNextSeqValue;

	-- Se establece el nombre del objeto por defecto
	CONFEXME_32.tbrcED_Item( '39286' ).itemobna := NULL;

	-- Se asigna el grupo de entidad asociado al item
	if ( CONFEXME_32.tbrcED_AtriFuda.exists( '33000' ) ) then
		CONFEXME_32.tbrcED_Item( '39286' ).itematfd := CONFEXME_32.tbrcED_AtriFuda( '33000' ).atfdcodi;
	end if;
	...
	VALUES
	(
		CONFEXME_32.tbrcED_Item( '39286' ).itemcodi,
		'CODIGODEBARRAS',
		CONFEXME_32.tbrcED_Item( '39286' ).itemceid,
		NULL,
		CONFEXME_32.tbrcED_Item( '39286' ).itemobna,
		0,
		NULL,
		NULL,
		CONFEXME_32.tbrcED_Item( '39286' ).itemgren,
		NULL,
		1,
		'<CODIGODEBARRAS>',
		'</CODIGODEBARRAS>',
		CONFEXME_32.tbrcED_Item( '39286' ).itematfd
	);
...
--{
	...
	CONFEXME_32.tbrcED_ItemBloq( '39256' ).itblcodi := nuNextSeqValue;

	-- Se asigna el bloque-franja
	CONFEXME_32.tbrcED_ItemBloq( '39256' ).itblblfr := CONFEXME_32.tbrcED_BloqFran( '7694' ).blfrcodi;

	-- Se asigna el item
	if ( CONFEXME_32.tbrcED_Item.exists( '39261' ) ) then
		CONFEXME_32.tbrcED_ItemBloq( '39256' ).itblitem := CONFEXME_32.tbrcED_Item( '39261' ).itemcodi;
	end if;
	...
	VALUES
	(
		CONFEXME_32.tbrcED_ItemBloq( '39256' ).itblcodi,
		CONFEXME_32.tbrcED_ItemBloq( '39256' ).itblitem,
		CONFEXME_32.tbrcED_ItemBloq( '39256' ).itblblfr,
		1
	);
...
--{
	...
	-- Se genera un identificador para el item-bloque
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEMBLOQ;
	CONFEXME_32.tbrcED_ItemBloq( '39257' ).itblcodi := nuNextSeqValue;

	-- Se asigna el bloque-franja
	CONFEXME_32.tbrcED_ItemBloq( '39257' ).itblblfr := CONFEXME_32.tbrcED_BloqFran( '7694' ).blfrcodi;

	-- Se asigna el item
	if ( CONFEXME_32.tbrcED_Item.exists( '39262' ) ) then
		CONFEXME_32.tbrcED_ItemBloq( '39257' ).itblitem := CONFEXME_32.tbrcED_Item( '39262' ).itemcodi;
	end if;
	...
	VALUES
	(
		CONFEXME_32.tbrcED_ItemBloq( '39257' ).itblcodi,
		CONFEXME_32.tbrcED_ItemBloq( '39257' ).itblitem,
		CONFEXME_32.tbrcED_ItemBloq( '39257' ).itblblfr,
		2
	);
...
--{
	...
	-- Se genera un identificador para el item-bloque
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEMBLOQ;
	CONFEXME_32.tbrcED_ItemBloq( '39270' ).itblcodi := nuNextSeqValue;

	-- Se asigna el bloque-franja
	CONFEXME_32.tbrcED_ItemBloq( '39270' ).itblblfr := CONFEXME_32.tbrcED_BloqFran( '7695' ).blfrcodi;

	-- Se asigna el item
	if ( CONFEXME_32.tbrcED_Item.exists( '39275' ) ) then
		CONFEXME_32.tbrcED_ItemBloq( '39270' ).itblitem := CONFEXME_32.tbrcED_Item( '39275' ).itemcodi;
	end if;
	...
	VALUES
	(
		CONFEXME_32.tbrcED_ItemBloq( '39270' ).itblcodi,
		CONFEXME_32.tbrcED_ItemBloq( '39270' ).itblitem,
		CONFEXME_32.tbrcED_ItemBloq( '39270' ).itblblfr,
		1
	);
...
--{
	...
	-- Se genera un identificador para el item-bloque
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEMBLOQ;
	CONFEXME_32.tbrcED_ItemBloq( '39271' ).itblcodi := nuNextSeqValue;

	-- Se asigna el bloque-franja
	CONFEXME_32.tbrcED_ItemBloq( '39271' ).itblblfr := CONFEXME_32.tbrcED_BloqFran( '7695' ).blfrcodi;

	-- Se asigna el item
	if ( CONFEXME_32.tbrcED_Item.exists( '39276' ) ) then
		CONFEXME_32.tbrcED_ItemBloq( '39271' ).itblitem := CONFEXME_32.tbrcED_Item( '39276' ).itemcodi;
	end if;
	...
	VALUES
	(
		CONFEXME_32.tbrcED_ItemBloq( '39271' ).itblcodi,
		CONFEXME_32.tbrcED_ItemBloq( '39271' ).itblitem,
		CONFEXME_32.tbrcED_ItemBloq( '39271' ).itblblfr,
		2
	);
...
--{
	...
	-- Se genera un identificador para el item-bloque
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEMBLOQ;
	CONFEXME_32.tbrcED_ItemBloq( '39272' ).itblcodi := nuNextSeqValue;

	-- Se asigna el bloque-franja
	CONFEXME_32.tbrcED_ItemBloq( '39272' ).itblblfr := CONFEXME_32.tbrcED_BloqFran( '7695' ).blfrcodi;

	-- Se asigna el item
	if ( CONFEXME_32.tbrcED_Item.exists( '39277' ) ) then
		CONFEXME_32.tbrcED_ItemBloq( '39272' ).itblitem := CONFEXME_32.tbrcED_Item( '39277' ).itemcodi;
	end if;
	...
	VALUES
	(
		CONFEXME_32.tbrcED_ItemBloq( '39272' ).itblcodi,
		CONFEXME_32.tbrcED_ItemBloq( '39272' ).itblitem,
		CONFEXME_32.tbrcED_ItemBloq( '39272' ).itblblfr,
		3
	);
...
--{
	...
	-- Se genera un identificador para el item-bloque
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEMBLOQ;
	CONFEXME_32.tbrcED_ItemBloq( '39273' ).itblcodi := nuNextSeqValue;

	-- Se asigna el bloque-franja
	CONFEXME_32.tbrcED_ItemBloq( '39273' ).itblblfr := CONFEXME_32.tbrcED_BloqFran( '7695' ).blfrcodi;

	-- Se asigna el item
	if ( CONFEXME_32.tbrcED_Item.exists( '39278' ) ) then
		CONFEXME_32.tbrcED_ItemBloq( '39273' ).itblitem := CONFEXME_32.tbrcED_Item( '39278' ).itemcodi;
	end if;
	...
	VALUES
	(
		CONFEXME_32.tbrcED_ItemBloq( '39273' ).itblcodi,
		CONFEXME_32.tbrcED_ItemBloq( '39273' ).itblitem,
		CONFEXME_32.tbrcED_ItemBloq( '39273' ).itblblfr,
		4
	);
...
--{
	...
	-- Se genera un identificador para el item-bloque
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEMBLOQ;
	CONFEXME_32.tbrcED_ItemBloq( '39274' ).itblcodi := nuNextSeqValue;

	-- Se asigna el bloque-franja
	CONFEXME_32.tbrcED_ItemBloq( '39274' ).itblblfr := CONFEXME_32.tbrcED_BloqFran( '7695' ).blfrcodi;

	-- Se asigna el item
	if ( CONFEXME_32.tbrcED_Item.exists( '39279' ) ) then
		CONFEXME_32.tbrcED_ItemBloq( '39274' ).itblitem := CONFEXME_32.tbrcED_Item( '39279' ).itemcodi;
	end if;
	...
	VALUES
	(
		CONFEXME_32.tbrcED_ItemBloq( '39274' ).itblcodi,
		CONFEXME_32.tbrcED_ItemBloq( '39274' ).itblitem,
		CONFEXME_32.tbrcED_ItemBloq( '39274' ).itblblfr,
		5
	);
...
--{
	...
	-- Se genera un identificador para el item-bloque
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEMBLOQ;
	CONFEXME_32.tbrcED_ItemBloq( '39275' ).itblcodi := nuNextSeqValue;

	-- Se asigna el bloque-franja
	CONFEXME_32.tbrcED_ItemBloq( '39275' ).itblblfr := CONFEXME_32.tbrcED_BloqFran( '7695' ).blfrcodi;

	-- Se asigna el item
	if ( CONFEXME_32.tbrcED_Item.exists( '39280' ) ) then
		CONFEXME_32.tbrcED_ItemBloq( '39275' ).itblitem := CONFEXME_32.tbrcED_Item( '39280' ).itemcodi;
	end if;
	...
	VALUES
	(
		CONFEXME_32.tbrcED_ItemBloq( '39275' ).itblcodi,
		CONFEXME_32.tbrcED_ItemBloq( '39275' ).itblitem,
		CONFEXME_32.tbrcED_ItemBloq( '39275' ).itblblfr,
		6
	);
...
--{
	...
	-- Se genera un identificador para el item-bloque
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEMBLOQ;
	CONFEXME_32.tbrcED_ItemBloq( '39276' ).itblcodi := nuNextSeqValue;

	-- Se asigna el bloque-franja
	CONFEXME_32.tbrcED_ItemBloq( '39276' ).itblblfr := CONFEXME_32.tbrcED_BloqFran( '7695' ).blfrcodi;

	-- Se asigna el item
	if ( CONFEXME_32.tbrcED_Item.exists( '39281' ) ) then
		CONFEXME_32.tbrcED_ItemBloq( '39276' ).itblitem := CONFEXME_32.tbrcED_Item( '39281' ).itemcodi;
	end if;
	...
	VALUES
	(
		CONFEXME_32.tbrcED_ItemBloq( '39276' ).itblcodi,
		CONFEXME_32.tbrcED_ItemBloq( '39276' ).itblitem,
		CONFEXME_32.tbrcED_ItemBloq( '39276' ).itblblfr,
		7
	);
...
--{
	...
	-- Se genera un identificador para el item-bloque
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEMBLOQ;
	CONFEXME_32.tbrcED_ItemBloq( '39277' ).itblcodi := nuNextSeqValue;

	-- Se asigna el bloque-franja
	CONFEXME_32.tbrcED_ItemBloq( '39277' ).itblblfr := CONFEXME_32.tbrcED_BloqFran( '7695' ).blfrcodi;

	-- Se asigna el item
	if ( CONFEXME_32.tbrcED_Item.exists( '39282' ) ) then
		CONFEXME_32.tbrcED_ItemBloq( '39277' ).itblitem := CONFEXME_32.tbrcED_Item( '39282' ).itemcodi;
	end if;
	...
	VALUES
	(
		CONFEXME_32.tbrcED_ItemBloq( '39277' ).itblcodi,
		CONFEXME_32.tbrcED_ItemBloq( '39277' ).itblitem,
		CONFEXME_32.tbrcED_ItemBloq( '39277' ).itblblfr,
		8
	);
...
--{
	...
	-- Se genera un identificador para el item-bloque
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEMBLOQ;
	CONFEXME_32.tbrcED_ItemBloq( '39278' ).itblcodi := nuNextSeqValue;

	-- Se asigna el bloque-franja
	CONFEXME_32.tbrcED_ItemBloq( '39278' ).itblblfr := CONFEXME_32.tbrcED_BloqFran( '7695' ).blfrcodi;

	-- Se asigna el item
	if ( CONFEXME_32.tbrcED_Item.exists( '39283' ) ) then
		CONFEXME_32.tbrcED_ItemBloq( '39278' ).itblitem := CONFEXME_32.tbrcED_Item( '39283' ).itemcodi;
	end if;
	...
	VALUES
	(
		CONFEXME_32.tbrcED_ItemBloq( '39278' ).itblcodi,
		CONFEXME_32.tbrcED_ItemBloq( '39278' ).itblitem,
		CONFEXME_32.tbrcED_ItemBloq( '39278' ).itblblfr,
		9
	);
...
--{
	...
	-- Se genera un identificador para el item-bloque
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEMBLOQ;
	CONFEXME_32.tbrcED_ItemBloq( '39279' ).itblcodi := nuNextSeqValue;

	-- Se asigna el bloque-franja
	CONFEXME_32.tbrcED_ItemBloq( '39279' ).itblblfr := CONFEXME_32.tbrcED_BloqFran( '7695' ).blfrcodi;

	-- Se asigna el item
	if ( CONFEXME_32.tbrcED_Item.exists( '39284' ) ) then
		CONFEXME_32.tbrcED_ItemBloq( '39279' ).itblitem := CONFEXME_32.tbrcED_Item( '39284' ).itemcodi;
	end if;
	...
	VALUES
	(
		CONFEXME_32.tbrcED_ItemBloq( '39279' ).itblcodi,
		CONFEXME_32.tbrcED_ItemBloq( '39279' ).itblitem,
		CONFEXME_32.tbrcED_ItemBloq( '39279' ).itblblfr,
		10
	);
...
--{
	...
	-- Se genera un identificador para el item-bloque
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEMBLOQ;
	CONFEXME_32.tbrcED_ItemBloq( '39280' ).itblcodi := nuNextSeqValue;

	-- Se asigna el bloque-franja
	CONFEXME_32.tbrcED_ItemBloq( '39280' ).itblblfr := CONFEXME_32.tbrcED_BloqFran( '7695' ).blfrcodi;

	-- Se asigna el item
	if ( CONFEXME_32.tbrcED_Item.exists( '39285' ) ) then
		CONFEXME_32.tbrcED_ItemBloq( '39280' ).itblitem := CONFEXME_32.tbrcED_Item( '39285' ).itemcodi;
	end if;
	...
	VALUES
	(
		CONFEXME_32.tbrcED_ItemBloq( '39280' ).itblcodi,
		CONFEXME_32.tbrcED_ItemBloq( '39280' ).itblitem,
		CONFEXME_32.tbrcED_ItemBloq( '39280' ).itblblfr,
		11
	);
...
--{
	...
	-- Se genera un identificador para el item-bloque
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEMBLOQ;
	CONFEXME_32.tbrcED_ItemBloq( '39281' ).itblcodi := nuNextSeqValue;

	-- Se asigna el bloque-franja
	CONFEXME_32.tbrcED_ItemBloq( '39281' ).itblblfr := CONFEXME_32.tbrcED_BloqFran( '7695' ).blfrcodi;

	-- Se asigna el item
	if ( CONFEXME_32.tbrcED_Item.exists( '39286' ) ) then
		CONFEXME_32.tbrcED_ItemBloq( '39281' ).itblitem := CONFEXME_32.tbrcED_Item( '39286' ).itemcodi;
	end if;
	...
	VALUES
	(
		CONFEXME_32.tbrcED_ItemBloq( '39281' ).itblcodi,
		CONFEXME_32.tbrcED_ItemBloq( '39281' ).itblitem,
		CONFEXME_32.tbrcED_ItemBloq( '39281' ).itblblfr,
		12
	);
...
--{
	...
	open  CONFEXME_32.cuPlantilla( 42 );
	fetch CONFEXME_32.cuPlantilla into nuIdPlantill;
	close CONFEXME_32.cuPlantilla;
...
--{
	...
	open  CONFEXME_32.cuExtractAndMix( 32 );
	fetch CONFEXME_32.cuExtractAndMix into nuExtractAndMixId;
	close CONFEXME_32.cuExtractAndMix;
	...
	--{
		-- Se modifica el formato
		UPDATE ed_confexme  
		SET    coemdesc = 'LDC_CUPON_VENTA_BRILLA',
		       coeminic = NULL,
		       coempada = '<83>',
...
BEGIN
--{
	if ( CONFEXME_32.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	...
	CONFEXME_32.DeleteGeneratedExpressions;
...
BEGIN
--{
	UT_Trace.Trace( '********************** Borrando paquete CONFEXME_32 ***********************', 5 );
