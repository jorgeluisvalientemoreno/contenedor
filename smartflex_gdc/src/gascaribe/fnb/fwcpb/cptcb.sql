...
BEGIN
sa_bocreatePackages.CreatePackage('CPTCB_',
'CREATE OR REPLACE PACKAGE CPTCB_ IS ' || chr(10) ||
...
'tb2_0 ty2_0;  executableName ge_catalog.tag_name%type := ''CPTCB''; ' || chr(10) ||
...
'( ' || chr(10) ||
'SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME=''CPTCB'' ' || chr(10) ||
'); ' || chr(10) ||
...
BEGIN ut_trace.trace('********************Comenzar proceso de objeto:CPTCB_******************************'); END;
/
...
Open CPTCB_.cuRoleExecutables;
loop
 fetch CPTCB_.cuRoleExecutables INTO CPTCB_.rcRoleExecutables;
 exit when  CPTCB_.cuRoleExecutables%notfound;
 CPTCB_.tbRoleExecutables(nuIndex) := CPTCB_.rcRoleExecutables;
 nuIndex := nuIndex + 1;
END loop;
close CPTCB_.cuRoleExecutables;
nuIndex := 0;
Open CPTCB_.cuUserExceptions ;
loop
 fetch CPTCB_.cuUserExceptions INTO  CPTCB_.rcUserExceptions;
 exit when CPTCB_.cuUserExceptions%notfound;
 CPTCB_.tbUserException(nuIndex):=CPTCB_.rcUserExceptions;
 nuIndex := nuIndex + 1;
END loop;
close CPTCB_.cuUserExceptions;
nuIndex := 0;
Open CPTCB_.cuExecEntities ;
loop
 fetch CPTCB_.cuExecEntities INTO  CPTCB_.rcExecEntities;
 exit when CPTCB_.cuExecEntities%notfound;
 CPTCB_.tbExecEntities(nuIndex):=CPTCB_.rcExecEntities;
 nuIndex := nuIndex + 1;
END loop;
close CPTCB_.cuExecEntities;
...
(
 SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='CPTCB'
);
    ...
    BEGIN
      ...
      IF (nuNumRecords > 0) THEN
        ...
        IF ( sbObjectName IS NOT null ) THEN
          ...
          if ( CPTCB_.tbObjectToDelete.last IS null ) then
            nuNumObjects := 1;
          else
            nuNumObjects := CPTCB_.tbObjectToDelete.last + 1;
          end if;
          CPTCB_.tbObjectToDelete(nuNumObjects) := sbObjectName;
        END IF;
...
WHERE   sa_executable.name = 'CPTCB'
...
if (not CPTCB_.blProcessStatus) then
 ...
 FOR crObjectsName IN cuObjectsName LOOP
  CPTCB_.tbObjectToDelete(nuIndex) := crObjectsName.OBJECT_NAME;
  nuIndex := nuIndex + 1;
 END LOOP;
EXCEPTION 
...
if (not CPTCB_.blProcessStatus) then
 return;
  ...
  DELETE FROM SA_ROLE_EXECUTABLES WHERE (EXECUTABLE_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='CPTCB') AND ROLE_ID=1;
...
if (not CPTCB_.blProcessStatus) then
 return;
  ...
  DELETE FROM SA_USER_EXCEPTIONS WHERE (EXECUTABLE_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='CPTCB') AND 1=2;
...
SELECT rowid
FROM SA_EXECUTABLE_SYNON WHERE (SYNONYMOUS_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='CPTCB');
nuIndex binary_integer;
BEGIN
...
if (not CPTCB_.blProcessStatus) then
 return;
  ...
  DELETE FROM SA_EXEC_ENTITIES WHERE (EXECUTABLE_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='CPTCB');
...
if (not CPTCB_.blProcessStatus) then
 return;
  ...
  DELETE FROM SA_MENU_OPTION WHERE (EXECUTABLE_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='CPTCB');
...
if (not CPTCB_.blProcessStatus) then
 return;
  ...
  DELETE FROM GI_CONFIG_COMP WHERE (CONFIG_ID) in (SELECT CONFIG_ID FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='CPTCB') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339);
...
if (not CPTCB_.blProcessStatus) then
 return;
  ...
  ut_trace.trace('Borrando tabla GI_CONFIG_COMP',1);
  DELETE FROM GI_CONFIG_COMP WHERE (COMPOSITION_ID) in (SELECT COMPOSITION_ID FROM GI_COMPOSITION WHERE (CONFIG_ID) in (SELECT CONFIG_ID FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='CPTCB') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339));
...
DECLARE 
 ...
 SELECT rowid FROM GE_STATEMENT WHERE (STATEMENT_ID) in (SELECT SELECT_STATEMENT_ID FROM GI_COMP_ATTRIBS WHERE (COMPOSITION_ID) in (SELECT COMPOSITION_ID FROM GI_COMPOSITION WHERE (CONFIG_ID) in (SELECT CONFIG_ID FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='CPTCB') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339)));
BEGIN 
...
CPTCB_.tbGE_STATEMENTRowId(rcData.rowid):=rcData.rowid; 
END loop; 
...
for rcData in cuLoadTemporaryTable loop
CPTCB_.tbGR_CONFIG_EXPRESSIONRowId(rcData.rowid):=rcData.rowid; 
END loop; 
...
if (not CPTCB_.blProcessStatus) then
 return;
  ...
  DELETE FROM GI_COMP_ATTRIBS WHERE (COMPOSITION_ID) in (SELECT COMPOSITION_ID FROM GI_COMPOSITION WHERE (CONFIG_ID) in (SELECT CONFIG_ID FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='CPTCB') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339));
...
nuVarcharIndex:=CPTCB_.tbGE_STATEMENTRowId.first;  
while (nuVarcharIndex is not null) loop 
begin
ut_trace.trace('Row id a borrar:'||nuVarcharIndex,1);
delete from GE_STATEMENT where rowid = CPTCB_.tbGE_STATEMENTRowId(nuVarcharIndex); 
...
CPTCB_.tbGE_STATEMENTRowId.delete(nuOldVarcharIndex);
end if;
nuVarcharIndex := CPTCB_.tbGE_STATEMENTRowId.next(nuVarcharIndex); 
END loop; 
...
nuVarcharIndex:=CPTCB_.tbGR_CONFIG_EXPRESSIONRowId.first;  
while (nuVarcharIndex is not null) loop 
ut_trace.trace('Row id a borrar:'||nuVarcharIndex,1);
delete from GR_CONFIG_EXPRESSION where rowid = CPTCB_.tbGR_CONFIG_EXPRESSIONRowId(nuVarcharIndex); 
nuOldVarcharIndex:=nuVarcharIndex;
nuVarcharIndex := CPTCB_.tbGR_CONFIG_EXPRESSIONRowId.next(nuVarcharIndex); 
CPTCB_.tbGR_CONFIG_EXPRESSIONRowId.delete(nuOldVarcharIndex); 
END loop; 
...
for rcData in cuLoadTemporaryTable loop
CPTCB_.tbGI_COMPOSITIONRowId(rcData.rowid):=rcData.rowid; 
END loop; 
...
if (not CPTCB_.blProcessStatus) then
 return;
  ...
  DELETE FROM GI_FRAME WHERE (COMPOSITION_ID) in (SELECT COMPOSITION_ID FROM GI_COMPOSITION WHERE (CONFIG_ID) in (SELECT CONFIG_ID FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='CPTCB') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339));
...
nuVarcharIndex:=CPTCB_.tbGI_COMPOSITIONRowId.first;  
while (nuVarcharIndex is not null) loop 
ut_trace.trace('Row id a borrar:'||nuVarcharIndex,1);
delete from GI_COMPOSITION where rowid = CPTCB_.tbGI_COMPOSITIONRowId(nuVarcharIndex); 
nuOldVarcharIndex:=nuVarcharIndex;
nuVarcharIndex := CPTCB_.tbGI_COMPOSITIONRowId.next(nuVarcharIndex); 
CPTCB_.tbGI_COMPOSITIONRowId.delete(nuOldVarcharIndex); 
END loop; 
...
if (not CPTCB_.blProcessStatus) then
 return;
  ...
  DELETE FROM GI_COMPOSITION WHERE (CONFIG_ID) in (SELECT CONFIG_ID FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='CPTCB') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339);
...
if (not CPTCB_.blProcessStatus) then
 return;
  ...
  DELETE FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='CPTCB') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339;
...
if (not CPTCB_.blProcessStatus) then
 return;
  ...
  DELETE FROM GE_DISTRIBUTION_FILE WHERE (DISTRIBUTION_FILE_ID) in (SELECT NAME FROM SA_EXECUTABLE WHERE NAME='CPTCB');
...
if (not CPTCB_.blProcessStatus) then
 return;
  ...
  DELETE FROM SA_EXECUTABLE WHERE NAME='CPTCB';
...
CPTCB_.old_tb0_0(0):='CPTCB'
;
CPTCB_.tb0_0(0):=UPPER(CPTCB_.old_tb0_0(0));
CPTCB_.tb0_0(0):=CPTCB_.tb0_0(0);
CPTCB_.old_tb0_1(0):=500000000002736;
CPTCB_.tb0_1(0):=GE_BOCATALOG.FNUGETIDSEQFROMCATALOG(CPTCB_.tb0_0(0), 'EXECUTABLE', 'SA_BOEXECUTABLE.GETNEXTID');
CPTCB_.tb0_1(0):=CPTCB_.tb0_1(0);
ut_trace.trace('insertando tabla: SA_EXECUTABLE fila (0)',1);
INSERT INTO SA_EXECUTABLE(NAME,EXECUTABLE_ID,DESCRIPTION,PATH,VERSION,EXECUTABLE_TYPE_ID,EXEC_OPER_TYPE_ID,MODULE_ID,SUBSYSTEM_ID,PARENT_EXECUTABLE_ID,LAST_RECORD_ALLOWED,PATH_FILE_HELP,WITHOUT_RESTR_POLICY,DIRECT_EXECUTION,TIMES_EXECUTED,EXEC_OWNER,LAST_DATE_EXECUTED,CLASS_ID) 
VALUES (CPTCB_.tb0_0(0),
CPTCB_.tb0_1(0),
...
CPTCB_.tb1_0(0):=1;
CPTCB_.tb1_1(0):=CPTCB_.tb0_1(0);
ut_trace.trace('insertando tabla: SA_ROLE_EXECUTABLES fila (0)',1);
INSERT INTO SA_ROLE_EXECUTABLES(ROLE_ID,EXECUTABLE_ID) 
VALUES (CPTCB_.tb1_0(0),
CPTCB_.tb1_1(0));
...
CPTCB_.tb2_0(0):=CPTCB_.tb0_0(0);
CPTCB_.clColumn_0 := '<?xml version="1.0" encoding="UTF-8"?>
...
'NAME>
      ...
      <VALID_EXPRESSION_ID/>
      <DEFAULT_VALUE/>
      <SELECT_STATEMENT>SELECT servcodi ID, servdesc DESCRIPTION
...
VALUES (CPTCB_.tb2_0(0),
XMLType(CPTCB_.clColumn_0),
'LDC - Crear Producto para Traslado de Cartera FNB'
,
'1.0.0.0'
,
...
UPDATE GE_DISTRIBUTION_FILE SET  GE_DISTRIBUTION_FILE.APP_XML = 
  ...
  '//COMPOSITION/ATTRIBUTE[ENTITY_ATTRIBUTE_ID = 897 and ENTITY_ID = 91]/VALID_EXPRESSION_ID/text()', 
  to_char(nuNewRule)) 
WHERE GE_DISTRIBUTION_FILE.DISTRIBUTION_FILE_ID = 'CPTCB';
...
VALUES (nuNewStatement, 16, 'LDC - Tipos de Producto FNB', 
'SELECT servcodi ID, servdesc DESCRIPTION
  FROM open.servicio s
 WHERE s.servcodi IN
                    (
                     SELECT COLUMN_VALUE FROM
                      TABLE(ldc_boutilities.splitstrings(dald_parameter.fsbgetvalue_chain('|| chr(39) ||'TIPOS_PRODUCTOS_CPTCB_REPETIR'|| chr(39) ||',NULL),'|| chr(39) ||','|| chr(39) ||'))
                     )
ORDER BY ID                     ', 'LDC - Tipos de Producto FNB'); 
...
BEGIN
 ...
 nuIndexInternal := CPTCB_.tb0_0.first;
 while nuIndexInternal IS NOT null loop
  IF (CPTCB_.tb0_0(nuIndexInternal)=isbExecutableName ) THEN
   IF (CPTCB_.tb0_1.exists(nuIndexInternal)) then
    tbExecutableEquivalence(isbExecutableName) := CPTCB_.tb0_1(nuIndexInternal);
   END IF;
   exit;
  END IF;
  nuIndexInternal := CPTCB_.tb0_0.next(nuIndexInternal);
 END loop; 
 return tbExecutableEquivalence(isbExecutableName);
END;
BEGIN
...
nuIndex :=  CPTCB_.tbRoleExecutables.first;
while nuIndex IS NOT null loop
 if (CPTCB_.tbRoleExecutables(nuIndex).role_id!=1) then
  nuNewExecutableId := fnuGetNewExecutableId(CPTCB_.tbRoleExecutables(nuIndex).name);
  IF nuNewExecutableId is not null then
    INSERT INTO sa_role_executables (role_id, executable_id)
    VALUES (CPTCB_.tbRoleExecutables(nuIndex).role_id, nuNewExecutableId);
  END IF;
  end if;
 nuIndex := CPTCB_.tbRoleExecutables.next(nuIndex);
END loop;
nuIndex :=  CPTCB_.tbUserException.first;
while nuIndex IS NOT null loop
 nuNewExecutableId := fnuGetNewExecutableId(CPTCB_.tbUserException(nuIndex).name);
 IF nuNewExecutableId is not null then
  insert INTO sa_user_exceptions (executable_id, user_id, status, usr_exc_type_id, comment_)
  VALUES (nuNewExecutableId ,CPTCB_.tbUserException(nuIndex).user_id, CPTCB_.tbUserException(nuIndex).status , CPTCB_.tbUserException(nuIndex).usr_exc_type_id, CPTCB_.tbUserException(nuIndex).comment_);
 END IF;
 nuIndex := CPTCB_.tbUserException.next(nuIndex);
END loop;
nuIndex :=  CPTCB_.tbExecEntities.first;
while nuIndex IS NOT null loop
 nuNewExecutableId := fnuGetNewExecutableId(CPTCB_.tbExecEntities(nuIndex).name);
 IF nuNewExecutableId is not null then
  select count(1) into nuRecCount
 from sa_exec_entities
  where executable_id = nuNewExecutableId and entity_id = CPTCB_.tbExecEntities(nuIndex).entity_id;
  IF nuRecCount = 0 then
    insert INTO sa_exec_entities (executable_id, entity_id )
    VALUES (nuNewExecutableId ,CPTCB_.tbExecEntities(nuIndex).entity_id );
  END IF;
 END IF;
 nuIndex := CPTCB_.tbExecEntities.next(nuIndex);
END loop;
...
BEGIN
 ...
 nuIndex := CPTCB_.tbObjectToDelete.first;
 while (nuIndex is not null) LOOP
  BEGIN
   ut_trace.trace('Objeto a borrar: ' || CPTCB_.tbObjectToDelete(nuIndex),1);
   IF(ut_object.fboExistObject(CPTCB_.tbObjectToDelete(nuIndex)) and not fblIsObjectUseByRule(CPTCB_.tbObjectToDelete(nuIndex))) THEN
    blObjectDeleted := ut_object.fboDeleteObject(CPTCB_.tbObjectToDelete(nuIndex));
    ut_trace.trace('  * Borrado ' || CPTCB_.tbObjectToDelete(nuIndex), 1);
   END IF;
  EXCEPTION when others then
   ut_trace.trace('- No se borra el objeto (' || CPTCB_.tbObjectToDelete(nuIndex) || '): ' || sqlerrm,1);
  END;
  nuIndex := CPTCB_.tbObjectToDelete.next(nuIndex);
 END loop;
...
begin
SA_BOCreatePackages.DropPackage('CPTCB_');
end;
/

BEGIN ut_trace.trace('********************FIN  proceso de objeto:CPTCB_******************************'); end;
/
