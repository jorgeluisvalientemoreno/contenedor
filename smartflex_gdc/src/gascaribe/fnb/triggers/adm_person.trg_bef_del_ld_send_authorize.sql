CREATE OR REPLACE trigger ADM_PERSON.TRG_BEF_DEL_LD_SEND_AUTHORIZE
  before delete on ld_send_authorized
  for each row
BEGIN
    /*******************************************************************************************
    ...
    *******************************************************************************************/
    BEGIN
        ...
        BEGIN
            ...
            IF NOT fblaplicaentrega(csbBSS_CAR_SMS_200792) AND
               SYS_CONTEXT('USERENV', 'MODULE') IN ('CRCAC', 'FWCMD') THEN
                ...
                sbError := 'Para poder borrar un registro debe tener aplicada la entrega ' ||
        ...
        EXCEPTION
            ...
            WHEN OTHERS THEN
                sbError := 'TERMINÿ CON ERROR NO CONTROLADO  TRG_AFT_INS_LDC_SEND_AUTHORIZE' ||
                           SQLERRM;
