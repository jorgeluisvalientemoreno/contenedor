...
namespace SINCECOMP.CANCELLATION.DAL
{
    class DALGENERAL
    {
        ...
        static String usuarioconectado = "ld_bopackagefnb.frfgetcontractorconnected"; //verificar que usuario se ha conectado al sistema
        ...
        public DataTable getUserConnect()
        {
            DataSet dsuserconnect = new DataSet("LDUserconnect");
            using (DbCommand cmdCommand = OpenDataBase.db.GetStoredProcCommand(usuarioconectado))
            {
                OpenDataBase.db.AddReturnRefCursor(cmdCommand);
                OpenDataBase.db.LoadDataSet(cmdCommand, dsuserconnect, "LDUserconnect");
            }
            return dsuserconnect.Tables["LDUserconnect"];
        }
