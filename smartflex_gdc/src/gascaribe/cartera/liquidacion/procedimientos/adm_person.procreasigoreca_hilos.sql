create or replace PROCEDURE ADM_PERSON.PROCREASIGORECA_HILOS (inuHilo   NUMBER,
  ...
  /*****************************************************************
	...
	09/05/2022		CGONZALEZ			OSF-266: Se modifica para crear la orden con la actividad nueva configurada en el parámetro ACT_GC_CASTIGADOS
  ...
  ******************************************************************/
    ...
    WHERE PRODUCTO=ProductId AND NUANO=ANO  AND NUMES=MES;
                ...
                IF  NUCANTORDEN = 0 THEN
                    ...
                    ELSE
                        ...
                        ELSE
                            ...
                            nAno := to_number(to_char(add_months(sysdate, -1),'yyyy'));
                            nMes := to_number(to_char(add_months(sysdate, -1),'mm'));
...
/
BEGIN
    pkg_utilidades.prAplicarPermisos('PROCREASIGORECA_HILOS', 'ADM_PERSON');
END;
/
