create or replace PACKAGE ADM_PERSON.LDC_PKG_CALC_GEST_CARTERA IS
  /******************************************************************************************************************************************************
    ...
    11/07/2023		OSF-1918	  jpinedc			Se modifica ldc_procnormalizados
    08/08/2023		OSF-1918	  jpinedc			Se modifica ldc_legaordergestcart_hilos
    21/08/2023		OSF-1918	  jpinedc			Se modifican 
                                                    * ldc_legaordergestcart_hilos 
                                                    * ldc_procnormalizados
	14/11/2024		OSF-3578	  jpinedc			* ldc_procinicmeta: Modificado
                                                    * ldc_proActuTarifa: Modificado
                                                    * ldc_procnormalizados: Modificado                                                    
  *******************************************************************************************************************************************************/
...
create or replace PACKAGE BODY ADM_PERSON.LDC_PKG_CALC_GEST_CARTERA IS

    csbSP_NAME      CONSTANT VARCHAR2(35)	:= $$PLSQL_UNIT||'.';
    csbNivelTraza   CONSTANT NUMBER(2)      := pkg_traza.fnuNivelTrzDef;
    csbVersion      VARCHAR2(10)            := 'OSF-3578';
    
    gnuTSess         NUMBER                  := PKG_SESSION.fnuGetSesion; 
    gsbParUser       VARCHAR2(30)            := PKG_SESSION.GetUser;    

    sbTt_Gc_Cartera		ld_parameter.value_chain%type	:=	pkg_BCLD_Parameter.fsbObtieneValorCadena('TT_GC_CARTERA');

    --Obtenemos las posibles causales de legalización de las ordenes de gestión de cartera
    nuCauProdR NUMBER := pkg_BCLD_Parameter.fnuObtieneValorNumerico('CAUSAL_USUARECU');

    nuCauProdNR NUMBER := pkg_BCLD_Parameter.fnuObtieneValorNumerico('CAUSAL_USUANREC');
    ...
    ***************************************************************************/    
    ...
    /******************************************************************************************************************************************************
	  ...
	  15/08/2023	OSF-1389	jerazomvm			Se reemplaza la separación de cadena LDC_BOUTILITIES.SPLITSTRINGS por 
      ...
      14/11/2024    OSF-3578    jpinedc			    * cudatos: Se agrega cantidad_contratos
                                                    * insert ldc_metas_cont_gestcobr: Se agrega cantidad_contratos
                                                    * cuCantContUnidOper: Cursor que se utiliza para actualizar la
                                                    cantidad de contratos en ldc_metas_cont_gestcobr sin
                                                    tener en cuentra los grupos de tipos de productos
    *******************************************************************************************************************************************************/
        ...
        IS
          SELECT o.OPERATING_UNIT_ID unidad_operativa,
                 Count(distinct x.subscription_id) cantidad_contratos
            FROM OR_ORDER             o,
                 GC_COLL_MGMT_PRO_DET x,
                 ((SELECT to_number(regexp_substr(sbTt_Gc_Cartera,
                                    '[^,]+',
  ...
  BEGIN
        ...
        pkg_Traza.trace('Eliminando datos en ldc_metas_cont_gestcobr donde el año es: ' || nupaanoact || ' y el mes: ' || nupamesact, csbNivelTraza);
        ...
        -- Se actualiza la cantidad de contratos
        FOR rgCantContUnidOper IN cuCantContUnidOper(dtpafein,dtpafefi) 
        LOOP
        
            UPDATE ldc_metas_cont_gestcobr
            SET cantidad_contratos = rgCantContUnidOper.cantidad_contratos
            WHERE  ano = nupaanoact            
            AND mes = nupamesact
            AND unidad_operativa = rgCantContUnidOper.unidad_operativa;
    ...
    *****************************************************************************************************************************************************/
        ...
        sbmensa    VARCHAR(100);
        sbCalMet   VARCHAR2(100) := pkg_BCLD_Parameter.fsbObtieneValorCadena('FORMA_CALC_META'); --CA 200-2704
        nuCanDec   NUMBER := pkg_BCLD_Parameter.fnuObtieneValorNumerico('PARAM_NUM_DECIMALES');
        nuMetaUsu  NUMBER;
        nuMetaCart NUMBER;
        nuGrupCat  NUMBER;
            ...
            IF (UPPER(sbCalMet) = 'A') THEN
                ...
                SET s.meta_usuarios       =
                     (i.producto) + ROUND((i.producto *
                                          (pkg_BCLD_Parameter.fnuObtieneValorNumerico('PARAM_PORC_META_GESTCOBR') / 100)),
                                          nuCanDec),
    ...
    **************************************************************************************************************************************/
	...
	15/08/2023		OSF-1389	jerazomvm			Se reemplaza la separación de cadena LDC_BOUTILITIES.SPLITSTRINGS por 
													...
													FROM dual
													CONNECT BY regexp_substr(variable, '[^,]+', 1, LEVEL) IS NOT NULL
    10/12/2024		OSF-3578	jpinedc             Se modifica cuDetaProd			
    **************************************************************************************************************************************/
        ...
        nuEdmode NUMBER := pkg_BCLD_Parameter.fnuObtieneValorNumerico('EDAD_MORA_DEU_NORMA'); ---- caso:846
            ...
            FROM (             
                        ...
                        ,
                                    ...
                                    (
                                        ...
                                        (
                                            ...
                                            (
                                                SELECT '1'
                                                FROM cuencobr cci
                                                WHERE cci.cuconuse = producto
                                                AND cci.cucocodi > 0
                                                AND trunc( sysdate ) - trunc(cci.cucofeve) >  60               
                                                AND (cci.cucovato - nvl(cci.cucovaab,0) - nvl(cci.cucovare,0) - nvl(cci.cucovrap,0)) > 0
                                            )
                                            AND ldc_pkg_calc_gest_cartera.fnugetRecaudo( producto,Finicio, Ffin ) > 0
                                        ) 
                                    )                                    
                                ),
                         ...
                         NVL((SELECT nvl(r.deuda_no_corriente, 0) +
                              ...
                              WHERE nuano = decode((numesact-1),0,(nuanoact-1),nuanoact) -- caso:846
                                AND numes = decode((numesact-1),0,12,(numesact-1)) -- caso:846
                                AND producto = x.product_id
                   ...
                   and l.nuano = decode((numesact-1),0,(nuanoact-1),nuanoact)
                   and l.numes = decode((numesact-1),0,12,(numesact-1))
                     --
                  )
  ...
  PROCEDURE ldc_proActuTarifa(nuanoact IN NUMBER,
    ...
    /*************************************************************************************************************************************
     ...
     =========       =========     =========         ====================
       Fecha          Ticket         Autor               Modificacion
     =========       =========     =========         ====================
     14/11/2024     OSF-3578        jpinedc         * Se usa cantidad_contratos para
                                                    buscar las tarifas en cuObtenerTarifa
    **************************************************************************************************************************************/
        ...
        (
            ...
            m.GRUPO,
            m.grupo_categoria,
            m.cantidad_contratos,
            m.usuarios_recuperados,
            m.TOTAL_RECAUDADO,
            CASE
                WHEN m.porc_cumpl_usua > m.porc_cumpl_dinero THEN
                    m.porc_cumpl_usua
                ELSE
                    m.porc_cumpl_dinero
            END porc
            FROM ldc_metas_cont_gestcobr m, LDC_GRUPGECA g
            WHERE m.ano = nuanoact
            AND m.mes = numesact
        ...
        AND SYSDATE BETWEEN t.fecha_inicial_vig AND t.fecha_final_vig
        AND t.tipo_tarifa = 'U'
        AND u.cantidad_contratos BETWEEN t.rango_inicial AND t.rango_final
        ...
        t.tipo_tarifa = 'P' AND
        u.cantidad_contratos BETWEEN t.rango_inicial AND t.rango_final AND 
        u.porc BETWEEN t.rango_inicump AND t.rango_fincump;
    ...
    BEGIN
        ...
        FOR reg IN cuObtenerTarifa LOOP
            ...
            pkg_Traza.trace('nuGrupo|' || nuGrupo, csbNivelTraza);
            ...
            --se actualiza el campo de total con el campo nuTotal
                ...
                UPDATE ldc_metas_cont_gestcobr
                 ...
                 WHERE ANO = nuanoact
                   ...
                   AND GRUPO = reg.grupo;
                nuTotal := 0;
                pkg_Traza.trace('Saliendo del total general ' || nuTotal, csbNivelTraza);
            END IF;
  
            pkg_Traza.trace('reg.tipo_tarifa|' || reg.tipo_tarifa, csbNivelTraza);
    ...
    **************************************************************************************************************************************/
        ...
        nuPersonaLega NUMBER := pkg_BCLD_Parameter.fnuObtieneValorNumerico('PERSON_LEGA_ORDCART');
        nuCauLiq      NUMBER := pkg_BCLD_Parameter.fnuObtieneValorNumerico('LDC_CAUSAL_LIQUIDACION');
                                                                         
        --Obtenemos las posibles causales de legalización de las ordenes de gestión de cartera
        sbDirGen       VARCHAR(4000) := pkg_BCLD_Parameter.fnuObtieneValorNumerico('LDC_DIR_GENERICA');
        nuActiGas      NUMBER := pkg_BCLD_Parameter.fnuObtieneValorNumerico('LDC_ACTIVIDAD_GAS');
        nuActiBrilla   NUMBER := pkg_BCLD_Parameter.fnuObtieneValorNumerico('LDC_ACTIVIDAD_BRILLA');
        nuactividadgen NUMBER;
  
        --Obtenemos los items de legalizacion de los tipos de producto
        nuItemIdUsu  NUMBER := pkg_BCLD_Parameter.fnuObtieneValorNumerico('ITEM_USUARIOS');
        nuItemIdCart NUMBER := pkg_BCLD_Parameter.fnuObtieneValorNumerico('ITEM_CARTERA');
        ...
        --Cursor de ordenes de gestión de cartera recuperadas
    ...
    **************************************************************************************************************************************/
    ...
    BEGIN
        ...
        --Se limpia las tablas temporales del proceso de optimización.(opt)
        ...
        pkg_Traza.Trace('sbdtpafein|' || sbdtpafein , csbNivelTraza); 
          
        sbdtpafefi := TO_CHAR(dtpafefi, 'dd/mm/yyyy HH24:MI:SS');

        pkg_Traza.Trace('sbdtpafefi|' || sbdtpafefi , csbNivelTraza);
  
        pkg_Traza.Trace('TO_CHAR(gNUTSESS)|' || TO_CHAR(gNUTSESS) , csbNivelTraza);

        pkg_Traza.Trace('TO_CHAR(NUANOACT)|' || TO_CHAR(NUANOACT) , csbNivelTraza);

        pkg_Traza.Trace('TO_CHAR(NUMESACT)|' || TO_CHAR(NUMESACT), csbNivelTraza);
        ...
        FOR I IN 0 .. 9 LOOP
            ...
            pkg_Traza.Trace('hIlo|' || I, csbNivelTraza);
            ...
            pkg_Traza.Trace('sbWhat|' || sbWhat, csbNivelTraza);
  ...
  PROCEDURE ldc_legaordergestcart_hilos(INUHILO    NUMBER,
    ...
    30/11/2023		OSF-1918	  jpinedc			Se modifica cuOrderR agregando con union all cursor para productos
                                                    con edad_catera mayor a 90 días, con reclamos pendiente por valor total
                                                    y sin cuentas con saldo posteriores con saldo exceptuando las no
                                                    vencidas
    08/08/2023		OSF-1918	  jpinedc			Se modifica cuOrderR  agregando minCucoSaPe
                                                    para calcular la cuenta de cobro con saldo más antigua
    21/08/2023		OSF-1918	  jpinedc			Se modifica cuOrderR  quitando minCucoSaPe
    **************************************************************************************************************************************/
        csbMetodo        CONSTANT VARCHAR2(70) := csbSP_NAME || 'ldc_legaordergestcart';
        ...
        --Cursor para obtener la persona que está legalizando
        ...
        nuEdadMora    NUMBER := pkg_BCLD_Parameter.fnuObtieneValorNumerico('PARAM_EDAD_MORA');

        nuPersonaLega NUMBER := pkg_BCLD_Parameter.fnuObtieneValorNumerico('PERSON_LEGA_ORDCART');
  
        sbCadenaLegalizacion    CONSTANTS_PER.TIPO_XML_SOL%TYPE;

        --Cursor de ordenes de gestión de cartera recuperadas -(Opt. Con hilos)
        CURSOR cuOrderR(dtcufein DATE, dtcufefi DATE, idtFinicio DATE, idtFin DATE  ) IS         
        WITH minCuCoNoVenc AS
            (
          ...
          SELECT o.order_id
           ...
           WHERE o.task_type_id = tt.columna
            ...
            (
                ...
                AND cci.cucocodi > 0 
                AND trunc( sysdate ) - trunc(cci.cucofeve) >  nuEdadMora               
                AND (cci.cucovato - nvl(cci.cucovaab,0) - nvl(cci.cucovare,0) - nvl(cci.cucovrap,0)) > 0
            )
            AND fnugetRecaudo( pr.product_id,idtFinicio, idtFin ) > 0;       
  
        --Cursor de ordenes de gestión de cartera no recuperadas
    ...
    BEGIN
        ...
        FOR i IN cuOrderR(dtpafein, dtpafefi, Finicio,Ffin ) LOOP
...
BEGIN
    pkg_utilidades.prAplicarPermisos('LDC_PKG_CALC_GEST_CARTERA', 'ADM_PERSON');
END;
/
