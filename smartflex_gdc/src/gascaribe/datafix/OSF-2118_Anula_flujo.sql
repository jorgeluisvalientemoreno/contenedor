...
declare
    cursor cuSolicitudes is
        ...
        WHERE p.package_id IN (207887973)
...
BEGIN
	...
	dbms_output.put_line('Inicia OSF-2118 - anula flujo');
	
	BEGIN
		nuPlanId := wf_boinstance.fnugetplanid(207887973, 17);
	EXCEPTION
	WHEN OTHERS THEN
		rollback;
		nuError:=1;
		dbms_output.put_line('Error anulando plan solicitud|'||207887973||'|'||sqlerrm);
	END;
	...
	OPEN cuErrorFlujo(207887973);
	FETCH cuErrorFlujo into rgErrorFlujo;

	IF cuErrorFlujo%notfound THEN
		COMMIT;
		dbms_output.put_line('SOLICITUD ANULADA SIN ERROR DE FLUJO|'||207887973);
	ELSE
		...
		END IF;
		COMMIT;
		dbms_output.put_line('SOLICITUD ANULADA CON ERROR DE FLUJO|'||207887973);
	END IF;
	
	CLOSE cuErrorFlujo;

	dbms_output.put_line('Finaliza OSF-2118 - anula flujo');
END;
/
