...
Descripción: Borrado tabla sesuasoc y actualiza grupo de impresión a cuenta de cobro 
    ...
    --Modificaciones    
    19/06/2025 - jcatuchemvm
    Creación
    
***********************************************************/
DECLARE
    csbCaso             constant varchar2(20)   := 'OSF4618';
    csbTitulo           constant varchar2(2000) := csbCaso||': Borrado  sesuasoc';
    ...
    select s.*,s.rowid row_id from sesuasoc s where 
    ssassesu = nvl(inusesu,ssassesu);
...
BEGIN
    ...
    sbcabecera := 'Tipo|Contrato|Prodcuto|Accion';
    ...
    for rcData in cuData(null) loop
        ...
        begin    
            ...
            delete sesuasoc where rowid = rcdata.row_id;
            nuRowcount := sql%rowcount;
        
            if nuRowcount != 1 then
                sbcomentario := 'No se realizó borrado del registro '||rcdata.ssassesu||' ['||nuRowcount||']';
    ...
    update cuencobr
    set cucogrim = 99
    where cucocodi = 3085108671;
    
    nucant := sql%rowcount;
                
    dbms_output.put_line('================================================');
    dbms_output.put_line('Cantidad de registros borrados: '||nucontador);
    dbms_output.put_line('Cantidad de cuentas actualizadas: '||nucant);
