...
declare
  ...
  and     mt.subscription_id in (
                                  67264979, 67251617, 67340607, 
                                  67242547, 67340794, 67342510, 
                                  67253018, 67324764
  );

begin
  dbms_output.put_line('---- Inicio OSF-811 ----');
  ...
  LOOP
    ...
    isbOrderComme  := 'La orden ['||reg.ORDER_ID||'] pertenecia a la solicitud ['||reg.PACKAGE_ID||'] y al motivo [' ||reg.MOTIVE_ID||'] - Se desvinculan por el Caso OSF-811';
  ...
  dbms_output.put_line('---- Fin OSF-811 ----');
EXCEPTION
  WHEN OTHERS THEN
    rollback;
    dbms_output.put_line('---- Error OSF-811----');
