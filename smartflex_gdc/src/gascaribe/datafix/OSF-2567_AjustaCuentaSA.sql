...
Ajusta cuenta descuadrada 2976779174 y genera el cargo SA 
...
declare 
    ...
    where cucocodi = 2976779174
    and factcodi = cucofact;
    
    s_out   varchar2(2000);
    
    procedure ajustaCuenta is
        inucuenta cuencobr.cucocodi%type := 2976779174;
        nuError NUMBER;
...
begin
    ...
    pkAccountMgr.GenPositiveBal(2976779174);
    dbms_output.put_line('Saldos Despues de generación SA');
    dbms_output.put_line('Cuenta|Producto|Factura|Cucosacu|Cucovato|Cucovaab|Cucovafa|Cucovrap|SaldoCargos');
    for rc in cuenta loop
        s_out := rc.cucocodi;
        s_out := s_out||'|'||rc.cuconuse;
