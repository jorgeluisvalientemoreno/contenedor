...
begin
  DECLARE
  /*
      OSF-4023  retiro de productos (16-Retirado sin instalación), 
                cambio de estado de corte (110-Retirado sin instalación), 
                ...
                Contrato 67658825 solicitud 222725934
  ...
  */
      -- Informacion general
      sbComment       CONSTANT VARCHAR2(200) := 'SE ANULA ORDEN CON EL CASO OSF-4023';
  ...
  */
      ...
      CURSOR cuLoadData IS
          select person_id
          from ge_person
          where person_id = 13577; -- Angelica Zequeda   
                          --13549; -- Pablo
  ...
  BEGIN
      ...
      LOOP
            ...
            AND NOT EXISTS (select 1 
                            from open.or_order_activity oa 
                                inner join open.or_order o  ON oa.order_id = o.order_id 
                                                            AND o.order_status_id not in (8, 12)
                                left join open.ge_causal gc ON o.causal_id = gc.causal_id and nvl(gc.class_causal_id,0) <> 1
          ...
          IF (nvl(nuEstado,0) > 0) THEN
              ...
              --
          ELSE
            dbms_output.put_line('nuEstado: ' ||  nuEstado);
          END IF;        
