...
***********************************************************/
DECLARE
    csbCaso             constant varchar2(20)   := 'OSF3356';
    csbTitulo           constant varchar2(2000) := csbCaso||': Borrado registro CM_VAVAFACO con vigencias erradas';
    csbFormato          constant varchar2(25)   := 'DD/MM/YYYY HH24:MI:SS';
    cdtFecha            constant date           := sysdate;
    cnuPackage          constant number         :=  211119780;
    
    cursor cudata is
    select * from cm_vavafaco 
    where vvfccons =298564;
...
BEGIN
    ...
    begin
        ...
        for rd in cudata loop
            begin 
                ...
                if nuRowcount != 1 then
                    sbcomentario := 'Borrado de registro diferente al esperado ['||nuRowcount||']';
                    raise raise_continuar;
                end if;
                
                s_out := rd.vvfccons;
                s_out := s_out||'|'||rd.vvfcvafc;
                s_out := s_out||'|'||rd.vvfcfeiv;
                s_out := s_out||'|'||rd.vvfcfefv;
                s_out := s_out||'|'||rd.vvfcvalo;
                s_out := s_out||'|'||rd.vvfcvapr;
                s_out := s_out||'|'||rd.vvfcsesu;
            ...
            exception
                ...
                when others then
                    ...
                    sbcomentario := 'Error en borrado de registro. Error '||sberror;
                    s_out := rd.vvfccons;
                    s_out := s_out||'|'||sbcomentario;
                    dbms_output.put_line(s_out);
            end;
        ...
        if nucontador = 0 then
            dbms_output.put_line('|Sin registros a procesar');
        end if;
