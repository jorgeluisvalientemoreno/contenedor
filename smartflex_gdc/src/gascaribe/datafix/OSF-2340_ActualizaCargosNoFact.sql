...
Actualiza cargos liquidados sin facturación por cambio de ciclo antes del cierre
    ...
    --Modificaciones    
    14/02/2024 - jcatuchemvm
    Creación
    
***********************************************************/
declare
    cnuCuenta   number := 3055663998;
    cnuProducto number := 50261610;
    cnuPefa     number := 108285;
    cnuPecs     number := 108260;
    cnuPefaAnt  number := 108290;
...
begin
    dbms_output.put_line('OSF-2340: Actualización de cargo sin facturar');
    ...
    begin
        ...
        for rc in cudata loop
            update cargos
            set cargcuco = cnuCuenta,cargpeco = rc.pecscons
        ...
        if nucontador = 0  then
            sbcomentario  := cnuProducto||'|'||cnuCuenta||'||Sin cargos para actualizar|NA';
            raise raise_continuar;
        end  if;
        
        update conssesu 
        set cosspefa = cnuPefa, cosspecs = cnuPecs
        where cosssesu = cnuProducto and cosspefa = cnuPefaAnt;
        
        nuRowcount := sql%rowcount;
        if nuRowcount <= 0 then
            sbcomentario :=  cnuProducto||'|'||cnuCuenta||'||No se realizó la actualización de periodos en conssesu|NA';
            raise raise_continuar;
        else
        ...
        where leemsesu = cnuProducto and leempefa = cnuPefaAnt;
        
        nuRowcount := sql%rowcount;
        if nuRowcount <= 0 then
            sbcomentario :=  cnuProducto||'|'||cnuCuenta||'||No se realizó la actualización de periodos en lectelme|NA';
            raise raise_continuar;
        else
    ...
    exception
        ...
        when pkg_error.CONTROLLED_ERROR then
            ...
            rollback;
            nuerr := nuerr + 1;
            sbcomentario := cnuProducto||'|'||cnuCuenta||'||Error en ajusta cuenta|'||sbError;
            dbms_output.put_line(sbcomentario);
    end;
