...
declare
  cursor cuSolicitudes is
        ...
        WHERE p.package_id IN (10660465)
		AND p2.motive_status_id = 13
        ORDER BY  p2.package_id;

  sbComment VARCHAR2(4000) := 'Se cambia estado a anulado por OSF-2817';
  ...
  CURSOR cuOrderActiv is
    ...
    where oa.package_id = 10660465--cnuPackage_id 
    and nvl(gc.class_causal_id,0) != 1 
    and o.order_status_id  in (12)
    and rownum= 1;

BEGIN
  dbms_output.put_line('Inicia datafix OSF-2817');
  FOR reg IN cuSolicitudes LOOP
    nuError := 0;
    IF nuError = 0 THEN
      --1 VALIDACION
      BEGIN
        dbms_output.put_line('Cambia Transición OSF-2817');
        ldc_pkg_changstatesolici.packageinttransition(reg.package_id,
                                                      ge_boparameter.fnuget('ANNUL_CAUSAL'));
        dbms_output.put_line('Termina Cambia Transición OSF-2817');
      EXCEPTION
        WHEN OTHERS THEN
          rollback;
          nuError := 0;
      ...
      FOR regOt IN cuOrdenes(reg.package_id) LOOP
        BEGIN
          dbms_output.put_line('Anula ordenes OSF-2817');
          ...
          dbms_output.put_line('Termina ordenes OSF-2817');
        EXCEPTION
          WHEN OTHERS THEN
            nuError := 1;
            rollback;
  ...
  END LOOP;
    ...
    COMMIT;
  dbms_output.put_line('Fin datafix OSF-2817');
END;
/
