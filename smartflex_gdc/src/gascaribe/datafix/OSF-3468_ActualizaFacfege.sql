...
Descripción
    ...
    --Modificaciones    
    11/10/2024 - jcatuchemvm
    Creación
    
***********************************************************/
DECLARE
    csbCaso             constant varchar2(20)   := 'OSF3468';
    csbTitulo           constant varchar2(2000) := csbCaso||': Actualización factfege al 11/10/2024';
    csbFormato          constant varchar2(25)   := 'DD/MM/YYYY HH24:MI:SS';
    cdtFecha            constant date           := sysdate;
    
    cursor cudata is
    select factcodi,factfege,factfege+(to_date('11102024','ddmmyyyy')-trunc(factfege)) factfege_
    from factura
    where factcodi in 
    (
    2139860223,
...
BEGIN
    ...
    sbcabecera := 'Factura|Factfege_ant|Factfege_act|Error';
    ...
    begin
        ...
        for rc in cudata loop
            begin 
                ...
                s_out := rc.factcodi;
                s_out := s_out||'|'||rc.factfege;
                
                if trunc(rc.factfege) = trunc(rc.factfege_) then
                    sbcomentario := 'La fecha de la factura ya se encuentra actualizada ['||rc.factfege||']';
                    raise raise_continuar;
                end if;
                
                update factura c
                set factfege = rc.factfege_
                where factcodi = rc.factcodi;
                
                nuRowcount := sql%rowcount;
                
                if nuRowcount != 1 then
                    sbcomentario := 'Actualización de fecha de factura diferente a la esperada ['||nuRowcount||']';
                    raise raise_continuar;
                end if;
                
                s_out := s_out||'|'||rc.factfege_;
                s_out := s_out||'|'||'Ok';
