...
declare
  cursor cuSolicitudes is
        ...
        WHERE p.package_id IN (62121796,63112009)
		AND p2.motive_status_id = 13
        ORDER BY  p2.package_id;

  sbComment VARCHAR2(4000) := 'Se cambia estado a anulado por OSF-1041';
    ...
    IF nuError = 0 THEN
      ...
      IF nuError = 0 THEN
        ...
        IF nuError = 0 THEN
          ...
          EXCEPTION
          WHEN OTHERS THEN
              dbms_output.put_line('Error anulando error de flujo|' ||
                                   reg.package_id || '|' || onuErrorCode||' : '||osbErrorMessage);
          END;
        END IF;
      END IF;
    END IF; 
