DECLARE
	...
	CURSOR cuInfoCuenta IS
		...
		where mm.product_id in (select sesunuse 
								from open.servsusc 
								where sesususc in (67199565, 67262750)) 
...
begin
  dbms_output.put_line('Inicia Anulación de contratos');
  ...
  FOR reg IN cuInfoCuenta LOOP
	...
	dbms_output.put_line('Actualizando el estado, fecha de anulación, causal de anulación de mo_motive, del motivo: ' || reg.motive_id);
	...
	OS_ADDORDERCOMMENT(reg.order_id,
                       nuCommentType,
                       'Se anula por solicitud en el caso OSF-1250',
                       nuerror,
                       sberror);
  ...
  dbms_output.put_line('Termina Anulación de contratos');
