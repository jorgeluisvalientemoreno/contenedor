...
***********************************************************/
DECLARE
    csbCaso             constant varchar2(20)   := 'OSF3737';
    csbTitulo           constant varchar2(2000) := csbCaso||': Atención de soliicutudes de reconexión';
    ...
    with base as
    (
        SELECT P.PACKAGE_ID, P.cust_care_reques_num,A.PRODUCT_ID, P.REQUEST_DATE, P.MOTIVE_STATUS_ID, O.ORDER_ID, O.LEGALIZATION_DATE, o.EXECUTION_FINAL_DATE,O.TASK_TYPE_ID, O.ORDER_STATUS_ID, P.PACKAGE_TYPE_ID,
        (
    ...
    MOTIVE_STATUS_ID,ORDER_ID,LEGALIZATION_DATE,EXECUTION_FINAL_DATE,TASK_TYPE_ID,ORDER_STATUS_ID,PACKAGE_TYPE_ID,SUCONUOR,SUCOFEAT,SOL_ALTERNA,ESTADO_ALTERNA,ERRORID,ERROR
    from agrupa a
    where package_id in 
    (
    209144394,
    ...
    procedure pcancelasusp (inuprod in number,idtfecha in date, onuerror out number, osberror out varchar2) 
    is
    ...
    begin
        ...
        if rcsuspension.product_id is not null and rcsuspension.inactive_date is null then
            if rcsuspension.aplication_date < idtfecha then
                dtfecha := idtfecha;
            else
                dtfecha := sysdate;
            end if;
            ...
            set inactive_date = dtfecha, active = 'N'
        ...
        for rc in cudata loop
            ...
            begin 
                ...
                if rc.sol_alterna is not null and rc.estado_alterna = 14 then
                    ...
                    update suspcone
                    set sucofeat = rc.execution_final_Date
                    where suconuor = rc.suconuor;
                    
                    if rc.sesuesco like '6-%' and rc.valida_susp = 'Menor' then
                        pcancelasusp(rc.product_id,rc.execution_final_Date,nuerror,sberror);
                        if nuerror != 0 then
                            raise raise_continuar;
                        end if;
