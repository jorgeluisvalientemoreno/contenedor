...
***********************************************************/
DECLARE
    csbCaso             constant varchar2(20)   := 'OSF2651';
    csbTitulo           constant varchar2(2000) := csbCaso||': Actualiza FLAG LDC_PECOFACT';
    csbFormato          constant varchar2(25)   := 'DD/MM/YYYY HH24:MI:SS';
    cdtFecha            constant date           := sysdate;
    
    
    cursor cudata is
    SELECT rowid ID_REG,  c.PCFAPEFA periodo,pcfaesta,pcfafere,pcfafepr
    FROM LDC_PECOFACT c
    WHERE 1 = 1
    AND pcfapefa in(111277 , 111278);
...
BEGIN
    ...
    sbcabecera := 'Periodo|Fechaproceso|Estado_ant|EStado_act|Gestion';
    ...
    begin
        ...
        for rc in cudata loop
            ...
            SET PCFAESTA = 'N'
            WHERE pcfapefa = rc.periodo
            and rowid = rc.id_reg;
            
            nuRowcount := sql%rowcount;
        
            if nuRowcount != 1 then
                sbcomentario := 'No se realizó actualización del LDC_PECOFACT ['||nuRowcount||']';
                sbesta := rc.pcfaesta;
                nuerr := nuerr + 1;
                rollback;
            else
            ...
            s_out := rc.periodo;
            s_out := s_out||'|'||rc.pcfafere;
            s_out := s_out||'|'||rc.pcfaesta;
            s_out := s_out||'|'||sbesta;
    ...
    dbms_output.put_line('Cantidad de flag actualizados: '||nuok);
