...
CURSOR CUDATOS IS
    ...
    A.TRANSIT_IN,
    A.TRANSIT_OUT,
    CASE WHEN (I.ITEMS_ID LIKE '4%' OR I.ITEMS_ID IN (100003008, 100003011)) AND act.balance IS NULL AND inv.balance IS NULL THEN 'S1'
         WHEN (I.ITEMS_ID LIKE '4%' OR I.ITEMS_ID IN (100003008, 100003011)) AND (act.balance IS NOT NULL AND inv.balance IS NOT NULL) THEN 'N1'
         WHEN es_externa ='Y' and  i.item_classif_id=21 and a.total_costs=0 and i.items_id not in (10011198) AND act.balance IS NULL AND inv.balance IS NULL THEN 'S2'
         WHEN es_externa ='Y' and  i.item_classif_id=21 and a.total_costs=0 and i.items_id not in (10011198) AND (act.balance IS NOT  NULL OR inv.balance IS NOT NULL) THEN 'S2'
         WHEN (I.ITEMS_ID in (10011198)) AND (A.BALANCE-NVL(ACT.BALANCE,0)-NVL(INV.BALANCE,0)=0) THEN 'S3'
         WHEN (I.ITEMS_ID in (10011198)) AND (A.BALANCE-NVL(ACT.BALANCE,0)-NVL(INV.BALANCE,0)!=0) THEN  'N3'
    ...
    inner join open.or_operating_unit u on a.operating_unit_id=u.operating_unit_id
...
BEGIN
  DELETE OPEN.ldc_act_ouib b  where b.operating_unit_id in (3499, 3506, 3954, 3551, 3507 , 3955, 3557, 3508, 3592) and exists(select null from open.or_ope_uni_item_bala a where a.items_id=b.items_id and a.operating_unit_id=b.operating_unit_id and a.balance>0);
  DELETE OPEN.ldc_inv_ouib b  where b.operating_unit_id in (3499, 3506, 3954, 3551, 3507 , 3955, 3557, 3508, 3592) and exists(select null from open.or_ope_uni_item_bala a where a.items_id=b.items_id and a.operating_unit_id=b.operating_unit_id and a.balance>0);
