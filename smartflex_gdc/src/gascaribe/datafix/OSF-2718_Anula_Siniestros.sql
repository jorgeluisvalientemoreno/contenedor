...
Adecuación script de anulación proporcionado por Edmundo Lara
    ...
    --Modificaciones    
    10/05/2023 - jcatuchemvm
    Se añade validacion cartram para actualización cuencobr.cucovare.
    Se agrega validación para caducar periodos de gracia registrados con la solicitud
    05/04/2023 - jcatuchemvm
    Creación
    ----------
    17/05/2024 - GDGuevara  - OSF-2712 Error registro siniestro
    Se adiciona la insercion del log del cambio de estado de la solicitud 
    en la tabla MO_PACKAGE_CHNG_LOG

***********************************************************/
declare
    --Constantes para el datafix
    csbCaso         constant varchar2(20)   := 'OSF-2712';
    cnuPackage_id1  constant MO_WF_PACK_INTERFAC.package_id%TYPE := 211838631;
    cnuPackage_id2  constant MO_WF_PACK_INTERFAC.package_id%TYPE := 211838631;
    ...
    (
        ...
        cant_ordenes        number,
        tipo_orden          or_task_type.task_type_id%type,
        estadoliq           ldc_liqsinibridet.status%type, 
    ...
    (
        select max(task_type_id) 
        from or_order_activity a
        where a.package_id = p.package_id
        and a.status = 'R'
    ) tipo_orden,
    (
        select min(status) from LDC_LIQSINIBRIDET l where l.package_id = p.package_id
    ) estadoLiq
    ...
    BEGIN
        ...
        sbCabecera := 'Solicitud|TipoSol|FechaSol|Motivo|Contrato|Cliente|Producto|TipoProd|Cant_Ordenes|TipoOrden|EstadoLiq|Estado_ant|Estado_act|Actualizado';
    ...
    BEGIN
        ...
        pGuardaLog(tbArchivos(cnuIdErr),'Total de Periodos de gracia caducados : '||nuGrac);
    ...
    begin
        ...
        s_out := s_out||'|'||ircRegistro.tipo_orden;
        s_out := s_out||'|'||ircRegistro.estadoliq;
    ...
    PROCEDURE pActualizaDatos(ircRegistro in out tyrcRegistro) is
        ...
        numoti          cargtram.catrmoti%type;
        
        /*cursor para obtener la información de la liquidación y su detalle*/
        cursor culiquisini(inuPackage number, inuSuscripc number) is
        select t.factcodi,
             t.cucocodi,
             t.difecodi,
             t.ano,
        ...
        where l.package_id = t.package_id
         ...
         and l.suscription_id = inuSuscripc
         and l.liquidation_id = t.liquidation_id
         and t.difecodi is not null;
        
        nuGracePeriod   cc_grace_peri_defe.grace_period_id%type;
        nuperigracdef   cc_grace_peri_defe.grace_peri_defe_id%type;
        vinidate        cc_grace_peri_defe.initial_date%type;
        --
        nuTotCuco               number;
    ...
    begin
        ...
        --Periodos de Gracia
        if ircRegistro.tipo_orden = 10148 and ircRegistro.estadoliq = 'R' then
            ...
            if ((nvl(nuGracePeriod, LD_BOConstans.cnuCero) <> LD_BOConstans.cnuCero)) then
                ...
                for rgliqu in culiquisini(ircRegistro.solicitud, ircRegistro.contrato) loop
                
                    nuperigracdef := LDC_PKCRMTRAMISEGBRI.Fnuconsperigracdif(rgliqu.difecodi);
                    vinidate := dacc_grace_peri_defe.fdtgetinitial_date(nuperigracdef);
                    
                    if dacc_grace_peri_defe.fdtgetend_date(nuperigracdef) > UT_Date.fdtSysdate then
                    
                        LDC_PKCRMTRAMISEGBRI.PROCANCELPERIGRACXDIFE
                        (
                            rgliqu.difecodi,
                            nuperigracdef,
                            nuGracePeriod,
        ...
        -- Valores en reclamo
        open CuCargtram(ircRegistro.motivo);
        fetch CuCargtram into numoti;
        
        if CuCargtram%NOTFOUND then
            nuTotCuco := 0;
            update CUENCOBR CO
            set CO.CUCOVARE = 0
            where CO.CUCONUSE = ircRegistro.producto
            and CO.CUCOVARE <> 0
            RETURNING COUNT(1) INTO nuTotCuco;
            IF nuTotCuco > 0 THEN
                dbms_output.put_line(nuTotCuco||' Cuentas de cobro actualizadas en CUENCOBR del Producto '||ircRegistro.producto);
            END IF;
        end if;  
        close CuCargtram;
            
        package_idvar           := nuSolicitud;
        cust_care_reques_numvar := damo_packages.fsbgetcust_care_reques_num(nusolicitud,null);
        package_type_idvar      := damo_packages.fnugetpackage_type_id(nusolicitud,null);
        o_motive_status_idvar   := damo_motive.fnugetmotive_status_id(mo_bcpackages.fnugetmotiveid(nusolicitud));
        ...
        dbms_output.put_line('Registro Insertado en MO_PACKAGE_CHNG_LOG para la solicitud: '||nuSolicitud);        
            ...
            for i in 1..tbPrincipal.count loop 
                begin
                    ...
                    if not tbRegistro.exists(sbHash) then
                        ...
                        tbRegistro(sbHash).fechasol     := tbPrincipal(i).request_date;
                        tbRegistro(sbHash).motivo       := tbPrincipal(i).motivo;
                        ...
                        tbRegistro(sbHash).tipo_orden   := tbPrincipal(i).tipo_orden;
                        tbRegistro(sbHash).estadoliq    := tbPrincipal(i).estadoliq;
