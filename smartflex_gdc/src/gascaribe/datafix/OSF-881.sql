...
declare
  ...
  and     mt.subscription_id in (67354054);

begin
  dbms_output.put_line('---- Inicio OSF-881 ----');
  ...
  LOOP
    ...
    isbOrderComme  := 'La orden ['||reg.ORDER_ID||'] pertenecia a la solicitud ['||reg.PACKAGE_ID||'] y al motivo [' ||reg.MOTIVE_ID||'] - Se desvinculan por el Caso OSF-881';
  ...
  dbms_output.put_line('---- Fin OSF-881 ----');
EXCEPTION
  WHEN OTHERS THEN
    rollback;
    dbms_output.put_line('---- Error OSF-881----');
