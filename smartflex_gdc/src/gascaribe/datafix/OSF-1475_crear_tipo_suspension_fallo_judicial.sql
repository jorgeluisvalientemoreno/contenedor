...
begin
  dbms_output.put_line('---- Inicio OSF-1475 ----');

  BEGIN

    MERGE INTO OPEN.GE_SUSPENSION_TYPE A USING
    (SELECT
      109 as SUSPENSION_TYPE_ID,
      'BI' as DIRECTIONALITY_ID,
      'SUSPENSION POR FALLO JUDICIAL' as DESCRIPTION,
      1 as PRIORITY,
      'Y' as ANALIZE,
    ...
    COMMIT;
    blInsertOK := true;
    dbms_output.put_line('OK - SE CREO TIPO SUSPENSION POR FALLO JUDICIAL');
  EXCEPTION
    WHEN OTHERS THEN
      rollback;
  ...
  IF (blInsertOK) THEN
    BEGIN
        ...
        COMMIT;
        dbms_output.put_line('OK - CREACION DE RELACION ENTRE SERVICIO 7014 Y NUEVO TIPO SUSPENSION POR FALLO JUDICIAL 109');
    EXCEPTION
      WHEN OTHERS THEN
        rollback;
  ...
  dbms_output.put_line('---- Fin OSF-1475 ----');
EXCEPTION
  WHEN OTHERS THEN
    rollback;
    dbms_output.put_line('---- Error OSF-1475 ----');
