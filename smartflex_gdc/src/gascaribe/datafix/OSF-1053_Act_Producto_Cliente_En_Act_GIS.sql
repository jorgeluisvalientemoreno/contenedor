...
DECLARE
	...
	SELECT oa.order_id, 
			case 
				when oa.order_id = 280859786 then  
					51447564 
				when oa.order_id = 277519574 then 
					51910534
			end ProdOk, 
			oa.package_id Soli,
	...
	mo_packages pk
	WHERE oa.ORDER_ID IN ( 280859786 , 277519574 )
	AND pk.package_id(+) = oa.package_id;
	
	CURSOR cuDatosOk( inuProdOk NUMBER)
	IS
	SELECT sc.susccodi ContOk, sc.suscclie ClieOk, sr.subscriber_name NombOk
    FROM servsusc ss, 
         suscripc sc,
         ge_subscriber sr
    ...
    PROCEDURE pUpdOr_Order_Activity( inuOrden NUMBER, inuProdOk NUMBER, inuClieOK NUMBER)
    IS
    ...
    BEGIN
        ...
        pFOTO( 'OR_ORDER_ACTIVITY', 'ORDER_ACTIVITY_ID,ORDER_ID,PRODUCT_ID,SUBSCRIBER_ID', tbRowIds, 'ANTES', TRUE );
        ...
        pFOTO( 'OR_ORDER_ACTIVITY', 'ORDER_ACTIVITY_ID,ORDER_ID,PRODUCT_ID,SUBSCRIBER_ID', tbRowIds, 'DESPU', FALSE );

        --dbms_output.put_line('Termina pUpdOr_Order_Activity' );
        
    END pUpdOr_Order_Activity;
    
    PROCEDURE pUpdOr_Extern_Systems_Id( inuOrden NUMBER, inuProdOk NUMBER, inuContOk NUMBER, inuClieOk NUMBER, isbNombOK VARCHAR2, inuSoli NUMBER, inuTiSo NUMBER)
    IS
    ...
    BEGIN
        ...
        pFOTO( 'OR_EXTERN_SYSTEMS_ID', 'ORDER_ID,PRODUCT_ID,SUBSCRIPTION_ID,SUBSCRIBER_ID,SUBSCRIBER_NAME,PACKAGE_ID,PACKAGE_TYPE_ID', tbRowIds, 'ANTES', TRUE );
        ...
        pFOTO( 'OR_EXTERN_SYSTEMS_ID', 'ORDER_ID,PRODUCT_ID,SUBSCRIPTION_ID,SUBSCRIBER_ID,SUBSCRIBER_NAME,PACKAGE_ID,PACKAGE_TYPE_ID', tbRowIds, 'DESPU', FALSE );

        --dbms_output.put_line('Termina pUpdOr_Order_Activity' );
        
    END pUpdOr_Extern_Systems_Id;    
    
    PROCEDURE pProcOrdenes( inuOrden NUMBER, inuProdOk NUMBER, inuContOk NUMBER, inuClieOk NUMBER, isbNombOk VARCHAR2, inuSoli NUMBER, inuTiSo NUMBER)
    IS
    
        onuCodError     NUMBER;
        osbMensError    VARCHAR2(4000);
		
		isbOrderComme varchar2(4000) := 'Se actauliza el producto y el cliente por OSF-1053';
    ...
    BEGIN
        ...
        pUpdOr_Order_Activity( inuOrden, inuProdOk, inuClieOk);

        pUpdOr_Extern_Systems_Id( inuOrden, inuProdOk, inuContOk, inuClieOk, isbNombOk, inuSoli, inuTiSo);
...
BEGIN
	...
	FOR rgOrd IN cuOrdenes LOOP
        ...
        FOR rgDatosOk IN cuDatosOk( rgOrd.ProdOk ) LOOP
            ...
            pProcOrdenes ( rgOrd.order_id, rgOrd.ProdOk,  rgDatosOk.ContOk, rgDatosOk.ClieOk, rgDatosOk.NombOk, rgOrd.Soli, rgOrd.TiSo );
