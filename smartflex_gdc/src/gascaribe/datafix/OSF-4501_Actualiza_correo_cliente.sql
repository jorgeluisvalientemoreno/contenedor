...
declare
    ...
    CURSOR cuClientesConError
    IS
    SELECT subscriber_id, REPLACE(e_mail, CHR(31)) AS e_mail_corregido, e_mail
    FROM ge_subscriber
    WHERE e_mail  like '%'||chr(31)||'%';
begin
    dtfecha := sysdate;
    nuok := 0;
    nuerr := 0;
    ...
    dbms_output.put_line('OSF-4501: Reprocesar Solicitudes');
    dbms_output.put_line('');
    dbms_output.put_line('Actualización estado MO_EXECUTOR_LOG_MOT');
    dbms_output.put_line('=======================================================');
    dbms_output.put_line(sbcabecera);

    execute immediate 'alter trigger TRG_AU_AI_GE_SUBSCRI_116_208 disable';

    -- Se actualizan los clientes con error en el correo
    FOR rcDataCorreos IN cuClientesConError LOOP
      dbms_output.put_line('subscriber_id : '||rcDataCorreos.subscriber_id);
      dbms_output.put_line('e_mail anterior: '||rcDataCorreos.e_mail);
      dbms_output.put_line('e_mail corregido: '||rcDataCorreos.e_mail_corregido);
      UPDATE ge_subscriber SET e_mail = rcDataCorreos.e_mail_corregido WHERE subscriber_id = rcDataCorreos.subscriber_id;
    END LOOP;
    ...
    execute immediate 'alter trigger TRG_AU_AI_GE_SUBSCRI_116_208 enable';
exception
    when others then
        sberror := sqlerrm;
        nuerror := sqlcode;
