...
declare
    ...
    gsbProgram      VARCHAR2(2000) := 'OSF1976';
    gnu_TOTAL_HILOS NUMBER := 4;
...
begin
    ...
    CREATE OR REPLACE PROCEDURE open.OSF1976(inuHilo in number) IS
        ...
        csbCaso     constant varchar2(10)   := ''OSF-1976'';
        ...
        (select cucofact from cuencobr where cucocodi = cargcuco) cucofact,c.*
        ...
        begin
            ...
            values(''OSF1976_''||inuHilo,to_char(cdtfecha,''dd/mm/yyyy hh24:mi''),nucontador2,isbmensaje);
            commit;
        ...
        for rctb in cutbl loop    
            begin
                ...
                if rcvalidacta.cargcuco is null then
                    ...
                    sbActualiza := ''N'';
                    sbComentario := ''El valor del traslado difiere del saldo pendiente del diferido|''||rcvalidacta.cargvalo;
                    nuerr := nuerr + 1;
                    raise raise_continuar;
                end if;
    ...
    begin
        ...
        (
            ...
            enabled             => FALSE,
            auto_drop           => TRUE,
            comments            => 'JOB para finalización de ejecucion DataFix OSF1976'
        );
    ...
    exception
        when others then
            dbms_output.put_line('Error en la creación del finalización OSF1976. Error: '||sqlerrm);
    end;
