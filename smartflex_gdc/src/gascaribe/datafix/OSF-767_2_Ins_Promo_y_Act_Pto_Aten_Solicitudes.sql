    ...
    BEGIN
		...
		FOR rgSol IN cuSolicitud LOOP
			...
			BEGIN
				...
				dbms_output.put_line( 'ANTES - Solicitud[' || rgSol.Package_Id || ']Canal Venta[' || rgSol.Sale_Channel_Id || ']Punto Atencion[' ||  rgSol.pos_oper_unit_id || ']' ); 
			    
                UPDATE mo_packages
                SET Sale_Channel_Id = 2853,
					pos_oper_unit_id = 4131
                WHERE ROWID = rgSol.RId;
			
				COMMIT;
				
				dbms_output.put_line( 'DESPUES - Solicitud[' || rgSol.Package_Id || ']Canal Venta[' || '2853'  || ']Punto Atencion[' ||  '4131' || ']' ); 
								
				EXCEPTION 
					WHEN OTHERS THEN
						dbms_output.put_line( 'ERROR - Solicitud[' || rgSol.Package_Id || ']Canal Venta[' || rgSol.Sale_Channel_Id || ']Punto Atencion[' ||  rgSol.pos_oper_unit_id || ']['|| SQLERRM || ']' ); 
						ROLLBACK;			
			END;
