...
***********************************************************/
DECLARE
    csbCaso             constant varchar2(20)   := 'OSF4455';
    csbTitulo           constant varchar2(2000) := csbCaso||': Genera Lote Manual';
    csbFormato          constant varchar2(25)   := 'DD/MM/YYYY HH24:MI:SS';
    cdtFecha            constant date           := sysdate;
    cnuFactura          constant number         := 2150563140;
    
    cursor cudata is
    select f.rowid row_id,f.* from lote_fact_electronica f     
    where periodo_facturacion = 116660
    and codigo_lote = 11642
    ;
...
BEGIN
    ...
    sbcabecera := 'Factura|Lote_ant|Lote_act|Reporte|Error';
    ...
    begin
        ...
        for rc in cudata loop
            begin 
                ...
                where CODIGO_LOTE = rc.codigo_lote;
                ...
                if nuRowcount != 1 then
                    s_out := s_out||'|'||rc.codigo_lote;
                    s_out := s_out||'|'||'';
                    sbcomentario := 'Insercióhn de lote de factura diferente al esperado ['||nuRowcount||']';
                ...
                if nuError != 0 then
                    s_out := s_out||'|'||rc.codigo_lote;
                     s_out := s_out||'|'||'';
                    sbcomentario := 'Error en llamado prGenerarEstrFactElec ['||sbError||']';
                    raise raise_continuar;
                end if;
            ...
            exception
                ...
                when others then
                    ...
                    sbcomentario := 'Error en generación de lote de factura. Error '||sberror;
                    s_out := s_out||'|'||sbcomentario;
                    dbms_output.put_line(s_out);
            end;
    ...
    dbms_output.put_line('Cantidad de lotes generados: '||nuok);
