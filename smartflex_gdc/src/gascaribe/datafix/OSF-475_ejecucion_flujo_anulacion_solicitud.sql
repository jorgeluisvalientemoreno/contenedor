...
BEGIN
  --Ejecucion de instancia de flujo Anulacion
  BEGIN
    ...
    WF_BOAnswer_Receptor.AnswerReceptor(265427970,
  ...
  --Ejecucion de instancia de flujo de la Interaccion
  if nuEjecucionFlujo = 0 then
  
    BEGIN
      dbms_output.put_line('Ejecucion de instancia de flujo de Interaccion');
      /*errors.Initialize;
      ut_trace.Init;
      ut_trace.SetOutPut(ut_trace.cnuTRACE_DBMS_OUTPUT);
      ut_trace.SetLevel(0);
      ut_trace.Trace('INICIO');*/
      --se notifican las ventas que tienen todas sus órdenes legalizadas, para que continuen
      WF_BOAnswer_Receptor.AnswerReceptor(265401301,
                                          MO_BOCausal.fnuGetSuccess); --se pasa el codigo del flujo
      commit;
      dbms_output.put_line('SALIDA onuErrorCode: ' || nuErrorCode);
      dbms_output.put_line('SALIDA osbErrorMess: ' || sbErrorMessage);
    ...
    if nuEjecucionFlujo = 0 then
      --------------------------------------
      dbms_output.put_line('Anulacion de la solicitud 183380777');
      BEGIN
        declare
          cursor cuSolicitudes is
            SELECT p2.package_id,
             ...
             INNER JOIN OPEN.mo_packages p2
                on p2.cust_care_reques_num =
                   to_char(p.cust_care_reques_num)
             WHERE p.package_id IN (183380777)
               AND p2.motive_status_id = 13
             ORDER BY p2.package_id;
        
          sbComment VARCHAR2(4000) := 'Se cambia estado a anulado por OSF-475';
            ...
            IF nuError = 0 THEN
              ...
              IF nuError = 0 THEN
                ...
                IF nuError = 0 THEN
                  COMMIT;
                  BEGIN
                    nuPlanId := wf_boinstance.fnugetplanid(reg.package_id,
                                                           17);
                  EXCEPTION
                    WHEN OTHERS THEN
                      rollback;
      ...
      dbms_output.put_line('Error en la ejecucion de flujos para la Anulacion de la solicitud 183380777');
    end IF;
  else
    dbms_output.put_line('Error en la ejecucion de flujos para la Anulacion de la solicitud 183380777');
  end if;
