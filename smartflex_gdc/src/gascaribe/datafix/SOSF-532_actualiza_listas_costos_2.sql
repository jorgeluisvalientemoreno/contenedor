...
select a.*,
       ...
       to_date(REGEXP_SUBSTR (previous_text, '[^|]+', 1, 3),'dd/mm/yyyy hh24:mi:ss') fech_ini,
       to_date(REGEXP_SUBSTR (previous_text, '[^|]+', 1, 4),'dd/mm/yyyy hh24:mi:ss') fech_fin,
...
left join open.ldc_const_unoprl r on r.unidad_operativa=l.operating_unit_id
left join open.ge_contrato c on c.id_contrato=r.contrato
...
and listas.current_date= (select min(listas2.current_date) from listas listas2 where listas2.lista=listas.lista)
order by listas.current_date;
begin
    dbms_output.put_line('LISTA|RESULTADO|CONTRATO|FECHA');
    for reg in cuListas loop
        begin
            update ge_list_unitary_cost l set l.validity_final_date = trunc(reg.fech_fin) where l.list_unitary_cost_id = reg.list_unitary_cost_id;
            commit;
