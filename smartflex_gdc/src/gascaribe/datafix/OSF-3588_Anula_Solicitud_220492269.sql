...
declare
  cursor cuSolicitudes is
        ...
        WHERE p.package_id IN (220492269)
		AND p2.motive_status_id = 13
        ORDER BY  p2.package_id;

  sbComment VARCHAR2(4000) := 'Se cambia estado a anulado por OSF-3588';
...
BEGIN
  ...
  dbms_output.put_line('Inicia datafix OSF-3588');
  ...
  FOR reg IN cuSolicitudes LOOP
    ...
    IF nuError = 0 THEN
      --1 VALIDACION
      BEGIN
        dbms_output.put_line('Cambia Transición OSF-3588');
        ldc_pkg_changstatesolici.packageinttransition(reg.package_id,
                                                      ge_boparameter.fnuget('ANNUL_CAUSAL'));
        dbms_output.put_line('Termina Cambia Transición OSF-3588');
      EXCEPTION
        WHEN OTHERS THEN
          rollback;
          nuError := 0;
      ...
      FOR regOt IN cuOrdenes(reg.package_id) LOOP
        BEGIN
          dbms_output.put_line('Anula ordenes OSF-3588');
          ...
          dbms_output.put_line('Termina ordenes OSF-3588');
        EXCEPTION
          WHEN OTHERS THEN
            nuError := 1;
            rollback;
  ...
  END LOOP;
	...
	dbms_output.put_line('Fin datafix OSF-3588');
END;
/
