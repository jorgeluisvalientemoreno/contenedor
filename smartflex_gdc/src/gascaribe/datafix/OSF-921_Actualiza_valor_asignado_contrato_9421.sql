...
BEGIN
  ...
  update open.ge_contrato co set valor_asignado =  nuvalor_asignado WHERE co.id_contrato = 9421;

  Commit;
  dbms_output.put_line('Actualizacion valor asignado del contrato 9421 a su nuevo valor de ' || nuvalor_asignado);


exception
  when others then
  rollback;
  dbms_output.put_line('No pudo actualizar valor asignado del contrato 9421 - ' || SQLERRM);
