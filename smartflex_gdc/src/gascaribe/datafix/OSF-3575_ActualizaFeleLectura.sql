...
Descripción
    ...
    --Modificaciones    
    24/04/2024 - jcatuchemvm
    Creación
    
***********************************************************/
DECLARE
    csbCaso             constant varchar2(20)   := 'OSF3575';
    csbTitulo           constant varchar2(2000) := csbCaso||': Ajuste lecturas Periodo 114012 ciclo 1201';
    ...
    cursor cudata is
    select /*+ index ( l IX_LECTELME02 ) use_nl ( l a )
    index ( a PK_OR_ORDER_ACTIVITY ) use_nl ( a o )
    index ( o PK_OR_ORDER )*/
    l.leemsesu,l.leempefa,l.leemfele,l.leemfele+(to_date('26102024','ddmmyyyy')-trunc(l.leemfele)) leemfele_act,l.rowid row_id,
    a.order_activity_id,o.order_id,o.task_type_id,o.created_date,o.legalization_date,o.execution_final_date
    from lectelme l,or_order_activity a,or_order o
    where leemsesu in 
    (
        51334581,
        50055897,
    ...
    and leempefa = 114012
    and trunc(leemfele)= to_date('27/07/2022','dd/mm/yyyy')
    ...
    BEGIN
        ...
        sbCabecera := 'Producto|Periodo|RowId|FechaAnterior|Orden|Tipo|Execution_final_date|FechaActual|Gestion';
    ...
    BEGIN
        ...
        pGuardaLog(tbArchivos(cnuIdErr),'Cantidad de Ordense actualizadas: '||nuor);
        ...
        for rc in cudata loop
            begin 
                ...
                if nuRowcount != 1 then
                    sbComentario := 'Error 1.2|'||nuProducto||'|Actualización de fecha de lectura diferente a la esperada ['||nuRowcount||']'||'|NA';
                    raise raise_continuar;
                else
                    sbGestion := 'Lectura Actualizada';
                    nuok := nuok + 1;
                end if;
                
                if rc.task_type_id = 12617 then
                    update or_order
                    set execution_final_date = rc.leemfele_act
                    where order_id = rc.order_id;
                    
                    nuRowcount := sql%rowcount;
                
                    if nuRowcount != 1 then
                        sbComentario := 'Error 1.3|'||nuProducto||'|Actualización de fecha de ejecución final diferente a la esperada ['||nuRowcount||']'||'|NA';
                        raise raise_continuar;
                    else
                        sbGestion := 'Lectura y orden Actualizada';
                        nuor := nuor + 1;
                    end if;
            ...
            exception
                when raise_continuar then
                    ...
                    nuerr := nuerr + 1; 
                    sbGestion := 'Fecha No Actualizada';
                when others then
                    pkg_error.seterror;
            ...
            s_out := s_out||'|'||rc.order_id;
            s_out := s_out||'|'||rc.task_type_id;
            s_out := s_out||'|'||rc.execution_final_date;
