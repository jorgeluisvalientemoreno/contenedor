...
IS
	...
	SELECT unique pefacicl,
       pefaano,
       pefames,
       p.*
	FROM open.procejec p, open.perifact pf
	WHERE     TRUNC (prejfech) BETWEEN ''10/07/2024'' AND ''09/08/2024''
	AND p.prejprog = ''FGCC''
	AND p.prejespr = ''T''
	AND p.prejcope = pefacodi
	and not exists (select ''x'' from open.ldc_repanalcons where pefaano = nuano and pefames = numes and pefacicl = ciclo);
