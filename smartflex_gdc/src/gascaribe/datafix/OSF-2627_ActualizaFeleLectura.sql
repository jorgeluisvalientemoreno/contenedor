...
***********************************************************/
DECLARE
    csbCaso             constant varchar2(20)   := 'OSF2627';
    csbTitulo           constant varchar2(2000) := csbCaso||': Ajsute lecturas Periodo 111263 ciclo 606';
    ...
    cursor cudata is
    select sesunuse,l.leemsesu,l.leempefa,l.leemfele,l.leemfele+868 leemfele_act,l.rowid row_id from servsusc,lectelme l
    where sesunuse in 
    (
        6051562,6084894,6094684,6094477,
        51128191,52196857,6095287,6093670,
        6100380,6134384,6077106,50044046,
        6101942,6123394,50023969,6021541,
        6066516,6066358,51128201,6103910,
        6085534,6101355,6102123,6094686,
        50247783,6111686,50146172
    )
    and leemsesu = sesunuse
    and leempefa = 111263
    and trunc(leemfele)= to_date('02/12/2021','dd/mm/yyyy');
    ...
    BEGIN
        ...
        sbCabecera := 'Producto|Periodo|RowId|FechaAnterior|FechaActual|Gestion';
        ...
        for rc in cudata loop
            begin 
                ...
                if nuRowcount != 1 then
                    sbComentario := 'Error 1.2|'||nuProducto||'|Actualización de cuenta a conceptos diferentes a los esperados ['||nuRowcount||']'||'|NA';
                    raise raise_continuar;
                else
            ...
            exception
                when raise_continuar then
                    ...
                    nuerr := nuerr + 1; 
                    sbGestion := 'Lectura No Actualizada';
                when others then
                    pkg_error.seterror;
