...
declare
  cursor cuSolicitudes is
        ...
        WHERE p.package_id IN (225962503)
		AND p2.motive_status_id = 13
        ORDER BY  p2.package_id;

  sbComment VARCHAR2(4000) := 'Se cambia estado a anulado por OSF-4261';
...
BEGIN
  ...
  dbms_output.put_line('Inicia datafix OSF-4261');
  ...
  FOR reg IN cuSolicitudes LOOP
    ...
    IF nuError = 0 THEN
      --1 VALIDACION
      BEGIN
        dbms_output.put_line('Cambia Transición OSF-4261');
        ldc_pkg_changstatesolici.packageinttransition(reg.package_id,
                                                      ge_boparameter.fnuget('ANNUL_CAUSAL'));
        dbms_output.put_line('Termina Cambia Transición OSF-4261');
      EXCEPTION
        WHEN OTHERS THEN
          rollback;
          nuError := 0;
      ...
      FOR regOt IN cuOrdenes(reg.package_id) LOOP
        BEGIN
          dbms_output.put_line('Anula ordenes OSF-4261');
          ...
          dbms_output.put_line('Termina ordenes OSF-4261');
        EXCEPTION
          WHEN OTHERS THEN
            nuError := 1;
            rollback;
  ...
  END LOOP;
	...
	dbms_output.put_line('Fin datafix OSF-4261');
END;
/
