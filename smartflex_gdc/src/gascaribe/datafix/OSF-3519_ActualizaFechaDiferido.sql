...
***********************************************************/
DECLARE
    csbCaso             constant varchar2(20)   := 'OSF3519';
    csbTitulo           constant varchar2(2000) := csbCaso||': Actualización concepto de interés diferido';
    csbFormato          constant varchar2(25)   := 'DD/MM/YYYY HH24:MI:SS';
    cdtFecha            constant date           := sysdate;
    
    cursor cudata is
    select difecodi,difenuse,difeconc,difecoin,
    (select conccoin from concepto where conccodi = difeconc) conccoin 
    from diferido 
...
BEGIN
    ...
    sbcabecera := 'Diferido|Difeconc|Difecoin_ant|Difecoin_act|Error';
    ...
    begin
        ...
        for rc in cudata loop
            begin 
                ...
                s_out := rc.difecodi;
                s_out := s_out||'|'||rc.difeconc;
                s_out := s_out||'|'||rc.difecoin;
                
                if trunc(rc.difecoin) = trunc(rc.conccoin) then
                    sbcomentario := 'El concepto de interes del diferido ya se encuentra actualizado ['||rc.conccoin||']';
                    raise raise_continuar;
                end if;
                
                update diferido c
                set difecoin = rc.conccoin
                where difecodi = rc.difecodi;
                
                nuRowcount := sql%rowcount;
                
                if nuRowcount != 1 then
                    sbcomentario := 'Actualización de concepto de interes del diferido diferente a la esperada ['||nuRowcount||']';
                    raise raise_continuar;
                end if;
                
                s_out := s_out||'|'||rc.conccoin;
                s_out := s_out||'|'||'Ok';
