...
BEGIN
  dbms_output.put_line('-------------------  Inicio OSF-1010 ------------------- ');
  ...
  LOOP
      ...
      BEGIN
          ...
          COMMIT;
          --WF_BOAnswer_Receptor.AnswerReceptor(regPoblacion.instacia_error, -- Código de la instancia del flujo
  ...
  dbms_output.put_line('------------------- Fin OSF-1010 ------------------- ');
EXCEPTION
  WHEN OTHERS THEN
    rollback;
    dbms_output.put_line('---- Error OSF-1010 ----');
    DBMS_OUTPUT.PUT_LINE('OSF-1010-Error General: --> '||sqlerrm);
end;
/
