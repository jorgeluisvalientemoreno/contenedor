DECLARE
  ...
  CURSOR c_escenario1 IS 
        ...
        and ca.cargfecr BETWEEN to_date(to_char('01/06/2023' || ' 00:00:00') , 'dd-mm-yyyy hh24:mi:ss')
  ...
  CURSOR c_escenario2 IS 
        ...
        from cargos ca, notas no , SERVSUSC ss, CAUSCARG cc, concepto co, ic_clascont icc, procesos pr, cuencobr cbr
        where    ca.cargnuse = ss.sesunuse
        and ca.cargcaca = cc.cacacodi
        and ca.cargconc = co.conccodi
        and co.concclco = icc.clcocodi
...
BEGIN
  DBMS_OUTPUT.PUT_LINE ('Inicia OSF-1274 para Junio 2023 Diferencias SA Notas');
  ...
  FOR v_escenario2 IN c_escenario2
   ...
   --   commit;
    ...
    EXCEPTION
        ...
        pkerrors.geterrorvar(onuCodError, osbMensError);
        dbms_output.put_line('ERROR NCONT|' || osbMensError );
        DBMS_OUTPUT.PUT_LINE ('Termina OSF-1274  escenario 2');
    END;
