...
declare
    ...
    where package_id in (219293770,219017464,219392927,219415330)
    ...
    (
        ...
        and i.unit_type_id in (100216,100221)
        
    )
    ;
    CURSOR cuTipoSolicitud
    (
...
IS
    ...
    WHERE
            p.package_id in (219106183,219129904,219193098,219225814,219253663,219321787,219345101,219392927,219415330,219428134,219066245)
        AND p2.motive_status_id = 13
    ORDER BY
        p2.package_id;

    sbcomment               VARCHAR2(4000) := 'Caso para anular Solciitud y sus Ordenes por OSF-3422';
...
begin
    ...
    dbms_output.put_line('OSF-3422: Reprocesar Solicitudes');
    ...
    for rcdata in cudata(4) loop
        ...
        OPEN cuTipoSolicitud(rcdata.Package_Id);
        FETCH cuTipoSolicitud INTO  nuTipoSolictiud;
        CLOSE cuTipoSolicitud;

        IF ( nuTipoSolictiud = 100221) THEN
