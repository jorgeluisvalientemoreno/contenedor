...
set linesize 1000
set timing on
execute dbms_application_info.set_action('APLICANDO DATAFIX OSF-2829');
...
prompt "------------------------------------------------------"
/*
OSF-2829: Asociar producto a las ordenes de trabajo
          producto 50084390 a la orden 323993221,
          producto 52498949 a la orden 323993250,
          producto 52652309 a la orden 323993251,
          producto 52356418 a la orden 323993252

14/06/2024 - GDGuevara - Actualiza registros de las tablas OR ORDER,
...
DECLARE
	...
	IS
        SELECT 
             ...
             CASE
                WHEN oa.order_id = 323993221 THEN 50084390
                WHEN oa.order_id = 323993250 THEN 52498949
                WHEN oa.order_id = 323993251 THEN 52652309
                WHEN oa.order_id = 323993252 THEN 52356418
             END                as ProdOk,
             oa.package_id      as Soli,
             pk.package_type_id as TiSo
        FROM or_order_ACTIVITY oa,
             mo_packages pk
        WHERE oa.order_id IN (323993221, 323993250, 323993251, 323993252)
    ...
    IS
		...
		isbOrderComme varchar2(4000) := 'Se actualiza producto, cliente y direccion por OSF-2829';
...
BEGIN
	...
	dbms_output.put_line('Inicia datafix OSF-2829');
    ...
    dbms_output.put_line('-----------------------------------------');
	dbms_output.put_line('Finaliza datafix OSF-2829');
