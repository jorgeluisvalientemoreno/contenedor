...
Corrige solicitudes de ajustes facturación erradas, en donde existe tanto la 
...
[fa_apromofa, fa_notaapro, fa_cargapro, fa_lectelma, fa_histlema, fa_conssesa]
    ...
    23/03/2023 - jcatuchemvm
    Creación
    
***********************************************************/
declare
    --Constantes para el datafix
    csbCaso         constant varchar2(20)   := 'OSF992';
    cnuPackage_id1  constant MO_WF_PACK_INTERFAC.package_id%TYPE := 63285228;
    cnuPackage_id2  constant MO_WF_PACK_INTERFAC.package_id%TYPE := 103182575;
    cnuPackage_id3  constant MO_WF_PACK_INTERFAC.package_id%TYPE := 143102757;
    cnuPackage_id4  constant MO_WF_PACK_INTERFAC.package_id%TYPE := 187323202;
    cnuPackage_id5  constant MO_WF_PACK_INTERFAC.package_id%TYPE := 191664651;
    ...
    (select count(*) from fa_conssesa where cosanoap = noapnume) cant_consumos,
    ...
    (
        ...
        nvl((select sum(decode(cargsign,'CR',-cargvalo,cargvalo)) from cargos where cargcodo = notanume and cargsign in ('DB','CR')),0) cargvalo
        ...
        and package_id in (cnuPackage_id1,cnuPackage_id2,cnuPackage_id3,cnuPackage_id4,cnuPackage_id5)
    ) a
    ;
    ...
    BEGIN
        ...
        sbCabecera := 'Apmosoli|Apmocons|Apmofere|Apmousre|Apmovalo|Noapnume|Noapsusc|Noapdoso|Noapfact|Caapvalo|Cargvalo|Cant_Lectemp|Cant_Constemp|Apmoesaf_ant|Apmoesaf_act|Motive_status_ant|Motive_status_act|Attention_date_ant|Attention_date_act|Actualizado';
    ...
    begin
        ...
        for nuHash in ircRegistro.tbNotas.first .. ircRegistro.tbNotas.last loop
            ...
            s_out := s_out||'|'||ircRegistro.cantlect;
            s_out := s_out||'|'||ircRegistro.cantcons;
    ...
    begin
        ...
        if ircRegistro.sbActualiza = 'N' then
            ...
            --Borrado de consumos pendientes
            FA_BCCONSSESA.DELCONSUMPTOAPPROVE(ircRegistro.apmocons);
            --Borrado de historico lecturas pendientes            
            FA_BCHISTLEMA.DELHISTREADSTOAPPROVE(ircRegistro.apmocons);
            --Borrado de lecturas pendientes            
            FA_BCLECTELMA.DELREADSTOAPPROVE(ircRegistro.apmocons);
            ...
            Fa_Bcapromofa.Updstate(ircRegistro.apmocons,sbMask,ut_date.fdtSysdate,Fa_Bcapromofa.Csbstat_Approved);
            ...
            rcPackInter.Attendance_Date := ut_date.fdtSysdate;
            --Actualiza el record
            ...
            for i in 1..tbPrincipal.count loop 
                begin
                    ...
                    if not tbRegistro.exists(sbHash) then
                        ...
                        tbRegistro(sbHash).cantcons     := tbPrincipal(i).cant_consumos;
