...
declare
  cursor cuSolicitudes is
    SELECT p2.package_id,
     ...
     WHERE p.package_id IN (204806737)
       AND p2.motive_status_id = 13
     ORDER BY p2.package_id;

  sbComment VARCHAR2(4000) := 'Caso para anular Solciitud 204806737 y sus Ordenes por OSF-1942';
    ...
    IF nuError = 0 THEN
      ...
      FOR regOt IN cuOrdenes(reg.package_id) LOOP
        BEGIN
          ...
          OS_ADDORDERCOMMENT(regOt.order_id,
                             nuCommentType,
                             sbComment,
                             nuErrorCode,
                             sbErrorMesse);
