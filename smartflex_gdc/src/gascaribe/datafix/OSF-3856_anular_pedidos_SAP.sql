...
BEGIN
  ...
  update OPEN.LDCI_TRANSOMA a
     set a.trsmunop = 4769, a.trsmesta = 4
...
BEGIN
  ...
  update OPEN.LDCI_TRANSOMA a
     set a.trsmesta = 4
   where a.trsmcodi = 251750;

  COMMIT;
  dbms_output.put_line('Se anula pedido 251750. Ok.');

EXCEPTION
  WHEN OTHERS THEN
    ROLLBACK;
    dbms_output.put_line('No se pudo anular pedido 251750, ' ||
                         sqlerrm);
END;
/
...
BEGIN
  ...
  update OPEN.LDC_RESULT_PROCESS_PEDIDOVENTA a
     set a.comment_ = 'Realiza cambio de estado de 1 a 4 con el caso OSF-3856'
   where a.request_material_id = 251750
     and a.estado_anterior = 1
     and a.estado_nuevo = 4
     and trunc(a.register_date) = trunc(sysdate);

  COMMIT;
  dbms_output.put_line('Se actualiza comentatio del cambio de estado de 1 a 4 del pedido 251750. Ok.');

EXCEPTION
  WHEN OTHERS THEN
    ROLLBACK;
    dbms_output.put_line('No se actualiza comentario del cambio de estado de 1 a 4 del pedido 251750, ' ||
                         sqlerrm);
END;
/
