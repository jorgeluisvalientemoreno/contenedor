CREATE OR REPLACE PROCEDURE PERSONALIZACIONES.prc_generasolisuspencdm(inuordenId IN or_order.order_id%TYPE)
IS

	/*****************************************************************
	Unidad         : prc_generasolisuspencdm
	...
	******************************************************************/
	...
	nuCausal          	ld_parameter.numeric_value%TYPE := dald_parameter.fnuGetNumeric_Value('PAR_C_SUSP_CDM_XNCAMMED', NULL);
...
EXCEPTION
	...
	WHEN others THEN
		...
		RAISE PKG_ERROR.CONTROLLED_ERROR;
END prc_generasolisuspencdm;
/
PROMPT Otorgando permisos de ejecución a prc_generasolisuspencdm
BEGIN
	pkg_utilidades.prAplicarPermisos(upper('prc_generasolisuspencdm'),'PERSONALIZACIONES'); 
END;
/
