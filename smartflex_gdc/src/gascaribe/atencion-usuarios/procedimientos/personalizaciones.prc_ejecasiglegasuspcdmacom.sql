CREATE OR REPLACE PROCEDURE PERSONALIZACIONES.prc_ejecasiglegasuspcdmacom IS
  ...
  Descripcion    : Ejecuta la Asignación y legalización las actividades 100009279 - CLM - SUSPENSION DESDE CDM_POR 
...
BEGIN
  ...
  FOR rfcuOrdeSuspCDMAcom IN cuOrdeSuspCDMAcom LOOP
    ...
    IF nuCodError = 0 THEN
      ...
      IF nuCodError = 0 THEN
        ...
        -- Comentario de legalización de la orden
        sbLegalComment := 'Legalización automatica por el procedimiento prc_ejecasiglegasuspcdmacom';
        ...
        -- Obtiene la cadena de legalización
  ...
  IF nuOrdenesError > 0 THEN
    pkg_estaproc.practualizaestaproc(sbproceso,
                                     'OK',
                                     'El proceso terminó con error y fallaron ' ||
                                     nuOrdenesError ||
...
/
PROMPT Otorgando permisos de ejecución a prc_ejecasiglegasuspcdmacom
BEGIN
	pkg_utilidades.prAplicarPermisos(upper('prc_ejecasiglegasuspcdmacom'),'PERSONALIZACIONES'); 
END;
/
