CREATE OR REPLACE PACKAGE PERSONALIZACIONES.PKG_BOCNCRM IS
	/*******************************************************************************
		Fuente=Propiedad Intelectual de Gases del Caribe
		
		Autor       :   Jhon Eduar Erazo
		Fecha       :   12-02-2025
		Descripcion :   Paquete con los metodos para el PI CNCRM
		ModIFicaciones  :
		Autor       Fecha       Caso     	Descripcion
		jerazomvm	12/02/2025	OSF-3760	Creación
		fvalencia   14/05/2025  OSF-4171    Se agregan los procedimientos prcObtInfoExencione,
											prcHIjoPadreInfoExenciones y prcCargaAtributosExenciones
	*******************************************************************************/
...
END PKG_BOCNCRM;
/
CREATE OR REPLACE PACKAGE BODY PERSONALIZACIONES.PKG_BOCNCRM IS
    ...
    csbVersion 	VARCHAR2(15) := 'OSF-4171';
	
	sbAtributos	VARCHAR2(4000);
    ...
    <Historial>
           <ModIFicacion Autor="Jhon.Erazo" Fecha="12-02-2025" Inc="OSF-3760" Empresa="GDC"> 
               Creación
           </ModIFicacion>
    </Historial>
    </Procedure>
    **************************************************************************/
    ...
    Programa        : prcCargaAtributosCorreo
	...
	***************************************************************************/	
	PROCEDURE prcCargaAtributosCorreo
	IS
	
		csbMETODO		CONSTANT VARCHAR2(100) := csbSP_NAME ||'prcCargaAtributosCorreo';
	...
	BEGIN
		...
		pkg_boutilidades.prAdicionAtributos('SUSCMAIL', 'SUSCMAIL', sbAtributos);
		pkg_boutilidades.prAdicionAtributos('SUSCCODI', 'PARENT_ID', sbAtributos);			
	...
	***************************************************************************/	
	...
	BEGIN
		...
		prcCargaAtributosCorreo;							 
		
		sbsql := 'SELECT '||sbAtributos ||
				 ' FROM SUSCRIPC WHERE SUSCCODI = :inucontrato';
				 
		pkg_traza.trace('sbsql: ' || sbsql, cnuNVLTRC);

        OPEN orfCursor 
		FOR sbsql USING  inucontrato;
	...
	***************************************************************************/	
	...
	BEGIN
		...
		prcObtInfoFactuElectr(inucontrato,
							   orfCursor
							   );
	...
	/***************************************************************************
    Propiedad Intelectual de Gases del Caribe
    Programa        : prcCargaAtributosExenciones
    Descripcion     : Carga atributos para servicios de consulta exenciones
	...
	***************************************************************************/	
	PROCEDURE prcCargaAtributosExenciones
	(
		osbAtributos		OUT VARCHAR2
	)
	IS	
		csbMETODO		CONSTANT VARCHAR2(100) := csbSP_NAME ||'prcCargaAtributosExenciones';
	...
	BEGIN
		...
		pkg_boutilidades.prAdicionAtributos('SOLICITUD', 'SOLICITUD', osbAtributos);
		pkg_boutilidades.prAdicionAtributos('PRODUCTO', 'PRODUCTO', osbAtributos);
		pkg_boutilidades.prAdicionAtributos('FECHA_INI_VIGENCIA', 'FECHA_INI_VIGENCIA', osbAtributos);
		pkg_boutilidades.prAdicionAtributos('FECHA_FIN_VIGENCIA', 'FECHA_FIN_VIGENCIA', osbAtributos);
		pkg_boutilidades.prAdicionAtributos('PRODUCTO', 'PARENT_ID', osbAtributos);			
	...
	EXCEPTION
		...
		WHEN others THEN
			...
			pkg_traza.trace(csbSP_NAME, cnuNVLTRC, pkg_traza.csbFIN_ERR); 
			RAISE pkg_Error.Controlled_Error;
	END prcCargaAtributosExenciones;
	...
	***************************************************************************/	
	...
	BEGIN
		...
		prcCargaAtributosExenciones(sbAtributos);							 
		
		sbsql := 'SELECT '||sbAtributos ||
				 ' FROM EXENCION_COBRO_FACTURA WHERE PRODUCTO = :inuProducto';
				 
		pkg_traza.trace('sbsql: ' || sbsql, cnuNVLTRC);

        OPEN orfCursor 
		FOR sbsql USING  inuProducto;
    ...
    =========================================================
    Autor       Fecha       Caso    	Descripcion
	fvalencia	15/05/2025	OSF-3760	Creación
	***************************************************************************/	
	PROCEDURE prcHIjoPadreInfoExenciones
	(
		inuProducto 	IN servsusc.sesunuse%type,
	...
	BEGIN
		...
		prcObtInfoExenciones(inuProducto, orfCursor);
...
END PKG_BOCNCRM;
/
PROMPT Otorgando permisos de ejecución para adm_person.PKG_BOCNCRM
BEGIN
    pkg_utilidades.prAplicarPermisos('PKG_BOCNCRM', 'PERSONALIZACIONES');
END;
/