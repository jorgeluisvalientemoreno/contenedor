BEGIN
--{
	SetSystemEnviroment;
--}
END;
/

BEGIN
--{
	UT_Trace.Trace( '********************** Creando paquete CONFEXME_16 ***********************', 5 );
	SA_BOCreatePackages.CreatePackage
	(
		'CONFEXME_16',
		'CREATE OR REPLACE PACKAGE CONFEXME_16 IS ' || chr(10) || chr(10) ||
		chr(9) || '-- Estado de procesamiento' || chr(10) ||
		chr(9) || 'boProcessStatusOK   boolean := TRUE;' || chr(10) || chr(10) ||

		chr(9) || '-- Indice para recorrer las colecciones' || chr(10) ||
		chr(9) || 'nuIndex             binary_integer;' || chr(10) || chr(10) ||

		chr(9) || '-- Registro para almacenar la información de un formato' || chr(10) ||
		chr(9) || 'rcED_Formato        DAED_Formato.styED_Formato;' || chr(10) || chr(10) ||

		chr(9) || '-- Registro para almacenar la información de una expresión' || chr(10) ||
		chr(9) || 'rcNullExpression    DAGR_Config_Expression.styGR_Config_Expression;' || chr(10) || chr(10) ||

		chr(9) || '-- Colección de sentencias' || chr(10) ||
		chr(9) || 'tbrcGE_Statement    ID_BCExportaFormato.tytbGE_Statement;' || chr(10) || chr(10) ||

		chr(9) || '-- Colección de tipos de franja' || chr(10) ||
		chr(9) || 'tbrcED_TipoFran     DAED_TipoFran.tytbED_TipoFran;' || chr(10) || chr(10) ||

		chr(9) || '-- Colección de tipos de bloque' || chr(10) ||
		chr(9) || 'tbrcED_TipoBloq     DAED_TipoBloq.tytbED_TipoBloq;' || chr(10) || chr(10) ||

		chr(9) || '-- Colección de entidades por formato' || chr(10) ||
		chr(9) || 'tbrcED_EntiForm     ID_BCExportaFormato.tytbED_EntiForm;' || chr(10) || chr(10) ||

		chr(9) || '-- Colección de franjas por formato' || chr(10) ||
		chr(9) || 'tbrcED_FranForm     ID_BCExportaFormato.tytbED_FranForm;' || chr(10) || chr(10) ||

		chr(9) || '-- Colección de servicios por franjas por formato' || chr(10) ||
		chr(9) || 'tbrcED_ServFran     DAED_ServFran.tytbED_ServFran;' || chr(10) || chr(10) ||

		chr(9) || '-- Colección de fuentes de datos' || chr(10) ||
		chr(9) || 'tbrcED_FuenDato     ID_BCExportaFormato.tytbED_FuenDato;' || chr(10) || chr(10) ||

		chr(9) || '-- Colección de atributos por fuentes de datos' || chr(10) ||
		chr(9) || 'tbrcED_AtriFuda     ID_BCExportaFormato.tytbED_AtriFuda;' || chr(10) || chr(10) ||

		chr(9) || '-- Colección de franjas' || chr(10) ||
		chr(9) || 'tbrcED_Franja       DAED_Franja.tytbED_Franja;' || chr(10) || chr(10) ||

		chr(9) || '-- Colección de bloques por franja' || chr(10) ||
		chr(9) || 'tbrcED_BloqFran     ID_BCExportaFormato.tytbED_BloqFran;' || chr(10) || chr(10) ||

		chr(9) || '-- Colección de bloques' || chr(10) ||
		chr(9) || 'tbrcED_Bloque       DAED_Bloque.tytbED_Bloque;' || chr(10) || chr(10) ||

		chr(9) || '-- Colección de ítems por bloque' || chr(10) ||
		chr(9) || 'tbrcED_ItemBloq     ID_BCExportaFormato.tytbED_ItemBloq;' || chr(10) || chr(10) ||

		chr(9) || '-- Colección de ítems' || chr(10) ||
		chr(9) || 'tbrcED_Item         ID_BCExportaFormato.tytbED_Item;' || chr(10) || chr(10) ||

		chr(9) || '-- Colección de expresiones' || chr(10) ||
		chr(9) || 'tbrcGR_Config_Expression    ID_BCExportaFormato.tytbGR_Config_Expression;' || chr(10) || chr(10) ||

		chr(9) || '-- extractor y Mezcla con identificador ' || chr(10) ||
		chr(9) || 'CURSOR cuExtractAndMix' || chr(10) ||
		chr(9) || '(' || chr(10) ||
		chr(9) || chr(9) || 'inuIdExtAndMix   ed_confexme.coemcodi%type' || chr(10) ||
		chr(9) || ')' || chr(10) ||
		chr(9) || 'IS' || chr(10) ||
		chr(9) || chr(9) || 'SELECT   coemcodi' || chr(10) ||
		chr(9) || chr(9) || 'FROM     ed_confexme' || chr(10) ||
		chr(9) || chr(9) || 'WHERE    coemcodi = inuIdExtAndMix;' || chr(10) || chr(10) ||

		chr(9) || '-- plantilla con nombre identificador ' || chr(10) ||
		chr(9) || 'CURSOR cuPlantilla' || chr(10) ||
		chr(9) || '(' || chr(10) ||
		chr(9) || chr(9) || 'isbPlantillId   ed_plantill.plancodi%type' || chr(10) ||
		chr(9) || ')' || chr(10) ||
		chr(9) || 'IS' || chr(10) ||
		chr(9) || chr(9) || 'SELECT   plancodi' || chr(10) ||
		chr(9) || chr(9) || 'FROM     ed_plantill' || chr(10) ||
		chr(9) || chr(9) || 'WHERE    plancodi = isbPlantillId;' || chr(10) || chr(10) ||

		chr(9) || '-- Formato con identificador de formato' || chr(10) ||
		chr(9) || 'CURSOR cuFormat' || chr(10) ||
		chr(9) || '(' || chr(10) ||
		chr(9) || chr(9) || 'isbFormatId     ed_formato.formcodi%type' || chr(10) ||
		chr(9) || ')' || chr(10) ||
		chr(9) || 'IS' || chr(10) ||
		chr(9) || chr(9) || 'SELECT   formcodi' || chr(10) ||
		chr(9) || chr(9) || 'FROM     ed_formato' || chr(10) ||
		chr(9) || chr(9) || 'WHERE    formcodi = isbFormatId;' || chr(10) || chr(10) ||

		chr(9) || '-- Reglas asociadas a Items' || chr(10) ||
		chr(9) || 'CURSOR cuGR_Config_Expression' || chr(10) ||
		chr(9) || '(' || chr(10) ||
		chr(9) || chr(9) || 'inuExpression    gr_config_expression.config_expression_id%type' || chr(10) ||
		chr(9) || ')' || chr(10) ||
		chr(9) || 'IS' || chr(10) ||
		chr(9) || chr(9) || 'SELECT   gr_config_expression.*, gr_config_expression.rowid' || chr(10) ||
		chr(9) || chr(9) || 'FROM     gr_config_expression' || chr(10) ||
		chr(9) || chr(9) || 'WHERE    config_expression_id = inuExpression;' || chr(10) || chr(10) ||

		chr(9) || '-- Tipos de Franja asociadas a una regla' || chr(10) ||
		chr(9) || 'CURSOR cuStripTypes' || chr(10) ||
		chr(9) || '(' || chr(10) ||
		chr(9) || chr(9) || 'isbStripTypeRule    ed_tipofran.tifrobna%type' || chr(10) ||
		chr(9) || ')' || chr(10) ||
		chr(9) || 'IS' || chr(10) ||
		chr(9) || chr(9) || 'SELECT   tifrcodi' || chr(10) ||
		chr(9) || chr(9) || 'FROM     ed_tipofran' || chr(10) ||
		chr(9) || chr(9) || 'WHERE    upper(tifrobna) = upper(isbStripTypeRule);' || chr(10) || chr(10) ||

		chr(9) || '-- Tipos de Bloque asociados a una regla' || chr(10) ||
		chr(9) || 'CURSOR cuBlockTypes' || chr(10) ||
		chr(9) || '(' || chr(10) ||
		chr(9) || chr(9) || 'isbBlockTypeRule    ed_tipobloq.tiblobna%type' || chr(10) ||
		chr(9) || ')' || chr(10) ||
		chr(9) || 'IS' || chr(10) ||
		chr(9) || chr(9) || 'SELECT   tiblcodi' || chr(10) ||
		chr(9) || chr(9) || 'FROM     ed_tipobloq' || chr(10) ||
		chr(9) || chr(9) || 'WHERE    upper(tiblobna) = upper(isbBlockTypeRule);' || chr(10) || chr(10) ||

		chr(9) || '-- Procedimiento para ejecutar una sentencia en una transacción autónoma' || chr(10) ||
		chr(9) || 'PROCEDURE ExecuteSQLSentence' || chr(10) ||
		chr(9) || '(' || chr(10) ||
		chr(9) || chr(9) || 'isbSQLSentence    in   varchar2' || chr(10) ||
		chr(9) || ');' || chr(10) || chr(10) ||

		chr(9) || '-- Procedimiento para eliminar en una transacción autónoma las expresiones generadas' || chr(10) ||
		chr(9) || 'PROCEDURE DeleteGeneratedExpressions;' || chr(10) || chr(10) ||

		'END CONFEXME_16;'
	);
--}
END;
/

BEGIN
--{
	SA_BOCreatePackages.CreatePackage
	(
		'CONFEXME_16',
		'CREATE OR REPLACE PACKAGE BODY CONFEXME_16 IS ' || chr(10) || chr(10) ||
		chr(9) || '-- Procedimiento para ejecutar una sentencia en una transacción autónoma' || chr(10) ||
		chr(9) || 'PROCEDURE ExecuteSQLSentence' || chr(10) ||
		chr(9) || '(' || chr(10) ||
		chr(9) || chr(9) || 'isbSQLSentence    in   varchar2' || chr(10) ||
		chr(9) || ')' || chr(10) ||
		chr(9) || 'IS' || chr(10) || chr(10) ||

		chr(9) || chr(9) || '-- Se define el procedimiento como una transacción autónoma' || chr(10) ||
		chr(9) || chr(9) || 'PRAGMA AUTONOMOUS_TRANSACTION;' || chr(10) || chr(10) ||

		chr(9) || 'BEGIN' || chr(10) ||
		chr(9) || '--{' || chr(10) ||
		chr(9) || chr(9) || 'EXECUTE IMMEDIATE isbSQLSentence;' || chr(10) || chr(10) ||

		chr(9) || 'EXCEPTION' || chr(10) || chr(10) ||

		chr(9) || chr(9) || 'when OTHERS then' || chr(10) ||
		chr(9) || chr(9) || chr(9) || 'raise;' || chr(10) ||
		chr(9) || '--}' || chr(10) ||
		chr(9) || 'END;' || chr(10) || chr(10) ||

		chr(9) || '-- Procedimiento para eliminar en una transacción autónoma las expresiones generadas' || chr(10) ||
		chr(9) || 'PROCEDURE DeleteGeneratedExpressions' || chr(10) ||
		chr(9) || 'IS' || chr(10) || chr(10) ||

		chr(9) || chr(9) || '-- Se define el procedimiento como una transacción autónoma' || chr(10) ||
		chr(9) || chr(9) || 'PRAGMA AUTONOMOUS_TRANSACTION;' || chr(10) || chr(10) ||

		chr(9) || chr(9) || '-- Indice para recorrer la colección de expresiones generadas' || chr(10) ||
		chr(9) || chr(9) || 'sbExprIdx            ID_BCExportaFormato.styStrIdx;' || chr(10) || chr(10) ||

		chr(9) || chr(9) || '-- Flag que indica si un objeto stand-alone fue eliminado' || chr(10) ||
		chr(9) || chr(9) || 'boObjectDeleted      boolean;' || chr(10) || chr(10) ||

		chr(9) || 'BEGIN' || chr(10) ||
		chr(9) || '--{' || chr(10) ||
		chr(9) || chr(9) || '-- Se elimina cada una de las expresiones generadas' || chr(10) ||
		chr(9) || chr(9) || 'sbExprIdx := tbrcGR_Config_Expression.first;' || chr(10) || chr(10) ||

		chr(9) || chr(9) || 'while ( sbExprIdx is not NULL ) loop' || chr(10) ||
		chr(9) || chr(9) || '--{' || chr(10) ||
		chr(9) || chr(9) || chr(9) || '-- Se verifica que el objeto exista' || chr(10) ||
		chr(9) || chr(9) || chr(9) || 'if ( tbrcGR_Config_Expression( sbExprIdx ).object_name is not NULL and' || chr(10) ||
		chr(9) || chr(9) || chr(9) || '     UT_Object.fboExistObject( tbrcGR_Config_Expression( sbExprIdx ).object_name )' || chr(10) ||
		chr(9) || chr(9) || chr(9) || ') then' || chr(10) ||
		chr(9) || chr(9) || chr(9) || '--{' || chr(10) ||
		chr(9) || chr(9) || chr(9) || chr(9) || '-- Se elimina el objeto stand-alone' || chr(10) ||
		chr(9) || chr(9) || chr(9) || chr(9) || 'UT_Trace.Trace( ''Eliminando el objeto ['' || tbrcGR_Config_Expression( sbExprIdx ).object_name || '']'', 6 );' || chr(10) ||
		chr(9) || chr(9) || chr(9) || chr(9) || 'boObjectDeleted := UT_Object.fboDeleteObject( tbrcGR_Config_Expression( sbExprIdx ).object_name );' || chr(10) ||
		chr(9) || chr(9) || chr(9) || '--}' || chr(10) ||
		chr(9) || chr(9) || chr(9) || 'end if;' || chr(10) || chr(10) ||

		chr(9) || chr(9) || chr(9) || '-- Se obtiene el índice de la siguiente expresión' || chr(10) ||
		chr(9) || chr(9) || chr(9) || 'sbExprIdx := tbrcGR_Config_Expression.next( sbExprIdx );' || chr(10) ||
		chr(9) || chr(9) || '--}' || chr(10) ||
		chr(9) || chr(9) || 'end loop;' || chr(10) || chr(10) || 

		chr(9) || 'EXCEPTION' || chr(10) || chr(10) ||

		chr(9) || chr(9) || 'when OTHERS then' || chr(10) ||
		chr(9) || chr(9) || chr(9) || 'raise;' || chr(10) ||
		chr(9) || '--}' || chr(10) ||
		chr(9) || 'END;' || chr(10) || chr(10) ||

		'END CONFEXME_16;'
	);
--}
END;
/

DECLARE

	-- Identificador de un formato
	nuFormatId          ed_formato.formcodi%type;

	-- Código de error
	nuErrorCode         ge_error_log.message_id%type;

	-- Mensaje de Error
	sbErrorMessage      ge_error_log.description%type;

BEGIN
--{
	UT_Trace.Trace( '***************************** Eliminando formato *******************************', 5 );
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se obtiene código del formato con el identificador especificado
	open CONFEXME_16.cuFormat( 23 );
	fetch CONFEXME_16.cuFormat into nuFormatId;
	close CONFEXME_16.cuFormat;

	-- Se verifica si existe un formato con el delimitador especificado, en cuyo caso se elimina
	if ( nuFormatId is not NULL ) then
	--{
		pkBSDataExtractor.DeleteFormat
		(
			nuFormatId,
			FALSE,
			nuErrorCode,
			sbErrorMessage
		);

		if ( nuErrorCode <> pkConstante.EXITO ) then
		--{
			-- Se eleva una excepción controlada
			raise LOGIN_DENIED;
		--}
		end if;
	--}
	end if;

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

BEGIN
--{
	UT_Trace.Trace( '****************************** Generando formato *******************************', 5 );
--}
END;
/

DECLARE

	-- Identificador de un formato
	nuFormatId          ed_formato.formcodi%type;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se obtiene código del formato con el identificador especificado
	open CONFEXME_16.cuFormat( 23 );
	fetch CONFEXME_16.cuFormat into nuFormatId;
	close CONFEXME_16.cuFormat;

	-- Se verifica si existe un formato con el delimitador especificado, en cuyo caso se elimina
	if ( nuFormatId is not NULL ) then
	--{
		-- Se establece el identificador para el formato
		CONFEXME_16.rcED_Formato.formcodi := nuFormatId;

		-- Se actualiza la información del formato
		UPDATE ED_Formato
		SET    formdesc = 'LDC_CERTIFICADO_CONSTANCIA_PAGOS',
		       formtido = 134,
		       formiden = '<23>',
		       formtico = 466,
		       formdein = '<LDC_CERTIFICADO_CONSTANCIA_PAGOS>',
		       formdefi = '</LDC_CERTIFICADO_CONSTANCIA_PAGOS>'
		WHERE  formcodi = CONFEXME_16.rcED_Formato.formcodi;
	--}
	else
	--{
		-- Se genera un identificador para el formato
		CONFEXME_16.rcED_Formato.formcodi := 23 ;

		-- Se inserta el formato
		INSERT INTO ED_Formato
		(
			formcodi,
			formdesc,
			formtido,
			formiden,
			formtico,
			formdein,
			formdefi
		)
		VALUES
		(
			CONFEXME_16.rcED_Formato.formcodi,
			'LDC_CERTIFICADO_CONSTANCIA_PAGOS',
			134,
			'<23>',
			466,
			'<LDC_CERTIFICADO_CONSTANCIA_PAGOS>',
			'</LDC_CERTIFICADO_CONSTANCIA_PAGOS>'
		);
	--}
	end if;

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

BEGIN
--{
	UT_Trace.Trace( '**************************** Generando sentencias ******************************', 5 );
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue       number;

	-- CLOBS correspondientes a la definición de las columnas de la sentencia
	clWizardColumns      clob;
	clSelectColumns      clob;
	clListValues         clob;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para la sentencia
	nuNextSeqValue := GE_BOSequence.NextGe_Statement;
	CONFEXME_16.tbrcGE_Statement( '120198056' ).statement_id := nuNextSeqValue;

	UT_Trace.Trace( 'Insertando Sentencia [LDC - Datos Generales de Constancia Pagos]', 5 );
	INSERT INTO GE_Statement
	(
		statement_id,
		module_id,
		description,
		statement,
		name
	)
	VALUES
	(
		CONFEXME_16.tbrcGE_Statement( '120198056' ).statement_id,
		16,
		'LDC - Datos Generales de Constancia Pagos',
		'select to_char(mo_motive.prov_initial_date,''DD/MM/YYYY'') FECHAINI,
       to_char(mo_motive.prov_final_date, ''DD/MM/YYYY'') FECHAFIN,
       to_char(prov_initial_date,''MON'') MESINI,
       to_char(prov_initial_date,''YYYY'') ANOINI,
       to_char(prov_final_date,''MON'') MESFIN,
       to_char(prov_final_date,''YYYY'') ANOFIN
from mo_motive
where   mo_motive.motive_id = cc_bocertificate.fnumotiveid',
		'LDC - Datos Generales de Constancia Pagos'
	);

	INSERT INTO GE_Statement_Columns VALUES
	(
		CONFEXME_16.tbrcGE_Statement( '120198056' ).statement_id,
		empty_clob(),
		empty_clob(),
		empty_clob()
	)
	RETURNING	wizard_columns, select_columns, list_values
	INTO		clWizardColumns, clSelectColumns, clListValues;

	dbms_lob.append( clSelectColumns, '<?xml version="1.0" encoding="utf-16"?>
<ArrayOfBaseStatementColumn xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema">
  <BaseStatementColumn>
    <Name>FECHAINI</Name>
    <Description>FECHAINI</Description>
    <DisplayType>2</DisplayType>
    <InternalType>2</InternalType>
    <Length>10</Length>
    <Scale>0</Scale>
  </BaseStatementColumn>
  <BaseStatementColumn>
    <Name>FECHAFIN</Name>
    <Description>FECHAFIN</Description>
    <DisplayType>2</DisplayType>
    <InternalType>2</InternalType>
    <Length>10</Length>
    <Scale>0</Scale>
  </BaseStatementColumn>
  <BaseStatementColumn>
    <Name>MESINI</Name>
    <Description>MESINI</Description>
    <DisplayType>2</DisplayType>
    <InternalType>2</InternalType>
    <Length>3</Length>
    <Scale>0</Scale>
  </BaseStatementColumn>
  <BaseStatementColumn>
    <Name>ANOINI</Name>
    <Description>ANOINI</Description>
    <DisplayType>2</DisplayType>
    <InternalType>2</InternalType>
    <Length>4</Length>
    <Scale>0</Scale>
  </BaseStatementColumn>
  <BaseStatementColumn>
    <Name>MESFIN</Name>
    <Description>MESFIN</Description>
    <DisplayType>2</DisplayType>
    <InternalType>2</InternalType>
    <Length>3</Length>
    <Scale>0</Scale>
  </BaseStatementColumn>
  <BaseStatementColumn>
    <Name>ANOFIN</Name>
    <Description>ANOFIN</Description>
    <DisplayType>2</DisplayType>
    <InternalType>2</InternalType>
    <Length>4</Length>
    <Scale>0</Scale>
  </BaseStatementColumn>
</ArrayOfBaseStatementColumn>' );

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue       number;

	-- CLOBS correspondientes a la definición de las columnas de la sentencia
	clWizardColumns      clob;
	clSelectColumns      clob;
	clListValues         clob;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para la sentencia
	nuNextSeqValue := GE_BOSequence.NextGe_Statement;
	CONFEXME_16.tbrcGE_Statement( '120198057' ).statement_id := nuNextSeqValue;

	UT_Trace.Trace( 'Insertando Sentencia [LDC - Detalle de Pagos]', 5 );
	INSERT INTO GE_Statement
	(
		statement_id,
		module_id,
		description,
		statement,
		name
	)
	VALUES
	(
		CONFEXME_16.tbrcGE_Statement( '120198057' ).statement_id,
		16,
		'LDC - Detalle de Pagos',
		'SELECT open.DAPR_PRODUCT.FNUGETPRODUCT_TYPE_ID(CARGNUSE,NULL) tip_prod,open.PKTBLSERVICIO.FSBGETDESCRIPTION(open.DAPR_PRODUCT.FNUGETPRODUCT_TYPE_ID(CARGNUSE,NULL),NULL) TIPOPRODUCTO,
       to_char(open.PKTBLFACTURA.FDTGETFACTFEGE(CUCOFACT,NULL),''DD/MM/YYYY'')FECHAGENFAC,
CASE WHEN cargcuco = (SELECT MAX(cargcuco) FROM open.cargos t WHERE t.cargsign = ''PA'' and t.cargcodo = PAGOCUPO) THEN
            (select sum(cargvalo) from open.CARGOS t where t.cargsign = ''PA'' and t.cargcodo = PAGOCUPO)
           ELSE
            0
      END TOTALFACT,
             to_char(pagos.PAGOFEPA, ''DD/MM/YYYY'') FECHARECAUDO,
       cargcuco CUENTACOBROPAGO,
       cargvalo VALORRECAUDO,
       to_char(pagos.pagofegr, ''DD/MM/YYYY'') FECHAAPLICACIONRECAUDO,
       (select subanomb
          from open.sucubanc
         where sucubanc.subabanc = pagos.pagobanc
           and sucubanc.subacodi = pagos.pagosuba) ENTIDADRECAUDADORA
  from open.pagos, open.mo_motive, open.CARGOS,open.CUENCOBR
 where mo_motive.motive_id = cc_bocertificate.fnuMotiveId
   and mo_motive.subscription_id = pagos.pagosusc
   AND cargcodo = pagocupo
   and trunc(pagos.pagofepa) >= mo_motive.prov_initial_date
   and trunc(pagos.pagofepa) <= mo_motive.prov_final_date
   and CARGOS.cargsign =''PA''
   AND CARGOS.CARGCUCO = CUENCOBR.CUCOCODI
ORDER BY pagos.PAGOFEPA, tip_prod asc',
		'LDC - Detalle de Pagos'
	);

	INSERT INTO GE_Statement_Columns VALUES
	(
		CONFEXME_16.tbrcGE_Statement( '120198057' ).statement_id,
		empty_clob(),
		empty_clob(),
		empty_clob()
	)
	RETURNING	wizard_columns, select_columns, list_values
	INTO		clWizardColumns, clSelectColumns, clListValues;

	dbms_lob.append( clSelectColumns, '<?xml version="1.0" encoding="utf-16"?>
<ArrayOfBaseStatementColumn xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema">
  <BaseStatementColumn>
    <Name>TIP_PROD</Name>
    <Description>TIP_PROD</Description>
    <DisplayType>1</DisplayType>
    <InternalType>1</InternalType>
    <Length>15</Length>
    <Scale>127</Scale>
  </BaseStatementColumn>
  <BaseStatementColumn>
    <Name>TIPOPRODUCTO</Name>
    <Description>TIPOPRODUCTO</Description>
    <DisplayType>2</DisplayType>
    <InternalType>2</InternalType>
    <Length>4000</Length>
    <Scale>0</Scale>
  </BaseStatementColumn>
  <BaseStatementColumn>
    <Name>FECHAGENFAC</Name>
    <Description>FECHAGENFAC</Description>
    <DisplayType>2</DisplayType>
    <InternalType>2</InternalType>
    <Length>10</Length>
    <Scale>0</Scale>
  </BaseStatementColumn>
  <BaseStatementColumn>
    <Name>TOTALFACT</Name>
    <Description>TOTALFACT</Description>
    <DisplayType>1</DisplayType>
    <InternalType>1</InternalType>
    <Length>15</Length>
    <Scale>127</Scale>
  </BaseStatementColumn>
  <BaseStatementColumn>
    <Name>FECHARECAUDO</Name>
    <Description>FECHARECAUDO</Description>
    <DisplayType>2</DisplayType>
    <InternalType>2</InternalType>
    <Length>10</Length>
    <Scale>0</Scale>
  </BaseStatementColumn>
  <BaseStatementColumn>
    <Name>CUENTACOBROPAGO</Name>
    <Description>CUENTACOBROPAGO</Description>
    <DisplayType>0</DisplayType>
    <InternalType>0</InternalType>
    <Length>10</Length>
    <Scale>0</Scale>
  </BaseStatementColumn>
  <BaseStatementColumn>
    <Name>VALORRECAUDO</Name>
    <Description>VALORRECAUDO</Description>
    <DisplayType>1</DisplayType>
    <InternalType>1</InternalType>
    <Length>13</Length>
    <Scale>2</Scale>
  </BaseStatementColumn>
  <BaseStatementColumn>
    <Name>FECHAAPLICACIONRECAUDO</Name>
    <Description>FECHAAPLICACIONRECAUDO</Description>
    <DisplayType>2</DisplayType>
    <InternalType>2</InternalType>
    <Length>10</Length>
    <Scale>0</Scale>
  </BaseStatementColumn>
  <BaseStatementColumn>
    <Name>ENTIDADRECAUDADORA</Name>
    <Description>ENTIDADRECAUDADORA</Description>
    <DisplayType>2</DisplayType>
    <InternalType>2</InternalType>
    <Length>40</Length>
    <Scale>0</Scale>
  </BaseStatementColumn>
</ArrayOfBaseStatementColumn>' );

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

BEGIN
--{
	UT_Trace.Trace( '*********************** Generando entidades por formato ************************', 5 );
--}
END;
/

BEGIN
--{
	UT_Trace.Trace( '************************** Generando tipos de franja ***************************', 5 );
--}
END;
/

BEGIN
--{
	UT_Trace.Trace( '************************** Generando tipos de bloque ***************************', 5 );
--}
END;
/

BEGIN
--{
	UT_Trace.Trace( '****************************** Generando franjas *******************************', 5 );
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para la franja
	nuNextSeqValue := pkBOInsertMgr.NextED_FRANJA;
	CONFEXME_16.tbrcED_Franja( 4927 ).francodi := nuNextSeqValue;

	UT_Trace.Trace( 'Insertando Franja [DATOS DE PAGOS]', 5 );
	INSERT INTO ED_Franja
	(
		francodi,
		frandesc,
		frantifr,
		frandein,
		frandefi
	)
	VALUES
	(
		CONFEXME_16.tbrcED_Franja( 4927 ).francodi,
		'DATOS DE PAGOS',
		CONFEXME_16.tbrcED_Franja( 4927 ).frantifr,
		NULL,
		NULL
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para la franja
	nuNextSeqValue := pkBOInsertMgr.NextED_FRANJA;
	CONFEXME_16.tbrcED_Franja( 4928 ).francodi := nuNextSeqValue;

	UT_Trace.Trace( 'Insertando Franja [DATOS GENERALES]', 5 );
	INSERT INTO ED_Franja
	(
		francodi,
		frandesc,
		frantifr,
		frandein,
		frandefi
	)
	VALUES
	(
		CONFEXME_16.tbrcED_Franja( 4928 ).francodi,
		'DATOS GENERALES',
		CONFEXME_16.tbrcED_Franja( 4928 ).frantifr,
		NULL,
		NULL
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

BEGIN
--{
	UT_Trace.Trace( '************************ Generando franjas por formato *************************', 5 );
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para la franja-formato
	nuNextSeqValue := pkBOInsertMgr.NextED_FRANFORM;
	CONFEXME_16.tbrcED_FranForm( '4776' ).frfocodi := nuNextSeqValue;

	-- Se asigna el formato
	CONFEXME_16.tbrcED_FranForm( '4776' ).frfoform := CONFEXME_16.rcED_Formato.formcodi;

	-- Se asigna la franja
	if ( CONFEXME_16.tbrcED_Franja.exists( 4927 ) ) then
		CONFEXME_16.tbrcED_FranForm( '4776' ).frfofran := CONFEXME_16.tbrcED_Franja( 4927 ).francodi;
	end if;

	INSERT INTO ED_FranForm
	(
		frfocodi,
		frfoform,
		frfofran,
		frfoorde,
		frfomult
	)
	VALUES
	(
		CONFEXME_16.tbrcED_FranForm( '4776' ).frfocodi,
		CONFEXME_16.tbrcED_FranForm( '4776' ).frfoform,
		CONFEXME_16.tbrcED_FranForm( '4776' ).frfofran,
		2,
		'N'
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para la franja-formato
	nuNextSeqValue := pkBOInsertMgr.NextED_FRANFORM;
	CONFEXME_16.tbrcED_FranForm( '4777' ).frfocodi := nuNextSeqValue;

	-- Se asigna el formato
	CONFEXME_16.tbrcED_FranForm( '4777' ).frfoform := CONFEXME_16.rcED_Formato.formcodi;

	-- Se asigna la franja
	if ( CONFEXME_16.tbrcED_Franja.exists( 4928 ) ) then
		CONFEXME_16.tbrcED_FranForm( '4777' ).frfofran := CONFEXME_16.tbrcED_Franja( 4928 ).francodi;
	end if;

	INSERT INTO ED_FranForm
	(
		frfocodi,
		frfoform,
		frfofran,
		frfoorde,
		frfomult
	)
	VALUES
	(
		CONFEXME_16.tbrcED_FranForm( '4777' ).frfocodi,
		CONFEXME_16.tbrcED_FranForm( '4777' ).frfoform,
		CONFEXME_16.tbrcED_FranForm( '4777' ).frfofran,
		1,
		'N'
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

BEGIN
--{
	UT_Trace.Trace( '******************* Generando servicios por franja-formato *********************', 5 );
--}
END;
/

BEGIN
--{
	UT_Trace.Trace( '************************* Generando fuentes de datos ***************************', 5 );
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para la fuente de datos
	nuNextSeqValue := pkBOInsertMgr.NextED_FUENDATO;
	CONFEXME_16.tbrcED_FuenDato( '4064' ).fudacodi := nuNextSeqValue;

	UT_Trace.Trace( 'Insertando Fuente de Datos [LDC - Aplicacion de Pagos]', 5 );
	INSERT INTO ED_FuenDato
	(
		fudacodi,
		fudadesc,
		fudaserv,
		fudasent
	)
	VALUES
	(
		CONFEXME_16.tbrcED_FuenDato( '4064' ).fudacodi,
		'LDC - Aplicacion de Pagos',
		'CC_BOCertificate.GetPaysDetail',
		CONFEXME_16.tbrcED_FuenDato( '4064' ).fudasent
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para la fuente de datos
	nuNextSeqValue := pkBOInsertMgr.NextED_FUENDATO;
	CONFEXME_16.tbrcED_FuenDato( '4065' ).fudacodi := nuNextSeqValue;

	-- Se asigna la sentencia asociada a la fuente de datos
	if ( CONFEXME_16.tbrcGE_Statement.exists( '120198057' ) ) then
		CONFEXME_16.tbrcED_FuenDato( '4065' ).fudasent := CONFEXME_16.tbrcGE_Statement( '120198057' ).statement_id;
	end if;

	UT_Trace.Trace( 'Insertando Fuente de Datos [LDC - Detalle de Pagos]', 5 );
	INSERT INTO ED_FuenDato
	(
		fudacodi,
		fudadesc,
		fudaserv,
		fudasent
	)
	VALUES
	(
		CONFEXME_16.tbrcED_FuenDato( '4065' ).fudacodi,
		'LDC - Detalle de Pagos',
		NULL,
		CONFEXME_16.tbrcED_FuenDato( '4065' ).fudasent
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para la fuente de datos
	nuNextSeqValue := pkBOInsertMgr.NextED_FUENDATO;
	CONFEXME_16.tbrcED_FuenDato( '4066' ).fudacodi := nuNextSeqValue;

	UT_Trace.Trace( 'Insertando Fuente de Datos [LDC - Datos del cliente]', 5 );
	INSERT INTO ED_FuenDato
	(
		fudacodi,
		fudadesc,
		fudaserv,
		fudasent
	)
	VALUES
	(
		CONFEXME_16.tbrcED_FuenDato( '4066' ).fudacodi,
		'LDC - Datos del cliente',
		'PKG_BOINFODATOSFORMATOS.prcDatosFormConstanciaPagos',
		CONFEXME_16.tbrcED_FuenDato( '4066' ).fudasent
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para la fuente de datos
	nuNextSeqValue := pkBOInsertMgr.NextED_FUENDATO;
	CONFEXME_16.tbrcED_FuenDato( '4067' ).fudacodi := nuNextSeqValue;

	-- Se asigna la sentencia asociada a la fuente de datos
	if ( CONFEXME_16.tbrcGE_Statement.exists( '120198056' ) ) then
		CONFEXME_16.tbrcED_FuenDato( '4067' ).fudasent := CONFEXME_16.tbrcGE_Statement( '120198056' ).statement_id;
	end if;

	UT_Trace.Trace( 'Insertando Fuente de Datos [LDC - Datos Generales de Constancia Pagos]', 5 );
	INSERT INTO ED_FuenDato
	(
		fudacodi,
		fudadesc,
		fudaserv,
		fudasent
	)
	VALUES
	(
		CONFEXME_16.tbrcED_FuenDato( '4067' ).fudacodi,
		'LDC - Datos Generales de Constancia Pagos',
		NULL,
		CONFEXME_16.tbrcED_FuenDato( '4067' ).fudasent
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

BEGIN
--{
	UT_Trace.Trace( '******************** Generando atributos por fuente de datos *******************', 5 );
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el atributo
	nuNextSeqValue := pkBOInsertMgr.NextED_ATRIFUDA;
	CONFEXME_16.tbrcED_AtriFuda( '32077' ).atfdcodi := nuNextSeqValue;

	-- Se asigna la fuente de datos
	if ( CONFEXME_16.tbrcED_FuenDato.exists( '4067' ) ) then
		CONFEXME_16.tbrcED_AtriFuda( '32077' ).atfdfuda := CONFEXME_16.tbrcED_FuenDato( '4067' ).fudacodi;
	end if;

	UT_Trace.Trace( 'Insertando Fuente de Datos [FECHAINI]', 5 );
	INSERT INTO ED_AtriFuda
	(
		atfdcodi,
		atfdnote,
		atfddesc,
		atfdorde,
		atfdvisi,
		atfdfuda,
		atfdtida
	)
	VALUES
	(
		CONFEXME_16.tbrcED_AtriFuda( '32077' ).atfdcodi,
		'FECHAINI',
		'FECHAINI',
		1,
		'S',
		CONFEXME_16.tbrcED_AtriFuda( '32077' ).atfdfuda,
		1
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el atributo
	nuNextSeqValue := pkBOInsertMgr.NextED_ATRIFUDA;
	CONFEXME_16.tbrcED_AtriFuda( '32078' ).atfdcodi := nuNextSeqValue;

	-- Se asigna la fuente de datos
	if ( CONFEXME_16.tbrcED_FuenDato.exists( '4067' ) ) then
		CONFEXME_16.tbrcED_AtriFuda( '32078' ).atfdfuda := CONFEXME_16.tbrcED_FuenDato( '4067' ).fudacodi;
	end if;

	UT_Trace.Trace( 'Insertando Fuente de Datos [FECHAFIN]', 5 );
	INSERT INTO ED_AtriFuda
	(
		atfdcodi,
		atfdnote,
		atfddesc,
		atfdorde,
		atfdvisi,
		atfdfuda,
		atfdtida
	)
	VALUES
	(
		CONFEXME_16.tbrcED_AtriFuda( '32078' ).atfdcodi,
		'FECHAFIN',
		'FECHAFIN',
		2,
		'S',
		CONFEXME_16.tbrcED_AtriFuda( '32078' ).atfdfuda,
		1
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el atributo
	nuNextSeqValue := pkBOInsertMgr.NextED_ATRIFUDA;
	CONFEXME_16.tbrcED_AtriFuda( '32079' ).atfdcodi := nuNextSeqValue;

	-- Se asigna la fuente de datos
	if ( CONFEXME_16.tbrcED_FuenDato.exists( '4067' ) ) then
		CONFEXME_16.tbrcED_AtriFuda( '32079' ).atfdfuda := CONFEXME_16.tbrcED_FuenDato( '4067' ).fudacodi;
	end if;

	UT_Trace.Trace( 'Insertando Fuente de Datos [MESINI]', 5 );
	INSERT INTO ED_AtriFuda
	(
		atfdcodi,
		atfdnote,
		atfddesc,
		atfdorde,
		atfdvisi,
		atfdfuda,
		atfdtida
	)
	VALUES
	(
		CONFEXME_16.tbrcED_AtriFuda( '32079' ).atfdcodi,
		'MESINI',
		'MESINI',
		3,
		'S',
		CONFEXME_16.tbrcED_AtriFuda( '32079' ).atfdfuda,
		1
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el atributo
	nuNextSeqValue := pkBOInsertMgr.NextED_ATRIFUDA;
	CONFEXME_16.tbrcED_AtriFuda( '32080' ).atfdcodi := nuNextSeqValue;

	-- Se asigna la fuente de datos
	if ( CONFEXME_16.tbrcED_FuenDato.exists( '4067' ) ) then
		CONFEXME_16.tbrcED_AtriFuda( '32080' ).atfdfuda := CONFEXME_16.tbrcED_FuenDato( '4067' ).fudacodi;
	end if;

	UT_Trace.Trace( 'Insertando Fuente de Datos [ANOINI]', 5 );
	INSERT INTO ED_AtriFuda
	(
		atfdcodi,
		atfdnote,
		atfddesc,
		atfdorde,
		atfdvisi,
		atfdfuda,
		atfdtida
	)
	VALUES
	(
		CONFEXME_16.tbrcED_AtriFuda( '32080' ).atfdcodi,
		'ANOINI',
		'ANOINI',
		4,
		'S',
		CONFEXME_16.tbrcED_AtriFuda( '32080' ).atfdfuda,
		1
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el atributo
	nuNextSeqValue := pkBOInsertMgr.NextED_ATRIFUDA;
	CONFEXME_16.tbrcED_AtriFuda( '32081' ).atfdcodi := nuNextSeqValue;

	-- Se asigna la fuente de datos
	if ( CONFEXME_16.tbrcED_FuenDato.exists( '4067' ) ) then
		CONFEXME_16.tbrcED_AtriFuda( '32081' ).atfdfuda := CONFEXME_16.tbrcED_FuenDato( '4067' ).fudacodi;
	end if;

	UT_Trace.Trace( 'Insertando Fuente de Datos [MESFIN]', 5 );
	INSERT INTO ED_AtriFuda
	(
		atfdcodi,
		atfdnote,
		atfddesc,
		atfdorde,
		atfdvisi,
		atfdfuda,
		atfdtida
	)
	VALUES
	(
		CONFEXME_16.tbrcED_AtriFuda( '32081' ).atfdcodi,
		'MESFIN',
		'MESFIN',
		5,
		'S',
		CONFEXME_16.tbrcED_AtriFuda( '32081' ).atfdfuda,
		1
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el atributo
	nuNextSeqValue := pkBOInsertMgr.NextED_ATRIFUDA;
	CONFEXME_16.tbrcED_AtriFuda( '32082' ).atfdcodi := nuNextSeqValue;

	-- Se asigna la fuente de datos
	if ( CONFEXME_16.tbrcED_FuenDato.exists( '4067' ) ) then
		CONFEXME_16.tbrcED_AtriFuda( '32082' ).atfdfuda := CONFEXME_16.tbrcED_FuenDato( '4067' ).fudacodi;
	end if;

	UT_Trace.Trace( 'Insertando Fuente de Datos [ANOFIN]', 5 );
	INSERT INTO ED_AtriFuda
	(
		atfdcodi,
		atfdnote,
		atfddesc,
		atfdorde,
		atfdvisi,
		atfdfuda,
		atfdtida
	)
	VALUES
	(
		CONFEXME_16.tbrcED_AtriFuda( '32082' ).atfdcodi,
		'ANOFIN',
		'ANOFIN',
		6,
		'S',
		CONFEXME_16.tbrcED_AtriFuda( '32082' ).atfdfuda,
		1
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el atributo
	nuNextSeqValue := pkBOInsertMgr.NextED_ATRIFUDA;
	CONFEXME_16.tbrcED_AtriFuda( '32083' ).atfdcodi := nuNextSeqValue;

	-- Se asigna la fuente de datos
	if ( CONFEXME_16.tbrcED_FuenDato.exists( '4064' ) ) then
		CONFEXME_16.tbrcED_AtriFuda( '32083' ).atfdfuda := CONFEXME_16.tbrcED_FuenDato( '4064' ).fudacodi;
	end if;

	UT_Trace.Trace( 'Insertando Fuente de Datos [Nuyear]', 5 );
	INSERT INTO ED_AtriFuda
	(
		atfdcodi,
		atfdnote,
		atfddesc,
		atfdorde,
		atfdvisi,
		atfdfuda,
		atfdtida
	)
	VALUES
	(
		CONFEXME_16.tbrcED_AtriFuda( '32083' ).atfdcodi,
		'NUYEAR',
		'Nuyear',
		1,
		'S',
		CONFEXME_16.tbrcED_AtriFuda( '32083' ).atfdfuda,
		1
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el atributo
	nuNextSeqValue := pkBOInsertMgr.NextED_ATRIFUDA;
	CONFEXME_16.tbrcED_AtriFuda( '32084' ).atfdcodi := nuNextSeqValue;

	-- Se asigna la fuente de datos
	if ( CONFEXME_16.tbrcED_FuenDato.exists( '4064' ) ) then
		CONFEXME_16.tbrcED_AtriFuda( '32084' ).atfdfuda := CONFEXME_16.tbrcED_FuenDato( '4064' ).fudacodi;
	end if;

	UT_Trace.Trace( 'Insertando Fuente de Datos [Numonth]', 5 );
	INSERT INTO ED_AtriFuda
	(
		atfdcodi,
		atfdnote,
		atfddesc,
		atfdorde,
		atfdvisi,
		atfdfuda,
		atfdtida
	)
	VALUES
	(
		CONFEXME_16.tbrcED_AtriFuda( '32084' ).atfdcodi,
		'NUMONTH',
		'Numonth',
		2,
		'S',
		CONFEXME_16.tbrcED_AtriFuda( '32084' ).atfdfuda,
		1
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el atributo
	nuNextSeqValue := pkBOInsertMgr.NextED_ATRIFUDA;
	CONFEXME_16.tbrcED_AtriFuda( '32085' ).atfdcodi := nuNextSeqValue;

	-- Se asigna la fuente de datos
	if ( CONFEXME_16.tbrcED_FuenDato.exists( '4064' ) ) then
		CONFEXME_16.tbrcED_AtriFuda( '32085' ).atfdfuda := CONFEXME_16.tbrcED_FuenDato( '4064' ).fudacodi;
	end if;

	UT_Trace.Trace( 'Insertando Fuente de Datos [Nupaymentid]', 5 );
	INSERT INTO ED_AtriFuda
	(
		atfdcodi,
		atfdnote,
		atfddesc,
		atfdorde,
		atfdvisi,
		atfdfuda,
		atfdtida
	)
	VALUES
	(
		CONFEXME_16.tbrcED_AtriFuda( '32085' ).atfdcodi,
		'NUPAYMENTID',
		'Nupaymentid',
		3,
		'N',
		CONFEXME_16.tbrcED_AtriFuda( '32085' ).atfdfuda,
		1
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el atributo
	nuNextSeqValue := pkBOInsertMgr.NextED_ATRIFUDA;
	CONFEXME_16.tbrcED_AtriFuda( '32086' ).atfdcodi := nuNextSeqValue;

	-- Se asigna la fuente de datos
	if ( CONFEXME_16.tbrcED_FuenDato.exists( '4064' ) ) then
		CONFEXME_16.tbrcED_AtriFuda( '32086' ).atfdfuda := CONFEXME_16.tbrcED_FuenDato( '4064' ).fudacodi;
	end if;

	UT_Trace.Trace( 'Insertando Fuente de Datos [Dtcollectdate]', 5 );
	INSERT INTO ED_AtriFuda
	(
		atfdcodi,
		atfdnote,
		atfddesc,
		atfdorde,
		atfdvisi,
		atfdfuda,
		atfdtida
	)
	VALUES
	(
		CONFEXME_16.tbrcED_AtriFuda( '32086' ).atfdcodi,
		'DTCOLLECTDATE',
		'Dtcollectdate',
		4,
		'N',
		CONFEXME_16.tbrcED_AtriFuda( '32086' ).atfdfuda,
		12
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el atributo
	nuNextSeqValue := pkBOInsertMgr.NextED_ATRIFUDA;
	CONFEXME_16.tbrcED_AtriFuda( '32087' ).atfdcodi := nuNextSeqValue;

	-- Se asigna la fuente de datos
	if ( CONFEXME_16.tbrcED_FuenDato.exists( '4064' ) ) then
		CONFEXME_16.tbrcED_AtriFuda( '32087' ).atfdfuda := CONFEXME_16.tbrcED_FuenDato( '4064' ).fudacodi;
	end if;

	UT_Trace.Trace( 'Insertando Fuente de Datos [Nucollectrecord]', 5 );
	INSERT INTO ED_AtriFuda
	(
		atfdcodi,
		atfdnote,
		atfddesc,
		atfdorde,
		atfdvisi,
		atfdfuda,
		atfdtida
	)
	VALUES
	(
		CONFEXME_16.tbrcED_AtriFuda( '32087' ).atfdcodi,
		'NUCOLLECTRECORD',
		'Nucollectrecord',
		5,
		'N',
		CONFEXME_16.tbrcED_AtriFuda( '32087' ).atfdfuda,
		2
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el atributo
	nuNextSeqValue := pkBOInsertMgr.NextED_ATRIFUDA;
	CONFEXME_16.tbrcED_AtriFuda( '32088' ).atfdcodi := nuNextSeqValue;

	-- Se asigna la fuente de datos
	if ( CONFEXME_16.tbrcED_FuenDato.exists( '4064' ) ) then
		CONFEXME_16.tbrcED_AtriFuda( '32088' ).atfdfuda := CONFEXME_16.tbrcED_FuenDato( '4064' ).fudacodi;
	end if;

	UT_Trace.Trace( 'Insertando Fuente de Datos [Dtcollectappli]', 5 );
	INSERT INTO ED_AtriFuda
	(
		atfdcodi,
		atfdnote,
		atfddesc,
		atfdorde,
		atfdvisi,
		atfdfuda,
		atfdtida
	)
	VALUES
	(
		CONFEXME_16.tbrcED_AtriFuda( '32088' ).atfdcodi,
		'DTCOLLECTAPPLI',
		'Dtcollectappli',
		6,
		'N',
		CONFEXME_16.tbrcED_AtriFuda( '32088' ).atfdfuda,
		12
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el atributo
	nuNextSeqValue := pkBOInsertMgr.NextED_ATRIFUDA;
	CONFEXME_16.tbrcED_AtriFuda( '32089' ).atfdcodi := nuNextSeqValue;

	-- Se asigna la fuente de datos
	if ( CONFEXME_16.tbrcED_FuenDato.exists( '4064' ) ) then
		CONFEXME_16.tbrcED_AtriFuda( '32089' ).atfdfuda := CONFEXME_16.tbrcED_FuenDato( '4064' ).fudacodi;
	end if;

	UT_Trace.Trace( 'Insertando Fuente de Datos [Nucollectentity]', 5 );
	INSERT INTO ED_AtriFuda
	(
		atfdcodi,
		atfdnote,
		atfddesc,
		atfdorde,
		atfdvisi,
		atfdfuda,
		atfdtida
	)
	VALUES
	(
		CONFEXME_16.tbrcED_AtriFuda( '32089' ).atfdcodi,
		'NUCOLLECTENTITY',
		'Nucollectentity',
		7,
		'N',
		CONFEXME_16.tbrcED_AtriFuda( '32089' ).atfdfuda,
		1
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el atributo
	nuNextSeqValue := pkBOInsertMgr.NextED_ATRIFUDA;
	CONFEXME_16.tbrcED_AtriFuda( '32090' ).atfdcodi := nuNextSeqValue;

	-- Se asigna la fuente de datos
	if ( CONFEXME_16.tbrcED_FuenDato.exists( '4064' ) ) then
		CONFEXME_16.tbrcED_AtriFuda( '32090' ).atfdfuda := CONFEXME_16.tbrcED_FuenDato( '4064' ).fudacodi;
	end if;

	UT_Trace.Trace( 'Insertando Fuente de Datos [Sbbankname]', 5 );
	INSERT INTO ED_AtriFuda
	(
		atfdcodi,
		atfdnote,
		atfddesc,
		atfdorde,
		atfdvisi,
		atfdfuda,
		atfdtida
	)
	VALUES
	(
		CONFEXME_16.tbrcED_AtriFuda( '32090' ).atfdcodi,
		'SBBANKNAME',
		'Sbbankname',
		8,
		'N',
		CONFEXME_16.tbrcED_AtriFuda( '32090' ).atfdfuda,
		1
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el atributo
	nuNextSeqValue := pkBOInsertMgr.NextED_ATRIFUDA;
	CONFEXME_16.tbrcED_AtriFuda( '32091' ).atfdcodi := nuNextSeqValue;

	-- Se asigna la fuente de datos
	if ( CONFEXME_16.tbrcED_FuenDato.exists( '4064' ) ) then
		CONFEXME_16.tbrcED_AtriFuda( '32091' ).atfdfuda := CONFEXME_16.tbrcED_FuenDato( '4064' ).fudacodi;
	end if;

	UT_Trace.Trace( 'Insertando Fuente de Datos [Nuproduct]', 5 );
	INSERT INTO ED_AtriFuda
	(
		atfdcodi,
		atfdnote,
		atfddesc,
		atfdorde,
		atfdvisi,
		atfdfuda,
		atfdtida
	)
	VALUES
	(
		CONFEXME_16.tbrcED_AtriFuda( '32091' ).atfdcodi,
		'NUPRODUCT',
		'Nuproduct',
		9,
		'N',
		CONFEXME_16.tbrcED_AtriFuda( '32091' ).atfdfuda,
		1
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el atributo
	nuNextSeqValue := pkBOInsertMgr.NextED_ATRIFUDA;
	CONFEXME_16.tbrcED_AtriFuda( '32092' ).atfdcodi := nuNextSeqValue;

	-- Se asigna la fuente de datos
	if ( CONFEXME_16.tbrcED_FuenDato.exists( '4064' ) ) then
		CONFEXME_16.tbrcED_AtriFuda( '32092' ).atfdfuda := CONFEXME_16.tbrcED_FuenDato( '4064' ).fudacodi;
	end if;

	UT_Trace.Trace( 'Insertando Fuente de Datos [Sbproddesc]', 5 );
	INSERT INTO ED_AtriFuda
	(
		atfdcodi,
		atfdnote,
		atfddesc,
		atfdorde,
		atfdvisi,
		atfdfuda,
		atfdtida
	)
	VALUES
	(
		CONFEXME_16.tbrcED_AtriFuda( '32092' ).atfdcodi,
		'SBPRODDESC',
		'Sbproddesc',
		10,
		'S',
		CONFEXME_16.tbrcED_AtriFuda( '32092' ).atfdfuda,
		1
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el atributo
	nuNextSeqValue := pkBOInsertMgr.NextED_ATRIFUDA;
	CONFEXME_16.tbrcED_AtriFuda( '32093' ).atfdcodi := nuNextSeqValue;

	-- Se asigna la fuente de datos
	if ( CONFEXME_16.tbrcED_FuenDato.exists( '4064' ) ) then
		CONFEXME_16.tbrcED_AtriFuda( '32093' ).atfdfuda := CONFEXME_16.tbrcED_FuenDato( '4064' ).fudacodi;
	end if;

	UT_Trace.Trace( 'Insertando Fuente de Datos [Nuconcept]', 5 );
	INSERT INTO ED_AtriFuda
	(
		atfdcodi,
		atfdnote,
		atfddesc,
		atfdorde,
		atfdvisi,
		atfdfuda,
		atfdtida
	)
	VALUES
	(
		CONFEXME_16.tbrcED_AtriFuda( '32093' ).atfdcodi,
		'NUCONCEPT',
		'Nuconcept',
		11,
		'N',
		CONFEXME_16.tbrcED_AtriFuda( '32093' ).atfdfuda,
		1
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el atributo
	nuNextSeqValue := pkBOInsertMgr.NextED_ATRIFUDA;
	CONFEXME_16.tbrcED_AtriFuda( '32094' ).atfdcodi := nuNextSeqValue;

	-- Se asigna la fuente de datos
	if ( CONFEXME_16.tbrcED_FuenDato.exists( '4064' ) ) then
		CONFEXME_16.tbrcED_AtriFuda( '32094' ).atfdfuda := CONFEXME_16.tbrcED_FuenDato( '4064' ).fudacodi;
	end if;

	UT_Trace.Trace( 'Insertando Fuente de Datos [Sbconcdesc]', 5 );
	INSERT INTO ED_AtriFuda
	(
		atfdcodi,
		atfdnote,
		atfddesc,
		atfdorde,
		atfdvisi,
		atfdfuda,
		atfdtida
	)
	VALUES
	(
		CONFEXME_16.tbrcED_AtriFuda( '32094' ).atfdcodi,
		'SBCONCDESC',
		'Sbconcdesc',
		12,
		'S',
		CONFEXME_16.tbrcED_AtriFuda( '32094' ).atfdfuda,
		1
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el atributo
	nuNextSeqValue := pkBOInsertMgr.NextED_ATRIFUDA;
	CONFEXME_16.tbrcED_AtriFuda( '32095' ).atfdcodi := nuNextSeqValue;

	-- Se asigna la fuente de datos
	if ( CONFEXME_16.tbrcED_FuenDato.exists( '4064' ) ) then
		CONFEXME_16.tbrcED_AtriFuda( '32095' ).atfdfuda := CONFEXME_16.tbrcED_FuenDato( '4064' ).fudacodi;
	end if;

	UT_Trace.Trace( 'Insertando Fuente de Datos [Nuaccount]', 5 );
	INSERT INTO ED_AtriFuda
	(
		atfdcodi,
		atfdnote,
		atfddesc,
		atfdorde,
		atfdvisi,
		atfdfuda,
		atfdtida
	)
	VALUES
	(
		CONFEXME_16.tbrcED_AtriFuda( '32095' ).atfdcodi,
		'NUACCOUNT',
		'Nuaccount',
		13,
		'S',
		CONFEXME_16.tbrcED_AtriFuda( '32095' ).atfdfuda,
		1
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el atributo
	nuNextSeqValue := pkBOInsertMgr.NextED_ATRIFUDA;
	CONFEXME_16.tbrcED_AtriFuda( '32096' ).atfdcodi := nuNextSeqValue;

	-- Se asigna la fuente de datos
	if ( CONFEXME_16.tbrcED_FuenDato.exists( '4064' ) ) then
		CONFEXME_16.tbrcED_AtriFuda( '32096' ).atfdfuda := CONFEXME_16.tbrcED_FuenDato( '4064' ).fudacodi;
	end if;

	UT_Trace.Trace( 'Insertando Fuente de Datos [Nuprevbalance]', 5 );
	INSERT INTO ED_AtriFuda
	(
		atfdcodi,
		atfdnote,
		atfddesc,
		atfdorde,
		atfdvisi,
		atfdfuda,
		atfdtida
	)
	VALUES
	(
		CONFEXME_16.tbrcED_AtriFuda( '32096' ).atfdcodi,
		'NUPREVBALANCE',
		'Nuprevbalance',
		14,
		'S',
		CONFEXME_16.tbrcED_AtriFuda( '32096' ).atfdfuda,
		2
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el atributo
	nuNextSeqValue := pkBOInsertMgr.NextED_ATRIFUDA;
	CONFEXME_16.tbrcED_AtriFuda( '32097' ).atfdcodi := nuNextSeqValue;

	-- Se asigna la fuente de datos
	if ( CONFEXME_16.tbrcED_FuenDato.exists( '4064' ) ) then
		CONFEXME_16.tbrcED_AtriFuda( '32097' ).atfdfuda := CONFEXME_16.tbrcED_FuenDato( '4064' ).fudacodi;
	end if;

	UT_Trace.Trace( 'Insertando Fuente de Datos [Nupayappli]', 5 );
	INSERT INTO ED_AtriFuda
	(
		atfdcodi,
		atfdnote,
		atfddesc,
		atfdorde,
		atfdvisi,
		atfdfuda,
		atfdtida
	)
	VALUES
	(
		CONFEXME_16.tbrcED_AtriFuda( '32097' ).atfdcodi,
		'NUPAYAPPLI',
		'Nupayappli',
		15,
		'S',
		CONFEXME_16.tbrcED_AtriFuda( '32097' ).atfdfuda,
		2
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el atributo
	nuNextSeqValue := pkBOInsertMgr.NextED_ATRIFUDA;
	CONFEXME_16.tbrcED_AtriFuda( '32098' ).atfdcodi := nuNextSeqValue;

	-- Se asigna la fuente de datos
	if ( CONFEXME_16.tbrcED_FuenDato.exists( '4064' ) ) then
		CONFEXME_16.tbrcED_AtriFuda( '32098' ).atfdfuda := CONFEXME_16.tbrcED_FuenDato( '4064' ).fudacodi;
	end if;

	UT_Trace.Trace( 'Insertando Fuente de Datos [Nuapplibalance]', 5 );
	INSERT INTO ED_AtriFuda
	(
		atfdcodi,
		atfdnote,
		atfddesc,
		atfdorde,
		atfdvisi,
		atfdfuda,
		atfdtida
	)
	VALUES
	(
		CONFEXME_16.tbrcED_AtriFuda( '32098' ).atfdcodi,
		'NUAPPLIBALANCE',
		'Nuapplibalance',
		16,
		'S',
		CONFEXME_16.tbrcED_AtriFuda( '32098' ).atfdfuda,
		2
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el atributo
	nuNextSeqValue := pkBOInsertMgr.NextED_ATRIFUDA;
	CONFEXME_16.tbrcED_AtriFuda( '32099' ).atfdcodi := nuNextSeqValue;

	-- Se asigna la fuente de datos
	if ( CONFEXME_16.tbrcED_FuenDato.exists( '4064' ) ) then
		CONFEXME_16.tbrcED_AtriFuda( '32099' ).atfdfuda := CONFEXME_16.tbrcED_FuenDato( '4064' ).fudacodi;
	end if;

	UT_Trace.Trace( 'Insertando Fuente de Datos [Billliquidation]', 5 );
	INSERT INTO ED_AtriFuda
	(
		atfdcodi,
		atfdnote,
		atfddesc,
		atfdorde,
		atfdvisi,
		atfdfuda,
		atfdtida
	)
	VALUES
	(
		CONFEXME_16.tbrcED_AtriFuda( '32099' ).atfdcodi,
		'BILLLIQUIDATION',
		'Billliquidation',
		17,
		'S',
		CONFEXME_16.tbrcED_AtriFuda( '32099' ).atfdfuda,
		2
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el atributo
	nuNextSeqValue := pkBOInsertMgr.NextED_ATRIFUDA;
	CONFEXME_16.tbrcED_AtriFuda( '32100' ).atfdcodi := nuNextSeqValue;

	-- Se asigna la fuente de datos
	if ( CONFEXME_16.tbrcED_FuenDato.exists( '4065' ) ) then
		CONFEXME_16.tbrcED_AtriFuda( '32100' ).atfdfuda := CONFEXME_16.tbrcED_FuenDato( '4065' ).fudacodi;
	end if;

	UT_Trace.Trace( 'Insertando Fuente de Datos [TIP_PROD]', 5 );
	INSERT INTO ED_AtriFuda
	(
		atfdcodi,
		atfdnote,
		atfddesc,
		atfdorde,
		atfdvisi,
		atfdfuda,
		atfdtida
	)
	VALUES
	(
		CONFEXME_16.tbrcED_AtriFuda( '32100' ).atfdcodi,
		'TIP_PROD',
		'TIP_PROD',
		1,
		'S',
		CONFEXME_16.tbrcED_AtriFuda( '32100' ).atfdfuda,
		2
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el atributo
	nuNextSeqValue := pkBOInsertMgr.NextED_ATRIFUDA;
	CONFEXME_16.tbrcED_AtriFuda( '32101' ).atfdcodi := nuNextSeqValue;

	-- Se asigna la fuente de datos
	if ( CONFEXME_16.tbrcED_FuenDato.exists( '4065' ) ) then
		CONFEXME_16.tbrcED_AtriFuda( '32101' ).atfdfuda := CONFEXME_16.tbrcED_FuenDato( '4065' ).fudacodi;
	end if;

	UT_Trace.Trace( 'Insertando Fuente de Datos [TIPOPRODUCTO]', 5 );
	INSERT INTO ED_AtriFuda
	(
		atfdcodi,
		atfdnote,
		atfddesc,
		atfdorde,
		atfdvisi,
		atfdfuda,
		atfdtida
	)
	VALUES
	(
		CONFEXME_16.tbrcED_AtriFuda( '32101' ).atfdcodi,
		'TIPOPRODUCTO',
		'TIPOPRODUCTO',
		2,
		'S',
		CONFEXME_16.tbrcED_AtriFuda( '32101' ).atfdfuda,
		1
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el atributo
	nuNextSeqValue := pkBOInsertMgr.NextED_ATRIFUDA;
	CONFEXME_16.tbrcED_AtriFuda( '32102' ).atfdcodi := nuNextSeqValue;

	-- Se asigna la fuente de datos
	if ( CONFEXME_16.tbrcED_FuenDato.exists( '4065' ) ) then
		CONFEXME_16.tbrcED_AtriFuda( '32102' ).atfdfuda := CONFEXME_16.tbrcED_FuenDato( '4065' ).fudacodi;
	end if;

	UT_Trace.Trace( 'Insertando Fuente de Datos [FECHAGENFAC]', 5 );
	INSERT INTO ED_AtriFuda
	(
		atfdcodi,
		atfdnote,
		atfddesc,
		atfdorde,
		atfdvisi,
		atfdfuda,
		atfdtida
	)
	VALUES
	(
		CONFEXME_16.tbrcED_AtriFuda( '32102' ).atfdcodi,
		'FECHAGENFAC',
		'FECHAGENFAC',
		3,
		'S',
		CONFEXME_16.tbrcED_AtriFuda( '32102' ).atfdfuda,
		1
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el atributo
	nuNextSeqValue := pkBOInsertMgr.NextED_ATRIFUDA;
	CONFEXME_16.tbrcED_AtriFuda( '32103' ).atfdcodi := nuNextSeqValue;

	-- Se asigna la fuente de datos
	if ( CONFEXME_16.tbrcED_FuenDato.exists( '4065' ) ) then
		CONFEXME_16.tbrcED_AtriFuda( '32103' ).atfdfuda := CONFEXME_16.tbrcED_FuenDato( '4065' ).fudacodi;
	end if;

	UT_Trace.Trace( 'Insertando Fuente de Datos [TOTALFACT]', 5 );
	INSERT INTO ED_AtriFuda
	(
		atfdcodi,
		atfdnote,
		atfddesc,
		atfdorde,
		atfdvisi,
		atfdfuda,
		atfdtida
	)
	VALUES
	(
		CONFEXME_16.tbrcED_AtriFuda( '32103' ).atfdcodi,
		'TOTALFACT',
		'TOTALFACT',
		4,
		'S',
		CONFEXME_16.tbrcED_AtriFuda( '32103' ).atfdfuda,
		2
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el atributo
	nuNextSeqValue := pkBOInsertMgr.NextED_ATRIFUDA;
	CONFEXME_16.tbrcED_AtriFuda( '32104' ).atfdcodi := nuNextSeqValue;

	-- Se asigna la fuente de datos
	if ( CONFEXME_16.tbrcED_FuenDato.exists( '4065' ) ) then
		CONFEXME_16.tbrcED_AtriFuda( '32104' ).atfdfuda := CONFEXME_16.tbrcED_FuenDato( '4065' ).fudacodi;
	end if;

	UT_Trace.Trace( 'Insertando Fuente de Datos [FECHARECAUDO]', 5 );
	INSERT INTO ED_AtriFuda
	(
		atfdcodi,
		atfdnote,
		atfddesc,
		atfdorde,
		atfdvisi,
		atfdfuda,
		atfdtida
	)
	VALUES
	(
		CONFEXME_16.tbrcED_AtriFuda( '32104' ).atfdcodi,
		'FECHARECAUDO',
		'FECHARECAUDO',
		5,
		'S',
		CONFEXME_16.tbrcED_AtriFuda( '32104' ).atfdfuda,
		1
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el atributo
	nuNextSeqValue := pkBOInsertMgr.NextED_ATRIFUDA;
	CONFEXME_16.tbrcED_AtriFuda( '32105' ).atfdcodi := nuNextSeqValue;

	-- Se asigna la fuente de datos
	if ( CONFEXME_16.tbrcED_FuenDato.exists( '4065' ) ) then
		CONFEXME_16.tbrcED_AtriFuda( '32105' ).atfdfuda := CONFEXME_16.tbrcED_FuenDato( '4065' ).fudacodi;
	end if;

	UT_Trace.Trace( 'Insertando Fuente de Datos [CUENTACOBROPAGO]', 5 );
	INSERT INTO ED_AtriFuda
	(
		atfdcodi,
		atfdnote,
		atfddesc,
		atfdorde,
		atfdvisi,
		atfdfuda,
		atfdtida
	)
	VALUES
	(
		CONFEXME_16.tbrcED_AtriFuda( '32105' ).atfdcodi,
		'CUENTACOBROPAGO',
		'CUENTACOBROPAGO',
		6,
		'S',
		CONFEXME_16.tbrcED_AtriFuda( '32105' ).atfdfuda,
		2
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el atributo
	nuNextSeqValue := pkBOInsertMgr.NextED_ATRIFUDA;
	CONFEXME_16.tbrcED_AtriFuda( '32106' ).atfdcodi := nuNextSeqValue;

	-- Se asigna la fuente de datos
	if ( CONFEXME_16.tbrcED_FuenDato.exists( '4065' ) ) then
		CONFEXME_16.tbrcED_AtriFuda( '32106' ).atfdfuda := CONFEXME_16.tbrcED_FuenDato( '4065' ).fudacodi;
	end if;

	UT_Trace.Trace( 'Insertando Fuente de Datos [VALORRECAUDO]', 5 );
	INSERT INTO ED_AtriFuda
	(
		atfdcodi,
		atfdnote,
		atfddesc,
		atfdorde,
		atfdvisi,
		atfdfuda,
		atfdtida
	)
	VALUES
	(
		CONFEXME_16.tbrcED_AtriFuda( '32106' ).atfdcodi,
		'VALORRECAUDO',
		'VALORRECAUDO',
		7,
		'S',
		CONFEXME_16.tbrcED_AtriFuda( '32106' ).atfdfuda,
		2
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el atributo
	nuNextSeqValue := pkBOInsertMgr.NextED_ATRIFUDA;
	CONFEXME_16.tbrcED_AtriFuda( '32107' ).atfdcodi := nuNextSeqValue;

	-- Se asigna la fuente de datos
	if ( CONFEXME_16.tbrcED_FuenDato.exists( '4065' ) ) then
		CONFEXME_16.tbrcED_AtriFuda( '32107' ).atfdfuda := CONFEXME_16.tbrcED_FuenDato( '4065' ).fudacodi;
	end if;

	UT_Trace.Trace( 'Insertando Fuente de Datos [FECHAAPLICACIONRECAUDO]', 5 );
	INSERT INTO ED_AtriFuda
	(
		atfdcodi,
		atfdnote,
		atfddesc,
		atfdorde,
		atfdvisi,
		atfdfuda,
		atfdtida
	)
	VALUES
	(
		CONFEXME_16.tbrcED_AtriFuda( '32107' ).atfdcodi,
		'FECHAAPLICACIONRECAUDO',
		'FECHAAPLICACIONRECAUDO',
		8,
		'S',
		CONFEXME_16.tbrcED_AtriFuda( '32107' ).atfdfuda,
		1
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el atributo
	nuNextSeqValue := pkBOInsertMgr.NextED_ATRIFUDA;
	CONFEXME_16.tbrcED_AtriFuda( '32108' ).atfdcodi := nuNextSeqValue;

	-- Se asigna la fuente de datos
	if ( CONFEXME_16.tbrcED_FuenDato.exists( '4065' ) ) then
		CONFEXME_16.tbrcED_AtriFuda( '32108' ).atfdfuda := CONFEXME_16.tbrcED_FuenDato( '4065' ).fudacodi;
	end if;

	UT_Trace.Trace( 'Insertando Fuente de Datos [ENTIDADRECAUDADORA]', 5 );
	INSERT INTO ED_AtriFuda
	(
		atfdcodi,
		atfdnote,
		atfddesc,
		atfdorde,
		atfdvisi,
		atfdfuda,
		atfdtida
	)
	VALUES
	(
		CONFEXME_16.tbrcED_AtriFuda( '32108' ).atfdcodi,
		'ENTIDADRECAUDADORA',
		'ENTIDADRECAUDADORA',
		9,
		'S',
		CONFEXME_16.tbrcED_AtriFuda( '32108' ).atfdfuda,
		1
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el atributo
	nuNextSeqValue := pkBOInsertMgr.NextED_ATRIFUDA;
	CONFEXME_16.tbrcED_AtriFuda( '32109' ).atfdcodi := nuNextSeqValue;

	-- Se asigna la fuente de datos
	if ( CONFEXME_16.tbrcED_FuenDato.exists( '4066' ) ) then
		CONFEXME_16.tbrcED_AtriFuda( '32109' ).atfdfuda := CONFEXME_16.tbrcED_FuenDato( '4066' ).fudacodi;
	end if;

	UT_Trace.Trace( 'Insertando Fuente de Datos [NOM_EMPRESA]', 5 );
	INSERT INTO ED_AtriFuda
	(
		atfdcodi,
		atfdnote,
		atfddesc,
		atfdorde,
		atfdvisi,
		atfdfuda,
		atfdtida
	)
	VALUES
	(
		CONFEXME_16.tbrcED_AtriFuda( '32109' ).atfdcodi,
		'NOM_EMPRESA',
		'NOM_EMPRESA',
		1,
		'S',
		CONFEXME_16.tbrcED_AtriFuda( '32109' ).atfdfuda,
		1
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el atributo
	nuNextSeqValue := pkBOInsertMgr.NextED_ATRIFUDA;
	CONFEXME_16.tbrcED_AtriFuda( '32110' ).atfdcodi := nuNextSeqValue;

	-- Se asigna la fuente de datos
	if ( CONFEXME_16.tbrcED_FuenDato.exists( '4066' ) ) then
		CONFEXME_16.tbrcED_AtriFuda( '32110' ).atfdfuda := CONFEXME_16.tbrcED_FuenDato( '4066' ).fudacodi;
	end if;

	UT_Trace.Trace( 'Insertando Fuente de Datos [CONTRATO]', 5 );
	INSERT INTO ED_AtriFuda
	(
		atfdcodi,
		atfdnote,
		atfddesc,
		atfdorde,
		atfdvisi,
		atfdfuda,
		atfdtida
	)
	VALUES
	(
		CONFEXME_16.tbrcED_AtriFuda( '32110' ).atfdcodi,
		'CONTRATO',
		'CONTRATO',
		2,
		'S',
		CONFEXME_16.tbrcED_AtriFuda( '32110' ).atfdfuda,
		2
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el atributo
	nuNextSeqValue := pkBOInsertMgr.NextED_ATRIFUDA;
	CONFEXME_16.tbrcED_AtriFuda( '32111' ).atfdcodi := nuNextSeqValue;

	-- Se asigna la fuente de datos
	if ( CONFEXME_16.tbrcED_FuenDato.exists( '4066' ) ) then
		CONFEXME_16.tbrcED_AtriFuda( '32111' ).atfdfuda := CONFEXME_16.tbrcED_FuenDato( '4066' ).fudacodi;
	end if;

	UT_Trace.Trace( 'Insertando Fuente de Datos [DIRECCION]', 5 );
	INSERT INTO ED_AtriFuda
	(
		atfdcodi,
		atfdnote,
		atfddesc,
		atfdorde,
		atfdvisi,
		atfdfuda,
		atfdtida
	)
	VALUES
	(
		CONFEXME_16.tbrcED_AtriFuda( '32111' ).atfdcodi,
		'DIRECCION',
		'DIRECCION',
		3,
		'S',
		CONFEXME_16.tbrcED_AtriFuda( '32111' ).atfdfuda,
		1
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el atributo
	nuNextSeqValue := pkBOInsertMgr.NextED_ATRIFUDA;
	CONFEXME_16.tbrcED_AtriFuda( '32112' ).atfdcodi := nuNextSeqValue;

	-- Se asigna la fuente de datos
	if ( CONFEXME_16.tbrcED_FuenDato.exists( '4066' ) ) then
		CONFEXME_16.tbrcED_AtriFuda( '32112' ).atfdfuda := CONFEXME_16.tbrcED_FuenDato( '4066' ).fudacodi;
	end if;

	UT_Trace.Trace( 'Insertando Fuente de Datos [MUNICIPIO]', 5 );
	INSERT INTO ED_AtriFuda
	(
		atfdcodi,
		atfdnote,
		atfddesc,
		atfdorde,
		atfdvisi,
		atfdfuda,
		atfdtida
	)
	VALUES
	(
		CONFEXME_16.tbrcED_AtriFuda( '32112' ).atfdcodi,
		'MUNICIPIO',
		'MUNICIPIO',
		4,
		'S',
		CONFEXME_16.tbrcED_AtriFuda( '32112' ).atfdfuda,
		1
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el atributo
	nuNextSeqValue := pkBOInsertMgr.NextED_ATRIFUDA;
	CONFEXME_16.tbrcED_AtriFuda( '32113' ).atfdcodi := nuNextSeqValue;

	-- Se asigna la fuente de datos
	if ( CONFEXME_16.tbrcED_FuenDato.exists( '4066' ) ) then
		CONFEXME_16.tbrcED_AtriFuda( '32113' ).atfdfuda := CONFEXME_16.tbrcED_FuenDato( '4066' ).fudacodi;
	end if;

	UT_Trace.Trace( 'Insertando Fuente de Datos [NOMBRE_CLIENTE]', 5 );
	INSERT INTO ED_AtriFuda
	(
		atfdcodi,
		atfdnote,
		atfddesc,
		atfdorde,
		atfdvisi,
		atfdfuda,
		atfdtida
	)
	VALUES
	(
		CONFEXME_16.tbrcED_AtriFuda( '32113' ).atfdcodi,
		'NOMBRE_CLIENTE',
		'NOMBRE_CLIENTE',
		5,
		'S',
		CONFEXME_16.tbrcED_AtriFuda( '32113' ).atfdfuda,
		1
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el atributo
	nuNextSeqValue := pkBOInsertMgr.NextED_ATRIFUDA;
	CONFEXME_16.tbrcED_AtriFuda( '32114' ).atfdcodi := nuNextSeqValue;

	-- Se asigna la fuente de datos
	if ( CONFEXME_16.tbrcED_FuenDato.exists( '4066' ) ) then
		CONFEXME_16.tbrcED_AtriFuda( '32114' ).atfdfuda := CONFEXME_16.tbrcED_FuenDato( '4066' ).fudacodi;
	end if;

	UT_Trace.Trace( 'Insertando Fuente de Datos [APELLIDO_CLIENTE]', 5 );
	INSERT INTO ED_AtriFuda
	(
		atfdcodi,
		atfdnote,
		atfddesc,
		atfdorde,
		atfdvisi,
		atfdfuda,
		atfdtida
	)
	VALUES
	(
		CONFEXME_16.tbrcED_AtriFuda( '32114' ).atfdcodi,
		'APELLIDO_CLIENTE',
		'APELLIDO_CLIENTE',
		6,
		'S',
		CONFEXME_16.tbrcED_AtriFuda( '32114' ).atfdfuda,
		1
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el atributo
	nuNextSeqValue := pkBOInsertMgr.NextED_ATRIFUDA;
	CONFEXME_16.tbrcED_AtriFuda( '32115' ).atfdcodi := nuNextSeqValue;

	-- Se asigna la fuente de datos
	if ( CONFEXME_16.tbrcED_FuenDato.exists( '4066' ) ) then
		CONFEXME_16.tbrcED_AtriFuda( '32115' ).atfdfuda := CONFEXME_16.tbrcED_FuenDato( '4066' ).fudacodi;
	end if;

	UT_Trace.Trace( 'Insertando Fuente de Datos [NOM_SOLICITANTE]', 5 );
	INSERT INTO ED_AtriFuda
	(
		atfdcodi,
		atfdnote,
		atfddesc,
		atfdorde,
		atfdvisi,
		atfdfuda,
		atfdtida
	)
	VALUES
	(
		CONFEXME_16.tbrcED_AtriFuda( '32115' ).atfdcodi,
		'NOM_SOLICITANTE',
		'NOM_SOLICITANTE',
		7,
		'S',
		CONFEXME_16.tbrcED_AtriFuda( '32115' ).atfdfuda,
		1
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el atributo
	nuNextSeqValue := pkBOInsertMgr.NextED_ATRIFUDA;
	CONFEXME_16.tbrcED_AtriFuda( '32116' ).atfdcodi := nuNextSeqValue;

	-- Se asigna la fuente de datos
	if ( CONFEXME_16.tbrcED_FuenDato.exists( '4066' ) ) then
		CONFEXME_16.tbrcED_AtriFuda( '32116' ).atfdfuda := CONFEXME_16.tbrcED_FuenDato( '4066' ).fudacodi;
	end if;

	UT_Trace.Trace( 'Insertando Fuente de Datos [DOC_SOLICITANTE]', 5 );
	INSERT INTO ED_AtriFuda
	(
		atfdcodi,
		atfdnote,
		atfddesc,
		atfdorde,
		atfdvisi,
		atfdfuda,
		atfdtida
	)
	VALUES
	(
		CONFEXME_16.tbrcED_AtriFuda( '32116' ).atfdcodi,
		'DOC_SOLICITANTE',
		'DOC_SOLICITANTE',
		8,
		'S',
		CONFEXME_16.tbrcED_AtriFuda( '32116' ).atfdfuda,
		1
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el atributo
	nuNextSeqValue := pkBOInsertMgr.NextED_ATRIFUDA;
	CONFEXME_16.tbrcED_AtriFuda( '32117' ).atfdcodi := nuNextSeqValue;

	-- Se asigna la fuente de datos
	if ( CONFEXME_16.tbrcED_FuenDato.exists( '4066' ) ) then
		CONFEXME_16.tbrcED_AtriFuda( '32117' ).atfdfuda := CONFEXME_16.tbrcED_FuenDato( '4066' ).fudacodi;
	end if;

	UT_Trace.Trace( 'Insertando Fuente de Datos [MUN_SOLICITANTE]', 5 );
	INSERT INTO ED_AtriFuda
	(
		atfdcodi,
		atfdnote,
		atfddesc,
		atfdorde,
		atfdvisi,
		atfdfuda,
		atfdtida
	)
	VALUES
	(
		CONFEXME_16.tbrcED_AtriFuda( '32117' ).atfdcodi,
		'MUN_SOLICITANTE',
		'MUN_SOLICITANTE',
		9,
		'S',
		CONFEXME_16.tbrcED_AtriFuda( '32117' ).atfdfuda,
		1
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el atributo
	nuNextSeqValue := pkBOInsertMgr.NextED_ATRIFUDA;
	CONFEXME_16.tbrcED_AtriFuda( '32118' ).atfdcodi := nuNextSeqValue;

	-- Se asigna la fuente de datos
	if ( CONFEXME_16.tbrcED_FuenDato.exists( '4066' ) ) then
		CONFEXME_16.tbrcED_AtriFuda( '32118' ).atfdfuda := CONFEXME_16.tbrcED_FuenDato( '4066' ).fudacodi;
	end if;

	UT_Trace.Trace( 'Insertando Fuente de Datos [FECHA]', 5 );
	INSERT INTO ED_AtriFuda
	(
		atfdcodi,
		atfdnote,
		atfddesc,
		atfdorde,
		atfdvisi,
		atfdfuda,
		atfdtida
	)
	VALUES
	(
		CONFEXME_16.tbrcED_AtriFuda( '32118' ).atfdcodi,
		'FECHA',
		'FECHA',
		10,
		'S',
		CONFEXME_16.tbrcED_AtriFuda( '32118' ).atfdfuda,
		1
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el atributo
	nuNextSeqValue := pkBOInsertMgr.NextED_ATRIFUDA;
	CONFEXME_16.tbrcED_AtriFuda( '32119' ).atfdcodi := nuNextSeqValue;

	-- Se asigna la fuente de datos
	if ( CONFEXME_16.tbrcED_FuenDato.exists( '4066' ) ) then
		CONFEXME_16.tbrcED_AtriFuda( '32119' ).atfdfuda := CONFEXME_16.tbrcED_FuenDato( '4066' ).fudacodi;
	end if;

	UT_Trace.Trace( 'Insertando Fuente de Datos [DIA]', 5 );
	INSERT INTO ED_AtriFuda
	(
		atfdcodi,
		atfdnote,
		atfddesc,
		atfdorde,
		atfdvisi,
		atfdfuda,
		atfdtida
	)
	VALUES
	(
		CONFEXME_16.tbrcED_AtriFuda( '32119' ).atfdcodi,
		'DIA',
		'DIA',
		11,
		'S',
		CONFEXME_16.tbrcED_AtriFuda( '32119' ).atfdfuda,
		1
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el atributo
	nuNextSeqValue := pkBOInsertMgr.NextED_ATRIFUDA;
	CONFEXME_16.tbrcED_AtriFuda( '32120' ).atfdcodi := nuNextSeqValue;

	-- Se asigna la fuente de datos
	if ( CONFEXME_16.tbrcED_FuenDato.exists( '4066' ) ) then
		CONFEXME_16.tbrcED_AtriFuda( '32120' ).atfdfuda := CONFEXME_16.tbrcED_FuenDato( '4066' ).fudacodi;
	end if;

	UT_Trace.Trace( 'Insertando Fuente de Datos [MES]', 5 );
	INSERT INTO ED_AtriFuda
	(
		atfdcodi,
		atfdnote,
		atfddesc,
		atfdorde,
		atfdvisi,
		atfdfuda,
		atfdtida
	)
	VALUES
	(
		CONFEXME_16.tbrcED_AtriFuda( '32120' ).atfdcodi,
		'MES',
		'MES',
		12,
		'S',
		CONFEXME_16.tbrcED_AtriFuda( '32120' ).atfdfuda,
		1
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el atributo
	nuNextSeqValue := pkBOInsertMgr.NextED_ATRIFUDA;
	CONFEXME_16.tbrcED_AtriFuda( '32121' ).atfdcodi := nuNextSeqValue;

	-- Se asigna la fuente de datos
	if ( CONFEXME_16.tbrcED_FuenDato.exists( '4066' ) ) then
		CONFEXME_16.tbrcED_AtriFuda( '32121' ).atfdfuda := CONFEXME_16.tbrcED_FuenDato( '4066' ).fudacodi;
	end if;

	UT_Trace.Trace( 'Insertando Fuente de Datos [ANO]', 5 );
	INSERT INTO ED_AtriFuda
	(
		atfdcodi,
		atfdnote,
		atfddesc,
		atfdorde,
		atfdvisi,
		atfdfuda,
		atfdtida
	)
	VALUES
	(
		CONFEXME_16.tbrcED_AtriFuda( '32121' ).atfdcodi,
		'ANO',
		'ANO',
		13,
		'S',
		CONFEXME_16.tbrcED_AtriFuda( '32121' ).atfdfuda,
		1
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el atributo
	nuNextSeqValue := pkBOInsertMgr.NextED_ATRIFUDA;
	CONFEXME_16.tbrcED_AtriFuda( '32122' ).atfdcodi := nuNextSeqValue;

	-- Se asigna la fuente de datos
	if ( CONFEXME_16.tbrcED_FuenDato.exists( '4066' ) ) then
		CONFEXME_16.tbrcED_AtriFuda( '32122' ).atfdfuda := CONFEXME_16.tbrcED_FuenDato( '4066' ).fudacodi;
	end if;

	UT_Trace.Trace( 'Insertando Fuente de Datos [MUNICIPIO_ATENCION]', 5 );
	INSERT INTO ED_AtriFuda
	(
		atfdcodi,
		atfdnote,
		atfddesc,
		atfdorde,
		atfdvisi,
		atfdfuda,
		atfdtida
	)
	VALUES
	(
		CONFEXME_16.tbrcED_AtriFuda( '32122' ).atfdcodi,
		'MUNICIPIO_ATENCION',
		'MUNICIPIO_ATENCION',
		14,
		'S',
		CONFEXME_16.tbrcED_AtriFuda( '32122' ).atfdfuda,
		1
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

BEGIN
--{
	UT_Trace.Trace( '****************************** Generando bloques *******************************', 5 );
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el bloque
	nuNextSeqValue := pkBOInsertMgr.NextED_BLOQUE;
	CONFEXME_16.tbrcED_Bloque( 6993 ).bloqcodi := nuNextSeqValue;

	-- Se asigna la fuente de datos del bloque
	if ( CONFEXME_16.tbrcED_FuenDato.exists( '4064' ) ) then
		CONFEXME_16.tbrcED_Bloque( 6993 ).bloqfuda := CONFEXME_16.tbrcED_FuenDato( '4064' ).fudacodi;
	end if;

	UT_Trace.Trace( 'Insertando Bloque [APLICACION_PAGOS]', 5 );
	INSERT INTO ED_Bloque
	(
		bloqcodi,
		bloqdesc,
		bloqtibl,
		bloqfuda,
		bloqdein,
		bloqdefi
	)
	VALUES
	(
		CONFEXME_16.tbrcED_Bloque( 6993 ).bloqcodi,
		'APLICACION_PAGOS',
		CONFEXME_16.tbrcED_Bloque( 6993 ).bloqtibl,
		CONFEXME_16.tbrcED_Bloque( 6993 ).bloqfuda,
		'<APLICACION_PAGOS>',
		'</APLICACION_PAGOS>'
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el bloque
	nuNextSeqValue := pkBOInsertMgr.NextED_BLOQUE;
	CONFEXME_16.tbrcED_Bloque( 6994 ).bloqcodi := nuNextSeqValue;

	-- Se asigna la fuente de datos del bloque
	if ( CONFEXME_16.tbrcED_FuenDato.exists( '4065' ) ) then
		CONFEXME_16.tbrcED_Bloque( 6994 ).bloqfuda := CONFEXME_16.tbrcED_FuenDato( '4065' ).fudacodi;
	end if;

	UT_Trace.Trace( 'Insertando Bloque [DETALLE_PAGOS]', 5 );
	INSERT INTO ED_Bloque
	(
		bloqcodi,
		bloqdesc,
		bloqtibl,
		bloqfuda,
		bloqdein,
		bloqdefi
	)
	VALUES
	(
		CONFEXME_16.tbrcED_Bloque( 6994 ).bloqcodi,
		'DETALLE_PAGOS',
		CONFEXME_16.tbrcED_Bloque( 6994 ).bloqtibl,
		CONFEXME_16.tbrcED_Bloque( 6994 ).bloqfuda,
		'<DETALLE_PAGOS>',
		'</DETALLE_PAGOS>'
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el bloque
	nuNextSeqValue := pkBOInsertMgr.NextED_BLOQUE;
	CONFEXME_16.tbrcED_Bloque( 6995 ).bloqcodi := nuNextSeqValue;

	-- Se asigna la fuente de datos del bloque
	if ( CONFEXME_16.tbrcED_FuenDato.exists( '4066' ) ) then
		CONFEXME_16.tbrcED_Bloque( 6995 ).bloqfuda := CONFEXME_16.tbrcED_FuenDato( '4066' ).fudacodi;
	end if;

	UT_Trace.Trace( 'Insertando Bloque [DATA_INFORMACION]', 5 );
	INSERT INTO ED_Bloque
	(
		bloqcodi,
		bloqdesc,
		bloqtibl,
		bloqfuda,
		bloqdein,
		bloqdefi
	)
	VALUES
	(
		CONFEXME_16.tbrcED_Bloque( 6995 ).bloqcodi,
		'DATA_INFORMACION',
		CONFEXME_16.tbrcED_Bloque( 6995 ).bloqtibl,
		CONFEXME_16.tbrcED_Bloque( 6995 ).bloqfuda,
		'<DATA_INFORMACION>',
		'</DATA_INFORMACION>'
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el bloque
	nuNextSeqValue := pkBOInsertMgr.NextED_BLOQUE;
	CONFEXME_16.tbrcED_Bloque( 6996 ).bloqcodi := nuNextSeqValue;

	-- Se asigna la fuente de datos del bloque
	if ( CONFEXME_16.tbrcED_FuenDato.exists( '4067' ) ) then
		CONFEXME_16.tbrcED_Bloque( 6996 ).bloqfuda := CONFEXME_16.tbrcED_FuenDato( '4067' ).fudacodi;
	end if;

	UT_Trace.Trace( 'Insertando Bloque [DATA_FECHAS]', 5 );
	INSERT INTO ED_Bloque
	(
		bloqcodi,
		bloqdesc,
		bloqtibl,
		bloqfuda,
		bloqdein,
		bloqdefi
	)
	VALUES
	(
		CONFEXME_16.tbrcED_Bloque( 6996 ).bloqcodi,
		'DATA_FECHAS',
		CONFEXME_16.tbrcED_Bloque( 6996 ).bloqtibl,
		CONFEXME_16.tbrcED_Bloque( 6996 ).bloqfuda,
		'<DATA_FECHAS>',
		'</DATA_FECHAS>'
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

BEGIN
--{
	UT_Trace.Trace( '********************* Generando bloques por franja-formato *********************', 5 );
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el bloque-franja
	nuNextSeqValue := pkBOInsertMgr.NextED_BLOQFRAN;
	CONFEXME_16.tbrcED_BloqFran( '7157' ).blfrcodi := nuNextSeqValue;

	-- Se asigna la franja-formato
	CONFEXME_16.tbrcED_BloqFran( '7157' ).blfrfrfo := CONFEXME_16.tbrcED_FranForm( '4776' ).frfocodi;

	-- Se asigna el bloque
	if ( CONFEXME_16.tbrcED_Bloque.exists( 6993 ) ) then
		CONFEXME_16.tbrcED_BloqFran( '7157' ).blfrbloq := CONFEXME_16.tbrcED_Bloque( 6993 ).bloqcodi;
	end if;

	INSERT INTO ED_BloqFran
	(
		blfrcodi,
		blfrbloq,
		blfrfrfo,
		blfrorde,
		blfrmult,
		blfrsait
	)
	VALUES
	(
		CONFEXME_16.tbrcED_BloqFran( '7157' ).blfrcodi,
		CONFEXME_16.tbrcED_BloqFran( '7157' ).blfrbloq,
		CONFEXME_16.tbrcED_BloqFran( '7157' ).blfrfrfo,
		4,
		'S',
		'I'
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el bloque-franja
	nuNextSeqValue := pkBOInsertMgr.NextED_BLOQFRAN;
	CONFEXME_16.tbrcED_BloqFran( '7158' ).blfrcodi := nuNextSeqValue;

	-- Se asigna la franja-formato
	CONFEXME_16.tbrcED_BloqFran( '7158' ).blfrfrfo := CONFEXME_16.tbrcED_FranForm( '4776' ).frfocodi;

	-- Se asigna el bloque
	if ( CONFEXME_16.tbrcED_Bloque.exists( 6994 ) ) then
		CONFEXME_16.tbrcED_BloqFran( '7158' ).blfrbloq := CONFEXME_16.tbrcED_Bloque( 6994 ).bloqcodi;
	end if;

	INSERT INTO ED_BloqFran
	(
		blfrcodi,
		blfrbloq,
		blfrfrfo,
		blfrorde,
		blfrmult,
		blfrsait
	)
	VALUES
	(
		CONFEXME_16.tbrcED_BloqFran( '7158' ).blfrcodi,
		CONFEXME_16.tbrcED_BloqFran( '7158' ).blfrbloq,
		CONFEXME_16.tbrcED_BloqFran( '7158' ).blfrfrfo,
		3,
		'S',
		'I'
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el bloque-franja
	nuNextSeqValue := pkBOInsertMgr.NextED_BLOQFRAN;
	CONFEXME_16.tbrcED_BloqFran( '7159' ).blfrcodi := nuNextSeqValue;

	-- Se asigna la franja-formato
	CONFEXME_16.tbrcED_BloqFran( '7159' ).blfrfrfo := CONFEXME_16.tbrcED_FranForm( '4777' ).frfocodi;

	-- Se asigna el bloque
	if ( CONFEXME_16.tbrcED_Bloque.exists( 6995 ) ) then
		CONFEXME_16.tbrcED_BloqFran( '7159' ).blfrbloq := CONFEXME_16.tbrcED_Bloque( 6995 ).bloqcodi;
	end if;

	INSERT INTO ED_BloqFran
	(
		blfrcodi,
		blfrbloq,
		blfrfrfo,
		blfrorde,
		blfrmult,
		blfrsait
	)
	VALUES
	(
		CONFEXME_16.tbrcED_BloqFran( '7159' ).blfrcodi,
		CONFEXME_16.tbrcED_BloqFran( '7159' ).blfrbloq,
		CONFEXME_16.tbrcED_BloqFran( '7159' ).blfrfrfo,
		0,
		'S',
		'I'
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el bloque-franja
	nuNextSeqValue := pkBOInsertMgr.NextED_BLOQFRAN;
	CONFEXME_16.tbrcED_BloqFran( '7160' ).blfrcodi := nuNextSeqValue;

	-- Se asigna la franja-formato
	CONFEXME_16.tbrcED_BloqFran( '7160' ).blfrfrfo := CONFEXME_16.tbrcED_FranForm( '4777' ).frfocodi;

	-- Se asigna el bloque
	if ( CONFEXME_16.tbrcED_Bloque.exists( 6996 ) ) then
		CONFEXME_16.tbrcED_BloqFran( '7160' ).blfrbloq := CONFEXME_16.tbrcED_Bloque( 6996 ).bloqcodi;
	end if;

	INSERT INTO ED_BloqFran
	(
		blfrcodi,
		blfrbloq,
		blfrfrfo,
		blfrorde,
		blfrmult,
		blfrsait
	)
	VALUES
	(
		CONFEXME_16.tbrcED_BloqFran( '7160' ).blfrcodi,
		CONFEXME_16.tbrcED_BloqFran( '7160' ).blfrbloq,
		CONFEXME_16.tbrcED_BloqFran( '7160' ).blfrfrfo,
		0,
		'S',
		'I'
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

BEGIN
--{
	UT_Trace.Trace( '**************************** Generando expresiones *****************************', 5 );
--}
END;
/

BEGIN
--{
	UT_Trace.Trace( '******************************* Generando ítems ********************************', 5 );
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el item
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEM;
	CONFEXME_16.tbrcED_Item( '37353' ).itemcodi := nuNextSeqValue;

	-- Se establece el nombre del objeto por defecto
	CONFEXME_16.tbrcED_Item( '37353' ).itemobna := NULL;

	-- Se asigna el grupo de entidad asociado al item
	if ( CONFEXME_16.tbrcED_AtriFuda.exists( '32077' ) ) then
		CONFEXME_16.tbrcED_Item( '37353' ).itematfd := CONFEXME_16.tbrcED_AtriFuda( '32077' ).atfdcodi;
	end if;

	UT_Trace.Trace( 'Insertando Item [FECHAINI]', 5 );
	INSERT INTO ED_Item
	(
		itemcodi,
		itemdesc,
		itemceid,
		itemcons,
		itemobna,
		itemlong,
		itemalin,
		itematri,
		itemgren,
		itemmasc,
		itemtida,
		itemdein,
		itemdefi,
		itematfd
	)
	VALUES
	(
		CONFEXME_16.tbrcED_Item( '37353' ).itemcodi,
		'FECHAINI',
		CONFEXME_16.tbrcED_Item( '37353' ).itemceid,
		NULL,
		CONFEXME_16.tbrcED_Item( '37353' ).itemobna,
		0,
		NULL,
		NULL,
		CONFEXME_16.tbrcED_Item( '37353' ).itemgren,
		NULL,
		1,
		'<FECHAINI>',
		'</FECHAINI>',
		CONFEXME_16.tbrcED_Item( '37353' ).itematfd
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el item
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEM;
	CONFEXME_16.tbrcED_Item( '37354' ).itemcodi := nuNextSeqValue;

	-- Se establece el nombre del objeto por defecto
	CONFEXME_16.tbrcED_Item( '37354' ).itemobna := NULL;

	-- Se asigna el grupo de entidad asociado al item
	if ( CONFEXME_16.tbrcED_AtriFuda.exists( '32078' ) ) then
		CONFEXME_16.tbrcED_Item( '37354' ).itematfd := CONFEXME_16.tbrcED_AtriFuda( '32078' ).atfdcodi;
	end if;

	UT_Trace.Trace( 'Insertando Item [FECHAFIN]', 5 );
	INSERT INTO ED_Item
	(
		itemcodi,
		itemdesc,
		itemceid,
		itemcons,
		itemobna,
		itemlong,
		itemalin,
		itematri,
		itemgren,
		itemmasc,
		itemtida,
		itemdein,
		itemdefi,
		itematfd
	)
	VALUES
	(
		CONFEXME_16.tbrcED_Item( '37354' ).itemcodi,
		'FECHAFIN',
		CONFEXME_16.tbrcED_Item( '37354' ).itemceid,
		NULL,
		CONFEXME_16.tbrcED_Item( '37354' ).itemobna,
		0,
		NULL,
		NULL,
		CONFEXME_16.tbrcED_Item( '37354' ).itemgren,
		NULL,
		1,
		'<FECHAFIN>',
		'</FECHAFIN>',
		CONFEXME_16.tbrcED_Item( '37354' ).itematfd
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el item
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEM;
	CONFEXME_16.tbrcED_Item( '37355' ).itemcodi := nuNextSeqValue;

	-- Se establece el nombre del objeto por defecto
	CONFEXME_16.tbrcED_Item( '37355' ).itemobna := NULL;

	-- Se asigna el grupo de entidad asociado al item
	if ( CONFEXME_16.tbrcED_AtriFuda.exists( '32079' ) ) then
		CONFEXME_16.tbrcED_Item( '37355' ).itematfd := CONFEXME_16.tbrcED_AtriFuda( '32079' ).atfdcodi;
	end if;

	UT_Trace.Trace( 'Insertando Item [MESINI]', 5 );
	INSERT INTO ED_Item
	(
		itemcodi,
		itemdesc,
		itemceid,
		itemcons,
		itemobna,
		itemlong,
		itemalin,
		itematri,
		itemgren,
		itemmasc,
		itemtida,
		itemdein,
		itemdefi,
		itematfd
	)
	VALUES
	(
		CONFEXME_16.tbrcED_Item( '37355' ).itemcodi,
		'MESINI',
		CONFEXME_16.tbrcED_Item( '37355' ).itemceid,
		NULL,
		CONFEXME_16.tbrcED_Item( '37355' ).itemobna,
		0,
		NULL,
		NULL,
		CONFEXME_16.tbrcED_Item( '37355' ).itemgren,
		NULL,
		1,
		'<MESINI>',
		'</MESINI>',
		CONFEXME_16.tbrcED_Item( '37355' ).itematfd
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el item
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEM;
	CONFEXME_16.tbrcED_Item( '37356' ).itemcodi := nuNextSeqValue;

	-- Se establece el nombre del objeto por defecto
	CONFEXME_16.tbrcED_Item( '37356' ).itemobna := NULL;

	-- Se asigna el grupo de entidad asociado al item
	if ( CONFEXME_16.tbrcED_AtriFuda.exists( '32080' ) ) then
		CONFEXME_16.tbrcED_Item( '37356' ).itematfd := CONFEXME_16.tbrcED_AtriFuda( '32080' ).atfdcodi;
	end if;

	UT_Trace.Trace( 'Insertando Item [ANOINI]', 5 );
	INSERT INTO ED_Item
	(
		itemcodi,
		itemdesc,
		itemceid,
		itemcons,
		itemobna,
		itemlong,
		itemalin,
		itematri,
		itemgren,
		itemmasc,
		itemtida,
		itemdein,
		itemdefi,
		itematfd
	)
	VALUES
	(
		CONFEXME_16.tbrcED_Item( '37356' ).itemcodi,
		'ANOINI',
		CONFEXME_16.tbrcED_Item( '37356' ).itemceid,
		NULL,
		CONFEXME_16.tbrcED_Item( '37356' ).itemobna,
		0,
		NULL,
		NULL,
		CONFEXME_16.tbrcED_Item( '37356' ).itemgren,
		NULL,
		1,
		'<ANOINI>',
		'</ANOINI>',
		CONFEXME_16.tbrcED_Item( '37356' ).itematfd
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el item
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEM;
	CONFEXME_16.tbrcED_Item( '37357' ).itemcodi := nuNextSeqValue;

	-- Se establece el nombre del objeto por defecto
	CONFEXME_16.tbrcED_Item( '37357' ).itemobna := NULL;

	-- Se asigna el grupo de entidad asociado al item
	if ( CONFEXME_16.tbrcED_AtriFuda.exists( '32081' ) ) then
		CONFEXME_16.tbrcED_Item( '37357' ).itematfd := CONFEXME_16.tbrcED_AtriFuda( '32081' ).atfdcodi;
	end if;

	UT_Trace.Trace( 'Insertando Item [MESFIN]', 5 );
	INSERT INTO ED_Item
	(
		itemcodi,
		itemdesc,
		itemceid,
		itemcons,
		itemobna,
		itemlong,
		itemalin,
		itematri,
		itemgren,
		itemmasc,
		itemtida,
		itemdein,
		itemdefi,
		itematfd
	)
	VALUES
	(
		CONFEXME_16.tbrcED_Item( '37357' ).itemcodi,
		'MESFIN',
		CONFEXME_16.tbrcED_Item( '37357' ).itemceid,
		NULL,
		CONFEXME_16.tbrcED_Item( '37357' ).itemobna,
		0,
		NULL,
		NULL,
		CONFEXME_16.tbrcED_Item( '37357' ).itemgren,
		NULL,
		1,
		'<MESFIN>',
		'</MESFIN>',
		CONFEXME_16.tbrcED_Item( '37357' ).itematfd
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el item
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEM;
	CONFEXME_16.tbrcED_Item( '37358' ).itemcodi := nuNextSeqValue;

	-- Se establece el nombre del objeto por defecto
	CONFEXME_16.tbrcED_Item( '37358' ).itemobna := NULL;

	-- Se asigna el grupo de entidad asociado al item
	if ( CONFEXME_16.tbrcED_AtriFuda.exists( '32082' ) ) then
		CONFEXME_16.tbrcED_Item( '37358' ).itematfd := CONFEXME_16.tbrcED_AtriFuda( '32082' ).atfdcodi;
	end if;

	UT_Trace.Trace( 'Insertando Item [ANOFIN]', 5 );
	INSERT INTO ED_Item
	(
		itemcodi,
		itemdesc,
		itemceid,
		itemcons,
		itemobna,
		itemlong,
		itemalin,
		itematri,
		itemgren,
		itemmasc,
		itemtida,
		itemdein,
		itemdefi,
		itematfd
	)
	VALUES
	(
		CONFEXME_16.tbrcED_Item( '37358' ).itemcodi,
		'ANOFIN',
		CONFEXME_16.tbrcED_Item( '37358' ).itemceid,
		NULL,
		CONFEXME_16.tbrcED_Item( '37358' ).itemobna,
		0,
		NULL,
		NULL,
		CONFEXME_16.tbrcED_Item( '37358' ).itemgren,
		NULL,
		1,
		'<ANOFIN>',
		'</ANOFIN>',
		CONFEXME_16.tbrcED_Item( '37358' ).itematfd
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el item
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEM;
	CONFEXME_16.tbrcED_Item( '37359' ).itemcodi := nuNextSeqValue;

	-- Se establece el nombre del objeto por defecto
	CONFEXME_16.tbrcED_Item( '37359' ).itemobna := NULL;

	-- Se asigna el grupo de entidad asociado al item
	if ( CONFEXME_16.tbrcED_AtriFuda.exists( '32083' ) ) then
		CONFEXME_16.tbrcED_Item( '37359' ).itematfd := CONFEXME_16.tbrcED_AtriFuda( '32083' ).atfdcodi;
	end if;

	UT_Trace.Trace( 'Insertando Item [Nuyear]', 5 );
	INSERT INTO ED_Item
	(
		itemcodi,
		itemdesc,
		itemceid,
		itemcons,
		itemobna,
		itemlong,
		itemalin,
		itematri,
		itemgren,
		itemmasc,
		itemtida,
		itemdein,
		itemdefi,
		itematfd
	)
	VALUES
	(
		CONFEXME_16.tbrcED_Item( '37359' ).itemcodi,
		'Nuyear',
		CONFEXME_16.tbrcED_Item( '37359' ).itemceid,
		NULL,
		CONFEXME_16.tbrcED_Item( '37359' ).itemobna,
		0,
		NULL,
		NULL,
		CONFEXME_16.tbrcED_Item( '37359' ).itemgren,
		NULL,
		1,
		'<NUYEAR>',
		'</NUYEAR>',
		CONFEXME_16.tbrcED_Item( '37359' ).itematfd
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el item
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEM;
	CONFEXME_16.tbrcED_Item( '37360' ).itemcodi := nuNextSeqValue;

	-- Se establece el nombre del objeto por defecto
	CONFEXME_16.tbrcED_Item( '37360' ).itemobna := NULL;

	-- Se asigna el grupo de entidad asociado al item
	if ( CONFEXME_16.tbrcED_AtriFuda.exists( '32084' ) ) then
		CONFEXME_16.tbrcED_Item( '37360' ).itematfd := CONFEXME_16.tbrcED_AtriFuda( '32084' ).atfdcodi;
	end if;

	UT_Trace.Trace( 'Insertando Item [Numonth]', 5 );
	INSERT INTO ED_Item
	(
		itemcodi,
		itemdesc,
		itemceid,
		itemcons,
		itemobna,
		itemlong,
		itemalin,
		itematri,
		itemgren,
		itemmasc,
		itemtida,
		itemdein,
		itemdefi,
		itematfd
	)
	VALUES
	(
		CONFEXME_16.tbrcED_Item( '37360' ).itemcodi,
		'Numonth',
		CONFEXME_16.tbrcED_Item( '37360' ).itemceid,
		NULL,
		CONFEXME_16.tbrcED_Item( '37360' ).itemobna,
		0,
		NULL,
		NULL,
		CONFEXME_16.tbrcED_Item( '37360' ).itemgren,
		NULL,
		1,
		'<NUMONTH>',
		'</NUMONTH>',
		CONFEXME_16.tbrcED_Item( '37360' ).itematfd
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el item
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEM;
	CONFEXME_16.tbrcED_Item( '37361' ).itemcodi := nuNextSeqValue;

	-- Se establece el nombre del objeto por defecto
	CONFEXME_16.tbrcED_Item( '37361' ).itemobna := NULL;

	-- Se asigna el grupo de entidad asociado al item
	if ( CONFEXME_16.tbrcED_AtriFuda.exists( '32092' ) ) then
		CONFEXME_16.tbrcED_Item( '37361' ).itematfd := CONFEXME_16.tbrcED_AtriFuda( '32092' ).atfdcodi;
	end if;

	UT_Trace.Trace( 'Insertando Item [Sbproddesc]', 5 );
	INSERT INTO ED_Item
	(
		itemcodi,
		itemdesc,
		itemceid,
		itemcons,
		itemobna,
		itemlong,
		itemalin,
		itematri,
		itemgren,
		itemmasc,
		itemtida,
		itemdein,
		itemdefi,
		itematfd
	)
	VALUES
	(
		CONFEXME_16.tbrcED_Item( '37361' ).itemcodi,
		'Sbproddesc',
		CONFEXME_16.tbrcED_Item( '37361' ).itemceid,
		NULL,
		CONFEXME_16.tbrcED_Item( '37361' ).itemobna,
		0,
		NULL,
		NULL,
		CONFEXME_16.tbrcED_Item( '37361' ).itemgren,
		NULL,
		1,
		'<SBPRODDESC>',
		'</SBPRODDESC>',
		CONFEXME_16.tbrcED_Item( '37361' ).itematfd
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el item
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEM;
	CONFEXME_16.tbrcED_Item( '37362' ).itemcodi := nuNextSeqValue;

	-- Se establece el nombre del objeto por defecto
	CONFEXME_16.tbrcED_Item( '37362' ).itemobna := NULL;

	-- Se asigna el grupo de entidad asociado al item
	if ( CONFEXME_16.tbrcED_AtriFuda.exists( '32094' ) ) then
		CONFEXME_16.tbrcED_Item( '37362' ).itematfd := CONFEXME_16.tbrcED_AtriFuda( '32094' ).atfdcodi;
	end if;

	UT_Trace.Trace( 'Insertando Item [Sbconcdesc]', 5 );
	INSERT INTO ED_Item
	(
		itemcodi,
		itemdesc,
		itemceid,
		itemcons,
		itemobna,
		itemlong,
		itemalin,
		itematri,
		itemgren,
		itemmasc,
		itemtida,
		itemdein,
		itemdefi,
		itematfd
	)
	VALUES
	(
		CONFEXME_16.tbrcED_Item( '37362' ).itemcodi,
		'Sbconcdesc',
		CONFEXME_16.tbrcED_Item( '37362' ).itemceid,
		NULL,
		CONFEXME_16.tbrcED_Item( '37362' ).itemobna,
		0,
		NULL,
		NULL,
		CONFEXME_16.tbrcED_Item( '37362' ).itemgren,
		NULL,
		1,
		'<SBCONCDESC>',
		'</SBCONCDESC>',
		CONFEXME_16.tbrcED_Item( '37362' ).itematfd
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el item
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEM;
	CONFEXME_16.tbrcED_Item( '37363' ).itemcodi := nuNextSeqValue;

	-- Se establece el nombre del objeto por defecto
	CONFEXME_16.tbrcED_Item( '37363' ).itemobna := NULL;

	-- Se asigna el grupo de entidad asociado al item
	if ( CONFEXME_16.tbrcED_AtriFuda.exists( '32095' ) ) then
		CONFEXME_16.tbrcED_Item( '37363' ).itematfd := CONFEXME_16.tbrcED_AtriFuda( '32095' ).atfdcodi;
	end if;

	UT_Trace.Trace( 'Insertando Item [Nuaccount]', 5 );
	INSERT INTO ED_Item
	(
		itemcodi,
		itemdesc,
		itemceid,
		itemcons,
		itemobna,
		itemlong,
		itemalin,
		itematri,
		itemgren,
		itemmasc,
		itemtida,
		itemdein,
		itemdefi,
		itematfd
	)
	VALUES
	(
		CONFEXME_16.tbrcED_Item( '37363' ).itemcodi,
		'Nuaccount',
		CONFEXME_16.tbrcED_Item( '37363' ).itemceid,
		NULL,
		CONFEXME_16.tbrcED_Item( '37363' ).itemobna,
		0,
		NULL,
		NULL,
		CONFEXME_16.tbrcED_Item( '37363' ).itemgren,
		NULL,
		1,
		'<NUACCOUNT>',
		'</NUACCOUNT>',
		CONFEXME_16.tbrcED_Item( '37363' ).itematfd
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el item
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEM;
	CONFEXME_16.tbrcED_Item( '37364' ).itemcodi := nuNextSeqValue;

	-- Se establece el nombre del objeto por defecto
	CONFEXME_16.tbrcED_Item( '37364' ).itemobna := NULL;

	-- Se asigna el grupo de entidad asociado al item
	if ( CONFEXME_16.tbrcED_AtriFuda.exists( '32096' ) ) then
		CONFEXME_16.tbrcED_Item( '37364' ).itematfd := CONFEXME_16.tbrcED_AtriFuda( '32096' ).atfdcodi;
	end if;

	UT_Trace.Trace( 'Insertando Item [Nuprevbalance]', 5 );
	INSERT INTO ED_Item
	(
		itemcodi,
		itemdesc,
		itemceid,
		itemcons,
		itemobna,
		itemlong,
		itemalin,
		itematri,
		itemgren,
		itemmasc,
		itemtida,
		itemdein,
		itemdefi,
		itematfd
	)
	VALUES
	(
		CONFEXME_16.tbrcED_Item( '37364' ).itemcodi,
		'Nuprevbalance',
		CONFEXME_16.tbrcED_Item( '37364' ).itemceid,
		NULL,
		CONFEXME_16.tbrcED_Item( '37364' ).itemobna,
		0,
		NULL,
		NULL,
		CONFEXME_16.tbrcED_Item( '37364' ).itemgren,
		NULL,
		2,
		'<NUPREVBALANCE>',
		'</NUPREVBALANCE>',
		CONFEXME_16.tbrcED_Item( '37364' ).itematfd
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el item
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEM;
	CONFEXME_16.tbrcED_Item( '37365' ).itemcodi := nuNextSeqValue;

	-- Se establece el nombre del objeto por defecto
	CONFEXME_16.tbrcED_Item( '37365' ).itemobna := NULL;

	-- Se asigna el grupo de entidad asociado al item
	if ( CONFEXME_16.tbrcED_AtriFuda.exists( '32097' ) ) then
		CONFEXME_16.tbrcED_Item( '37365' ).itematfd := CONFEXME_16.tbrcED_AtriFuda( '32097' ).atfdcodi;
	end if;

	UT_Trace.Trace( 'Insertando Item [Nupayappli]', 5 );
	INSERT INTO ED_Item
	(
		itemcodi,
		itemdesc,
		itemceid,
		itemcons,
		itemobna,
		itemlong,
		itemalin,
		itematri,
		itemgren,
		itemmasc,
		itemtida,
		itemdein,
		itemdefi,
		itematfd
	)
	VALUES
	(
		CONFEXME_16.tbrcED_Item( '37365' ).itemcodi,
		'Nupayappli',
		CONFEXME_16.tbrcED_Item( '37365' ).itemceid,
		NULL,
		CONFEXME_16.tbrcED_Item( '37365' ).itemobna,
		0,
		NULL,
		NULL,
		CONFEXME_16.tbrcED_Item( '37365' ).itemgren,
		NULL,
		2,
		'<NUPAYAPPLI>',
		'</NUPAYAPPLI>',
		CONFEXME_16.tbrcED_Item( '37365' ).itematfd
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el item
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEM;
	CONFEXME_16.tbrcED_Item( '37366' ).itemcodi := nuNextSeqValue;

	-- Se establece el nombre del objeto por defecto
	CONFEXME_16.tbrcED_Item( '37366' ).itemobna := NULL;

	-- Se asigna el grupo de entidad asociado al item
	if ( CONFEXME_16.tbrcED_AtriFuda.exists( '32098' ) ) then
		CONFEXME_16.tbrcED_Item( '37366' ).itematfd := CONFEXME_16.tbrcED_AtriFuda( '32098' ).atfdcodi;
	end if;

	UT_Trace.Trace( 'Insertando Item [Nuapplibalance]', 5 );
	INSERT INTO ED_Item
	(
		itemcodi,
		itemdesc,
		itemceid,
		itemcons,
		itemobna,
		itemlong,
		itemalin,
		itematri,
		itemgren,
		itemmasc,
		itemtida,
		itemdein,
		itemdefi,
		itematfd
	)
	VALUES
	(
		CONFEXME_16.tbrcED_Item( '37366' ).itemcodi,
		'Nuapplibalance',
		CONFEXME_16.tbrcED_Item( '37366' ).itemceid,
		NULL,
		CONFEXME_16.tbrcED_Item( '37366' ).itemobna,
		0,
		NULL,
		NULL,
		CONFEXME_16.tbrcED_Item( '37366' ).itemgren,
		NULL,
		2,
		'<NUAPPLIBALANCE>',
		'</NUAPPLIBALANCE>',
		CONFEXME_16.tbrcED_Item( '37366' ).itematfd
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el item
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEM;
	CONFEXME_16.tbrcED_Item( '37367' ).itemcodi := nuNextSeqValue;

	-- Se establece el nombre del objeto por defecto
	CONFEXME_16.tbrcED_Item( '37367' ).itemobna := NULL;

	-- Se asigna el grupo de entidad asociado al item
	if ( CONFEXME_16.tbrcED_AtriFuda.exists( '32099' ) ) then
		CONFEXME_16.tbrcED_Item( '37367' ).itematfd := CONFEXME_16.tbrcED_AtriFuda( '32099' ).atfdcodi;
	end if;

	UT_Trace.Trace( 'Insertando Item [Billliquidation]', 5 );
	INSERT INTO ED_Item
	(
		itemcodi,
		itemdesc,
		itemceid,
		itemcons,
		itemobna,
		itemlong,
		itemalin,
		itematri,
		itemgren,
		itemmasc,
		itemtida,
		itemdein,
		itemdefi,
		itematfd
	)
	VALUES
	(
		CONFEXME_16.tbrcED_Item( '37367' ).itemcodi,
		'Billliquidation',
		CONFEXME_16.tbrcED_Item( '37367' ).itemceid,
		NULL,
		CONFEXME_16.tbrcED_Item( '37367' ).itemobna,
		0,
		NULL,
		NULL,
		CONFEXME_16.tbrcED_Item( '37367' ).itemgren,
		NULL,
		2,
		'<BILLLIQUIDATION>',
		'</BILLLIQUIDATION>',
		CONFEXME_16.tbrcED_Item( '37367' ).itematfd
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el item
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEM;
	CONFEXME_16.tbrcED_Item( '37368' ).itemcodi := nuNextSeqValue;

	-- Se establece el nombre del objeto por defecto
	CONFEXME_16.tbrcED_Item( '37368' ).itemobna := NULL;

	-- Se asigna el grupo de entidad asociado al item
	if ( CONFEXME_16.tbrcED_AtriFuda.exists( '32100' ) ) then
		CONFEXME_16.tbrcED_Item( '37368' ).itematfd := CONFEXME_16.tbrcED_AtriFuda( '32100' ).atfdcodi;
	end if;

	UT_Trace.Trace( 'Insertando Item [TIP_PROD]', 5 );
	INSERT INTO ED_Item
	(
		itemcodi,
		itemdesc,
		itemceid,
		itemcons,
		itemobna,
		itemlong,
		itemalin,
		itematri,
		itemgren,
		itemmasc,
		itemtida,
		itemdein,
		itemdefi,
		itematfd
	)
	VALUES
	(
		CONFEXME_16.tbrcED_Item( '37368' ).itemcodi,
		'TIP_PROD',
		CONFEXME_16.tbrcED_Item( '37368' ).itemceid,
		NULL,
		CONFEXME_16.tbrcED_Item( '37368' ).itemobna,
		0,
		NULL,
		NULL,
		CONFEXME_16.tbrcED_Item( '37368' ).itemgren,
		NULL,
		2,
		NULL,
		NULL,
		CONFEXME_16.tbrcED_Item( '37368' ).itematfd
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el item
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEM;
	CONFEXME_16.tbrcED_Item( '37369' ).itemcodi := nuNextSeqValue;

	-- Se establece el nombre del objeto por defecto
	CONFEXME_16.tbrcED_Item( '37369' ).itemobna := NULL;

	-- Se asigna el grupo de entidad asociado al item
	if ( CONFEXME_16.tbrcED_AtriFuda.exists( '32101' ) ) then
		CONFEXME_16.tbrcED_Item( '37369' ).itematfd := CONFEXME_16.tbrcED_AtriFuda( '32101' ).atfdcodi;
	end if;

	UT_Trace.Trace( 'Insertando Item [TIPOPRODUCTO]', 5 );
	INSERT INTO ED_Item
	(
		itemcodi,
		itemdesc,
		itemceid,
		itemcons,
		itemobna,
		itemlong,
		itemalin,
		itematri,
		itemgren,
		itemmasc,
		itemtida,
		itemdein,
		itemdefi,
		itematfd
	)
	VALUES
	(
		CONFEXME_16.tbrcED_Item( '37369' ).itemcodi,
		'TIPOPRODUCTO',
		CONFEXME_16.tbrcED_Item( '37369' ).itemceid,
		NULL,
		CONFEXME_16.tbrcED_Item( '37369' ).itemobna,
		0,
		NULL,
		NULL,
		CONFEXME_16.tbrcED_Item( '37369' ).itemgren,
		NULL,
		1,
		'<TIPOPRODUCTO>',
		'</TIPOPRODUCTO>',
		CONFEXME_16.tbrcED_Item( '37369' ).itematfd
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el item
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEM;
	CONFEXME_16.tbrcED_Item( '37370' ).itemcodi := nuNextSeqValue;

	-- Se establece el nombre del objeto por defecto
	CONFEXME_16.tbrcED_Item( '37370' ).itemobna := NULL;

	-- Se asigna el grupo de entidad asociado al item
	if ( CONFEXME_16.tbrcED_AtriFuda.exists( '32102' ) ) then
		CONFEXME_16.tbrcED_Item( '37370' ).itematfd := CONFEXME_16.tbrcED_AtriFuda( '32102' ).atfdcodi;
	end if;

	UT_Trace.Trace( 'Insertando Item [FECHAGENFAC]', 5 );
	INSERT INTO ED_Item
	(
		itemcodi,
		itemdesc,
		itemceid,
		itemcons,
		itemobna,
		itemlong,
		itemalin,
		itematri,
		itemgren,
		itemmasc,
		itemtida,
		itemdein,
		itemdefi,
		itematfd
	)
	VALUES
	(
		CONFEXME_16.tbrcED_Item( '37370' ).itemcodi,
		'FECHAGENFAC',
		CONFEXME_16.tbrcED_Item( '37370' ).itemceid,
		NULL,
		CONFEXME_16.tbrcED_Item( '37370' ).itemobna,
		0,
		NULL,
		NULL,
		CONFEXME_16.tbrcED_Item( '37370' ).itemgren,
		NULL,
		1,
		'<FECHAGENFAC>',
		'</FECHAGENFAC>',
		CONFEXME_16.tbrcED_Item( '37370' ).itematfd
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el item
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEM;
	CONFEXME_16.tbrcED_Item( '37371' ).itemcodi := nuNextSeqValue;

	-- Se establece el nombre del objeto por defecto
	CONFEXME_16.tbrcED_Item( '37371' ).itemobna := NULL;

	-- Se asigna el grupo de entidad asociado al item
	if ( CONFEXME_16.tbrcED_AtriFuda.exists( '32103' ) ) then
		CONFEXME_16.tbrcED_Item( '37371' ).itematfd := CONFEXME_16.tbrcED_AtriFuda( '32103' ).atfdcodi;
	end if;

	UT_Trace.Trace( 'Insertando Item [TOTALFACT]', 5 );
	INSERT INTO ED_Item
	(
		itemcodi,
		itemdesc,
		itemceid,
		itemcons,
		itemobna,
		itemlong,
		itemalin,
		itematri,
		itemgren,
		itemmasc,
		itemtida,
		itemdein,
		itemdefi,
		itematfd
	)
	VALUES
	(
		CONFEXME_16.tbrcED_Item( '37371' ).itemcodi,
		'TOTALFACT',
		CONFEXME_16.tbrcED_Item( '37371' ).itemceid,
		NULL,
		CONFEXME_16.tbrcED_Item( '37371' ).itemobna,
		0,
		NULL,
		NULL,
		CONFEXME_16.tbrcED_Item( '37371' ).itemgren,
		NULL,
		2,
		'<TOTALFACT>',
		'</TOTALFACT>',
		CONFEXME_16.tbrcED_Item( '37371' ).itematfd
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el item
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEM;
	CONFEXME_16.tbrcED_Item( '37372' ).itemcodi := nuNextSeqValue;

	-- Se establece el nombre del objeto por defecto
	CONFEXME_16.tbrcED_Item( '37372' ).itemobna := NULL;

	-- Se asigna el grupo de entidad asociado al item
	if ( CONFEXME_16.tbrcED_AtriFuda.exists( '32104' ) ) then
		CONFEXME_16.tbrcED_Item( '37372' ).itematfd := CONFEXME_16.tbrcED_AtriFuda( '32104' ).atfdcodi;
	end if;

	UT_Trace.Trace( 'Insertando Item [FECHARECAUDO]', 5 );
	INSERT INTO ED_Item
	(
		itemcodi,
		itemdesc,
		itemceid,
		itemcons,
		itemobna,
		itemlong,
		itemalin,
		itematri,
		itemgren,
		itemmasc,
		itemtida,
		itemdein,
		itemdefi,
		itematfd
	)
	VALUES
	(
		CONFEXME_16.tbrcED_Item( '37372' ).itemcodi,
		'FECHARECAUDO',
		CONFEXME_16.tbrcED_Item( '37372' ).itemceid,
		NULL,
		CONFEXME_16.tbrcED_Item( '37372' ).itemobna,
		0,
		NULL,
		NULL,
		CONFEXME_16.tbrcED_Item( '37372' ).itemgren,
		NULL,
		1,
		'<FECHARECAUDO>',
		'</FECHARECAUDO>',
		CONFEXME_16.tbrcED_Item( '37372' ).itematfd
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el item
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEM;
	CONFEXME_16.tbrcED_Item( '37373' ).itemcodi := nuNextSeqValue;

	-- Se establece el nombre del objeto por defecto
	CONFEXME_16.tbrcED_Item( '37373' ).itemobna := NULL;

	-- Se asigna el grupo de entidad asociado al item
	if ( CONFEXME_16.tbrcED_AtriFuda.exists( '32105' ) ) then
		CONFEXME_16.tbrcED_Item( '37373' ).itematfd := CONFEXME_16.tbrcED_AtriFuda( '32105' ).atfdcodi;
	end if;

	UT_Trace.Trace( 'Insertando Item [CUENTACOBROPAGO]', 5 );
	INSERT INTO ED_Item
	(
		itemcodi,
		itemdesc,
		itemceid,
		itemcons,
		itemobna,
		itemlong,
		itemalin,
		itematri,
		itemgren,
		itemmasc,
		itemtida,
		itemdein,
		itemdefi,
		itematfd
	)
	VALUES
	(
		CONFEXME_16.tbrcED_Item( '37373' ).itemcodi,
		'CUENTACOBROPAGO',
		CONFEXME_16.tbrcED_Item( '37373' ).itemceid,
		NULL,
		CONFEXME_16.tbrcED_Item( '37373' ).itemobna,
		0,
		NULL,
		NULL,
		CONFEXME_16.tbrcED_Item( '37373' ).itemgren,
		NULL,
		2,
		'<CUENTACOBROPAGO>',
		'</CUENTACOBROPAGO>',
		CONFEXME_16.tbrcED_Item( '37373' ).itematfd
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el item
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEM;
	CONFEXME_16.tbrcED_Item( '37374' ).itemcodi := nuNextSeqValue;

	-- Se establece el nombre del objeto por defecto
	CONFEXME_16.tbrcED_Item( '37374' ).itemobna := NULL;

	-- Se asigna el grupo de entidad asociado al item
	if ( CONFEXME_16.tbrcED_AtriFuda.exists( '32106' ) ) then
		CONFEXME_16.tbrcED_Item( '37374' ).itematfd := CONFEXME_16.tbrcED_AtriFuda( '32106' ).atfdcodi;
	end if;

	UT_Trace.Trace( 'Insertando Item [VALORRECAUDO]', 5 );
	INSERT INTO ED_Item
	(
		itemcodi,
		itemdesc,
		itemceid,
		itemcons,
		itemobna,
		itemlong,
		itemalin,
		itematri,
		itemgren,
		itemmasc,
		itemtida,
		itemdein,
		itemdefi,
		itematfd
	)
	VALUES
	(
		CONFEXME_16.tbrcED_Item( '37374' ).itemcodi,
		'VALORRECAUDO',
		CONFEXME_16.tbrcED_Item( '37374' ).itemceid,
		NULL,
		CONFEXME_16.tbrcED_Item( '37374' ).itemobna,
		0,
		NULL,
		NULL,
		CONFEXME_16.tbrcED_Item( '37374' ).itemgren,
		NULL,
		2,
		'<VALORRECAUDO>',
		'</VALORRECAUDO>',
		CONFEXME_16.tbrcED_Item( '37374' ).itematfd
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el item
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEM;
	CONFEXME_16.tbrcED_Item( '37375' ).itemcodi := nuNextSeqValue;

	-- Se establece el nombre del objeto por defecto
	CONFEXME_16.tbrcED_Item( '37375' ).itemobna := NULL;

	-- Se asigna el grupo de entidad asociado al item
	if ( CONFEXME_16.tbrcED_AtriFuda.exists( '32107' ) ) then
		CONFEXME_16.tbrcED_Item( '37375' ).itematfd := CONFEXME_16.tbrcED_AtriFuda( '32107' ).atfdcodi;
	end if;

	UT_Trace.Trace( 'Insertando Item [FECHAAPLICACIONRECAUDO]', 5 );
	INSERT INTO ED_Item
	(
		itemcodi,
		itemdesc,
		itemceid,
		itemcons,
		itemobna,
		itemlong,
		itemalin,
		itematri,
		itemgren,
		itemmasc,
		itemtida,
		itemdein,
		itemdefi,
		itematfd
	)
	VALUES
	(
		CONFEXME_16.tbrcED_Item( '37375' ).itemcodi,
		'FECHAAPLICACIONRECAUDO',
		CONFEXME_16.tbrcED_Item( '37375' ).itemceid,
		NULL,
		CONFEXME_16.tbrcED_Item( '37375' ).itemobna,
		0,
		NULL,
		NULL,
		CONFEXME_16.tbrcED_Item( '37375' ).itemgren,
		NULL,
		1,
		'<FECHAAPLICACIONRECAUDO>',
		'</FECHAAPLICACIONRECAUDO>',
		CONFEXME_16.tbrcED_Item( '37375' ).itematfd
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el item
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEM;
	CONFEXME_16.tbrcED_Item( '37376' ).itemcodi := nuNextSeqValue;

	-- Se establece el nombre del objeto por defecto
	CONFEXME_16.tbrcED_Item( '37376' ).itemobna := NULL;

	-- Se asigna el grupo de entidad asociado al item
	if ( CONFEXME_16.tbrcED_AtriFuda.exists( '32108' ) ) then
		CONFEXME_16.tbrcED_Item( '37376' ).itematfd := CONFEXME_16.tbrcED_AtriFuda( '32108' ).atfdcodi;
	end if;

	UT_Trace.Trace( 'Insertando Item [ENTIDADRECAUDADORA]', 5 );
	INSERT INTO ED_Item
	(
		itemcodi,
		itemdesc,
		itemceid,
		itemcons,
		itemobna,
		itemlong,
		itemalin,
		itematri,
		itemgren,
		itemmasc,
		itemtida,
		itemdein,
		itemdefi,
		itematfd
	)
	VALUES
	(
		CONFEXME_16.tbrcED_Item( '37376' ).itemcodi,
		'ENTIDADRECAUDADORA',
		CONFEXME_16.tbrcED_Item( '37376' ).itemceid,
		NULL,
		CONFEXME_16.tbrcED_Item( '37376' ).itemobna,
		0,
		NULL,
		NULL,
		CONFEXME_16.tbrcED_Item( '37376' ).itemgren,
		NULL,
		1,
		'<ENTIDADRECAUDADORA>',
		'</ENTIDADRECAUDADORA>',
		CONFEXME_16.tbrcED_Item( '37376' ).itematfd
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el item
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEM;
	CONFEXME_16.tbrcED_Item( '37377' ).itemcodi := nuNextSeqValue;

	-- Se establece el nombre del objeto por defecto
	CONFEXME_16.tbrcED_Item( '37377' ).itemobna := NULL;

	-- Se asigna el grupo de entidad asociado al item
	if ( CONFEXME_16.tbrcED_AtriFuda.exists( '32109' ) ) then
		CONFEXME_16.tbrcED_Item( '37377' ).itematfd := CONFEXME_16.tbrcED_AtriFuda( '32109' ).atfdcodi;
	end if;

	UT_Trace.Trace( 'Insertando Item [NOM_EMPRESA]', 5 );
	INSERT INTO ED_Item
	(
		itemcodi,
		itemdesc,
		itemceid,
		itemcons,
		itemobna,
		itemlong,
		itemalin,
		itematri,
		itemgren,
		itemmasc,
		itemtida,
		itemdein,
		itemdefi,
		itematfd
	)
	VALUES
	(
		CONFEXME_16.tbrcED_Item( '37377' ).itemcodi,
		'NOM_EMPRESA',
		CONFEXME_16.tbrcED_Item( '37377' ).itemceid,
		NULL,
		CONFEXME_16.tbrcED_Item( '37377' ).itemobna,
		0,
		NULL,
		NULL,
		CONFEXME_16.tbrcED_Item( '37377' ).itemgren,
		NULL,
		1,
		'<NOM_EMPRESA>',
		'</NOM_EMPRESA>',
		CONFEXME_16.tbrcED_Item( '37377' ).itematfd
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el item
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEM;
	CONFEXME_16.tbrcED_Item( '37378' ).itemcodi := nuNextSeqValue;

	-- Se establece el nombre del objeto por defecto
	CONFEXME_16.tbrcED_Item( '37378' ).itemobna := NULL;

	-- Se asigna el grupo de entidad asociado al item
	if ( CONFEXME_16.tbrcED_AtriFuda.exists( '32110' ) ) then
		CONFEXME_16.tbrcED_Item( '37378' ).itematfd := CONFEXME_16.tbrcED_AtriFuda( '32110' ).atfdcodi;
	end if;

	UT_Trace.Trace( 'Insertando Item [CONTRATO]', 5 );
	INSERT INTO ED_Item
	(
		itemcodi,
		itemdesc,
		itemceid,
		itemcons,
		itemobna,
		itemlong,
		itemalin,
		itematri,
		itemgren,
		itemmasc,
		itemtida,
		itemdein,
		itemdefi,
		itematfd
	)
	VALUES
	(
		CONFEXME_16.tbrcED_Item( '37378' ).itemcodi,
		'CONTRATO',
		CONFEXME_16.tbrcED_Item( '37378' ).itemceid,
		NULL,
		CONFEXME_16.tbrcED_Item( '37378' ).itemobna,
		0,
		NULL,
		NULL,
		CONFEXME_16.tbrcED_Item( '37378' ).itemgren,
		NULL,
		2,
		'<CONTRATO>',
		'</CONTRATO>',
		CONFEXME_16.tbrcED_Item( '37378' ).itematfd
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el item
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEM;
	CONFEXME_16.tbrcED_Item( '37379' ).itemcodi := nuNextSeqValue;

	-- Se establece el nombre del objeto por defecto
	CONFEXME_16.tbrcED_Item( '37379' ).itemobna := NULL;

	-- Se asigna el grupo de entidad asociado al item
	if ( CONFEXME_16.tbrcED_AtriFuda.exists( '32111' ) ) then
		CONFEXME_16.tbrcED_Item( '37379' ).itematfd := CONFEXME_16.tbrcED_AtriFuda( '32111' ).atfdcodi;
	end if;

	UT_Trace.Trace( 'Insertando Item [DIRECCION]', 5 );
	INSERT INTO ED_Item
	(
		itemcodi,
		itemdesc,
		itemceid,
		itemcons,
		itemobna,
		itemlong,
		itemalin,
		itematri,
		itemgren,
		itemmasc,
		itemtida,
		itemdein,
		itemdefi,
		itematfd
	)
	VALUES
	(
		CONFEXME_16.tbrcED_Item( '37379' ).itemcodi,
		'DIRECCION',
		CONFEXME_16.tbrcED_Item( '37379' ).itemceid,
		NULL,
		CONFEXME_16.tbrcED_Item( '37379' ).itemobna,
		0,
		NULL,
		NULL,
		CONFEXME_16.tbrcED_Item( '37379' ).itemgren,
		NULL,
		1,
		'<DIRECCION>',
		'</DIRECCION>',
		CONFEXME_16.tbrcED_Item( '37379' ).itematfd
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el item
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEM;
	CONFEXME_16.tbrcED_Item( '37380' ).itemcodi := nuNextSeqValue;

	-- Se establece el nombre del objeto por defecto
	CONFEXME_16.tbrcED_Item( '37380' ).itemobna := NULL;

	-- Se asigna el grupo de entidad asociado al item
	if ( CONFEXME_16.tbrcED_AtriFuda.exists( '32112' ) ) then
		CONFEXME_16.tbrcED_Item( '37380' ).itematfd := CONFEXME_16.tbrcED_AtriFuda( '32112' ).atfdcodi;
	end if;

	UT_Trace.Trace( 'Insertando Item [MUNICIPIO]', 5 );
	INSERT INTO ED_Item
	(
		itemcodi,
		itemdesc,
		itemceid,
		itemcons,
		itemobna,
		itemlong,
		itemalin,
		itematri,
		itemgren,
		itemmasc,
		itemtida,
		itemdein,
		itemdefi,
		itematfd
	)
	VALUES
	(
		CONFEXME_16.tbrcED_Item( '37380' ).itemcodi,
		'MUNICIPIO',
		CONFEXME_16.tbrcED_Item( '37380' ).itemceid,
		NULL,
		CONFEXME_16.tbrcED_Item( '37380' ).itemobna,
		0,
		NULL,
		NULL,
		CONFEXME_16.tbrcED_Item( '37380' ).itemgren,
		NULL,
		1,
		'<MUNICIPIO>',
		'</MUNICIPIO>',
		CONFEXME_16.tbrcED_Item( '37380' ).itematfd
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el item
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEM;
	CONFEXME_16.tbrcED_Item( '37381' ).itemcodi := nuNextSeqValue;

	-- Se establece el nombre del objeto por defecto
	CONFEXME_16.tbrcED_Item( '37381' ).itemobna := NULL;

	-- Se asigna el grupo de entidad asociado al item
	if ( CONFEXME_16.tbrcED_AtriFuda.exists( '32113' ) ) then
		CONFEXME_16.tbrcED_Item( '37381' ).itematfd := CONFEXME_16.tbrcED_AtriFuda( '32113' ).atfdcodi;
	end if;

	UT_Trace.Trace( 'Insertando Item [NOMBRE_CLIENTE]', 5 );
	INSERT INTO ED_Item
	(
		itemcodi,
		itemdesc,
		itemceid,
		itemcons,
		itemobna,
		itemlong,
		itemalin,
		itematri,
		itemgren,
		itemmasc,
		itemtida,
		itemdein,
		itemdefi,
		itematfd
	)
	VALUES
	(
		CONFEXME_16.tbrcED_Item( '37381' ).itemcodi,
		'NOMBRE_CLIENTE',
		CONFEXME_16.tbrcED_Item( '37381' ).itemceid,
		NULL,
		CONFEXME_16.tbrcED_Item( '37381' ).itemobna,
		0,
		NULL,
		NULL,
		CONFEXME_16.tbrcED_Item( '37381' ).itemgren,
		NULL,
		1,
		'<NOMBRE_CLIENTE>',
		'</NOMBRE_CLIENTE>',
		CONFEXME_16.tbrcED_Item( '37381' ).itematfd
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el item
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEM;
	CONFEXME_16.tbrcED_Item( '37382' ).itemcodi := nuNextSeqValue;

	-- Se establece el nombre del objeto por defecto
	CONFEXME_16.tbrcED_Item( '37382' ).itemobna := NULL;

	-- Se asigna el grupo de entidad asociado al item
	if ( CONFEXME_16.tbrcED_AtriFuda.exists( '32114' ) ) then
		CONFEXME_16.tbrcED_Item( '37382' ).itematfd := CONFEXME_16.tbrcED_AtriFuda( '32114' ).atfdcodi;
	end if;

	UT_Trace.Trace( 'Insertando Item [APELLIDO_CLIENTE]', 5 );
	INSERT INTO ED_Item
	(
		itemcodi,
		itemdesc,
		itemceid,
		itemcons,
		itemobna,
		itemlong,
		itemalin,
		itematri,
		itemgren,
		itemmasc,
		itemtida,
		itemdein,
		itemdefi,
		itematfd
	)
	VALUES
	(
		CONFEXME_16.tbrcED_Item( '37382' ).itemcodi,
		'APELLIDO_CLIENTE',
		CONFEXME_16.tbrcED_Item( '37382' ).itemceid,
		NULL,
		CONFEXME_16.tbrcED_Item( '37382' ).itemobna,
		0,
		NULL,
		NULL,
		CONFEXME_16.tbrcED_Item( '37382' ).itemgren,
		NULL,
		1,
		'<APELLIDO_CLIENTE>',
		'</APELLIDO_CLIENTE>',
		CONFEXME_16.tbrcED_Item( '37382' ).itematfd
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el item
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEM;
	CONFEXME_16.tbrcED_Item( '37383' ).itemcodi := nuNextSeqValue;

	-- Se establece el nombre del objeto por defecto
	CONFEXME_16.tbrcED_Item( '37383' ).itemobna := NULL;

	-- Se asigna el grupo de entidad asociado al item
	if ( CONFEXME_16.tbrcED_AtriFuda.exists( '32115' ) ) then
		CONFEXME_16.tbrcED_Item( '37383' ).itematfd := CONFEXME_16.tbrcED_AtriFuda( '32115' ).atfdcodi;
	end if;

	UT_Trace.Trace( 'Insertando Item [NOM_SOLICITANTE]', 5 );
	INSERT INTO ED_Item
	(
		itemcodi,
		itemdesc,
		itemceid,
		itemcons,
		itemobna,
		itemlong,
		itemalin,
		itematri,
		itemgren,
		itemmasc,
		itemtida,
		itemdein,
		itemdefi,
		itematfd
	)
	VALUES
	(
		CONFEXME_16.tbrcED_Item( '37383' ).itemcodi,
		'NOM_SOLICITANTE',
		CONFEXME_16.tbrcED_Item( '37383' ).itemceid,
		NULL,
		CONFEXME_16.tbrcED_Item( '37383' ).itemobna,
		0,
		NULL,
		NULL,
		CONFEXME_16.tbrcED_Item( '37383' ).itemgren,
		NULL,
		1,
		'<NOM_SOLICITANTE>',
		'</NOM_SOLICITANTE>',
		CONFEXME_16.tbrcED_Item( '37383' ).itematfd
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el item
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEM;
	CONFEXME_16.tbrcED_Item( '37384' ).itemcodi := nuNextSeqValue;

	-- Se establece el nombre del objeto por defecto
	CONFEXME_16.tbrcED_Item( '37384' ).itemobna := NULL;

	-- Se asigna el grupo de entidad asociado al item
	if ( CONFEXME_16.tbrcED_AtriFuda.exists( '32116' ) ) then
		CONFEXME_16.tbrcED_Item( '37384' ).itematfd := CONFEXME_16.tbrcED_AtriFuda( '32116' ).atfdcodi;
	end if;

	UT_Trace.Trace( 'Insertando Item [DOC_SOLICITANTE]', 5 );
	INSERT INTO ED_Item
	(
		itemcodi,
		itemdesc,
		itemceid,
		itemcons,
		itemobna,
		itemlong,
		itemalin,
		itematri,
		itemgren,
		itemmasc,
		itemtida,
		itemdein,
		itemdefi,
		itematfd
	)
	VALUES
	(
		CONFEXME_16.tbrcED_Item( '37384' ).itemcodi,
		'DOC_SOLICITANTE',
		CONFEXME_16.tbrcED_Item( '37384' ).itemceid,
		NULL,
		CONFEXME_16.tbrcED_Item( '37384' ).itemobna,
		0,
		NULL,
		NULL,
		CONFEXME_16.tbrcED_Item( '37384' ).itemgren,
		NULL,
		1,
		'<DOC_SOLICITANTE>',
		'</DOC_SOLICITANTE>',
		CONFEXME_16.tbrcED_Item( '37384' ).itematfd
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el item
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEM;
	CONFEXME_16.tbrcED_Item( '37385' ).itemcodi := nuNextSeqValue;

	-- Se establece el nombre del objeto por defecto
	CONFEXME_16.tbrcED_Item( '37385' ).itemobna := NULL;

	-- Se asigna el grupo de entidad asociado al item
	if ( CONFEXME_16.tbrcED_AtriFuda.exists( '32117' ) ) then
		CONFEXME_16.tbrcED_Item( '37385' ).itematfd := CONFEXME_16.tbrcED_AtriFuda( '32117' ).atfdcodi;
	end if;

	UT_Trace.Trace( 'Insertando Item [MUN_SOLICITANTE]', 5 );
	INSERT INTO ED_Item
	(
		itemcodi,
		itemdesc,
		itemceid,
		itemcons,
		itemobna,
		itemlong,
		itemalin,
		itematri,
		itemgren,
		itemmasc,
		itemtida,
		itemdein,
		itemdefi,
		itematfd
	)
	VALUES
	(
		CONFEXME_16.tbrcED_Item( '37385' ).itemcodi,
		'MUN_SOLICITANTE',
		CONFEXME_16.tbrcED_Item( '37385' ).itemceid,
		NULL,
		CONFEXME_16.tbrcED_Item( '37385' ).itemobna,
		0,
		NULL,
		NULL,
		CONFEXME_16.tbrcED_Item( '37385' ).itemgren,
		NULL,
		1,
		'<MUN_SOLICITANTE>',
		'</MUN_SOLICITANTE>',
		CONFEXME_16.tbrcED_Item( '37385' ).itematfd
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el item
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEM;
	CONFEXME_16.tbrcED_Item( '37386' ).itemcodi := nuNextSeqValue;

	-- Se establece el nombre del objeto por defecto
	CONFEXME_16.tbrcED_Item( '37386' ).itemobna := NULL;

	-- Se asigna el grupo de entidad asociado al item
	if ( CONFEXME_16.tbrcED_AtriFuda.exists( '32118' ) ) then
		CONFEXME_16.tbrcED_Item( '37386' ).itematfd := CONFEXME_16.tbrcED_AtriFuda( '32118' ).atfdcodi;
	end if;

	UT_Trace.Trace( 'Insertando Item [FECHA]', 5 );
	INSERT INTO ED_Item
	(
		itemcodi,
		itemdesc,
		itemceid,
		itemcons,
		itemobna,
		itemlong,
		itemalin,
		itematri,
		itemgren,
		itemmasc,
		itemtida,
		itemdein,
		itemdefi,
		itematfd
	)
	VALUES
	(
		CONFEXME_16.tbrcED_Item( '37386' ).itemcodi,
		'FECHA',
		CONFEXME_16.tbrcED_Item( '37386' ).itemceid,
		NULL,
		CONFEXME_16.tbrcED_Item( '37386' ).itemobna,
		0,
		NULL,
		NULL,
		CONFEXME_16.tbrcED_Item( '37386' ).itemgren,
		NULL,
		1,
		'<FECHA>',
		'</FECHA>',
		CONFEXME_16.tbrcED_Item( '37386' ).itematfd
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el item
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEM;
	CONFEXME_16.tbrcED_Item( '37387' ).itemcodi := nuNextSeqValue;

	-- Se establece el nombre del objeto por defecto
	CONFEXME_16.tbrcED_Item( '37387' ).itemobna := NULL;

	-- Se asigna el grupo de entidad asociado al item
	if ( CONFEXME_16.tbrcED_AtriFuda.exists( '32119' ) ) then
		CONFEXME_16.tbrcED_Item( '37387' ).itematfd := CONFEXME_16.tbrcED_AtriFuda( '32119' ).atfdcodi;
	end if;

	UT_Trace.Trace( 'Insertando Item [DIA]', 5 );
	INSERT INTO ED_Item
	(
		itemcodi,
		itemdesc,
		itemceid,
		itemcons,
		itemobna,
		itemlong,
		itemalin,
		itematri,
		itemgren,
		itemmasc,
		itemtida,
		itemdein,
		itemdefi,
		itematfd
	)
	VALUES
	(
		CONFEXME_16.tbrcED_Item( '37387' ).itemcodi,
		'DIA',
		CONFEXME_16.tbrcED_Item( '37387' ).itemceid,
		NULL,
		CONFEXME_16.tbrcED_Item( '37387' ).itemobna,
		0,
		NULL,
		NULL,
		CONFEXME_16.tbrcED_Item( '37387' ).itemgren,
		NULL,
		1,
		'<DIA>',
		'</DIA>',
		CONFEXME_16.tbrcED_Item( '37387' ).itematfd
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el item
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEM;
	CONFEXME_16.tbrcED_Item( '37388' ).itemcodi := nuNextSeqValue;

	-- Se establece el nombre del objeto por defecto
	CONFEXME_16.tbrcED_Item( '37388' ).itemobna := NULL;

	-- Se asigna el grupo de entidad asociado al item
	if ( CONFEXME_16.tbrcED_AtriFuda.exists( '32120' ) ) then
		CONFEXME_16.tbrcED_Item( '37388' ).itematfd := CONFEXME_16.tbrcED_AtriFuda( '32120' ).atfdcodi;
	end if;

	UT_Trace.Trace( 'Insertando Item [MES]', 5 );
	INSERT INTO ED_Item
	(
		itemcodi,
		itemdesc,
		itemceid,
		itemcons,
		itemobna,
		itemlong,
		itemalin,
		itematri,
		itemgren,
		itemmasc,
		itemtida,
		itemdein,
		itemdefi,
		itematfd
	)
	VALUES
	(
		CONFEXME_16.tbrcED_Item( '37388' ).itemcodi,
		'MES',
		CONFEXME_16.tbrcED_Item( '37388' ).itemceid,
		NULL,
		CONFEXME_16.tbrcED_Item( '37388' ).itemobna,
		0,
		NULL,
		NULL,
		CONFEXME_16.tbrcED_Item( '37388' ).itemgren,
		NULL,
		1,
		'<MES>',
		'</MES>',
		CONFEXME_16.tbrcED_Item( '37388' ).itematfd
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el item
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEM;
	CONFEXME_16.tbrcED_Item( '37389' ).itemcodi := nuNextSeqValue;

	-- Se establece el nombre del objeto por defecto
	CONFEXME_16.tbrcED_Item( '37389' ).itemobna := NULL;

	-- Se asigna el grupo de entidad asociado al item
	if ( CONFEXME_16.tbrcED_AtriFuda.exists( '32121' ) ) then
		CONFEXME_16.tbrcED_Item( '37389' ).itematfd := CONFEXME_16.tbrcED_AtriFuda( '32121' ).atfdcodi;
	end if;

	UT_Trace.Trace( 'Insertando Item [ANO]', 5 );
	INSERT INTO ED_Item
	(
		itemcodi,
		itemdesc,
		itemceid,
		itemcons,
		itemobna,
		itemlong,
		itemalin,
		itematri,
		itemgren,
		itemmasc,
		itemtida,
		itemdein,
		itemdefi,
		itematfd
	)
	VALUES
	(
		CONFEXME_16.tbrcED_Item( '37389' ).itemcodi,
		'ANO',
		CONFEXME_16.tbrcED_Item( '37389' ).itemceid,
		NULL,
		CONFEXME_16.tbrcED_Item( '37389' ).itemobna,
		0,
		NULL,
		NULL,
		CONFEXME_16.tbrcED_Item( '37389' ).itemgren,
		NULL,
		1,
		'<ANO>',
		'</ANO>',
		CONFEXME_16.tbrcED_Item( '37389' ).itematfd
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el item
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEM;
	CONFEXME_16.tbrcED_Item( '37390' ).itemcodi := nuNextSeqValue;

	-- Se establece el nombre del objeto por defecto
	CONFEXME_16.tbrcED_Item( '37390' ).itemobna := NULL;

	-- Se asigna el grupo de entidad asociado al item
	if ( CONFEXME_16.tbrcED_AtriFuda.exists( '32122' ) ) then
		CONFEXME_16.tbrcED_Item( '37390' ).itematfd := CONFEXME_16.tbrcED_AtriFuda( '32122' ).atfdcodi;
	end if;

	UT_Trace.Trace( 'Insertando Item [MUNICIPIO_ATENCION]', 5 );
	INSERT INTO ED_Item
	(
		itemcodi,
		itemdesc,
		itemceid,
		itemcons,
		itemobna,
		itemlong,
		itemalin,
		itematri,
		itemgren,
		itemmasc,
		itemtida,
		itemdein,
		itemdefi,
		itematfd
	)
	VALUES
	(
		CONFEXME_16.tbrcED_Item( '37390' ).itemcodi,
		'MUNICIPIO_ATENCION',
		CONFEXME_16.tbrcED_Item( '37390' ).itemceid,
		NULL,
		CONFEXME_16.tbrcED_Item( '37390' ).itemobna,
		0,
		NULL,
		NULL,
		CONFEXME_16.tbrcED_Item( '37390' ).itemgren,
		NULL,
		1,
		'<MUNICIPIO_ATENCION>',
		'</MUNICIPIO_ATENCION>',
		CONFEXME_16.tbrcED_Item( '37390' ).itematfd
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

BEGIN
--{
	UT_Trace.Trace( '********************** Generando ítems por bloque-franja ***********************', 5 );
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el item-bloque
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEMBLOQ;
	CONFEXME_16.tbrcED_ItemBloq( '37328' ).itblcodi := nuNextSeqValue;

	-- Se asigna el bloque-franja
	CONFEXME_16.tbrcED_ItemBloq( '37328' ).itblblfr := CONFEXME_16.tbrcED_BloqFran( '7160' ).blfrcodi;

	-- Se asigna el item
	if ( CONFEXME_16.tbrcED_Item.exists( '37353' ) ) then
		CONFEXME_16.tbrcED_ItemBloq( '37328' ).itblitem := CONFEXME_16.tbrcED_Item( '37353' ).itemcodi;
	end if;

	INSERT INTO ED_ItemBloq
	(
		itblcodi,
		itblitem,
		itblblfr,
		itblorde
	)
	VALUES
	(
		CONFEXME_16.tbrcED_ItemBloq( '37328' ).itblcodi,
		CONFEXME_16.tbrcED_ItemBloq( '37328' ).itblitem,
		CONFEXME_16.tbrcED_ItemBloq( '37328' ).itblblfr,
		1
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el item-bloque
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEMBLOQ;
	CONFEXME_16.tbrcED_ItemBloq( '37329' ).itblcodi := nuNextSeqValue;

	-- Se asigna el bloque-franja
	CONFEXME_16.tbrcED_ItemBloq( '37329' ).itblblfr := CONFEXME_16.tbrcED_BloqFran( '7160' ).blfrcodi;

	-- Se asigna el item
	if ( CONFEXME_16.tbrcED_Item.exists( '37354' ) ) then
		CONFEXME_16.tbrcED_ItemBloq( '37329' ).itblitem := CONFEXME_16.tbrcED_Item( '37354' ).itemcodi;
	end if;

	INSERT INTO ED_ItemBloq
	(
		itblcodi,
		itblitem,
		itblblfr,
		itblorde
	)
	VALUES
	(
		CONFEXME_16.tbrcED_ItemBloq( '37329' ).itblcodi,
		CONFEXME_16.tbrcED_ItemBloq( '37329' ).itblitem,
		CONFEXME_16.tbrcED_ItemBloq( '37329' ).itblblfr,
		2
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el item-bloque
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEMBLOQ;
	CONFEXME_16.tbrcED_ItemBloq( '37330' ).itblcodi := nuNextSeqValue;

	-- Se asigna el bloque-franja
	CONFEXME_16.tbrcED_ItemBloq( '37330' ).itblblfr := CONFEXME_16.tbrcED_BloqFran( '7160' ).blfrcodi;

	-- Se asigna el item
	if ( CONFEXME_16.tbrcED_Item.exists( '37355' ) ) then
		CONFEXME_16.tbrcED_ItemBloq( '37330' ).itblitem := CONFEXME_16.tbrcED_Item( '37355' ).itemcodi;
	end if;

	INSERT INTO ED_ItemBloq
	(
		itblcodi,
		itblitem,
		itblblfr,
		itblorde
	)
	VALUES
	(
		CONFEXME_16.tbrcED_ItemBloq( '37330' ).itblcodi,
		CONFEXME_16.tbrcED_ItemBloq( '37330' ).itblitem,
		CONFEXME_16.tbrcED_ItemBloq( '37330' ).itblblfr,
		3
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el item-bloque
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEMBLOQ;
	CONFEXME_16.tbrcED_ItemBloq( '37331' ).itblcodi := nuNextSeqValue;

	-- Se asigna el bloque-franja
	CONFEXME_16.tbrcED_ItemBloq( '37331' ).itblblfr := CONFEXME_16.tbrcED_BloqFran( '7160' ).blfrcodi;

	-- Se asigna el item
	if ( CONFEXME_16.tbrcED_Item.exists( '37356' ) ) then
		CONFEXME_16.tbrcED_ItemBloq( '37331' ).itblitem := CONFEXME_16.tbrcED_Item( '37356' ).itemcodi;
	end if;

	INSERT INTO ED_ItemBloq
	(
		itblcodi,
		itblitem,
		itblblfr,
		itblorde
	)
	VALUES
	(
		CONFEXME_16.tbrcED_ItemBloq( '37331' ).itblcodi,
		CONFEXME_16.tbrcED_ItemBloq( '37331' ).itblitem,
		CONFEXME_16.tbrcED_ItemBloq( '37331' ).itblblfr,
		4
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el item-bloque
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEMBLOQ;
	CONFEXME_16.tbrcED_ItemBloq( '37332' ).itblcodi := nuNextSeqValue;

	-- Se asigna el bloque-franja
	CONFEXME_16.tbrcED_ItemBloq( '37332' ).itblblfr := CONFEXME_16.tbrcED_BloqFran( '7160' ).blfrcodi;

	-- Se asigna el item
	if ( CONFEXME_16.tbrcED_Item.exists( '37357' ) ) then
		CONFEXME_16.tbrcED_ItemBloq( '37332' ).itblitem := CONFEXME_16.tbrcED_Item( '37357' ).itemcodi;
	end if;

	INSERT INTO ED_ItemBloq
	(
		itblcodi,
		itblitem,
		itblblfr,
		itblorde
	)
	VALUES
	(
		CONFEXME_16.tbrcED_ItemBloq( '37332' ).itblcodi,
		CONFEXME_16.tbrcED_ItemBloq( '37332' ).itblitem,
		CONFEXME_16.tbrcED_ItemBloq( '37332' ).itblblfr,
		5
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el item-bloque
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEMBLOQ;
	CONFEXME_16.tbrcED_ItemBloq( '37333' ).itblcodi := nuNextSeqValue;

	-- Se asigna el bloque-franja
	CONFEXME_16.tbrcED_ItemBloq( '37333' ).itblblfr := CONFEXME_16.tbrcED_BloqFran( '7160' ).blfrcodi;

	-- Se asigna el item
	if ( CONFEXME_16.tbrcED_Item.exists( '37358' ) ) then
		CONFEXME_16.tbrcED_ItemBloq( '37333' ).itblitem := CONFEXME_16.tbrcED_Item( '37358' ).itemcodi;
	end if;

	INSERT INTO ED_ItemBloq
	(
		itblcodi,
		itblitem,
		itblblfr,
		itblorde
	)
	VALUES
	(
		CONFEXME_16.tbrcED_ItemBloq( '37333' ).itblcodi,
		CONFEXME_16.tbrcED_ItemBloq( '37333' ).itblitem,
		CONFEXME_16.tbrcED_ItemBloq( '37333' ).itblblfr,
		6
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el item-bloque
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEMBLOQ;
	CONFEXME_16.tbrcED_ItemBloq( '37334' ).itblcodi := nuNextSeqValue;

	-- Se asigna el bloque-franja
	CONFEXME_16.tbrcED_ItemBloq( '37334' ).itblblfr := CONFEXME_16.tbrcED_BloqFran( '7157' ).blfrcodi;

	-- Se asigna el item
	if ( CONFEXME_16.tbrcED_Item.exists( '37359' ) ) then
		CONFEXME_16.tbrcED_ItemBloq( '37334' ).itblitem := CONFEXME_16.tbrcED_Item( '37359' ).itemcodi;
	end if;

	INSERT INTO ED_ItemBloq
	(
		itblcodi,
		itblitem,
		itblblfr,
		itblorde
	)
	VALUES
	(
		CONFEXME_16.tbrcED_ItemBloq( '37334' ).itblcodi,
		CONFEXME_16.tbrcED_ItemBloq( '37334' ).itblitem,
		CONFEXME_16.tbrcED_ItemBloq( '37334' ).itblblfr,
		1
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el item-bloque
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEMBLOQ;
	CONFEXME_16.tbrcED_ItemBloq( '37335' ).itblcodi := nuNextSeqValue;

	-- Se asigna el bloque-franja
	CONFEXME_16.tbrcED_ItemBloq( '37335' ).itblblfr := CONFEXME_16.tbrcED_BloqFran( '7157' ).blfrcodi;

	-- Se asigna el item
	if ( CONFEXME_16.tbrcED_Item.exists( '37360' ) ) then
		CONFEXME_16.tbrcED_ItemBloq( '37335' ).itblitem := CONFEXME_16.tbrcED_Item( '37360' ).itemcodi;
	end if;

	INSERT INTO ED_ItemBloq
	(
		itblcodi,
		itblitem,
		itblblfr,
		itblorde
	)
	VALUES
	(
		CONFEXME_16.tbrcED_ItemBloq( '37335' ).itblcodi,
		CONFEXME_16.tbrcED_ItemBloq( '37335' ).itblitem,
		CONFEXME_16.tbrcED_ItemBloq( '37335' ).itblblfr,
		2
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el item-bloque
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEMBLOQ;
	CONFEXME_16.tbrcED_ItemBloq( '37336' ).itblcodi := nuNextSeqValue;

	-- Se asigna el bloque-franja
	CONFEXME_16.tbrcED_ItemBloq( '37336' ).itblblfr := CONFEXME_16.tbrcED_BloqFran( '7157' ).blfrcodi;

	-- Se asigna el item
	if ( CONFEXME_16.tbrcED_Item.exists( '37361' ) ) then
		CONFEXME_16.tbrcED_ItemBloq( '37336' ).itblitem := CONFEXME_16.tbrcED_Item( '37361' ).itemcodi;
	end if;

	INSERT INTO ED_ItemBloq
	(
		itblcodi,
		itblitem,
		itblblfr,
		itblorde
	)
	VALUES
	(
		CONFEXME_16.tbrcED_ItemBloq( '37336' ).itblcodi,
		CONFEXME_16.tbrcED_ItemBloq( '37336' ).itblitem,
		CONFEXME_16.tbrcED_ItemBloq( '37336' ).itblblfr,
		10
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el item-bloque
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEMBLOQ;
	CONFEXME_16.tbrcED_ItemBloq( '37337' ).itblcodi := nuNextSeqValue;

	-- Se asigna el bloque-franja
	CONFEXME_16.tbrcED_ItemBloq( '37337' ).itblblfr := CONFEXME_16.tbrcED_BloqFran( '7157' ).blfrcodi;

	-- Se asigna el item
	if ( CONFEXME_16.tbrcED_Item.exists( '37362' ) ) then
		CONFEXME_16.tbrcED_ItemBloq( '37337' ).itblitem := CONFEXME_16.tbrcED_Item( '37362' ).itemcodi;
	end if;

	INSERT INTO ED_ItemBloq
	(
		itblcodi,
		itblitem,
		itblblfr,
		itblorde
	)
	VALUES
	(
		CONFEXME_16.tbrcED_ItemBloq( '37337' ).itblcodi,
		CONFEXME_16.tbrcED_ItemBloq( '37337' ).itblitem,
		CONFEXME_16.tbrcED_ItemBloq( '37337' ).itblblfr,
		12
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el item-bloque
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEMBLOQ;
	CONFEXME_16.tbrcED_ItemBloq( '37338' ).itblcodi := nuNextSeqValue;

	-- Se asigna el bloque-franja
	CONFEXME_16.tbrcED_ItemBloq( '37338' ).itblblfr := CONFEXME_16.tbrcED_BloqFran( '7157' ).blfrcodi;

	-- Se asigna el item
	if ( CONFEXME_16.tbrcED_Item.exists( '37363' ) ) then
		CONFEXME_16.tbrcED_ItemBloq( '37338' ).itblitem := CONFEXME_16.tbrcED_Item( '37363' ).itemcodi;
	end if;

	INSERT INTO ED_ItemBloq
	(
		itblcodi,
		itblitem,
		itblblfr,
		itblorde
	)
	VALUES
	(
		CONFEXME_16.tbrcED_ItemBloq( '37338' ).itblcodi,
		CONFEXME_16.tbrcED_ItemBloq( '37338' ).itblitem,
		CONFEXME_16.tbrcED_ItemBloq( '37338' ).itblblfr,
		13
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el item-bloque
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEMBLOQ;
	CONFEXME_16.tbrcED_ItemBloq( '37339' ).itblcodi := nuNextSeqValue;

	-- Se asigna el bloque-franja
	CONFEXME_16.tbrcED_ItemBloq( '37339' ).itblblfr := CONFEXME_16.tbrcED_BloqFran( '7157' ).blfrcodi;

	-- Se asigna el item
	if ( CONFEXME_16.tbrcED_Item.exists( '37364' ) ) then
		CONFEXME_16.tbrcED_ItemBloq( '37339' ).itblitem := CONFEXME_16.tbrcED_Item( '37364' ).itemcodi;
	end if;

	INSERT INTO ED_ItemBloq
	(
		itblcodi,
		itblitem,
		itblblfr,
		itblorde
	)
	VALUES
	(
		CONFEXME_16.tbrcED_ItemBloq( '37339' ).itblcodi,
		CONFEXME_16.tbrcED_ItemBloq( '37339' ).itblitem,
		CONFEXME_16.tbrcED_ItemBloq( '37339' ).itblblfr,
		14
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el item-bloque
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEMBLOQ;
	CONFEXME_16.tbrcED_ItemBloq( '37340' ).itblcodi := nuNextSeqValue;

	-- Se asigna el bloque-franja
	CONFEXME_16.tbrcED_ItemBloq( '37340' ).itblblfr := CONFEXME_16.tbrcED_BloqFran( '7157' ).blfrcodi;

	-- Se asigna el item
	if ( CONFEXME_16.tbrcED_Item.exists( '37365' ) ) then
		CONFEXME_16.tbrcED_ItemBloq( '37340' ).itblitem := CONFEXME_16.tbrcED_Item( '37365' ).itemcodi;
	end if;

	INSERT INTO ED_ItemBloq
	(
		itblcodi,
		itblitem,
		itblblfr,
		itblorde
	)
	VALUES
	(
		CONFEXME_16.tbrcED_ItemBloq( '37340' ).itblcodi,
		CONFEXME_16.tbrcED_ItemBloq( '37340' ).itblitem,
		CONFEXME_16.tbrcED_ItemBloq( '37340' ).itblblfr,
		15
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el item-bloque
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEMBLOQ;
	CONFEXME_16.tbrcED_ItemBloq( '37341' ).itblcodi := nuNextSeqValue;

	-- Se asigna el bloque-franja
	CONFEXME_16.tbrcED_ItemBloq( '37341' ).itblblfr := CONFEXME_16.tbrcED_BloqFran( '7157' ).blfrcodi;

	-- Se asigna el item
	if ( CONFEXME_16.tbrcED_Item.exists( '37366' ) ) then
		CONFEXME_16.tbrcED_ItemBloq( '37341' ).itblitem := CONFEXME_16.tbrcED_Item( '37366' ).itemcodi;
	end if;

	INSERT INTO ED_ItemBloq
	(
		itblcodi,
		itblitem,
		itblblfr,
		itblorde
	)
	VALUES
	(
		CONFEXME_16.tbrcED_ItemBloq( '37341' ).itblcodi,
		CONFEXME_16.tbrcED_ItemBloq( '37341' ).itblitem,
		CONFEXME_16.tbrcED_ItemBloq( '37341' ).itblblfr,
		16
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el item-bloque
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEMBLOQ;
	CONFEXME_16.tbrcED_ItemBloq( '37342' ).itblcodi := nuNextSeqValue;

	-- Se asigna el bloque-franja
	CONFEXME_16.tbrcED_ItemBloq( '37342' ).itblblfr := CONFEXME_16.tbrcED_BloqFran( '7157' ).blfrcodi;

	-- Se asigna el item
	if ( CONFEXME_16.tbrcED_Item.exists( '37367' ) ) then
		CONFEXME_16.tbrcED_ItemBloq( '37342' ).itblitem := CONFEXME_16.tbrcED_Item( '37367' ).itemcodi;
	end if;

	INSERT INTO ED_ItemBloq
	(
		itblcodi,
		itblitem,
		itblblfr,
		itblorde
	)
	VALUES
	(
		CONFEXME_16.tbrcED_ItemBloq( '37342' ).itblcodi,
		CONFEXME_16.tbrcED_ItemBloq( '37342' ).itblitem,
		CONFEXME_16.tbrcED_ItemBloq( '37342' ).itblblfr,
		17
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el item-bloque
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEMBLOQ;
	CONFEXME_16.tbrcED_ItemBloq( '37343' ).itblcodi := nuNextSeqValue;

	-- Se asigna el bloque-franja
	CONFEXME_16.tbrcED_ItemBloq( '37343' ).itblblfr := CONFEXME_16.tbrcED_BloqFran( '7158' ).blfrcodi;

	-- Se asigna el item
	if ( CONFEXME_16.tbrcED_Item.exists( '37368' ) ) then
		CONFEXME_16.tbrcED_ItemBloq( '37343' ).itblitem := CONFEXME_16.tbrcED_Item( '37368' ).itemcodi;
	end if;

	INSERT INTO ED_ItemBloq
	(
		itblcodi,
		itblitem,
		itblblfr,
		itblorde
	)
	VALUES
	(
		CONFEXME_16.tbrcED_ItemBloq( '37343' ).itblcodi,
		CONFEXME_16.tbrcED_ItemBloq( '37343' ).itblitem,
		CONFEXME_16.tbrcED_ItemBloq( '37343' ).itblblfr,
		1
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el item-bloque
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEMBLOQ;
	CONFEXME_16.tbrcED_ItemBloq( '37344' ).itblcodi := nuNextSeqValue;

	-- Se asigna el bloque-franja
	CONFEXME_16.tbrcED_ItemBloq( '37344' ).itblblfr := CONFEXME_16.tbrcED_BloqFran( '7158' ).blfrcodi;

	-- Se asigna el item
	if ( CONFEXME_16.tbrcED_Item.exists( '37369' ) ) then
		CONFEXME_16.tbrcED_ItemBloq( '37344' ).itblitem := CONFEXME_16.tbrcED_Item( '37369' ).itemcodi;
	end if;

	INSERT INTO ED_ItemBloq
	(
		itblcodi,
		itblitem,
		itblblfr,
		itblorde
	)
	VALUES
	(
		CONFEXME_16.tbrcED_ItemBloq( '37344' ).itblcodi,
		CONFEXME_16.tbrcED_ItemBloq( '37344' ).itblitem,
		CONFEXME_16.tbrcED_ItemBloq( '37344' ).itblblfr,
		2
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el item-bloque
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEMBLOQ;
	CONFEXME_16.tbrcED_ItemBloq( '37345' ).itblcodi := nuNextSeqValue;

	-- Se asigna el bloque-franja
	CONFEXME_16.tbrcED_ItemBloq( '37345' ).itblblfr := CONFEXME_16.tbrcED_BloqFran( '7158' ).blfrcodi;

	-- Se asigna el item
	if ( CONFEXME_16.tbrcED_Item.exists( '37370' ) ) then
		CONFEXME_16.tbrcED_ItemBloq( '37345' ).itblitem := CONFEXME_16.tbrcED_Item( '37370' ).itemcodi;
	end if;

	INSERT INTO ED_ItemBloq
	(
		itblcodi,
		itblitem,
		itblblfr,
		itblorde
	)
	VALUES
	(
		CONFEXME_16.tbrcED_ItemBloq( '37345' ).itblcodi,
		CONFEXME_16.tbrcED_ItemBloq( '37345' ).itblitem,
		CONFEXME_16.tbrcED_ItemBloq( '37345' ).itblblfr,
		3
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el item-bloque
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEMBLOQ;
	CONFEXME_16.tbrcED_ItemBloq( '37346' ).itblcodi := nuNextSeqValue;

	-- Se asigna el bloque-franja
	CONFEXME_16.tbrcED_ItemBloq( '37346' ).itblblfr := CONFEXME_16.tbrcED_BloqFran( '7158' ).blfrcodi;

	-- Se asigna el item
	if ( CONFEXME_16.tbrcED_Item.exists( '37371' ) ) then
		CONFEXME_16.tbrcED_ItemBloq( '37346' ).itblitem := CONFEXME_16.tbrcED_Item( '37371' ).itemcodi;
	end if;

	INSERT INTO ED_ItemBloq
	(
		itblcodi,
		itblitem,
		itblblfr,
		itblorde
	)
	VALUES
	(
		CONFEXME_16.tbrcED_ItemBloq( '37346' ).itblcodi,
		CONFEXME_16.tbrcED_ItemBloq( '37346' ).itblitem,
		CONFEXME_16.tbrcED_ItemBloq( '37346' ).itblblfr,
		4
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el item-bloque
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEMBLOQ;
	CONFEXME_16.tbrcED_ItemBloq( '37347' ).itblcodi := nuNextSeqValue;

	-- Se asigna el bloque-franja
	CONFEXME_16.tbrcED_ItemBloq( '37347' ).itblblfr := CONFEXME_16.tbrcED_BloqFran( '7158' ).blfrcodi;

	-- Se asigna el item
	if ( CONFEXME_16.tbrcED_Item.exists( '37372' ) ) then
		CONFEXME_16.tbrcED_ItemBloq( '37347' ).itblitem := CONFEXME_16.tbrcED_Item( '37372' ).itemcodi;
	end if;

	INSERT INTO ED_ItemBloq
	(
		itblcodi,
		itblitem,
		itblblfr,
		itblorde
	)
	VALUES
	(
		CONFEXME_16.tbrcED_ItemBloq( '37347' ).itblcodi,
		CONFEXME_16.tbrcED_ItemBloq( '37347' ).itblitem,
		CONFEXME_16.tbrcED_ItemBloq( '37347' ).itblblfr,
		5
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el item-bloque
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEMBLOQ;
	CONFEXME_16.tbrcED_ItemBloq( '37348' ).itblcodi := nuNextSeqValue;

	-- Se asigna el bloque-franja
	CONFEXME_16.tbrcED_ItemBloq( '37348' ).itblblfr := CONFEXME_16.tbrcED_BloqFran( '7158' ).blfrcodi;

	-- Se asigna el item
	if ( CONFEXME_16.tbrcED_Item.exists( '37373' ) ) then
		CONFEXME_16.tbrcED_ItemBloq( '37348' ).itblitem := CONFEXME_16.tbrcED_Item( '37373' ).itemcodi;
	end if;

	INSERT INTO ED_ItemBloq
	(
		itblcodi,
		itblitem,
		itblblfr,
		itblorde
	)
	VALUES
	(
		CONFEXME_16.tbrcED_ItemBloq( '37348' ).itblcodi,
		CONFEXME_16.tbrcED_ItemBloq( '37348' ).itblitem,
		CONFEXME_16.tbrcED_ItemBloq( '37348' ).itblblfr,
		6
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el item-bloque
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEMBLOQ;
	CONFEXME_16.tbrcED_ItemBloq( '37349' ).itblcodi := nuNextSeqValue;

	-- Se asigna el bloque-franja
	CONFEXME_16.tbrcED_ItemBloq( '37349' ).itblblfr := CONFEXME_16.tbrcED_BloqFran( '7158' ).blfrcodi;

	-- Se asigna el item
	if ( CONFEXME_16.tbrcED_Item.exists( '37374' ) ) then
		CONFEXME_16.tbrcED_ItemBloq( '37349' ).itblitem := CONFEXME_16.tbrcED_Item( '37374' ).itemcodi;
	end if;

	INSERT INTO ED_ItemBloq
	(
		itblcodi,
		itblitem,
		itblblfr,
		itblorde
	)
	VALUES
	(
		CONFEXME_16.tbrcED_ItemBloq( '37349' ).itblcodi,
		CONFEXME_16.tbrcED_ItemBloq( '37349' ).itblitem,
		CONFEXME_16.tbrcED_ItemBloq( '37349' ).itblblfr,
		7
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el item-bloque
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEMBLOQ;
	CONFEXME_16.tbrcED_ItemBloq( '37350' ).itblcodi := nuNextSeqValue;

	-- Se asigna el bloque-franja
	CONFEXME_16.tbrcED_ItemBloq( '37350' ).itblblfr := CONFEXME_16.tbrcED_BloqFran( '7158' ).blfrcodi;

	-- Se asigna el item
	if ( CONFEXME_16.tbrcED_Item.exists( '37375' ) ) then
		CONFEXME_16.tbrcED_ItemBloq( '37350' ).itblitem := CONFEXME_16.tbrcED_Item( '37375' ).itemcodi;
	end if;

	INSERT INTO ED_ItemBloq
	(
		itblcodi,
		itblitem,
		itblblfr,
		itblorde
	)
	VALUES
	(
		CONFEXME_16.tbrcED_ItemBloq( '37350' ).itblcodi,
		CONFEXME_16.tbrcED_ItemBloq( '37350' ).itblitem,
		CONFEXME_16.tbrcED_ItemBloq( '37350' ).itblblfr,
		8
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el item-bloque
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEMBLOQ;
	CONFEXME_16.tbrcED_ItemBloq( '37351' ).itblcodi := nuNextSeqValue;

	-- Se asigna el bloque-franja
	CONFEXME_16.tbrcED_ItemBloq( '37351' ).itblblfr := CONFEXME_16.tbrcED_BloqFran( '7158' ).blfrcodi;

	-- Se asigna el item
	if ( CONFEXME_16.tbrcED_Item.exists( '37376' ) ) then
		CONFEXME_16.tbrcED_ItemBloq( '37351' ).itblitem := CONFEXME_16.tbrcED_Item( '37376' ).itemcodi;
	end if;

	INSERT INTO ED_ItemBloq
	(
		itblcodi,
		itblitem,
		itblblfr,
		itblorde
	)
	VALUES
	(
		CONFEXME_16.tbrcED_ItemBloq( '37351' ).itblcodi,
		CONFEXME_16.tbrcED_ItemBloq( '37351' ).itblitem,
		CONFEXME_16.tbrcED_ItemBloq( '37351' ).itblblfr,
		9
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el item-bloque
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEMBLOQ;
	CONFEXME_16.tbrcED_ItemBloq( '37352' ).itblcodi := nuNextSeqValue;

	-- Se asigna el bloque-franja
	CONFEXME_16.tbrcED_ItemBloq( '37352' ).itblblfr := CONFEXME_16.tbrcED_BloqFran( '7159' ).blfrcodi;

	-- Se asigna el item
	if ( CONFEXME_16.tbrcED_Item.exists( '37377' ) ) then
		CONFEXME_16.tbrcED_ItemBloq( '37352' ).itblitem := CONFEXME_16.tbrcED_Item( '37377' ).itemcodi;
	end if;

	INSERT INTO ED_ItemBloq
	(
		itblcodi,
		itblitem,
		itblblfr,
		itblorde
	)
	VALUES
	(
		CONFEXME_16.tbrcED_ItemBloq( '37352' ).itblcodi,
		CONFEXME_16.tbrcED_ItemBloq( '37352' ).itblitem,
		CONFEXME_16.tbrcED_ItemBloq( '37352' ).itblblfr,
		1
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el item-bloque
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEMBLOQ;
	CONFEXME_16.tbrcED_ItemBloq( '37353' ).itblcodi := nuNextSeqValue;

	-- Se asigna el bloque-franja
	CONFEXME_16.tbrcED_ItemBloq( '37353' ).itblblfr := CONFEXME_16.tbrcED_BloqFran( '7159' ).blfrcodi;

	-- Se asigna el item
	if ( CONFEXME_16.tbrcED_Item.exists( '37378' ) ) then
		CONFEXME_16.tbrcED_ItemBloq( '37353' ).itblitem := CONFEXME_16.tbrcED_Item( '37378' ).itemcodi;
	end if;

	INSERT INTO ED_ItemBloq
	(
		itblcodi,
		itblitem,
		itblblfr,
		itblorde
	)
	VALUES
	(
		CONFEXME_16.tbrcED_ItemBloq( '37353' ).itblcodi,
		CONFEXME_16.tbrcED_ItemBloq( '37353' ).itblitem,
		CONFEXME_16.tbrcED_ItemBloq( '37353' ).itblblfr,
		2
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el item-bloque
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEMBLOQ;
	CONFEXME_16.tbrcED_ItemBloq( '37354' ).itblcodi := nuNextSeqValue;

	-- Se asigna el bloque-franja
	CONFEXME_16.tbrcED_ItemBloq( '37354' ).itblblfr := CONFEXME_16.tbrcED_BloqFran( '7159' ).blfrcodi;

	-- Se asigna el item
	if ( CONFEXME_16.tbrcED_Item.exists( '37379' ) ) then
		CONFEXME_16.tbrcED_ItemBloq( '37354' ).itblitem := CONFEXME_16.tbrcED_Item( '37379' ).itemcodi;
	end if;

	INSERT INTO ED_ItemBloq
	(
		itblcodi,
		itblitem,
		itblblfr,
		itblorde
	)
	VALUES
	(
		CONFEXME_16.tbrcED_ItemBloq( '37354' ).itblcodi,
		CONFEXME_16.tbrcED_ItemBloq( '37354' ).itblitem,
		CONFEXME_16.tbrcED_ItemBloq( '37354' ).itblblfr,
		3
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el item-bloque
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEMBLOQ;
	CONFEXME_16.tbrcED_ItemBloq( '37355' ).itblcodi := nuNextSeqValue;

	-- Se asigna el bloque-franja
	CONFEXME_16.tbrcED_ItemBloq( '37355' ).itblblfr := CONFEXME_16.tbrcED_BloqFran( '7159' ).blfrcodi;

	-- Se asigna el item
	if ( CONFEXME_16.tbrcED_Item.exists( '37380' ) ) then
		CONFEXME_16.tbrcED_ItemBloq( '37355' ).itblitem := CONFEXME_16.tbrcED_Item( '37380' ).itemcodi;
	end if;

	INSERT INTO ED_ItemBloq
	(
		itblcodi,
		itblitem,
		itblblfr,
		itblorde
	)
	VALUES
	(
		CONFEXME_16.tbrcED_ItemBloq( '37355' ).itblcodi,
		CONFEXME_16.tbrcED_ItemBloq( '37355' ).itblitem,
		CONFEXME_16.tbrcED_ItemBloq( '37355' ).itblblfr,
		4
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el item-bloque
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEMBLOQ;
	CONFEXME_16.tbrcED_ItemBloq( '37356' ).itblcodi := nuNextSeqValue;

	-- Se asigna el bloque-franja
	CONFEXME_16.tbrcED_ItemBloq( '37356' ).itblblfr := CONFEXME_16.tbrcED_BloqFran( '7159' ).blfrcodi;

	-- Se asigna el item
	if ( CONFEXME_16.tbrcED_Item.exists( '37381' ) ) then
		CONFEXME_16.tbrcED_ItemBloq( '37356' ).itblitem := CONFEXME_16.tbrcED_Item( '37381' ).itemcodi;
	end if;

	INSERT INTO ED_ItemBloq
	(
		itblcodi,
		itblitem,
		itblblfr,
		itblorde
	)
	VALUES
	(
		CONFEXME_16.tbrcED_ItemBloq( '37356' ).itblcodi,
		CONFEXME_16.tbrcED_ItemBloq( '37356' ).itblitem,
		CONFEXME_16.tbrcED_ItemBloq( '37356' ).itblblfr,
		5
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el item-bloque
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEMBLOQ;
	CONFEXME_16.tbrcED_ItemBloq( '37357' ).itblcodi := nuNextSeqValue;

	-- Se asigna el bloque-franja
	CONFEXME_16.tbrcED_ItemBloq( '37357' ).itblblfr := CONFEXME_16.tbrcED_BloqFran( '7159' ).blfrcodi;

	-- Se asigna el item
	if ( CONFEXME_16.tbrcED_Item.exists( '37382' ) ) then
		CONFEXME_16.tbrcED_ItemBloq( '37357' ).itblitem := CONFEXME_16.tbrcED_Item( '37382' ).itemcodi;
	end if;

	INSERT INTO ED_ItemBloq
	(
		itblcodi,
		itblitem,
		itblblfr,
		itblorde
	)
	VALUES
	(
		CONFEXME_16.tbrcED_ItemBloq( '37357' ).itblcodi,
		CONFEXME_16.tbrcED_ItemBloq( '37357' ).itblitem,
		CONFEXME_16.tbrcED_ItemBloq( '37357' ).itblblfr,
		6
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el item-bloque
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEMBLOQ;
	CONFEXME_16.tbrcED_ItemBloq( '37358' ).itblcodi := nuNextSeqValue;

	-- Se asigna el bloque-franja
	CONFEXME_16.tbrcED_ItemBloq( '37358' ).itblblfr := CONFEXME_16.tbrcED_BloqFran( '7159' ).blfrcodi;

	-- Se asigna el item
	if ( CONFEXME_16.tbrcED_Item.exists( '37383' ) ) then
		CONFEXME_16.tbrcED_ItemBloq( '37358' ).itblitem := CONFEXME_16.tbrcED_Item( '37383' ).itemcodi;
	end if;

	INSERT INTO ED_ItemBloq
	(
		itblcodi,
		itblitem,
		itblblfr,
		itblorde
	)
	VALUES
	(
		CONFEXME_16.tbrcED_ItemBloq( '37358' ).itblcodi,
		CONFEXME_16.tbrcED_ItemBloq( '37358' ).itblitem,
		CONFEXME_16.tbrcED_ItemBloq( '37358' ).itblblfr,
		7
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el item-bloque
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEMBLOQ;
	CONFEXME_16.tbrcED_ItemBloq( '37359' ).itblcodi := nuNextSeqValue;

	-- Se asigna el bloque-franja
	CONFEXME_16.tbrcED_ItemBloq( '37359' ).itblblfr := CONFEXME_16.tbrcED_BloqFran( '7159' ).blfrcodi;

	-- Se asigna el item
	if ( CONFEXME_16.tbrcED_Item.exists( '37384' ) ) then
		CONFEXME_16.tbrcED_ItemBloq( '37359' ).itblitem := CONFEXME_16.tbrcED_Item( '37384' ).itemcodi;
	end if;

	INSERT INTO ED_ItemBloq
	(
		itblcodi,
		itblitem,
		itblblfr,
		itblorde
	)
	VALUES
	(
		CONFEXME_16.tbrcED_ItemBloq( '37359' ).itblcodi,
		CONFEXME_16.tbrcED_ItemBloq( '37359' ).itblitem,
		CONFEXME_16.tbrcED_ItemBloq( '37359' ).itblblfr,
		8
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el item-bloque
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEMBLOQ;
	CONFEXME_16.tbrcED_ItemBloq( '37360' ).itblcodi := nuNextSeqValue;

	-- Se asigna el bloque-franja
	CONFEXME_16.tbrcED_ItemBloq( '37360' ).itblblfr := CONFEXME_16.tbrcED_BloqFran( '7159' ).blfrcodi;

	-- Se asigna el item
	if ( CONFEXME_16.tbrcED_Item.exists( '37385' ) ) then
		CONFEXME_16.tbrcED_ItemBloq( '37360' ).itblitem := CONFEXME_16.tbrcED_Item( '37385' ).itemcodi;
	end if;

	INSERT INTO ED_ItemBloq
	(
		itblcodi,
		itblitem,
		itblblfr,
		itblorde
	)
	VALUES
	(
		CONFEXME_16.tbrcED_ItemBloq( '37360' ).itblcodi,
		CONFEXME_16.tbrcED_ItemBloq( '37360' ).itblitem,
		CONFEXME_16.tbrcED_ItemBloq( '37360' ).itblblfr,
		9
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el item-bloque
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEMBLOQ;
	CONFEXME_16.tbrcED_ItemBloq( '37361' ).itblcodi := nuNextSeqValue;

	-- Se asigna el bloque-franja
	CONFEXME_16.tbrcED_ItemBloq( '37361' ).itblblfr := CONFEXME_16.tbrcED_BloqFran( '7159' ).blfrcodi;

	-- Se asigna el item
	if ( CONFEXME_16.tbrcED_Item.exists( '37386' ) ) then
		CONFEXME_16.tbrcED_ItemBloq( '37361' ).itblitem := CONFEXME_16.tbrcED_Item( '37386' ).itemcodi;
	end if;

	INSERT INTO ED_ItemBloq
	(
		itblcodi,
		itblitem,
		itblblfr,
		itblorde
	)
	VALUES
	(
		CONFEXME_16.tbrcED_ItemBloq( '37361' ).itblcodi,
		CONFEXME_16.tbrcED_ItemBloq( '37361' ).itblitem,
		CONFEXME_16.tbrcED_ItemBloq( '37361' ).itblblfr,
		10
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el item-bloque
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEMBLOQ;
	CONFEXME_16.tbrcED_ItemBloq( '37362' ).itblcodi := nuNextSeqValue;

	-- Se asigna el bloque-franja
	CONFEXME_16.tbrcED_ItemBloq( '37362' ).itblblfr := CONFEXME_16.tbrcED_BloqFran( '7159' ).blfrcodi;

	-- Se asigna el item
	if ( CONFEXME_16.tbrcED_Item.exists( '37387' ) ) then
		CONFEXME_16.tbrcED_ItemBloq( '37362' ).itblitem := CONFEXME_16.tbrcED_Item( '37387' ).itemcodi;
	end if;

	INSERT INTO ED_ItemBloq
	(
		itblcodi,
		itblitem,
		itblblfr,
		itblorde
	)
	VALUES
	(
		CONFEXME_16.tbrcED_ItemBloq( '37362' ).itblcodi,
		CONFEXME_16.tbrcED_ItemBloq( '37362' ).itblitem,
		CONFEXME_16.tbrcED_ItemBloq( '37362' ).itblblfr,
		11
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el item-bloque
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEMBLOQ;
	CONFEXME_16.tbrcED_ItemBloq( '37363' ).itblcodi := nuNextSeqValue;

	-- Se asigna el bloque-franja
	CONFEXME_16.tbrcED_ItemBloq( '37363' ).itblblfr := CONFEXME_16.tbrcED_BloqFran( '7159' ).blfrcodi;

	-- Se asigna el item
	if ( CONFEXME_16.tbrcED_Item.exists( '37388' ) ) then
		CONFEXME_16.tbrcED_ItemBloq( '37363' ).itblitem := CONFEXME_16.tbrcED_Item( '37388' ).itemcodi;
	end if;

	INSERT INTO ED_ItemBloq
	(
		itblcodi,
		itblitem,
		itblblfr,
		itblorde
	)
	VALUES
	(
		CONFEXME_16.tbrcED_ItemBloq( '37363' ).itblcodi,
		CONFEXME_16.tbrcED_ItemBloq( '37363' ).itblitem,
		CONFEXME_16.tbrcED_ItemBloq( '37363' ).itblblfr,
		12
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el item-bloque
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEMBLOQ;
	CONFEXME_16.tbrcED_ItemBloq( '37364' ).itblcodi := nuNextSeqValue;

	-- Se asigna el bloque-franja
	CONFEXME_16.tbrcED_ItemBloq( '37364' ).itblblfr := CONFEXME_16.tbrcED_BloqFran( '7159' ).blfrcodi;

	-- Se asigna el item
	if ( CONFEXME_16.tbrcED_Item.exists( '37389' ) ) then
		CONFEXME_16.tbrcED_ItemBloq( '37364' ).itblitem := CONFEXME_16.tbrcED_Item( '37389' ).itemcodi;
	end if;

	INSERT INTO ED_ItemBloq
	(
		itblcodi,
		itblitem,
		itblblfr,
		itblorde
	)
	VALUES
	(
		CONFEXME_16.tbrcED_ItemBloq( '37364' ).itblcodi,
		CONFEXME_16.tbrcED_ItemBloq( '37364' ).itblitem,
		CONFEXME_16.tbrcED_ItemBloq( '37364' ).itblblfr,
		13
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	nuNextSeqValue      number;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se genera un identificador para el item-bloque
	nuNextSeqValue := pkBOInsertMgr.NextED_ITEMBLOQ;
	CONFEXME_16.tbrcED_ItemBloq( '37365' ).itblcodi := nuNextSeqValue;

	-- Se asigna el bloque-franja
	CONFEXME_16.tbrcED_ItemBloq( '37365' ).itblblfr := CONFEXME_16.tbrcED_BloqFran( '7159' ).blfrcodi;

	-- Se asigna el item
	if ( CONFEXME_16.tbrcED_Item.exists( '37390' ) ) then
		CONFEXME_16.tbrcED_ItemBloq( '37365' ).itblitem := CONFEXME_16.tbrcED_Item( '37390' ).itemcodi;
	end if;

	INSERT INTO ED_ItemBloq
	(
		itblcodi,
		itblitem,
		itblblfr,
		itblorde
	)
	VALUES
	(
		CONFEXME_16.tbrcED_ItemBloq( '37365' ).itblcodi,
		CONFEXME_16.tbrcED_ItemBloq( '37365' ).itblitem,
		CONFEXME_16.tbrcED_ItemBloq( '37365' ).itblblfr,
		14
	);

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

BEGIN
--{
	UT_Trace.Trace( '****************************** Generando plantilla *******************************', 5 );
--}
END;
/

DECLARE

	-- Almacena el valor generado por una secuencia
	blContent           blob;

	-- Identificador plantilla
	nuIdPlantill        ed_plantill.plancodi%type;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se escribe el CLOB
	dbms_lob.createtemporary(blContent , true); 

	dbms_lob.writeappend 
	(
		blContent , 
		16000, 
		hextoraw
		(
			''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		)
	);

	dbms_lob.writeappend 
	(
		blContent , 
		16000, 
		hextoraw
		(
			''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		)
	);

	dbms_lob.writeappend 
	(
		blContent , 
		16000, 
		hextoraw
		(
			''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		)
	);

	dbms_lob.writeappend 
	(
		blContent , 
		16000, 
		hextoraw
		(
			''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		)
	);

	dbms_lob.writeappend 
	(
		blContent , 
		16000, 
		hextoraw
		(
			''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		)
	);

	dbms_lob.writeappend 
	(
		blContent , 
		16000, 
		hextoraw
		(
			''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		)
	);

	dbms_lob.writeappend 
	(
		blContent , 
		16000, 
		hextoraw
		(
			''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		)
	);

	dbms_lob.writeappend 
	(
		blContent , 
		16000, 
		hextoraw
		(
			''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		)
	);

	dbms_lob.writeappend 
	(
		blContent , 
		9190, 
		hextoraw
		(
			''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		||	''
		)
	);

	-- Se obtiene código de la plantilla con el identificador especificado
	open  CONFEXME_16.cuPlantilla( 2 );
	fetch CONFEXME_16.cuPlantilla into nuIdPlantill;
	close CONFEXME_16.cuPlantilla;

	-- Se verifica si existe un formato con el delimitador especificado, en cuyo caso se elimina
	if ( nuIdPlantill is not NULL ) then
	--{
		-- Se modifica el formato
		UPDATE ed_plantill  
		SET    plancont = blContent ,
		       plandesc = 'PLANTILLA CONSTANCIA PAGOS',
		       plannomb = 'LDC_CONSTANCIA_PAGOS',
		       planopen = 'N',
		       plansist = 99
		WHERE  plancodi = 2;
	--}
	else
	--{
		-- Se inserta el formato
		INSERT INTO ed_plantill  
		(
			plancodi,
			plancont,
			plandesc,
			plannomb,
			planopen,
			plansist
		)
		VALUES
		(
			2,
			blContent ,
			'PLANTILLA CONSTANCIA PAGOS',
			'LDC_CONSTANCIA_PAGOS',
			'N',
			99
		);
	--}
	end if;

		dbms_lob.freetemporary(blContent); 
EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

BEGIN
--{
	UT_Trace.Trace( '****************************** Generando Configuración de Extracción y mezcla *******************************', 5 );
--}
END;
/

DECLARE

	-- Identificador del extractor y mezcla
	nuExtractAndMixId    ed_confexme.coemcodi%type;

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	-- Se obtiene código del Extractor y mezcla con el identificador especificado
	open  CONFEXME_16.cuExtractAndMix( 16 );
	fetch CONFEXME_16.cuExtractAndMix into nuExtractAndMixId;
	close CONFEXME_16.cuExtractAndMix;

	-- Se verifica si existe un formato con el delimitador especificado, en cuyo caso se elimina
	if ( nuExtractAndMixId is not NULL ) then
	--{
		-- Se modifica el formato
		UPDATE ed_confexme  
		SET    coemdesc = 'CONSTANCIA_PAGOS',
		       coeminic = NULL,
		       coempada = '<23>',
		       coempadi = 'LDC_CONSTANCIA_PAGOS',
		       coempame = NULL,
		       coemtido = 134,
		       coemvers = 0,
		       coemvige = 'S'
		WHERE  coemcodi = 16;
	--}
	else
	--{
		-- Se inserta configuración de extracción y mezcla
		INSERT INTO ed_confexme 
		(
			coemcodi,
			coemdesc,
			coeminic,
			coempada,
			coempadi,
			coempame,
			coemtido,
			coemvers,
			coemvige
		)
			VALUES
		(
			16,
			'CONSTANCIA_PAGOS',
			NULL,
			'<23>',
			'LDC_CONSTANCIA_PAGOS',
			NULL,
			134,
			0,
			'S'
		);
	--}
	end if;

EXCEPTION

	when OTHERS then
		CONFEXME_16.boProcessStatusOK := FALSE;
		ROLLBACK;
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

BEGIN
--{
	if ( CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	UT_Trace.Trace( '*********************** Borrando expresiones generadas *************************', 5 );
	CONFEXME_16.DeleteGeneratedExpressions;

EXCEPTION

	when OTHERS then
		UT_Trace.Trace( 'ERROR -> ' || sqlerrm, 5 );
		raise;
--}
END;
/

BEGIN
--{
	if ( not CONFEXME_16.boProcessStatusOK ) then
	--{
		-- No realiza nada
		return;
	--}
	end if;

	UT_Trace.Trace( '***************** Realizando persistencia en la base de datos ******************', 5 );
	COMMIT;
--}
END;
/

BEGIN
--{
	UT_Trace.Trace( '********************** Borrando paquete CONFEXME_16 ***********************', 5 );
	SA_BOCreatePackages.DropPackage
	(
		'CONFEXME_16'
	);
--}
END;
/

