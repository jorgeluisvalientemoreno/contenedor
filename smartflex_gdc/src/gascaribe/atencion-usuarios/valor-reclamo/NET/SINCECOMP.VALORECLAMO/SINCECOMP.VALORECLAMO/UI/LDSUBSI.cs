...
{
    public partial class LDSUBSI : OpenForm
    {
        ...
        public LDSUBSI(Int64 subscriptionId, dataLDVALREC DataLDVALREC)
        {
            InitializeComponent();
        ...
        void ListasValores(Boolean comienzo)
        {
            ...
            ohl_causal.Select_Statement = string.Join(string.Empty, new string[] { BLConsultas.CausalesReclamoApelacion });
            ...
            if (comienzo)
            {
                ...
                solicitudOrigen = general.cursorProcedure(BLConsultas.SolicitudesReclamoSubsidio, 2, tipos, campos, valores);
                //
        ...
        private void btn_agregar_Click(object sender, EventArgs ef)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        Question form_quest = new Question("LDSUBSI - Advertencia", "Usted escogió una Solicitud Asociada a una Cuenta de Cobro que excede los " + tiempoLegal.ToString() + " meses legales de Reclamación, ¿Desea Continuar?", "Si", "No");
        ...
        //no
        private void dtg_cargos_AfterExitEditMode_1(object sender, EventArgs e)
        {
            //ACTUALIZO EL VALOR TOTAL EN LOS RECLAMOS
            actualizarReclamo();
        }
        ...
        String generadorXML(String Cuenta)
        {
            ...
            String XmlRegister = "<?xml version=\"1.0\" encoding=\"ISO-8859-1\"?>" +
                "<P_SOLICITUD_REPOSICION_EN_SUBSIDIO_DE_APELACION_100338 ID_TIPOPAQUETE=\"100338\">" +
            ...
            for (int x = 0; x <= og_cargos.Rows.Count - 1; x++)
            {
                ...
                if (og_cargos.Rows[x].Cells[a].Value.ToString() == Cuenta)
                {
                    ...
                    if (og_cargos.Rows[x].Cells[0].Value.ToString() == "True" && og_cargos.Rows[x].Cells[p].Value.ToString() == "S")
                    {
                        ...
                        XmlRegister += "<M_MOTIVO_DE_SUBSIDIO_DE_APELACION_100324>" +
            ...
            //FINAL DEL XML
            XmlRegister += "</P_SOLICITUD_REPOSICION_EN_SUBSIDIO_DE_APELACION_100338>";
            return XmlRegister;
        }
        ...
        String generadorXMLMemoriaUnico(String Cuenta)
        {
            ...
            for (int x = 0; x <= og_cargos.Rows.Count - 1; x++)
            {
                //CASO 200-1468
                //VALIDACION SOBRE LA CUENTA
                if (og_cargos.Rows[x].Cells[0].Value.ToString() == "True")//(og_cargos.Rows[x].Cells[a].Value.ToString() == Cuenta)
                {
                    //13.10.17 VALOR EN RECLAMO
                    Double valorReclamo = 0;
                    valorReclamo = Double.Parse(og_cargos.Rows[x].Cells[i].Value.ToString());
        ...
        private void btn_guardar_Click(object sender, EventArgs ef)
        {
            if (validar())
            {
                ...
                else
                {
                    ...
                    if (!entro)
                    {
                        Question pregunta = new Question("LDSUBSI - Guardar", "Esta seguro que desea guardar?", "Si", "No");
                        pregunta.ShowDialog();
                ...
                if (resp != "")
                {
                    general.mensajeERROR("Los Valores en el Recurso de Reposición en Subsidio de Apelación de la Cuenta " + resp + " no puede quedar en Negativo");
                    ejecutar = false;
                }
                //valido
                    ...
                    if (this.lista.Count > 0)
                    {
                        ...
                        if (sbReclamoUnico == "S")
                        {
                            ...
                            if (packageId != 0)
                            {
                                ...
                                codigosC += "Registro del Recurso de Reposición en Subsidio de Apelación Exitoso. Solicitud: " + packageId.ToString() + "\n";
                                //13.08.17 SE SOLICITO CERRAR EL FORMULARIO AL TERMINAR EL PROCESO
                                this.Close();
                            }
                            else
                            {
                                codigosC += "Error Registrando el Recurso de Reposición en Subsidio de Apelación" + "\n";
                            }
                        ...
                        else
                        {
                            foreach (LDVALREC_proyectar cuenta in this.lista)
                            {
                                ...
                                if (packageId != 0)
                                {
                                    ...
                                    //
                                    _blldvalrec.doCommit();
                                    codigosC += "Registro del Recurso de Reposición en Subsidio de Apelación Exitoso para la Cuenta " + cuenta.cuenta.ToString() + ". Solicitud: " + packageId.ToString() + "\n";
                                    //13.08.17 SE SOLICITO CERRAR EL FORMULARIO AL TERMINAR EL PROCESO
                                    this.Close();
                                }
                                else
                                {
                                    codigosC += "Error Registrando el Recurso de Reposición en Subsidio de Apelación sobre la Cuenta " + cuenta.cuenta.ToString() + "\n";
                                }
                            }
                        }
                    }
        ...
        private void btn_norec_Click(object sender, EventArgs ef)
        {
            Boolean aplicar = true;
            //DETERMINO LOS CARGOS APLICADOS AL RECLAMO SELECCIONADO
            int totalFila = og_cargos.Rows.Count;
