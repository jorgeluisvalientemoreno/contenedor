CREATE OR REPLACE Package adm_person.pkordenessinacta Is

  /*****************************************************************
    Propiedad intelectual de LUDYCOM.

    nombre paquete;          pkOrdenesSinActa
...
End pkordenessinacta;
/
CREATE OR REPLACE Package Body adm_person.pkordenessinacta Is

  gsbpaquete Varchar2(4000) := 'pkOrdenesSinActa';
...
--                                 isbaccion            Varchar2,
  ...
  ******************************************************************/
  ...
  Begin
    nupaso := 10;
    ut_trace.trace('Inicio pkOrdenesSinActa.proMueveInventarioUnidOper',
                   10);
    ...
    Begin
      ...
      */
      Update or_ope_uni_item_bala oouitb
         Set oouitb.total_costs = oouitb.total_costs - inuvalor
       Where oouitb.items_id = inuitem
    ...
    Exception
      When Others Then
        ...
        osberror := 'No fue posible realizar el movimiento de cantidades para la unidad operativa ' ||
                    nuunidadoperativa;
        Raise exerror;
    End;
    ...
    ut_trace.trace('FIN pkOrdenesSinActa.proMueveInventarioUnidOper', 10);
  Exception
  ...
  ******************************************************************/
  Procedure procreaordenajuste(inuorderid            In or_order.order_id%Type,
    ...
    tbitems             or_bcadjustmentorder.tytbadjusteditems;
    ...
    Cursor cuitem Is
      ...
      Select ionuitemid, inuserialitems_id, inucantidaddefinitiva
        From dual
  ...
  Begin
    ...
    If (tbitems.count > 0) Then
      ...
      Else
        ...
        Select ooi.items_id, legal_item_amount, ooi.total_price
          Into ionuitemid, nucantidad, nucostoitem
          From or_order_items ooi
 ...
 osberror := NULL;
    ...
    If osberror Is Not Null Then
      ut_trace.trace('Se present? un error al intentar generar la ' ||
                     ...
                     osberror || ');',
                     10);
      osberror := 'Se present? un error al intentar generar la orden de ajuste comod?n por la cantidad corregida. ';
      Raise exerror;
    End If;
    nupaso := 90;
...
/*     jjjm Se dejan los item en la tabla temporal en caso de reversar.
  ...
  ******************************************************************/
  Procedure process(inuorden_id       in out ge_boinstancecontrol.stynuindex,
    ...
    sbEstadoCargos       or_order.CHARGE_STATUS%type; --TICKET 2001786 LJLB -- se almacena estado de generacion de cargo de la orden
    nuTipoTrab           or_order.TASK_TYPE_ID%type; --TICKET 2001786 LJLB -- se almacena tipo de trabajo de la orden
    nuConcepto           OR_TASK_TYPE.CONCEPT%type; --TICKET 2001786 LJLB -- se almacena estado de generacion de cargo de la orden
...
--                         isbaccion            => isbaccion,
                         osberror             => osberror);
    If osberror Is Not Null Then
      osberror := 'procedimiento : pkordenessinacta.provalidainformacion. '||osberror;
      Raise exerror;
    End If;

    --TICKET 2001786 LJLB -- se consulta si la entrega aplica para la Gasera
    IF fsbAplicaEntrega('OSS_CON_JGB_2001786') = 'S' THEN

      sbEstadoCargos :=  DAOR_ORDER.FSBGETCHARGE_STATUS(inuorden_id, null); --TICKET 2001786 LJLB -- se consulta estado de genracion de orden
      nuTipoTrab := DAOR_ORDER.FNUGETTASK_TYPE_ID(inuorden_id, null); --TICKET 2001786 LJLB -- se consulta tipo d etrabajo de la orden
      nuConcepto := DAOR_TASK_TYPE.FNUGETCONCEPT(nuTipoTrab, null); --TICKET 2001786 LJLB -- se consulta el concepto configurado del tipo de trabajo

      --TICKET 2001786 LJLB -- se valida si la orden de trabajo genrro cargo al usuario
      IF sbEstadoCargos = '3'  THEN
          osberror := 'Error, orden de trabajo['||inuorden_id||'] no se le puede hacer movimientos, ya que le genero cargos al usuario';
          Raise exerror;
      END IF;

      --TICKET 2001786 LJLB -- se valida si el tipo de trabajo tiene concepto configurado
      IF sbEstadoCargos <> '1' AND nuConcepto IS NOT NULL  THEN
          osberror := 'Error, orden de trabajo['||inuorden_id||'] no se le puede hacer movimientos, ya que el tipo de trabajo tiene concepto configurado';
          Raise exerror;
      END IF;
    END IF;
    ...
    If isbaccion = 'AGREGAR' Then
      ...
      proadicionaitemalaorden(inuorderid     => inuorden_id,
                              ...
                              osberror       => osberror);
      If osberror Is Not Null Then
        osberror := 'procedimiento : pkordenessinacta.proadicionaitemalaorden. '||osberror;
        Raise exerror;
      End If;
    End If;
    ...
    proinicializaregistroenlog(inuorden_id       => inuorden_id,
                               ...
                               osberror          => osberror);
    If osberror Is Not Null Then
      osberror := 'procedimiento : pkordenessinacta.proinicializaregistroenlog. '||osberror;
      Raise exerror;
    End If;
    ...
    procorregirorden(inuorderid           => inuorden_id,
                     ...
                     osberror             => osberror);
    IF osberror Is Not Null Then
      osberror := 'procedimiento : pkordenessinacta.procorregirorden. '||osberror;
      RAISE exerror;
    END IF;
    ...
    If osberror Is Not Null Then
      osberror := 'procedimiento : pkordenessinacta.proterminaregistroenlog. '||osberror;
      Raise exerror;
    End If;
--    Commit;
    ut_trace.trace('Termina ' || gsbpaquete || '.' || sbmetodo, 10);
  ...
  ******************************************************************/
  ...
  Begin
    ...
    If rfcuvalidaitemcotizado.cantidad = 1 And
      ...
      sberror := 'Al Item Cotizado NO se le permite modificar desde la forma MIOSA';
      Raise exerror;
    Else
      ---CASO 200-699
...
--      Raise ex.controlled_error;
  ...
  ******************************************************************/
  ...
  Begin
    ...
    If rfcuvalidaitemcotizado.cantidad = 1 And
       fblaplicaentrega('OSS_CON_JLV_200699_3') Then
      sberror := 'Al Item Cotizado NO se le permite ingresar desde la forma AIOSA';
      Raise exerror;
    Else
      ---CASO 200-699
      provalidadatosobligatorios(isbaccion         => 'AGREGAR',
                                 ...
                                 osberror          => sberror);
      If sberror Is Not Null Then
        sberror := 'procedimiento : pkordenessinacta.provalidadatosobligatorios. '||sberror;
        Raise exerror;
      End If;
      nupaso := 90;
      ut_trace.trace('Procesando la informacion '||nuorden_id, 10);
      process(inuorden_id       => nuorden_id,
              ...
              osberror          => sberror);
      If sberror Is Not Null Then
        sberror := 'procedimiento : pkordenessinacta.process. '||sberror;
        Raise exerror;
      End If;
      --CASO 200-699
    End If; --final de validacion de item cotizado
...
End pkordenessinacta;
/
PROMPT OTORGA PERMISOS ESQUEMA sobre PKORDENESSINACTA
BEGIN
    pkg_utilidades.prAplicarPermisos('PKORDENESSINACTA', 'ADM_PERSON'); 
END;
/