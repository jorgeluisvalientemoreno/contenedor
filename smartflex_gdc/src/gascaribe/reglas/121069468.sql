...
BEGIN
  ...
  dbms_output.put_line('Inicia Proceso ajustes regla');  
	...
	set expression = '
nuCategoria = CATEGORY();nuSubcategoria = SUBCATEGORY();nuValSubsidio = GETGENERATEDCHARGE(196) * (-1);nuValConsumo = GETGENERATEDCHARGE(31);if (nuCategoria = 1 '||chr(38)||chr(38)||' nuSubcategoria <= 2,nuValor = nuValConsumo - nuValSubsidio;if (nuValor > 0,nuValDescuento = (nuValor * 0.70) * (-1);,);,if (nuCategoria = 1 '||chr(38)||chr(38)||' nuSubcategoria > 2,nuValor = nuValConsumo;if (nuValor > 0,nuValDescuento = (nuValor * 0.70) * (-1);,);,);)', author = 'GERPOL',
generation_date = to_date ('30/09/14','DD/MM/YYYY HH24:MI:SS'),
last_modifi_date = to_date ('30/09/14','DD/MM/YYYY HH24:MI:SS'),
object_type = 'PF',
...
V5 NUMBER;
V6 NUMBER;
nuValSubsidio NUMBER;
V7 NUMBER;
V8 NUMBER;
nuValConsumo NUMBER;
V9 NUMBER;
V10 NUMBER;
nuValCargoFijo NUMBER;
V11 NUMBER;
V12 NUMBER;
V13 NUMBER;
nuValor NUMBER;
...
nuSubcategoria:=V1;
V3 := 196;
V2:=GETGENERATEDCHARGE(V3);
V4 := 1;
V5:= -V4;
V6:=V2 * V5;
nuValSubsidio:=V6;
V8 := 31;
V7:=GETGENERATEDCHARGE(V8);
nuValConsumo:=V7;
V10 := 17;
V9:=GETGENERATEDCHARGE(V10);
nuValCargoFijo:=V9;
V11 := 1;
V12 := 2;
IF ( nuCategoria = V11 AND nuSubcategoria <= V12 )
THEN
V13:=nuValConsumo - nuValSubsidio;
nuValor:=V13;
V14 := 0;
IF ( nuValor > V14 )
THEN
...
V22 := 1;
V23 := 2;
IF ( nuCategoria = V22 AND nuSubcategoria > V23 )
THEN
V24:=nuValConsumo + nuValCargoFijo;
nuValor:=V24;
V25 := 0;
IF ( nuValor > V25 )
THEN
...
END;
'
 where config_expression_id = 121069468;
