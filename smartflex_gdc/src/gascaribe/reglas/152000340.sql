...
BEGIN
  ...
  open cuNum(IdConfExpre);
	...
	set expression = '
inuPromo = 73;MesActual = OBTMESESTRANSPROMO(inuPromo);TotalMeses = OBTTOTALMESESPROMO(inuPromo);if (MesActual > 0 && MesActual <= TotalMeses,VlrContribucion = LDC_FNCRETORCARGCONTRIBFGCC(37);Dscto = VlrContribucion * -1;,'||
')', author = 'OPEN',
generation_date = to_date ('14-07-2022 08:58:32','DD/MM/YYYY HH24:MI:SS'),
last_modifi_date = to_date ('14-07-2022 08:58:32','DD/MM/YYYY HH24:MI:SS'),
object_type = 'PF',
...
V0 := 73;
inuPromo:=V0;
V1:=OBTMESESTRANSPROMO(inuPromo);
MesActual:=V1;
V2:=OBTTOTALMESESPROMO(inuPromo);
TotalMeses:=V2;
V3 := 0;
IF ( MesActual > V3 AND MesActual <= TotalMeses )
THEN
V5 := 37;
V4:=LDC_FNCRETORCARGCONTRIBFGCC(V5);
VlrContribucion:=V4;
V6 := 1;
V7:= -V6;
V8:=VlrContribucion * V7;
Dscto:=V8;
V9:=V8;
ELSE
null;
...
END;
'
 where config_expression_id = 152000340;
