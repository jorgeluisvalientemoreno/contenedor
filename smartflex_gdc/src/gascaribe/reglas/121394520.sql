...
BEGIN
  ...
  open cuNum(IdConfExpre);
	...
	BEGIN
        dbms_output.put_line('Inserta regla GR_CONFIG_EXPRESSION');
        INSERT INTO GR_CONFIG_EXPRESSION(CONFIG_EXPRESSION_ID, CONFIGURA_TYPE_ID, EXPRESSION, AUTHOR, CREATION_DATE, GENERATION_DATE, LAST_MODIFI_DATE, STATUS, USED_OTHER_EXPRESION, MODIFICATION_TYPE, PASSWORD, EXECUTION_TYPE, DESCRIPTION, OBJECT_NAME, OBJECT_TYPE, CODE)
        VALUES (IdConfExpre, 404, 'nuMotivo = OR_BOINSTANCE.FNUGETFATHEXTSYSIDFROMINSTANCE();nuProducto = MO_BOMOTIVE.FNUGETPRODUCTID(nuMotivo);DAPR_PRODUCT.SETUSECACHE(false);nuUltActSupProdId = PR_BOSUSPENSION.FNUGETLASTPRODSUSPORDERACTI(nuProducto);nuTipoActividadId = OR_BOACTIVITIESRULES.FNUGETACTIFROMORDERACT(nuUltActSupProdId);if (nuTipoActividadId = 100009978,OR_BOINSTANCE.SETSUCCESSININSTANCE();,OR_BOINSTANCE.SETUN_SUCCESSININSTANCE();)',
            'OPEN',SYSDATE,SYSDATE,SYSDATE, 'G','N','PU',NULL,'DS','ACTIVIDAD DE SUSPENSION 100009978','OR_SEL_TASTYP_CT404E121394520','PP',NULL);
    EXCEPTION
        WHEN OTHERS THEN
            dbms_output.put_line('Actualiza regla GR_CONFIG_EXPRESSION');
            UPDATE GR_CONFIG_EXPRESSION
            SET EXPRESSION = 'nuMotivo = OR_BOINSTANCE.FNUGETFATHEXTSYSIDFROMINSTANCE();nuProducto = MO_BOMOTIVE.FNUGETPRODUCTID(nuMotivo);DAPR_PRODUCT.SETUSECACHE(false);nuUltActSupProdId = PR_BOSUSPENSION.FNUGETLASTPRODSUSPORDERACTI(nuProducto);nuTipoActividadId = OR_BOACTIVITIESRULES.FNUGETACTIFROMORDERACT(nuUltActSupProdId);if (nuTipoActividadId = 100009978,OR_BOINSTANCE.SETSUCCESSININSTANCE();,OR_BOINSTANCE.SETUN_SUCCESSININSTANCE();)'
