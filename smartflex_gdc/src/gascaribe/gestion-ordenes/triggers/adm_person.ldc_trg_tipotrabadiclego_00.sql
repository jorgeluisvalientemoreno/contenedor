CREATE OR REPLACE TRIGGER ADM_PERSON.LDC_TRG_TIPOTRABADICLEGO_00
  BEFORE INSERT OR UPDATE ON LDC_TIPOTRABADICLEGO
...
DECLARE
  ...
  cursor CUOR_TASK_TYPE_CAUSAL is
    SELECT COUNT(OR_TASK_TYPE_CAUSAL.CAUSAL_ID) CANTIDAD
     ...
     WHERE OR_TASK_TYPE_CAUSAL.TASK_TYPE_ID = :NEW.TIPOTRABADICLEGO_ID
       AND OR_TASK_TYPE_CAUSAL.CAUSAL_ID = :NEW.CAUSALADICLEGO_ID;
...
BEGIN
  ...
  if RFCUOR_TASK_TYPE_CAUSAL.CANTIDAD <= 0 then
    errors.seterror(2741,
                    'La causal [' || :NEW.CAUSALADICLEGO_ID ||
                    '] no se relaciona con el Tipo de Trabajo [' ||
                    :NEW.TIPOTRABADICLEGO_ID || '].');
