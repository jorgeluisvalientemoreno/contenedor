CREATE OR REPLACE TRIGGER ADM_PERSON.LDC_TRG_BIUD_ROLUNIDAD
BEFORE INSERT OR UPDATE OR DELETE ON OR_ROL_UNIDAD_TRAB
/*******************************************************************************
    Propiedad intelectual de CSC

    Trigger     : LDC_TRG_BIUD_ROLUNIDAD
...
BEGIN
    ut_trace.Trace('Inicio: LDC_TRG_BIUD_ROLUNIDAD', 10);
    ...
    IF fblaplicaentregaxcaso(sbEntrega) AND nuFlagValida = 1 THEN
      ...
      IF nuResponsable is null THEN
         CLOSE cuValidaOr;
         ut_trace.Trace('El responsable de la unidad debe ser ingresado obligatoriamente en la unidad: '||:new.id_unidad_operativa, 10);
           RAISE erValidacion;
      END IF;
      CLOSE cuValidaOr;
    END IF;

  ut_trace.Trace('Fin: LDC_TRG_BIUD_ROLUNIDAD', 10);
