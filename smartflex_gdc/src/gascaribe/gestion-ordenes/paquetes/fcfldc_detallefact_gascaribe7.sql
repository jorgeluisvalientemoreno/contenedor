create or replace PACKAGE fcfldc_detallefact_gascaribe7 IS
  /*****************************************************************
...
END fcfldc_detallefact_gascaribe7;
/
create or replace PACKAGE BODY fcfLDC_DetalleFact_GasCaribe7 IS
  ...
  **************************************************************************/
  ...
  BEGIN
    ...
    sbEncabezado := sbEncabezado ||
                    ...
                    'TOTAL|IVA|SUBTOTAL|CARGOSMES|CANTIDAD_CONC|';
  ...
  **************************************************************************/
  ...
  BEGIN
    ...
    sbEncabezado := 'DESC_CONCEP61|SALDO_ANT61|CAPITAL61|INTERES61|TOTAL61|SALDO_DIF61|CUOTAS61|DESC_CONCEP62|SALDO_ANT62|CAPITAL62|INTERES62|TOTAL62|SALDO_DIF62|CUOTAS62|DESC_CONCEP63|SALDO_ANT63|CAPITAL63|INTERES63|TOTAL63|SALDO_DIF63|CUOTAS63|DESC_CONCEP64|SALDO_ANT64|CAPITAL64|INTERES64|TOTAL64|SALDO_DIF64|CUOTAS64|' ||
                    ...
                    'DESC_CONCEP77|SALDO_ANT77|CAPITAL77|INTERES77|TOTAL77|SALDO_DIF77|CUOTAS77|DESC_CONCEP78|SALDO_ANT78|CAPITAL78|INTERES78|TOTAL78|SALDO_DIF78|CUOTAS78|';
    RETURN sbEncabezado;
  ...
  ******************************************************************/
  ...
  Fecha             Autor             Modificacion
  =========       =========           ====================
  16/04/2019      ELAL                CA 200-2032 se adiciona logica para colocar reglas de negocio a la impresion de spooll

  03/09/2018      Jorge Valiente      CASO 200-1899: Modificar el contenido de la descripcion de categoria por la
                                                     etiqueta de UNICO si la categoria no esta en el paremtro COD_CAT_SPO_GDC
                                                     Se realizara el llamado del servicio FSBCategoriaUNICO
    ...
    IF NOT blNRegulado THEN
      ...
      OPEN orfcursor FOR
        SELECT fc.factcodi FACTURA,
               ...
               CASE
                 --TICKET 200-2032 ELAL --se cambia logica para obtencion de cuentas de cobro
                 ...
                 WHEN (LDC_BOFORMATOFACTURA.fnuObtNCuentaSaldo(s.sesunuse) > 1) AND
                      (s.sesucate != 1) THEN
                  'INMEDIATO'*/
                 WHEN fnuMesesDeuda(fc.factsusc) >= 2 THEN
                      'INMEDIATO'
                 ELSE
               ...
               ' - ' || substr(dage_geogra_location.fsbgetdescription(dage_geogra_location.fnugetgeo_loca_father_id(ca.geograp_location_id)),
                               ...
                               3) LOCALIDAD,
               --Inicio CASO 200-1633 Anexo al CASO 200-1899
               replace(open.pktblcategori.fsbgetdescription(s.sesucate),'COMERCIAL',sbUsoNoresidencial) CATEGORIA, --TICKET 200-2032 ELAL -- se cambia palabra comercial por el valor no residencial
               --FSBCategoriaUNICO(s.sesucate, s.sesusuca) CATEGORIA,
               --pktblsubcateg.fsbgetdescription(s.sesucate, s.sesusuca) ESTRATO,
               TRIM(REPLACE(FSBCategoriaUNICO(s.sesucate, s.sesusuca),'ESTRATO','')) ESTRATO, --TICKET 200-2032 ELAL -- se quita palabra ESTRATO
               --Fin CASO 200-1633 Anexo al CASO 200-1899
               b.susccicl CICLO,
               ...
               NULL SALDO_ANT,
               CASE
                 WHEN fnuMesesDeuda(fc.factsusc) >= 2 THEN --TICKET 200-2032 ELAL --  se adiciona validacion de meses de deuda
                      'INMEDIATO'
               ...
               CASE WHEN fnuMesesDeuda(fc.factsusc) >= 2 THEN --TICKET 200-2032 ELAL --  se adiciona validacion de meses de deuda
                      'INMEDIATO'
    ...
    ELSE
      OPEN orfcursor FOR
        SELECT fc.factcodi FACTURA,
               ...
               --Inicio CASO 200-1633 Anexo al CASO 200-1899
               NULL CATEGORIA,
               --FSBCategoriaUNICO(s.sesucate) CATEGORIA,
               FSBCategoriaUNICO(s.sesucate, s.sesusuca) ESTRATO,
               --NULL ESTRATO,
               --Fin CASO 200-1633 Anexo al CASO 200-1899
               NULL CICLO,
               NULL RUTA,
  ...
  ******************************************************************/
  ...
  sbfactpefa number;
    ...
    LECTURA_ANTERIOR VARCHAR2(100);--NUMBER;
    LECTURA_ACTUAL   VARCHAR2(100);--NUMBER;
    NUM_MEDIDOR      VARCHAR2(50);
    OBS_LECTURA      VARCHAR2(100);

    --CASO 200-1633 Anexo al CASO 200-1899
  ...
  FUNCTION fsbgetConsumo ( inuperifact in perifact.pefacodi%type,
                           inuproducto in pr_product.product_id%TYPE,
               inuCiclo  IN NUMBER) RETURN VARCHAR2 Is
     /*****************************************************************
      Propiedad intelectual de GDC.
  ...
  BEGIN
      ...
      nuperidocons := ldc_boformatofactura.fnuobtperconsumo(inuCiclo,
                                                              inuperifact);
    BEGIN
      ...
      IF calculo_cons IS NOT NULL THEN
        SELECT COUNT(*)
          ...
          FROM TABLE(ldc_boutilities.splitstrings(dald_parameter.fsbGetValue_Chain('CICLO_TELEMEDIDOS_GDC'),
                                                  ','))
         WHERE column_value = pktblservsusc.fnugetsesucicl(inuproducto);
  ...
  sbfactpefa  := wpefa;
    ...
    BEGIN
      ...
      sbconsumo := fsbgetConsumo(sbfactpefa, nuSesunuse, nucicloc ); --TICKET 200-2032 ELAL -- se obtiene consumo
        --01-03-2015 Llozada: Se modifica para que no envie mal las lecturas
        SELECT num_medidor,
               TO_CHAR(ROUND(SUM(leac), 2)) lectura_actual,
               TO_CHAR(ROUND(SUM(NVL(lean, 0)), 2)) lectura_anterior,
               causal_no_lec
  ...
  ******************************************************************/
  PROCEDURE rfdatosconsumohist(orfcursor OUT constants.tyrefcursor) AS
    ...
    --Incio CASO 200-1633 Anexo al CASO 200-1899
    --CAUSA_DESVIACION      VARCHAR2(200):= ' ';
    --Fin CASO 200-1633 Anexo al CASO 200-1899
    ...
    supercompres        NUMBER;
    temperatura         VARCHAR2(100);--NUMBER;
  ...
  BEGIN
    ...
    IF NOT blnregulado THEN
      ...
      BEGIN
        ...
        --' ' CAUSA_DESVIACION -- CASO 200-1899
          INTO consumo_actual,
               sbFactor_correccion,
               cons_correg,
               supercompres,
        ...
        sbconsumo_promedio := to_char(consumo_promedio, 'FM999,999,990.9990');
    ...
    ELSE
      ...
      /*--Inicio CASO 200-1633 Anexo al CASO 200-1899
      Begin
        SELECT fsbgetCalifVariConsumos(MAX(sesunuse), MAX(cosspecs))
          into CAUSA_DESVIACION
          FROM factura f
         INNER JOIN servsusc s
            ON (sesususc = factsusc AND
               sesuserv =
    ...
    OPEN orfcursor FOR
      ...
      --CAUSA_DESVIACION    CAUSA_DESVIACION --CASO 200-1899
        FROM dual;
  ...
  ******************************************************************/
      ...
      SELECT sesunuse
       ...
       WHERE f.factsusc = p.sesususc
         ...
         AND p.sesuserv =
             dald_parameter.fnuGetNumeric_Value('COD_SERV_GAS');
   --INICIO CA 200-2032
   sbTipoNotiFesu VARCHAR2(30) := dald_parameter.fsbGetValue_Chain('LDC_TIPONOTIFS',null); --se almacena tipo de notificacion para fecha de suspension
   sbTipoNoti VARCHAR2(30) := dald_parameter.fsbGetValue_Chain('LDC_TIPONOTI',null); --se almacena tipo de notificacion para fecha maxima

   --se valida tipo de notifica
   CURSOR cuValidaNoti(sbTipodeNoti  VARCHAR2) Is
   SELECT 'X'
   FROM ( select to_number(regexp_substr(sbTipodeNoti,'[^,]+', 1, level))  tipo_not
       from   dual
       connect by regexp_substr(sbTipodeNoti, '[^,]+', 1, level) is not null)
    WHERE tipo_not = TIPO_NOTI;

  sbValiFesu VARCHAR2(1);
  sbValiFema  VARCHAR2(1);
  ...
  --TICKET 200-2032 ELAL -- se valida tipo de noti para fech_susp
  OPEN cuValidaNoti(sbTipoNotiFesu);
  FETCH cuValidaNoti INTO sbValiFesu;
  IF cuValidaNoti%NOTFOUND THEN
     sbValiFesu := NULL;
  END IF;
  CLOSE cuValidaNoti;

  --TICKET 200-2032 ELAL -- se valida tipo de noti para FECH_MAXIMA
  OPEN cuValidaNoti(sbTipoNoti);
  FETCH cuValidaNoti INTO sbValiFema;
  IF cuValidaNoti%NOTFOUND THEN
     sbValiFema := NULL;
  END IF;
  CLOSE cuValidaNoti;
    ...
    OPEN orfcursor FOR
      SELECT TIPO_NOTI   TIPO_NOTI,
             MENS_NOTI   MENS_NOTI,
             CASE WHEN sbValiFema IS NULL THEN '' ELSE FECH_MAXIMA END  FECH_MAXIMA, --TICKET 200-2032 ELAL -- se coloca regla de negocio
             CASE WHEN sbValiFesu IS NOT NULL THEN 'INMEDIATO' ELSE FECH_SUSP END  FECH_SUSP --TICKET 200-2032 ELAL -- se coloca regla de negocio
        FROM dual;
  ...
  ******************************************************************/
  ...
  03/09/2018      Jorge Valiente      CASO 20-1899: Realizar el llamado del parametro COD_CON_IVA_GDC para excluir en el detalle
                                                    de la factura y mostrar en el total del IVA
  ...
  ******************************************************************/
  ...
  PROCEDURE RfConcepParcial(orfcursor OUT constants.tyRefCursor) IS
    ...
    SBCOD_CON_IVA_GDC ld_parameter.value_chain%type := open.dald_parameter.fsbgetvalue_chain('COD_CON_IVA_GDC',
                                                                                             NULL);
    --Fin CASO 200-1899
   ...
   nuIndeBri NUMBER := 1;
   TYPE tytProducDiferidos IS RECORD ( nuProducto NUMBER, nuSaldoDife NUMBER, nuCuota NUMBER);
  ...
  BEGIN
    ...
    nuSaldoDife := 0;
        ...
        IF nuCodiFactProt =   tbCargosOrdered(i).CONCEPTO_ID THEN
   ...
   -----------------
   FOR i IN tbCargosOrdered.first .. tbCargosOrdered.last LOOP
     FOR j IN 1..tblProdDife.COUNT LOOP
     dbms_output.put_line('INDICE I: ' || I);
     --dbms_output.put_line('ETIQUETA' || tbCargosOrdered(i).ETIQUETA);
     dbms_output.put_line('  CONCEPTO_ID: ' || tbCargosOrdered(i).CONCEPTO_ID);
    /* dbms_output.put_line('CONCEPTOS' || tbCargosOrdered(i).CONCEPTOS);
     dbms_output.put_line('SIGNO' || tbCargosOrdered(i).SIGNO);
     dbms_output.put_line('ORDEN' || tbCargosOrdered(i).ORDEN);
     dbms_output.put_line('SALDO_ANT' || tbCargosOrdered(i).SALDO_ANT);
     dbms_output.put_line('CAPITAL' || tbCargosOrdered(i).CAPITAL);
     dbms_output.put_line('INTERES' || tbCargosOrdered(i).INTERES);
     dbms_output.put_line('TOTAL' || tbCargosOrdered(i).TOTAL);
     dbms_output.put_line('SALDO_DIF' || tbCargosOrdered(i).SALDO_DIF);
     dbms_output.put_line('CUOTAS' || tbCargosOrdered(i).CUOTAS);
     dbms_output.put_line('CAR_DOSO' || tbCargosOrdered(i).CAR_DOSO);
     dbms_output.put_line('CAR_CACA' || tbCargosOrdered(i).CAR_CACA);*/
     dbms_output.put_line('  servicio: ' || tbCargosOrdered(i).servicio);
     
     dbms_output.put_line('INDICE J: ' || J);
     dbms_output.put_line('  nuProducto' || tblProdDife(j).nuProducto);
     dbms_output.put_line('  nuSaldoDife' || tblProdDife(j).nuSaldoDife);
  END LOOP;
 END LOOP;
 /*  dbms_output.put_line(' ');
   ...
   dbms_output.put_line(' ');*/
  /*FOR j IN 1..tblProdDife.COUNT LOOP
    dbms_output.put_line('INDICE ' || J);
     dbms_output.put_line('nuProducto' || tblProdDife(j).nuProducto);
     dbms_output.put_line('nuSaldoDife' || tblProdDife(j).nuSaldoDife);
     dbms_output.put_line('nuCuota' || tblProdDife(j).nuCuota);
  END LOOP;*/
   dbms_output.put_line(' ');
   dbms_output.put_line('---------------------------------------------------------------------');
   dbms_output.put_line(' ');
   ...
   -----------------
    ...
    FOR i IN tbCargosOrdered.first .. tbCargosOrdered.last LOOP
      ...
      FOR j IN 1..tblProdDife.COUNT LOOP
         ...
         dbms_output.put_line('tblProdDife(j).nuSaldoDife: ' || tblProdDife(j).nuSaldoDife);
         dbms_output.put_line('tblProdDife(j).nuCuota: ' || tblProdDife(j).nuCuota);
         dbms_output.put_line('tbCargosOrdered(i).CUOTAS ' || tbCargosOrdered(i).CUOTAS);
         dbms_output.put_line('tbCargosOrdered(i).servicio: ' || tbCargosOrdered(i).servicio);
         dbms_output.put_line('tblProdDife(j).nuProducto: ' || tblProdDife(j).nuProducto);
         dbms_output.put_line('tbCargosOrdered(i).CONCEPTOSS ' || tbCargosOrdered(i).CONCEPTOS);
         dbms_output.put_line('sbDescriConBri: ' || sbDescriConBri);
          dbms_output.put_line('------------------------------------------------------------------------------------------');
  ...
  **************************************************************************/
  ...
  BEGIN
    ...
    ELSE
      ...
      --Inicio CASO 200-1633 Anexo al CASO 200-1899
      --Comentariado para obtener tarifas configuradas en OSF
      /*
      OPEN orfcursor FOR
  ...
  ******************************************************************/
  ...
  --Inicio CASO 200-1874 Anexo al CASO 200-1899
  /*****************************************************************
  Propiedad intelectual de PETI (c).
  ...
  Fecha             Autor             Modificacion
  =========       =========           ====================
  15/03/2019      Jorge Valiente      CASO 200-2479: Modifificacion de cursor cuprotecciondatos para establecer
                                                     la busqueda del paraemtro COD_TIP_SOL_PRO_DAT retirando el uso del
                                                     paquete de 1er nivel   .
                                                     Creacion de cursor para obtener el valor del paraemtro  COD_TIP_SOL_PRO_DAT
  ******************************************************************/
  PROCEDURE RfProteccion_Datos(orfcursor OUT constants.tyRefCursor) AS
    sbFactcodi ge_boInstanceControl.stysbValue;
    sbFactsusc ge_boInstanceControl.stysbValue;

    cursor cuprotecciondatos(IsbFactsusc         VARCHAR2,
                             InuCOD_TIPSOLPRODAT number) is
    /*select nvl(lpd1.estado, 'N') PROTECCION_ESTADO
            from LDC_PROTECCION_DATOS lpd1, suscripc s
           where lpd1.id_cliente = s.suscclie
             and s.susccodi = IsbFactsusc
             and lpd1.fecha_creacion =
                 (select max(lpd.fecha_creacion)
                    from LDC_PROTECCION_DATOS lpd, suscripc s
                   where lpd.id_cliente = s.suscclie
                     and s.susccodi = IsbFactsusc); */
    --/*
      select nvl(lpd.estado, 'N') PROTECCION_ESTADO,
             max(lpd.fecha_creacion) MAXIMA_FECHA
        from LDC_PROTECCION_DATOS lpd, open.mo_packages mp, suscripc s
    ...
    -- */
    /*select nvl(lpd.estado, 'N') PROTECCION_ESTADO
     from LDC_PROTECCION_DATOS lpd, open.mo_packages mp
    where mp.package_id = lpd.package_id
      and mp.package_type_id in
          (select to_number(column_value)
             from table(ldc_boutilities.splitstrings(dald_parameter.fsbgetvalue_chain('COD_TIP_SOL_PRO_DAT',
                                                                                      NULL),
                                                     ',')))
      and mp.subscriber_id = IsbFactsusc;*/
  ...
  BEGIN
    ...
    fetch cunuLDPATAMETER
      into rfcunuLDPATAMETER;
    ...
    fetch cuprotecciondatos
      into rfcuprotecciondatos;
    ...
    OPEN orfcursor FOR
      select 0 VISIBLE, NULL IMPRESO, sbPROTECCION_ESTADO PROTECCION_ESTADO
        from dual;
  ...
  --Fin CASO 200-1874 Anexo al CASO 200-1899
  ...
  =========       =========           ====================
  ******************************************************************/
  PROCEDURE RfDatosAdicionales(orfcursor OUT constants.tyRefCursor) IS
    sbFactcodi  ge_boInstanceControl.stysbValue;
    sbFactsusc  ge_boInstanceControl.stysbValue;
    blNRegulado BOOLEAN;
    ...
    --Calificacion Causal Desviacion de consumo
    sbCAUSA_DESVIACION VARCHAR2(200) := ' ';

    cursor cucausadesviacion(Isbfactcodi varchar2) is
      SELECT fsbgetCalifVariConsumos(MAX(sesunuse), MAX(cosspecs)) CAUSA_DESVIACION
        FROM factura f
       INNER JOIN servsusc s
          ON (sesususc = factsusc AND
  ...
  BEGIN
    ...
    ut_trace.trace('Inicio -- LDC_DETALLEFACT_GASCARIBE.RfDatosAdicionales',
                   15);

    open cupagareunico(sbFactsusc);
    fetch cupagareunico
      into rfcupagareunico;
    ...
    ELSE
      ...
      fetch cucausadesviacion
        into rfcucausadesviacion;
      close cucausadesviacion;

      open cudireccionproducto(nuProductoPrincipal);
      fetch cudireccionproducto
        into rfcudireccionproducto;
    ...
    ut_trace.trace('Fin -- LDC_DETALLEFACT_GASCARIBE.RfDatosAdicionales',
                   15);
  ...
  ******************************************************************/
  ...
  BEGIN
    ...
    ut_trace.trace('Inicio -- LDC_DETALLEFACT_GASCARIBE.FSBCategoriaUNICO',
                   15);

    SBUnico :=UPPER( pktblsubcateg.fsbgetdescription(InuCATEGORIA,
                                               InuSubCATEGORIA));
    ...
    ut_trace.trace('Fin -- LDC_DETALLEFACT_GASCARIBE.FSBCategoriaUNICO',
                   15);
  ...
  ******************************************************************/
  ...
  BEGIN
    BEGIN
      ...
      if CUEXISTE%found then
        if rfCUEXISTE.Cantidad = 1 then
          sbCAUSA_DESVIACION := 'Desviaci?n Significativa';
        end if;
      end if;
      close CUEXISTE;
