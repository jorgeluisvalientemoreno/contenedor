...
BEGIN
sa_bocreatePackages.CreatePackage('ACOTESDO_',
'CREATE OR REPLACE PACKAGE ACOTESDO_ IS ' || chr(10) ||
...
'tb2_0 ty2_0;  executableName ge_catalog.tag_name%type := ''ACOTESDO''; ' || chr(10) ||
...
'( ' || chr(10) ||
'SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME=''ACOTESDO'' ' || chr(10) ||
'); ' || chr(10) ||
...
BEGIN ut_trace.trace('********************Comenzar proceso de objeto:ACOTESDO_******************************'); END;
/
...
if (not ACOTESDO_.blProcessStatus) then
 return;
end if;
Open ACOTESDO_.cuRoleExecutables;
loop
 fetch ACOTESDO_.cuRoleExecutables INTO ACOTESDO_.rcRoleExecutables;
 exit when  ACOTESDO_.cuRoleExecutables%notfound;
 ACOTESDO_.tbRoleExecutables(nuIndex) := ACOTESDO_.rcRoleExecutables;
 nuIndex := nuIndex + 1;
END loop;
close ACOTESDO_.cuRoleExecutables;
nuIndex := 0;
Open ACOTESDO_.cuUserExceptions ;
loop
 fetch ACOTESDO_.cuUserExceptions INTO  ACOTESDO_.rcUserExceptions;
 exit when ACOTESDO_.cuUserExceptions%notfound;
 ACOTESDO_.tbUserException(nuIndex):=ACOTESDO_.rcUserExceptions;
 nuIndex := nuIndex + 1;
END loop;
close ACOTESDO_.cuUserExceptions;
nuIndex := 0;
Open ACOTESDO_.cuExecEntities ;
loop
 fetch ACOTESDO_.cuExecEntities INTO  ACOTESDO_.rcExecEntities;
 exit when ACOTESDO_.cuExecEntities%notfound;
 ACOTESDO_.tbExecEntities(nuIndex):=ACOTESDO_.rcExecEntities;
 nuIndex := nuIndex + 1;
END loop;
close ACOTESDO_.cuExecEntities;

exception when others then
ACOTESDO_.blProcessStatus := false;
rollback;
...
(
 SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='ACOTESDO'
);
    ...
    BEGIN
      ...
      IF (nuNumRecords > 0) THEN
        ...
        IF ( sbObjectName IS NOT null ) THEN
          ...
          if ( ACOTESDO_.tbObjectToDelete.last IS null ) then
            nuNumObjects := 1;
          else
            nuNumObjects := ACOTESDO_.tbObjectToDelete.last + 1;
          end if;
          ACOTESDO_.tbObjectToDelete(nuNumObjects) := sbObjectName;
        END IF;
...
WHERE   sa_executable.name = 'ACOTESDO'
...
if (not ACOTESDO_.blProcessStatus) then
 ...
 FOR crObjectsName IN cuObjectsName LOOP
  ACOTESDO_.tbObjectToDelete(nuIndex) := crObjectsName.OBJECT_NAME;
  nuIndex := nuIndex + 1;
 END LOOP;
EXCEPTION 
...
if (not ACOTESDO_.blProcessStatus) then
 return;
  ...
  DELETE FROM SA_ROLE_EXECUTABLES WHERE (EXECUTABLE_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='ACOTESDO') AND ROLE_ID=1;
...
if (not ACOTESDO_.blProcessStatus) then
 return;
  ...
  DELETE FROM SA_USER_EXCEPTIONS WHERE (EXECUTABLE_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='ACOTESDO') AND 1=2;
...
SELECT rowid
FROM SA_EXECUTABLE_SYNON WHERE (SYNONYMOUS_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='ACOTESDO');
nuIndex binary_integer;
BEGIN
...
if (not ACOTESDO_.blProcessStatus) then
 return;
  ...
  DELETE FROM SA_EXEC_ENTITIES WHERE (EXECUTABLE_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='ACOTESDO');
...
if (not ACOTESDO_.blProcessStatus) then
 return;
  ...
  DELETE FROM SA_MENU_OPTION WHERE (EXECUTABLE_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='ACOTESDO');
...
if (not ACOTESDO_.blProcessStatus) then
 return;
  ...
  DELETE FROM GI_CONFIG_COMP WHERE (CONFIG_ID) in (SELECT CONFIG_ID FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='ACOTESDO') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339);
...
if (not ACOTESDO_.blProcessStatus) then
 return;
  ...
  ut_trace.trace('Borrando tabla GI_CONFIG_COMP',1);
  DELETE FROM GI_CONFIG_COMP WHERE (COMPOSITION_ID) in (SELECT COMPOSITION_ID FROM GI_COMPOSITION WHERE (CONFIG_ID) in (SELECT CONFIG_ID FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='ACOTESDO') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339));
...
DECLARE 
 ...
 SELECT rowid FROM GE_STATEMENT WHERE (STATEMENT_ID) in (SELECT SELECT_STATEMENT_ID FROM GI_COMP_ATTRIBS WHERE (COMPOSITION_ID) in (SELECT COMPOSITION_ID FROM GI_COMPOSITION WHERE (CONFIG_ID) in (SELECT CONFIG_ID FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='ACOTESDO') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339)));
BEGIN 
...
ACOTESDO_.tbGE_STATEMENTRowId(rcData.rowid):=rcData.rowid; 
END loop; 
...
for rcData in cuLoadTemporaryTable loop
ACOTESDO_.tbGR_CONFIG_EXPRESSIONRowId(rcData.rowid):=rcData.rowid; 
END loop; 
...
if (not ACOTESDO_.blProcessStatus) then
 return;
  ...
  DELETE FROM GI_COMP_ATTRIBS WHERE (COMPOSITION_ID) in (SELECT COMPOSITION_ID FROM GI_COMPOSITION WHERE (CONFIG_ID) in (SELECT CONFIG_ID FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='ACOTESDO') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339));
...
nuVarcharIndex:=ACOTESDO_.tbGE_STATEMENTRowId.first;  
while (nuVarcharIndex is not null) loop 
begin
ut_trace.trace('Row id a borrar:'||nuVarcharIndex,1);
delete from GE_STATEMENT where rowid = ACOTESDO_.tbGE_STATEMENTRowId(nuVarcharIndex); 
...
ACOTESDO_.tbGE_STATEMENTRowId.delete(nuOldVarcharIndex);
end if;
nuVarcharIndex := ACOTESDO_.tbGE_STATEMENTRowId.next(nuVarcharIndex); 
END loop; 
...
nuVarcharIndex:=ACOTESDO_.tbGR_CONFIG_EXPRESSIONRowId.first;  
while (nuVarcharIndex is not null) loop 
ut_trace.trace('Row id a borrar:'||nuVarcharIndex,1);
delete from GR_CONFIG_EXPRESSION where rowid = ACOTESDO_.tbGR_CONFIG_EXPRESSIONRowId(nuVarcharIndex); 
nuOldVarcharIndex:=nuVarcharIndex;
nuVarcharIndex := ACOTESDO_.tbGR_CONFIG_EXPRESSIONRowId.next(nuVarcharIndex); 
ACOTESDO_.tbGR_CONFIG_EXPRESSIONRowId.delete(nuOldVarcharIndex); 
END loop; 
...
for rcData in cuLoadTemporaryTable loop
ACOTESDO_.tbGI_COMPOSITIONRowId(rcData.rowid):=rcData.rowid; 
END loop; 
...
if (not ACOTESDO_.blProcessStatus) then
 return;
  ...
  DELETE FROM GI_FRAME WHERE (COMPOSITION_ID) in (SELECT COMPOSITION_ID FROM GI_COMPOSITION WHERE (CONFIG_ID) in (SELECT CONFIG_ID FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='ACOTESDO') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339));
...
nuVarcharIndex:=ACOTESDO_.tbGI_COMPOSITIONRowId.first;  
while (nuVarcharIndex is not null) loop 
ut_trace.trace('Row id a borrar:'||nuVarcharIndex,1);
delete from GI_COMPOSITION where rowid = ACOTESDO_.tbGI_COMPOSITIONRowId(nuVarcharIndex); 
nuOldVarcharIndex:=nuVarcharIndex;
nuVarcharIndex := ACOTESDO_.tbGI_COMPOSITIONRowId.next(nuVarcharIndex); 
ACOTESDO_.tbGI_COMPOSITIONRowId.delete(nuOldVarcharIndex); 
END loop; 
...
if (not ACOTESDO_.blProcessStatus) then
 return;
  ...
  DELETE FROM GI_COMPOSITION WHERE (CONFIG_ID) in (SELECT CONFIG_ID FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='ACOTESDO') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339);
...
if (not ACOTESDO_.blProcessStatus) then
 return;
  ...
  DELETE FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='ACOTESDO') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339;
...
if (not ACOTESDO_.blProcessStatus) then
 return;
  ...
  DELETE FROM GE_DISTRIBUTION_FILE WHERE (DISTRIBUTION_FILE_ID) in (SELECT NAME FROM SA_EXECUTABLE WHERE NAME='ACOTESDO');
...
if (not ACOTESDO_.blProcessStatus) then
 return;
  ...
  DELETE FROM SA_EXECUTABLE WHERE NAME='ACOTESDO';
...
ACOTESDO_.old_tb0_0(0):='ACOTESDO'
;
ACOTESDO_.tb0_0(0):=UPPER(ACOTESDO_.old_tb0_0(0));
ACOTESDO_.tb0_0(0):=ACOTESDO_.tb0_0(0);
ACOTESDO_.old_tb0_1(0):=500000000013061;
ACOTESDO_.tb0_1(0):=GE_BOCATALOG.FNUGETIDSEQFROMCATALOG(ACOTESDO_.tb0_0(0), 'EXECUTABLE', 'SA_BOEXECUTABLE.GETNEXTID');
ACOTESDO_.tb0_1(0):=ACOTESDO_.tb0_1(0);
ut_trace.trace('insertando tabla: SA_EXECUTABLE fila (0)',1);
INSERT INTO SA_EXECUTABLE(NAME,EXECUTABLE_ID,DESCRIPTION,PATH,VERSION,EXECUTABLE_TYPE_ID,EXEC_OPER_TYPE_ID,MODULE_ID,SUBSYSTEM_ID,PARENT_EXECUTABLE_ID,LAST_RECORD_ALLOWED,PATH_FILE_HELP,WITHOUT_RESTR_POLICY,DIRECT_EXECUTION,TIMES_EXECUTED,EXEC_OWNER,LAST_DATE_EXECUTED,CLASS_ID) 
VALUES (ACOTESDO_.tb0_0(0),
ACOTESDO_.tb0_1(0),
...
ACOTESDO_.tb1_0(0):=1;
ACOTESDO_.tb1_1(0):=ACOTESDO_.tb0_1(0);
ut_trace.trace('insertando tabla: SA_ROLE_EXECUTABLES fila (0)',1);
INSERT INTO SA_ROLE_EXECUTABLES(ROLE_ID,EXECUTABLE_ID) 
VALUES (ACOTESDO_.tb1_0(0),
ACOTESDO_.tb1_1(0));
...
ACOTESDO_.tb2_0(0):=ACOTESDO_.tb0_0(0);
ACOTESDO_.clColumn_0 := '<?xml version="1.0" encoding="UTF-8"?>
...
'ITY_NAME/>
    ...
    <ATTRIBUTE>
      ...
      <DISPLAY_NAME>Estado Orden (Actual)</DISPLAY_NAME>
      <INIT_BASIC_EXP_ID/>
      <VALID_BASIC_EXP_ID/>
      <INIT_EXPRESSION_ID/>
      <VALID_EXPRESSION_ID/>
...
'EW>
      ...
      <VALID_EXPRESSION_ID/>
      <DEFAULT_VALUE/>
      <SELECT_STATEMENT>SELECT TASK_TYPE_ID ID, DESCRIPTION DESCRIPTION FROM OR_TASK_TYPE
union
...
'999992</CONFIGURATION_ID>
      ...
      <DISPLAY_NAME>Nuevo Estado</DISPLAY_NAME>
      <INIT_BASIC_EXP_ID/>
      <VALID_BASIC_EXP_ID/>
      <INIT_EXPRESSION_ID/>
      <VALID_EXPRESSION_ID/>
...
VALUES (ACOTESDO_.tb2_0(0),
XMLType(ACOTESDO_.clColumn_0),
'LDC - Actualizacion De Estado De Documentos'
,
'1.0.0.0'
,
...
'select decode(OPERATING_UNIT_ID, 1, -1, OPERATING_UNIT_ID) id, NAME description
from OR_OPERATING_UNIT
ORDER BY ID', 'Código De La Unidad Operativa A La Cual Fue Asignada La Orden De Trabajo'); 
...
ut_trace.trace('insertando tabla: GE_STATEMENT ['||nuNewStatement||']',1);
INSERT INTO GE_STATEMENT(STATEMENT_ID, MODULE_ID, DESCRIPTION, STATEMENT, NAME)  
VALUES (nuNewStatement, 4, 'Estado Actual De La Orden', 
'SELECT '|| chr(39) ||'CO'|| chr(39) ||' id, '|| chr(39) ||'EN PODER DE CONTRATISTA'|| chr(39) ||' description FROM DUAL
UNION
SELECT '|| chr(39) ||'EP'|| chr(39) ||' id, '|| chr(39) ||'EN PODER DE LA EMPRESA'|| chr(39) ||' description FROM DUAL', 'Estado Actual De La Orden'); 
...
'SELECT TASK_TYPE_ID ID, DESCRIPTION DESCRIPTION FROM OR_TASK_TYPE
union
select -1 id, '|| chr(39) ||'TODOS'|| chr(39) ||' description from DUAL
ORDER BY ID', 'Tipo De Trabajo'); 
...
ut_trace.trace('insertando tabla: GE_STATEMENT ['||nuNewStatement||']',1);
INSERT INTO GE_STATEMENT(STATEMENT_ID, MODULE_ID, DESCRIPTION, STATEMENT, NAME)  
VALUES (nuNewStatement, 4, 'Nuevo Estado', 
'SELECT '|| chr(39) ||'EP'|| chr(39) ||' id, '|| chr(39) ||'EN PODER DE LA EMPRESA'|| chr(39) ||' description FROM DUAL', 'Nuevo Estado'); 

ut_trace.trace('Modificando GE_DISTRIBUTION_FILE',1);
UPDATE GE_DISTRIBUTION_FILE SET  GE_DISTRIBUTION_FILE.APP_XML = 
  UPDATEXML(GE_DISTRIBUTION_FILE.APP_XML, 
  '//COMPOSITION/ATTRIBUTE[ENTITY_ATTRIBUTE_ID = 104880 and ENTITY_ID = 3294]/SELECT_STATEMENT_ID/text()', 
  to_char(nuNewStatement)) 
WHERE GE_DISTRIBUTION_FILE.DISTRIBUTION_FILE_ID = 'ACOTESDO';
...
BEGIN
 ...
 nuIndexInternal := ACOTESDO_.tb0_0.first;
 while nuIndexInternal IS NOT null loop
  IF (ACOTESDO_.tb0_0(nuIndexInternal)=isbExecutableName ) THEN
   IF (ACOTESDO_.tb0_1.exists(nuIndexInternal)) then
    tbExecutableEquivalence(isbExecutableName) := ACOTESDO_.tb0_1(nuIndexInternal);
   END IF;
   exit;
  END IF;
  nuIndexInternal := ACOTESDO_.tb0_0.next(nuIndexInternal);
 END loop; 
 return tbExecutableEquivalence(isbExecutableName);
END;
BEGIN
...
nuIndex :=  ACOTESDO_.tbRoleExecutables.first;
while nuIndex IS NOT null loop
 if (ACOTESDO_.tbRoleExecutables(nuIndex).role_id!=1) then
  nuNewExecutableId := fnuGetNewExecutableId(ACOTESDO_.tbRoleExecutables(nuIndex).name);
  IF nuNewExecutableId is not null then
    INSERT INTO sa_role_executables (role_id, executable_id)
    VALUES (ACOTESDO_.tbRoleExecutables(nuIndex).role_id, nuNewExecutableId);
  END IF;
  end if;
 nuIndex := ACOTESDO_.tbRoleExecutables.next(nuIndex);
END loop;
nuIndex :=  ACOTESDO_.tbUserException.first;
while nuIndex IS NOT null loop
 nuNewExecutableId := fnuGetNewExecutableId(ACOTESDO_.tbUserException(nuIndex).name);
 IF nuNewExecutableId is not null then
  insert INTO sa_user_exceptions (executable_id, user_id, status, usr_exc_type_id, comment_)
  VALUES (nuNewExecutableId ,ACOTESDO_.tbUserException(nuIndex).user_id, ACOTESDO_.tbUserException(nuIndex).status , ACOTESDO_.tbUserException(nuIndex).usr_exc_type_id, ACOTESDO_.tbUserException(nuIndex).comment_);
 END IF;
 nuIndex := ACOTESDO_.tbUserException.next(nuIndex);
END loop;
nuIndex :=  ACOTESDO_.tbExecEntities.first;
while nuIndex IS NOT null loop
 nuNewExecutableId := fnuGetNewExecutableId(ACOTESDO_.tbExecEntities(nuIndex).name);
 IF nuNewExecutableId is not null then
  select count(1) into nuRecCount
 from sa_exec_entities
  where executable_id = nuNewExecutableId and entity_id = ACOTESDO_.tbExecEntities(nuIndex).entity_id;
  IF nuRecCount = 0 then
    insert INTO sa_exec_entities (executable_id, entity_id )
    VALUES (nuNewExecutableId ,ACOTESDO_.tbExecEntities(nuIndex).entity_id );
  END IF;
 END IF;
 nuIndex := ACOTESDO_.tbExecEntities.next(nuIndex);
END loop;
...
BEGIN
 ...
 nuIndex := ACOTESDO_.tbObjectToDelete.first;
 while (nuIndex is not null) LOOP
  BEGIN
   ut_trace.trace('Objeto a borrar: ' || ACOTESDO_.tbObjectToDelete(nuIndex),1);
   IF(ut_object.fboExistObject(ACOTESDO_.tbObjectToDelete(nuIndex)) and not fblIsObjectUseByRule(ACOTESDO_.tbObjectToDelete(nuIndex))) THEN
    blObjectDeleted := ut_object.fboDeleteObject(ACOTESDO_.tbObjectToDelete(nuIndex));
    ut_trace.trace('  * Borrado ' || ACOTESDO_.tbObjectToDelete(nuIndex), 1);
   END IF;
  EXCEPTION when others then
   ut_trace.trace('- No se borra el objeto (' || ACOTESDO_.tbObjectToDelete(nuIndex) || '): ' || sqlerrm,1);
  END;
  nuIndex := ACOTESDO_.tbObjectToDelete.next(nuIndex);
 END loop;
...
begin
SA_BOCreatePackages.DropPackage('ACOTESDO_');
end;
/
