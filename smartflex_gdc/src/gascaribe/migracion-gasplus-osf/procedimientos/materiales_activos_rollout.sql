CREATE OR REPLACE PROCEDURE MATERIALES_ACTIVOS_ROLLOUT (nuInicio number, nuFinal number, nuBD number) AS 
/*******************************************************************
 PROGRAMA    	:	MATERIALES_ACTIVOS_ROLLOUT
 FECHA		:	25/11/2014
 AUTOR		:	OLSoftware
 DESCRIPCION	:	Migra la informacion de los materiales de tipo activo a LDC_ACT_OUIB
...
BEGIN
   ...
   PKLOG_MIGRACION.prInsLogMigra (1003,1003,1,'MIGRA_ACTIVOS',0,0,'Inicia Proceso','INICIO',nuLogError);
   
   UPDATE MIGR_RANGO_PROCESOS SET RAPRFEIN = SYSDATE, RAPRTERM = 'P' WHERE RAPRCODI = 1003 AND RAPRBASE = nuBD AND RAPRRAIN = nuInicio AND RAPRRAFI = nuFinal;
   COMMIT;
      ...
      LOOP
	    ...
	    nuCupo := 0;
         ...
         EXCEPTION
            WHEN OTHERS
            THEN
               PKLOG_MIGRACION.PRINSLOGMIGRA (1003,1003,2,'MIGRA_ACTIVOS: '||NUITEM,0,0,'Bodega: '||TBL_DATOS ( NUINDICE).CUADHOMO||' Code Error: '||SQLERRM,TO_CHAR(SQLCODE),NULOGERROR);
	  END;
   ...
   END IF; 
    ...
    -- Termina Log
   PKLOG_MIGRACION.prInsLogMigra (1003,1003,3,'MIGRA_ACTIVOS',nuTotalRegs,nuErrores,'TERMINO PROCESO #regs: '||vcontIns,'FIN',nuLogError);
   
   UPDATE MIGR_RANGO_PROCESOS SET RAPRFEFI = SYSDATE, RAPRTERM = 'T' where RAPRCODI = 1003 AND RAPRBASE = nuBD AND RAPRRAIN = nuInicio AND RAPRRAFI = nuFinal;
   COMMIT;
...
EXCEPTION
   ...
   THEN
     ...
     PKLOG_MIGRACION.prInsLogMigra (1003,1003,2,'MIGRA_ACTIVOS '||vcontIns,0,0,'Error: '||sqlerrm,to_char(sqlcode),nuLogError);
     
END MATERIALES_ACTIVOS_ROLLOUT; 
/
