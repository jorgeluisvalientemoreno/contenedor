CREATE OR REPLACE PROCEDURE PR_LD_POLICY_ROLLOUT1(nuInicio number,
                                                 nuFinal  number,
                                                 nuBD     number) AS
  /*******************************************************************
  ...
  *******************************************************************/
  ...
  cursor cuDatos is
    SELECT CASE
             WHEN A.BASEDATO IN (1, 4, 5) THEN
              A.POSUCODI
             WHEN A.BASEDATO = 2 THEN
              A.POSUCODI + 5000000
             WHEN A.BASEDATO = 3 THEN
              A.POSUCODI + 6000000
           END POLICY_ID,
           ...
           NULL FEES_TO_RETURN,
           CASE
             WHEN A.BASEDATO IN (1, 2, 3) THEN
              'POLIZA MIGRADA, POLIPOAS: [' || A.POSUPOAS || ']'
             WHEN A.BASEDATO = 4 THEN
              'POLIZA MIGRADA, POLIPOAS: [' || A.POSUPOAS || '] '
             WHEN A.BASEDATO = 5 THEN
              DECODE(A.POSULIPR,
                     62,
                     'POLIZA MIGRADA, POLICEDU: [' || A.POSUNICE || ']',
                     63,
                     'POLIZA MIGRADA, POLICOAS: [' || A.POSUCODI || ']')
           END COMMENTS,
      ...
      FROM MIGRA.LDC_TEMP_POLISUSC_SGE A,
           LDC_MIG_LOCALIDAD           B,
           LDC_MIG_TIPOPOLI            C,
           LDC_TEMP_SERVSUSC_SGE       D,
           DIFERIDO                    E,
           LD_VALIDITY_POLICY_TYPE     F
     WHERE A.POSUDEPA = B.CODIDEPA
      ...
      /* AND A.POSUSUSC >= nuInicio
       ...
       AND A.POSUCODI IN (158168, 163561, 160815, 159301)
  ...
  CURSOR CUGESUBS(nuSusc number) IS
    SELECT * FROM GE_SUBSCRIBER WHERE SUBSCRIBER_ID = nuSusc;
  ...
  FUNCTION existe_cliente(p_identification VARCHAR2,
                          p_sbNombre       varchar2,
                          p_sbApellido1    varchar2,
                          p_sbApellido2    varchar2) return number is
    CURSOR cucliente IS
  ...
  BEGIN
    ...
    if (p_sbNombre IS not null AND p_sbApellido1 IS not null AND
       p_sbApellido2 IS not null) then
      open cucliente;
      fetch cucliente
        INTO nusubscriber_id;
      close cucliente;
    elsif (p_sbNombre IS not null AND p_sbApellido1 IS not null AND
          p_sbApellido2 IS null) then
      open cucliente_1;
      fetch cucliente_1
        INTO nusubscriber_id;
      close cucliente_1;
    elsif (p_sbNombre IS not null AND p_sbApellido1 IS null AND
          p_sbApellido2 IS null) then
      open cucliente_2;
      fetch cucliente_2
        INTO nusubscriber_id;
...
BEGIN
  ...
  PKLOG_MIGRACION.prInsLogMigra(235,
                                235,
                                1,
                                'PR_LD_POLICY',
                                0,
  ...
  pkg_constantes.COMPLEMENTO(nuBD,
                             nuComplementoPR,
                             nuComplementoSU,
                             nuComplementoFA,
                             nuComplementoCU,
  ...
  LOOP
    ...
    FOR nuindice IN 1 .. tbl_datos.COUNT LOOP
        ...
        when others then
          BEGIN
            ...
            SELECT subscriber_id
              INTO nuSubscriberId
              FROM ge_subscriber
             WHERE subscriber_name = tbl_datos(nuindice)
            .NOMBRE
               AND subs_last_name IS NULL
               AND identification =
                   to_char(tbl_datos(nuindice).IDENTIFICATION_ID);
      ...
      IF RGCLIENTE.IDENTIFICATION IS NULL THEN
        ...
        nuSubscriberId := existe_cliente(TBL_DATOS(NUINDICE)
                                         .IDENTIFICATION_ID,
                                         TBL_DATOS(NUINDICE).NOMBRE,
                                         TBL_DATOS(NUINDICE).APELLIDO,
                                         NULL);
      ...
      BEGIN
        ...
        /*INSERT INTO LD_POLICY
          (POLICY_ID,
           ...
           COLLECTIVE_NUMBER)
        VALUES
          (TBL_DATOS(NUINDICE).POLICY_ID,
           TBL_DATOS(NUINDICE).POLICY_NUMBER,
           TBL_DATOS(NUINDICE).STATE_POLICY,
           TBL_DATOS(NUINDICE).LAUNCH_POLICY,
           TBL_DATOS(NUINDICE).CONTRATIST_CODE,
           TBL_DATOS(NUINDICE).PRODUCT_LINE_ID,
           TBL_DATOS(NUINDICE).DT_IN_POLICY,
           TBL_DATOS(NUINDICE).DT_EN_POLICY,
           TBL_DATOS(NUINDICE).VALUE_POLICY,
           TBL_DATOS(NUINDICE).PREM_POLICY,
           TBL_DATOS(NUINDICE).NOMBRE || ' ' || TBL_DATOS(NUINDICE)
           .APELLIDO,
           TBL_DATOS(NUINDICE).SUSCRIPTION_ID,
           TBL_DATOS(NUINDICE).PRODUCT_ID,
           NUCCIDENT,
           TBL_DATOS(NUINDICE).PERIOD_POLICY,
           TBL_DATOS(NUINDICE).YEAR_POLICY,
           TBL_DATOS(NUINDICE).MONTH_POLICY,
           TBL_DATOS(NUINDICE).DEFERRED_POLICY_ID,
           TBL_DATOS(NUINDICE).DTCREATE_POLICY,
           TBL_DATOS(NUINDICE).SHARE_POLICY,
           TBL_DATOS(NUINDICE).DTRET_POLICY,
           TBL_DATOS(NUINDICE).VALUEACR_POLICY,
           TBL_DATOS(NUINDICE).REPORT_POLICY,
           TBL_DATOS(NUINDICE).DT_REPORT_POLICY,
           TBL_DATOS(NUINDICE) . DT_INSURED_POLICY,
           TBL_DATOS(NUINDICE).PER_REPORT_POLICY,
           TBL_DATOS(NUINDICE).POLICY_TYPE_ID,
           TBL_DATOS(NUINDICE).ID_REPORT_POLICY,
           TBL_DATOS(NUINDICE).CANCEL_CAUSAL_ID,
           TBL_DATOS(NUINDICE).FEES_TO_RETURN,
           TBL_DATOS(NUINDICE).COMMENTS,
           TBL_DATOS(NUINDICE).POLICY_EXQ,
           TBL_DATOS(NUINDICE).NUMBER_ACTA,
           TBL_DATOS(NUINDICE).GEOGRAP_LOCATION_ID,
           TBL_DATOS(NUINDICE).VALIDITY_POLICY_TYPE_ID,
        ...
        VALUES
          (SEQ_MO_MOTIVE.NEXTVAL,
           ...
           NUATENCION,
           Null,
           'M_INSTALACION_DEBRILLA_SEGUROS_100251',
           Null,
           -1,
           Null,
           99,
      ...
      EXCEPTION
        WHEN OTHERS THEN
          BEGIN
            verror    := 'Error en la Poliza : ' || NUSUSCRIPTOR || ' - ' ||
                         nuSubscriberId || ' - ' || tbl_datos(nuindice)
                        .IDENTIFICATION_ID || ' - ' || SQLERRM;
