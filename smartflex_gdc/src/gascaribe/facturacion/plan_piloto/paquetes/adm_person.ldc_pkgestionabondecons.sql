set serveroutput on;
PROMPT BORRAR LDC_PKGESTIONABONDECONS
DECLARE
  nuConta       NUMBER;
  sbDueno	    VARCHAR2(30);
...
BEGIN
    BEGIN
    dbms_output.put_Line('Preparando borrado de paquete ADM_PERSON.LDC_PKGESTIONABONDECONS');
        SELECT COUNT(*), OWNER, OBJECT_NAME,  OBJECT_TYPE INTO nuConta, sbDueno, sbobject_name, sbobject_type
          FROM dba_objects
         WHERE object_name = 'LDC_PKGESTIONABONDECONS'  
    ...
    EXCEPTION WHEN OTHERS THEN
        nuConta:= 0;
        dbms_output.put_Line('paquete ADM_PERSON.LDC_PKGESTIONABONDECONS no existe o fue borrado');
    END;
