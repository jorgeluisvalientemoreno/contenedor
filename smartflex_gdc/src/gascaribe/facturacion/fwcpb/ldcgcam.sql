...
BEGIN
sa_bocreatePackages.CreatePackage('LDCGCAM_',
'CREATE OR REPLACE PACKAGE LDCGCAM_ IS ' || chr(10) ||
...
'tb2_0 ty2_0;  executableName ge_catalog.tag_name%type := ''LDCGCAM''; ' || chr(10) ||
...
'( ' || chr(10) ||
'SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME=''LDCGCAM'' ' || chr(10) ||
'); ' || chr(10) ||
...
BEGIN ut_trace.trace('********************Comenzar proceso de objeto:LDCGCAM_******************************'); END;
/
...
if (not LDCGCAM_.blProcessStatus) then
 return;
end if;
Open LDCGCAM_.cuRoleExecutables;
loop
 fetch LDCGCAM_.cuRoleExecutables INTO LDCGCAM_.rcRoleExecutables;
 exit when  LDCGCAM_.cuRoleExecutables%notfound;
 LDCGCAM_.tbRoleExecutables(nuIndex) := LDCGCAM_.rcRoleExecutables;
 nuIndex := nuIndex + 1;
END loop;
close LDCGCAM_.cuRoleExecutables;
nuIndex := 0;
Open LDCGCAM_.cuUserExceptions ;
loop
 fetch LDCGCAM_.cuUserExceptions INTO  LDCGCAM_.rcUserExceptions;
 exit when LDCGCAM_.cuUserExceptions%notfound;
 LDCGCAM_.tbUserException(nuIndex):=LDCGCAM_.rcUserExceptions;
 nuIndex := nuIndex + 1;
END loop;
close LDCGCAM_.cuUserExceptions;
nuIndex := 0;
Open LDCGCAM_.cuExecEntities ;
loop
 fetch LDCGCAM_.cuExecEntities INTO  LDCGCAM_.rcExecEntities;
 exit when LDCGCAM_.cuExecEntities%notfound;
 LDCGCAM_.tbExecEntities(nuIndex):=LDCGCAM_.rcExecEntities;
 nuIndex := nuIndex + 1;
END loop;
close LDCGCAM_.cuExecEntities;

exception when others then
LDCGCAM_.blProcessStatus := false;
rollback;
...
(
 SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDCGCAM'
);
    ...
    BEGIN
      ...
      IF (nuNumRecords > 0) THEN
        ...
        IF ( sbObjectName IS NOT null ) THEN
          ...
          if ( LDCGCAM_.tbObjectToDelete.last IS null ) then
            nuNumObjects := 1;
          else
            nuNumObjects := LDCGCAM_.tbObjectToDelete.last + 1;
          end if;
          LDCGCAM_.tbObjectToDelete(nuNumObjects) := sbObjectName;
        END IF;
...
if (not LDCGCAM_.blProcessStatus) then
 return;
  ...
  WHERE GE_DISTRIBUTION_FILE.DISTRIBUTION_FILE_ID = 'LDCGCAM';
  IF (nuNumRecords > 0) THEN
...
WHERE   sa_executable.name = 'LDCGCAM'
...
if (not LDCGCAM_.blProcessStatus) then
 ...
 FOR crObjectsName IN cuObjectsName LOOP
  LDCGCAM_.tbObjectToDelete(nuIndex) := crObjectsName.OBJECT_NAME;
  nuIndex := nuIndex + 1;
 END LOOP;
EXCEPTION 
...
if (not LDCGCAM_.blProcessStatus) then
 return;
  ...
  DELETE FROM SA_ROLE_EXECUTABLES WHERE (EXECUTABLE_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDCGCAM') AND ROLE_ID=1;
...
if (not LDCGCAM_.blProcessStatus) then
 return;
  ...
  DELETE FROM SA_USER_EXCEPTIONS WHERE (EXECUTABLE_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDCGCAM') AND 1=2;
...
SELECT rowid
FROM SA_EXECUTABLE_SYNON WHERE (SYNONYMOUS_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDCGCAM');
nuIndex binary_integer;
BEGIN
...
if (not LDCGCAM_.blProcessStatus) then
 return;
  ...
  DELETE FROM SA_EXEC_ENTITIES WHERE (EXECUTABLE_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDCGCAM');
...
if (not LDCGCAM_.blProcessStatus) then
 return;
  ...
  DELETE FROM SA_MENU_OPTION WHERE (EXECUTABLE_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDCGCAM');
...
if (not LDCGCAM_.blProcessStatus) then
 return;
  ...
  DELETE FROM GI_CONFIG_COMP WHERE (CONFIG_ID) in (SELECT CONFIG_ID FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDCGCAM') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339);
...
if (not LDCGCAM_.blProcessStatus) then
 return;
  ...
  ut_trace.trace('Borrando tabla GI_CONFIG_COMP',1);
  DELETE FROM GI_CONFIG_COMP WHERE (COMPOSITION_ID) in (SELECT COMPOSITION_ID FROM GI_COMPOSITION WHERE (CONFIG_ID) in (SELECT CONFIG_ID FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDCGCAM') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339));
...
DECLARE 
 ...
 SELECT rowid FROM GE_STATEMENT WHERE (STATEMENT_ID) in (SELECT SELECT_STATEMENT_ID FROM GI_COMP_ATTRIBS WHERE (COMPOSITION_ID) in (SELECT COMPOSITION_ID FROM GI_COMPOSITION WHERE (CONFIG_ID) in (SELECT CONFIG_ID FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDCGCAM') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339)));
BEGIN 
...
LDCGCAM_.tbGE_STATEMENTRowId(rcData.rowid):=rcData.rowid; 
END loop; 
...
for rcData in cuLoadTemporaryTable loop
LDCGCAM_.tbGR_CONFIG_EXPRESSIONRowId(rcData.rowid):=rcData.rowid; 
END loop; 
...
if (not LDCGCAM_.blProcessStatus) then
 return;
  ...
  DELETE FROM GI_COMP_ATTRIBS WHERE (COMPOSITION_ID) in (SELECT COMPOSITION_ID FROM GI_COMPOSITION WHERE (CONFIG_ID) in (SELECT CONFIG_ID FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDCGCAM') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339));
...
nuVarcharIndex:=LDCGCAM_.tbGE_STATEMENTRowId.first;  
while (nuVarcharIndex is not null) loop 
begin
ut_trace.trace('Row id a borrar:'||nuVarcharIndex,1);
delete from GE_STATEMENT where rowid = LDCGCAM_.tbGE_STATEMENTRowId(nuVarcharIndex); 
...
LDCGCAM_.tbGE_STATEMENTRowId.delete(nuOldVarcharIndex);
end if;
nuVarcharIndex := LDCGCAM_.tbGE_STATEMENTRowId.next(nuVarcharIndex); 
END loop; 
...
nuVarcharIndex:=LDCGCAM_.tbGR_CONFIG_EXPRESSIONRowId.first;  
while (nuVarcharIndex is not null) loop 
ut_trace.trace('Row id a borrar:'||nuVarcharIndex,1);
delete from GR_CONFIG_EXPRESSION where rowid = LDCGCAM_.tbGR_CONFIG_EXPRESSIONRowId(nuVarcharIndex); 
nuOldVarcharIndex:=nuVarcharIndex;
nuVarcharIndex := LDCGCAM_.tbGR_CONFIG_EXPRESSIONRowId.next(nuVarcharIndex); 
LDCGCAM_.tbGR_CONFIG_EXPRESSIONRowId.delete(nuOldVarcharIndex); 
END loop; 
...
for rcData in cuLoadTemporaryTable loop
LDCGCAM_.tbGI_COMPOSITIONRowId(rcData.rowid):=rcData.rowid; 
END loop; 
...
if (not LDCGCAM_.blProcessStatus) then
 return;
  ...
  DELETE FROM GI_FRAME WHERE (COMPOSITION_ID) in (SELECT COMPOSITION_ID FROM GI_COMPOSITION WHERE (CONFIG_ID) in (SELECT CONFIG_ID FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDCGCAM') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339));
...
nuVarcharIndex:=LDCGCAM_.tbGI_COMPOSITIONRowId.first;  
while (nuVarcharIndex is not null) loop 
ut_trace.trace('Row id a borrar:'||nuVarcharIndex,1);
delete from GI_COMPOSITION where rowid = LDCGCAM_.tbGI_COMPOSITIONRowId(nuVarcharIndex); 
nuOldVarcharIndex:=nuVarcharIndex;
nuVarcharIndex := LDCGCAM_.tbGI_COMPOSITIONRowId.next(nuVarcharIndex); 
LDCGCAM_.tbGI_COMPOSITIONRowId.delete(nuOldVarcharIndex); 
END loop; 
...
if (not LDCGCAM_.blProcessStatus) then
 return;
  ...
  DELETE FROM GI_COMPOSITION WHERE (CONFIG_ID) in (SELECT CONFIG_ID FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDCGCAM') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339);
...
if (not LDCGCAM_.blProcessStatus) then
 return;
  ...
  DELETE FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDCGCAM') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339;
...
if (not LDCGCAM_.blProcessStatus) then
 return;
  ...
  DELETE FROM GE_DISTRIBUTION_FILE WHERE (DISTRIBUTION_FILE_ID) in (SELECT NAME FROM SA_EXECUTABLE WHERE NAME='LDCGCAM');
...
if (not LDCGCAM_.blProcessStatus) then
 return;
  ...
  DELETE FROM SA_EXECUTABLE WHERE NAME='LDCGCAM';
...
LDCGCAM_.old_tb0_0(0):='LDCGCAM'
;
LDCGCAM_.tb0_0(0):=UPPER(LDCGCAM_.old_tb0_0(0));
LDCGCAM_.tb0_0(0):=LDCGCAM_.tb0_0(0);
LDCGCAM_.old_tb0_1(0):=500000000014968;
LDCGCAM_.tb0_1(0):=GE_BOCATALOG.FNUGETIDSEQFROMCATALOG(LDCGCAM_.tb0_0(0), 'EXECUTABLE', 'SA_BOEXECUTABLE.GETNEXTID');
LDCGCAM_.tb0_1(0):=LDCGCAM_.tb0_1(0);
ut_trace.trace('insertando tabla: SA_EXECUTABLE fila (0)',1);
INSERT INTO SA_EXECUTABLE(NAME,EXECUTABLE_ID,DESCRIPTION,PATH,VERSION,EXECUTABLE_TYPE_ID,EXEC_OPER_TYPE_ID,MODULE_ID,SUBSYSTEM_ID,PARENT_EXECUTABLE_ID,LAST_RECORD_ALLOWED,PATH_FILE_HELP,WITHOUT_RESTR_POLICY,DIRECT_EXECUTION,TIMES_EXECUTED,EXEC_OWNER,LAST_DATE_EXECUTED,CLASS_ID) 
VALUES (LDCGCAM_.tb0_0(0),
LDCGCAM_.tb0_1(0),
...
LDCGCAM_.tb1_0(0):=1;
LDCGCAM_.tb1_1(0):=LDCGCAM_.tb0_1(0);
ut_trace.trace('insertando tabla: SA_ROLE_EXECUTABLES fila (0)',1);
INSERT INTO SA_ROLE_EXECUTABLES(ROLE_ID,EXECUTABLE_ID) 
VALUES (LDCGCAM_.tb1_0(0),
LDCGCAM_.tb1_1(0));
...
LDCGCAM_.tb2_0(0):=LDCGCAM_.tb0_0(0);
LDCGCAM_.clColumn_0 := '<?xml version="1.0" encoding="UTF-8"?>
...
'SCALE/>
      ...
      <DISPLAY_NAME>Proceso a Ejecutar</DISPLAY_NAME>
      <INIT_BASIC_EXP_ID/>
      <VALID_BASIC_EXP_ID/>
      <INIT_EXPRESSION_ID/>
      <VALID_EXPRESSION_ID/>
...
'
      ...
      <VALID_EXPRESSION_ID/>
      <DEFAULT_VALUE/>
      <SELECT_STATEMENT>select 1 id, '||chr(38)||'apos;FGCA'||chr(38)||'apos; description
from dual
union all
select 2, '||chr(38)||'apos;FGCC'||chr(38)||'apos;
from dual
union all
select 3, '||chr(38)||'apos;FIDF'||chr(38)||'apos;
from dual
union all
...
'AME>
      ...
      <DISPLAY_NAME>Programacion</DISPLAY_NAME>
      <INIT_BASIC_EXP_ID/>
      <VALID_BASIC_EXP_ID/>
      <INIT_EXPRESSION_ID/>
      <VALID_EXPRESSION_ID/>
...
'>
    ...
    <ATTRIBUTE>
      <ENTITY_ATTRIBUTE_ID>18248</ENTITY_ATTRIBUTE_ID>
      <CONFIGURATION_ID>-999999993</CONFIGURATION_ID>
      <ATTRIBUTE_NAME>PEFAFEGE</ATTRIBUTE_NAME>
      <ENTITY_ID>999</ENTITY_ID>
      <ENTITY_NAME>PERIFACT</ENTITY_NAME>
      <PARENT_CONF_ID>-999999998</PARENT_CONF_ID>
      <TYPE_ATTRIB_ID>2</TYPE_ATTRIB_ID>
...
'LE/>
      ...
      <DISPLAY_NAME>El proceso FIDF su ordenamiento por defecto es: Ruta y Funcion</DISPLAY_NAME>
      <INIT_BASIC_EXP_ID/>
      <VALID_BASIC_EXP_ID/>
      <INIT_EXPRESSION_ID/>
      <VALID_EXPRESSION_ID/>
...
VALUES (LDCGCAM_.tb2_0(0),
XMLType(LDCGCAM_.clColumn_0),
'Generacion de Cargos Masivos'
,
'1.0.0.0'
,
17,
...
VALUES (nuNewStatement, 21, 'lista de procesos', 
'select 1 id, '|| chr(39) ||'FGCA'|| chr(39) ||' description
from dual
union all
select 2, '|| chr(39) ||'FGCC'|| chr(39) ||'
from dual
union all
select 3, '|| chr(39) ||'FIDF'|| chr(39) ||'
from dual
union all
select 4, '|| chr(39) ||'FCPE'|| chr(39) ||'
from dual
', 'lista de procesos'); 
...
BEGIN
V2:=UT_DATE.FDTSYSDATE;
V1:=UT_DATE.FDTDATEWITHFORMAT(V2);
...
UPDATE GE_DISTRIBUTION_FILE SET  GE_DISTRIBUTION_FILE.APP_XML = 
  UPDATEXML(GE_DISTRIBUTION_FILE.APP_XML, 
  '//COMPOSITION/ATTRIBUTE[ENTITY_ATTRIBUTE_ID = 18248 and ENTITY_ID = 999]/INIT_EXPRESSION_ID/text()', 
  to_char(nuNewRule)) 
WHERE GE_DISTRIBUTION_FILE.DISTRIBUTION_FILE_ID = 'LDCGCAM';
...
BEGIN
 ...
 nuIndexInternal := LDCGCAM_.tb0_0.first;
 while nuIndexInternal IS NOT null loop
  IF (LDCGCAM_.tb0_0(nuIndexInternal)=isbExecutableName ) THEN
   IF (LDCGCAM_.tb0_1.exists(nuIndexInternal)) then
    tbExecutableEquivalence(isbExecutableName) := LDCGCAM_.tb0_1(nuIndexInternal);
   END IF;
   exit;
  END IF;
  nuIndexInternal := LDCGCAM_.tb0_0.next(nuIndexInternal);
 END loop; 
 return tbExecutableEquivalence(isbExecutableName);
END;
BEGIN
...
nuIndex :=  LDCGCAM_.tbRoleExecutables.first;
while nuIndex IS NOT null loop
 if (LDCGCAM_.tbRoleExecutables(nuIndex).role_id!=1) then
  nuNewExecutableId := fnuGetNewExecutableId(LDCGCAM_.tbRoleExecutables(nuIndex).name);
  IF nuNewExecutableId is not null then
    INSERT INTO sa_role_executables (role_id, executable_id)
    VALUES (LDCGCAM_.tbRoleExecutables(nuIndex).role_id, nuNewExecutableId);
  END IF;
  end if;
 nuIndex := LDCGCAM_.tbRoleExecutables.next(nuIndex);
END loop;
nuIndex :=  LDCGCAM_.tbUserException.first;
while nuIndex IS NOT null loop
 nuNewExecutableId := fnuGetNewExecutableId(LDCGCAM_.tbUserException(nuIndex).name);
 IF nuNewExecutableId is not null then
  insert INTO sa_user_exceptions (executable_id, user_id, status, usr_exc_type_id, comment_)
  VALUES (nuNewExecutableId ,LDCGCAM_.tbUserException(nuIndex).user_id, LDCGCAM_.tbUserException(nuIndex).status , LDCGCAM_.tbUserException(nuIndex).usr_exc_type_id, LDCGCAM_.tbUserException(nuIndex).comment_);
 END IF;
 nuIndex := LDCGCAM_.tbUserException.next(nuIndex);
END loop;
nuIndex :=  LDCGCAM_.tbExecEntities.first;
while nuIndex IS NOT null loop
 nuNewExecutableId := fnuGetNewExecutableId(LDCGCAM_.tbExecEntities(nuIndex).name);
 IF nuNewExecutableId is not null then
  select count(1) into nuRecCount
 from sa_exec_entities
  where executable_id = nuNewExecutableId and entity_id = LDCGCAM_.tbExecEntities(nuIndex).entity_id;
  IF nuRecCount = 0 then
    insert INTO sa_exec_entities (executable_id, entity_id )
    VALUES (nuNewExecutableId ,LDCGCAM_.tbExecEntities(nuIndex).entity_id );
  END IF;
 END IF;
 nuIndex := LDCGCAM_.tbExecEntities.next(nuIndex);
END loop;
...
BEGIN
 ...
 nuIndex := LDCGCAM_.tbObjectToDelete.first;
 while (nuIndex is not null) LOOP
  BEGIN
   ut_trace.trace('Objeto a borrar: ' || LDCGCAM_.tbObjectToDelete(nuIndex),1);
   IF(ut_object.fboExistObject(LDCGCAM_.tbObjectToDelete(nuIndex)) and not fblIsObjectUseByRule(LDCGCAM_.tbObjectToDelete(nuIndex))) THEN
    blObjectDeleted := ut_object.fboDeleteObject(LDCGCAM_.tbObjectToDelete(nuIndex));
    ut_trace.trace('  * Borrado ' || LDCGCAM_.tbObjectToDelete(nuIndex), 1);
   END IF;
  EXCEPTION when others then
   ut_trace.trace('- No se borra el objeto (' || LDCGCAM_.tbObjectToDelete(nuIndex) || '): ' || sqlerrm,1);
  END;
  nuIndex := LDCGCAM_.tbObjectToDelete.next(nuIndex);
 END loop;
...
begin
SA_BOCreatePackages.DropPackage('LDCGCAM_');
end;
/
