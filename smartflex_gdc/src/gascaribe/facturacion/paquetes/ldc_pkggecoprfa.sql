CREATE OR REPLACE package ldc_pkggecoprfa IS
/***************************************************************************************************************
...
END ldc_pkggecoprfa;
/
CREATE OR REPLACE package BODY ldc_pkggecoprfa IS
...
****************************************************************************************************************/
 ...
 vgCicloPrPago NUMBER :=0 ;
  ...
  -- Destinatarios
  sbto        Varchar2(4000) := dald_parameter.fsbgetvalue_chain('LDC_EMAILPRALI', NULL);
  sbsubject   Varchar2(4000) := 'Proceso de Alivios';
  sbmsg  VARCHAR2(4000);
...
***************************************************************************/
  ...
  sbejecutaPP VARCHAR2(1) := DALD_PARAMETER.FSBGETVALUE_CHAIN('LDC_FLAGEJPP',NULL);
  ...
  END IF;
   ...
   ldc_pkggecoprfa.prFinaDeudpConti;
     ...
     ldc_proactualizaestaprog(nutsess,osberror,'PRJOBGESTIOCONTI','Ok.');
	   ...
	   sbmsg := 'Proceso de Alivio termino correctamente, por favor validar ciclos procesados.';
        --se envia correo
...
***************************************************************************/
  ...
  FROM (
    ...
    VALUES
    (
      inuFactura, inucuenta, inuprod, idtfechaVen, idtfechaVefi, inuvalCAL, inucupon,  inuvalor, inuConsDocu, SYSDATE, InuPeriActu,inuciclo,inuConc
    );
   exception
    WHEN EX.CONTROLLED_ERROR THEN
...
***************************************************************************/
  ...
  sbCausaDife VARCHAR2(4000) := dald_parameter.fsbgetvalue_chain('LDC_CACADIFE',null);
  nuPeriCons   NUMBER;
  nuFactura    number;
  dtFechaIni   DATE;
  dtFechapago   DATE;
  ...
  --Se consulta saldo de la cuenta
  cursor cuSaldoCuenta(inucuenta NUMBER, inuconcepto NUMBER) IS
  select sum(decode(cargsign,'CR',-cargvalo,'AS',-cargvalo,'TS',-cargvalo,cargvalo)) saldocc
	from open.cargos
	where cargcuco=inucuenta
	AND cargconc != inuconcepto
	and cargsign NOT IN ( 'PA','SA');


  sbExisPag VARCHAR2(1);
    --se consulta si el pago fue antes de fecha de vencimiento
  CURSOR cugetPagos(inucupon number, inuSusc number, idtfechaVen date) IS
  SELECT 'X'
  FROM pagos p
  WHERE p.pagosusc = inuSusc
    AND p.pagocupo = inucupon
  ...
  FROM (
	  SELECT CARGCUCO,
		   ...
		   cargfecr fecha_pago
	  FROM CARGOS, cuencobr cu, factura f, perifact, SERVSUSC
	  WHERE CARGSIGN ='PA'
	   ...
	   and sesunuse = cargnuse
	    and nvl(cu.cucosacu,0) = 0
	   AND NOT EXISTS ( SELECT 1
					  FROM CARGOS CA
					  WHERE CA.CARGCUCO = CU.CUCOCODI
						AND CA.CARGCACA in (SELECT to_number(regexp_substr(sbCausaDife,'[^,]+', 1, LEVEL)) AS CAUSAL
											  FROM dual
											CONNECT BY regexp_substr(sbCausaDife, '[^,]+', 1, LEVEL) IS NOT NULL)
						AND CA.CARGCONC NOT IN (select ticoconc
												from OPEN.ldc_tiprodcondes )
												)
      ...
      FOR reg IN cuPeriodos LOOP
         IF sbFlagDpat = 'S' THEN
															   ...
															   WHERE ( nvl(cu.cucosacu,0) > 0
																	  OR ( nvl(cu.cucosacu,0) = 0
																		AND  EXISTS ( SELECT 1
																					  FROM CARGOS CA
																					  WHERE CA.CARGCUCO = CU.CUCOCODI
																						AND CA.CARGCACA in ('||sbCausaDife||')
																						AND CA.CARGCONC NOT IN (select ticoconc
																												from OPEN.ldc_tiprodcondes )
																								)))
																AND f.factcodi = cu.cucofact)
		   ...
		   --FOR regf IN cuFacturaSinSaldo(REG.PEFACODI) LOOP
				...
				OPEN cugetPagos(nuCuponPago, vtblfacturaSinSaldo(idx).factsusc, REG.pefafepa );
				FETCH cugetPagos INTO sbExisPag;
				CLOSE cugetPagos;

				IF sbExisPag IS NOT NULL THEN
					...
					END IF;
					 ...
					 FOR regCuen IN cuGetCuentAfect( nuFactura, reg.pefafimo, REG.PEFACICL,  nuCuponPago) LOOP
						...
						--se consulta saldo de cuenta
						IF cuSaldoCuenta%ISOPEN THEN
					       CLOSE cuSaldoCuenta;
						END IF;

						open cuSaldoCuenta(regCuen.CARGCUCO, regCuen.concepto);
						fetch cuSaldoCuenta into nuvalorDesc;
						close cuSaldoCuenta;

						if nuvalorDesc > 0 then
							...
							prgeneraNotas( vtblfacturaSinSaldo(idx).factsusc,
										   ...
										   nuvalorDesc,--regCuen.valor,
										   round(nuvalorDesc * (nuPorcDesc/100), 0) ,--regCuen.valor_DESC,
										   regCuen.concepto,
										   nuerror,
										   osberror) ;
							   IF nuerror = 0 THEN
		   ...
		   --FOR regf IN cuFacturaSinSaldo(nuperiante) LOOP
					...
					nuFactura := null;
                     ...
                     sbExisPag := null;
					  ...
					  OPEN cugetPagos(nuCuponPago, vtblfacturaSinSaldo(idx).factsusc, dtFechapago );
					FETCH cugetPagos INTO sbExisPag;
					CLOSE cugetPagos;
  ...
  ***************************************************************************/
    ...
    FROM (
      ...
      FROM(
        ...
        FROM (
            SELECT  causal, concepto, sum(valor) valor
            FROM (
              SELECT  (CASE WHEN catrconc in (31,17, 196) and cr.CATRCACA in (15,inuCausal) /*and isbTotConc = 'N'*/ then 1 else 0 end)  causal, decode(cr.catrconc,196,31,cr.catrconc) concepto, decode(catrsign, 'DB',-cr.catrvare,cr.catrvare)  valor
              FROM OPEN.cargtram cr, OPEN.mo_motive m, OPEN.mo_packages s
              WHERE cr.catrcuco = inucuenta
               AND cr.catrmoti = m.motive_id
               AND m.package_id  = s.package_id
             ...
             -- and CATRCACA = DECODE(isbTotConc, 'N', 15, CATRCACA)
              UNION ALL
              SELECT  (CASE WHEN cargconc in (31,17, 196) and cargcaca in (15,inuCausal) /*and isbTotConc = 'N'*/ then 1 else 0 end)  causal, decode(c.cargconc, 196, 31, c.cargconc), decode(c.cargsign, 'DB', c.cargvalo, - c.cargvalo) valor
              FROM OPEN.cargos c
              WHERE cargcuco = inucuenta
              and cargsign IN ('DB', 'CR')
  ...
  ( SELECT sum(valor)*-1 valor
   ...
   FROM (
       ...
       FROM (
         SELECT causal, concepto, sum(valor) valor
         FROM ( SELECT  CASE WHEN cargconc in (31,17, 196) and cargcaca in (15,inuCausal) /*and isbTotConc = 'N'*/ then 1 else 0 end  causal, decode(c.cargconc, 196, 31, c.cargconc) concepto, decode(c.cargsign, 'DB', c.cargvalo, - c.cargvalo) valor
                 FROM OPEN.cargos c
                WHERE cargcuco = inucuenta
                 and cargsign IN ('DB', 'CR')
               --  and cargcaca = DECODE(isbTotConc, 'N', 15, cargcaca)
...
***************************************************************************/
 ...
 BEGIN
  ...
  IF fblaplicaentregaxcaso('0000379') THEN
   ...
   IF sbFlagConsEjec = 'N' AND sbFlagOtCoEjec = 'N' THEN
     FOR reg IN cuPeriodos LOOP
       nuerror := null;
       osberror := null;
   ...
   END IF;
    ...
    -- consumo por fecha  de vencimiento y otros por fecha final de movimientos
   IF sbFlagConsEjec = 'N' AND sbFlagOtCoEjec = 'S' THEN
     --se financia solo consumo
     FOR reg IN cuPeriodos LOOP
       nuerror := null;
       osberror := null;
       ...
       END IF;
     END LOOP;
     --se financia otros conceptos diferentes al consumo
     FOR reg IN cuPeriodosFF LOOP
      nuerror := null;
      osberror := null;
      nuperiante := NULL;
      ...
      --
      OPEN cuperiodoant(reg.pefacicl, reg.pefafimo);
      FETCH cuperiodoant INTO nuperiante, dtFechaPago;
      CLOSE cuperiodoant;

      IF nuperiante IS NOT NULL THEN
         prGeneFinanConc ( nuperiante,
                           'O',
                            dtFechaPago,
                           nuerror,
                           osberror);
   ...
   -- otros  por fecha  de vencimiento
   IF sbFlagConsEjec = 'S' AND sbFlagOtCoEjec = 'N' THEN
     --se financia otros conceptos
     FOR reg IN cuPeriodos LOOP
       nuerror := null;
       osberror := null;
   ...
   IF sbFlagConsEjec = 'S' AND sbFlagOtCoEjec = 'S' THEN
    ...
    --
        ...
        FETCH cuperiodoant INTO nuperiante, dtFechaPago;
        CLOSE cuperiodoant;
       -- dbms_output.put_line('nuperiante '||nuperiante);
        IF nuperiante IS NOT NULL THEN
           prGeneFinanConc ( nuperiante,
                             'T',
                             dtFechaPago,
