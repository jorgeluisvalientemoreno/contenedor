set serveroutput on;
PROMPT BORRAR LDC_PROVALIREGENSERVNUEVOS_PR  
DECLARE
  nuConta       NUMBER;
  sbDueno	    VARCHAR2(30);
...
BEGIN
    BEGIN
    dbms_output.put_Line('Preparando borrado de synonym ADM_PERSON.LDC_PROVALIREGENSERVNUEVOS_PR');
        SELECT  COUNT(*), OWNER, OBJECT_NAME, OBJECT_TYPE INTO nuConta, sbDueno, sbobject_name, sbobject_type
          FROM dba_objects
         WHERE object_name = 'LDC_PROVALIREGENSERVNUEVOS_PR'
...
--
    BEGIN
    dbms_output.put_Line('Preparando borrado de procedimiento OPEN.LDC_PROVALIREGENSERVNUEVOS_PR');
        SELECT COUNT(*), OWNER, OBJECT_NAME, OBJECT_TYPE INTO nuConta, sbDueno, sbobject_name, sbobject_type
          FROM dba_objects
         WHERE object_name = 'LDC_PROVALIREGENSERVNUEVOS_PR'  
           AND OWNER = 'OPEN'
