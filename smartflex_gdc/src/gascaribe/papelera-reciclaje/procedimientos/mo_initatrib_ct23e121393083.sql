set serveroutput on;
PROMPT BORRAR MO_INITATRIB_CT23E121393083  
DECLARE
  nuConta       NUMBER;
  sbDueno	    VARCHAR2(30);
...
BEGIN
    BEGIN
    dbms_output.put_Line('***Preparando borrado de Procedure OPEN.MO_INITATRIB_CT23E121393083');
        SELECT COUNT(*), OWNER, OBJECT_NAME, OBJECT_TYPE INTO nuConta, sbDueno, sbobject_name, sbobject_type
          FROM dba_objects
         WHERE object_name = 'MO_INITATRIB_CT23E121393083'  
           AND OWNER = 'OPEN'
    ...
    EXCEPTION WHEN OTHERS THEN
        nuConta:= 0;
        dbms_output.put_Line('Procedure OPEN.MO_INITATRIB_CT23E121393083 no existe o fue borrado');
    END;
    IF nuConta > 0 then
