set serveroutput on;
PROMPT BORRAR LDC_PARAMEJECUTAPROCNOVE  
DECLARE
  nuConta       NUMBER;
  sbDueno	      VARCHAR2(30);
...
BEGIN
    BEGIN
        SELECT COUNT(*), OWNER, OBJECT_NAME, OBJECT_TYPE INTO nuConta, sbDueno, sbobject_name, sbobject_type
          FROM dba_objects
         WHERE object_name = 'LDC_PARAMEJECUTAPROCNOVE'  
           AND OWNER = 'OPEN'
           AND OBJECT_TYPE <> 'SYNONYM'
         GROUP BY OWNER, OBJECT_NAME, OBJECT_TYPE; 
    EXCEPTION WHEN OTHERS THEN
        nuConta:= 0;
        dbms_output.put_Line('OPEN.LDC_PARAMEJECUTAPROCNOVE no existe o fue borrado');
    END;
  IF nuConta > 0 then
