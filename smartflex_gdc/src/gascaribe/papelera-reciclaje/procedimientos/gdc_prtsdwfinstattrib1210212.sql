set serveroutput on;
PROMPT BORRAR GDC_PRTSDWFINSTATTRIB1210212  
DECLARE
  nuConta       NUMBER;
  sbDueno	    VARCHAR2(30);
...
BEGIN
    BEGIN
    dbms_output.put_Line('***Preparando borrado de Procedure OPEN.GDC_PRTSDWFINSTATTRIB1210212');
        SELECT COUNT(*), OWNER, OBJECT_NAME, OBJECT_TYPE INTO nuConta, sbDueno, sbobject_name, sbobject_type
          FROM dba_objects
         WHERE object_name = 'GDC_PRTSDWFINSTATTRIB1210212'  
           AND OWNER = 'OPEN'
    ...
    EXCEPTION WHEN OTHERS THEN
        nuConta:= 0;
        dbms_output.put_Line('Procedure OPEN.GDC_PRTSDWFINSTATTRIB1210212 no existe o fue borrado');
    END;
    IF nuConta > 0 then
