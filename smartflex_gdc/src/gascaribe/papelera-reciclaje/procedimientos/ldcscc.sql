CREATE OR REPLACE procedure LDCSCC(inuschedule In ge_process_schedule.process_schedule_id%Type) is
  /*****************************************************************
  Propiedad intelectual de Gascaribe-Efigas.

  Unidad         : LDCSCC
  Descripcion    : Método de PB programado LDCSCC
                   proveedor.
  ...
  30/05/2024	  JSOTO	OSF-2602				        Se elimina uso de ldc_boreportecartbrilla.proGenRepSeguimientoACierre
  ******************************************************************/
   ...
   BEGIN
    UT_Trace.Trace( 'Inicia LDCSCC.getparameters', 10);
    ...
    UT_Trace.Trace( 'Fin LDCSCC.getparameters', 10);
    pkerrors.pop;
...
BEGIN
    UT_Trace.Trace( 'Inicia LDCSCC', 10);
    ...
    UT_Trace.Trace( 'Fin LDCSCC', 10);
EXCEPTION
  WHEN ex.controlled_error THEN
    errors.geterror(nuerrorcode, sberrormessage);
    ut_trace.trace('ERROR  LDCSCC[' || nuerrorcode || ']Mensaje[' || sberrormessage || ']');
  WHEN OTHERS THEN
