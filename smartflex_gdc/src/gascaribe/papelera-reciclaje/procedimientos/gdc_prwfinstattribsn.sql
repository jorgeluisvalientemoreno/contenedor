set serveroutput on;
PROMPT BORRAR GDC_PRWFINSTATTRIBSN  
DECLARE
  nuConta       NUMBER;
  sbDueno	    VARCHAR2(30);
...
BEGIN
    BEGIN
    dbms_output.put_Line('***Preparando borrado de Procedure OPEN.GDC_PRWFINSTATTRIBSN');
        SELECT COUNT(*), OWNER, OBJECT_NAME, OBJECT_TYPE INTO nuConta, sbDueno, sbobject_name, sbobject_type
          FROM dba_objects
         WHERE object_name = 'GDC_PRWFINSTATTRIBSN'  
           AND OWNER = 'OPEN'
    ...
    EXCEPTION WHEN OTHERS THEN
        nuConta:= 0;
        dbms_output.put_Line('Procedure OPEN.GDC_PRWFINSTATTRIBSN no existe o fue borrado');
    END;
    IF nuConta > 0 then
