set serveroutput on;
PROMPT BORRAR LDC_PROCCREASOLICITUDNOTIFI
DECLARE
  nuConta       NUMBER;
  sbDueno	    VARCHAR2(30);
...
BEGIN
    BEGIN
    dbms_output.put_Line('Preparando borrado de PROCEDURE OPEN.LDC_PROCCREASOLICITUDNOTIFI');
        SELECT COUNT(*), OWNER, OBJECT_NAME,  OBJECT_TYPE INTO nuConta, sbDueno, sbobject_name, sbobject_type
          FROM dba_objects
         WHERE object_name = 'LDC_PROCCREASOLICITUDNOTIFI'  
           AND OWNER = 'OPEN'
    ...
    EXCEPTION WHEN OTHERS THEN
        nuConta:= 0;
        dbms_output.put_Line('PROCEDURE OPEN.LDC_PROCCREASOLICITUDNOTIFI no existe o fue borrado');
    END;
