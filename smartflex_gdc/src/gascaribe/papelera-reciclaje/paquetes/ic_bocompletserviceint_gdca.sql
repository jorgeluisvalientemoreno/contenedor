set serveroutput on;
PROMPT BORRAR IC_BOCOMPLETSERVICEINT_GDCA
DECLARE
  nuConta       NUMBER;
  sbDueno	    VARCHAR2(30);
...
BEGIN
    BEGIN
    dbms_output.put_Line('Preparando borrado de paquete OPEN.IC_BOCOMPLETSERVICEINT_GDCA');
        SELECT COUNT(*), OWNER, OBJECT_NAME,  OBJECT_TYPE INTO nuConta, sbDueno, sbobject_name, sbobject_type
          FROM dba_objects
         WHERE object_name = 'IC_BOCOMPLETSERVICEINT_GDCA'  
           AND OWNER = 'OPEN'
    ...
    EXCEPTION WHEN OTHERS THEN
        nuConta:= 0;
        dbms_output.put_Line('paquete OPEN.IC_BOCOMPLETSERVICEINT_GDCA no existe o fue borrado');
    END;
