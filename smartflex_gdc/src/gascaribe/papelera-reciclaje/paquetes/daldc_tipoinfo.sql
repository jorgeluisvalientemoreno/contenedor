...
BEGIN
    BEGIN
    dbms_output.put_Line('Preparando borrado de paquete OPEN.DALDC_TIPOINFO');
        SELECT COUNT(*), OWNER, OBJECT_NAME,  OBJECT_TYPE INTO nuConta, sbDueno, sbobject_name, sbobject_type
          FROM dba_objects
         WHERE object_name = 'DALDC_TIPOINFO'  
           AND OWNER = 'OPEN'
    ...
    EXCEPTION WHEN OTHERS THEN
        nuConta:= 0;
        dbms_output.put_Line('paquete OPEN.DALDC_TIPOINFO no existe o fue borrado');
    END;
