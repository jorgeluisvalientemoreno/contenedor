set serveroutput on;
PROMPT BORRAR DALDC_CA_BONO_LIQUIDARECA
DECLARE
  nuConta       NUMBER;
  sbDueno	    VARCHAR2(30);
...
BEGIN
    BEGIN
    dbms_output.put_Line('Preparando borrado de paquete OPEN.DALDC_CA_BONO_LIQUIDARECA');
        SELECT COUNT(*), OWNER, OBJECT_NAME,  OBJECT_TYPE INTO nuConta, sbDueno, sbobject_name, sbobject_type
          FROM dba_objects
         WHERE object_name = 'DALDC_CA_BONO_LIQUIDARECA'  
           AND OWNER = 'OPEN'
    ...
    EXCEPTION WHEN OTHERS THEN
        nuConta:= 0;
        dbms_output.put_Line('paquete OPEN.DALDC_CA_BONO_LIQUIDARECA no existe o fue borrado');
    END;
