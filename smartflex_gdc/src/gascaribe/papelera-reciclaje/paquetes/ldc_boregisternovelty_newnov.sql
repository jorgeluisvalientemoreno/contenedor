set serveroutput on;
PROMPT BORRAR LDC_BOREGISTERNOVELTY_NEWNOV
DECLARE
  nuConta       NUMBER;
  sbDueno	    VARCHAR2(30);
...
BEGIN
    BEGIN
    dbms_output.put_Line('Preparando borrado de paquete OPEN.LDC_BOREGISTERNOVELTY_NEWNOV');
        SELECT COUNT(*), OWNER, OBJECT_NAME,  OBJECT_TYPE INTO nuConta, sbDueno, sbobject_name, sbobject_type
          FROM dba_objects
         WHERE object_name = 'LDC_BOREGISTERNOVELTY_NEWNOV'  
           AND OWNER = 'OPEN'
    ...
    EXCEPTION WHEN OTHERS THEN
        nuConta:= 0;
        dbms_output.put_Line('paquete OPEN.LDC_BOREGISTERNOVELTY_NEWNOV no existe o fue borrado');
    END;
