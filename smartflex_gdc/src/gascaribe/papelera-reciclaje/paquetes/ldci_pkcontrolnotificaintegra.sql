set serveroutput on;
PROMPT BORRAR LDCI_PKCONTROLNOTIFICAINTEGRA  
DECLARE
  nuConta       NUMBER;
  sbDueno	    VARCHAR2(30);
...
BEGIN
    BEGIN
    dbms_output.put_Line('Preparando borrado de paquete OPEN.DALD_BINE_CENCOSUD');
        SELECT COUNT(*), OWNER, OBJECT_NAME,  OBJECT_TYPE INTO nuConta, sbDueno, sbobject_name, sbobject_type
          FROM dba_objects
         WHERE object_name = 'LDCI_PKCONTROLNOTIFICAINTEGRA'  
           AND OWNER = 'OPEN'
    ...
    EXCEPTION WHEN OTHERS THEN
        nuConta:= 0;
        dbms_output.put_Line('paquete OPEN.LDCI_PKCONTROLNOTIFICAINTEGRA no existe o fue borrado');
    END;
