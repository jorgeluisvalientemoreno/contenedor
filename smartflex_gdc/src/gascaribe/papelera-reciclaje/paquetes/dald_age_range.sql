...
BEGIN
    BEGIN
    dbms_output.put_Line('Preparando borrado de paquete OPEN.DALD_AGE_RANGE');
        SELECT COUNT(*), OWNER, OBJECT_NAME,  OBJECT_TYPE INTO nuConta, sbDueno, sbobject_name, sbobject_type
          FROM dba_objects
         WHERE object_name = 'DALD_AGE_RANGE'  
           AND OWNER = 'OPEN'
    ...
    EXCEPTION WHEN OTHERS THEN
        nuConta:= 0;
        dbms_output.put_Line('paquete OPEN.DALD_AGE_RANGE no existe o fue borrado');
    END;
