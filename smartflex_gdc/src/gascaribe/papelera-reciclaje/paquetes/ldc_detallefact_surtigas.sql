set serveroutput on;
PROMPT BORRAR LDC_DETALLEFACT_SURTIGAS  
DECLARE
  nuConta       NUMBER;
  sbDueno	    VARCHAR2(30);
...
BEGIN
    ...
    dbms_output.put_Line('Preparando borrado de paquete OPEN.DALD_BINE_CENCOSUD');
        SELECT COUNT(*), OWNER, OBJECT_NAME,  OBJECT_TYPE INTO nuConta, sbDueno, sbobject_name, sbobject_type
          FROM dba_objects
         WHERE object_name = 'LDC_DETALLEFACT_SURTIGAS'  
           AND OWNER = 'OPEN'
    ...
    EXCEPTION WHEN OTHERS THEN
        nuConta:= 0;
        dbms_output.put_Line('paquete OPEN.LDC_DETALLEFACT_SURTIGAS no existe o fue borrado');
    END;
