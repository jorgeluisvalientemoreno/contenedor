set serveroutput on;
PROMPT BORRAR DALDC_CMMITEMSXTT EN OPEN y SYNONYM PUBLIC EN OPEN
DECLARE
  nuConta       NUMBER;
  sbDueno	    VARCHAR2(30);
...
BEGIN
    BEGIN
    dbms_output.put_Line('Preparando borrado de Sinónimo publico DALDC_CMMITEMSXTT');
      SELECT COUNT(*), OWNER, OBJECT_NAME,  OBJECT_TYPE INTO nuConta, sbDueno, sbobject_name, sbobject_type
      FROM dba_objects
      WHERE object_name = 'DALDC_CMMITEMSXTT'
       AND OWNER = 'PUBLIC'
    ...
    EXCEPTION
        WHEN OTHERS THEN 
        nuConta:= 0;
        dbms_output.put_Line('Sinónimo publico DALDC_CMMITEMSXTT no existe o fue borrado');
    END; 
    ...
    BEGIN
    dbms_output.put_Line('Preparando borrado de paquete OPEN.DALDC_CMMITEMSXTT');
        SELECT COUNT(*), OWNER, OBJECT_NAME,  OBJECT_TYPE INTO nuConta, sbDueno, sbobject_name, sbobject_type
          FROM dba_objects
         WHERE object_name = 'DALDC_CMMITEMSXTT'  
           AND OWNER = 'OPEN'
    ...
    EXCEPTION WHEN OTHERS THEN
        nuConta:= 0;
        dbms_output.put_Line('paquete OPEN.DALDC_CMMITEMSXTT no existe o fue borrado');
    END;
