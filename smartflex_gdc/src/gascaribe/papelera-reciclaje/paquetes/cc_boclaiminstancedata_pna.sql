set serveroutput on;
PROMPT BORRAR CC_BOCLAIMINSTANCEDATA_PNA
DECLARE
  nuConta       NUMBER;
  sbDueno	    VARCHAR2(30);
...
BEGIN
    BEGIN
    dbms_output.put_Line('Preparando borrado de paquete OPEN.CC_BOCLAIMINSTANCEDATA_PNA');
        SELECT COUNT(*), OWNER, OBJECT_NAME,  OBJECT_TYPE INTO nuConta, sbDueno, sbobject_name, sbobject_type
          FROM dba_objects
         WHERE object_name = 'CC_BOCLAIMINSTANCEDATA_PNA'  
           AND OWNER = 'OPEN'
    ...
    EXCEPTION WHEN OTHERS THEN
        nuConta:= 0;
        dbms_output.put_Line('paquete OPEN.CC_BOCLAIMINSTANCEDATA_PNA no existe o fue borrado');
    END;
