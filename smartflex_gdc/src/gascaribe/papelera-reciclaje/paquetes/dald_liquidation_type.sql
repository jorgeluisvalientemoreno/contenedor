set serveroutput on;
PROMPT BORRAR DALD_LIQUIDATION_TYPE
DECLARE
  nuConta       NUMBER;
  sbDueno	    VARCHAR2(30);
...
BEGIN
    BEGIN
    dbms_output.put_Line('Preparando borrado de procedimiento OPEN.DALD_LIQUIDATION_TYPE');
        SELECT COUNT(*), OWNER, OBJECT_NAME, OBJECT_TYPE INTO nuConta, sbDueno, sbobject_name, sbobject_type
          FROM dba_objects
         WHERE object_name = 'DALD_LIQUIDATION_TYPE'  
           AND OWNER = 'OPEN'
    ...
    EXCEPTION WHEN OTHERS THEN
        nuConta:= 0;
        dbms_output.put_Line('procedimiento OPEN.DALD_LIQUIDATION_TYPE no existe o fue borrado');
    END;
