set serveroutput on;
PROMPT BORRAR LDC_VISTA_USUARIOS_NOTIFICAR
DECLARE
  nuConta       NUMBER;
  sbDueno	    VARCHAR2(30);
...
BEGIN
    BEGIN
    dbms_output.put_Line('Preparando borrado de VIEW OPEN.LDC_VISTA_USUARIOS_NOTIFICAR');
        SELECT COUNT(*), OWNER, OBJECT_NAME,  OBJECT_TYPE INTO nuConta, sbDueno, sbobject_name, sbobject_type
          FROM dba_objects
         WHERE object_name = 'LDC_VISTA_USUARIOS_NOTIFICAR'  
           AND OWNER = 'OPEN'
    ...
    EXCEPTION WHEN OTHERS THEN
        nuConta:= 0;
        dbms_output.put_Line('VIEW OPEN.LDC_VISTA_USUARIOS_NOTIFICAR no existe o fue borrado');
    END;
