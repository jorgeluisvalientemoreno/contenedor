set serveroutput on;
PROMPT BORRAR JOB LDC_PROANALISUSPENSION2  
DECLARE
  nuConta       NUMBER;
  sbDueno	    VARCHAR2(30);
...
BEGIN
    BEGIN
         ...
         WHERE job_name = 'LDC_PROANALISUSPENSION2'  
    ...
    END;
  IF nuconta > 0 THEN
    dbms_scheduler.drop_job(job_name => 'LDC_PROANALISUSPENSION2');
    dbms_output.put_Line('Job llamado LDC_PROANALISUSPENSION2 borrado con exito');
  COMMIT;
  END IF;
EXCEPTION
    WHEN OTHERS THEN 
