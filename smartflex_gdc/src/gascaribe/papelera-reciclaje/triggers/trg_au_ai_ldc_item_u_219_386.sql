CREATE OR REPLACE TRIGGER   TRG_AU_AI_LDC_ITEM_U_219_386 AFTER INSERT OR UPDATE OR DELETE ON  LDC_ITEM_UO_LR
  REFERENCING   OLD   AS   OLD   NEW   AS   NEW  
 FOR EACH ROW 
 DECLARE 
AUDIT_POLICY_IDvar AU_AUDIT_POLICY_LOG.AUDIT_POLICY_ID%type := 219;
AUDIT_LOG_IDvar AU_LOG_POLICY.AUDIT_LOG_ID%type := 386;
AUDIT_POLICY_LOG_IDvar AU_AUDIT_POLICY_LOG.AUDIT_POLICY_LOG_ID%type :=null;
CURRENT_TABLE_NAMEvar AU_AUDIT_POLICY_LOG.CURRENT_TABLE_NAME%type := 'LDC_ITEM_UO_LR';
...
BEGIN 
    ...
    -- checks if user the current process is audited BY the Audit Policy
      ...
      tbFIELD(1).FieldName := 'UNIDAD_OPERATIVA';
      tbFIELD(1).OldValue := to_char(:OLD.UNIDAD_OPERATIVA);
      tbFIELD(1).NewValue := to_char(:NEW.UNIDAD_OPERATIVA);
      tbFIELD(1).IsMonitored := 1;
      tbFIELD(1).AttrTypeId := 1;
      -----------------------
      tbFIELD(2).FieldName := 'ACTIVIDAD';
      tbFIELD(2).OldValue := to_char(:OLD.ACTIVIDAD);
      tbFIELD(2).NewValue := to_char(:NEW.ACTIVIDAD);
      tbFIELD(2).IsMonitored := 1;
      tbFIELD(2).AttrTypeId := 1;
      -----------------------
      tbFIELD(3).FieldName := 'ITEM';
      tbFIELD(3).OldValue := to_char(:OLD.ITEM);
      tbFIELD(3).NewValue := to_char(:NEW.ITEM);
      tbFIELD(3).IsMonitored := 1;
      tbFIELD(3).AttrTypeId := 1;
      -----------------------
      tbFIELD(4).FieldName := 'LIQUIDACION';
      tbFIELD(4).OldValue := :OLD.LIQUIDACION;
      tbFIELD(4).NewValue := :NEW.LIQUIDACION;
