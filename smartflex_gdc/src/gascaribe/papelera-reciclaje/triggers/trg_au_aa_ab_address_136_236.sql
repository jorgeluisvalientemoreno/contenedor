CREATE OR REPLACE TRIGGER OPEN.TRG_AU_AA_AB_ADDRESS_136_236 AFTER UPDATE ON OPEN.AB_ADDRESS
  REFERENCING   OLD   AS   OLD   NEW   AS   NEW  
 FOR EACH ROW
DECLARE 
AUDIT_POLICY_IDvar AU_AUDIT_POLICY_LOG.AUDIT_POLICY_ID%type := 136;
AUDIT_LOG_IDvar AU_LOG_POLICY.AUDIT_LOG_ID%type := 236;
...
BEGIN 
    ...
    -- checks if user the current process is audited BY the Audit Policy
      ...
      tbFIELD(1).FieldName := 'GEOGRAP_LOCATION_ID';
      tbFIELD(1).OldValue := to_char(:OLD.GEOGRAP_LOCATION_ID);
      tbFIELD(1).NewValue := to_char(:NEW.GEOGRAP_LOCATION_ID);
