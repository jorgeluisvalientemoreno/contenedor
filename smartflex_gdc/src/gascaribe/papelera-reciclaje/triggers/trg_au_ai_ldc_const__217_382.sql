CREATE OR REPLACE TRIGGER   TRG_AU_AI_LDC_CONST__217_382 AFTER INSERT OR UPDATE OR DELETE ON  LDC_CONST_UNOPRL
  REFERENCING   OLD   AS   OLD   NEW   AS   NEW  
 FOR EACH ROW 
 DECLARE 
AUDIT_POLICY_IDvar AU_AUDIT_POLICY_LOG.AUDIT_POLICY_ID%type := 217;
AUDIT_LOG_IDvar AU_LOG_POLICY.AUDIT_LOG_ID%type := 382;
AUDIT_POLICY_LOG_IDvar AU_AUDIT_POLICY_LOG.AUDIT_POLICY_LOG_ID%type :=null;
CURRENT_TABLE_NAMEvar AU_AUDIT_POLICY_LOG.CURRENT_TABLE_NAME%type := 'LDC_CONST_UNOPRL';
...
BEGIN 
    ...
    -- checks if user the current process is audited BY the Audit Policy
      ...
      -----------------------
      tbFIELD(2).FieldName := 'TIPO_OFERTADO';
      tbFIELD(2).OldValue := to_char(:OLD.TIPO_OFERTADO);
      tbFIELD(2).NewValue := to_char(:NEW.TIPO_OFERTADO);
      tbFIELD(2).IsMonitored := 1;
      tbFIELD(2).AttrTypeId := 1;
      -----------------------
      tbFIELD(3).FieldName := 'DIRECCION_GEN_NOV';
      tbFIELD(3).OldValue := to_char(:OLD.DIRECCION_GEN_NOV);
      tbFIELD(3).NewValue := to_char(:NEW.DIRECCION_GEN_NOV);
