CREATE OR REPLACE TRIGGER   TRG_AU_AI_LDC_CMMITE_191_336 AFTER INSERT OR UPDATE OR DELETE ON  LDC_CMMITEMSXTT
  REFERENCING   OLD   AS   OLD   NEW   AS   NEW  
 FOR EACH ROW 
 DECLARE 
AUDIT_POLICY_IDvar AU_AUDIT_POLICY_LOG.AUDIT_POLICY_ID%type := 191;
AUDIT_LOG_IDvar AU_LOG_POLICY.AUDIT_LOG_ID%type := 336;
AUDIT_POLICY_LOG_IDvar AU_AUDIT_POLICY_LOG.AUDIT_POLICY_LOG_ID%type :=null;
CURRENT_TABLE_NAMEvar AU_AUDIT_POLICY_LOG.CURRENT_TABLE_NAME%type := 'LDC_CMMITEMSXTT';
...
BEGIN 
    ...
    -- checks if user the current process is audited BY the Audit Policy
      ...
      tbFIELD(1).FieldName := 'ITEMSXTT_ID';
      tbFIELD(1).OldValue := to_char(:OLD.ITEMSXTT_ID);
      tbFIELD(1).NewValue := to_char(:NEW.ITEMSXTT_ID);
      tbFIELD(1).IsMonitored := 1;
      tbFIELD(1).AttrTypeId := 1;
      -----------------------
      tbFIELD(2).FieldName := 'TASK_TYPE_ID';
      tbFIELD(2).OldValue := to_char(:OLD.TASK_TYPE_ID);
      tbFIELD(2).NewValue := to_char(:NEW.TASK_TYPE_ID);
      tbFIELD(2).IsMonitored := 1;
      tbFIELD(2).AttrTypeId := 1;
      -----------------------
      tbFIELD(3).FieldName := 'ITEMS_ID';
      tbFIELD(3).OldValue := to_char(:OLD.ITEMS_ID);
      tbFIELD(3).NewValue := to_char(:NEW.ITEMS_ID);
      tbFIELD(3).IsMonitored := 1;
      tbFIELD(3).AttrTypeId := 1;
      -----------------------
      tbFIELD(4).FieldName := 'ACTIVITY_ID';
      tbFIELD(4).OldValue := to_char(:OLD.ACTIVITY_ID);
      tbFIELD(4).NewValue := to_char(:NEW.ACTIVITY_ID);
      tbFIELD(4).IsMonitored := 1;
      tbFIELD(4).AttrTypeId := 1;
      -----------------------
      tbFIELD(5).FieldName := 'ITEM_AMOUNT_MIN';
      tbFIELD(5).OldValue := to_char(:OLD.ITEM_AMOUNT_MIN);
      tbFIELD(5).NewValue := to_char(:NEW.ITEM_AMOUNT_MIN);
      tbFIELD(5).IsMonitored := 1;
      tbFIELD(5).AttrTypeId := 1;
      -----------------------
      tbFIELD(6).FieldName := 'ITEM_AMOUNT_MAX';
      tbFIELD(6).OldValue := to_char(:OLD.ITEM_AMOUNT_MAX);
      tbFIELD(6).NewValue := to_char(:NEW.ITEM_AMOUNT_MAX);
