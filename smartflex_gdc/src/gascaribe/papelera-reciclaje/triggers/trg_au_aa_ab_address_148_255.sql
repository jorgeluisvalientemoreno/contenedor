CREATE OR REPLACE TRIGGER OPEN.TRG_AU_AA_AB_ADDRESS_148_255 AFTER UPDATE ON OPEN.AB_ADDRESS
  REFERENCING   OLD   AS   OLD   NEW   AS   NEW  
 FOR EACH ROW
DECLARE 
AUDIT_POLICY_IDvar AU_AUDIT_POLICY_LOG.AUDIT_POLICY_ID%type := 148;
AUDIT_LOG_IDvar AU_LOG_POLICY.AUDIT_LOG_ID%type := 255;
...
BEGIN 
    ...
    -- checks if user the current process is audited BY the Audit Policy
      ...
      -----------------------
      tbFIELD(2).FieldName := 'GEOGRAP_LOCATION_ID';
      tbFIELD(2).OldValue := to_char(:OLD.GEOGRAP_LOCATION_ID);
      tbFIELD(2).NewValue := to_char(:NEW.GEOGRAP_LOCATION_ID);
      tbFIELD(2).IsMonitored := 1;
      tbFIELD(2).AttrTypeId := 1;
      -----------------------
      tbFIELD(3).FieldName := 'NEIGHBORTHOOD_ID';
      tbFIELD(3).OldValue := to_char(:OLD.NEIGHBORTHOOD_ID);
      tbFIELD(3).NewValue := to_char(:NEW.NEIGHBORTHOOD_ID);
