CREATE OR REPLACE TRIGGER   TRG_AU_AI_GE_PERSON_225_396 AFTER INSERT OR UPDATE OR DELETE ON  GE_PERSON
  REFERENCING   OLD   AS   OLD   NEW   AS   NEW  
 FOR EACH ROW 
 DECLARE 
AUDIT_POLICY_IDvar AU_AUDIT_POLICY_LOG.AUDIT_POLICY_ID%type := 225;
AUDIT_LOG_IDvar AU_LOG_POLICY.AUDIT_LOG_ID%type := 396;
AUDIT_POLICY_LOG_IDvar AU_AUDIT_POLICY_LOG.AUDIT_POLICY_LOG_ID%type :=null;
CURRENT_TABLE_NAMEvar AU_AUDIT_POLICY_LOG.CURRENT_TABLE_NAME%type := 'GE_PERSON';
...
BEGIN 
    ...
    -- checks if user the current process is audited BY the Audit Policy
      ...
      tbFIELD(1).FieldName := 'PERSON_ID';
      tbFIELD(1).OldValue := to_char(:OLD.PERSON_ID);
      tbFIELD(1).NewValue := to_char(:NEW.PERSON_ID);
      tbFIELD(1).IsMonitored := 1;
      tbFIELD(1).AttrTypeId := 1;
      -----------------------
      tbFIELD(2).FieldName := 'NAME_';
      tbFIELD(2).OldValue := :OLD.NAME_;
      tbFIELD(2).NewValue := :NEW.NAME_;
      tbFIELD(2).IsMonitored := 1;
      tbFIELD(2).AttrTypeId := 2;
      -----------------------
      tbFIELD(3).FieldName := 'NUMBER_ID';
      tbFIELD(3).OldValue := :OLD.NUMBER_ID;
      tbFIELD(3).NewValue := :NEW.NUMBER_ID;
      tbFIELD(3).IsMonitored := 1;
      tbFIELD(3).AttrTypeId := 2;
      -----------------------
      tbFIELD(4).FieldName := 'IDENT_TYPE_ID';
      tbFIELD(4).OldValue := to_char(:OLD.IDENT_TYPE_ID);
      tbFIELD(4).NewValue := to_char(:NEW.IDENT_TYPE_ID);
