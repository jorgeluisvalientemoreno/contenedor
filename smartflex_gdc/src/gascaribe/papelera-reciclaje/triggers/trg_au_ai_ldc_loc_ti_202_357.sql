CREATE OR REPLACE TRIGGER   TRG_AU_AI_LDC_LOC_TI_202_357 AFTER INSERT OR UPDATE OR DELETE ON  LDC_LOC_TIPO_LIST_DEP
  REFERENCING   OLD   AS   OLD   NEW   AS   NEW  
 FOR EACH ROW 
 DECLARE 
AUDIT_POLICY_IDvar AU_AUDIT_POLICY_LOG.AUDIT_POLICY_ID%type := 202;
AUDIT_LOG_IDvar AU_LOG_POLICY.AUDIT_LOG_ID%type := 357;
AUDIT_POLICY_LOG_IDvar AU_AUDIT_POLICY_LOG.AUDIT_POLICY_LOG_ID%type :=null;
CURRENT_TABLE_NAMEvar AU_AUDIT_POLICY_LOG.CURRENT_TABLE_NAME%type := 'LDC_LOC_TIPO_LIST_DEP';
...
BEGIN 
    ...
    -- checks if user the current process is audited BY the Audit Policy
      ...
      tbFIELD(1).FieldName := 'TIPO_LISTA';
      tbFIELD(1).OldValue := to_char(:OLD.TIPO_LISTA);
      tbFIELD(1).NewValue := to_char(:NEW.TIPO_LISTA);
      tbFIELD(1).IsMonitored := 1;
      tbFIELD(1).AttrTypeId := 1;
      -----------------------
      tbFIELD(2).FieldName := 'DEPARTAMENTO';
      tbFIELD(2).OldValue := to_char(:OLD.DEPARTAMENTO);
      tbFIELD(2).NewValue := to_char(:NEW.DEPARTAMENTO);
      tbFIELD(2).IsMonitored := 1;
      tbFIELD(2).AttrTypeId := 1;
      -----------------------
      tbFIELD(3).FieldName := 'LOCALIDAD';
      tbFIELD(3).OldValue := to_char(:OLD.LOCALIDAD);
      tbFIELD(3).NewValue := to_char(:NEW.LOCALIDAD);
