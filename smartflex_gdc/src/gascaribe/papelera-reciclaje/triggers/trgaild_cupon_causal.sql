DECLARE
  ...
  csbObjeto  CONSTANT VARCHAR2(70) := UPPER('TRGAILD_CUPON_CAUSAL');
...
begin
  ...
  IF nuCantObj = 1 THEN
    ...
    dbms_output.put_line('Se hizo drop al trigger ' || csbEsquema || '.' ||
                         csbObjeto);
  
  ELSE
  
    dbms_output.put_line('No existe el trigger ' || csbEsquema || '.' ||
                         csbObjeto);
