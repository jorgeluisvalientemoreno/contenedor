CREATE OR REPLACE TRIGGER   TRG_AU_AI_LD_PARAMET_233_405 AFTER INSERT OR UPDATE OR DELETE ON  LD_PARAMETER
  REFERENCING   OLD   AS   OLD   NEW   AS   NEW  
 FOR EACH ROW 
 DECLARE 
AUDIT_POLICY_IDvar AU_AUDIT_POLICY_LOG.AUDIT_POLICY_ID%type := 233;
AUDIT_LOG_IDvar AU_LOG_POLICY.AUDIT_LOG_ID%type := 405;
AUDIT_POLICY_LOG_IDvar AU_AUDIT_POLICY_LOG.AUDIT_POLICY_LOG_ID%type :=null;
CURRENT_TABLE_NAMEvar AU_AUDIT_POLICY_LOG.CURRENT_TABLE_NAME%type := 'LD_PARAMETER';
...
BEGIN 
    ...
    -- checks if user the current process is audited BY the Audit Policy
      ...
      tbFIELD(1).FieldName := 'PARAMETER_ID';
      tbFIELD(1).OldValue := :OLD.PARAMETER_ID;
      tbFIELD(1).NewValue := :NEW.PARAMETER_ID;
      tbFIELD(1).IsMonitored := 1;
      tbFIELD(1).AttrTypeId := 2;
      -----------------------
      tbFIELD(2).FieldName := 'NUMERIC_VALUE';
      tbFIELD(2).OldValue := to_char(:OLD.NUMERIC_VALUE);
      tbFIELD(2).NewValue := to_char(:NEW.NUMERIC_VALUE);
      tbFIELD(2).IsMonitored := 1;
      tbFIELD(2).AttrTypeId := 1;
      -----------------------
      tbFIELD(3).FieldName := 'VALUE_CHAIN';
      tbFIELD(3).OldValue := :OLD.VALUE_CHAIN;
      tbFIELD(3).NewValue := :NEW.VALUE_CHAIN;
      tbFIELD(3).IsMonitored := 1;
      tbFIELD(3).AttrTypeId := 2;
      -----------------------
      tbFIELD(4).FieldName := 'DESCRIPTION';
      tbFIELD(4).OldValue := :OLD.DESCRIPTION;
      tbFIELD(4).NewValue := :NEW.DESCRIPTION;
      tbFIELD(4).IsMonitored := 1;
      tbFIELD(4).AttrTypeId := 2;
      -----------------------
