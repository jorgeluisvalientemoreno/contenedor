CREATE OR REPLACE TRIGGER   TRG_AU_AI_LDC_ACT_FA_215_378 AFTER INSERT OR UPDATE OR DELETE ON  LDC_ACT_FATHER_ACT_HIJA
  REFERENCING   OLD   AS   OLD   NEW   AS   NEW  
 FOR EACH ROW 
 DECLARE 
AUDIT_POLICY_IDvar AU_AUDIT_POLICY_LOG.AUDIT_POLICY_ID%type := 215;
AUDIT_LOG_IDvar AU_LOG_POLICY.AUDIT_LOG_ID%type := 378;
AUDIT_POLICY_LOG_IDvar AU_AUDIT_POLICY_LOG.AUDIT_POLICY_LOG_ID%type :=null;
CURRENT_TABLE_NAMEvar AU_AUDIT_POLICY_LOG.CURRENT_TABLE_NAME%type := 'LDC_ACT_FATHER_ACT_HIJA';
...
BEGIN 
    ...
    -- checks if user the current process is audited BY the Audit Policy
      ...
      tbFIELD(1).FieldName := 'ACTIVIDAD_PADRE';
      tbFIELD(1).OldValue := to_char(:OLD.ACTIVIDAD_PADRE);
      tbFIELD(1).NewValue := to_char(:NEW.ACTIVIDAD_PADRE);
      tbFIELD(1).IsMonitored := 1;
      tbFIELD(1).AttrTypeId := 1;
      -----------------------
      tbFIELD(2).FieldName := 'ACTIVIDAD_HIJA';
      tbFIELD(2).OldValue := to_char(:OLD.ACTIVIDAD_HIJA);
      tbFIELD(2).NewValue := to_char(:NEW.ACTIVIDAD_HIJA);
