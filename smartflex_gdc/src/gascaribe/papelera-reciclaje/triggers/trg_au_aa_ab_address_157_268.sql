CREATE OR REPLACE TRIGGER OPEN.TRG_AU_AA_AB_ADDRESS_157_268 AFTER UPDATE ON OPEN.AB_ADDRESS
  REFERENCING   OLD   AS   OLD   NEW   AS   NEW  
 FOR EACH ROW
DECLARE 
AUDIT_POLICY_IDvar AU_AUDIT_POLICY_LOG.AUDIT_POLICY_ID%type := 157;
AUDIT_LOG_IDvar AU_LOG_POLICY.AUDIT_LOG_ID%type := 268;
AUDIT_POLICY_LOG_IDvar AU_AUDIT_POLICY_LOG.AUDIT_POLICY_LOG_ID%type :=null;
CURRENT_TABLE_NAMEvar AU_AUDIT_POLICY_LOG.CURRENT_TABLE_NAME%type := 'AB_ADDRESS';
CURRENT_EVEN_DESCvar AU_AUDIT_POLICY_LOG.CURRENT_EVEN_DESC%type := 'UPDATE';
...
BEGIN 
    ...
    -- checks if user the current process is audited BY the Audit Policy
      ...
      tbFIELD(1).FieldName := 'SEGMENT_ID';
      tbFIELD(1).OldValue := to_char(:OLD.SEGMENT_ID);
      tbFIELD(1).NewValue := to_char(:NEW.SEGMENT_ID);
      tbFIELD(1).IsMonitored := 1;
      tbFIELD(1).AttrTypeId := 1;
      -----------------------
      tbFIELD(2).FieldName := 'NEIGHBORTHOOD_ID';
      tbFIELD(2).OldValue := to_char(:OLD.NEIGHBORTHOOD_ID);
      tbFIELD(2).NewValue := to_char(:NEW.NEIGHBORTHOOD_ID);
      tbFIELD(2).IsMonitored := 1;
      tbFIELD(2).AttrTypeId := 1;
      -----------------------
      tbFIELD(3).FieldName := 'GEOGRAP_LOCATION_ID';
      tbFIELD(3).OldValue := to_char(:OLD.GEOGRAP_LOCATION_ID);
      tbFIELD(3).NewValue := to_char(:NEW.GEOGRAP_LOCATION_ID);
      tbFIELD(3).IsMonitored := 1;
      tbFIELD(3).AttrTypeId := 1;
      -----------------------
      tbFIELD(4).FieldName := 'ESTATE_NUMBER';
      tbFIELD(4).OldValue := to_char(:OLD.ESTATE_NUMBER);
      tbFIELD(4).NewValue := to_char(:NEW.ESTATE_NUMBER);
