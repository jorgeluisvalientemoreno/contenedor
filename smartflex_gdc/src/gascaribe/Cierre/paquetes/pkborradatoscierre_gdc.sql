set serveroutput on;
PROMPT BORRAR PKBORRADATOSCIERRE_GDC
DECLARE
  nuConta       NUMBER;
  sbDueno	    VARCHAR2(30);
...
BEGIN
    BEGIN
    dbms_output.put_Line('Preparando borrado de paquete OPEN.PKBORRADATOSCIERRE_GDC');
        SELECT COUNT(*), OWNER, OBJECT_NAME,  OBJECT_TYPE INTO nuConta, sbDueno, sbobject_name, sbobject_type
          FROM dba_objects
         WHERE object_name = 'PKBORRADATOSCIERRE_GDC'  
           AND OWNER = 'OPEN'
    ...
    EXCEPTION WHEN OTHERS THEN
        nuConta:= 0;
        dbms_output.put_Line('paquete OPEN.PKBORRADATOSCIERRE_GDC no existe o fue borrado');
    END;
