CREATE OR REPLACE PROCEDURE ldc_actconcceroultplafi9(
                                              nupano  NUMBER,
                                              nupmes  NUMBER,
 ...
 LOOP
  ...
  --EXIT WHEN cuservcus%NOTFOUND;
   ...
   FORALL i IN 1 .. v_array_cursor.count
      UPDATE ldc_osf_sesucier_tmp9 y
...
EXCEPTION
 WHEN OTHERS THEN
  ROLLBACK;
  sbmensa := 'Error en ldc_actconcceroultplafi9 lineas error '||nuerror||' '||sqlerrm;
  error := -1;
END;
/
