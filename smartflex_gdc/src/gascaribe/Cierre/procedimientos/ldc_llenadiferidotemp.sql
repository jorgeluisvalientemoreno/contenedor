CREATE OR REPLACE PROCEDURE      ldc_llenadiferidotemp(
/**************************************************************************
...
insert into LDCHILOSLLENADIFE (hilo_,inicia,termina,observacion,estado,REGPROCS) values(10,sysdate,null,'CREAD0','C',0);
...
FOR I IN 1..10 LOOP
  ...
  --  xlogpno('LLENADIFERIDO: VA A CORRER EL SIGUIENTE JOB '||'LDC_LLENADIFERIDOHILO('||    TO_CHAR(I)||','|| TO_CHAR(NUPANO) ||','||TO_CHAR(NUPMES) ||');');
    DBMS_JOB.SUBMIT ( nujob,'LDC_LLENADIFERIDO_H'||TO_CHAR(I)||'(' || TO_CHAR(NUPANO) ||','||TO_CHAR(NUPMES) ||');', DTFECHA);
...
--  nucantiregtot := nucantiregtot + nucantiregcom;
  ...
  --se carga informacion a la tabla
  ldc_proinsertaestaprog(nuPano,nuPmes,'LDC_PROCCARGDATDIFERIDO','En ejecucion..',nusession,sbuser);
  FOR I IN 1..10 LOOP
    dtFecha := sysdate;
  --  xlogpno('LLENADIFERIDO: VA A CORRER EL SIGUIENTE JOB '||'LDC_LLENADIFERIDOHILO('||    TO_CHAR(I)||','|| TO_CHAR(NUPANO) ||','||TO_CHAR(NUPMES) ||');');
    DBMS_JOB.SUBMIT ( nujob,'DECLARE sbmensa VARCHAR2(4000);'||
                   ' Error NUMBER; '||
                   ' BEGIN LDC_PROCCARGDATDIFERIDO(''LDC_OSF_DIFERIDO'||TO_CHAR(I)||''',Error, sbmensa);', DTFECHA);
  ...
  WHILE SW LOOP
     -- SELECT COUNT(1) INTO NUJOB FROM dba_ WHERE ESTADO IN ('T','E');
      ...
      from DBA_JOBS_RUNNING, tmphilocargdif
      where job = nujob;
      IF NUJOB=0 THEN
       SW:=FALSE;
      END IF;
  END LOOP;
   ldc_proactualizaestaprog(nusession,nvl(sbmensa,'ok'),'LDC_PROCCARGDATDIFERIDO','Termino '||error);
  COMMIT;
--END IF;
EXCEPTION
 WHEN OTHERS THEN
  ...
  ldc_proactualizaestaprog(nusession,nvl(sbmensa,'error'),'LDC_LLENADIFERIDO','Termino '||error);
END;
/
