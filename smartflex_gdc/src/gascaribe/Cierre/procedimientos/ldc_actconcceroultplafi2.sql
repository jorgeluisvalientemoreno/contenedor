CREATE OR REPLACE PROCEDURE      ldc_actconcceroultplafi2(
                                              nupano  NUMBER,
                                              nupmes  NUMBER,
...
BEGIN
 ...
 -- EXIT WHEN cuservcus%NOTFOUND;
   ...
   FORALL i IN 1 .. v_array_cursor.count
      UPDATE ldc_osf_sesucier_tmp2 y
...
EXCEPTION
 WHEN OTHERS THEN
  ROLLBACK;
  sbmensa := 'Error en ldc_actconcceroultplafi2 lineas error '||nuerror||' '||sqlerrm;
  error := -1;
END;
/
