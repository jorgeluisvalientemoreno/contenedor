DECLARE
    ...
    CURSOR cuDBA_SCHEDULER_JOBS IS
        SELECT job_name
         ...
         WHERE JOB_NAME LIKE 'JOB_CREATABLAIC_CARTCOCO';
BEGIN
    ...
    END IF;
    sbNombreJob := 'JOB_CREATABLAIC_CARTCOCO';
    ...
    BEGIN
        DBMS_SCHEDULER.Create_Job (
            ...
            Job_Action        => 'BEGIN
                                    EXECUTE IMMEDIATE ''Create table OPEN.IC_CARTCOCO_NEW
                                                        tablespace TSD_HIST
                                                         AS (
                                                              select * from open.IC_CARTCOCO 
                                                              where CACCFEGE >= TO_DATE(''''01/12/2022'''')
                                                         )'';
                                                         END;',
            Start_Date        => systimestamp,
            End_Date          => NULL,
            Comments          => 'Crea tabla para Recorte IC_CARTCOCO',
            Enabled           => TRUE,
            Auto_Drop         => FALSE);
    END;
