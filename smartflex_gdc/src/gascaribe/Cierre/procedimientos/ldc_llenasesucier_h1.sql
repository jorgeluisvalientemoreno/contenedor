CREATE OR REPLACE PROCEDURE ldc_llenasesucier_h1(
/**************************************************************************
  Fecha       : 2015-09-25
  Descripcion : Generamos informaci?n del producto a cierre nuevo esquema Hilo1
...
WHERE s.sesususc              = u.susccodi
   ...
   AND mod(sesunuse,dald_parameter.fnuGetNumeric_Value('CIERRE_HILOS'))+1= 1;
...
BEGIN
 ...
 LOOP
  ...
  -- Registramos la informaci?n del producto a cierre
        nuerror := -15;
        INSERT INTO ldc_osf_sesucier_tmp
         (
           tipo_producto
          ,nuano
          ,numes
...
WHEN err_fecha_per_conta THEN
  ROLLBACK;
  sbmensa := 'Error en ldc_llenasesucier_h1 lineas error '||nuerror||' recuperando fecha periodo contable. '||sbmensa;
  error := -20;
 WHEN OTHERS THEN
  ROLLBACK;
  sbmensa := 'Error en ldc_llenasesucier_h1 lineas error '||nuerror||' producto '||nunuse||' '||sqlerrm;
  error := -21;
END;
/
