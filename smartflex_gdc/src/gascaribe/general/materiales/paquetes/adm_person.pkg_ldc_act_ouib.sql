CREATE OR REPLACE PACKAGE adm_person.pkg_ldc_act_ouib AS
    TYPE tytbRowIds IS TABLE OF ROWID INDEX BY BINARY_INTEGER;
    TYPE tytbRegistros IS TABLE OF ldc_act_ouib%ROWTYPE INDEX BY BINARY_INTEGER;
    CURSOR culdc_act_ouib IS SELECT * FROM ldc_act_ouib;
    /***************************************************************************
    ...
    ***************************************************************************/
    ...
    -- Inserta un registro
    PROCEDURE prinsRegistro( ircRegistro culdc_act_ouib%ROWTYPE);
    ...
    FUNCTION fnuObtCostoTotal(
        inuItem                 IN  ldc_act_ouib.items_id%TYPE,
        inunuUnidadOperativa    IN  ldc_act_ouib.operating_unit_id%TYPE
    )
    RETURN ldc_act_ouib.total_costs%TYPE;
    ...
    RETURN ldc_act_ouib.quota%TYPE;    

    -- Obtiene la cantidad disponible a la unidad del item de tipo activo
    FUNCTION fnuObtCantidadDisponible(
        inuItem                 IN  ldc_act_ouib.items_id%TYPE,
        inunuUnidadOperativa    IN  ldc_act_ouib.operating_unit_id%TYPE
    )
    RETURN ldc_act_ouib.balance%TYPE;    
 
END pkg_ldc_act_ouib;
/

CREATE OR REPLACE PACKAGE BODY adm_person.pkg_ldc_act_ouib AS
    ...
    BEGIN
        ...
        IF NOT fblExiste(inuITEMS_ID,inuOPERATING_UNIT_ID) THEN
            pkg_error.setErrorMessage( isbMsgErrr =>'No existe el registro ['||inuITEMS_ID||','||inuOPERATING_UNIT_ID||'] en la tabla[ldc_act_ouib]');
        END IF;
        pkg_traza.trace(csbMetodo, csbNivelTraza, pkg_traza.csbFIN);
        EXCEPTION
            WHEN pkg_error.Controlled_Error THEN
    ...
    -- Inserta un registro
    PROCEDURE prInsRegistro( ircRegistro culdc_act_ouib%ROWTYPE) IS
    ...
    IS
        ...
        nuError         NUMBER;
        sbError         VARCHAR2(4000);
        nuCostoTotal    ldc_act_ouib.total_costs%TYPE;
    ...
    IS
        ...
        nuError         NUMBER;
        sbError         VARCHAR2(4000);
        nuCantidadAsignada    ldc_act_ouib.quota%TYPE;
    ...
    -- Obtiene la cantidad disponible del item de tipo activo
    FUNCTION fnuObtCantidadDisponible(
        inuItem                 IN  ldc_act_ouib.items_id%TYPE,
        inunuUnidadOperativa    IN  ldc_act_ouib.operating_unit_id%TYPE
    )
    ...
    IS
        ...
        nuError         NUMBER;
        sbError         VARCHAR2(4000);
        nuCantidadDisponible    ldc_act_ouib.balance%TYPE;
...
BEGIN
    -- OSF-3976
    pkg_Utilidades.prAplicarPermisos( UPPER('pkg_ldc_act_ouib'), UPPER('adm_person'));
END;
/
