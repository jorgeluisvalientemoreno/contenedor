prompt "Creando la tabla open.ldci_oficvent"
declare

    -- OSF-XXXX
    sbSentencia     VARCHAR2(32767);

    sbTabla         VARCHAR2(30)    := UPPER('ldci_oficvent' ); 
...
begin
    ...
    IF rcDBA_TABLES.TABLE_NAME IS NULL THEN
        ...
        sbSentencia := sbSentencia || 'OFVECODI  VARCHAR2(4),'|| CHR(10);
        sbSentencia := sbSentencia || 'OFVEDESC  VARCHAR2(60))'|| CHR(10);       
        ...
        pCreaComenColumna('OFVECODI','Codigo de la oficina');

        pCreaComenColumna('OFVEDESC','Descripcion');
...
END;
/
prompt "Creando llave primaria para la tabla open.ldci_oficvent"
DECLARE
	...
	sbTableO	VARCHAR2(30)    := UPPER('ldci_oficvent');
	sbColumnO	VARCHAR2(30)    := UPPER('OFVECODI');
...
Prompt Agregando columna open.ldci_oficvent.empresa
DECLARE
