prompt "Creando la tabla open.ldci_motipedi"
declare

    -- OSF-XXXX
    sbSentencia     VARCHAR2(32767);

    sbTabla         VARCHAR2(30)    := UPPER('ldci_motipedi' ); 
...
begin
    ...
    IF rcDBA_TABLES.TABLE_NAME IS NULL THEN
        ...
        sbSentencia := sbSentencia || 'MOPECODI	NUMBER (8),' || CHR(10);
        sbSentencia := sbSentencia || 'MOPEDESC VARCHAR2 (200) NOT NULL,' || CHR(10);
        sbSentencia := sbSentencia || 'MOPECLDO VARCHAR2 (4),' || CHR(10);
        sbSentencia := sbSentencia || 'MOPEFLAG VARCHAR2 (2))' || CHR(10);
        ...
        pCreaComenColumna('MOPECODI','Codigo motivo');

        pCreaComenColumna('MOPEDESC','Descripcion');
 
        pCreaComenColumna('MOPECLDO','Clase de Documento');       

        pCreaComenColumna('MOPEFLAG','FLAG QUE INDICA SI SE SOLICITA ORDEN LDCISOMA');
...
END;
/
prompt "Creando llave primaria para la tabla open.ldci_motipedi"
DECLARE
	...
	sbTableO	VARCHAR2(30)    := UPPER('ldci_motipedi');
	sbColumnO	VARCHAR2(30)    := UPPER('MOPECODI');
...
Prompt Agregando columna open.ldci_motipedi.empresa
DECLARE
...
BEGIN
    ...
    dbms_output.put_line('Otorgando permisos sobre LDCI_OFICVENT a MIGRA, ROLESELOPEN, RSELOPEN, RSELUSELOPEN');
    EXECUTE IMMEDIATE ('GRANT SELECT ON OPEN.LDCI_OFICVENT TO MIGRA, ROLESELOPEN, RSELOPEN, RSELUSELOPEN ');

    dbms_output.put_line('Otorgando permisos sobre LDCI_OFICVENT a RDMLOPEN');
    EXECUTE IMMEDIATE ('GRANT DELETE, INSERT, UPDATE ON OPEN.LDCI_OFICVENT TO RDMLOPEN');

    dbms_output.put_line('Otorgando permisos sobre LDCI_OFICVENT a PERSONALIZACIONES y SYSTEM_OBJ_PRIVS_ROLE');
    pkg_utilidades.prAplicarPermisos( 'LDCI_OFICVENT' , 'OPEN');
