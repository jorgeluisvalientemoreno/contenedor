...
BEGIN
sa_bocreatePackages.CreatePackage('VW_EMPRESA_',
'CREATE OR REPLACE PACKAGE VW_EMPRESA_ IS ' || chr(10) ||
...
BEGIN ut_trace.trace('********************Comenzar proceso de objeto:VW_EMPRESA_******************************'); END;
/
...
DECLARE
    ...
    CURSOR  cuConfigExpression IS
        ...
        WHERE   GR_CONFIG_EXPRESSION.CONFIG_EXPRESSION_ID IN (
            ...
            WHERE   ENTITY_ID in (SELECT ENTITY_ID FROM GE_ENTITY WHERE NAME_='VW_EMPRESA')
            UNION
...
if (not VW_EMPRESA_.blProcessStatus) then
 return;
end if;
    ...
    DELETE FROM GE_ATTR_VAL_EXPRESS WHERE (ENTITY_ATTRIBUTE_ID) in (SELECT ENTITY_ATTRIBUTE_ID FROM GE_ENTITY_ATTRIBUTES WHERE (ENTITY_ID) in (SELECT ENTITY_ID FROM GE_ENTITY WHERE NAME_='VW_EMPRESA'));
    DELETE FROM GE_ATTR_ALLOWED_VALUES WHERE (ENTITY_ATTRIBUTE_ID) in (SELECT ENTITY_ATTRIBUTE_ID FROM GE_ENTITY_ATTRIBUTES WHERE (ENTITY_ID) in (SELECT ENTITY_ID FROM GE_ENTITY WHERE NAME_='VW_EMPRESA'));
...
BEGIN 
  VW_EMPRESA_.tbEntityAttributeName(90200431) := 'VW_EMPRESA@CODIGO'; 
  VW_EMPRESA_.tbEntityAttributeName(90200432) := 'VW_EMPRESA@NOMBRE'; 
END;
/
...
VW_EMPRESA_.old_tb0_0(0):='VW_EMPRESA'
;
VW_EMPRESA_.tb0_0(0):='VW_EMPRESA';
VW_EMPRESA_.old_tb0_1(0):=6288;
VW_EMPRESA_.tb0_1(0):=GE_BOCATALOG.FNUGETIDSEQFROMCATALOG(VW_EMPRESA_.tb0_0(0), 'ENTITY', 'GE_BOSEQUENCE.NEXTGE_ENTITY');
VW_EMPRESA_.tb0_1(0):=VW_EMPRESA_.tb0_1(0);
ut_trace.trace('Actualizar tabla: GE_ENTITY fila (0)',1);
UPDATE GE_ENTITY SET DESCRIPTION='MULTIEMPRESA - EMPRESA'
,
DISPLAY_NAME='VW_EMPRESA'
,
ALLOWED_FULL_SCAN='N'

 WHERE ENTITY_ID = VW_EMPRESA_.tb0_1(0);

exception when others then
VW_EMPRESA_.blProcessStatus := false;
rollback;
ut_trace.trace('**ERROR:'||sqlerrm);
raise;
END;
...
VW_EMPRESA_.tb1_0(0):=VW_EMPRESA_.tb0_1(0);
VW_EMPRESA_.old_tb1_1(0):=90200431;
VW_EMPRESA_.tb1_1(0):=GE_BOCATALOG.FNUGETIDSEQFROMCATALOG(VW_EMPRESA_.TBENTITYATTRIBUTENAME(VW_EMPRESA_.old_tb1_1(0)), 'ATTRIBUTE', GE_BOSEQUENCE.NEXTGE_ENTITY_ATTRIBUTES);
VW_EMPRESA_.tb1_1(0):=VW_EMPRESA_.tb1_1(0);
...
UNCHECKED_VALUE=null
 WHERE ENTITY_ATTRIBUTE_ID = VW_EMPRESA_.tb1_1(0);
...
VW_EMPRESA_.tb1_0(1):=VW_EMPRESA_.tb0_1(0);
VW_EMPRESA_.old_tb1_1(1):=90200432;
VW_EMPRESA_.tb1_1(1):=GE_BOCATALOG.FNUGETIDSEQFROMCATALOG(VW_EMPRESA_.TBENTITYATTRIBUTENAME(VW_EMPRESA_.old_tb1_1(1)), 'ATTRIBUTE', GE_BOSEQUENCE.NEXTGE_ENTITY_ATTRIBUTES);
VW_EMPRESA_.tb1_1(1):=VW_EMPRESA_.tb1_1(1);
...
CHECKED_VALUE=null,
UNCHECKED_VALUE=null
 WHERE ENTITY_ATTRIBUTE_ID = VW_EMPRESA_.tb1_1(1);
...
begin
SA_BOCreatePackages.DropPackage('VW_EMPRESA_');
end;
/

BEGIN ut_trace.trace('********************FIN  proceso de objeto:VW_EMPRESA_******************************'); end;
/
