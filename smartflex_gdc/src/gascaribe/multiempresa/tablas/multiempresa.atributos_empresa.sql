prompt "Creando la tabla multiempresa.atributos_empresa"
declare

    -- OSF-4616
    sbSentencia     VARCHAR2(32767);

    sbTabla         VARCHAR2(30)    := 'ATRIBUTOS_EMPRESA'; 
...
begin
    ...
    IF rcDBA_TABLES.TABLE_NAME IS NULL THEN
        ...
        sbSentencia := sbSentencia || 'ATRIBUTO                 VARCHAR2(100),' || CHR(10);
        sbSentencia := sbSentencia || 'VALOR                    VARCHAR2(100))' || CHR(10);
        sbSentencia := sbSentencia || 'TABLESPACE TSD_MULTI_M' || CHR(10);
		
        pExecImm( sbSentencia  );

        sbSentencia :=  'COMMENT ON TABLE ' || sbEsquema || '.' ||sbTabla || ' IS ' || '''' || 'Atributos Adicionales de Empresa' || '''';
        pExecImm( sbSentencia  );

        pCreaComenColumna('EMPRESA','Código de la empresa');
        pCreaComenColumna('ATRIBUTO', 'Nombre del atributo');        
        pCreaComenColumna('VALOR' , 'Valor del Atributo' );
...
END;
/
prompt "Creando llave primaria tabla multiempresa.atributos_empresa"
DECLARE
	...
	sbTableO	VARCHAR2(30)    := UPPER('ATRIBUTOS_EMPRESA');
	sbColumnO	VARCHAR2(30)    := UPPER('EMPRESA, ATRIBUTO');
...
DECLARE
    ...
    sbConstraint_Type 	VARCHAR2(50) 		:= 'R';
...
/
    BEGIN
      pkg_utilidades.prAplicarPermisos('ATRIBUTOS_EMPRESA', 'MULTIEMPRESA');
    END;
/