declare
    ...
    sbTabla         VARCHAR2(30)    := 'SUCURSAL_BANCARIA'; 
...
begin
    ...
    IF rcDBA_TABLES.TABLE_NAME IS NULL THEN
        ...
        sbSentencia := sbSentencia || 'BANCO             		NUMBER(10),' || CHR(10);
        sbSentencia := sbSentencia || 'SUCURSAL           		VARCHAR2(15),' || CHR(10);
        ...
        sbSentencia :=  'COMMENT ON TABLE ' || sbEsquema || '.' ||sbTabla || ' IS ' || '''' || 'Multiempresa - Sucursales Bancarias configuradas para la Empresa' || '''';
        pExecImm( sbSentencia  );

        pCreaComenColumna('BANCO', 'Codigo de Entidad financiera ');
        pCreaComenColumna('SUCURSAL', 'Codigo de Sucursal de la entidad');        
        pCreaComenColumna('EMPRESA', 'Codigo de empresa');     
...
DECLARE
	...
	sbTableO	VARCHAR2(30)    := UPPER('SUCURSAL_BANCARIA');
	sbColumnO	VARCHAR2(30)    := UPPER('BANCO'||','||'SUCURSAL');
