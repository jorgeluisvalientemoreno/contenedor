declare
    ...
    sbTabla         VARCHAR2(30)    := 'CICLO_FACTURACION'; 
...
begin
    ...
    IF rcDBA_TABLES.TABLE_NAME IS NULL THEN
        ...
        sbSentencia := sbSentencia || 'CICLO             		NUMBER(4,0),' || CHR(10);
        ...
        sbSentencia :=  'COMMENT ON TABLE ' || sbEsquema || '.' ||sbTabla || ' IS ' || '''' || 'Ciclos del Grupo Gases del Caribe' || '''';
        pExecImm( sbSentencia  );

        pCreaComenColumna('CICLO',  'Código del ciclo');
...
DECLARE
	...
	sbTableO	VARCHAR2(30)    := UPPER('CICLO_FACTURACION');
	sbColumnO	VARCHAR2(30)    := UPPER('CICLO');
