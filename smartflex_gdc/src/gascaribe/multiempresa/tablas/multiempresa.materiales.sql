prompt "Creando la tabla multiempresa.materiales"
declare

    -- OSF-4204
    sbSentencia     VARCHAR2(32767);

    sbTabla         VARCHAR2(30)    := 'MATERIALES'; 
...
begin
    ...
    IF rcDBA_TABLES.TABLE_NAME IS NULL THEN
        ...
        sbSentencia := sbSentencia || 'MATERIAL                 NUMBER(15),' || CHR(10);
        sbSentencia := sbSentencia || 'EMPRESA                  VARCHAR2(10),' || CHR(10);
        sbSentencia := sbSentencia || 'HABILITADO               VARCHAR2(1) NOT NULL)' || CHR(10);
        ...
        pCreaComenColumna('MATERIAL','Código del material de ge_items.items_id');

        pCreaComenColumna('EMPRESA','Código de la empresa: GDCA o GDGU');

        pCreaComenColumna('HABILITADO','S: El material esta habilitado para la empresa. N: El material NO esta habilitado para la empresa');
...
END;
/
prompt "Creando llave primaria para la tabla multiempresa.material"
DECLARE
	...
	sbTableO	VARCHAR2(30)    := UPPER('MATERIALES');
	sbColumnO	VARCHAR2(30)    := UPPER('MATERIAL,EMPRESA');
