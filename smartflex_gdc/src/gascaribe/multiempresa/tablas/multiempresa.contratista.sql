declare
    ...
    sbTabla         VARCHAR2(30)    := 'CONTRATISTA'; 
...
begin
    ...
    IF rcDBA_TABLES.TABLE_NAME IS NULL THEN
        ...
        sbSentencia := sbSentencia || 'CONTRATISTA             	NUMBER(4,0),' || CHR(10);
        sbSentencia := sbSentencia || 'EMPRESA                 	VARCHAR2(10)' || CHR(10)||')';
        sbSentencia := sbSentencia || 'TABLESPACE TSD_MULTI_M' || CHR(10);        
		
        pExecImm( sbSentencia  );

        sbSentencia :=  'COMMENT ON TABLE ' || sbEsquema || '.' ||sbTabla || ' IS ' || '''' || 'Contratistas del Grupo Gases del Caribe' || '''';
        pExecImm( sbSentencia  );

        pCreaComenColumna('CONTRATISTA',  'Código del contratista');
        pCreaComenColumna('EMPRESA', 'Codigo de empresa al cual pertenece el contratista');        
...
DECLARE
	...
	sbTableO	VARCHAR2(30)    := UPPER('CONTRATISTA');
	sbColumnO	VARCHAR2(30)    := UPPER('CONTRATISTA');
