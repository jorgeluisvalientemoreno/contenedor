prompt "Creando la tabla multiempresa.empresa"
declare

    -- OSF-3940
    sbSentencia     VARCHAR2(32767);

    sbTabla         VARCHAR2(30)    := 'EMPRESA'; 
...
begin
    ...
    IF rcDBA_TABLES.TABLE_NAME IS NULL THEN
        ...
        sbSentencia := sbSentencia || 'CODIGO                  VARCHAR2(10),' || CHR(10);
        sbSentencia := sbSentencia || 'NOMBRE                  VARCHAR2(100),' || CHR(10);
        sbSentencia := sbSentencia || 'NIT                     VARCHAR2(20),' || CHR(10);
        sbSentencia := sbSentencia || 'DIRECCION               VARCHAR2(130),' || CHR(10);
        sbSentencia := sbSentencia || 'LOCALIDAD               NUMBER(6),' || CHR(10);
        sbSentencia := sbSentencia || 'DEPARTAMENTO            NUMBER(6),' || CHR(10);
        sbSentencia := sbSentencia || 'MATRICULA_MERCANTIL     VARCHAR2(100),' || CHR(10);
        sbSentencia := sbSentencia || 'ZONA_POSTAL_EMISOR      VARCHAR2(6),' || CHR(10);
        sbSentencia := sbSentencia || 'RESP_FISCAL_EMISOR      VARCHAR2(100),' || CHR(10);
        sbSentencia := sbSentencia || 'COD_TRIBUTO_EMISOR      VARCHAR2(100),' || CHR(10);
        sbSentencia := sbSentencia || 'NOMBRE_TRIBUTO_EMISOR   VARCHAR2(100),' || CHR(10);
        sbSentencia := sbSentencia || 'EMAIL_EMISOR            VARCHAR2(100),' || CHR(10);
        sbSentencia := sbSentencia || 'EMAIL_CONTROLER_EMISOR  VARCHAR2(100),' || CHR(10);
        sbSentencia := sbSentencia || 'TELEFONO_EMISOR         VARCHAR2(50),' || CHR(10);
        sbSentencia := sbSentencia || 'FAX_EMISOR              VARCHAR2(50),' || CHR(10);
        sbSentencia := sbSentencia || 'RAZON_SOCIAL_EMISOR     VARCHAR2(100),' || CHR(10);
        sbSentencia := sbSentencia || 'DIRECCION_EMISOR        VARCHAR2(130),' || CHR(10);
        sbSentencia := sbSentencia || 'TIPO_REGIMEN_EMISOR     VARCHAR2(50))' || CHR(10); 
        sbSentencia := sbSentencia || 'TABLESPACE TSD_MULTI_M' || CHR(10);
		
        pExecImm( sbSentencia  );

        sbSentencia :=  'COMMENT ON TABLE ' || sbEsquema || '.' ||sbTabla || ' IS ' || '''' || 'Empresas del Grupo Gases del Caribe' || '''';
        pExecImm( sbSentencia  );

        pCreaComenColumna('CODIGO','Código de la empresa');
        pCreaComenColumna('NOMBRE', 'Nombre de la empresa');        
        pCreaComenColumna('NIT' , 'Nit de la empresa' );
        pCreaComenColumna('DIRECCION' , 'Dirección de la empresa' );        
        pCreaComenColumna('LOCALIDAD' , 'Localidad de la empresa' );
        pCreaComenColumna('DEPARTAMENTO' , 'Departamento base de la empresa' );
        pCreaComenColumna('MATRICULA_MERCANTIL' , 'Matricula Mercantil de la empresa' );
        pCreaComenColumna('ZONA_POSTAL_EMISOR' , 'Zona Postal del emisor de las facturas' );
        pCreaComenColumna('RESP_FISCAL_EMISOR' , 'Responsable fiscal del emisor de las facturas' );
        pCreaComenColumna('COD_TRIBUTO_EMISOR' , 'Código tributario del emisor de las facturas' );
        pCreaComenColumna('NOMBRE_TRIBUTO_EMISOR' , 'Nombre tributario del emisor de las facturas' );
        pCreaComenColumna('EMAIL_EMISOR' , 'Correo electrónico del emisor de las facturas' );
        pCreaComenColumna('EMAIL_CONTROLER_EMISOR' , 'Correo electrónico del controller del emisor de las facturas' );
        pCreaComenColumna('TELEFONO_EMISOR' , 'Teléfono del emisor de las facturas' );
        pCreaComenColumna('FAX_EMISOR' , 'Fax del emisor de las facturas' );
        pCreaComenColumna('RAZON_SOCIAL_EMISOR' , 'Razón social del emisor de las facturas' );
        pCreaComenColumna('DIRECCION_EMISOR' , 'Dirección del emisor de las facturas' );
        pCreaComenColumna('TIPO_REGIMEN_EMISOR' , 'Regimen cotributibo del emisor de las facturas' );
...
END;
/
prompt "Creando llave primaria tabla multiempresa.empresa"
DECLARE
	...
	sbTableO	VARCHAR2(30)    := UPPER('EMPRESA');
	sbColumnO	VARCHAR2(30)    := UPPER('CODIGO');
    ...
    IF rcDBA_CONSTRAINTS.CONSTRAINT_NAME IS NULL THEN
        ...
        CASE sbConstraint_Mnemonico 
            ...
            WHEN 'PK' THEN
                ...
                sbSentencia := sbSentencia || 'USING INDEX TABLESPACE TSI_MULTI_M';                                    
                sbSentencia := sbSentencia || ')' || CHR(10);
                    
            WHEN 'FK' THEN
    
                sbSentencia := sbSentencia || 'ALTER TABLE ' || sbTableO || ' ADD (';
                sbSentencia := sbSentencia || 'CONSTRAINT ' ||  sbNombreRestriccion || CHR(10);
                sbSentencia := sbSentencia || tbConstraintTypes(sbConstraint_Mnemonico) || '(' || sbColumnO || ') ';
...
Prompt Agregando columna multiempres.empresa.organizacion_venta
DECLARE

    -- OSF-4268
    sbEsquema   VARCHAR2(30 ) := 'MULTIEMPRESA';
    sbTabla      VARCHAR2(30 ) := 'EMPRESA';
    sbColumna   VARCHAR2(30 ) := 'ORGANIZACION_VENTA';
...
BEGIN
    ...
    IF rcDba_Tab_COLUMNS.COLUMN_NAME IS NULL THEN
        ...
        sbComando := 'ALTER TABLE '|| sbEsquema || '.' || sbTabla || ' ADD ' || sbColumna || ' VARCHAR2(4)';
        
        pExecImm( sbComando );
            
        pCreaComenColumna( sbColumna, 'ORGANIZACION DE VENTA' ) ;
