...
BEGIN
sa_bocreatePackages.CreatePackage('EXP_PROCESS_165_',
'CREATE OR REPLACE PACKAGE EXP_PROCESS_165_ IS ' || chr(10) ||
...
' ' || chr(10) ||
'END EXP_PROCESS_165_;');
END;
/


BEGIN ut_trace.trace('********************Comenzar proceso de objeto:EXP_PROCESS_165_******************************'); END;
/


BEGIN
   ut_trace.trace('Cargando tabla en memoria para posterior borrado GR_CONFIG_EXPRESSION',1);
   open EXP_PROCESS_165_.cuExpression;
   fetch EXP_PROCESS_165_.cuExpression bulk collect INTO EXP_PROCESS_165_.tbGR_CONFIG_EXPRESSIONId;
   close EXP_PROCESS_165_.cuExpression;
END;
/ 
BEGIN
...
BEGIN
sa_bocreatePackages.CreatePackage('DEL_ROOT_259_',
'CREATE OR REPLACE PACKAGE DEL_ROOT_259_ IS ' || chr(10) ||
'blProcessStatus boolean := true; ' || chr(10) ||
'type tyWF_UNIT_TYPERowId is table of rowid index by varchar2(100); ' || chr(10) ||
'tbWF_UNIT_TYPERowId tyWF_UNIT_TYPERowId;type tyWF_UNIT_CATEGORYRowId is table of rowid index by varchar2(100); ' || chr(10) ||
'tbWF_UNIT_CATEGORYRowId tyWF_UNIT_CATEGORYRowId;type tyWF_ATTRIBUTES_EQUIVRowId is table of rowid index by varchar2(100); ' || chr(10) ||
...
'WHERE   UNIT_ID IN ( ' || chr(10) ||
'        SELECT UNIT_ID         FROM WF_UNIT  ' || chr(10) ||
'        WHERE UNIT_ID = 259 ' || chr(10) ||
'        UNION ALL         SELECT UNIT_ID         FROM WF_UNIT  ' || chr(10) ||
'        WHERE PROCESS_ID = 259  ' || chr(10) ||
'       ) ' || chr(10) ||
'AND   PRE_EXPRESSION_ID IS NOT NULL ' || chr(10) ||
'UNION ALL  ' || chr(10) ||
'SELECT POS_EXPRESSION_ID ' || chr(10) ||
...
BEGIN ut_trace.trace('********************Comenzar proceso de objeto:DEL_ROOT_259_******************************'); END;
/


BEGIN
   ut_trace.trace('Cargando tabla en memoria para posterior borrado GR_CONFIG_EXPRESSION',1);
   open DEL_ROOT_259_.cuExpression;
   fetch DEL_ROOT_259_.cuExpression bulk collect INTO DEL_ROOT_259_.tbGR_CONFIG_EXPRESSIONId;
   close DEL_ROOT_259_.cuExpression;
END;
/ 
...
AND     A.UNIT_ID IN (
        ...
        WHERE PROCESS_ID = 259 
       )
;
BEGIN

if (not DEL_ROOT_259_.blProcessStatus) then
 return;
end if;
 nuIndex := 1;
 FOR crObjectsName IN cuObjectsName LOOP
  DEL_ROOT_259_.tbObjectToDelete(nuIndex) := crObjectsName.OBJECT_NAME;
  nuIndex := nuIndex + 1;
 END LOOP;
EXCEPTION 
...
DECLARE 
 ...
 SELECT rowid FROM GR_CONFIG_EXPRESSION WHERE (CONFIG_EXPRESSION_ID) in (SELECT PRE_EXPRESSION_ID FROM WF_UNIT WHERE PROCESS_ID = 259);
BEGIN 
...
DEL_ROOT_259_.tbGR_CONFIG_EXPRESSIONRowId(rcData.rowid):=rcData.rowid; 
END loop; 

exception when others then
DEL_ROOT_259_.blProcessStatus := false;
rollback;
...
DECLARE 
 CURSOR cuLoadTemporaryTable IS 
 SELECT rowid FROM GR_CONFIG_EXPRESSION WHERE (CONFIG_EXPRESSION_ID) in (SELECT POS_EXPRESSION_ID FROM WF_UNIT WHERE PROCESS_ID = 259);
BEGIN 
...
DECLARE 
 CURSOR cuLoadTemporaryTable IS 
 SELECT rowid FROM GR_CONFIG_EXPRESSION WHERE (CONFIG_EXPRESSION_ID) in (SELECT PRE_EXPRESSION_ID FROM WF_UNIT WHERE (UNIT_ID) in (SELECT TARGET_ID FROM WF_TRANSITION WHERE (TARGET_ID) in (SELECT UNIT_ID FROM WF_UNIT WHERE PROCESS_ID = 259)));
BEGIN 
...
DECLARE 
 CURSOR cuLoadTemporaryTable IS 
 SELECT rowid FROM GR_CONFIG_EXPRESSION WHERE (CONFIG_EXPRESSION_ID) in (SELECT POS_EXPRESSION_ID FROM WF_UNIT WHERE (UNIT_ID) in (SELECT TARGET_ID FROM WF_TRANSITION WHERE (TARGET_ID) in (SELECT UNIT_ID FROM WF_UNIT WHERE PROCESS_ID = 259)));
BEGIN 
...
DECLARE 
 CURSOR cuLoadTemporaryTable IS 
 SELECT rowid FROM WF_UNIT WHERE (UNIT_ID) in (SELECT TARGET_ID FROM WF_TRANSITION WHERE (TARGET_ID) in (SELECT UNIT_ID FROM WF_UNIT WHERE PROCESS_ID = 259));
BEGIN 
...
for rcData in cuLoadTemporaryTable loop
DEL_ROOT_259_.tbWF_UNITRowId(rcData.rowid):=rcData.rowid; 
END loop; 
...
SELECT rowid
FROM WF_TRANSITION WHERE (TARGET_ID) in (SELECT UNIT_ID FROM WF_UNIT WHERE PROCESS_ID = 259);
nuIndex binary_integer;
BEGIN
...
nuVarcharIndex:=DEL_ROOT_259_.tbGR_CONFIG_EXPRESSIONRowId.first;  
while (nuVarcharIndex is not null) loop 
begin
ut_trace.trace('Row id a borrar:'||nuVarcharIndex,1);
delete from GR_CONFIG_EXPRESSION where rowid = DEL_ROOT_259_.tbGR_CONFIG_EXPRESSIONRowId(nuVarcharIndex); 
...
DEL_ROOT_259_.tbGR_CONFIG_EXPRESSIONRowId.delete(nuOldVarcharIndex);
end if;
nuVarcharIndex := DEL_ROOT_259_.tbGR_CONFIG_EXPRESSIONRowId.next(nuVarcharIndex); 
END loop; 
...
nuVarcharIndex:=DEL_ROOT_259_.tbWF_UNITRowId.first;  
while (nuVarcharIndex is not null) loop 
begin
ut_trace.trace('Row id a borrar:'||nuVarcharIndex,1);
delete from WF_UNIT where rowid = DEL_ROOT_259_.tbWF_UNITRowId(nuVarcharIndex); 
nuOldVarcharIndex:=nuVarcharIndex;
EXCEPTION
when ex.RECORD_HAVE_CHILDREN then
ut_trace.trace('No se pudo borrar el registro '||nuVarcharIndex,1);
...
END;
if nuOldVarcharIndex is not null then
DEL_ROOT_259_.tbWF_UNITRowId.delete(nuOldVarcharIndex);
end if;
nuVarcharIndex := DEL_ROOT_259_.tbWF_UNITRowId.next(nuVarcharIndex); 
END loop; 
...
CURSOR cuRowIdToDelete is
SELECT rowid
FROM WF_UNIT_DATA_MAP WHERE (SOURCE_ID) in (SELECT UNIT_ATTRIBUTE_ID FROM WF_UNIT_ATTRIBUTE WHERE (UNIT_ID) in (SELECT UNIT_ID FROM WF_UNIT WHERE PROCESS_ID = 259));
nuIndex binary_integer;
BEGIN
...
CURSOR cuRowIdToDelete is
SELECT rowid
FROM WF_UNIT_DATA_MAP WHERE (TARGET_ID) in (SELECT UNIT_ATTRIBUTE_ID FROM WF_UNIT_ATTRIBUTE WHERE (UNIT_ID) in (SELECT UNIT_ID FROM WF_UNIT WHERE PROCESS_ID = 259));
nuIndex binary_integer;
BEGIN
...
CURSOR cuRowIdToDelete is
SELECT rowid
FROM WF_UNIT_ATTRIBUTE WHERE (UNIT_ID) in (SELECT UNIT_ID FROM WF_UNIT WHERE PROCESS_ID = 259);
nuIndex binary_integer;
BEGIN
...
CURSOR cuRowIdToDelete is
SELECT rowid
FROM WF_UNIT WHERE PROCESS_ID = 259;
nuIndex binary_integer;
BEGIN
...
BEGIN 
    nuBinaryIndex := DEL_ROOT_259_.tbGR_CONFIG_EXPRESSIONId.first;
    while (nuBinaryIndex IS not null) loop 
       BEGIN
           delete from GR_CONFIG_EXPRESSION where config_expression_id = DEL_ROOT_259_.tbGR_CONFIG_EXPRESSIONId(nuBinaryIndex);
           ut_trace.trace('Delete config_expression_id = : '||DEL_ROOT_259_.tbGR_CONFIG_EXPRESSIONId(nuBinaryIndex) || ' OK', 1);
       EXCEPTION
           when ex.RECORD_HAVE_CHILDREN then
               ut_trace.trace('No se pudo borrar la regla config_expression_id '||DEL_ROOT_259_.tbGR_CONFIG_EXPRESSIONId(nuBinaryIndex),1);
       END;
       nuBinaryIndex := DEL_ROOT_259_.tbGR_CONFIG_EXPRESSIONId.next(nuBinaryIndex);
    END loop;
...
/ 
BEGIN
ut_trace.trace('Actualizar Variable de Proceso: EXP_PROCESS_165',1);
EXP_PROCESS_165_.blProcessStatus := DEL_ROOT_259_.blProcessStatus ; 
exception when others then
rollback;
ut_trace.trace('**ERROR:'||sqlerrm,1);
raise;
...
BEGIN
 ...
 nuIndex := DEL_ROOT_259_.tbObjectToDelete.first;
 while (nuIndex is not null) LOOP
  BEGIN
   ut_trace.trace('Objeto a borrar: ' || DEL_ROOT_259_.tbObjectToDelete(nuIndex),1);
   IF(ut_object.fboExistObject(DEL_ROOT_259_.tbObjectToDelete(nuIndex)) and not fblIsObjectUseByRule(DEL_ROOT_259_.tbObjectToDelete(nuIndex))) THEN
    blObjectDeleted := ut_object.fboDeleteObject(DEL_ROOT_259_.tbObjectToDelete(nuIndex));
    ut_trace.trace('  * Borrado ' || DEL_ROOT_259_.tbObjectToDelete(nuIndex), 1);
   END IF;
  EXCEPTION when others then
   ut_trace.trace('- No se borra el objeto (' || DEL_ROOT_259_.tbObjectToDelete(nuIndex) || '): ' || sqlerrm,1);
  END;
  nuIndex := DEL_ROOT_259_.tbObjectToDelete.next(nuIndex);
 END loop;
...
begin
SA_BOCreatePackages.DropPackage('DEL_ROOT_259_');
end;
/

BEGIN ut_trace.trace('********************FIN  proceso de objeto:DEL_ROOT_259_******************************'); end;
/
...
DECLARE 
 CURSOR cuLoadTemporaryTable IS 
 SELECT rowid FROM GR_CONFIG_EXPRESSION WHERE (CONFIG_EXPRESSION_ID) in (SELECT PRE_EXPRESSION_ID FROM WF_UNIT WHERE UNIT_ID = 259);
BEGIN 
...
DECLARE 
 CURSOR cuLoadTemporaryTable IS 
 SELECT rowid FROM GR_CONFIG_EXPRESSION WHERE (CONFIG_EXPRESSION_ID) in (SELECT POS_EXPRESSION_ID FROM WF_UNIT WHERE UNIT_ID = 259);
BEGIN 
...
DECLARE 
 CURSOR cuLoadTemporaryTable IS 
 SELECT rowid FROM GR_CONFIG_EXPRESSION WHERE (CONFIG_EXPRESSION_ID) in (SELECT PRE_EXPRESSION_ID FROM WF_UNIT WHERE (UNIT_ID) in (SELECT TARGET_ID FROM WF_TRANSITION WHERE (TARGET_ID) in (SELECT UNIT_ID FROM WF_UNIT WHERE UNIT_ID = 259)));
BEGIN 
...
DECLARE 
 CURSOR cuLoadTemporaryTable IS 
 SELECT rowid FROM GR_CONFIG_EXPRESSION WHERE (CONFIG_EXPRESSION_ID) in (SELECT POS_EXPRESSION_ID FROM WF_UNIT WHERE (UNIT_ID) in (SELECT TARGET_ID FROM WF_TRANSITION WHERE (TARGET_ID) in (SELECT UNIT_ID FROM WF_UNIT WHERE UNIT_ID = 259)));
BEGIN 
...
DECLARE 
 CURSOR cuLoadTemporaryTable IS 
 SELECT rowid FROM WF_UNIT WHERE (UNIT_ID) in (SELECT TARGET_ID FROM WF_TRANSITION WHERE (TARGET_ID) in (SELECT UNIT_ID FROM WF_UNIT WHERE UNIT_ID = 259));
BEGIN 
...
CURSOR cuRowIdToDelete is
SELECT rowid
FROM WF_TRANSITION WHERE (TARGET_ID) in (SELECT UNIT_ID FROM WF_UNIT WHERE UNIT_ID = 259);
nuIndex binary_integer;
BEGIN
...
CURSOR cuRowIdToDelete is
SELECT rowid
FROM WF_UNIT_DATA_MAP WHERE (SOURCE_ID) in (SELECT UNIT_ATTRIBUTE_ID FROM WF_UNIT_ATTRIBUTE WHERE (UNIT_ID) in (SELECT UNIT_ID FROM WF_UNIT WHERE UNIT_ID = 259));
nuIndex binary_integer;
BEGIN
...
CURSOR cuRowIdToDelete is
SELECT rowid
FROM WF_UNIT_DATA_MAP WHERE (TARGET_ID) in (SELECT UNIT_ATTRIBUTE_ID FROM WF_UNIT_ATTRIBUTE WHERE (UNIT_ID) in (SELECT UNIT_ID FROM WF_UNIT WHERE UNIT_ID = 259));
nuIndex binary_integer;
BEGIN
...
CURSOR cuRowIdToDelete is
SELECT rowid
FROM WF_UNIT_ATTRIBUTE WHERE (UNIT_ID) in (SELECT UNIT_ID FROM WF_UNIT WHERE UNIT_ID = 259);
nuIndex binary_integer;
BEGIN
...
CURSOR cuRowIdToDelete is
SELECT rowid
FROM WF_UNIT WHERE UNIT_ID = 259;
nuIndex binary_integer;
BEGIN
...
if (not EXP_PROCESS_165_.blProcessStatus) then
 return;
end if;
 nuIndex := 1;
 FOR crObjectsName IN cuObjectsName LOOP
  EXP_PROCESS_165_.tbObjectToDelete(nuIndex) := crObjectsName.OBJECT_NAME;
  nuIndex := nuIndex + 1;
 END LOOP;
EXCEPTION 
 when ex.controlled_error then
...
DECLARE 
 CURSOR cuLoadTemporaryTable IS 
 SELECT rowid FROM GR_CONFIG_EXPRESSION WHERE (CONFIG_EXPRESSION_ID) in (SELECT PRE_EXPRESSION_ID FROM WF_UNIT WHERE (UNIT_TYPE_ID) in (SELECT UNIT_TYPE_ID FROM WF_UNIT_TYPE WHERE UNIT_TYPE_ID=165) AND NODE_TYPE_ID=0);
BEGIN 
...
ut_trace.trace('Cargando a memoria tabla GR_CONFIG_EXPRESSION',1);
for rcData in cuLoadTemporaryTable loop
EXP_PROCESS_165_.tbGR_CONFIG_EXPRESSIONRowId(rcData.rowid):=rcData.rowid; 
END loop; 

exception when others then
EXP_PROCESS_165_.blProcessStatus := false;
rollback;
ut_trace.trace('**ERROR:'||sqlerrm);
raise;
END; 
...
DECLARE 
 CURSOR cuLoadTemporaryTable IS 
 SELECT rowid FROM GR_CONFIG_EXPRESSION WHERE (CONFIG_EXPRESSION_ID) in (SELECT POS_EXPRESSION_ID FROM WF_UNIT WHERE (UNIT_TYPE_ID) in (SELECT UNIT_TYPE_ID FROM WF_UNIT_TYPE WHERE UNIT_TYPE_ID=165) AND NODE_TYPE_ID=0);
BEGIN 
...
CURSOR cuRowIdToDelete is
SELECT rowid
FROM WF_UNIT WHERE (UNIT_TYPE_ID) in (SELECT UNIT_TYPE_ID FROM WF_UNIT_TYPE WHERE UNIT_TYPE_ID=165) AND NODE_TYPE_ID=0;
nuIndex binary_integer;
BEGIN
...
ut_trace.trace('Borrando tabla cargada en memoria GR_CONFIG_EXPRESSION',1);
nuVarcharIndex:=EXP_PROCESS_165_.tbGR_CONFIG_EXPRESSIONRowId.first;  
while (nuVarcharIndex is not null) loop 
begin
ut_trace.trace('Row id a borrar:'||nuVarcharIndex,1);
delete from GR_CONFIG_EXPRESSION where rowid = EXP_PROCESS_165_.tbGR_CONFIG_EXPRESSIONRowId(nuVarcharIndex); 
nuOldVarcharIndex:=nuVarcharIndex;
EXCEPTION
when ex.RECORD_HAVE_CHILDREN then
ut_trace.trace('No se pudo borrar el registro '||nuVarcharIndex,1);
...
END;
if nuOldVarcharIndex is not null then
EXP_PROCESS_165_.tbGR_CONFIG_EXPRESSIONRowId.delete(nuOldVarcharIndex);
end if;
nuVarcharIndex := EXP_PROCESS_165_.tbGR_CONFIG_EXPRESSIONRowId.next(nuVarcharIndex); 
END loop; 
...
CURSOR cuRowIdToDelete is
SELECT rowid
FROM WF_UNIT_TYPE WHERE UNIT_TYPE_ID=165;
nuIndex binary_integer;
BEGIN
...
EXP_PROCESS_165_.tb0_0(0):=3;
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT_CATEGORY fila (0)',1);
UPDATE WF_UNIT_CATEGORY SET CATEGORY_ID=EXP_PROCESS_165_.tb0_0(0),
DISPLAY_NUMBER='Proceso Principal'

 WHERE CATEGORY_ID = EXP_PROCESS_165_.tb0_0(0);
if not (sql%found) then
INSERT INTO WF_UNIT_CATEGORY(CATEGORY_ID,DISPLAY_NUMBER) 
VALUES (EXP_PROCESS_165_.tb0_0(0),
'Proceso Principal'
);
end if;
...
EXP_PROCESS_165_.tb1_0(0):=9;
ut_trace.trace('Actualizar o insertar tabla: GE_MODULE fila (0)',1);
UPDATE GE_MODULE SET MODULE_ID=EXP_PROCESS_165_.tb1_0(0),
...
LOCALIZATION='IN'
 ...
 WHERE MODULE_ID = EXP_PROCESS_165_.tb1_0(0);
if not (sql%found) then
INSERT INTO GE_MODULE(MODULE_ID,DESCRIPTION,MNEMONIC,LAST_MESSAGE,PATH_MODULE,ICON_NAME,LOCALIZATION) 
VALUES (EXP_PROCESS_165_.tb1_0(0),
'WorkFlow'
,
'WF'
,
...
EXP_PROCESS_165_.tb2_0(0):=165;
EXP_PROCESS_165_.tb2_1(0):=EXP_PROCESS_165_.tb0_0(0);
EXP_PROCESS_165_.tb2_2(0):=EXP_PROCESS_165_.tb1_0(0);
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT_TYPE fila (0)',1);
UPDATE WF_UNIT_TYPE SET UNIT_TYPE_ID=EXP_PROCESS_165_.tb2_0(0),
CATEGORY_ID=EXP_PROCESS_165_.tb2_1(0),
MODULE_ID=EXP_PROCESS_165_.tb2_2(0),
ACTION_ID=null,
PARENT_ID=983,
...
TAG_NAME='UNIT_TYPE_165'
,
DESCRIPTION='Gestión de Quejas'
,
DISPLAY='Gestión de Quejas'
,
ICON=null,
IS_STAGE_PROCESS='N'
,
...
ASSIGNED_USER_ONLY=null
 WHERE UNIT_TYPE_ID = EXP_PROCESS_165_.tb2_0(0);
if not (sql%found) then
...
VALUES (EXP_PROCESS_165_.tb2_0(0),
EXP_PROCESS_165_.tb2_1(0),
EXP_PROCESS_165_.tb2_2(0),
null,
983,
null,
null,
...
EXP_PROCESS_165_.tb3_0(0):=34;
EXP_PROCESS_165_.tb3_1(0):=EXP_PROCESS_165_.tb2_0(0);
ut_trace.trace('Actualizar o insertar tabla: WF_ATTRIBUTES_EQUIV fila (0)',1);
UPDATE WF_ATTRIBUTES_EQUIV SET ATTRIBUTES_EQUIV_ID=EXP_PROCESS_165_.tb3_0(0),
UNIT_TYPE_ID=EXP_PROCESS_165_.tb3_1(0),
INTERFACE_CONFIG_ID=1,
STD_TIME=0,
MAX_TIME=31536000,
AVG_TIME=0,
DESCRIPTION='VAL1 [468] VAL2 [1092]'
,
VALUE_1='0'
,
VALUE_2='0'
...
VALUE_19=null,
VALUE_20=null
 WHERE ATTRIBUTES_EQUIV_ID = EXP_PROCESS_165_.tb3_0(0);
if not (sql%found) then
...
VALUES (EXP_PROCESS_165_.tb3_0(0),
EXP_PROCESS_165_.tb3_1(0),
1,
0,
31536000,
0,
...
EXP_PROCESS_165_.tb3_0(1):=36;
EXP_PROCESS_165_.tb3_1(1):=EXP_PROCESS_165_.tb2_0(0);
ut_trace.trace('Actualizar o insertar tabla: WF_ATTRIBUTES_EQUIV fila (1)',1);
UPDATE WF_ATTRIBUTES_EQUIV SET ATTRIBUTES_EQUIV_ID=EXP_PROCESS_165_.tb3_0(1),
UNIT_TYPE_ID=EXP_PROCESS_165_.tb3_1(1),
INTERFACE_CONFIG_ID=1,
STD_TIME=0,
MAX_TIME=31536000,
AVG_TIME=0,
DESCRIPTION='Val1 [634] Val2 [1092]'
,
VALUE_1='0'
,
VALUE_2='0'
...
VALUE_19=null,
VALUE_20=null
 WHERE ATTRIBUTES_EQUIV_ID = EXP_PROCESS_165_.tb3_0(1);
if not (sql%found) then
INSERT INTO WF_ATTRIBUTES_EQUIV(ATTRIBUTES_EQUIV_ID,UNIT_TYPE_ID,INTERFACE_CONFIG_ID,STD_TIME,MAX_TIME,AVG_TIME,DESCRIPTION,VALUE_1,VALUE_2,VALUE_3,VALUE_4,VALUE_5,VALUE_6,VALUE_7,VALUE_8,VALUE_9,VALUE_10,VALUE_11,VALUE_12,VALUE_13,VALUE_14,VALUE_15,VALUE_16,VALUE_17,VALUE_18,VALUE_19,VALUE_20) 
VALUES (EXP_PROCESS_165_.tb3_0(1),
EXP_PROCESS_165_.tb3_1(1),
1,
0,
31536000,
0,
...
EXP_PROCESS_165_.tb3_0(2):=20;
EXP_PROCESS_165_.tb3_1(2):=EXP_PROCESS_165_.tb2_0(0);
ut_trace.trace('Actualizar o insertar tabla: WF_ATTRIBUTES_EQUIV fila (2)',1);
UPDATE WF_ATTRIBUTES_EQUIV SET ATTRIBUTES_EQUIV_ID=EXP_PROCESS_165_.tb3_0(2),
UNIT_TYPE_ID=EXP_PROCESS_165_.tb3_1(2),
INTERFACE_CONFIG_ID=21,
STD_TIME=0,
MAX_TIME=31536000,
AVG_TIME=0,
DESCRIPTION='Val1 [100030] Val2 []'
,
VALUE_1='0'
,
VALUE_2='0'
...
VALUE_19=null,
VALUE_20=null
 WHERE ATTRIBUTES_EQUIV_ID = EXP_PROCESS_165_.tb3_0(2);
if not (sql%found) then
INSERT INTO WF_ATTRIBUTES_EQUIV(ATTRIBUTES_EQUIV_ID,UNIT_TYPE_ID,INTERFACE_CONFIG_ID,STD_TIME,MAX_TIME,AVG_TIME,DESCRIPTION,VALUE_1,VALUE_2,VALUE_3,VALUE_4,VALUE_5,VALUE_6,VALUE_7,VALUE_8,VALUE_9,VALUE_10,VALUE_11,VALUE_12,VALUE_13,VALUE_14,VALUE_15,VALUE_16,VALUE_17,VALUE_18,VALUE_19,VALUE_20) 
VALUES (EXP_PROCESS_165_.tb3_0(2),
EXP_PROCESS_165_.tb3_1(2),
21,
0,
31536000,
0,
...
EXP_PROCESS_165_.tb3_0(3):=31;
EXP_PROCESS_165_.tb3_1(3):=EXP_PROCESS_165_.tb2_0(0);
ut_trace.trace('Actualizar o insertar tabla: WF_ATTRIBUTES_EQUIV fila (3)',1);
UPDATE WF_ATTRIBUTES_EQUIV SET ATTRIBUTES_EQUIV_ID=EXP_PROCESS_165_.tb3_0(3),
UNIT_TYPE_ID=EXP_PROCESS_165_.tb3_1(3),
INTERFACE_CONFIG_ID=21,
STD_TIME=0,
MAX_TIME=31536000,
AVG_TIME=0,
...
VALUE_19=null,
VALUE_20=null
 WHERE ATTRIBUTES_EQUIV_ID = EXP_PROCESS_165_.tb3_0(3);
if not (sql%found) then
INSERT INTO WF_ATTRIBUTES_EQUIV(ATTRIBUTES_EQUIV_ID,UNIT_TYPE_ID,INTERFACE_CONFIG_ID,STD_TIME,MAX_TIME,AVG_TIME,DESCRIPTION,VALUE_1,VALUE_2,VALUE_3,VALUE_4,VALUE_5,VALUE_6,VALUE_7,VALUE_8,VALUE_9,VALUE_10,VALUE_11,VALUE_12,VALUE_13,VALUE_14,VALUE_15,VALUE_16,VALUE_17,VALUE_18,VALUE_19,VALUE_20) 
VALUES (EXP_PROCESS_165_.tb3_0(3),
EXP_PROCESS_165_.tb3_1(3),
21,
0,
31536000,
0,
...
EXP_PROCESS_165_.tb4_0(0):=0;
ut_trace.trace('Actualizar o insertar tabla: WF_NODE_TYPE fila (0)',1);
UPDATE WF_NODE_TYPE SET NODE_TYPE_ID=EXP_PROCESS_165_.tb4_0(0),
DESCRIPTION='Ra¿z'

 WHERE NODE_TYPE_ID = EXP_PROCESS_165_.tb4_0(0);
if not (sql%found) then
INSERT INTO WF_NODE_TYPE(NODE_TYPE_ID,DESCRIPTION) 
VALUES (EXP_PROCESS_165_.tb4_0(0),
'Ra¿z'
);
end if;
...
EXP_PROCESS_165_.tb5_0(0):=259;
EXP_PROCESS_165_.tb5_2(0):=EXP_PROCESS_165_.tb2_0(0);
EXP_PROCESS_165_.tb5_3(0):=EXP_PROCESS_165_.tb4_0(0);
EXP_PROCESS_165_.tb5_4(0):=EXP_PROCESS_165_.tb1_0(0);
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT fila (0)',1);
UPDATE WF_UNIT SET UNIT_ID=EXP_PROCESS_165_.tb5_0(0),
PROCESS_ID=null,
UNIT_TYPE_ID=EXP_PROCESS_165_.tb5_2(0),
NODE_TYPE_ID=EXP_PROCESS_165_.tb5_3(0),
MODULE_ID=EXP_PROCESS_165_.tb5_4(0),
ACTION_ID=null,
...
VIEWABLE='N'
 ...
 WHERE UNIT_ID = EXP_PROCESS_165_.tb5_0(0);
if not (sql%found) then
...
VALUES (EXP_PROCESS_165_.tb5_0(0),
null,
EXP_PROCESS_165_.tb5_2(0),
EXP_PROCESS_165_.tb5_3(0),
EXP_PROCESS_165_.tb5_4(0),
null,
null,
null,
null,
...
EXP_PROCESS_165_.tb0_0(1):=2;
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT_CATEGORY fila (1)',1);
UPDATE WF_UNIT_CATEGORY SET CATEGORY_ID=EXP_PROCESS_165_.tb0_0(1),
DISPLAY_NUMBER='Actividad'

 WHERE CATEGORY_ID = EXP_PROCESS_165_.tb0_0(1);
if not (sql%found) then
INSERT INTO WF_UNIT_CATEGORY(CATEGORY_ID,DISPLAY_NUMBER) 
VALUES (EXP_PROCESS_165_.tb0_0(1),
'Actividad'
);
end if;
...
EXP_PROCESS_165_.tb2_0(1):=283;
EXP_PROCESS_165_.tb2_1(1):=EXP_PROCESS_165_.tb0_0(1);
EXP_PROCESS_165_.tb2_2(1):=EXP_PROCESS_165_.tb1_0(0);
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT_TYPE fila (1)',1);
UPDATE WF_UNIT_TYPE SET UNIT_TYPE_ID=EXP_PROCESS_165_.tb2_0(1),
CATEGORY_ID=EXP_PROCESS_165_.tb2_1(1),
MODULE_ID=EXP_PROCESS_165_.tb2_2(1),
ACTION_ID=null,
PARENT_ID=983,
INIT_AREA_EXPRESSION_ID=null,
ASSIGN_COMMENT_CLASS=null,
...
ASSIGN_NOTIFICATION_ID=null,
ASSIGNED_USER_ONLY=null
 WHERE UNIT_TYPE_ID = EXP_PROCESS_165_.tb2_0(1);
if not (sql%found) then
INSERT INTO WF_UNIT_TYPE(UNIT_TYPE_ID,CATEGORY_ID,MODULE_ID,ACTION_ID,PARENT_ID,INIT_AREA_EXPRESSION_ID,ASSIGN_COMMENT_CLASS,ATTEND_COMMENT_CLASS,UNASSIGN_COMMENT_CLASS,TAG_NAME,DESCRIPTION,DISPLAY,ICON,IS_STAGE_PROCESS,USABLE_IN_FLOW_STAGE,ENTITY_ID,MULTI_INSTANCE,IS_COUNTABLE,NOTIFICATION_ID,VIEWABLE,DEFAULT_PRIORITY_ID,INITIAL_NOTIFY_TIME,IS_ADMIN_PROCESS,ASSIGN_NOTIFICATION_ID,ASSIGNED_USER_ONLY) 
VALUES (EXP_PROCESS_165_.tb2_0(1),
EXP_PROCESS_165_.tb2_1(1),
EXP_PROCESS_165_.tb2_2(1),
null,
983,
null,
null,
...
EXP_PROCESS_165_.tb4_0(1):=1;
ut_trace.trace('Actualizar o insertar tabla: WF_NODE_TYPE fila (1)',1);
UPDATE WF_NODE_TYPE SET NODE_TYPE_ID=EXP_PROCESS_165_.tb4_0(1),
DESCRIPTION='Arranque'

 WHERE NODE_TYPE_ID = EXP_PROCESS_165_.tb4_0(1);
if not (sql%found) then
INSERT INTO WF_NODE_TYPE(NODE_TYPE_ID,DESCRIPTION) 
VALUES (EXP_PROCESS_165_.tb4_0(1),
'Arranque'
);
end if;
...
EXP_PROCESS_165_.tb5_0(1):=260;
EXP_PROCESS_165_.tb5_1(1):=EXP_PROCESS_165_.tb5_0(0);
EXP_PROCESS_165_.tb5_2(1):=EXP_PROCESS_165_.tb2_0(1);
EXP_PROCESS_165_.tb5_3(1):=EXP_PROCESS_165_.tb4_0(1);
EXP_PROCESS_165_.tb5_4(1):=EXP_PROCESS_165_.tb1_0(0);
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT fila (1)',1);
UPDATE WF_UNIT SET UNIT_ID=EXP_PROCESS_165_.tb5_0(1),
PROCESS_ID=EXP_PROCESS_165_.tb5_1(1),
UNIT_TYPE_ID=EXP_PROCESS_165_.tb5_2(1),
NODE_TYPE_ID=EXP_PROCESS_165_.tb5_3(1),
MODULE_ID=EXP_PROCESS_165_.tb5_4(1),
ACTION_ID=null,
PRE_EXPRESSION_ID=null,
POS_EXPRESSION_ID=null,
NOTIFICATION_ID=null,
...
VIEWABLE='N'
 ...
 WHERE UNIT_ID = EXP_PROCESS_165_.tb5_0(1);
if not (sql%found) then
INSERT INTO WF_UNIT(UNIT_ID,PROCESS_ID,UNIT_TYPE_ID,NODE_TYPE_ID,MODULE_ID,ACTION_ID,PRE_EXPRESSION_ID,POS_EXPRESSION_ID,NOTIFICATION_ID,GEOMETRY,DESCRIPTION,ONLINE_EXEC_ID,MULTI_INSTANCE,SINCRONIC_TIMEOUT,ASINCRONIC_TIMEOUT,FUNCTION_TYPE,IS_COUNTABLE,MIN_GROUP_SIZE,EXECUTION_ORDER,ANNULATION_ORDER,ENTITY_ID,VIEWABLE) 
VALUES (EXP_PROCESS_165_.tb5_0(1),
EXP_PROCESS_165_.tb5_1(1),
EXP_PROCESS_165_.tb5_2(1),
EXP_PROCESS_165_.tb5_3(1),
EXP_PROCESS_165_.tb5_4(1),
null,
null,
null,
null,
...
EXP_PROCESS_165_.tb0_0(2):=1;
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT_CATEGORY fila (2)',1);
UPDATE WF_UNIT_CATEGORY SET CATEGORY_ID=EXP_PROCESS_165_.tb0_0(2),
DISPLAY_NUMBER='Proceso B¿sico'

 WHERE CATEGORY_ID = EXP_PROCESS_165_.tb0_0(2);
if not (sql%found) then
INSERT INTO WF_UNIT_CATEGORY(CATEGORY_ID,DISPLAY_NUMBER) 
VALUES (EXP_PROCESS_165_.tb0_0(2),
'Proceso B¿sico'
);
end if;
...
EXP_PROCESS_165_.tb2_0(2):=150;
EXP_PROCESS_165_.tb2_1(2):=EXP_PROCESS_165_.tb0_0(2);
EXP_PROCESS_165_.tb2_2(2):=EXP_PROCESS_165_.tb1_0(0);
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT_TYPE fila (2)',1);
UPDATE WF_UNIT_TYPE SET UNIT_TYPE_ID=EXP_PROCESS_165_.tb2_0(2),
CATEGORY_ID=EXP_PROCESS_165_.tb2_1(2),
MODULE_ID=EXP_PROCESS_165_.tb2_2(2),
ACTION_ID=null,
PARENT_ID=983,
INIT_AREA_EXPRESSION_ID=null,
ASSIGN_COMMENT_CLASS=null,
...
ASSIGN_NOTIFICATION_ID=null,
ASSIGNED_USER_ONLY=null
 WHERE UNIT_TYPE_ID = EXP_PROCESS_165_.tb2_0(2);
if not (sql%found) then
INSERT INTO WF_UNIT_TYPE(UNIT_TYPE_ID,CATEGORY_ID,MODULE_ID,ACTION_ID,PARENT_ID,INIT_AREA_EXPRESSION_ID,ASSIGN_COMMENT_CLASS,ATTEND_COMMENT_CLASS,UNASSIGN_COMMENT_CLASS,TAG_NAME,DESCRIPTION,DISPLAY,ICON,IS_STAGE_PROCESS,USABLE_IN_FLOW_STAGE,ENTITY_ID,MULTI_INSTANCE,IS_COUNTABLE,NOTIFICATION_ID,VIEWABLE,DEFAULT_PRIORITY_ID,INITIAL_NOTIFY_TIME,IS_ADMIN_PROCESS,ASSIGN_NOTIFICATION_ID,ASSIGNED_USER_ONLY) 
VALUES (EXP_PROCESS_165_.tb2_0(2),
EXP_PROCESS_165_.tb2_1(2),
EXP_PROCESS_165_.tb2_2(2),
null,
983,
null,
null,
...
EXP_PROCESS_165_.tb4_0(2):=2;
ut_trace.trace('Actualizar o insertar tabla: WF_NODE_TYPE fila (2)',1);
UPDATE WF_NODE_TYPE SET NODE_TYPE_ID=EXP_PROCESS_165_.tb4_0(2),
DESCRIPTION='Normal'

 WHERE NODE_TYPE_ID = EXP_PROCESS_165_.tb4_0(2);
if not (sql%found) then
INSERT INTO WF_NODE_TYPE(NODE_TYPE_ID,DESCRIPTION) 
VALUES (EXP_PROCESS_165_.tb4_0(2),
'Normal'
);
end if;
...
EXP_PROCESS_165_.tb5_0(2):=262;
EXP_PROCESS_165_.tb5_1(2):=EXP_PROCESS_165_.tb5_0(0);
EXP_PROCESS_165_.tb5_2(2):=EXP_PROCESS_165_.tb2_0(2);
EXP_PROCESS_165_.tb5_3(2):=EXP_PROCESS_165_.tb4_0(2);
EXP_PROCESS_165_.tb5_4(2):=EXP_PROCESS_165_.tb1_0(0);
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT fila (2)',1);
UPDATE WF_UNIT SET UNIT_ID=EXP_PROCESS_165_.tb5_0(2),
PROCESS_ID=EXP_PROCESS_165_.tb5_1(2),
UNIT_TYPE_ID=EXP_PROCESS_165_.tb5_2(2),
NODE_TYPE_ID=EXP_PROCESS_165_.tb5_3(2),
MODULE_ID=EXP_PROCESS_165_.tb5_4(2),
ACTION_ID=null,
PRE_EXPRESSION_ID=null,
POS_EXPRESSION_ID=null,
NOTIFICATION_ID=null,
...
VIEWABLE='N'
 ...
 WHERE UNIT_ID = EXP_PROCESS_165_.tb5_0(2);
if not (sql%found) then
INSERT INTO WF_UNIT(UNIT_ID,PROCESS_ID,UNIT_TYPE_ID,NODE_TYPE_ID,MODULE_ID,ACTION_ID,PRE_EXPRESSION_ID,POS_EXPRESSION_ID,NOTIFICATION_ID,GEOMETRY,DESCRIPTION,ONLINE_EXEC_ID,MULTI_INSTANCE,SINCRONIC_TIMEOUT,ASINCRONIC_TIMEOUT,FUNCTION_TYPE,IS_COUNTABLE,MIN_GROUP_SIZE,EXECUTION_ORDER,ANNULATION_ORDER,ENTITY_ID,VIEWABLE) 
VALUES (EXP_PROCESS_165_.tb5_0(2),
EXP_PROCESS_165_.tb5_1(2),
EXP_PROCESS_165_.tb5_2(2),
EXP_PROCESS_165_.tb5_3(2),
EXP_PROCESS_165_.tb5_4(2),
null,
null,
null,
null,
...
EXP_PROCESS_165_.tb6_0(0):=123583;
EXP_PROCESS_165_.tb6_1(0):=EXP_PROCESS_165_.tb5_0(1);
EXP_PROCESS_165_.tb6_2(0):=EXP_PROCESS_165_.tb5_0(2);
ut_trace.trace('Actualizar o insertar tabla: WF_TRANSITION fila (0)',1);
UPDATE WF_TRANSITION SET TRANS_ID=EXP_PROCESS_165_.tb6_0(0),
ORIGIN_ID=EXP_PROCESS_165_.tb6_1(0),
TARGET_ID=EXP_PROCESS_165_.tb6_2(0),
GEOMETRY=null,
GROUP_ID=0,
...
TRANSITION_TYPE_ID=1
 WHERE TRANS_ID = EXP_PROCESS_165_.tb6_0(0);
if not (sql%found) then
...
VALUES (EXP_PROCESS_165_.tb6_0(0),
EXP_PROCESS_165_.tb6_1(0),
EXP_PROCESS_165_.tb6_2(0),
null,
0,
'FLAG_VALIDATE == NO'
,
...
EXP_PROCESS_165_.tb2_0(3):=183;
EXP_PROCESS_165_.tb2_1(3):=EXP_PROCESS_165_.tb0_0(2);
EXP_PROCESS_165_.tb2_2(3):=EXP_PROCESS_165_.tb1_0(0);
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT_TYPE fila (3)',1);
UPDATE WF_UNIT_TYPE SET UNIT_TYPE_ID=EXP_PROCESS_165_.tb2_0(3),
CATEGORY_ID=EXP_PROCESS_165_.tb2_1(3),
MODULE_ID=EXP_PROCESS_165_.tb2_2(3),
ACTION_ID=null,
PARENT_ID=983,
INIT_AREA_EXPRESSION_ID=null,
ASSIGN_COMMENT_CLASS=null,
...
ASSIGN_NOTIFICATION_ID=null,
ASSIGNED_USER_ONLY=null
 WHERE UNIT_TYPE_ID = EXP_PROCESS_165_.tb2_0(3);
if not (sql%found) then
INSERT INTO WF_UNIT_TYPE(UNIT_TYPE_ID,CATEGORY_ID,MODULE_ID,ACTION_ID,PARENT_ID,INIT_AREA_EXPRESSION_ID,ASSIGN_COMMENT_CLASS,ATTEND_COMMENT_CLASS,UNASSIGN_COMMENT_CLASS,TAG_NAME,DESCRIPTION,DISPLAY,ICON,IS_STAGE_PROCESS,USABLE_IN_FLOW_STAGE,ENTITY_ID,MULTI_INSTANCE,IS_COUNTABLE,NOTIFICATION_ID,VIEWABLE,DEFAULT_PRIORITY_ID,INITIAL_NOTIFY_TIME,IS_ADMIN_PROCESS,ASSIGN_NOTIFICATION_ID,ASSIGNED_USER_ONLY) 
VALUES (EXP_PROCESS_165_.tb2_0(3),
EXP_PROCESS_165_.tb2_1(3),
EXP_PROCESS_165_.tb2_2(3),
null,
983,
null,
null,
...
EXP_PROCESS_165_.tb5_0(3):=300;
EXP_PROCESS_165_.tb5_1(3):=EXP_PROCESS_165_.tb5_0(0);
EXP_PROCESS_165_.tb5_2(3):=EXP_PROCESS_165_.tb2_0(3);
EXP_PROCESS_165_.tb5_3(3):=EXP_PROCESS_165_.tb4_0(2);
EXP_PROCESS_165_.tb5_4(3):=EXP_PROCESS_165_.tb1_0(0);
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT fila (3)',1);
UPDATE WF_UNIT SET UNIT_ID=EXP_PROCESS_165_.tb5_0(3),
PROCESS_ID=EXP_PROCESS_165_.tb5_1(3),
UNIT_TYPE_ID=EXP_PROCESS_165_.tb5_2(3),
NODE_TYPE_ID=EXP_PROCESS_165_.tb5_3(3),
MODULE_ID=EXP_PROCESS_165_.tb5_4(3),
ACTION_ID=null,
PRE_EXPRESSION_ID=null,
POS_EXPRESSION_ID=null,
NOTIFICATION_ID=null,
...
VIEWABLE='N'
 ...
 WHERE UNIT_ID = EXP_PROCESS_165_.tb5_0(3);
if not (sql%found) then
INSERT INTO WF_UNIT(UNIT_ID,PROCESS_ID,UNIT_TYPE_ID,NODE_TYPE_ID,MODULE_ID,ACTION_ID,PRE_EXPRESSION_ID,POS_EXPRESSION_ID,NOTIFICATION_ID,GEOMETRY,DESCRIPTION,ONLINE_EXEC_ID,MULTI_INSTANCE,SINCRONIC_TIMEOUT,ASINCRONIC_TIMEOUT,FUNCTION_TYPE,IS_COUNTABLE,MIN_GROUP_SIZE,EXECUTION_ORDER,ANNULATION_ORDER,ENTITY_ID,VIEWABLE) 
VALUES (EXP_PROCESS_165_.tb5_0(3),
EXP_PROCESS_165_.tb5_1(3),
EXP_PROCESS_165_.tb5_2(3),
EXP_PROCESS_165_.tb5_3(3),
EXP_PROCESS_165_.tb5_4(3),
null,
null,
null,
null,
...
EXP_PROCESS_165_.tb6_0(1):=123637;
EXP_PROCESS_165_.tb6_1(1):=EXP_PROCESS_165_.tb5_0(1);
EXP_PROCESS_165_.tb6_2(1):=EXP_PROCESS_165_.tb5_0(3);
ut_trace.trace('Actualizar o insertar tabla: WF_TRANSITION fila (1)',1);
UPDATE WF_TRANSITION SET TRANS_ID=EXP_PROCESS_165_.tb6_0(1),
ORIGIN_ID=EXP_PROCESS_165_.tb6_1(1),
TARGET_ID=EXP_PROCESS_165_.tb6_2(1),
GEOMETRY='63368510368'
,
GROUP_ID=0,
...
,
TRANSITION_TYPE_ID=1
 WHERE TRANS_ID = EXP_PROCESS_165_.tb6_0(1);
if not (sql%found) then
INSERT INTO WF_TRANSITION(TRANS_ID,ORIGIN_ID,TARGET_ID,GEOMETRY,GROUP_ID,EXPRESSION,EXPRESSION_TYPE,DESCRIPTION,TRANSITION_TYPE_ID) 
VALUES (EXP_PROCESS_165_.tb6_0(1),
EXP_PROCESS_165_.tb6_1(1),
EXP_PROCESS_165_.tb6_2(1),
'63368510368'
,
0,
'FLAG_VALIDATE == SI'
...
EXP_PROCESS_165_.tb9_0(0):=442;
EXP_PROCESS_165_.tb9_1(0):=EXP_PROCESS_165_.tb7_0(0);
EXP_PROCESS_165_.tb9_2(0):=EXP_PROCESS_165_.tb8_0(0);
ut_trace.trace('Actualizar o insertar tabla: GE_ATTRIBUTES fila (0)',1);
UPDATE GE_ATTRIBUTES SET ATTRIBUTE_ID=EXP_PROCESS_165_.tb9_0(0),
ATTRIBUTE_CLASS_ID=EXP_PROCESS_165_.tb9_1(0),
ATTRIBUTE_TYPE_ID=EXP_PROCESS_165_.tb9_2(0),
...
DISPLAY_NAME='Bandera de validaci¿n para la ejecuci¿n de transicciones'
 ...
 WHERE ATTRIBUTE_ID = EXP_PROCESS_165_.tb9_0(0);
if not (sql%found) then
...
VALUES (EXP_PROCESS_165_.tb9_0(0),
EXP_PROCESS_165_.tb9_1(0),
EXP_PROCESS_165_.tb9_2(0),
null,
null,
9,
'FLAG_VALIDATE'
...
EXP_PROCESS_165_.old_tb10_0(0):=120002095;
EXP_PROCESS_165_.tb10_0(0):=GE_BOSEQUENCE.NEXTGE_STATEMENT;
EXP_PROCESS_165_.tb10_0(0):=EXP_PROCESS_165_.tb10_0(0);
ut_trace.trace('insertando tabla: GE_STATEMENT fila (0)',1);
INSERT INTO GE_STATEMENT(STATEMENT_ID,MODULE_ID,DESCRIPTION,STATEMENT,NAME) 
VALUES (EXP_PROCESS_165_.tb10_0(0),
5,
...
EXP_PROCESS_165_.tb11_0(0):=133381;
EXP_PROCESS_165_.tb11_1(0):=EXP_PROCESS_165_.tb5_0(1);
EXP_PROCESS_165_.tb11_2(0):=EXP_PROCESS_165_.tb9_0(0);
EXP_PROCESS_165_.tb11_3(0):=EXP_PROCESS_165_.tb10_0(0);
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT_ATTRIBUTE fila (0)',1);
UPDATE WF_UNIT_ATTRIBUTE SET UNIT_ATTRIBUTE_ID=EXP_PROCESS_165_.tb11_0(0),
UNIT_ID=EXP_PROCESS_165_.tb11_1(0),
ATTRIBUTE_ID=EXP_PROCESS_165_.tb11_2(0),
STATEMENT_ID=EXP_PROCESS_165_.tb11_3(0),
VALUE=null,
IS_DUPLICABLE='N'
,
IN_OUT=2,
MANDATORY='Y'
 ...
 WHERE UNIT_ATTRIBUTE_ID = EXP_PROCESS_165_.tb11_0(0);
if not (sql%found) then
INSERT INTO WF_UNIT_ATTRIBUTE(UNIT_ATTRIBUTE_ID,UNIT_ID,ATTRIBUTE_ID,STATEMENT_ID,VALUE,IS_DUPLICABLE,IN_OUT,MANDATORY) 
VALUES (EXP_PROCESS_165_.tb11_0(0),
EXP_PROCESS_165_.tb11_1(0),
EXP_PROCESS_165_.tb11_2(0),
EXP_PROCESS_165_.tb11_3(0),
null,
'N'
,
2,
...
EXP_PROCESS_165_.tb7_0(1):=1;
ut_trace.trace('Actualizar o insertar tabla: GE_ATTRIBUTE_CLASS fila (1)',1);
UPDATE GE_ATTRIBUTE_CLASS SET ATTRIBUTE_CLASS_ID=EXP_PROCESS_165_.tb7_0(1),
NAME='De Entitdad'
,
DESCRIPTION='Valores que ser¿n tra¿dos de la tabla base usando old record'

 WHERE ATTRIBUTE_CLASS_ID = EXP_PROCESS_165_.tb7_0(1);
if not (sql%found) then
INSERT INTO GE_ATTRIBUTE_CLASS(ATTRIBUTE_CLASS_ID,NAME,DESCRIPTION) 
VALUES (EXP_PROCESS_165_.tb7_0(1),
'De Entitdad'
,
...
EXP_PROCESS_165_.tb9_0(1):=400;
EXP_PROCESS_165_.tb9_1(1):=EXP_PROCESS_165_.tb7_0(1);
EXP_PROCESS_165_.tb9_2(1):=EXP_PROCESS_165_.tb8_0(0);
ut_trace.trace('Actualizar o insertar tabla: GE_ATTRIBUTES fila (1)',1);
UPDATE GE_ATTRIBUTES SET ATTRIBUTE_ID=EXP_PROCESS_165_.tb9_0(1),
ATTRIBUTE_CLASS_ID=EXP_PROCESS_165_.tb9_1(1),
ATTRIBUTE_TYPE_ID=EXP_PROCESS_165_.tb9_2(1),
VALID_EXPRESSION=null,
FATHER_ID=null,
MODULE_ID=9,
...
DISPLAY_NAME='Causal'
 ...
 WHERE ATTRIBUTE_ID = EXP_PROCESS_165_.tb9_0(1);
if not (sql%found) then
INSERT INTO GE_ATTRIBUTES(ATTRIBUTE_ID,ATTRIBUTE_CLASS_ID,ATTRIBUTE_TYPE_ID,VALID_EXPRESSION,FATHER_ID,MODULE_ID,NAME_ATTRIBUTE,LENGTH,PRECISION,SCALE,DEFAULT_VALUE,IS_FIX_OR_VARIABLE,COMMENT_,DISPLAY_NAME) 
VALUES (EXP_PROCESS_165_.tb9_0(1),
EXP_PROCESS_165_.tb9_1(1),
EXP_PROCESS_165_.tb9_2(1),
null,
null,
9,
'CAUSAL'
...
EXP_PROCESS_165_.tb11_0(1):=133292;
EXP_PROCESS_165_.tb11_1(1):=EXP_PROCESS_165_.tb5_0(1);
EXP_PROCESS_165_.tb11_2(1):=EXP_PROCESS_165_.tb9_0(1);
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT_ATTRIBUTE fila (1)',1);
UPDATE WF_UNIT_ATTRIBUTE SET UNIT_ATTRIBUTE_ID=EXP_PROCESS_165_.tb11_0(1),
UNIT_ID=EXP_PROCESS_165_.tb11_1(1),
ATTRIBUTE_ID=EXP_PROCESS_165_.tb11_2(1),
STATEMENT_ID=null,
VALUE=null,
IS_DUPLICABLE='N'
,
...
MANDATORY='N'
 ...
 WHERE UNIT_ATTRIBUTE_ID = EXP_PROCESS_165_.tb11_0(1);
if not (sql%found) then
INSERT INTO WF_UNIT_ATTRIBUTE(UNIT_ATTRIBUTE_ID,UNIT_ID,ATTRIBUTE_ID,STATEMENT_ID,VALUE,IS_DUPLICABLE,IN_OUT,MANDATORY) 
VALUES (EXP_PROCESS_165_.tb11_0(1),
EXP_PROCESS_165_.tb11_1(1),
EXP_PROCESS_165_.tb11_2(1),
null,
null,
'N'
,
...
EXP_PROCESS_165_.tb6_0(2):=123635;
EXP_PROCESS_165_.tb6_1(2):=EXP_PROCESS_165_.tb5_0(2);
EXP_PROCESS_165_.tb6_2(2):=EXP_PROCESS_165_.tb5_0(3);
ut_trace.trace('Actualizar o insertar tabla: WF_TRANSITION fila (2)',1);
UPDATE WF_TRANSITION SET TRANS_ID=EXP_PROCESS_165_.tb6_0(2),
ORIGIN_ID=EXP_PROCESS_165_.tb6_1(2),
TARGET_ID=EXP_PROCESS_165_.tb6_2(2),
GEOMETRY=null,
GROUP_ID=0,
EXPRESSION=null,
EXPRESSION_TYPE=0,
DESCRIPTION=null,
TRANSITION_TYPE_ID=1
 WHERE TRANS_ID = EXP_PROCESS_165_.tb6_0(2);
if not (sql%found) then
INSERT INTO WF_TRANSITION(TRANS_ID,ORIGIN_ID,TARGET_ID,GEOMETRY,GROUP_ID,EXPRESSION,EXPRESSION_TYPE,DESCRIPTION,TRANSITION_TYPE_ID) 
VALUES (EXP_PROCESS_165_.tb6_0(2),
EXP_PROCESS_165_.tb6_1(2),
EXP_PROCESS_165_.tb6_2(2),
null,
0,
null,
0,
...
EXP_PROCESS_165_.tb2_0(4):=252;
EXP_PROCESS_165_.tb2_1(4):=EXP_PROCESS_165_.tb0_0(1);
EXP_PROCESS_165_.tb2_2(4):=EXP_PROCESS_165_.tb1_0(0);
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT_TYPE fila (4)',1);
UPDATE WF_UNIT_TYPE SET UNIT_TYPE_ID=EXP_PROCESS_165_.tb2_0(4),
CATEGORY_ID=EXP_PROCESS_165_.tb2_1(4),
MODULE_ID=EXP_PROCESS_165_.tb2_2(4),
ACTION_ID=null,
PARENT_ID=983,
INIT_AREA_EXPRESSION_ID=null,
ASSIGN_COMMENT_CLASS=null,
...
ASSIGN_NOTIFICATION_ID=null,
ASSIGNED_USER_ONLY=null
 WHERE UNIT_TYPE_ID = EXP_PROCESS_165_.tb2_0(4);
if not (sql%found) then
INSERT INTO WF_UNIT_TYPE(UNIT_TYPE_ID,CATEGORY_ID,MODULE_ID,ACTION_ID,PARENT_ID,INIT_AREA_EXPRESSION_ID,ASSIGN_COMMENT_CLASS,ATTEND_COMMENT_CLASS,UNASSIGN_COMMENT_CLASS,TAG_NAME,DESCRIPTION,DISPLAY,ICON,IS_STAGE_PROCESS,USABLE_IN_FLOW_STAGE,ENTITY_ID,MULTI_INSTANCE,IS_COUNTABLE,NOTIFICATION_ID,VIEWABLE,DEFAULT_PRIORITY_ID,INITIAL_NOTIFY_TIME,IS_ADMIN_PROCESS,ASSIGN_NOTIFICATION_ID,ASSIGNED_USER_ONLY) 
VALUES (EXP_PROCESS_165_.tb2_0(4),
EXP_PROCESS_165_.tb2_1(4),
EXP_PROCESS_165_.tb2_2(4),
null,
983,
null,
null,
...
EXP_PROCESS_165_.tb4_0(3):=3;
ut_trace.trace('Actualizar o insertar tabla: WF_NODE_TYPE fila (3)',1);
UPDATE WF_NODE_TYPE SET NODE_TYPE_ID=EXP_PROCESS_165_.tb4_0(3),
DESCRIPTION='Final'

 WHERE NODE_TYPE_ID = EXP_PROCESS_165_.tb4_0(3);
if not (sql%found) then
INSERT INTO WF_NODE_TYPE(NODE_TYPE_ID,DESCRIPTION) 
VALUES (EXP_PROCESS_165_.tb4_0(3),
'Final'
);
end if;
...
EXP_PROCESS_165_.tb5_0(4):=261;
EXP_PROCESS_165_.tb5_1(4):=EXP_PROCESS_165_.tb5_0(0);
EXP_PROCESS_165_.tb5_2(4):=EXP_PROCESS_165_.tb2_0(4);
EXP_PROCESS_165_.tb5_3(4):=EXP_PROCESS_165_.tb4_0(3);
EXP_PROCESS_165_.tb5_4(4):=EXP_PROCESS_165_.tb1_0(0);
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT fila (4)',1);
UPDATE WF_UNIT SET UNIT_ID=EXP_PROCESS_165_.tb5_0(4),
PROCESS_ID=EXP_PROCESS_165_.tb5_1(4),
UNIT_TYPE_ID=EXP_PROCESS_165_.tb5_2(4),
NODE_TYPE_ID=EXP_PROCESS_165_.tb5_3(4),
MODULE_ID=EXP_PROCESS_165_.tb5_4(4),
ACTION_ID=null,
PRE_EXPRESSION_ID=null,
POS_EXPRESSION_ID=null,
NOTIFICATION_ID=null,
...
VIEWABLE='N'
 ...
 WHERE UNIT_ID = EXP_PROCESS_165_.tb5_0(4);
if not (sql%found) then
INSERT INTO WF_UNIT(UNIT_ID,PROCESS_ID,UNIT_TYPE_ID,NODE_TYPE_ID,MODULE_ID,ACTION_ID,PRE_EXPRESSION_ID,POS_EXPRESSION_ID,NOTIFICATION_ID,GEOMETRY,DESCRIPTION,ONLINE_EXEC_ID,MULTI_INSTANCE,SINCRONIC_TIMEOUT,ASINCRONIC_TIMEOUT,FUNCTION_TYPE,IS_COUNTABLE,MIN_GROUP_SIZE,EXECUTION_ORDER,ANNULATION_ORDER,ENTITY_ID,VIEWABLE) 
VALUES (EXP_PROCESS_165_.tb5_0(4),
EXP_PROCESS_165_.tb5_1(4),
EXP_PROCESS_165_.tb5_2(4),
EXP_PROCESS_165_.tb5_3(4),
EXP_PROCESS_165_.tb5_4(4),
null,
null,
null,
null,
...
EXP_PROCESS_165_.tb6_0(3):=123636;
EXP_PROCESS_165_.tb6_1(3):=EXP_PROCESS_165_.tb5_0(3);
EXP_PROCESS_165_.tb6_2(3):=EXP_PROCESS_165_.tb5_0(4);
ut_trace.trace('Actualizar o insertar tabla: WF_TRANSITION fila (3)',1);
UPDATE WF_TRANSITION SET TRANS_ID=EXP_PROCESS_165_.tb6_0(3),
ORIGIN_ID=EXP_PROCESS_165_.tb6_1(3),
TARGET_ID=EXP_PROCESS_165_.tb6_2(3),
GEOMETRY=null,
GROUP_ID=0,
EXPRESSION=null,
EXPRESSION_TYPE=0,
DESCRIPTION=null,
TRANSITION_TYPE_ID=1
 WHERE TRANS_ID = EXP_PROCESS_165_.tb6_0(3);
if not (sql%found) then
INSERT INTO WF_TRANSITION(TRANS_ID,ORIGIN_ID,TARGET_ID,GEOMETRY,GROUP_ID,EXPRESSION,EXPRESSION_TYPE,DESCRIPTION,TRANSITION_TYPE_ID) 
VALUES (EXP_PROCESS_165_.tb6_0(3),
EXP_PROCESS_165_.tb6_1(3),
EXP_PROCESS_165_.tb6_2(3),
null,
0,
null,
0,
...
EXP_PROCESS_165_.tb11_0(2):=133293;
EXP_PROCESS_165_.tb11_1(2):=EXP_PROCESS_165_.tb5_0(4);
EXP_PROCESS_165_.tb11_2(2):=EXP_PROCESS_165_.tb9_0(1);
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT_ATTRIBUTE fila (2)',1);
UPDATE WF_UNIT_ATTRIBUTE SET UNIT_ATTRIBUTE_ID=EXP_PROCESS_165_.tb11_0(2),
UNIT_ID=EXP_PROCESS_165_.tb11_1(2),
ATTRIBUTE_ID=EXP_PROCESS_165_.tb11_2(2),
STATEMENT_ID=null,
VALUE=null,
IS_DUPLICABLE='N'
,
...
MANDATORY='N'
 ...
 WHERE UNIT_ATTRIBUTE_ID = EXP_PROCESS_165_.tb11_0(2);
if not (sql%found) then
INSERT INTO WF_UNIT_ATTRIBUTE(UNIT_ATTRIBUTE_ID,UNIT_ID,ATTRIBUTE_ID,STATEMENT_ID,VALUE,IS_DUPLICABLE,IN_OUT,MANDATORY) 
VALUES (EXP_PROCESS_165_.tb11_0(2),
EXP_PROCESS_165_.tb11_1(2),
EXP_PROCESS_165_.tb11_2(2),
null,
null,
'N'
,
...
EXP_PROCESS_165_.tb11_0(3):=133294;
EXP_PROCESS_165_.tb11_1(3):=EXP_PROCESS_165_.tb5_0(4);
EXP_PROCESS_165_.tb11_2(3):=EXP_PROCESS_165_.tb9_0(1);
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT_ATTRIBUTE fila (3)',1);
UPDATE WF_UNIT_ATTRIBUTE SET UNIT_ATTRIBUTE_ID=EXP_PROCESS_165_.tb11_0(3),
UNIT_ID=EXP_PROCESS_165_.tb11_1(3),
ATTRIBUTE_ID=EXP_PROCESS_165_.tb11_2(3),
STATEMENT_ID=null,
VALUE=null,
IS_DUPLICABLE='N'
,
...
MANDATORY='N'
 ...
 WHERE UNIT_ATTRIBUTE_ID = EXP_PROCESS_165_.tb11_0(3);
if not (sql%found) then
INSERT INTO WF_UNIT_ATTRIBUTE(UNIT_ATTRIBUTE_ID,UNIT_ID,ATTRIBUTE_ID,STATEMENT_ID,VALUE,IS_DUPLICABLE,IN_OUT,MANDATORY) 
VALUES (EXP_PROCESS_165_.tb11_0(3),
EXP_PROCESS_165_.tb11_1(3),
EXP_PROCESS_165_.tb11_2(3),
null,
null,
'N'
,
...
BEGIN
sa_bocreatePackages.CreatePackage('EXP_UNITTYPE_165_',
'CREATE OR REPLACE PACKAGE EXP_UNITTYPE_165_ IS ' || chr(10) ||
'blProcessStatus boolean := true; ' || chr(10) ||
'type tyWF_UNIT_TYPERowId is table of rowid index by varchar2(100); ' || chr(10) ||
...
'tbObjectToDelete tyObjectToDelete; ' || chr(10) ||
' ' || chr(10) ||
'END EXP_UNITTYPE_165_;');
END;
/


BEGIN ut_trace.trace('********************Comenzar proceso de objeto:EXP_UNITTYPE_165_******************************'); END;
/

BEGIN
ut_trace.trace('Actualizar Variable de Proceso: EXP_UNITTYPE_165',1);
EXP_UNITTYPE_165_.blProcessStatus := EXP_PROCESS_165_.blProcessStatus ; 
exception when others then
rollback;
ut_trace.trace('**ERROR:'||sqlerrm,1);
raise;
...
if (not EXP_UNITTYPE_165_.blProcessStatus) then
 return;
end if;
 nuIndex := 1;
 FOR crObjectsName IN cuObjectsName LOOP
  EXP_UNITTYPE_165_.tbObjectToDelete(nuIndex) := crObjectsName.OBJECT_NAME;
  nuIndex := nuIndex + 1;
 END LOOP;
EXCEPTION 
 when ex.controlled_error then
...
CURSOR cuRowIdToDelete is
SELECT rowid
FROM WF_UNIT_TYPE_ATTRIB WHERE (UNIT_TYPE_ID) in (SELECT UNIT_TYPE_ID FROM WF_UNIT_TYPE WHERE UNIT_TYPE_ID=165);
nuIndex binary_integer;
BEGIN
...
exception when others then
EXP_UNITTYPE_165_.blProcessStatus := false;
rollback;
ut_trace.trace('**ERROR:'||sqlerrm);
raise;
END; 
...
CURSOR cuRowIdToDelete is
SELECT rowid
FROM WF_CAUSAL_UNIT_TYPE WHERE (UNIT_TYPE_ID) in (SELECT UNIT_TYPE_ID FROM WF_UNIT_TYPE WHERE UNIT_TYPE_ID=165);
nuIndex binary_integer;
BEGIN
...
CURSOR cuRowIdToDelete is
SELECT rowid
FROM MO_TIME_UNI_TYP_PRIO WHERE (UNIT_TYPE_ID) in (SELECT UNIT_TYPE_ID FROM WF_UNIT_TYPE WHERE UNIT_TYPE_ID=165);
nuIndex binary_integer;
BEGIN
...
CURSOR cuRowIdToDelete is
SELECT rowid
FROM OR_ACT_BY_TASK_MOD WHERE (TASK_CODE) in (SELECT UNIT_TYPE_ID FROM WF_UNIT_TYPE WHERE UNIT_TYPE_ID=165);
nuIndex binary_integer;
BEGIN
...
BEGIN
ut_trace.trace('Actualizar Variable de Proceso: EXP_PROCESS_165',1);
EXP_PROCESS_165_.blProcessStatus := EXP_UNITTYPE_165_.blProcessStatus ; 
exception when others then
rollback;
ut_trace.trace('**ERROR:'||sqlerrm,1);
raise;
...
BEGIN
 ut_trace.trace('Inicia borrado de objetos de reglas',1);
 nuIndex := EXP_UNITTYPE_165_.tbObjectToDelete.first;
 while (nuIndex is not null) LOOP
  BEGIN
   ut_trace.trace('Objeto a borrar: ' || EXP_UNITTYPE_165_.tbObjectToDelete(nuIndex),1);
   IF(ut_object.fboExistObject(EXP_UNITTYPE_165_.tbObjectToDelete(nuIndex)) and not fblIsObjectUseByRule(EXP_UNITTYPE_165_.tbObjectToDelete(nuIndex))) THEN
    blObjectDeleted := ut_object.fboDeleteObject(EXP_UNITTYPE_165_.tbObjectToDelete(nuIndex));
    ut_trace.trace('  * Borrado ' || EXP_UNITTYPE_165_.tbObjectToDelete(nuIndex), 1);
   END IF;
  EXCEPTION when others then
   ut_trace.trace('- No se borra el objeto (' || EXP_UNITTYPE_165_.tbObjectToDelete(nuIndex) || '): ' || sqlerrm,1);
  END;
  nuIndex := EXP_UNITTYPE_165_.tbObjectToDelete.next(nuIndex);
 END loop;
 ut_trace.trace('Finaliza borrado de objetos de reglas',1);
EXCEPTION 
 when ex.controlled_error then
...
begin
SA_BOCreatePackages.DropPackage('EXP_UNITTYPE_165_');
end;
/

BEGIN ut_trace.trace('********************FIN  proceso de objeto:EXP_UNITTYPE_165_******************************'); end;
/
...
EXP_UNITTYPE_283_.blProcessStatus := EXP_PROCESS_165_.blProcessStatus ; 
exception when others then
rollback;
ut_trace.trace('**ERROR:'||sqlerrm,1);
raise;
...
BEGIN
ut_trace.trace('Actualizar Variable de Proceso: EXP_PROCESS_165',1);
EXP_PROCESS_165_.blProcessStatus := EXP_UNITTYPE_283_.blProcessStatus ; 
exception when others then
rollback;
ut_trace.trace('**ERROR:'||sqlerrm,1);
raise;
...
EXP_UNITTYPE_252_.blProcessStatus := EXP_PROCESS_165_.blProcessStatus ; 
exception when others then
rollback;
ut_trace.trace('**ERROR:'||sqlerrm,1);
raise;
...
BEGIN
ut_trace.trace('Actualizar Variable de Proceso: EXP_PROCESS_165',1);
EXP_PROCESS_165_.blProcessStatus := EXP_UNITTYPE_252_.blProcessStatus ; 
exception when others then
rollback;
ut_trace.trace('**ERROR:'||sqlerrm,1);
raise;
...
EXP_UNITTYPE_150_.blProcessStatus := EXP_PROCESS_165_.blProcessStatus ; 
exception when others then
rollback;
ut_trace.trace('**ERROR:'||sqlerrm,1);
raise;
...
BEGIN
ut_trace.trace('Actualizar Variable de Proceso: EXP_PROCESS_165',1);
EXP_PROCESS_165_.blProcessStatus := EXP_UNITTYPE_150_.blProcessStatus ; 
exception when others then
rollback;
ut_trace.trace('**ERROR:'||sqlerrm,1);
raise;
...
EXP_UNITTYPE_183_.blProcessStatus := EXP_PROCESS_165_.blProcessStatus ; 
exception when others then
rollback;
ut_trace.trace('**ERROR:'||sqlerrm,1);
raise;
...
BEGIN
ut_trace.trace('Actualizar Variable de Proceso: EXP_PROCESS_165',1);
EXP_PROCESS_165_.blProcessStatus := EXP_UNITTYPE_183_.blProcessStatus ; 
exception when others then
rollback;
ut_trace.trace('**ERROR:'||sqlerrm,1);
raise;
...
EXP_PROCESS_150_.blProcessStatus := EXP_PROCESS_165_.blProcessStatus ; 
exception when others then
rollback;
ut_trace.trace('**ERROR:'||sqlerrm,1);
raise;
...
to_date('07-07-2016 23:17:29','DD-MM-YYYY HH24:MI:SS'),
to_date('07-07-2016 23:17:29','DD-MM-YYYY HH24:MI:SS'),
'G'
,
'N'
...
to_date('07-07-2016 23:17:30','DD-MM-YYYY HH24:MI:SS'),
to_date('07-07-2016 23:17:30','DD-MM-YYYY HH24:MI:SS'),
'G'
,
'N'
...
EXP_PROCESS_150_.old_tb10_0(0):=120108187;
...
to_date('02-08-2016 20:54:42','DD-MM-YYYY HH24:MI:SS'),
to_date('02-08-2016 20:54:42','DD-MM-YYYY HH24:MI:SS'),
'G'
,
'N'
...
to_date('02-08-2016 20:54:43','DD-MM-YYYY HH24:MI:SS'),
to_date('02-08-2016 20:54:43','DD-MM-YYYY HH24:MI:SS'),
'G'
,
'N'
...
BEGIN
ut_trace.trace('Actualizar Variable de Proceso: EXP_PROCESS_165',1);
EXP_PROCESS_165_.blProcessStatus := EXP_PROCESS_150_.blProcessStatus ; 
exception when others then
rollback;
ut_trace.trace('**ERROR:'||sqlerrm,1);
raise;
...
EXP_PROCESS_183_.blProcessStatus := EXP_PROCESS_165_.blProcessStatus ; 
exception when others then
rollback;
ut_trace.trace('**ERROR:'||sqlerrm,1);
raise;
...
to_date('07-07-2016 23:16:01','DD-MM-YYYY HH24:MI:SS'),
to_date('07-07-2016 23:16:01','DD-MM-YYYY HH24:MI:SS'),
'G'
,
'N'
...
BEGIN
ut_trace.trace('Actualizar Variable de Proceso: EXP_PROCESS_165',1);
EXP_PROCESS_165_.blProcessStatus := EXP_PROCESS_183_.blProcessStatus ; 
exception when others then
rollback;
ut_trace.trace('**ERROR:'||sqlerrm,1);
raise;
...
ut_trace.trace('Realizar Commit de EXP_PROCESS_165 ',1);
COMMIT;
exception when others then
rollback;
ut_trace.trace('**ERROR:'||sqlerrm,1);
...
DECLARE 
    nuBinaryIndex Binary_Integer;
BEGIN 
    nuBinaryIndex := EXP_PROCESS_165_.tbGR_CONFIG_EXPRESSIONId.first;
    while (nuBinaryIndex IS not null) loop 
       BEGIN
           delete from GR_CONFIG_EXPRESSION where config_expression_id = EXP_PROCESS_165_.tbGR_CONFIG_EXPRESSIONId(nuBinaryIndex);
           ut_trace.trace('Delete config_expression_id = : '||EXP_PROCESS_165_.tbGR_CONFIG_EXPRESSIONId(nuBinaryIndex) || ' OK', 1);
       EXCEPTION
           when ex.RECORD_HAVE_CHILDREN then
               ut_trace.trace('No se pudo borrar la regla config_expression_id '||EXP_PROCESS_165_.tbGR_CONFIG_EXPRESSIONId(nuBinaryIndex),1);
       END;
       nuBinaryIndex := EXP_PROCESS_165_.tbGR_CONFIG_EXPRESSIONId.next(nuBinaryIndex);
    END loop;
...
BEGIN
 ut_trace.trace('Inicia borrado de objetos de reglas',1);
 nuIndex := EXP_PROCESS_165_.tbObjectToDelete.first;
 while (nuIndex is not null) LOOP
  BEGIN
   ut_trace.trace('Objeto a borrar: ' || EXP_PROCESS_165_.tbObjectToDelete(nuIndex),1);
   IF(ut_object.fboExistObject(EXP_PROCESS_165_.tbObjectToDelete(nuIndex)) and not fblIsObjectUseByRule(EXP_PROCESS_165_.tbObjectToDelete(nuIndex))) THEN
    blObjectDeleted := ut_object.fboDeleteObject(EXP_PROCESS_165_.tbObjectToDelete(nuIndex));
    ut_trace.trace('  * Borrado ' || EXP_PROCESS_165_.tbObjectToDelete(nuIndex), 1);
   END IF;
  EXCEPTION when others then
   ut_trace.trace('- No se borra el objeto (' || EXP_PROCESS_165_.tbObjectToDelete(nuIndex) || '): ' || sqlerrm,1);
  END;
  nuIndex := EXP_PROCESS_165_.tbObjectToDelete.next(nuIndex);
 END loop;
 ut_trace.trace('Finaliza borrado de objetos de reglas',1);
EXCEPTION 
 when ex.controlled_error then
...
begin
SA_BOCreatePackages.DropPackage('EXP_PROCESS_165_');
end;
/

BEGIN ut_trace.trace('********************FIN  proceso de objeto:EXP_PROCESS_165_******************************'); end;
/
