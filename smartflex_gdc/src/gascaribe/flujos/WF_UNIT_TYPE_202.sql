...
BEGIN
sa_bocreatePackages.CreatePackage('EXP_PROCESS_202_',
'CREATE OR REPLACE PACKAGE EXP_PROCESS_202_ IS ' || chr(10) ||
...
' ' || chr(10) ||
'END EXP_PROCESS_202_;');
END;
/


BEGIN ut_trace.trace('********************Comenzar proceso de objeto:EXP_PROCESS_202_******************************'); END;
/


BEGIN
   ut_trace.trace('Cargando tabla en memoria para posterior borrado GR_CONFIG_EXPRESSION',1);
   open EXP_PROCESS_202_.cuExpression;
   fetch EXP_PROCESS_202_.cuExpression bulk collect INTO EXP_PROCESS_202_.tbGR_CONFIG_EXPRESSIONId;
   close EXP_PROCESS_202_.cuExpression;
END;
/ 
BEGIN
...
BEGIN
sa_bocreatePackages.CreatePackage('DEL_ROOT_348_',
'CREATE OR REPLACE PACKAGE DEL_ROOT_348_ IS ' || chr(10) ||
'blProcessStatus boolean := true; ' || chr(10) ||
'type tyWF_UNIT_TYPERowId is table of rowid index by varchar2(100); ' || chr(10) ||
'tbWF_UNIT_TYPERowId tyWF_UNIT_TYPERowId;type tyWF_UNIT_CATEGORYRowId is table of rowid index by varchar2(100); ' || chr(10) ||
'tbWF_UNIT_CATEGORYRowId tyWF_UNIT_CATEGORYRowId;type tyWF_ATTRIBUTES_EQUIVRowId is table of rowid index by varchar2(100); ' || chr(10) ||
...
'WHERE   UNIT_ID IN ( ' || chr(10) ||
'        SELECT UNIT_ID         FROM WF_UNIT  ' || chr(10) ||
'        WHERE UNIT_ID = 348 ' || chr(10) ||
'        UNION ALL         SELECT UNIT_ID         FROM WF_UNIT  ' || chr(10) ||
'        WHERE PROCESS_ID = 348  ' || chr(10) ||
'       ) ' || chr(10) ||
'AND   PRE_EXPRESSION_ID IS NOT NULL ' || chr(10) ||
'UNION ALL  ' || chr(10) ||
'SELECT POS_EXPRESSION_ID ' || chr(10) ||
...
BEGIN ut_trace.trace('********************Comenzar proceso de objeto:DEL_ROOT_348_******************************'); END;
/


BEGIN
   ut_trace.trace('Cargando tabla en memoria para posterior borrado GR_CONFIG_EXPRESSION',1);
   open DEL_ROOT_348_.cuExpression;
   fetch DEL_ROOT_348_.cuExpression bulk collect INTO DEL_ROOT_348_.tbGR_CONFIG_EXPRESSIONId;
   close DEL_ROOT_348_.cuExpression;
END;
/ 
...
AND     A.UNIT_ID IN (
        ...
        WHERE PROCESS_ID = 348 
       )
;
BEGIN

if (not DEL_ROOT_348_.blProcessStatus) then
 return;
end if;
 nuIndex := 1;
 FOR crObjectsName IN cuObjectsName LOOP
  DEL_ROOT_348_.tbObjectToDelete(nuIndex) := crObjectsName.OBJECT_NAME;
  nuIndex := nuIndex + 1;
 END LOOP;
EXCEPTION 
...
DECLARE 
 ...
 SELECT rowid FROM GR_CONFIG_EXPRESSION WHERE (CONFIG_EXPRESSION_ID) in (SELECT PRE_EXPRESSION_ID FROM WF_UNIT WHERE PROCESS_ID = 348);
BEGIN 
...
DEL_ROOT_348_.tbGR_CONFIG_EXPRESSIONRowId(rcData.rowid):=rcData.rowid; 
END loop; 

exception when others then
DEL_ROOT_348_.blProcessStatus := false;
rollback;
...
DECLARE 
 CURSOR cuLoadTemporaryTable IS 
 SELECT rowid FROM GR_CONFIG_EXPRESSION WHERE (CONFIG_EXPRESSION_ID) in (SELECT POS_EXPRESSION_ID FROM WF_UNIT WHERE PROCESS_ID = 348);
BEGIN 
...
DECLARE 
 CURSOR cuLoadTemporaryTable IS 
 SELECT rowid FROM GR_CONFIG_EXPRESSION WHERE (CONFIG_EXPRESSION_ID) in (SELECT PRE_EXPRESSION_ID FROM WF_UNIT WHERE (UNIT_ID) in (SELECT TARGET_ID FROM WF_TRANSITION WHERE (TARGET_ID) in (SELECT UNIT_ID FROM WF_UNIT WHERE PROCESS_ID = 348)));
BEGIN 
...
DECLARE 
 CURSOR cuLoadTemporaryTable IS 
 SELECT rowid FROM GR_CONFIG_EXPRESSION WHERE (CONFIG_EXPRESSION_ID) in (SELECT POS_EXPRESSION_ID FROM WF_UNIT WHERE (UNIT_ID) in (SELECT TARGET_ID FROM WF_TRANSITION WHERE (TARGET_ID) in (SELECT UNIT_ID FROM WF_UNIT WHERE PROCESS_ID = 348)));
BEGIN 
...
DECLARE 
 CURSOR cuLoadTemporaryTable IS 
 SELECT rowid FROM WF_UNIT WHERE (UNIT_ID) in (SELECT TARGET_ID FROM WF_TRANSITION WHERE (TARGET_ID) in (SELECT UNIT_ID FROM WF_UNIT WHERE PROCESS_ID = 348));
BEGIN 
...
for rcData in cuLoadTemporaryTable loop
DEL_ROOT_348_.tbWF_UNITRowId(rcData.rowid):=rcData.rowid; 
END loop; 
...
SELECT rowid
FROM WF_TRANSITION WHERE (TARGET_ID) in (SELECT UNIT_ID FROM WF_UNIT WHERE PROCESS_ID = 348);
nuIndex binary_integer;
BEGIN
...
nuVarcharIndex:=DEL_ROOT_348_.tbGR_CONFIG_EXPRESSIONRowId.first;  
while (nuVarcharIndex is not null) loop 
begin
ut_trace.trace('Row id a borrar:'||nuVarcharIndex,1);
delete from GR_CONFIG_EXPRESSION where rowid = DEL_ROOT_348_.tbGR_CONFIG_EXPRESSIONRowId(nuVarcharIndex); 
...
DEL_ROOT_348_.tbGR_CONFIG_EXPRESSIONRowId.delete(nuOldVarcharIndex);
end if;
nuVarcharIndex := DEL_ROOT_348_.tbGR_CONFIG_EXPRESSIONRowId.next(nuVarcharIndex); 
END loop; 
...
nuVarcharIndex:=DEL_ROOT_348_.tbWF_UNITRowId.first;  
while (nuVarcharIndex is not null) loop 
begin
ut_trace.trace('Row id a borrar:'||nuVarcharIndex,1);
delete from WF_UNIT where rowid = DEL_ROOT_348_.tbWF_UNITRowId(nuVarcharIndex); 
nuOldVarcharIndex:=nuVarcharIndex;
EXCEPTION
when ex.RECORD_HAVE_CHILDREN then
ut_trace.trace('No se pudo borrar el registro '||nuVarcharIndex,1);
...
END;
if nuOldVarcharIndex is not null then
DEL_ROOT_348_.tbWF_UNITRowId.delete(nuOldVarcharIndex);
end if;
nuVarcharIndex := DEL_ROOT_348_.tbWF_UNITRowId.next(nuVarcharIndex); 
END loop; 
...
CURSOR cuRowIdToDelete is
SELECT rowid
FROM WF_UNIT_DATA_MAP WHERE (SOURCE_ID) in (SELECT UNIT_ATTRIBUTE_ID FROM WF_UNIT_ATTRIBUTE WHERE (UNIT_ID) in (SELECT UNIT_ID FROM WF_UNIT WHERE PROCESS_ID = 348));
nuIndex binary_integer;
BEGIN
...
CURSOR cuRowIdToDelete is
SELECT rowid
FROM WF_UNIT_DATA_MAP WHERE (TARGET_ID) in (SELECT UNIT_ATTRIBUTE_ID FROM WF_UNIT_ATTRIBUTE WHERE (UNIT_ID) in (SELECT UNIT_ID FROM WF_UNIT WHERE PROCESS_ID = 348));
nuIndex binary_integer;
BEGIN
...
CURSOR cuRowIdToDelete is
SELECT rowid
FROM WF_UNIT_ATTRIBUTE WHERE (UNIT_ID) in (SELECT UNIT_ID FROM WF_UNIT WHERE PROCESS_ID = 348);
nuIndex binary_integer;
BEGIN
...
CURSOR cuRowIdToDelete is
SELECT rowid
FROM WF_UNIT WHERE PROCESS_ID = 348;
nuIndex binary_integer;
BEGIN
...
BEGIN 
    nuBinaryIndex := DEL_ROOT_348_.tbGR_CONFIG_EXPRESSIONId.first;
    while (nuBinaryIndex IS not null) loop 
       BEGIN
           delete from GR_CONFIG_EXPRESSION where config_expression_id = DEL_ROOT_348_.tbGR_CONFIG_EXPRESSIONId(nuBinaryIndex);
           ut_trace.trace('Delete config_expression_id = : '||DEL_ROOT_348_.tbGR_CONFIG_EXPRESSIONId(nuBinaryIndex) || ' OK', 1);
       EXCEPTION
           when ex.RECORD_HAVE_CHILDREN then
               ut_trace.trace('No se pudo borrar la regla config_expression_id '||DEL_ROOT_348_.tbGR_CONFIG_EXPRESSIONId(nuBinaryIndex),1);
       END;
       nuBinaryIndex := DEL_ROOT_348_.tbGR_CONFIG_EXPRESSIONId.next(nuBinaryIndex);
    END loop;
...
/ 
BEGIN
ut_trace.trace('Actualizar Variable de Proceso: EXP_PROCESS_202',1);
EXP_PROCESS_202_.blProcessStatus := DEL_ROOT_348_.blProcessStatus ; 
exception when others then
rollback;
ut_trace.trace('**ERROR:'||sqlerrm,1);
raise;
...
BEGIN
 ...
 nuIndex := DEL_ROOT_348_.tbObjectToDelete.first;
 while (nuIndex is not null) LOOP
  BEGIN
   ut_trace.trace('Objeto a borrar: ' || DEL_ROOT_348_.tbObjectToDelete(nuIndex),1);
   IF(ut_object.fboExistObject(DEL_ROOT_348_.tbObjectToDelete(nuIndex)) and not fblIsObjectUseByRule(DEL_ROOT_348_.tbObjectToDelete(nuIndex))) THEN
    blObjectDeleted := ut_object.fboDeleteObject(DEL_ROOT_348_.tbObjectToDelete(nuIndex));
    ut_trace.trace('  * Borrado ' || DEL_ROOT_348_.tbObjectToDelete(nuIndex), 1);
   END IF;
  EXCEPTION when others then
   ut_trace.trace('- No se borra el objeto (' || DEL_ROOT_348_.tbObjectToDelete(nuIndex) || '): ' || sqlerrm,1);
  END;
  nuIndex := DEL_ROOT_348_.tbObjectToDelete.next(nuIndex);
 END loop;
...
begin
SA_BOCreatePackages.DropPackage('DEL_ROOT_348_');
end;
/

BEGIN ut_trace.trace('********************FIN  proceso de objeto:DEL_ROOT_348_******************************'); end;
/
...
DECLARE 
 CURSOR cuLoadTemporaryTable IS 
 SELECT rowid FROM GR_CONFIG_EXPRESSION WHERE (CONFIG_EXPRESSION_ID) in (SELECT PRE_EXPRESSION_ID FROM WF_UNIT WHERE UNIT_ID = 348);
BEGIN 
...
DECLARE 
 CURSOR cuLoadTemporaryTable IS 
 SELECT rowid FROM GR_CONFIG_EXPRESSION WHERE (CONFIG_EXPRESSION_ID) in (SELECT POS_EXPRESSION_ID FROM WF_UNIT WHERE UNIT_ID = 348);
BEGIN 
...
DECLARE 
 CURSOR cuLoadTemporaryTable IS 
 SELECT rowid FROM GR_CONFIG_EXPRESSION WHERE (CONFIG_EXPRESSION_ID) in (SELECT PRE_EXPRESSION_ID FROM WF_UNIT WHERE (UNIT_ID) in (SELECT TARGET_ID FROM WF_TRANSITION WHERE (TARGET_ID) in (SELECT UNIT_ID FROM WF_UNIT WHERE UNIT_ID = 348)));
BEGIN 
...
DECLARE 
 CURSOR cuLoadTemporaryTable IS 
 SELECT rowid FROM GR_CONFIG_EXPRESSION WHERE (CONFIG_EXPRESSION_ID) in (SELECT POS_EXPRESSION_ID FROM WF_UNIT WHERE (UNIT_ID) in (SELECT TARGET_ID FROM WF_TRANSITION WHERE (TARGET_ID) in (SELECT UNIT_ID FROM WF_UNIT WHERE UNIT_ID = 348)));
BEGIN 
...
DECLARE 
 CURSOR cuLoadTemporaryTable IS 
 SELECT rowid FROM WF_UNIT WHERE (UNIT_ID) in (SELECT TARGET_ID FROM WF_TRANSITION WHERE (TARGET_ID) in (SELECT UNIT_ID FROM WF_UNIT WHERE UNIT_ID = 348));
BEGIN 
...
CURSOR cuRowIdToDelete is
SELECT rowid
FROM WF_TRANSITION WHERE (TARGET_ID) in (SELECT UNIT_ID FROM WF_UNIT WHERE UNIT_ID = 348);
nuIndex binary_integer;
BEGIN
...
CURSOR cuRowIdToDelete is
SELECT rowid
FROM WF_UNIT_DATA_MAP WHERE (SOURCE_ID) in (SELECT UNIT_ATTRIBUTE_ID FROM WF_UNIT_ATTRIBUTE WHERE (UNIT_ID) in (SELECT UNIT_ID FROM WF_UNIT WHERE UNIT_ID = 348));
nuIndex binary_integer;
BEGIN
...
CURSOR cuRowIdToDelete is
SELECT rowid
FROM WF_UNIT_DATA_MAP WHERE (TARGET_ID) in (SELECT UNIT_ATTRIBUTE_ID FROM WF_UNIT_ATTRIBUTE WHERE (UNIT_ID) in (SELECT UNIT_ID FROM WF_UNIT WHERE UNIT_ID = 348));
nuIndex binary_integer;
BEGIN
...
CURSOR cuRowIdToDelete is
SELECT rowid
FROM WF_UNIT_ATTRIBUTE WHERE (UNIT_ID) in (SELECT UNIT_ID FROM WF_UNIT WHERE UNIT_ID = 348);
nuIndex binary_integer;
BEGIN
...
CURSOR cuRowIdToDelete is
SELECT rowid
FROM WF_UNIT WHERE UNIT_ID = 348;
nuIndex binary_integer;
BEGIN
...
if (not EXP_PROCESS_202_.blProcessStatus) then
 return;
end if;
 nuIndex := 1;
 FOR crObjectsName IN cuObjectsName LOOP
  EXP_PROCESS_202_.tbObjectToDelete(nuIndex) := crObjectsName.OBJECT_NAME;
  nuIndex := nuIndex + 1;
 END LOOP;
EXCEPTION 
 when ex.controlled_error then
...
DECLARE 
 CURSOR cuLoadTemporaryTable IS 
 SELECT rowid FROM GR_CONFIG_EXPRESSION WHERE (CONFIG_EXPRESSION_ID) in (SELECT PRE_EXPRESSION_ID FROM WF_UNIT WHERE (UNIT_TYPE_ID) in (SELECT UNIT_TYPE_ID FROM WF_UNIT_TYPE WHERE UNIT_TYPE_ID=202) AND NODE_TYPE_ID=0);
BEGIN 
...
ut_trace.trace('Cargando a memoria tabla GR_CONFIG_EXPRESSION',1);
for rcData in cuLoadTemporaryTable loop
EXP_PROCESS_202_.tbGR_CONFIG_EXPRESSIONRowId(rcData.rowid):=rcData.rowid; 
END loop; 

exception when others then
EXP_PROCESS_202_.blProcessStatus := false;
rollback;
ut_trace.trace('**ERROR:'||sqlerrm);
raise;
END; 
...
DECLARE 
 CURSOR cuLoadTemporaryTable IS 
 SELECT rowid FROM GR_CONFIG_EXPRESSION WHERE (CONFIG_EXPRESSION_ID) in (SELECT POS_EXPRESSION_ID FROM WF_UNIT WHERE (UNIT_TYPE_ID) in (SELECT UNIT_TYPE_ID FROM WF_UNIT_TYPE WHERE UNIT_TYPE_ID=202) AND NODE_TYPE_ID=0);
BEGIN 
...
CURSOR cuRowIdToDelete is
SELECT rowid
FROM WF_UNIT WHERE (UNIT_TYPE_ID) in (SELECT UNIT_TYPE_ID FROM WF_UNIT_TYPE WHERE UNIT_TYPE_ID=202) AND NODE_TYPE_ID=0;
nuIndex binary_integer;
BEGIN
...
ut_trace.trace('Borrando tabla cargada en memoria GR_CONFIG_EXPRESSION',1);
nuVarcharIndex:=EXP_PROCESS_202_.tbGR_CONFIG_EXPRESSIONRowId.first;  
while (nuVarcharIndex is not null) loop 
begin
ut_trace.trace('Row id a borrar:'||nuVarcharIndex,1);
delete from GR_CONFIG_EXPRESSION where rowid = EXP_PROCESS_202_.tbGR_CONFIG_EXPRESSIONRowId(nuVarcharIndex); 
nuOldVarcharIndex:=nuVarcharIndex;
EXCEPTION
when ex.RECORD_HAVE_CHILDREN then
ut_trace.trace('No se pudo borrar el registro '||nuVarcharIndex,1);
...
END;
if nuOldVarcharIndex is not null then
EXP_PROCESS_202_.tbGR_CONFIG_EXPRESSIONRowId.delete(nuOldVarcharIndex);
end if;
nuVarcharIndex := EXP_PROCESS_202_.tbGR_CONFIG_EXPRESSIONRowId.next(nuVarcharIndex); 
END loop; 
...
CURSOR cuRowIdToDelete is
SELECT rowid
FROM WF_UNIT_TYPE WHERE UNIT_TYPE_ID=202;
nuIndex binary_integer;
BEGIN
...
EXP_PROCESS_202_.tb0_0(0):=3;
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT_CATEGORY fila (0)',1);
UPDATE WF_UNIT_CATEGORY SET CATEGORY_ID=EXP_PROCESS_202_.tb0_0(0),
DISPLAY_NUMBER='Proceso Principal'

 WHERE CATEGORY_ID = EXP_PROCESS_202_.tb0_0(0);
if not (sql%found) then
INSERT INTO WF_UNIT_CATEGORY(CATEGORY_ID,DISPLAY_NUMBER) 
VALUES (EXP_PROCESS_202_.tb0_0(0),
'Proceso Principal'
);
end if;
...
EXP_PROCESS_202_.tb1_0(0):=9;
ut_trace.trace('Actualizar o insertar tabla: GE_MODULE fila (0)',1);
UPDATE GE_MODULE SET MODULE_ID=EXP_PROCESS_202_.tb1_0(0),
...
LOCALIZATION='IN'
 ...
 WHERE MODULE_ID = EXP_PROCESS_202_.tb1_0(0);
if not (sql%found) then
INSERT INTO GE_MODULE(MODULE_ID,DESCRIPTION,MNEMONIC,LAST_MESSAGE,PATH_MODULE,ICON_NAME,LOCALIZATION) 
VALUES (EXP_PROCESS_202_.tb1_0(0),
'WorkFlow'
,
'WF'
,
...
EXP_PROCESS_202_.tb2_0(0):=202;
EXP_PROCESS_202_.tb2_1(0):=EXP_PROCESS_202_.tb0_0(0);
EXP_PROCESS_202_.tb2_2(0):=EXP_PROCESS_202_.tb1_0(0);
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT_TYPE fila (0)',1);
UPDATE WF_UNIT_TYPE SET UNIT_TYPE_ID=EXP_PROCESS_202_.tb2_0(0),
CATEGORY_ID=EXP_PROCESS_202_.tb2_1(0),
MODULE_ID=EXP_PROCESS_202_.tb2_2(0),
ACTION_ID=null,
PARENT_ID=983,
...
TAG_NAME='UNIT_TYPE_202'
,
DESCRIPTION='Gestion de Recurso de Reposición con Subsidio de Apelación'
,
DISPLAY='Gestion de Recurso de Reposición con Subsidio de Apelación'
,
ICON=null,
IS_STAGE_PROCESS='N'
,
...
ASSIGNED_USER_ONLY=null
 WHERE UNIT_TYPE_ID = EXP_PROCESS_202_.tb2_0(0);
if not (sql%found) then
...
VALUES (EXP_PROCESS_202_.tb2_0(0),
EXP_PROCESS_202_.tb2_1(0),
EXP_PROCESS_202_.tb2_2(0),
null,
983,
null,
null,
...
'UNIT_TYPE_202'
,
'Gestion de Recurso de Reposición con Subsidio de Apelación'
,
'Gestion de Recurso de Reposición con Subsidio de Apelación'
,
null,
...
EXP_PROCESS_202_.tb3_0(0):=24;
EXP_PROCESS_202_.tb3_1(0):=EXP_PROCESS_202_.tb2_0(0);
ut_trace.trace('Actualizar o insertar tabla: WF_ATTRIBUTES_EQUIV fila (0)',1);
UPDATE WF_ATTRIBUTES_EQUIV SET ATTRIBUTES_EQUIV_ID=EXP_PROCESS_202_.tb3_0(0),
UNIT_TYPE_ID=EXP_PROCESS_202_.tb3_1(0),
INTERFACE_CONFIG_ID=21,
STD_TIME=0,
MAX_TIME=31536000,
AVG_TIME=0,
DESCRIPTION='Recurso de Reposición con Subsidio de Apelación'
,
VALUE_1='52'
,
VALUE_2=null,
...
VALUE_19=null,
VALUE_20=null
 WHERE ATTRIBUTES_EQUIV_ID = EXP_PROCESS_202_.tb3_0(0);
if not (sql%found) then
...
VALUES (EXP_PROCESS_202_.tb3_0(0),
EXP_PROCESS_202_.tb3_1(0),
21,
0,
31536000,
0,
...
EXP_PROCESS_202_.tb4_0(0):=0;
ut_trace.trace('Actualizar o insertar tabla: WF_NODE_TYPE fila (0)',1);
UPDATE WF_NODE_TYPE SET NODE_TYPE_ID=EXP_PROCESS_202_.tb4_0(0),
DESCRIPTION='Ra¿z'

 WHERE NODE_TYPE_ID = EXP_PROCESS_202_.tb4_0(0);
if not (sql%found) then
INSERT INTO WF_NODE_TYPE(NODE_TYPE_ID,DESCRIPTION) 
VALUES (EXP_PROCESS_202_.tb4_0(0),
'Ra¿z'
);
end if;
...
EXP_PROCESS_202_.tb5_0(0):=348;
EXP_PROCESS_202_.tb5_2(0):=EXP_PROCESS_202_.tb2_0(0);
EXP_PROCESS_202_.tb5_3(0):=EXP_PROCESS_202_.tb4_0(0);
EXP_PROCESS_202_.tb5_4(0):=EXP_PROCESS_202_.tb1_0(0);
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT fila (0)',1);
UPDATE WF_UNIT SET UNIT_ID=EXP_PROCESS_202_.tb5_0(0),
PROCESS_ID=null,
UNIT_TYPE_ID=EXP_PROCESS_202_.tb5_2(0),
NODE_TYPE_ID=EXP_PROCESS_202_.tb5_3(0),
MODULE_ID=EXP_PROCESS_202_.tb5_4(0),
ACTION_ID=null,
...
VIEWABLE='N'
 ...
 WHERE UNIT_ID = EXP_PROCESS_202_.tb5_0(0);
if not (sql%found) then
...
VALUES (EXP_PROCESS_202_.tb5_0(0),
null,
EXP_PROCESS_202_.tb5_2(0),
EXP_PROCESS_202_.tb5_3(0),
EXP_PROCESS_202_.tb5_4(0),
null,
null,
null,
null,
...
EXP_PROCESS_202_.tb0_0(1):=2;
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT_CATEGORY fila (1)',1);
UPDATE WF_UNIT_CATEGORY SET CATEGORY_ID=EXP_PROCESS_202_.tb0_0(1),
DISPLAY_NUMBER='Actividad'

 WHERE CATEGORY_ID = EXP_PROCESS_202_.tb0_0(1);
if not (sql%found) then
INSERT INTO WF_UNIT_CATEGORY(CATEGORY_ID,DISPLAY_NUMBER) 
VALUES (EXP_PROCESS_202_.tb0_0(1),
'Actividad'
);
end if;
...
EXP_PROCESS_202_.tb2_0(1):=283;
EXP_PROCESS_202_.tb2_1(1):=EXP_PROCESS_202_.tb0_0(1);
EXP_PROCESS_202_.tb2_2(1):=EXP_PROCESS_202_.tb1_0(0);
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT_TYPE fila (1)',1);
UPDATE WF_UNIT_TYPE SET UNIT_TYPE_ID=EXP_PROCESS_202_.tb2_0(1),
CATEGORY_ID=EXP_PROCESS_202_.tb2_1(1),
MODULE_ID=EXP_PROCESS_202_.tb2_2(1),
ACTION_ID=null,
PARENT_ID=983,
INIT_AREA_EXPRESSION_ID=null,
ASSIGN_COMMENT_CLASS=null,
...
ASSIGN_NOTIFICATION_ID=null,
ASSIGNED_USER_ONLY=null
 WHERE UNIT_TYPE_ID = EXP_PROCESS_202_.tb2_0(1);
if not (sql%found) then
INSERT INTO WF_UNIT_TYPE(UNIT_TYPE_ID,CATEGORY_ID,MODULE_ID,ACTION_ID,PARENT_ID,INIT_AREA_EXPRESSION_ID,ASSIGN_COMMENT_CLASS,ATTEND_COMMENT_CLASS,UNASSIGN_COMMENT_CLASS,TAG_NAME,DESCRIPTION,DISPLAY,ICON,IS_STAGE_PROCESS,USABLE_IN_FLOW_STAGE,ENTITY_ID,MULTI_INSTANCE,IS_COUNTABLE,NOTIFICATION_ID,VIEWABLE,DEFAULT_PRIORITY_ID,INITIAL_NOTIFY_TIME,IS_ADMIN_PROCESS,ASSIGN_NOTIFICATION_ID,ASSIGNED_USER_ONLY) 
VALUES (EXP_PROCESS_202_.tb2_0(1),
EXP_PROCESS_202_.tb2_1(1),
EXP_PROCESS_202_.tb2_2(1),
null,
983,
null,
null,
...
EXP_PROCESS_202_.tb4_0(1):=1;
ut_trace.trace('Actualizar o insertar tabla: WF_NODE_TYPE fila (1)',1);
UPDATE WF_NODE_TYPE SET NODE_TYPE_ID=EXP_PROCESS_202_.tb4_0(1),
DESCRIPTION='Arranque'

 WHERE NODE_TYPE_ID = EXP_PROCESS_202_.tb4_0(1);
if not (sql%found) then
INSERT INTO WF_NODE_TYPE(NODE_TYPE_ID,DESCRIPTION) 
VALUES (EXP_PROCESS_202_.tb4_0(1),
'Arranque'
);
end if;
...
EXP_PROCESS_202_.tb5_0(1):=349;
EXP_PROCESS_202_.tb5_1(1):=EXP_PROCESS_202_.tb5_0(0);
EXP_PROCESS_202_.tb5_2(1):=EXP_PROCESS_202_.tb2_0(1);
EXP_PROCESS_202_.tb5_3(1):=EXP_PROCESS_202_.tb4_0(1);
EXP_PROCESS_202_.tb5_4(1):=EXP_PROCESS_202_.tb1_0(0);
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT fila (1)',1);
UPDATE WF_UNIT SET UNIT_ID=EXP_PROCESS_202_.tb5_0(1),
PROCESS_ID=EXP_PROCESS_202_.tb5_1(1),
UNIT_TYPE_ID=EXP_PROCESS_202_.tb5_2(1),
NODE_TYPE_ID=EXP_PROCESS_202_.tb5_3(1),
MODULE_ID=EXP_PROCESS_202_.tb5_4(1),
ACTION_ID=null,
PRE_EXPRESSION_ID=null,
POS_EXPRESSION_ID=null,
NOTIFICATION_ID=null,
...
VIEWABLE='N'
 ...
 WHERE UNIT_ID = EXP_PROCESS_202_.tb5_0(1);
if not (sql%found) then
INSERT INTO WF_UNIT(UNIT_ID,PROCESS_ID,UNIT_TYPE_ID,NODE_TYPE_ID,MODULE_ID,ACTION_ID,PRE_EXPRESSION_ID,POS_EXPRESSION_ID,NOTIFICATION_ID,GEOMETRY,DESCRIPTION,ONLINE_EXEC_ID,MULTI_INSTANCE,SINCRONIC_TIMEOUT,ASINCRONIC_TIMEOUT,FUNCTION_TYPE,IS_COUNTABLE,MIN_GROUP_SIZE,EXECUTION_ORDER,ANNULATION_ORDER,ENTITY_ID,VIEWABLE) 
VALUES (EXP_PROCESS_202_.tb5_0(1),
EXP_PROCESS_202_.tb5_1(1),
EXP_PROCESS_202_.tb5_2(1),
EXP_PROCESS_202_.tb5_3(1),
EXP_PROCESS_202_.tb5_4(1),
null,
null,
null,
null,
...
EXP_PROCESS_202_.tb0_0(2):=1;
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT_CATEGORY fila (2)',1);
UPDATE WF_UNIT_CATEGORY SET CATEGORY_ID=EXP_PROCESS_202_.tb0_0(2),
DISPLAY_NUMBER='Proceso B¿sico'

 WHERE CATEGORY_ID = EXP_PROCESS_202_.tb0_0(2);
if not (sql%found) then
INSERT INTO WF_UNIT_CATEGORY(CATEGORY_ID,DISPLAY_NUMBER) 
VALUES (EXP_PROCESS_202_.tb0_0(2),
'Proceso B¿sico'
);
end if;
...
EXP_PROCESS_202_.tb2_0(2):=183;
EXP_PROCESS_202_.tb2_1(2):=EXP_PROCESS_202_.tb0_0(2);
EXP_PROCESS_202_.tb2_2(2):=EXP_PROCESS_202_.tb1_0(0);
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT_TYPE fila (2)',1);
UPDATE WF_UNIT_TYPE SET UNIT_TYPE_ID=EXP_PROCESS_202_.tb2_0(2),
CATEGORY_ID=EXP_PROCESS_202_.tb2_1(2),
MODULE_ID=EXP_PROCESS_202_.tb2_2(2),
ACTION_ID=null,
PARENT_ID=983,
INIT_AREA_EXPRESSION_ID=null,
ASSIGN_COMMENT_CLASS=null,
...
ASSIGN_NOTIFICATION_ID=null,
ASSIGNED_USER_ONLY=null
 WHERE UNIT_TYPE_ID = EXP_PROCESS_202_.tb2_0(2);
if not (sql%found) then
INSERT INTO WF_UNIT_TYPE(UNIT_TYPE_ID,CATEGORY_ID,MODULE_ID,ACTION_ID,PARENT_ID,INIT_AREA_EXPRESSION_ID,ASSIGN_COMMENT_CLASS,ATTEND_COMMENT_CLASS,UNASSIGN_COMMENT_CLASS,TAG_NAME,DESCRIPTION,DISPLAY,ICON,IS_STAGE_PROCESS,USABLE_IN_FLOW_STAGE,ENTITY_ID,MULTI_INSTANCE,IS_COUNTABLE,NOTIFICATION_ID,VIEWABLE,DEFAULT_PRIORITY_ID,INITIAL_NOTIFY_TIME,IS_ADMIN_PROCESS,ASSIGN_NOTIFICATION_ID,ASSIGNED_USER_ONLY) 
VALUES (EXP_PROCESS_202_.tb2_0(2),
EXP_PROCESS_202_.tb2_1(2),
EXP_PROCESS_202_.tb2_2(2),
null,
983,
null,
null,
...
EXP_PROCESS_202_.tb4_0(2):=2;
ut_trace.trace('Actualizar o insertar tabla: WF_NODE_TYPE fila (2)',1);
UPDATE WF_NODE_TYPE SET NODE_TYPE_ID=EXP_PROCESS_202_.tb4_0(2),
DESCRIPTION='Normal'

 WHERE NODE_TYPE_ID = EXP_PROCESS_202_.tb4_0(2);
if not (sql%found) then
INSERT INTO WF_NODE_TYPE(NODE_TYPE_ID,DESCRIPTION) 
VALUES (EXP_PROCESS_202_.tb4_0(2),
'Normal'
);
end if;
...
EXP_PROCESS_202_.tb5_0(2):=357;
EXP_PROCESS_202_.tb5_1(2):=EXP_PROCESS_202_.tb5_0(0);
EXP_PROCESS_202_.tb5_2(2):=EXP_PROCESS_202_.tb2_0(2);
EXP_PROCESS_202_.tb5_3(2):=EXP_PROCESS_202_.tb4_0(2);
EXP_PROCESS_202_.tb5_4(2):=EXP_PROCESS_202_.tb1_0(0);
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT fila (2)',1);
UPDATE WF_UNIT SET UNIT_ID=EXP_PROCESS_202_.tb5_0(2),
PROCESS_ID=EXP_PROCESS_202_.tb5_1(2),
UNIT_TYPE_ID=EXP_PROCESS_202_.tb5_2(2),
NODE_TYPE_ID=EXP_PROCESS_202_.tb5_3(2),
MODULE_ID=EXP_PROCESS_202_.tb5_4(2),
ACTION_ID=null,
PRE_EXPRESSION_ID=null,
POS_EXPRESSION_ID=null,
NOTIFICATION_ID=null,
...
VIEWABLE='N'
 ...
 WHERE UNIT_ID = EXP_PROCESS_202_.tb5_0(2);
if not (sql%found) then
INSERT INTO WF_UNIT(UNIT_ID,PROCESS_ID,UNIT_TYPE_ID,NODE_TYPE_ID,MODULE_ID,ACTION_ID,PRE_EXPRESSION_ID,POS_EXPRESSION_ID,NOTIFICATION_ID,GEOMETRY,DESCRIPTION,ONLINE_EXEC_ID,MULTI_INSTANCE,SINCRONIC_TIMEOUT,ASINCRONIC_TIMEOUT,FUNCTION_TYPE,IS_COUNTABLE,MIN_GROUP_SIZE,EXECUTION_ORDER,ANNULATION_ORDER,ENTITY_ID,VIEWABLE) 
VALUES (EXP_PROCESS_202_.tb5_0(2),
EXP_PROCESS_202_.tb5_1(2),
EXP_PROCESS_202_.tb5_2(2),
EXP_PROCESS_202_.tb5_3(2),
EXP_PROCESS_202_.tb5_4(2),
null,
null,
null,
null,
...
EXP_PROCESS_202_.tb6_0(0):=123760;
EXP_PROCESS_202_.tb6_1(0):=EXP_PROCESS_202_.tb5_0(1);
EXP_PROCESS_202_.tb6_2(0):=EXP_PROCESS_202_.tb5_0(2);
ut_trace.trace('Actualizar o insertar tabla: WF_TRANSITION fila (0)',1);
UPDATE WF_TRANSITION SET TRANS_ID=EXP_PROCESS_202_.tb6_0(0),
ORIGIN_ID=EXP_PROCESS_202_.tb6_1(0),
TARGET_ID=EXP_PROCESS_202_.tb6_2(0),
GEOMETRY='64
124
722
124'
...
TRANSITION_TYPE_ID=1
 WHERE TRANS_ID = EXP_PROCESS_202_.tb6_0(0);
if not (sql%found) then
...
VALUES (EXP_PROCESS_202_.tb6_0(0),
EXP_PROCESS_202_.tb6_1(0),
EXP_PROCESS_202_.tb6_2(0),
'64
124
722
124'
...
EXP_PROCESS_202_.tb2_0(3):=166;
EXP_PROCESS_202_.tb2_1(3):=EXP_PROCESS_202_.tb0_0(2);
EXP_PROCESS_202_.tb2_2(3):=EXP_PROCESS_202_.tb1_0(0);
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT_TYPE fila (3)',1);
UPDATE WF_UNIT_TYPE SET UNIT_TYPE_ID=EXP_PROCESS_202_.tb2_0(3),
CATEGORY_ID=EXP_PROCESS_202_.tb2_1(3),
MODULE_ID=EXP_PROCESS_202_.tb2_2(3),
ACTION_ID=null,
PARENT_ID=983,
INIT_AREA_EXPRESSION_ID=null,
ASSIGN_COMMENT_CLASS=null,
...
ASSIGN_NOTIFICATION_ID=null,
ASSIGNED_USER_ONLY=null
 WHERE UNIT_TYPE_ID = EXP_PROCESS_202_.tb2_0(3);
if not (sql%found) then
INSERT INTO WF_UNIT_TYPE(UNIT_TYPE_ID,CATEGORY_ID,MODULE_ID,ACTION_ID,PARENT_ID,INIT_AREA_EXPRESSION_ID,ASSIGN_COMMENT_CLASS,ATTEND_COMMENT_CLASS,UNASSIGN_COMMENT_CLASS,TAG_NAME,DESCRIPTION,DISPLAY,ICON,IS_STAGE_PROCESS,USABLE_IN_FLOW_STAGE,ENTITY_ID,MULTI_INSTANCE,IS_COUNTABLE,NOTIFICATION_ID,VIEWABLE,DEFAULT_PRIORITY_ID,INITIAL_NOTIFY_TIME,IS_ADMIN_PROCESS,ASSIGN_NOTIFICATION_ID,ASSIGNED_USER_ONLY) 
VALUES (EXP_PROCESS_202_.tb2_0(3),
EXP_PROCESS_202_.tb2_1(3),
EXP_PROCESS_202_.tb2_2(3),
null,
983,
null,
null,
...
EXP_PROCESS_202_.tb5_0(3):=352;
EXP_PROCESS_202_.tb5_1(3):=EXP_PROCESS_202_.tb5_0(0);
EXP_PROCESS_202_.tb5_2(3):=EXP_PROCESS_202_.tb2_0(3);
EXP_PROCESS_202_.tb5_3(3):=EXP_PROCESS_202_.tb4_0(2);
EXP_PROCESS_202_.tb5_4(3):=EXP_PROCESS_202_.tb1_0(0);
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT fila (3)',1);
UPDATE WF_UNIT SET UNIT_ID=EXP_PROCESS_202_.tb5_0(3),
PROCESS_ID=EXP_PROCESS_202_.tb5_1(3),
UNIT_TYPE_ID=EXP_PROCESS_202_.tb5_2(3),
NODE_TYPE_ID=EXP_PROCESS_202_.tb5_3(3),
MODULE_ID=EXP_PROCESS_202_.tb5_4(3),
ACTION_ID=null,
PRE_EXPRESSION_ID=null,
POS_EXPRESSION_ID=null,
NOTIFICATION_ID=null,
...
VIEWABLE='N'
 ...
 WHERE UNIT_ID = EXP_PROCESS_202_.tb5_0(3);
if not (sql%found) then
INSERT INTO WF_UNIT(UNIT_ID,PROCESS_ID,UNIT_TYPE_ID,NODE_TYPE_ID,MODULE_ID,ACTION_ID,PRE_EXPRESSION_ID,POS_EXPRESSION_ID,NOTIFICATION_ID,GEOMETRY,DESCRIPTION,ONLINE_EXEC_ID,MULTI_INSTANCE,SINCRONIC_TIMEOUT,ASINCRONIC_TIMEOUT,FUNCTION_TYPE,IS_COUNTABLE,MIN_GROUP_SIZE,EXECUTION_ORDER,ANNULATION_ORDER,ENTITY_ID,VIEWABLE) 
VALUES (EXP_PROCESS_202_.tb5_0(3),
EXP_PROCESS_202_.tb5_1(3),
EXP_PROCESS_202_.tb5_2(3),
EXP_PROCESS_202_.tb5_3(3),
EXP_PROCESS_202_.tb5_4(3),
null,
null,
null,
null,
...
EXP_PROCESS_202_.tb6_0(1):=100001065;
EXP_PROCESS_202_.tb6_1(1):=EXP_PROCESS_202_.tb5_0(1);
EXP_PROCESS_202_.tb6_2(1):=EXP_PROCESS_202_.tb5_0(3);
ut_trace.trace('Actualizar o insertar tabla: WF_TRANSITION fila (1)',1);
UPDATE WF_TRANSITION SET TRANS_ID=EXP_PROCESS_202_.tb6_0(1),
ORIGIN_ID=EXP_PROCESS_202_.tb6_1(1),
TARGET_ID=EXP_PROCESS_202_.tb6_2(1),
GEOMETRY=null,
GROUP_ID=0,
...
,
TRANSITION_TYPE_ID=1
 WHERE TRANS_ID = EXP_PROCESS_202_.tb6_0(1);
if not (sql%found) then
INSERT INTO WF_TRANSITION(TRANS_ID,ORIGIN_ID,TARGET_ID,GEOMETRY,GROUP_ID,EXPRESSION,EXPRESSION_TYPE,DESCRIPTION,TRANSITION_TYPE_ID) 
VALUES (EXP_PROCESS_202_.tb6_0(1),
EXP_PROCESS_202_.tb6_1(1),
EXP_PROCESS_202_.tb6_2(1),
null,
0,
'FLAG_VALIDATE == NO'
,
...
EXP_PROCESS_202_.tb9_0(0):=400;
EXP_PROCESS_202_.tb9_1(0):=EXP_PROCESS_202_.tb7_0(0);
EXP_PROCESS_202_.tb9_2(0):=EXP_PROCESS_202_.tb8_0(0);
ut_trace.trace('Actualizar o insertar tabla: GE_ATTRIBUTES fila (0)',1);
UPDATE GE_ATTRIBUTES SET ATTRIBUTE_ID=EXP_PROCESS_202_.tb9_0(0),
ATTRIBUTE_CLASS_ID=EXP_PROCESS_202_.tb9_1(0),
ATTRIBUTE_TYPE_ID=EXP_PROCESS_202_.tb9_2(0),
...
DISPLAY_NAME='Causal'
 ...
 WHERE ATTRIBUTE_ID = EXP_PROCESS_202_.tb9_0(0);
if not (sql%found) then
...
VALUES (EXP_PROCESS_202_.tb9_0(0),
EXP_PROCESS_202_.tb9_1(0),
EXP_PROCESS_202_.tb9_2(0),
null,
null,
9,
'CAUSAL'
...
EXP_PROCESS_202_.tb10_0(0):=133407;
EXP_PROCESS_202_.tb10_1(0):=EXP_PROCESS_202_.tb5_0(1);
EXP_PROCESS_202_.tb10_2(0):=EXP_PROCESS_202_.tb9_0(0);
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT_ATTRIBUTE fila (0)',1);
UPDATE WF_UNIT_ATTRIBUTE SET UNIT_ATTRIBUTE_ID=EXP_PROCESS_202_.tb10_0(0),
UNIT_ID=EXP_PROCESS_202_.tb10_1(0),
ATTRIBUTE_ID=EXP_PROCESS_202_.tb10_2(0),
STATEMENT_ID=null,
VALUE=null,
IS_DUPLICABLE='N'
,
...
MANDATORY='N'
 ...
 WHERE UNIT_ATTRIBUTE_ID = EXP_PROCESS_202_.tb10_0(0);
if not (sql%found) then
INSERT INTO WF_UNIT_ATTRIBUTE(UNIT_ATTRIBUTE_ID,UNIT_ID,ATTRIBUTE_ID,STATEMENT_ID,VALUE,IS_DUPLICABLE,IN_OUT,MANDATORY) 
VALUES (EXP_PROCESS_202_.tb10_0(0),
EXP_PROCESS_202_.tb10_1(0),
EXP_PROCESS_202_.tb10_2(0),
null,
null,
'N'
,
...
EXP_PROCESS_202_.tb7_0(1):=8;
ut_trace.trace('Actualizar o insertar tabla: GE_ATTRIBUTE_CLASS fila (1)',1);
UPDATE GE_ATTRIBUTE_CLASS SET ATTRIBUTE_CLASS_ID=EXP_PROCESS_202_.tb7_0(1),
NAME='Por Defecto General'
,
DESCRIPTION='Valores que ser¿n utilizados para clasificaci¿n gen¿rica'

 WHERE ATTRIBUTE_CLASS_ID = EXP_PROCESS_202_.tb7_0(1);
if not (sql%found) then
INSERT INTO GE_ATTRIBUTE_CLASS(ATTRIBUTE_CLASS_ID,NAME,DESCRIPTION) 
VALUES (EXP_PROCESS_202_.tb7_0(1),
'Por Defecto General'
,
...
EXP_PROCESS_202_.tb9_0(1):=442;
EXP_PROCESS_202_.tb9_1(1):=EXP_PROCESS_202_.tb7_0(1);
EXP_PROCESS_202_.tb9_2(1):=EXP_PROCESS_202_.tb8_0(0);
ut_trace.trace('Actualizar o insertar tabla: GE_ATTRIBUTES fila (1)',1);
UPDATE GE_ATTRIBUTES SET ATTRIBUTE_ID=EXP_PROCESS_202_.tb9_0(1),
ATTRIBUTE_CLASS_ID=EXP_PROCESS_202_.tb9_1(1),
ATTRIBUTE_TYPE_ID=EXP_PROCESS_202_.tb9_2(1),
VALID_EXPRESSION=null,
FATHER_ID=null,
MODULE_ID=9,
...
DISPLAY_NAME='Bandera de validaci¿n para la ejecuci¿n de transicciones'
 ...
 WHERE ATTRIBUTE_ID = EXP_PROCESS_202_.tb9_0(1);
if not (sql%found) then
INSERT INTO GE_ATTRIBUTES(ATTRIBUTE_ID,ATTRIBUTE_CLASS_ID,ATTRIBUTE_TYPE_ID,VALID_EXPRESSION,FATHER_ID,MODULE_ID,NAME_ATTRIBUTE,LENGTH,PRECISION,SCALE,DEFAULT_VALUE,IS_FIX_OR_VARIABLE,COMMENT_,DISPLAY_NAME) 
VALUES (EXP_PROCESS_202_.tb9_0(1),
EXP_PROCESS_202_.tb9_1(1),
EXP_PROCESS_202_.tb9_2(1),
null,
null,
9,
'FLAG_VALIDATE'
...
EXP_PROCESS_202_.old_tb11_0(0):=120165306;
EXP_PROCESS_202_.tb11_0(0):=GE_BOSEQUENCE.NEXTGE_STATEMENT;
EXP_PROCESS_202_.tb11_0(0):=EXP_PROCESS_202_.tb11_0(0);
ut_trace.trace('insertando tabla: GE_STATEMENT fila (0)',1);
INSERT INTO GE_STATEMENT(STATEMENT_ID,MODULE_ID,DESCRIPTION,STATEMENT,NAME) 
VALUES (EXP_PROCESS_202_.tb11_0(0),
5,
...
EXP_PROCESS_202_.tb10_0(1):=133410;
EXP_PROCESS_202_.tb10_1(1):=EXP_PROCESS_202_.tb5_0(1);
EXP_PROCESS_202_.tb10_2(1):=EXP_PROCESS_202_.tb9_0(1);
EXP_PROCESS_202_.tb10_3(1):=EXP_PROCESS_202_.tb11_0(0);
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT_ATTRIBUTE fila (1)',1);
UPDATE WF_UNIT_ATTRIBUTE SET UNIT_ATTRIBUTE_ID=EXP_PROCESS_202_.tb10_0(1),
UNIT_ID=EXP_PROCESS_202_.tb10_1(1),
ATTRIBUTE_ID=EXP_PROCESS_202_.tb10_2(1),
STATEMENT_ID=EXP_PROCESS_202_.tb10_3(1),
VALUE=null,
IS_DUPLICABLE='N'
,
IN_OUT=2,
MANDATORY='Y'
 ...
 WHERE UNIT_ATTRIBUTE_ID = EXP_PROCESS_202_.tb10_0(1);
if not (sql%found) then
INSERT INTO WF_UNIT_ATTRIBUTE(UNIT_ATTRIBUTE_ID,UNIT_ID,ATTRIBUTE_ID,STATEMENT_ID,VALUE,IS_DUPLICABLE,IN_OUT,MANDATORY) 
VALUES (EXP_PROCESS_202_.tb10_0(1),
EXP_PROCESS_202_.tb10_1(1),
EXP_PROCESS_202_.tb10_2(1),
EXP_PROCESS_202_.tb10_3(1),
null,
'N'
,
2,
...
EXP_PROCESS_202_.tb2_0(4):=252;
EXP_PROCESS_202_.tb2_1(4):=EXP_PROCESS_202_.tb0_0(1);
EXP_PROCESS_202_.tb2_2(4):=EXP_PROCESS_202_.tb1_0(0);
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT_TYPE fila (4)',1);
UPDATE WF_UNIT_TYPE SET UNIT_TYPE_ID=EXP_PROCESS_202_.tb2_0(4),
CATEGORY_ID=EXP_PROCESS_202_.tb2_1(4),
MODULE_ID=EXP_PROCESS_202_.tb2_2(4),
ACTION_ID=null,
PARENT_ID=983,
INIT_AREA_EXPRESSION_ID=null,
ASSIGN_COMMENT_CLASS=null,
...
ASSIGN_NOTIFICATION_ID=null,
ASSIGNED_USER_ONLY=null
 WHERE UNIT_TYPE_ID = EXP_PROCESS_202_.tb2_0(4);
if not (sql%found) then
INSERT INTO WF_UNIT_TYPE(UNIT_TYPE_ID,CATEGORY_ID,MODULE_ID,ACTION_ID,PARENT_ID,INIT_AREA_EXPRESSION_ID,ASSIGN_COMMENT_CLASS,ATTEND_COMMENT_CLASS,UNASSIGN_COMMENT_CLASS,TAG_NAME,DESCRIPTION,DISPLAY,ICON,IS_STAGE_PROCESS,USABLE_IN_FLOW_STAGE,ENTITY_ID,MULTI_INSTANCE,IS_COUNTABLE,NOTIFICATION_ID,VIEWABLE,DEFAULT_PRIORITY_ID,INITIAL_NOTIFY_TIME,IS_ADMIN_PROCESS,ASSIGN_NOTIFICATION_ID,ASSIGNED_USER_ONLY) 
VALUES (EXP_PROCESS_202_.tb2_0(4),
EXP_PROCESS_202_.tb2_1(4),
EXP_PROCESS_202_.tb2_2(4),
null,
983,
null,
null,
...
EXP_PROCESS_202_.tb4_0(3):=3;
ut_trace.trace('Actualizar o insertar tabla: WF_NODE_TYPE fila (3)',1);
UPDATE WF_NODE_TYPE SET NODE_TYPE_ID=EXP_PROCESS_202_.tb4_0(3),
DESCRIPTION='Final'

 WHERE NODE_TYPE_ID = EXP_PROCESS_202_.tb4_0(3);
if not (sql%found) then
INSERT INTO WF_NODE_TYPE(NODE_TYPE_ID,DESCRIPTION) 
VALUES (EXP_PROCESS_202_.tb4_0(3),
'Final'
);
end if;
...
EXP_PROCESS_202_.tb5_0(4):=350;
EXP_PROCESS_202_.tb5_1(4):=EXP_PROCESS_202_.tb5_0(0);
EXP_PROCESS_202_.tb5_2(4):=EXP_PROCESS_202_.tb2_0(4);
EXP_PROCESS_202_.tb5_3(4):=EXP_PROCESS_202_.tb4_0(3);
EXP_PROCESS_202_.tb5_4(4):=EXP_PROCESS_202_.tb1_0(0);
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT fila (4)',1);
UPDATE WF_UNIT SET UNIT_ID=EXP_PROCESS_202_.tb5_0(4),
PROCESS_ID=EXP_PROCESS_202_.tb5_1(4),
UNIT_TYPE_ID=EXP_PROCESS_202_.tb5_2(4),
NODE_TYPE_ID=EXP_PROCESS_202_.tb5_3(4),
MODULE_ID=EXP_PROCESS_202_.tb5_4(4),
ACTION_ID=null,
PRE_EXPRESSION_ID=null,
POS_EXPRESSION_ID=null,
NOTIFICATION_ID=null,
...
VIEWABLE='N'
 ...
 WHERE UNIT_ID = EXP_PROCESS_202_.tb5_0(4);
if not (sql%found) then
INSERT INTO WF_UNIT(UNIT_ID,PROCESS_ID,UNIT_TYPE_ID,NODE_TYPE_ID,MODULE_ID,ACTION_ID,PRE_EXPRESSION_ID,POS_EXPRESSION_ID,NOTIFICATION_ID,GEOMETRY,DESCRIPTION,ONLINE_EXEC_ID,MULTI_INSTANCE,SINCRONIC_TIMEOUT,ASINCRONIC_TIMEOUT,FUNCTION_TYPE,IS_COUNTABLE,MIN_GROUP_SIZE,EXECUTION_ORDER,ANNULATION_ORDER,ENTITY_ID,VIEWABLE) 
VALUES (EXP_PROCESS_202_.tb5_0(4),
EXP_PROCESS_202_.tb5_1(4),
EXP_PROCESS_202_.tb5_2(4),
EXP_PROCESS_202_.tb5_3(4),
EXP_PROCESS_202_.tb5_4(4),
null,
null,
null,
null,
...
EXP_PROCESS_202_.tb6_0(2):=123692;
EXP_PROCESS_202_.tb6_1(2):=EXP_PROCESS_202_.tb5_0(2);
EXP_PROCESS_202_.tb6_2(2):=EXP_PROCESS_202_.tb5_0(4);
ut_trace.trace('Actualizar o insertar tabla: WF_TRANSITION fila (2)',1);
UPDATE WF_TRANSITION SET TRANS_ID=EXP_PROCESS_202_.tb6_0(2),
ORIGIN_ID=EXP_PROCESS_202_.tb6_1(2),
TARGET_ID=EXP_PROCESS_202_.tb6_2(2),
GEOMETRY=null,
GROUP_ID=0,
EXPRESSION=null,
EXPRESSION_TYPE=0,
DESCRIPTION=null,
TRANSITION_TYPE_ID=1
 WHERE TRANS_ID = EXP_PROCESS_202_.tb6_0(2);
if not (sql%found) then
INSERT INTO WF_TRANSITION(TRANS_ID,ORIGIN_ID,TARGET_ID,GEOMETRY,GROUP_ID,EXPRESSION,EXPRESSION_TYPE,DESCRIPTION,TRANSITION_TYPE_ID) 
VALUES (EXP_PROCESS_202_.tb6_0(2),
EXP_PROCESS_202_.tb6_1(2),
EXP_PROCESS_202_.tb6_2(2),
null,
0,
null,
0,
...
EXP_PROCESS_202_.tb2_0(5):=168;
EXP_PROCESS_202_.tb2_1(5):=EXP_PROCESS_202_.tb0_0(2);
EXP_PROCESS_202_.tb2_2(5):=EXP_PROCESS_202_.tb1_0(0);
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT_TYPE fila (5)',1);
UPDATE WF_UNIT_TYPE SET UNIT_TYPE_ID=EXP_PROCESS_202_.tb2_0(5),
CATEGORY_ID=EXP_PROCESS_202_.tb2_1(5),
MODULE_ID=EXP_PROCESS_202_.tb2_2(5),
ACTION_ID=null,
PARENT_ID=983,
INIT_AREA_EXPRESSION_ID=null,
ASSIGN_COMMENT_CLASS=null,
...
ASSIGN_NOTIFICATION_ID=null,
ASSIGNED_USER_ONLY=null
 WHERE UNIT_TYPE_ID = EXP_PROCESS_202_.tb2_0(5);
if not (sql%found) then
INSERT INTO WF_UNIT_TYPE(UNIT_TYPE_ID,CATEGORY_ID,MODULE_ID,ACTION_ID,PARENT_ID,INIT_AREA_EXPRESSION_ID,ASSIGN_COMMENT_CLASS,ATTEND_COMMENT_CLASS,UNASSIGN_COMMENT_CLASS,TAG_NAME,DESCRIPTION,DISPLAY,ICON,IS_STAGE_PROCESS,USABLE_IN_FLOW_STAGE,ENTITY_ID,MULTI_INSTANCE,IS_COUNTABLE,NOTIFICATION_ID,VIEWABLE,DEFAULT_PRIORITY_ID,INITIAL_NOTIFY_TIME,IS_ADMIN_PROCESS,ASSIGN_NOTIFICATION_ID,ASSIGNED_USER_ONLY) 
VALUES (EXP_PROCESS_202_.tb2_0(5),
EXP_PROCESS_202_.tb2_1(5),
EXP_PROCESS_202_.tb2_2(5),
null,
983,
null,
null,
...
EXP_PROCESS_202_.tb5_0(5):=355;
EXP_PROCESS_202_.tb5_1(5):=EXP_PROCESS_202_.tb5_0(0);
EXP_PROCESS_202_.tb5_2(5):=EXP_PROCESS_202_.tb2_0(5);
EXP_PROCESS_202_.tb5_3(5):=EXP_PROCESS_202_.tb4_0(2);
EXP_PROCESS_202_.tb5_4(5):=EXP_PROCESS_202_.tb1_0(0);
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT fila (5)',1);
UPDATE WF_UNIT SET UNIT_ID=EXP_PROCESS_202_.tb5_0(5),
PROCESS_ID=EXP_PROCESS_202_.tb5_1(5),
UNIT_TYPE_ID=EXP_PROCESS_202_.tb5_2(5),
NODE_TYPE_ID=EXP_PROCESS_202_.tb5_3(5),
MODULE_ID=EXP_PROCESS_202_.tb5_4(5),
ACTION_ID=null,
PRE_EXPRESSION_ID=null,
POS_EXPRESSION_ID=null,
NOTIFICATION_ID=null,
...
VIEWABLE='N'
 ...
 WHERE UNIT_ID = EXP_PROCESS_202_.tb5_0(5);
if not (sql%found) then
INSERT INTO WF_UNIT(UNIT_ID,PROCESS_ID,UNIT_TYPE_ID,NODE_TYPE_ID,MODULE_ID,ACTION_ID,PRE_EXPRESSION_ID,POS_EXPRESSION_ID,NOTIFICATION_ID,GEOMETRY,DESCRIPTION,ONLINE_EXEC_ID,MULTI_INSTANCE,SINCRONIC_TIMEOUT,ASINCRONIC_TIMEOUT,FUNCTION_TYPE,IS_COUNTABLE,MIN_GROUP_SIZE,EXECUTION_ORDER,ANNULATION_ORDER,ENTITY_ID,VIEWABLE) 
VALUES (EXP_PROCESS_202_.tb5_0(5),
EXP_PROCESS_202_.tb5_1(5),
EXP_PROCESS_202_.tb5_2(5),
EXP_PROCESS_202_.tb5_3(5),
EXP_PROCESS_202_.tb5_4(5),
null,
null,
null,
null,
...
EXP_PROCESS_202_.tb6_0(3):=123691;
EXP_PROCESS_202_.tb6_1(3):=EXP_PROCESS_202_.tb5_0(5);
EXP_PROCESS_202_.tb6_2(3):=EXP_PROCESS_202_.tb5_0(2);
ut_trace.trace('Actualizar o insertar tabla: WF_TRANSITION fila (3)',1);
UPDATE WF_TRANSITION SET TRANS_ID=EXP_PROCESS_202_.tb6_0(3),
ORIGIN_ID=EXP_PROCESS_202_.tb6_1(3),
TARGET_ID=EXP_PROCESS_202_.tb6_2(3),
GEOMETRY=null,
GROUP_ID=0,
EXPRESSION=null,
EXPRESSION_TYPE=0,
DESCRIPTION=null,
TRANSITION_TYPE_ID=1
 WHERE TRANS_ID = EXP_PROCESS_202_.tb6_0(3);
if not (sql%found) then
INSERT INTO WF_TRANSITION(TRANS_ID,ORIGIN_ID,TARGET_ID,GEOMETRY,GROUP_ID,EXPRESSION,EXPRESSION_TYPE,DESCRIPTION,TRANSITION_TYPE_ID) 
VALUES (EXP_PROCESS_202_.tb6_0(3),
EXP_PROCESS_202_.tb6_1(3),
EXP_PROCESS_202_.tb6_2(3),
null,
0,
null,
0,
...
EXP_PROCESS_202_.tb2_0(7):=150;
EXP_PROCESS_202_.tb2_1(7):=EXP_PROCESS_202_.tb0_0(2);
EXP_PROCESS_202_.tb2_2(7):=EXP_PROCESS_202_.tb1_0(0);
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT_TYPE fila (7)',1);
UPDATE WF_UNIT_TYPE SET UNIT_TYPE_ID=EXP_PROCESS_202_.tb2_0(7),
CATEGORY_ID=EXP_PROCESS_202_.tb2_1(7),
MODULE_ID=EXP_PROCESS_202_.tb2_2(7),
ACTION_ID=null,
PARENT_ID=983,
INIT_AREA_EXPRESSION_ID=null,
ASSIGN_COMMENT_CLASS=null,
...
ASSIGN_NOTIFICATION_ID=null,
ASSIGNED_USER_ONLY=null
 WHERE UNIT_TYPE_ID = EXP_PROCESS_202_.tb2_0(7);
if not (sql%found) then
INSERT INTO WF_UNIT_TYPE(UNIT_TYPE_ID,CATEGORY_ID,MODULE_ID,ACTION_ID,PARENT_ID,INIT_AREA_EXPRESSION_ID,ASSIGN_COMMENT_CLASS,ATTEND_COMMENT_CLASS,UNASSIGN_COMMENT_CLASS,TAG_NAME,DESCRIPTION,DISPLAY,ICON,IS_STAGE_PROCESS,USABLE_IN_FLOW_STAGE,ENTITY_ID,MULTI_INSTANCE,IS_COUNTABLE,NOTIFICATION_ID,VIEWABLE,DEFAULT_PRIORITY_ID,INITIAL_NOTIFY_TIME,IS_ADMIN_PROCESS,ASSIGN_NOTIFICATION_ID,ASSIGNED_USER_ONLY) 
VALUES (EXP_PROCESS_202_.tb2_0(7),
EXP_PROCESS_202_.tb2_1(7),
EXP_PROCESS_202_.tb2_2(7),
null,
983,
null,
null,
...
EXP_PROCESS_202_.tb5_0(7):=353;
EXP_PROCESS_202_.tb5_1(7):=EXP_PROCESS_202_.tb5_0(0);
EXP_PROCESS_202_.tb5_2(7):=EXP_PROCESS_202_.tb2_0(7);
EXP_PROCESS_202_.tb5_3(7):=EXP_PROCESS_202_.tb4_0(2);
EXP_PROCESS_202_.tb5_4(7):=EXP_PROCESS_202_.tb1_0(0);
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT fila (7)',1);
UPDATE WF_UNIT SET UNIT_ID=EXP_PROCESS_202_.tb5_0(7),
PROCESS_ID=EXP_PROCESS_202_.tb5_1(7),
UNIT_TYPE_ID=EXP_PROCESS_202_.tb5_2(7),
NODE_TYPE_ID=EXP_PROCESS_202_.tb5_3(7),
MODULE_ID=EXP_PROCESS_202_.tb5_4(7),
ACTION_ID=null,
PRE_EXPRESSION_ID=null,
POS_EXPRESSION_ID=null,
NOTIFICATION_ID=null,
...
VIEWABLE='N'
 ...
 WHERE UNIT_ID = EXP_PROCESS_202_.tb5_0(7);
if not (sql%found) then
INSERT INTO WF_UNIT(UNIT_ID,PROCESS_ID,UNIT_TYPE_ID,NODE_TYPE_ID,MODULE_ID,ACTION_ID,PRE_EXPRESSION_ID,POS_EXPRESSION_ID,NOTIFICATION_ID,GEOMETRY,DESCRIPTION,ONLINE_EXEC_ID,MULTI_INSTANCE,SINCRONIC_TIMEOUT,ASINCRONIC_TIMEOUT,FUNCTION_TYPE,IS_COUNTABLE,MIN_GROUP_SIZE,EXECUTION_ORDER,ANNULATION_ORDER,ENTITY_ID,VIEWABLE) 
VALUES (EXP_PROCESS_202_.tb5_0(7),
EXP_PROCESS_202_.tb5_1(7),
EXP_PROCESS_202_.tb5_2(7),
EXP_PROCESS_202_.tb5_3(7),
EXP_PROCESS_202_.tb5_4(7),
null,
null,
null,
null,
...
EXP_PROCESS_202_.tb6_0(5):=123688;
EXP_PROCESS_202_.tb6_1(5):=EXP_PROCESS_202_.tb5_0(3);
EXP_PROCESS_202_.tb6_2(5):=EXP_PROCESS_202_.tb5_0(7);
ut_trace.trace('Actualizar o insertar tabla: WF_TRANSITION fila (5)',1);
UPDATE WF_TRANSITION SET TRANS_ID=EXP_PROCESS_202_.tb6_0(5),
ORIGIN_ID=EXP_PROCESS_202_.tb6_1(5),
TARGET_ID=EXP_PROCESS_202_.tb6_2(5),
GEOMETRY=null,
GROUP_ID=0,
EXPRESSION=null,
EXPRESSION_TYPE=0,
DESCRIPTION=null,
TRANSITION_TYPE_ID=1
 WHERE TRANS_ID = EXP_PROCESS_202_.tb6_0(5);
if not (sql%found) then
INSERT INTO WF_TRANSITION(TRANS_ID,ORIGIN_ID,TARGET_ID,GEOMETRY,GROUP_ID,EXPRESSION,EXPRESSION_TYPE,DESCRIPTION,TRANSITION_TYPE_ID) 
VALUES (EXP_PROCESS_202_.tb6_0(5),
EXP_PROCESS_202_.tb6_1(5),
EXP_PROCESS_202_.tb6_2(5),
null,
0,
null,
0,
...
EXP_PROCESS_202_.tb10_0(2):=133408;
EXP_PROCESS_202_.tb10_1(2):=EXP_PROCESS_202_.tb5_0(4);
EXP_PROCESS_202_.tb10_2(2):=EXP_PROCESS_202_.tb9_0(0);
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT_ATTRIBUTE fila (2)',1);
UPDATE WF_UNIT_ATTRIBUTE SET UNIT_ATTRIBUTE_ID=EXP_PROCESS_202_.tb10_0(2),
UNIT_ID=EXP_PROCESS_202_.tb10_1(2),
ATTRIBUTE_ID=EXP_PROCESS_202_.tb10_2(2),
STATEMENT_ID=null,
VALUE=null,
IS_DUPLICABLE='N'
,
...
MANDATORY='N'
 ...
 WHERE UNIT_ATTRIBUTE_ID = EXP_PROCESS_202_.tb10_0(2);
if not (sql%found) then
INSERT INTO WF_UNIT_ATTRIBUTE(UNIT_ATTRIBUTE_ID,UNIT_ID,ATTRIBUTE_ID,STATEMENT_ID,VALUE,IS_DUPLICABLE,IN_OUT,MANDATORY) 
VALUES (EXP_PROCESS_202_.tb10_0(2),
EXP_PROCESS_202_.tb10_1(2),
EXP_PROCESS_202_.tb10_2(2),
null,
null,
'N'
,
...
EXP_PROCESS_202_.tb10_0(3):=133409;
EXP_PROCESS_202_.tb10_1(3):=EXP_PROCESS_202_.tb5_0(4);
EXP_PROCESS_202_.tb10_2(3):=EXP_PROCESS_202_.tb9_0(0);
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT_ATTRIBUTE fila (3)',1);
UPDATE WF_UNIT_ATTRIBUTE SET UNIT_ATTRIBUTE_ID=EXP_PROCESS_202_.tb10_0(3),
UNIT_ID=EXP_PROCESS_202_.tb10_1(3),
ATTRIBUTE_ID=EXP_PROCESS_202_.tb10_2(3),
STATEMENT_ID=null,
VALUE=null,
IS_DUPLICABLE='N'
,
...
MANDATORY='N'
 ...
 WHERE UNIT_ATTRIBUTE_ID = EXP_PROCESS_202_.tb10_0(3);
if not (sql%found) then
INSERT INTO WF_UNIT_ATTRIBUTE(UNIT_ATTRIBUTE_ID,UNIT_ID,ATTRIBUTE_ID,STATEMENT_ID,VALUE,IS_DUPLICABLE,IN_OUT,MANDATORY) 
VALUES (EXP_PROCESS_202_.tb10_0(3),
EXP_PROCESS_202_.tb10_1(3),
EXP_PROCESS_202_.tb10_2(3),
null,
null,
'N'
,
...
EXP_PROCESS_202_.tb2_0(6):=155;
EXP_PROCESS_202_.tb2_1(6):=EXP_PROCESS_202_.tb0_0(2);
EXP_PROCESS_202_.tb2_2(6):=EXP_PROCESS_202_.tb1_0(0);
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT_TYPE fila (6)',1);
UPDATE WF_UNIT_TYPE SET UNIT_TYPE_ID=EXP_PROCESS_202_.tb2_0(6),
CATEGORY_ID=EXP_PROCESS_202_.tb2_1(6),
MODULE_ID=EXP_PROCESS_202_.tb2_2(6),
ACTION_ID=null,
PARENT_ID=983,
INIT_AREA_EXPRESSION_ID=null,
ASSIGN_COMMENT_CLASS=null,
...
ASSIGN_NOTIFICATION_ID=null,
ASSIGNED_USER_ONLY=null
 WHERE UNIT_TYPE_ID = EXP_PROCESS_202_.tb2_0(6);
if not (sql%found) then
INSERT INTO WF_UNIT_TYPE(UNIT_TYPE_ID,CATEGORY_ID,MODULE_ID,ACTION_ID,PARENT_ID,INIT_AREA_EXPRESSION_ID,ASSIGN_COMMENT_CLASS,ATTEND_COMMENT_CLASS,UNASSIGN_COMMENT_CLASS,TAG_NAME,DESCRIPTION,DISPLAY,ICON,IS_STAGE_PROCESS,USABLE_IN_FLOW_STAGE,ENTITY_ID,MULTI_INSTANCE,IS_COUNTABLE,NOTIFICATION_ID,VIEWABLE,DEFAULT_PRIORITY_ID,INITIAL_NOTIFY_TIME,IS_ADMIN_PROCESS,ASSIGN_NOTIFICATION_ID,ASSIGNED_USER_ONLY) 
VALUES (EXP_PROCESS_202_.tb2_0(6),
EXP_PROCESS_202_.tb2_1(6),
EXP_PROCESS_202_.tb2_2(6),
null,
983,
null,
null,
...
EXP_PROCESS_202_.tb5_0(6):=354;
EXP_PROCESS_202_.tb5_1(6):=EXP_PROCESS_202_.tb5_0(0);
EXP_PROCESS_202_.tb5_2(6):=EXP_PROCESS_202_.tb2_0(6);
EXP_PROCESS_202_.tb5_3(6):=EXP_PROCESS_202_.tb4_0(2);
EXP_PROCESS_202_.tb5_4(6):=EXP_PROCESS_202_.tb1_0(0);
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT fila (6)',1);
UPDATE WF_UNIT SET UNIT_ID=EXP_PROCESS_202_.tb5_0(6),
PROCESS_ID=EXP_PROCESS_202_.tb5_1(6),
UNIT_TYPE_ID=EXP_PROCESS_202_.tb5_2(6),
NODE_TYPE_ID=EXP_PROCESS_202_.tb5_3(6),
MODULE_ID=EXP_PROCESS_202_.tb5_4(6),
ACTION_ID=null,
PRE_EXPRESSION_ID=null,
POS_EXPRESSION_ID=null,
NOTIFICATION_ID=null,
...
VIEWABLE='N'
 ...
 WHERE UNIT_ID = EXP_PROCESS_202_.tb5_0(6);
if not (sql%found) then
INSERT INTO WF_UNIT(UNIT_ID,PROCESS_ID,UNIT_TYPE_ID,NODE_TYPE_ID,MODULE_ID,ACTION_ID,PRE_EXPRESSION_ID,POS_EXPRESSION_ID,NOTIFICATION_ID,GEOMETRY,DESCRIPTION,ONLINE_EXEC_ID,MULTI_INSTANCE,SINCRONIC_TIMEOUT,ASINCRONIC_TIMEOUT,FUNCTION_TYPE,IS_COUNTABLE,MIN_GROUP_SIZE,EXECUTION_ORDER,ANNULATION_ORDER,ENTITY_ID,VIEWABLE) 
VALUES (EXP_PROCESS_202_.tb5_0(6),
EXP_PROCESS_202_.tb5_1(6),
EXP_PROCESS_202_.tb5_2(6),
EXP_PROCESS_202_.tb5_3(6),
EXP_PROCESS_202_.tb5_4(6),
null,
null,
null,
null,
...
EXP_PROCESS_202_.tb6_0(4):=123690;
EXP_PROCESS_202_.tb6_1(4):=EXP_PROCESS_202_.tb5_0(6);
EXP_PROCESS_202_.tb6_2(4):=EXP_PROCESS_202_.tb5_0(5);
ut_trace.trace('Actualizar o insertar tabla: WF_TRANSITION fila (4)',1);
UPDATE WF_TRANSITION SET TRANS_ID=EXP_PROCESS_202_.tb6_0(4),
ORIGIN_ID=EXP_PROCESS_202_.tb6_1(4),
TARGET_ID=EXP_PROCESS_202_.tb6_2(4),
GEOMETRY=null,
GROUP_ID=0,
EXPRESSION=null,
EXPRESSION_TYPE=0,
DESCRIPTION=null,
TRANSITION_TYPE_ID=1
 WHERE TRANS_ID = EXP_PROCESS_202_.tb6_0(4);
if not (sql%found) then
INSERT INTO WF_TRANSITION(TRANS_ID,ORIGIN_ID,TARGET_ID,GEOMETRY,GROUP_ID,EXPRESSION,EXPRESSION_TYPE,DESCRIPTION,TRANSITION_TYPE_ID) 
VALUES (EXP_PROCESS_202_.tb6_0(4),
EXP_PROCESS_202_.tb6_1(4),
EXP_PROCESS_202_.tb6_2(4),
null,
0,
null,
0,
...
EXP_PROCESS_202_.tb6_0(6):=123689;
EXP_PROCESS_202_.tb6_1(6):=EXP_PROCESS_202_.tb5_0(7);
EXP_PROCESS_202_.tb6_2(6):=EXP_PROCESS_202_.tb5_0(6);
ut_trace.trace('Actualizar o insertar tabla: WF_TRANSITION fila (6)',1);
UPDATE WF_TRANSITION SET TRANS_ID=EXP_PROCESS_202_.tb6_0(6),
ORIGIN_ID=EXP_PROCESS_202_.tb6_1(6),
TARGET_ID=EXP_PROCESS_202_.tb6_2(6),
GEOMETRY=null,
GROUP_ID=0,
EXPRESSION=null,
EXPRESSION_TYPE=0,
DESCRIPTION=null,
TRANSITION_TYPE_ID=1
 WHERE TRANS_ID = EXP_PROCESS_202_.tb6_0(6);
if not (sql%found) then
INSERT INTO WF_TRANSITION(TRANS_ID,ORIGIN_ID,TARGET_ID,GEOMETRY,GROUP_ID,EXPRESSION,EXPRESSION_TYPE,DESCRIPTION,TRANSITION_TYPE_ID) 
VALUES (EXP_PROCESS_202_.tb6_0(6),
EXP_PROCESS_202_.tb6_1(6),
EXP_PROCESS_202_.tb6_2(6),
null,
0,
null,
0,
...
BEGIN
sa_bocreatePackages.CreatePackage('EXP_UNITTYPE_202_',
'CREATE OR REPLACE PACKAGE EXP_UNITTYPE_202_ IS ' || chr(10) ||
'blProcessStatus boolean := true; ' || chr(10) ||
'type tyWF_UNIT_TYPERowId is table of rowid index by varchar2(100); ' || chr(10) ||
...
'tbObjectToDelete tyObjectToDelete; ' || chr(10) ||
' ' || chr(10) ||
'END EXP_UNITTYPE_202_;');
END;
/


BEGIN ut_trace.trace('********************Comenzar proceso de objeto:EXP_UNITTYPE_202_******************************'); END;
/

BEGIN
ut_trace.trace('Actualizar Variable de Proceso: EXP_UNITTYPE_202',1);
EXP_UNITTYPE_202_.blProcessStatus := EXP_PROCESS_202_.blProcessStatus ; 
exception when others then
rollback;
ut_trace.trace('**ERROR:'||sqlerrm,1);
raise;
...
if (not EXP_UNITTYPE_202_.blProcessStatus) then
 return;
end if;
 nuIndex := 1;
 FOR crObjectsName IN cuObjectsName LOOP
  EXP_UNITTYPE_202_.tbObjectToDelete(nuIndex) := crObjectsName.OBJECT_NAME;
  nuIndex := nuIndex + 1;
 END LOOP;
EXCEPTION 
 when ex.controlled_error then
...
CURSOR cuRowIdToDelete is
SELECT rowid
FROM WF_UNIT_TYPE_ATTRIB WHERE (UNIT_TYPE_ID) in (SELECT UNIT_TYPE_ID FROM WF_UNIT_TYPE WHERE UNIT_TYPE_ID=202);
nuIndex binary_integer;
BEGIN
...
exception when others then
EXP_UNITTYPE_202_.blProcessStatus := false;
rollback;
ut_trace.trace('**ERROR:'||sqlerrm);
raise;
END; 
...
CURSOR cuRowIdToDelete is
SELECT rowid
FROM WF_CAUSAL_UNIT_TYPE WHERE (UNIT_TYPE_ID) in (SELECT UNIT_TYPE_ID FROM WF_UNIT_TYPE WHERE UNIT_TYPE_ID=202);
nuIndex binary_integer;
BEGIN
...
CURSOR cuRowIdToDelete is
SELECT rowid
FROM MO_TIME_UNI_TYP_PRIO WHERE (UNIT_TYPE_ID) in (SELECT UNIT_TYPE_ID FROM WF_UNIT_TYPE WHERE UNIT_TYPE_ID=202);
nuIndex binary_integer;
BEGIN
...
CURSOR cuRowIdToDelete is
SELECT rowid
FROM OR_ACT_BY_TASK_MOD WHERE (TASK_CODE) in (SELECT UNIT_TYPE_ID FROM WF_UNIT_TYPE WHERE UNIT_TYPE_ID=202);
nuIndex binary_integer;
BEGIN
...
BEGIN
ut_trace.trace('Actualizar Variable de Proceso: EXP_PROCESS_202',1);
EXP_PROCESS_202_.blProcessStatus := EXP_UNITTYPE_202_.blProcessStatus ; 
exception when others then
rollback;
ut_trace.trace('**ERROR:'||sqlerrm,1);
raise;
...
BEGIN
 ut_trace.trace('Inicia borrado de objetos de reglas',1);
 nuIndex := EXP_UNITTYPE_202_.tbObjectToDelete.first;
 while (nuIndex is not null) LOOP
  BEGIN
   ut_trace.trace('Objeto a borrar: ' || EXP_UNITTYPE_202_.tbObjectToDelete(nuIndex),1);
   IF(ut_object.fboExistObject(EXP_UNITTYPE_202_.tbObjectToDelete(nuIndex)) and not fblIsObjectUseByRule(EXP_UNITTYPE_202_.tbObjectToDelete(nuIndex))) THEN
    blObjectDeleted := ut_object.fboDeleteObject(EXP_UNITTYPE_202_.tbObjectToDelete(nuIndex));
    ut_trace.trace('  * Borrado ' || EXP_UNITTYPE_202_.tbObjectToDelete(nuIndex), 1);
   END IF;
  EXCEPTION when others then
   ut_trace.trace('- No se borra el objeto (' || EXP_UNITTYPE_202_.tbObjectToDelete(nuIndex) || '): ' || sqlerrm,1);
  END;
  nuIndex := EXP_UNITTYPE_202_.tbObjectToDelete.next(nuIndex);
 END loop;
 ut_trace.trace('Finaliza borrado de objetos de reglas',1);
EXCEPTION 
 when ex.controlled_error then
...
begin
SA_BOCreatePackages.DropPackage('EXP_UNITTYPE_202_');
end;
/

BEGIN ut_trace.trace('********************FIN  proceso de objeto:EXP_UNITTYPE_202_******************************'); end;
/
...
EXP_UNITTYPE_283_.blProcessStatus := EXP_PROCESS_202_.blProcessStatus ; 
exception when others then
rollback;
ut_trace.trace('**ERROR:'||sqlerrm,1);
raise;
...
BEGIN
ut_trace.trace('Actualizar Variable de Proceso: EXP_PROCESS_202',1);
EXP_PROCESS_202_.blProcessStatus := EXP_UNITTYPE_283_.blProcessStatus ; 
exception when others then
rollback;
ut_trace.trace('**ERROR:'||sqlerrm,1);
raise;
...
EXP_UNITTYPE_252_.blProcessStatus := EXP_PROCESS_202_.blProcessStatus ; 
exception when others then
rollback;
ut_trace.trace('**ERROR:'||sqlerrm,1);
raise;
...
BEGIN
ut_trace.trace('Actualizar Variable de Proceso: EXP_PROCESS_202',1);
EXP_PROCESS_202_.blProcessStatus := EXP_UNITTYPE_252_.blProcessStatus ; 
exception when others then
rollback;
ut_trace.trace('**ERROR:'||sqlerrm,1);
raise;
...
EXP_UNITTYPE_183_.blProcessStatus := EXP_PROCESS_202_.blProcessStatus ; 
exception when others then
rollback;
ut_trace.trace('**ERROR:'||sqlerrm,1);
raise;
...
BEGIN
ut_trace.trace('Actualizar Variable de Proceso: EXP_PROCESS_202',1);
EXP_PROCESS_202_.blProcessStatus := EXP_UNITTYPE_183_.blProcessStatus ; 
exception when others then
rollback;
ut_trace.trace('**ERROR:'||sqlerrm,1);
raise;
...
EXP_UNITTYPE_168_.blProcessStatus := EXP_PROCESS_202_.blProcessStatus ; 
exception when others then
rollback;
ut_trace.trace('**ERROR:'||sqlerrm,1);
raise;
...
BEGIN
ut_trace.trace('Actualizar Variable de Proceso: EXP_PROCESS_202',1);
EXP_PROCESS_202_.blProcessStatus := EXP_UNITTYPE_168_.blProcessStatus ; 
exception when others then
rollback;
ut_trace.trace('**ERROR:'||sqlerrm,1);
raise;
...
EXP_UNITTYPE_166_.blProcessStatus := EXP_PROCESS_202_.blProcessStatus ; 
exception when others then
rollback;
ut_trace.trace('**ERROR:'||sqlerrm,1);
raise;
...
BEGIN
ut_trace.trace('Actualizar Variable de Proceso: EXP_PROCESS_202',1);
EXP_PROCESS_202_.blProcessStatus := EXP_UNITTYPE_166_.blProcessStatus ; 
exception when others then
rollback;
ut_trace.trace('**ERROR:'||sqlerrm,1);
raise;
...
EXP_UNITTYPE_150_.blProcessStatus := EXP_PROCESS_202_.blProcessStatus ; 
exception when others then
rollback;
ut_trace.trace('**ERROR:'||sqlerrm,1);
raise;
...
BEGIN
ut_trace.trace('Actualizar Variable de Proceso: EXP_PROCESS_202',1);
EXP_PROCESS_202_.blProcessStatus := EXP_UNITTYPE_150_.blProcessStatus ; 
exception when others then
rollback;
ut_trace.trace('**ERROR:'||sqlerrm,1);
raise;
...
EXP_UNITTYPE_155_.blProcessStatus := EXP_PROCESS_202_.blProcessStatus ; 
exception when others then
rollback;
ut_trace.trace('**ERROR:'||sqlerrm,1);
raise;
...
BEGIN
ut_trace.trace('Actualizar Variable de Proceso: EXP_PROCESS_202',1);
EXP_PROCESS_202_.blProcessStatus := EXP_UNITTYPE_155_.blProcessStatus ; 
exception when others then
rollback;
ut_trace.trace('**ERROR:'||sqlerrm,1);
raise;
...
EXP_PROCESS_183_.blProcessStatus := EXP_PROCESS_202_.blProcessStatus ; 
exception when others then
rollback;
ut_trace.trace('**ERROR:'||sqlerrm,1);
raise;
...
BEGIN
ut_trace.trace('Actualizar Variable de Proceso: EXP_PROCESS_202',1);
EXP_PROCESS_202_.blProcessStatus := EXP_PROCESS_183_.blProcessStatus ; 
exception when others then
rollback;
ut_trace.trace('**ERROR:'||sqlerrm,1);
raise;
...
EXP_PROCESS_168_.blProcessStatus := EXP_PROCESS_202_.blProcessStatus ; 
exception when others then
rollback;
ut_trace.trace('**ERROR:'||sqlerrm,1);
raise;
...
BEGIN
ut_trace.trace('Actualizar Variable de Proceso: EXP_PROCESS_202',1);
EXP_PROCESS_202_.blProcessStatus := EXP_PROCESS_168_.blProcessStatus ; 
exception when others then
rollback;
ut_trace.trace('**ERROR:'||sqlerrm,1);
raise;
...
EXP_PROCESS_166_.blProcessStatus := EXP_PROCESS_202_.blProcessStatus ; 
exception when others then
rollback;
ut_trace.trace('**ERROR:'||sqlerrm,1);
raise;
...
BEGIN
ut_trace.trace('Actualizar Variable de Proceso: EXP_PROCESS_202',1);
EXP_PROCESS_202_.blProcessStatus := EXP_PROCESS_166_.blProcessStatus ; 
exception when others then
rollback;
ut_trace.trace('**ERROR:'||sqlerrm,1);
raise;
...
EXP_PROCESS_150_.blProcessStatus := EXP_PROCESS_202_.blProcessStatus ; 
exception when others then
rollback;
ut_trace.trace('**ERROR:'||sqlerrm,1);
raise;
...
BEGIN
ut_trace.trace('Actualizar Variable de Proceso: EXP_PROCESS_202',1);
EXP_PROCESS_202_.blProcessStatus := EXP_PROCESS_150_.blProcessStatus ; 
exception when others then
rollback;
ut_trace.trace('**ERROR:'||sqlerrm,1);
raise;
...
EXP_PROCESS_155_.blProcessStatus := EXP_PROCESS_202_.blProcessStatus ; 
exception when others then
rollback;
ut_trace.trace('**ERROR:'||sqlerrm,1);
raise;
...
BEGIN
ut_trace.trace('Actualizar Variable de Proceso: EXP_PROCESS_202',1);
EXP_PROCESS_202_.blProcessStatus := EXP_PROCESS_155_.blProcessStatus ; 
exception when others then
rollback;
ut_trace.trace('**ERROR:'||sqlerrm,1);
raise;
...
ut_trace.trace('Realizar Commit de EXP_PROCESS_202 ',1);
COMMIT;
exception when others then
rollback;
ut_trace.trace('**ERROR:'||sqlerrm,1);
...
DECLARE 
    nuBinaryIndex Binary_Integer;
BEGIN 
    nuBinaryIndex := EXP_PROCESS_202_.tbGR_CONFIG_EXPRESSIONId.first;
    while (nuBinaryIndex IS not null) loop 
       BEGIN
           delete from GR_CONFIG_EXPRESSION where config_expression_id = EXP_PROCESS_202_.tbGR_CONFIG_EXPRESSIONId(nuBinaryIndex);
           ut_trace.trace('Delete config_expression_id = : '||EXP_PROCESS_202_.tbGR_CONFIG_EXPRESSIONId(nuBinaryIndex) || ' OK', 1);
       EXCEPTION
           when ex.RECORD_HAVE_CHILDREN then
               ut_trace.trace('No se pudo borrar la regla config_expression_id '||EXP_PROCESS_202_.tbGR_CONFIG_EXPRESSIONId(nuBinaryIndex),1);
       END;
       nuBinaryIndex := EXP_PROCESS_202_.tbGR_CONFIG_EXPRESSIONId.next(nuBinaryIndex);
    END loop;
...
BEGIN
 ut_trace.trace('Inicia borrado de objetos de reglas',1);
 nuIndex := EXP_PROCESS_202_.tbObjectToDelete.first;
 while (nuIndex is not null) LOOP
  BEGIN
   ut_trace.trace('Objeto a borrar: ' || EXP_PROCESS_202_.tbObjectToDelete(nuIndex),1);
   IF(ut_object.fboExistObject(EXP_PROCESS_202_.tbObjectToDelete(nuIndex)) and not fblIsObjectUseByRule(EXP_PROCESS_202_.tbObjectToDelete(nuIndex))) THEN
    blObjectDeleted := ut_object.fboDeleteObject(EXP_PROCESS_202_.tbObjectToDelete(nuIndex));
    ut_trace.trace('  * Borrado ' || EXP_PROCESS_202_.tbObjectToDelete(nuIndex), 1);
   END IF;
  EXCEPTION when others then
   ut_trace.trace('- No se borra el objeto (' || EXP_PROCESS_202_.tbObjectToDelete(nuIndex) || '): ' || sqlerrm,1);
  END;
  nuIndex := EXP_PROCESS_202_.tbObjectToDelete.next(nuIndex);
 END loop;
 ut_trace.trace('Finaliza borrado de objetos de reglas',1);
EXCEPTION 
 when ex.controlled_error then
...
begin
SA_BOCreatePackages.DropPackage('EXP_PROCESS_202_');
end;
/

BEGIN ut_trace.trace('********************FIN  proceso de objeto:EXP_PROCESS_202_******************************'); end;
/
