...
BEGIN
sa_bocreatePackages.CreatePackage('EXP_PROCESS_342_',
'CREATE OR REPLACE PACKAGE EXP_PROCESS_342_ IS ' || chr(10) ||
...
'tb11_3 ty11_3;type ty12_0 is table of GR_CONFIGURA_TYPE.CONFIGURA_TYPE_ID%type index by binary_integer; ' || chr(10) ||
...
' ' || chr(10) ||
'END EXP_PROCESS_342_;');
END;
/


BEGIN ut_trace.trace('********************Comenzar proceso de objeto:EXP_PROCESS_342_******************************'); END;
/


BEGIN
   ut_trace.trace('Cargando tabla en memoria para posterior borrado GR_CONFIG_EXPRESSION',1);
   open EXP_PROCESS_342_.cuExpression;
   fetch EXP_PROCESS_342_.cuExpression bulk collect INTO EXP_PROCESS_342_.tbGR_CONFIG_EXPRESSIONId;
   close EXP_PROCESS_342_.cuExpression;
END;
/ 
BEGIN
...
BEGIN
sa_bocreatePackages.CreatePackage('DEL_ROOT_668_',
'CREATE OR REPLACE PACKAGE DEL_ROOT_668_ IS ' || chr(10) ||
'blProcessStatus boolean := true; ' || chr(10) ||
'type tyWF_UNIT_TYPERowId is table of rowid index by varchar2(100); ' || chr(10) ||
'tbWF_UNIT_TYPERowId tyWF_UNIT_TYPERowId;type tyWF_UNIT_CATEGORYRowId is table of rowid index by varchar2(100); ' || chr(10) ||
'tbWF_UNIT_CATEGORYRowId tyWF_UNIT_CATEGORYRowId;type tyWF_ATTRIBUTES_EQUIVRowId is table of rowid index by varchar2(100); ' || chr(10) ||
...
'WHERE   UNIT_ID IN ( ' || chr(10) ||
'        SELECT UNIT_ID         FROM WF_UNIT  ' || chr(10) ||
'        WHERE UNIT_ID = 668 ' || chr(10) ||
'        UNION ALL         SELECT UNIT_ID         FROM WF_UNIT  ' || chr(10) ||
'        WHERE PROCESS_ID = 668  ' || chr(10) ||
'       ) ' || chr(10) ||
'AND   PRE_EXPRESSION_ID IS NOT NULL ' || chr(10) ||
'UNION ALL  ' || chr(10) ||
'SELECT POS_EXPRESSION_ID ' || chr(10) ||
...
BEGIN ut_trace.trace('********************Comenzar proceso de objeto:DEL_ROOT_668_******************************'); END;
/


BEGIN
   ut_trace.trace('Cargando tabla en memoria para posterior borrado GR_CONFIG_EXPRESSION',1);
   open DEL_ROOT_668_.cuExpression;
   fetch DEL_ROOT_668_.cuExpression bulk collect INTO DEL_ROOT_668_.tbGR_CONFIG_EXPRESSIONId;
   close DEL_ROOT_668_.cuExpression;
END;
/ 
...
AND     A.UNIT_ID IN (
        ...
        WHERE PROCESS_ID = 668 
       )
;
BEGIN

if (not DEL_ROOT_668_.blProcessStatus) then
 return;
end if;
 nuIndex := 1;
 FOR crObjectsName IN cuObjectsName LOOP
  DEL_ROOT_668_.tbObjectToDelete(nuIndex) := crObjectsName.OBJECT_NAME;
  nuIndex := nuIndex + 1;
 END LOOP;
EXCEPTION 
...
DECLARE 
 ...
 SELECT rowid FROM GR_CONFIG_EXPRESSION WHERE (CONFIG_EXPRESSION_ID) in (SELECT PRE_EXPRESSION_ID FROM WF_UNIT WHERE PROCESS_ID = 668);
BEGIN 
...
DEL_ROOT_668_.tbGR_CONFIG_EXPRESSIONRowId(rcData.rowid):=rcData.rowid; 
END loop; 

exception when others then
DEL_ROOT_668_.blProcessStatus := false;
rollback;
...
DECLARE 
 CURSOR cuLoadTemporaryTable IS 
 SELECT rowid FROM GR_CONFIG_EXPRESSION WHERE (CONFIG_EXPRESSION_ID) in (SELECT POS_EXPRESSION_ID FROM WF_UNIT WHERE PROCESS_ID = 668);
BEGIN 
...
DECLARE 
 CURSOR cuLoadTemporaryTable IS 
 SELECT rowid FROM GR_CONFIG_EXPRESSION WHERE (CONFIG_EXPRESSION_ID) in (SELECT PRE_EXPRESSION_ID FROM WF_UNIT WHERE (UNIT_ID) in (SELECT TARGET_ID FROM WF_TRANSITION WHERE (TARGET_ID) in (SELECT UNIT_ID FROM WF_UNIT WHERE PROCESS_ID = 668)));
BEGIN 
...
DECLARE 
 CURSOR cuLoadTemporaryTable IS 
 SELECT rowid FROM GR_CONFIG_EXPRESSION WHERE (CONFIG_EXPRESSION_ID) in (SELECT POS_EXPRESSION_ID FROM WF_UNIT WHERE (UNIT_ID) in (SELECT TARGET_ID FROM WF_TRANSITION WHERE (TARGET_ID) in (SELECT UNIT_ID FROM WF_UNIT WHERE PROCESS_ID = 668)));
BEGIN 
...
DECLARE 
 CURSOR cuLoadTemporaryTable IS 
 SELECT rowid FROM WF_UNIT WHERE (UNIT_ID) in (SELECT TARGET_ID FROM WF_TRANSITION WHERE (TARGET_ID) in (SELECT UNIT_ID FROM WF_UNIT WHERE PROCESS_ID = 668));
BEGIN 
...
for rcData in cuLoadTemporaryTable loop
DEL_ROOT_668_.tbWF_UNITRowId(rcData.rowid):=rcData.rowid; 
END loop; 
...
SELECT rowid
FROM WF_TRANSITION WHERE (TARGET_ID) in (SELECT UNIT_ID FROM WF_UNIT WHERE PROCESS_ID = 668);
nuIndex binary_integer;
BEGIN
...
nuVarcharIndex:=DEL_ROOT_668_.tbGR_CONFIG_EXPRESSIONRowId.first;  
while (nuVarcharIndex is not null) loop 
begin
ut_trace.trace('Row id a borrar:'||nuVarcharIndex,1);
delete from GR_CONFIG_EXPRESSION where rowid = DEL_ROOT_668_.tbGR_CONFIG_EXPRESSIONRowId(nuVarcharIndex); 
...
DEL_ROOT_668_.tbGR_CONFIG_EXPRESSIONRowId.delete(nuOldVarcharIndex);
end if;
nuVarcharIndex := DEL_ROOT_668_.tbGR_CONFIG_EXPRESSIONRowId.next(nuVarcharIndex); 
END loop; 
...
nuVarcharIndex:=DEL_ROOT_668_.tbWF_UNITRowId.first;  
while (nuVarcharIndex is not null) loop 
begin
ut_trace.trace('Row id a borrar:'||nuVarcharIndex,1);
delete from WF_UNIT where rowid = DEL_ROOT_668_.tbWF_UNITRowId(nuVarcharIndex); 
nuOldVarcharIndex:=nuVarcharIndex;
EXCEPTION
when ex.RECORD_HAVE_CHILDREN then
ut_trace.trace('No se pudo borrar el registro '||nuVarcharIndex,1);
...
END;
if nuOldVarcharIndex is not null then
DEL_ROOT_668_.tbWF_UNITRowId.delete(nuOldVarcharIndex);
end if;
nuVarcharIndex := DEL_ROOT_668_.tbWF_UNITRowId.next(nuVarcharIndex); 
END loop; 
...
CURSOR cuRowIdToDelete is
SELECT rowid
FROM WF_UNIT_DATA_MAP WHERE (SOURCE_ID) in (SELECT UNIT_ATTRIBUTE_ID FROM WF_UNIT_ATTRIBUTE WHERE (UNIT_ID) in (SELECT UNIT_ID FROM WF_UNIT WHERE PROCESS_ID = 668));
nuIndex binary_integer;
BEGIN
...
CURSOR cuRowIdToDelete is
SELECT rowid
FROM WF_UNIT_DATA_MAP WHERE (TARGET_ID) in (SELECT UNIT_ATTRIBUTE_ID FROM WF_UNIT_ATTRIBUTE WHERE (UNIT_ID) in (SELECT UNIT_ID FROM WF_UNIT WHERE PROCESS_ID = 668));
nuIndex binary_integer;
BEGIN
...
CURSOR cuRowIdToDelete is
SELECT rowid
FROM WF_UNIT_ATTRIBUTE WHERE (UNIT_ID) in (SELECT UNIT_ID FROM WF_UNIT WHERE PROCESS_ID = 668);
nuIndex binary_integer;
BEGIN
...
CURSOR cuRowIdToDelete is
SELECT rowid
FROM WF_UNIT WHERE PROCESS_ID = 668;
nuIndex binary_integer;
BEGIN
...
BEGIN 
    nuBinaryIndex := DEL_ROOT_668_.tbGR_CONFIG_EXPRESSIONId.first;
    while (nuBinaryIndex IS not null) loop 
       BEGIN
           delete from GR_CONFIG_EXPRESSION where config_expression_id = DEL_ROOT_668_.tbGR_CONFIG_EXPRESSIONId(nuBinaryIndex);
           ut_trace.trace('Delete config_expression_id = : '||DEL_ROOT_668_.tbGR_CONFIG_EXPRESSIONId(nuBinaryIndex) || ' OK', 1);
       EXCEPTION
           when ex.RECORD_HAVE_CHILDREN then
               ut_trace.trace('No se pudo borrar la regla config_expression_id '||DEL_ROOT_668_.tbGR_CONFIG_EXPRESSIONId(nuBinaryIndex),1);
       END;
       nuBinaryIndex := DEL_ROOT_668_.tbGR_CONFIG_EXPRESSIONId.next(nuBinaryIndex);
    END loop;
...
/ 
BEGIN
ut_trace.trace('Actualizar Variable de Proceso: EXP_PROCESS_342',1);
EXP_PROCESS_342_.blProcessStatus := DEL_ROOT_668_.blProcessStatus ; 
exception when others then
rollback;
ut_trace.trace('**ERROR:'||sqlerrm,1);
raise;
...
nuRowProcess:=DEL_ROOT_668_.tb3_0.first;
while (nuRowProcess is not null) loop 
ut_trace.trace('Genera expresión regla:'|| DEL_ROOT_668_.tb3_0(nuRowProcess),1); 
begin
GR_BOINTERFACE_BODY.MakeExpression(DEL_ROOT_668_.tb3_0(nuRowProcess)); 
exception when others then
ut_trace.trace('Fallo la generacion de la regla:'|| DEL_ROOT_668_.tb3_0(nuRowProcess),1);
end;
nuRowProcess := DEL_ROOT_668_.tb3_0.next(nuRowProcess);
END loop;
COMMIT;
...
BEGIN
 ...
 nuIndex := DEL_ROOT_668_.tbObjectToDelete.first;
 while (nuIndex is not null) LOOP
  BEGIN
   ut_trace.trace('Objeto a borrar: ' || DEL_ROOT_668_.tbObjectToDelete(nuIndex),1);
   IF(ut_object.fboExistObject(DEL_ROOT_668_.tbObjectToDelete(nuIndex)) and not fblIsObjectUseByRule(DEL_ROOT_668_.tbObjectToDelete(nuIndex))) THEN
    blObjectDeleted := ut_object.fboDeleteObject(DEL_ROOT_668_.tbObjectToDelete(nuIndex));
    ut_trace.trace('  * Borrado ' || DEL_ROOT_668_.tbObjectToDelete(nuIndex), 1);
   END IF;
  EXCEPTION when others then
   ut_trace.trace('- No se borra el objeto (' || DEL_ROOT_668_.tbObjectToDelete(nuIndex) || '): ' || sqlerrm,1);
  END;
  nuIndex := DEL_ROOT_668_.tbObjectToDelete.next(nuIndex);
 END loop;
...
begin
SA_BOCreatePackages.DropPackage('DEL_ROOT_668_');
end;
/

BEGIN ut_trace.trace('********************FIN  proceso de objeto:DEL_ROOT_668_******************************'); end;
/
...
DECLARE 
 CURSOR cuLoadTemporaryTable IS 
 SELECT rowid FROM GR_CONFIG_EXPRESSION WHERE (CONFIG_EXPRESSION_ID) in (SELECT PRE_EXPRESSION_ID FROM WF_UNIT WHERE UNIT_ID = 668);
BEGIN 
...
DECLARE 
 CURSOR cuLoadTemporaryTable IS 
 SELECT rowid FROM GR_CONFIG_EXPRESSION WHERE (CONFIG_EXPRESSION_ID) in (SELECT POS_EXPRESSION_ID FROM WF_UNIT WHERE UNIT_ID = 668);
BEGIN 
...
DECLARE 
 CURSOR cuLoadTemporaryTable IS 
 SELECT rowid FROM GR_CONFIG_EXPRESSION WHERE (CONFIG_EXPRESSION_ID) in (SELECT PRE_EXPRESSION_ID FROM WF_UNIT WHERE (UNIT_ID) in (SELECT TARGET_ID FROM WF_TRANSITION WHERE (TARGET_ID) in (SELECT UNIT_ID FROM WF_UNIT WHERE UNIT_ID = 668)));
BEGIN 
...
DECLARE 
 CURSOR cuLoadTemporaryTable IS 
 SELECT rowid FROM GR_CONFIG_EXPRESSION WHERE (CONFIG_EXPRESSION_ID) in (SELECT POS_EXPRESSION_ID FROM WF_UNIT WHERE (UNIT_ID) in (SELECT TARGET_ID FROM WF_TRANSITION WHERE (TARGET_ID) in (SELECT UNIT_ID FROM WF_UNIT WHERE UNIT_ID = 668)));
BEGIN 
...
DECLARE 
 CURSOR cuLoadTemporaryTable IS 
 SELECT rowid FROM WF_UNIT WHERE (UNIT_ID) in (SELECT TARGET_ID FROM WF_TRANSITION WHERE (TARGET_ID) in (SELECT UNIT_ID FROM WF_UNIT WHERE UNIT_ID = 668));
BEGIN 
...
CURSOR cuRowIdToDelete is
SELECT rowid
FROM WF_TRANSITION WHERE (TARGET_ID) in (SELECT UNIT_ID FROM WF_UNIT WHERE UNIT_ID = 668);
nuIndex binary_integer;
BEGIN
...
CURSOR cuRowIdToDelete is
SELECT rowid
FROM WF_UNIT_DATA_MAP WHERE (SOURCE_ID) in (SELECT UNIT_ATTRIBUTE_ID FROM WF_UNIT_ATTRIBUTE WHERE (UNIT_ID) in (SELECT UNIT_ID FROM WF_UNIT WHERE UNIT_ID = 668));
nuIndex binary_integer;
BEGIN
...
CURSOR cuRowIdToDelete is
SELECT rowid
FROM WF_UNIT_DATA_MAP WHERE (TARGET_ID) in (SELECT UNIT_ATTRIBUTE_ID FROM WF_UNIT_ATTRIBUTE WHERE (UNIT_ID) in (SELECT UNIT_ID FROM WF_UNIT WHERE UNIT_ID = 668));
nuIndex binary_integer;
BEGIN
...
CURSOR cuRowIdToDelete is
SELECT rowid
FROM WF_UNIT_ATTRIBUTE WHERE (UNIT_ID) in (SELECT UNIT_ID FROM WF_UNIT WHERE UNIT_ID = 668);
nuIndex binary_integer;
BEGIN
...
CURSOR cuRowIdToDelete is
SELECT rowid
FROM WF_UNIT WHERE UNIT_ID = 668;
nuIndex binary_integer;
BEGIN
...
EXP_ACTION_52_.blProcessStatus := EXP_PROCESS_342_.blProcessStatus ; 
exception when others then
rollback;
ut_trace.trace('**ERROR:'||sqlerrm,1);
raise;
...
BEGIN
ut_trace.trace('Actualizar Variable de Proceso: EXP_PROCESS_342',1);
EXP_PROCESS_342_.blProcessStatus := EXP_ACTION_52_.blProcessStatus ; 
exception when others then
rollback;
ut_trace.trace('**ERROR:'||sqlerrm,1);
raise;
...
if (not EXP_PROCESS_342_.blProcessStatus) then
 return;
end if;
 nuIndex := 1;
 FOR crObjectsName IN cuObjectsName LOOP
  EXP_PROCESS_342_.tbObjectToDelete(nuIndex) := crObjectsName.OBJECT_NAME;
  nuIndex := nuIndex + 1;
 END LOOP;
EXCEPTION 
 when ex.controlled_error then
...
DECLARE 
 CURSOR cuLoadTemporaryTable IS 
 SELECT rowid FROM GR_CONFIG_EXPRESSION WHERE (CONFIG_EXPRESSION_ID) in (SELECT PRE_EXPRESSION_ID FROM WF_UNIT WHERE (UNIT_TYPE_ID) in (SELECT UNIT_TYPE_ID FROM WF_UNIT_TYPE WHERE UNIT_TYPE_ID=342) AND NODE_TYPE_ID=0);
BEGIN 
...
ut_trace.trace('Cargando a memoria tabla GR_CONFIG_EXPRESSION',1);
for rcData in cuLoadTemporaryTable loop
EXP_PROCESS_342_.tbGR_CONFIG_EXPRESSIONRowId(rcData.rowid):=rcData.rowid; 
END loop; 

exception when others then
EXP_PROCESS_342_.blProcessStatus := false;
rollback;
ut_trace.trace('**ERROR:'||sqlerrm);
raise;
END; 
...
DECLARE 
 CURSOR cuLoadTemporaryTable IS 
 SELECT rowid FROM GR_CONFIG_EXPRESSION WHERE (CONFIG_EXPRESSION_ID) in (SELECT POS_EXPRESSION_ID FROM WF_UNIT WHERE (UNIT_TYPE_ID) in (SELECT UNIT_TYPE_ID FROM WF_UNIT_TYPE WHERE UNIT_TYPE_ID=342) AND NODE_TYPE_ID=0);
BEGIN 
...
CURSOR cuRowIdToDelete is
SELECT rowid
FROM WF_UNIT WHERE (UNIT_TYPE_ID) in (SELECT UNIT_TYPE_ID FROM WF_UNIT_TYPE WHERE UNIT_TYPE_ID=342) AND NODE_TYPE_ID=0;
nuIndex binary_integer;
BEGIN
...
ut_trace.trace('Borrando tabla cargada en memoria GR_CONFIG_EXPRESSION',1);
nuVarcharIndex:=EXP_PROCESS_342_.tbGR_CONFIG_EXPRESSIONRowId.first;  
while (nuVarcharIndex is not null) loop 
begin
ut_trace.trace('Row id a borrar:'||nuVarcharIndex,1);
delete from GR_CONFIG_EXPRESSION where rowid = EXP_PROCESS_342_.tbGR_CONFIG_EXPRESSIONRowId(nuVarcharIndex); 
nuOldVarcharIndex:=nuVarcharIndex;
EXCEPTION
when ex.RECORD_HAVE_CHILDREN then
ut_trace.trace('No se pudo borrar el registro '||nuVarcharIndex,1);
...
END;
if nuOldVarcharIndex is not null then
EXP_PROCESS_342_.tbGR_CONFIG_EXPRESSIONRowId.delete(nuOldVarcharIndex);
end if;
nuVarcharIndex := EXP_PROCESS_342_.tbGR_CONFIG_EXPRESSIONRowId.next(nuVarcharIndex); 
END loop; 
...
CURSOR cuRowIdToDelete is
SELECT rowid
FROM WF_UNIT_TYPE WHERE UNIT_TYPE_ID=342;
nuIndex binary_integer;
BEGIN
...
EXP_PROCESS_342_.tb0_0(0):=3;
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT_CATEGORY fila (0)',1);
UPDATE WF_UNIT_CATEGORY SET CATEGORY_ID=EXP_PROCESS_342_.tb0_0(0),
DISPLAY_NUMBER='Proceso Principal'

 WHERE CATEGORY_ID = EXP_PROCESS_342_.tb0_0(0);
if not (sql%found) then
INSERT INTO WF_UNIT_CATEGORY(CATEGORY_ID,DISPLAY_NUMBER) 
VALUES (EXP_PROCESS_342_.tb0_0(0),
'Proceso Principal'
);
end if;
...
EXP_PROCESS_342_.tb1_0(0):=9;
ut_trace.trace('Actualizar o insertar tabla: GE_MODULE fila (0)',1);
UPDATE GE_MODULE SET MODULE_ID=EXP_PROCESS_342_.tb1_0(0),
...
LOCALIZATION='IN'
 ...
 WHERE MODULE_ID = EXP_PROCESS_342_.tb1_0(0);
if not (sql%found) then
INSERT INTO GE_MODULE(MODULE_ID,DESCRIPTION,MNEMONIC,LAST_MESSAGE,PATH_MODULE,ICON_NAME,LOCALIZATION) 
VALUES (EXP_PROCESS_342_.tb1_0(0),
'WorkFlow'
,
'WF'
,
...
EXP_PROCESS_342_.tb2_0(0):=342;
EXP_PROCESS_342_.tb2_1(0):=EXP_PROCESS_342_.tb0_0(0);
EXP_PROCESS_342_.tb2_2(0):=EXP_PROCESS_342_.tb1_0(0);
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT_TYPE fila (0)',1);
UPDATE WF_UNIT_TYPE SET UNIT_TYPE_ID=EXP_PROCESS_342_.tb2_0(0),
CATEGORY_ID=EXP_PROCESS_342_.tb2_1(0),
MODULE_ID=EXP_PROCESS_342_.tb2_2(0),
ACTION_ID=null,
PARENT_ID=983,
...
TAG_NAME='UNIT_TYPE_342'
,
DESCRIPTION='Financiación de Deuda'
,
DISPLAY='Financiación de Deuda'
,
ICON=null,
IS_STAGE_PROCESS='N'
,
...
ASSIGNED_USER_ONLY=null
 WHERE UNIT_TYPE_ID = EXP_PROCESS_342_.tb2_0(0);
if not (sql%found) then
...
VALUES (EXP_PROCESS_342_.tb2_0(0),
EXP_PROCESS_342_.tb2_1(0),
EXP_PROCESS_342_.tb2_2(0),
null,
983,
null,
null,
...
EXP_PROCESS_342_.tb3_0(0):=65;
EXP_PROCESS_342_.tb3_1(0):=EXP_PROCESS_342_.tb2_0(0);
ut_trace.trace('Actualizar o insertar tabla: WF_ATTRIBUTES_EQUIV fila (0)',1);
UPDATE WF_ATTRIBUTES_EQUIV SET ATTRIBUTES_EQUIV_ID=EXP_PROCESS_342_.tb3_0(0),
UNIT_TYPE_ID=EXP_PROCESS_342_.tb3_1(0),
...
VALUE_19=null,
VALUE_20=null
 WHERE ATTRIBUTES_EQUIV_ID = EXP_PROCESS_342_.tb3_0(0);
if not (sql%found) then
...
VALUES (EXP_PROCESS_342_.tb3_0(0),
EXP_PROCESS_342_.tb3_1(0),
21,
0,
31536000,
0,
...
EXP_PROCESS_342_.tb4_0(0):=0;
ut_trace.trace('Actualizar o insertar tabla: WF_NODE_TYPE fila (0)',1);
UPDATE WF_NODE_TYPE SET NODE_TYPE_ID=EXP_PROCESS_342_.tb4_0(0),
DESCRIPTION='Ra¿z'

 WHERE NODE_TYPE_ID = EXP_PROCESS_342_.tb4_0(0);
if not (sql%found) then
INSERT INTO WF_NODE_TYPE(NODE_TYPE_ID,DESCRIPTION) 
VALUES (EXP_PROCESS_342_.tb4_0(0),
'Ra¿z'
);
end if;
...
EXP_PROCESS_342_.tb5_0(0):=668;
EXP_PROCESS_342_.tb5_2(0):=EXP_PROCESS_342_.tb2_0(0);
EXP_PROCESS_342_.tb5_3(0):=EXP_PROCESS_342_.tb4_0(0);
EXP_PROCESS_342_.tb5_4(0):=EXP_PROCESS_342_.tb1_0(0);
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT fila (0)',1);
UPDATE WF_UNIT SET UNIT_ID=EXP_PROCESS_342_.tb5_0(0),
PROCESS_ID=null,
UNIT_TYPE_ID=EXP_PROCESS_342_.tb5_2(0),
NODE_TYPE_ID=EXP_PROCESS_342_.tb5_3(0),
MODULE_ID=EXP_PROCESS_342_.tb5_4(0),
ACTION_ID=null,
...
VIEWABLE='N'
 ...
 WHERE UNIT_ID = EXP_PROCESS_342_.tb5_0(0);
if not (sql%found) then
...
VALUES (EXP_PROCESS_342_.tb5_0(0),
null,
EXP_PROCESS_342_.tb5_2(0),
EXP_PROCESS_342_.tb5_3(0),
EXP_PROCESS_342_.tb5_4(0),
null,
null,
null,
null,
...
EXP_PROCESS_342_.tb0_0(1):=2;
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT_CATEGORY fila (1)',1);
UPDATE WF_UNIT_CATEGORY SET CATEGORY_ID=EXP_PROCESS_342_.tb0_0(1),
DISPLAY_NUMBER='Actividad'

 WHERE CATEGORY_ID = EXP_PROCESS_342_.tb0_0(1);
if not (sql%found) then
INSERT INTO WF_UNIT_CATEGORY(CATEGORY_ID,DISPLAY_NUMBER) 
VALUES (EXP_PROCESS_342_.tb0_0(1),
'Actividad'
);
end if;
...
EXP_PROCESS_342_.tb2_0(1):=283;
EXP_PROCESS_342_.tb2_1(1):=EXP_PROCESS_342_.tb0_0(1);
EXP_PROCESS_342_.tb2_2(1):=EXP_PROCESS_342_.tb1_0(0);
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT_TYPE fila (1)',1);
UPDATE WF_UNIT_TYPE SET UNIT_TYPE_ID=EXP_PROCESS_342_.tb2_0(1),
CATEGORY_ID=EXP_PROCESS_342_.tb2_1(1),
MODULE_ID=EXP_PROCESS_342_.tb2_2(1),
ACTION_ID=null,
PARENT_ID=983,
INIT_AREA_EXPRESSION_ID=null,
ASSIGN_COMMENT_CLASS=null,
...
ASSIGN_NOTIFICATION_ID=null,
ASSIGNED_USER_ONLY=null
 WHERE UNIT_TYPE_ID = EXP_PROCESS_342_.tb2_0(1);
if not (sql%found) then
INSERT INTO WF_UNIT_TYPE(UNIT_TYPE_ID,CATEGORY_ID,MODULE_ID,ACTION_ID,PARENT_ID,INIT_AREA_EXPRESSION_ID,ASSIGN_COMMENT_CLASS,ATTEND_COMMENT_CLASS,UNASSIGN_COMMENT_CLASS,TAG_NAME,DESCRIPTION,DISPLAY,ICON,IS_STAGE_PROCESS,USABLE_IN_FLOW_STAGE,ENTITY_ID,MULTI_INSTANCE,IS_COUNTABLE,NOTIFICATION_ID,VIEWABLE,DEFAULT_PRIORITY_ID,INITIAL_NOTIFY_TIME,IS_ADMIN_PROCESS,ASSIGN_NOTIFICATION_ID,ASSIGNED_USER_ONLY) 
VALUES (EXP_PROCESS_342_.tb2_0(1),
EXP_PROCESS_342_.tb2_1(1),
EXP_PROCESS_342_.tb2_2(1),
null,
983,
null,
null,
...
EXP_PROCESS_342_.tb4_0(1):=1;
ut_trace.trace('Actualizar o insertar tabla: WF_NODE_TYPE fila (1)',1);
UPDATE WF_NODE_TYPE SET NODE_TYPE_ID=EXP_PROCESS_342_.tb4_0(1),
DESCRIPTION='Arranque'

 WHERE NODE_TYPE_ID = EXP_PROCESS_342_.tb4_0(1);
if not (sql%found) then
INSERT INTO WF_NODE_TYPE(NODE_TYPE_ID,DESCRIPTION) 
VALUES (EXP_PROCESS_342_.tb4_0(1),
'Arranque'
);
end if;
...
EXP_PROCESS_342_.tb5_0(1):=669;
EXP_PROCESS_342_.tb5_1(1):=EXP_PROCESS_342_.tb5_0(0);
EXP_PROCESS_342_.tb5_2(1):=EXP_PROCESS_342_.tb2_0(1);
EXP_PROCESS_342_.tb5_3(1):=EXP_PROCESS_342_.tb4_0(1);
EXP_PROCESS_342_.tb5_4(1):=EXP_PROCESS_342_.tb1_0(0);
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT fila (1)',1);
UPDATE WF_UNIT SET UNIT_ID=EXP_PROCESS_342_.tb5_0(1),
PROCESS_ID=EXP_PROCESS_342_.tb5_1(1),
UNIT_TYPE_ID=EXP_PROCESS_342_.tb5_2(1),
NODE_TYPE_ID=EXP_PROCESS_342_.tb5_3(1),
MODULE_ID=EXP_PROCESS_342_.tb5_4(1),
ACTION_ID=null,
PRE_EXPRESSION_ID=null,
POS_EXPRESSION_ID=null,
NOTIFICATION_ID=null,
...
VIEWABLE='N'
 ...
 WHERE UNIT_ID = EXP_PROCESS_342_.tb5_0(1);
if not (sql%found) then
INSERT INTO WF_UNIT(UNIT_ID,PROCESS_ID,UNIT_TYPE_ID,NODE_TYPE_ID,MODULE_ID,ACTION_ID,PRE_EXPRESSION_ID,POS_EXPRESSION_ID,NOTIFICATION_ID,GEOMETRY,DESCRIPTION,ONLINE_EXEC_ID,MULTI_INSTANCE,SINCRONIC_TIMEOUT,ASINCRONIC_TIMEOUT,FUNCTION_TYPE,IS_COUNTABLE,MIN_GROUP_SIZE,EXECUTION_ORDER,ANNULATION_ORDER,ENTITY_ID,VIEWABLE) 
VALUES (EXP_PROCESS_342_.tb5_0(1),
EXP_PROCESS_342_.tb5_1(1),
EXP_PROCESS_342_.tb5_2(1),
EXP_PROCESS_342_.tb5_3(1),
EXP_PROCESS_342_.tb5_4(1),
null,
null,
null,
null,
...
EXP_PROCESS_342_.tb0_0(2):=1;
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT_CATEGORY fila (2)',1);
UPDATE WF_UNIT_CATEGORY SET CATEGORY_ID=EXP_PROCESS_342_.tb0_0(2),
DISPLAY_NUMBER='Proceso B¿sico'

 WHERE CATEGORY_ID = EXP_PROCESS_342_.tb0_0(2);
if not (sql%found) then
INSERT INTO WF_UNIT_CATEGORY(CATEGORY_ID,DISPLAY_NUMBER) 
VALUES (EXP_PROCESS_342_.tb0_0(2),
'Proceso B¿sico'
);
end if;
...
EXP_PROCESS_342_.tb2_0(2):=345;
EXP_PROCESS_342_.tb2_1(2):=EXP_PROCESS_342_.tb0_0(2);
EXP_PROCESS_342_.tb2_2(2):=EXP_PROCESS_342_.tb1_0(0);
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT_TYPE fila (2)',1);
UPDATE WF_UNIT_TYPE SET UNIT_TYPE_ID=EXP_PROCESS_342_.tb2_0(2),
CATEGORY_ID=EXP_PROCESS_342_.tb2_1(2),
MODULE_ID=EXP_PROCESS_342_.tb2_2(2),
ACTION_ID=null,
PARENT_ID=983,
INIT_AREA_EXPRESSION_ID=null,
ASSIGN_COMMENT_CLASS=null,
...
ASSIGN_NOTIFICATION_ID=null,
ASSIGNED_USER_ONLY=null
 WHERE UNIT_TYPE_ID = EXP_PROCESS_342_.tb2_0(2);
if not (sql%found) then
INSERT INTO WF_UNIT_TYPE(UNIT_TYPE_ID,CATEGORY_ID,MODULE_ID,ACTION_ID,PARENT_ID,INIT_AREA_EXPRESSION_ID,ASSIGN_COMMENT_CLASS,ATTEND_COMMENT_CLASS,UNASSIGN_COMMENT_CLASS,TAG_NAME,DESCRIPTION,DISPLAY,ICON,IS_STAGE_PROCESS,USABLE_IN_FLOW_STAGE,ENTITY_ID,MULTI_INSTANCE,IS_COUNTABLE,NOTIFICATION_ID,VIEWABLE,DEFAULT_PRIORITY_ID,INITIAL_NOTIFY_TIME,IS_ADMIN_PROCESS,ASSIGN_NOTIFICATION_ID,ASSIGNED_USER_ONLY) 
VALUES (EXP_PROCESS_342_.tb2_0(2),
EXP_PROCESS_342_.tb2_1(2),
EXP_PROCESS_342_.tb2_2(2),
null,
983,
null,
null,
...
EXP_PROCESS_342_.tb4_0(2):=2;
ut_trace.trace('Actualizar o insertar tabla: WF_NODE_TYPE fila (2)',1);
UPDATE WF_NODE_TYPE SET NODE_TYPE_ID=EXP_PROCESS_342_.tb4_0(2),
DESCRIPTION='Normal'

 WHERE NODE_TYPE_ID = EXP_PROCESS_342_.tb4_0(2);
if not (sql%found) then
INSERT INTO WF_NODE_TYPE(NODE_TYPE_ID,DESCRIPTION) 
VALUES (EXP_PROCESS_342_.tb4_0(2),
'Normal'
);
end if;
...
EXP_PROCESS_342_.tb5_0(2):=674;
EXP_PROCESS_342_.tb5_1(2):=EXP_PROCESS_342_.tb5_0(0);
EXP_PROCESS_342_.tb5_2(2):=EXP_PROCESS_342_.tb2_0(2);
EXP_PROCESS_342_.tb5_3(2):=EXP_PROCESS_342_.tb4_0(2);
EXP_PROCESS_342_.tb5_4(2):=EXP_PROCESS_342_.tb1_0(0);
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT fila (2)',1);
UPDATE WF_UNIT SET UNIT_ID=EXP_PROCESS_342_.tb5_0(2),
PROCESS_ID=EXP_PROCESS_342_.tb5_1(2),
UNIT_TYPE_ID=EXP_PROCESS_342_.tb5_2(2),
NODE_TYPE_ID=EXP_PROCESS_342_.tb5_3(2),
MODULE_ID=EXP_PROCESS_342_.tb5_4(2),
ACTION_ID=null,
PRE_EXPRESSION_ID=null,
POS_EXPRESSION_ID=null,
NOTIFICATION_ID=null,
...
VIEWABLE='N'
 ...
 WHERE UNIT_ID = EXP_PROCESS_342_.tb5_0(2);
if not (sql%found) then
INSERT INTO WF_UNIT(UNIT_ID,PROCESS_ID,UNIT_TYPE_ID,NODE_TYPE_ID,MODULE_ID,ACTION_ID,PRE_EXPRESSION_ID,POS_EXPRESSION_ID,NOTIFICATION_ID,GEOMETRY,DESCRIPTION,ONLINE_EXEC_ID,MULTI_INSTANCE,SINCRONIC_TIMEOUT,ASINCRONIC_TIMEOUT,FUNCTION_TYPE,IS_COUNTABLE,MIN_GROUP_SIZE,EXECUTION_ORDER,ANNULATION_ORDER,ENTITY_ID,VIEWABLE) 
VALUES (EXP_PROCESS_342_.tb5_0(2),
EXP_PROCESS_342_.tb5_1(2),
EXP_PROCESS_342_.tb5_2(2),
EXP_PROCESS_342_.tb5_3(2),
EXP_PROCESS_342_.tb5_4(2),
null,
null,
null,
null,
...
EXP_PROCESS_342_.tb6_0(0):=123951;
EXP_PROCESS_342_.tb6_1(0):=EXP_PROCESS_342_.tb5_0(1);
EXP_PROCESS_342_.tb6_2(0):=EXP_PROCESS_342_.tb5_0(2);
ut_trace.trace('Actualizar o insertar tabla: WF_TRANSITION fila (0)',1);
UPDATE WF_TRANSITION SET TRANS_ID=EXP_PROCESS_342_.tb6_0(0),
ORIGIN_ID=EXP_PROCESS_342_.tb6_1(0),
TARGET_ID=EXP_PROCESS_342_.tb6_2(0),
GEOMETRY='73
26
565
28'
,
GROUP_ID=0,
EXPRESSION='FLAG_VALIDATE == 0 Y FINANCING_PLAN_ == 0'
,
EXPRESSION_TYPE=0,
DESCRIPTION='Sin cuota Inicial y No necesita  Visado'
,
TRANSITION_TYPE_ID=1
 WHERE TRANS_ID = EXP_PROCESS_342_.tb6_0(0);
if not (sql%found) then
...
VALUES (EXP_PROCESS_342_.tb6_0(0),
EXP_PROCESS_342_.tb6_1(0),
EXP_PROCESS_342_.tb6_2(0),
'73
26
565
28'
,
0,
'FLAG_VALIDATE == 0 Y FINANCING_PLAN_ == 0'
,
0,
'Sin cuota Inicial y No necesita  Visado'
,
1);
end if;
...
EXP_PROCESS_342_.tb2_0(3):=347;
EXP_PROCESS_342_.tb2_1(3):=EXP_PROCESS_342_.tb0_0(1);
EXP_PROCESS_342_.tb2_2(3):=EXP_PROCESS_342_.tb1_0(0);
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT_TYPE fila (3)',1);
UPDATE WF_UNIT_TYPE SET UNIT_TYPE_ID=EXP_PROCESS_342_.tb2_0(3),
CATEGORY_ID=EXP_PROCESS_342_.tb2_1(3),
MODULE_ID=EXP_PROCESS_342_.tb2_2(3),
ACTION_ID=null,
PARENT_ID=983,
INIT_AREA_EXPRESSION_ID=null,
ASSIGN_COMMENT_CLASS=null,
...
,
DESCRIPTION='Espera Fecha máxima para anulación'
,
DISPLAY='Espera Fecha máxima para anulación'
,
ICON=null,
IS_STAGE_PROCESS='N'
,
...
ASSIGN_NOTIFICATION_ID=null,
ASSIGNED_USER_ONLY=null
 WHERE UNIT_TYPE_ID = EXP_PROCESS_342_.tb2_0(3);
if not (sql%found) then
INSERT INTO WF_UNIT_TYPE(UNIT_TYPE_ID,CATEGORY_ID,MODULE_ID,ACTION_ID,PARENT_ID,INIT_AREA_EXPRESSION_ID,ASSIGN_COMMENT_CLASS,ATTEND_COMMENT_CLASS,UNASSIGN_COMMENT_CLASS,TAG_NAME,DESCRIPTION,DISPLAY,ICON,IS_STAGE_PROCESS,USABLE_IN_FLOW_STAGE,ENTITY_ID,MULTI_INSTANCE,IS_COUNTABLE,NOTIFICATION_ID,VIEWABLE,DEFAULT_PRIORITY_ID,INITIAL_NOTIFY_TIME,IS_ADMIN_PROCESS,ASSIGN_NOTIFICATION_ID,ASSIGNED_USER_ONLY) 
VALUES (EXP_PROCESS_342_.tb2_0(3),
EXP_PROCESS_342_.tb2_1(3),
EXP_PROCESS_342_.tb2_2(3),
null,
983,
null,
null,
...
'UNIT_TYPE_347'
,
'Espera Fecha máxima para anulación'
,
'Espera Fecha máxima para anulación'
,
null,
...
EXP_PROCESS_342_.tb5_0(3):=681;
EXP_PROCESS_342_.tb5_1(3):=EXP_PROCESS_342_.tb5_0(0);
EXP_PROCESS_342_.tb5_2(3):=EXP_PROCESS_342_.tb2_0(3);
EXP_PROCESS_342_.tb5_3(3):=EXP_PROCESS_342_.tb4_0(2);
EXP_PROCESS_342_.tb5_4(3):=EXP_PROCESS_342_.tb1_0(0);
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT fila (3)',1);
UPDATE WF_UNIT SET UNIT_ID=EXP_PROCESS_342_.tb5_0(3),
PROCESS_ID=EXP_PROCESS_342_.tb5_1(3),
UNIT_TYPE_ID=EXP_PROCESS_342_.tb5_2(3),
NODE_TYPE_ID=EXP_PROCESS_342_.tb5_3(3),
MODULE_ID=EXP_PROCESS_342_.tb5_4(3),
ACTION_ID=null,
PRE_EXPRESSION_ID=null,
POS_EXPRESSION_ID=null,
NOTIFICATION_ID=null,
...
VIEWABLE='Y'
 ...
 WHERE UNIT_ID = EXP_PROCESS_342_.tb5_0(3);
if not (sql%found) then
INSERT INTO WF_UNIT(UNIT_ID,PROCESS_ID,UNIT_TYPE_ID,NODE_TYPE_ID,MODULE_ID,ACTION_ID,PRE_EXPRESSION_ID,POS_EXPRESSION_ID,NOTIFICATION_ID,GEOMETRY,DESCRIPTION,ONLINE_EXEC_ID,MULTI_INSTANCE,SINCRONIC_TIMEOUT,ASINCRONIC_TIMEOUT,FUNCTION_TYPE,IS_COUNTABLE,MIN_GROUP_SIZE,EXECUTION_ORDER,ANNULATION_ORDER,ENTITY_ID,VIEWABLE) 
VALUES (EXP_PROCESS_342_.tb5_0(3),
EXP_PROCESS_342_.tb5_1(3),
EXP_PROCESS_342_.tb5_2(3),
EXP_PROCESS_342_.tb5_3(3),
EXP_PROCESS_342_.tb5_4(3),
null,
null,
null,
null,
...
EXP_PROCESS_342_.tb6_0(1):=123955;
EXP_PROCESS_342_.tb6_1(1):=EXP_PROCESS_342_.tb5_0(1);
EXP_PROCESS_342_.tb6_2(1):=EXP_PROCESS_342_.tb5_0(3);
ut_trace.trace('Actualizar o insertar tabla: WF_TRANSITION fila (1)',1);
UPDATE WF_TRANSITION SET TRANS_ID=EXP_PROCESS_342_.tb6_0(1),
ORIGIN_ID=EXP_PROCESS_342_.tb6_1(1),
TARGET_ID=EXP_PROCESS_342_.tb6_2(1),
GEOMETRY='31
167
32
373'
,
GROUP_ID=0,
EXPRESSION='FLAG_VALIDATE == 1 O FINANCING_PLAN_ == 1'
,
EXPRESSION_TYPE=0,
DESCRIPTION=null,
TRANSITION_TYPE_ID=1
 WHERE TRANS_ID = EXP_PROCESS_342_.tb6_0(1);
if not (sql%found) then
INSERT INTO WF_TRANSITION(TRANS_ID,ORIGIN_ID,TARGET_ID,GEOMETRY,GROUP_ID,EXPRESSION,EXPRESSION_TYPE,DESCRIPTION,TRANSITION_TYPE_ID) 
VALUES (EXP_PROCESS_342_.tb6_0(1),
EXP_PROCESS_342_.tb6_1(1),
EXP_PROCESS_342_.tb6_2(1),
'31
167
32
373'
,
0,
'FLAG_VALIDATE == 1 O FINANCING_PLAN_ == 1'
,
0,
null,
1);
...
EXP_PROCESS_342_.tb2_0(4):=393;
EXP_PROCESS_342_.tb2_1(4):=EXP_PROCESS_342_.tb0_0(2);
EXP_PROCESS_342_.tb2_2(4):=EXP_PROCESS_342_.tb1_0(0);
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT_TYPE fila (4)',1);
UPDATE WF_UNIT_TYPE SET UNIT_TYPE_ID=EXP_PROCESS_342_.tb2_0(4),
CATEGORY_ID=EXP_PROCESS_342_.tb2_1(4),
MODULE_ID=EXP_PROCESS_342_.tb2_2(4),
ACTION_ID=null,
PARENT_ID=983,
INIT_AREA_EXPRESSION_ID=null,
ASSIGN_COMMENT_CLASS=null,
...
ASSIGN_NOTIFICATION_ID=null,
ASSIGNED_USER_ONLY=null
 WHERE UNIT_TYPE_ID = EXP_PROCESS_342_.tb2_0(4);
if not (sql%found) then
INSERT INTO WF_UNIT_TYPE(UNIT_TYPE_ID,CATEGORY_ID,MODULE_ID,ACTION_ID,PARENT_ID,INIT_AREA_EXPRESSION_ID,ASSIGN_COMMENT_CLASS,ATTEND_COMMENT_CLASS,UNASSIGN_COMMENT_CLASS,TAG_NAME,DESCRIPTION,DISPLAY,ICON,IS_STAGE_PROCESS,USABLE_IN_FLOW_STAGE,ENTITY_ID,MULTI_INSTANCE,IS_COUNTABLE,NOTIFICATION_ID,VIEWABLE,DEFAULT_PRIORITY_ID,INITIAL_NOTIFY_TIME,IS_ADMIN_PROCESS,ASSIGN_NOTIFICATION_ID,ASSIGNED_USER_ONLY) 
VALUES (EXP_PROCESS_342_.tb2_0(4),
EXP_PROCESS_342_.tb2_1(4),
EXP_PROCESS_342_.tb2_2(4),
null,
983,
null,
null,
...
EXP_PROCESS_342_.tb5_0(4):=813;
EXP_PROCESS_342_.tb5_1(4):=EXP_PROCESS_342_.tb5_0(0);
EXP_PROCESS_342_.tb5_2(4):=EXP_PROCESS_342_.tb2_0(4);
EXP_PROCESS_342_.tb5_3(4):=EXP_PROCESS_342_.tb4_0(2);
EXP_PROCESS_342_.tb5_4(4):=EXP_PROCESS_342_.tb1_0(0);
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT fila (4)',1);
UPDATE WF_UNIT SET UNIT_ID=EXP_PROCESS_342_.tb5_0(4),
PROCESS_ID=EXP_PROCESS_342_.tb5_1(4),
UNIT_TYPE_ID=EXP_PROCESS_342_.tb5_2(4),
NODE_TYPE_ID=EXP_PROCESS_342_.tb5_3(4),
MODULE_ID=EXP_PROCESS_342_.tb5_4(4),
ACTION_ID=null,
PRE_EXPRESSION_ID=null,
POS_EXPRESSION_ID=null,
NOTIFICATION_ID=null,
...
VIEWABLE='N'
 ...
 WHERE UNIT_ID = EXP_PROCESS_342_.tb5_0(4);
if not (sql%found) then
INSERT INTO WF_UNIT(UNIT_ID,PROCESS_ID,UNIT_TYPE_ID,NODE_TYPE_ID,MODULE_ID,ACTION_ID,PRE_EXPRESSION_ID,POS_EXPRESSION_ID,NOTIFICATION_ID,GEOMETRY,DESCRIPTION,ONLINE_EXEC_ID,MULTI_INSTANCE,SINCRONIC_TIMEOUT,ASINCRONIC_TIMEOUT,FUNCTION_TYPE,IS_COUNTABLE,MIN_GROUP_SIZE,EXECUTION_ORDER,ANNULATION_ORDER,ENTITY_ID,VIEWABLE) 
VALUES (EXP_PROCESS_342_.tb5_0(4),
EXP_PROCESS_342_.tb5_1(4),
EXP_PROCESS_342_.tb5_2(4),
EXP_PROCESS_342_.tb5_3(4),
EXP_PROCESS_342_.tb5_4(4),
null,
null,
null,
null,
...
EXP_PROCESS_342_.tb6_0(2):=124085;
EXP_PROCESS_342_.tb6_1(2):=EXP_PROCESS_342_.tb5_0(1);
EXP_PROCESS_342_.tb6_2(2):=EXP_PROCESS_342_.tb5_0(4);
ut_trace.trace('Actualizar o insertar tabla: WF_TRANSITION fila (2)',1);
UPDATE WF_TRANSITION SET TRANS_ID=EXP_PROCESS_342_.tb6_0(2),
ORIGIN_ID=EXP_PROCESS_342_.tb6_1(2),
TARGET_ID=EXP_PROCESS_342_.tb6_2(2),
GEOMETRY=null,
GROUP_ID=0,
EXPRESSION='FLAG_VALIDATE == 1'
,
EXPRESSION_TYPE=0,
DESCRIPTION='Necesita Visado'
,
TRANSITION_TYPE_ID=1
 WHERE TRANS_ID = EXP_PROCESS_342_.tb6_0(2);
if not (sql%found) then
INSERT INTO WF_TRANSITION(TRANS_ID,ORIGIN_ID,TARGET_ID,GEOMETRY,GROUP_ID,EXPRESSION,EXPRESSION_TYPE,DESCRIPTION,TRANSITION_TYPE_ID) 
VALUES (EXP_PROCESS_342_.tb6_0(2),
EXP_PROCESS_342_.tb6_1(2),
EXP_PROCESS_342_.tb6_2(2),
null,
0,
'FLAG_VALIDATE == 1'
,
...
EXP_PROCESS_342_.tb2_0(5):=394;
EXP_PROCESS_342_.tb2_1(5):=EXP_PROCESS_342_.tb0_0(2);
EXP_PROCESS_342_.tb2_2(5):=EXP_PROCESS_342_.tb1_0(0);
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT_TYPE fila (5)',1);
UPDATE WF_UNIT_TYPE SET UNIT_TYPE_ID=EXP_PROCESS_342_.tb2_0(5),
CATEGORY_ID=EXP_PROCESS_342_.tb2_1(5),
MODULE_ID=EXP_PROCESS_342_.tb2_2(5),
ACTION_ID=null,
PARENT_ID=983,
INIT_AREA_EXPRESSION_ID=null,
ASSIGN_COMMENT_CLASS=null,
...
ASSIGN_NOTIFICATION_ID=null,
ASSIGNED_USER_ONLY=null
 WHERE UNIT_TYPE_ID = EXP_PROCESS_342_.tb2_0(5);
if not (sql%found) then
INSERT INTO WF_UNIT_TYPE(UNIT_TYPE_ID,CATEGORY_ID,MODULE_ID,ACTION_ID,PARENT_ID,INIT_AREA_EXPRESSION_ID,ASSIGN_COMMENT_CLASS,ATTEND_COMMENT_CLASS,UNASSIGN_COMMENT_CLASS,TAG_NAME,DESCRIPTION,DISPLAY,ICON,IS_STAGE_PROCESS,USABLE_IN_FLOW_STAGE,ENTITY_ID,MULTI_INSTANCE,IS_COUNTABLE,NOTIFICATION_ID,VIEWABLE,DEFAULT_PRIORITY_ID,INITIAL_NOTIFY_TIME,IS_ADMIN_PROCESS,ASSIGN_NOTIFICATION_ID,ASSIGNED_USER_ONLY) 
VALUES (EXP_PROCESS_342_.tb2_0(5),
EXP_PROCESS_342_.tb2_1(5),
EXP_PROCESS_342_.tb2_2(5),
null,
983,
null,
null,
...
EXP_PROCESS_342_.tb5_0(5):=815;
EXP_PROCESS_342_.tb5_1(5):=EXP_PROCESS_342_.tb5_0(0);
EXP_PROCESS_342_.tb5_2(5):=EXP_PROCESS_342_.tb2_0(5);
EXP_PROCESS_342_.tb5_3(5):=EXP_PROCESS_342_.tb4_0(2);
EXP_PROCESS_342_.tb5_4(5):=EXP_PROCESS_342_.tb1_0(0);
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT fila (5)',1);
UPDATE WF_UNIT SET UNIT_ID=EXP_PROCESS_342_.tb5_0(5),
PROCESS_ID=EXP_PROCESS_342_.tb5_1(5),
UNIT_TYPE_ID=EXP_PROCESS_342_.tb5_2(5),
NODE_TYPE_ID=EXP_PROCESS_342_.tb5_3(5),
MODULE_ID=EXP_PROCESS_342_.tb5_4(5),
ACTION_ID=null,
PRE_EXPRESSION_ID=null,
POS_EXPRESSION_ID=null,
NOTIFICATION_ID=null,
...
VIEWABLE='N'
 ...
 WHERE UNIT_ID = EXP_PROCESS_342_.tb5_0(5);
if not (sql%found) then
INSERT INTO WF_UNIT(UNIT_ID,PROCESS_ID,UNIT_TYPE_ID,NODE_TYPE_ID,MODULE_ID,ACTION_ID,PRE_EXPRESSION_ID,POS_EXPRESSION_ID,NOTIFICATION_ID,GEOMETRY,DESCRIPTION,ONLINE_EXEC_ID,MULTI_INSTANCE,SINCRONIC_TIMEOUT,ASINCRONIC_TIMEOUT,FUNCTION_TYPE,IS_COUNTABLE,MIN_GROUP_SIZE,EXECUTION_ORDER,ANNULATION_ORDER,ENTITY_ID,VIEWABLE) 
VALUES (EXP_PROCESS_342_.tb5_0(5),
EXP_PROCESS_342_.tb5_1(5),
EXP_PROCESS_342_.tb5_2(5),
EXP_PROCESS_342_.tb5_3(5),
EXP_PROCESS_342_.tb5_4(5),
null,
null,
null,
null,
...
EXP_PROCESS_342_.tb6_0(3):=124087;
EXP_PROCESS_342_.tb6_1(3):=EXP_PROCESS_342_.tb5_0(1);
EXP_PROCESS_342_.tb6_2(3):=EXP_PROCESS_342_.tb5_0(5);
ut_trace.trace('Actualizar o insertar tabla: WF_TRANSITION fila (3)',1);
UPDATE WF_TRANSITION SET TRANS_ID=EXP_PROCESS_342_.tb6_0(3),
ORIGIN_ID=EXP_PROCESS_342_.tb6_1(3),
TARGET_ID=EXP_PROCESS_342_.tb6_2(3),
GEOMETRY='74
186
191
186
...
,
GROUP_ID=0,
EXPRESSION='FLAG_VALIDATE == 0 Y FINANCING_PLAN_ == 1'
,
EXPRESSION_TYPE=0,
DESCRIPTION='Con cuota Inicial y No necesita Visado'
,
TRANSITION_TYPE_ID=1
 WHERE TRANS_ID = EXP_PROCESS_342_.tb6_0(3);
if not (sql%found) then
INSERT INTO WF_TRANSITION(TRANS_ID,ORIGIN_ID,TARGET_ID,GEOMETRY,GROUP_ID,EXPRESSION,EXPRESSION_TYPE,DESCRIPTION,TRANSITION_TYPE_ID) 
VALUES (EXP_PROCESS_342_.tb6_0(3),
EXP_PROCESS_342_.tb6_1(3),
EXP_PROCESS_342_.tb6_2(3),
'74
186
191
186
...
,
0,
'FLAG_VALIDATE == 0 Y FINANCING_PLAN_ == 1'
,
0,
'Con cuota Inicial y No necesita Visado'
,
1);
end if;
...
EXP_PROCESS_342_.tb9_0(0):=174;
EXP_PROCESS_342_.tb9_1(0):=EXP_PROCESS_342_.tb7_0(0);
EXP_PROCESS_342_.tb9_2(0):=EXP_PROCESS_342_.tb8_0(0);
ut_trace.trace('Actualizar o insertar tabla: GE_ATTRIBUTES fila (0)',1);
UPDATE GE_ATTRIBUTES SET ATTRIBUTE_ID=EXP_PROCESS_342_.tb9_0(0),
ATTRIBUTE_CLASS_ID=EXP_PROCESS_342_.tb9_1(0),
ATTRIBUTE_TYPE_ID=EXP_PROCESS_342_.tb9_2(0),
...
COMMENT_='Plan de Financiación'
,
DISPLAY_NAME='Plan de Financiación'

 WHERE ATTRIBUTE_ID = EXP_PROCESS_342_.tb9_0(0);
if not (sql%found) then
...
VALUES (EXP_PROCESS_342_.tb9_0(0),
EXP_PROCESS_342_.tb9_1(0),
EXP_PROCESS_342_.tb9_2(0),
null,
null,
4,
'FINANCING_PLAN_'
...
EXP_PROCESS_342_.old_tb10_0(0):=120196715;
EXP_PROCESS_342_.tb10_0(0):=GE_BOSEQUENCE.NEXTGE_STATEMENT;
EXP_PROCESS_342_.tb10_0(0):=EXP_PROCESS_342_.tb10_0(0);
ut_trace.trace('insertando tabla: GE_STATEMENT fila (0)',1);
INSERT INTO GE_STATEMENT(STATEMENT_ID,MODULE_ID,DESCRIPTION,STATEMENT,NAME) 
VALUES (EXP_PROCESS_342_.tb10_0(0),
16,
'Valida si la solicitud de financiación requiere el pago de una cuota inicial.'
,
...
EXP_PROCESS_342_.tb11_0(0):=133714;
EXP_PROCESS_342_.tb11_1(0):=EXP_PROCESS_342_.tb5_0(1);
EXP_PROCESS_342_.tb11_2(0):=EXP_PROCESS_342_.tb9_0(0);
EXP_PROCESS_342_.tb11_3(0):=EXP_PROCESS_342_.tb10_0(0);
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT_ATTRIBUTE fila (0)',1);
UPDATE WF_UNIT_ATTRIBUTE SET UNIT_ATTRIBUTE_ID=EXP_PROCESS_342_.tb11_0(0),
UNIT_ID=EXP_PROCESS_342_.tb11_1(0),
ATTRIBUTE_ID=EXP_PROCESS_342_.tb11_2(0),
STATEMENT_ID=EXP_PROCESS_342_.tb11_3(0),
VALUE=null,
IS_DUPLICABLE='N'
,
IN_OUT=2,
MANDATORY='Y'
 ...
 WHERE UNIT_ATTRIBUTE_ID = EXP_PROCESS_342_.tb11_0(0);
if not (sql%found) then
INSERT INTO WF_UNIT_ATTRIBUTE(UNIT_ATTRIBUTE_ID,UNIT_ID,ATTRIBUTE_ID,STATEMENT_ID,VALUE,IS_DUPLICABLE,IN_OUT,MANDATORY) 
VALUES (EXP_PROCESS_342_.tb11_0(0),
EXP_PROCESS_342_.tb11_1(0),
EXP_PROCESS_342_.tb11_2(0),
EXP_PROCESS_342_.tb11_3(0),
null,
'N'
,
2,
...
EXP_PROCESS_342_.tb7_0(1):=1;
ut_trace.trace('Actualizar o insertar tabla: GE_ATTRIBUTE_CLASS fila (1)',1);
UPDATE GE_ATTRIBUTE_CLASS SET ATTRIBUTE_CLASS_ID=EXP_PROCESS_342_.tb7_0(1),
NAME='De Entitdad'
,
DESCRIPTION='Valores que ser¿n tra¿dos de la tabla base usando old record'

 WHERE ATTRIBUTE_CLASS_ID = EXP_PROCESS_342_.tb7_0(1);
if not (sql%found) then
INSERT INTO GE_ATTRIBUTE_CLASS(ATTRIBUTE_CLASS_ID,NAME,DESCRIPTION) 
VALUES (EXP_PROCESS_342_.tb7_0(1),
'De Entitdad'
,
...
EXP_PROCESS_342_.tb9_0(1):=400;
EXP_PROCESS_342_.tb9_1(1):=EXP_PROCESS_342_.tb7_0(1);
EXP_PROCESS_342_.tb9_2(1):=EXP_PROCESS_342_.tb8_0(0);
ut_trace.trace('Actualizar o insertar tabla: GE_ATTRIBUTES fila (1)',1);
UPDATE GE_ATTRIBUTES SET ATTRIBUTE_ID=EXP_PROCESS_342_.tb9_0(1),
ATTRIBUTE_CLASS_ID=EXP_PROCESS_342_.tb9_1(1),
ATTRIBUTE_TYPE_ID=EXP_PROCESS_342_.tb9_2(1),
VALID_EXPRESSION=null,
FATHER_ID=null,
MODULE_ID=9,
...
DISPLAY_NAME='Causal'
 ...
 WHERE ATTRIBUTE_ID = EXP_PROCESS_342_.tb9_0(1);
if not (sql%found) then
INSERT INTO GE_ATTRIBUTES(ATTRIBUTE_ID,ATTRIBUTE_CLASS_ID,ATTRIBUTE_TYPE_ID,VALID_EXPRESSION,FATHER_ID,MODULE_ID,NAME_ATTRIBUTE,LENGTH,PRECISION,SCALE,DEFAULT_VALUE,IS_FIX_OR_VARIABLE,COMMENT_,DISPLAY_NAME) 
VALUES (EXP_PROCESS_342_.tb9_0(1),
EXP_PROCESS_342_.tb9_1(1),
EXP_PROCESS_342_.tb9_2(1),
null,
null,
9,
'CAUSAL'
...
EXP_PROCESS_342_.tb11_0(1):=133710;
EXP_PROCESS_342_.tb11_1(1):=EXP_PROCESS_342_.tb5_0(1);
EXP_PROCESS_342_.tb11_2(1):=EXP_PROCESS_342_.tb9_0(1);
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT_ATTRIBUTE fila (1)',1);
UPDATE WF_UNIT_ATTRIBUTE SET UNIT_ATTRIBUTE_ID=EXP_PROCESS_342_.tb11_0(1),
UNIT_ID=EXP_PROCESS_342_.tb11_1(1),
ATTRIBUTE_ID=EXP_PROCESS_342_.tb11_2(1),
STATEMENT_ID=null,
VALUE=null,
IS_DUPLICABLE='N'
,
...
MANDATORY='N'
 ...
 WHERE UNIT_ATTRIBUTE_ID = EXP_PROCESS_342_.tb11_0(1);
if not (sql%found) then
INSERT INTO WF_UNIT_ATTRIBUTE(UNIT_ATTRIBUTE_ID,UNIT_ID,ATTRIBUTE_ID,STATEMENT_ID,VALUE,IS_DUPLICABLE,IN_OUT,MANDATORY) 
VALUES (EXP_PROCESS_342_.tb11_0(1),
EXP_PROCESS_342_.tb11_1(1),
EXP_PROCESS_342_.tb11_2(1),
null,
null,
'N'
,
...
EXP_PROCESS_342_.tb7_0(2):=8;
ut_trace.trace('Actualizar o insertar tabla: GE_ATTRIBUTE_CLASS fila (2)',1);
UPDATE GE_ATTRIBUTE_CLASS SET ATTRIBUTE_CLASS_ID=EXP_PROCESS_342_.tb7_0(2),
NAME='Por Defecto General'
,
DESCRIPTION='Valores que ser¿n utilizados para clasificaci¿n gen¿rica'

 WHERE ATTRIBUTE_CLASS_ID = EXP_PROCESS_342_.tb7_0(2);
if not (sql%found) then
INSERT INTO GE_ATTRIBUTE_CLASS(ATTRIBUTE_CLASS_ID,NAME,DESCRIPTION) 
VALUES (EXP_PROCESS_342_.tb7_0(2),
'Por Defecto General'
,
...
EXP_PROCESS_342_.tb9_0(2):=442;
EXP_PROCESS_342_.tb9_1(2):=EXP_PROCESS_342_.tb7_0(2);
EXP_PROCESS_342_.tb9_2(2):=EXP_PROCESS_342_.tb8_0(0);
ut_trace.trace('Actualizar o insertar tabla: GE_ATTRIBUTES fila (2)',1);
UPDATE GE_ATTRIBUTES SET ATTRIBUTE_ID=EXP_PROCESS_342_.tb9_0(2),
ATTRIBUTE_CLASS_ID=EXP_PROCESS_342_.tb9_1(2),
ATTRIBUTE_TYPE_ID=EXP_PROCESS_342_.tb9_2(2),
VALID_EXPRESSION=null,
FATHER_ID=null,
MODULE_ID=9,
...
DISPLAY_NAME='Bandera de validaci¿n para la ejecuci¿n de transicciones'
 ...
 WHERE ATTRIBUTE_ID = EXP_PROCESS_342_.tb9_0(2);
if not (sql%found) then
INSERT INTO GE_ATTRIBUTES(ATTRIBUTE_ID,ATTRIBUTE_CLASS_ID,ATTRIBUTE_TYPE_ID,VALID_EXPRESSION,FATHER_ID,MODULE_ID,NAME_ATTRIBUTE,LENGTH,PRECISION,SCALE,DEFAULT_VALUE,IS_FIX_OR_VARIABLE,COMMENT_,DISPLAY_NAME) 
VALUES (EXP_PROCESS_342_.tb9_0(2),
EXP_PROCESS_342_.tb9_1(2),
EXP_PROCESS_342_.tb9_2(2),
null,
null,
9,
'FLAG_VALIDATE'
...
EXP_PROCESS_342_.old_tb10_0(1):=120196716;
EXP_PROCESS_342_.tb10_0(1):=GE_BOSEQUENCE.NEXTGE_STATEMENT;
EXP_PROCESS_342_.tb10_0(1):=EXP_PROCESS_342_.tb10_0(1);
ut_trace.trace('insertando tabla: GE_STATEMENT fila (1)',1);
INSERT INTO GE_STATEMENT(STATEMENT_ID,MODULE_ID,DESCRIPTION,STATEMENT,NAME) 
VALUES (EXP_PROCESS_342_.tb10_0(1),
16,
'Valida si la financiación requiere un proceso de visado o aprobación del valor total a financiar'
,
'SELECT decode(WAIT_BY_SIGN,'|| chr(39) ||'Y'|| chr(39) ||',1,0) FLAG_VALIDATE
FROM   cc_financing_request
WHERE financing_request_id = :INST.EXTERNAL_ID:
...
EXP_PROCESS_342_.tb11_0(2):=133713;
EXP_PROCESS_342_.tb11_1(2):=EXP_PROCESS_342_.tb5_0(1);
EXP_PROCESS_342_.tb11_2(2):=EXP_PROCESS_342_.tb9_0(2);
EXP_PROCESS_342_.tb11_3(2):=EXP_PROCESS_342_.tb10_0(1);
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT_ATTRIBUTE fila (2)',1);
UPDATE WF_UNIT_ATTRIBUTE SET UNIT_ATTRIBUTE_ID=EXP_PROCESS_342_.tb11_0(2),
UNIT_ID=EXP_PROCESS_342_.tb11_1(2),
ATTRIBUTE_ID=EXP_PROCESS_342_.tb11_2(2),
STATEMENT_ID=EXP_PROCESS_342_.tb11_3(2),
VALUE=null,
IS_DUPLICABLE='N'
,
IN_OUT=2,
MANDATORY='Y'
 ...
 WHERE UNIT_ATTRIBUTE_ID = EXP_PROCESS_342_.tb11_0(2);
if not (sql%found) then
INSERT INTO WF_UNIT_ATTRIBUTE(UNIT_ATTRIBUTE_ID,UNIT_ID,ATTRIBUTE_ID,STATEMENT_ID,VALUE,IS_DUPLICABLE,IN_OUT,MANDATORY) 
VALUES (EXP_PROCESS_342_.tb11_0(2),
EXP_PROCESS_342_.tb11_1(2),
EXP_PROCESS_342_.tb11_2(2),
EXP_PROCESS_342_.tb11_3(2),
null,
'N'
,
2,
...
EXP_PROCESS_342_.tb2_0(6):=252;
EXP_PROCESS_342_.tb2_1(6):=EXP_PROCESS_342_.tb0_0(1);
EXP_PROCESS_342_.tb2_2(6):=EXP_PROCESS_342_.tb1_0(0);
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT_TYPE fila (6)',1);
UPDATE WF_UNIT_TYPE SET UNIT_TYPE_ID=EXP_PROCESS_342_.tb2_0(6),
CATEGORY_ID=EXP_PROCESS_342_.tb2_1(6),
MODULE_ID=EXP_PROCESS_342_.tb2_2(6),
ACTION_ID=null,
PARENT_ID=983,
INIT_AREA_EXPRESSION_ID=null,
ASSIGN_COMMENT_CLASS=null,
...
ASSIGN_NOTIFICATION_ID=null,
ASSIGNED_USER_ONLY=null
 WHERE UNIT_TYPE_ID = EXP_PROCESS_342_.tb2_0(6);
if not (sql%found) then
INSERT INTO WF_UNIT_TYPE(UNIT_TYPE_ID,CATEGORY_ID,MODULE_ID,ACTION_ID,PARENT_ID,INIT_AREA_EXPRESSION_ID,ASSIGN_COMMENT_CLASS,ATTEND_COMMENT_CLASS,UNASSIGN_COMMENT_CLASS,TAG_NAME,DESCRIPTION,DISPLAY,ICON,IS_STAGE_PROCESS,USABLE_IN_FLOW_STAGE,ENTITY_ID,MULTI_INSTANCE,IS_COUNTABLE,NOTIFICATION_ID,VIEWABLE,DEFAULT_PRIORITY_ID,INITIAL_NOTIFY_TIME,IS_ADMIN_PROCESS,ASSIGN_NOTIFICATION_ID,ASSIGNED_USER_ONLY) 
VALUES (EXP_PROCESS_342_.tb2_0(6),
EXP_PROCESS_342_.tb2_1(6),
EXP_PROCESS_342_.tb2_2(6),
null,
983,
null,
null,
...
EXP_PROCESS_342_.tb4_0(3):=3;
ut_trace.trace('Actualizar o insertar tabla: WF_NODE_TYPE fila (3)',1);
UPDATE WF_NODE_TYPE SET NODE_TYPE_ID=EXP_PROCESS_342_.tb4_0(3),
DESCRIPTION='Final'

 WHERE NODE_TYPE_ID = EXP_PROCESS_342_.tb4_0(3);
if not (sql%found) then
INSERT INTO WF_NODE_TYPE(NODE_TYPE_ID,DESCRIPTION) 
VALUES (EXP_PROCESS_342_.tb4_0(3),
'Final'
);
end if;
...
EXP_PROCESS_342_.tb12_0(0):=9;
ut_trace.trace('Actualizar o insertar tabla: GR_CONFIGURA_TYPE fila (0)',1);
UPDATE GR_CONFIGURA_TYPE SET CONFIGURA_TYPE_ID=EXP_PROCESS_342_.tb12_0(0),
MODULE_ID=9,
...
GENERATION_MASK='WF_POST_RULE'
 ...
 WHERE CONFIGURA_TYPE_ID = EXP_PROCESS_342_.tb12_0(0);
if not (sql%found) then
INSERT INTO GR_CONFIGURA_TYPE(CONFIGURA_TYPE_ID,MODULE_ID,DESCRIPTION,GENERATION_TYPE,EXECUTION_MODE,CALL_TYPE,GENERATION_MASK) 
VALUES (EXP_PROCESS_342_.tb12_0(0),
9,
'Reglas Post'
,
'PL'
...
EXP_PROCESS_342_.old_tb13_0(0):=121400395;
EXP_PROCESS_342_.tb13_0(0):=GR_BOSEQUENCE.NEXTGR_CONFIG_EXPRESSION;
EXP_PROCESS_342_.tb13_0(0):=EXP_PROCESS_342_.tb13_0(0);
EXP_PROCESS_342_.old_tb13_1(0):='WFWF_POST_RULECT9E121400395'
;
EXP_PROCESS_342_.tb13_1(0):=TO_CHAR(EXP_PROCESS_342_.tb13_0(0));
EXP_PROCESS_342_.tb13_2(0):=EXP_PROCESS_342_.tb12_0(0);
ut_trace.trace('insertando tabla: GR_CONFIG_EXPRESSION fila (0)',1);
INSERT INTO GR_CONFIG_EXPRESSION(CONFIG_EXPRESSION_ID,OBJECT_NAME,CONFIGURA_TYPE_ID,EXPRESSION,AUTHOR,CREATION_DATE,GENERATION_DATE,LAST_MODIFI_DATE,STATUS,USED_OTHER_EXPRESION,MODIFICATION_TYPE,PASSWORD,EXECUTION_TYPE,DESCRIPTION,OBJECT_TYPE,CODE) 
VALUES (EXP_PROCESS_342_.tb13_0(0),
EXP_PROCESS_342_.tb13_1(0),
EXP_PROCESS_342_.tb13_2(0),
...
'LBTEST'
,
to_date('25-09-2012 11:46:23','DD-MM-YYYY HH24:MI:SS'),
to_date('20-12-2023 15:53:41','DD-MM-YYYY HH24:MI:SS'),
to_date('20-12-2023 15:53:41','DD-MM-YYYY HH24:MI:SS'),
'G'
,
'N'
...
'DS'
,
'POST- FIN terminación del proceso de espera.'
,
'PP'
,
null);
...
EXP_PROCESS_342_.tb5_0(6):=670;
EXP_PROCESS_342_.tb5_1(6):=EXP_PROCESS_342_.tb5_0(0);
EXP_PROCESS_342_.tb5_2(6):=EXP_PROCESS_342_.tb2_0(6);
EXP_PROCESS_342_.tb5_3(6):=EXP_PROCESS_342_.tb4_0(3);
EXP_PROCESS_342_.tb5_4(6):=EXP_PROCESS_342_.tb1_0(0);
EXP_PROCESS_342_.tb5_7(6):=EXP_PROCESS_342_.tb13_0(0);
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT fila (6)',1);
UPDATE WF_UNIT SET UNIT_ID=EXP_PROCESS_342_.tb5_0(6),
PROCESS_ID=EXP_PROCESS_342_.tb5_1(6),
UNIT_TYPE_ID=EXP_PROCESS_342_.tb5_2(6),
NODE_TYPE_ID=EXP_PROCESS_342_.tb5_3(6),
MODULE_ID=EXP_PROCESS_342_.tb5_4(6),
ACTION_ID=null,
PRE_EXPRESSION_ID=null,
POS_EXPRESSION_ID=EXP_PROCESS_342_.tb5_7(6),
NOTIFICATION_ID=null,
GEOMETRY='691
93'
,
...
VIEWABLE='N'
 ...
 WHERE UNIT_ID = EXP_PROCESS_342_.tb5_0(6);
if not (sql%found) then
INSERT INTO WF_UNIT(UNIT_ID,PROCESS_ID,UNIT_TYPE_ID,NODE_TYPE_ID,MODULE_ID,ACTION_ID,PRE_EXPRESSION_ID,POS_EXPRESSION_ID,NOTIFICATION_ID,GEOMETRY,DESCRIPTION,ONLINE_EXEC_ID,MULTI_INSTANCE,SINCRONIC_TIMEOUT,ASINCRONIC_TIMEOUT,FUNCTION_TYPE,IS_COUNTABLE,MIN_GROUP_SIZE,EXECUTION_ORDER,ANNULATION_ORDER,ENTITY_ID,VIEWABLE) 
VALUES (EXP_PROCESS_342_.tb5_0(6),
EXP_PROCESS_342_.tb5_1(6),
EXP_PROCESS_342_.tb5_2(6),
EXP_PROCESS_342_.tb5_3(6),
EXP_PROCESS_342_.tb5_4(6),
null,
null,
EXP_PROCESS_342_.tb5_7(6),
null,
'691
93'
,
...
EXP_PROCESS_342_.tb6_0(4):=123954;
EXP_PROCESS_342_.tb6_1(4):=EXP_PROCESS_342_.tb5_0(2);
EXP_PROCESS_342_.tb6_2(4):=EXP_PROCESS_342_.tb5_0(6);
ut_trace.trace('Actualizar o insertar tabla: WF_TRANSITION fila (4)',1);
UPDATE WF_TRANSITION SET TRANS_ID=EXP_PROCESS_342_.tb6_0(4),
ORIGIN_ID=EXP_PROCESS_342_.tb6_1(4),
TARGET_ID=EXP_PROCESS_342_.tb6_2(4),
GEOMETRY=null,
GROUP_ID=0,
EXPRESSION=null,
EXPRESSION_TYPE=0,
DESCRIPTION=null,
TRANSITION_TYPE_ID=1
 WHERE TRANS_ID = EXP_PROCESS_342_.tb6_0(4);
if not (sql%found) then
INSERT INTO WF_TRANSITION(TRANS_ID,ORIGIN_ID,TARGET_ID,GEOMETRY,GROUP_ID,EXPRESSION,EXPRESSION_TYPE,DESCRIPTION,TRANSITION_TYPE_ID) 
VALUES (EXP_PROCESS_342_.tb6_0(4),
EXP_PROCESS_342_.tb6_1(4),
EXP_PROCESS_342_.tb6_2(4),
null,
0,
null,
0,
...
EXP_PROCESS_342_.tb6_0(5):=124086;
EXP_PROCESS_342_.tb6_1(5):=EXP_PROCESS_342_.tb5_0(4);
EXP_PROCESS_342_.tb6_2(5):=EXP_PROCESS_342_.tb5_0(2);
ut_trace.trace('Actualizar o insertar tabla: WF_TRANSITION fila (5)',1);
UPDATE WF_TRANSITION SET TRANS_ID=EXP_PROCESS_342_.tb6_0(5),
ORIGIN_ID=EXP_PROCESS_342_.tb6_1(5),
TARGET_ID=EXP_PROCESS_342_.tb6_2(5),
GEOMETRY=null,
GROUP_ID=0,
...
,
TRANSITION_TYPE_ID=1
 WHERE TRANS_ID = EXP_PROCESS_342_.tb6_0(5);
if not (sql%found) then
INSERT INTO WF_TRANSITION(TRANS_ID,ORIGIN_ID,TARGET_ID,GEOMETRY,GROUP_ID,EXPRESSION,EXPRESSION_TYPE,DESCRIPTION,TRANSITION_TYPE_ID) 
VALUES (EXP_PROCESS_342_.tb6_0(5),
EXP_PROCESS_342_.tb6_1(5),
EXP_PROCESS_342_.tb6_2(5),
null,
0,
'FINANCING_PLAN_ == 0'
,
...
EXP_PROCESS_342_.tb6_0(6):=124101;
EXP_PROCESS_342_.tb6_1(6):=EXP_PROCESS_342_.tb5_0(5);
EXP_PROCESS_342_.tb6_2(6):=EXP_PROCESS_342_.tb5_0(2);
ut_trace.trace('Actualizar o insertar tabla: WF_TRANSITION fila (6)',1);
UPDATE WF_TRANSITION SET TRANS_ID=EXP_PROCESS_342_.tb6_0(6),
ORIGIN_ID=EXP_PROCESS_342_.tb6_1(6),
TARGET_ID=EXP_PROCESS_342_.tb6_2(6),
GEOMETRY='565
247'
,
GROUP_ID=0,
...
DESCRIPTION=null,
TRANSITION_TYPE_ID=1
 WHERE TRANS_ID = EXP_PROCESS_342_.tb6_0(6);
if not (sql%found) then
INSERT INTO WF_TRANSITION(TRANS_ID,ORIGIN_ID,TARGET_ID,GEOMETRY,GROUP_ID,EXPRESSION,EXPRESSION_TYPE,DESCRIPTION,TRANSITION_TYPE_ID) 
VALUES (EXP_PROCESS_342_.tb6_0(6),
EXP_PROCESS_342_.tb6_1(6),
EXP_PROCESS_342_.tb6_2(6),
'565
247'
,
0,
...
EXP_PROCESS_342_.tb6_0(7):=124017;
EXP_PROCESS_342_.tb6_1(7):=EXP_PROCESS_342_.tb5_0(3);
EXP_PROCESS_342_.tb6_2(7):=EXP_PROCESS_342_.tb5_0(6);
ut_trace.trace('Actualizar o insertar tabla: WF_TRANSITION fila (7)',1);
UPDATE WF_TRANSITION SET TRANS_ID=EXP_PROCESS_342_.tb6_0(7),
ORIGIN_ID=EXP_PROCESS_342_.tb6_1(7),
TARGET_ID=EXP_PROCESS_342_.tb6_2(7),
GEOMETRY='728
374'
,
GROUP_ID=0,
...
DESCRIPTION=null,
TRANSITION_TYPE_ID=1
 WHERE TRANS_ID = EXP_PROCESS_342_.tb6_0(7);
if not (sql%found) then
INSERT INTO WF_TRANSITION(TRANS_ID,ORIGIN_ID,TARGET_ID,GEOMETRY,GROUP_ID,EXPRESSION,EXPRESSION_TYPE,DESCRIPTION,TRANSITION_TYPE_ID) 
VALUES (EXP_PROCESS_342_.tb6_0(7),
EXP_PROCESS_342_.tb6_1(7),
EXP_PROCESS_342_.tb6_2(7),
'728
374'
,
0,
...
EXP_PROCESS_342_.tb1_0(1):=5;
ut_trace.trace('Actualizar o insertar tabla: GE_MODULE fila (1)',1);
UPDATE GE_MODULE SET MODULE_ID=EXP_PROCESS_342_.tb1_0(1),
...
LOCALIZATION='IN'
 ...
 WHERE MODULE_ID = EXP_PROCESS_342_.tb1_0(1);
if not (sql%found) then
INSERT INTO GE_MODULE(MODULE_ID,DESCRIPTION,MNEMONIC,LAST_MESSAGE,PATH_MODULE,ICON_NAME,LOCALIZATION) 
VALUES (EXP_PROCESS_342_.tb1_0(1),
'GESTI¿N DE MOTIVOS'
,
'MO'
,
...
EXP_PROCESS_342_.tb2_0(7):=348;
EXP_PROCESS_342_.tb2_1(7):=EXP_PROCESS_342_.tb0_0(1);
EXP_PROCESS_342_.tb2_2(7):=EXP_PROCESS_342_.tb1_0(1);
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT_TYPE fila (7)',1);
UPDATE WF_UNIT_TYPE SET UNIT_TYPE_ID=EXP_PROCESS_342_.tb2_0(7),
CATEGORY_ID=EXP_PROCESS_342_.tb2_1(7),
MODULE_ID=EXP_PROCESS_342_.tb2_2(7),
ACTION_ID=null,
PARENT_ID=983,
INIT_AREA_EXPRESSION_ID=null,
ASSIGN_COMMENT_CLASS=null,
...
ASSIGN_NOTIFICATION_ID=null,
ASSIGNED_USER_ONLY=null
 WHERE UNIT_TYPE_ID = EXP_PROCESS_342_.tb2_0(7);
if not (sql%found) then
INSERT INTO WF_UNIT_TYPE(UNIT_TYPE_ID,CATEGORY_ID,MODULE_ID,ACTION_ID,PARENT_ID,INIT_AREA_EXPRESSION_ID,ASSIGN_COMMENT_CLASS,ATTEND_COMMENT_CLASS,UNASSIGN_COMMENT_CLASS,TAG_NAME,DESCRIPTION,DISPLAY,ICON,IS_STAGE_PROCESS,USABLE_IN_FLOW_STAGE,ENTITY_ID,MULTI_INSTANCE,IS_COUNTABLE,NOTIFICATION_ID,VIEWABLE,DEFAULT_PRIORITY_ID,INITIAL_NOTIFY_TIME,IS_ADMIN_PROCESS,ASSIGN_NOTIFICATION_ID,ASSIGNED_USER_ONLY) 
VALUES (EXP_PROCESS_342_.tb2_0(7),
EXP_PROCESS_342_.tb2_1(7),
EXP_PROCESS_342_.tb2_2(7),
null,
983,
null,
null,
...
EXP_PROCESS_342_.tb4_0(4):=4;
ut_trace.trace('Actualizar o insertar tabla: WF_NODE_TYPE fila (4)',1);
UPDATE WF_NODE_TYPE SET NODE_TYPE_ID=EXP_PROCESS_342_.tb4_0(4),
DESCRIPTION='Aut¿nomo'

 WHERE NODE_TYPE_ID = EXP_PROCESS_342_.tb4_0(4);
if not (sql%found) then
INSERT INTO WF_NODE_TYPE(NODE_TYPE_ID,DESCRIPTION) 
VALUES (EXP_PROCESS_342_.tb4_0(4),
'Aut¿nomo'
);
end if;
...
EXP_PROCESS_342_.old_tb13_0(1):=121400396;
EXP_PROCESS_342_.tb13_0(1):=GR_BOSEQUENCE.NEXTGR_CONFIG_EXPRESSION;
EXP_PROCESS_342_.tb13_0(1):=EXP_PROCESS_342_.tb13_0(1);
EXP_PROCESS_342_.old_tb13_1(1):='WFWF_POST_RULECT9E121400396'
;
EXP_PROCESS_342_.tb13_1(1):=TO_CHAR(EXP_PROCESS_342_.tb13_0(1));
EXP_PROCESS_342_.tb13_2(1):=EXP_PROCESS_342_.tb12_0(0);
ut_trace.trace('insertando tabla: GR_CONFIG_EXPRESSION fila (1)',1);
INSERT INTO GR_CONFIG_EXPRESSION(CONFIG_EXPRESSION_ID,OBJECT_NAME,CONFIGURA_TYPE_ID,EXPRESSION,AUTHOR,CREATION_DATE,GENERATION_DATE,LAST_MODIFI_DATE,STATUS,USED_OTHER_EXPRESION,MODIFICATION_TYPE,PASSWORD,EXECUTION_TYPE,DESCRIPTION,OBJECT_TYPE,CODE) 
VALUES (EXP_PROCESS_342_.tb13_0(1),
EXP_PROCESS_342_.tb13_1(1),
EXP_PROCESS_342_.tb13_2(1),
...
'LBTEST'
,
to_date('04-09-2012 16:14:10','DD-MM-YYYY HH24:MI:SS'),
to_date('19-01-2024 11:42:36','DD-MM-YYYY HH24:MI:SS'),
to_date('19-01-2024 11:42:36','DD-MM-YYYY HH24:MI:SS'),
'R'
,
'N'
...
EXP_PROCESS_342_.tb5_0(7):=684;
EXP_PROCESS_342_.tb5_1(7):=EXP_PROCESS_342_.tb5_0(0);
EXP_PROCESS_342_.tb5_2(7):=EXP_PROCESS_342_.tb2_0(7);
EXP_PROCESS_342_.tb5_3(7):=EXP_PROCESS_342_.tb4_0(4);
EXP_PROCESS_342_.tb5_4(7):=EXP_PROCESS_342_.tb1_0(1);
EXP_PROCESS_342_.tb5_7(7):=EXP_PROCESS_342_.tb13_0(1);
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT fila (7)',1);
UPDATE WF_UNIT SET UNIT_ID=EXP_PROCESS_342_.tb5_0(7),
PROCESS_ID=EXP_PROCESS_342_.tb5_1(7),
UNIT_TYPE_ID=EXP_PROCESS_342_.tb5_2(7),
NODE_TYPE_ID=EXP_PROCESS_342_.tb5_3(7),
MODULE_ID=EXP_PROCESS_342_.tb5_4(7),
ACTION_ID=52,
PRE_EXPRESSION_ID=null,
POS_EXPRESSION_ID=EXP_PROCESS_342_.tb5_7(7),
...
VIEWABLE='Y'
 ...
 WHERE UNIT_ID = EXP_PROCESS_342_.tb5_0(7);
if not (sql%found) then
INSERT INTO WF_UNIT(UNIT_ID,PROCESS_ID,UNIT_TYPE_ID,NODE_TYPE_ID,MODULE_ID,ACTION_ID,PRE_EXPRESSION_ID,POS_EXPRESSION_ID,NOTIFICATION_ID,GEOMETRY,DESCRIPTION,ONLINE_EXEC_ID,MULTI_INSTANCE,SINCRONIC_TIMEOUT,ASINCRONIC_TIMEOUT,FUNCTION_TYPE,IS_COUNTABLE,MIN_GROUP_SIZE,EXECUTION_ORDER,ANNULATION_ORDER,ENTITY_ID,VIEWABLE) 
VALUES (EXP_PROCESS_342_.tb5_0(7),
EXP_PROCESS_342_.tb5_1(7),
EXP_PROCESS_342_.tb5_2(7),
EXP_PROCESS_342_.tb5_3(7),
EXP_PROCESS_342_.tb5_4(7),
52,
null,
EXP_PROCESS_342_.tb5_7(7),
9000,
'329
503'
,
...
EXP_PROCESS_342_.tb6_0(8):=123956;
EXP_PROCESS_342_.tb6_1(8):=EXP_PROCESS_342_.tb5_0(3);
EXP_PROCESS_342_.tb6_2(8):=EXP_PROCESS_342_.tb5_0(7);
ut_trace.trace('Actualizar o insertar tabla: WF_TRANSITION fila (8)',1);
UPDATE WF_TRANSITION SET TRANS_ID=EXP_PROCESS_342_.tb6_0(8),
ORIGIN_ID=EXP_PROCESS_342_.tb6_1(8),
TARGET_ID=EXP_PROCESS_342_.tb6_2(8),
GEOMETRY=null,
GROUP_ID=0,
...
DESCRIPTION=null,
TRANSITION_TYPE_ID=1
 WHERE TRANS_ID = EXP_PROCESS_342_.tb6_0(8);
if not (sql%found) then
INSERT INTO WF_TRANSITION(TRANS_ID,ORIGIN_ID,TARGET_ID,GEOMETRY,GROUP_ID,EXPRESSION,EXPRESSION_TYPE,DESCRIPTION,TRANSITION_TYPE_ID) 
VALUES (EXP_PROCESS_342_.tb6_0(8),
EXP_PROCESS_342_.tb6_1(8),
EXP_PROCESS_342_.tb6_2(8),
null,
0,
'FLAG_VALIDATE == 0'
,
...
EXP_PROCESS_342_.tb8_0(1):=3;
ut_trace.trace('Actualizar o insertar tabla: GE_ATTRIBUTES_TYPE fila (1)',1);
UPDATE GE_ATTRIBUTES_TYPE SET ATTRIBUTE_TYPE_ID=EXP_PROCESS_342_.tb8_0(1),
DESCRIPTION='DATE'
,
INTERNAL_TYPE=12,
INTERNAL_JAVA_TYPE=91
 WHERE ATTRIBUTE_TYPE_ID = EXP_PROCESS_342_.tb8_0(1);
if not (sql%found) then
INSERT INTO GE_ATTRIBUTES_TYPE(ATTRIBUTE_TYPE_ID,DESCRIPTION,INTERNAL_TYPE,INTERNAL_JAVA_TYPE) 
VALUES (EXP_PROCESS_342_.tb8_0(1),
'DATE'
,
12,
91);
...
EXP_PROCESS_342_.tb9_0(3):=406;
EXP_PROCESS_342_.tb9_1(3):=EXP_PROCESS_342_.tb7_0(2);
EXP_PROCESS_342_.tb9_2(3):=EXP_PROCESS_342_.tb8_0(1);
ut_trace.trace('Actualizar o insertar tabla: GE_ATTRIBUTES fila (3)',1);
UPDATE GE_ATTRIBUTES SET ATTRIBUTE_ID=EXP_PROCESS_342_.tb9_0(3),
ATTRIBUTE_CLASS_ID=EXP_PROCESS_342_.tb9_1(3),
ATTRIBUTE_TYPE_ID=EXP_PROCESS_342_.tb9_2(3),
VALID_EXPRESSION=null,
FATHER_ID=null,
MODULE_ID=9,
...
DISPLAY_NAME='Fecha a Iniciar'
 ...
 WHERE ATTRIBUTE_ID = EXP_PROCESS_342_.tb9_0(3);
if not (sql%found) then
INSERT INTO GE_ATTRIBUTES(ATTRIBUTE_ID,ATTRIBUTE_CLASS_ID,ATTRIBUTE_TYPE_ID,VALID_EXPRESSION,FATHER_ID,MODULE_ID,NAME_ATTRIBUTE,LENGTH,PRECISION,SCALE,DEFAULT_VALUE,IS_FIX_OR_VARIABLE,COMMENT_,DISPLAY_NAME) 
VALUES (EXP_PROCESS_342_.tb9_0(3),
EXP_PROCESS_342_.tb9_1(3),
EXP_PROCESS_342_.tb9_2(3),
null,
null,
9,
'$INITIAL_DATE'
...
EXP_PROCESS_342_.old_tb10_0(2):=120196717;
EXP_PROCESS_342_.tb10_0(2):=GE_BOSEQUENCE.NEXTGE_STATEMENT;
EXP_PROCESS_342_.tb10_0(2):=EXP_PROCESS_342_.tb10_0(2);
ut_trace.trace('insertando tabla: GE_STATEMENT fila (2)',1);
INSERT INTO GE_STATEMENT(STATEMENT_ID,MODULE_ID,DESCRIPTION,STATEMENT,NAME) 
VALUES (EXP_PROCESS_342_.tb10_0(2),
16,
'Fecha máxima para anulación'
,
'SELECT ut_date.fsbSTR_DATE(b.init_pay_expiration) INITIAL_DATE
FROM cc_financing_request b
WHERE b.financing_request_id = :INST.EXTERNAL_ID:
'
,
'Fecha máxima para anulación'
);
...
EXP_PROCESS_342_.tb11_0(3):=133719;
EXP_PROCESS_342_.tb11_1(3):=EXP_PROCESS_342_.tb5_0(3);
EXP_PROCESS_342_.tb11_2(3):=EXP_PROCESS_342_.tb9_0(3);
EXP_PROCESS_342_.tb11_3(3):=EXP_PROCESS_342_.tb10_0(2);
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT_ATTRIBUTE fila (3)',1);
UPDATE WF_UNIT_ATTRIBUTE SET UNIT_ATTRIBUTE_ID=EXP_PROCESS_342_.tb11_0(3),
UNIT_ID=EXP_PROCESS_342_.tb11_1(3),
ATTRIBUTE_ID=EXP_PROCESS_342_.tb11_2(3),
STATEMENT_ID=EXP_PROCESS_342_.tb11_3(3),
VALUE=null,
IS_DUPLICABLE='N'
,
IN_OUT=1,
MANDATORY='Y'
 ...
 WHERE UNIT_ATTRIBUTE_ID = EXP_PROCESS_342_.tb11_0(3);
if not (sql%found) then
INSERT INTO WF_UNIT_ATTRIBUTE(UNIT_ATTRIBUTE_ID,UNIT_ID,ATTRIBUTE_ID,STATEMENT_ID,VALUE,IS_DUPLICABLE,IN_OUT,MANDATORY) 
VALUES (EXP_PROCESS_342_.tb11_0(3),
EXP_PROCESS_342_.tb11_1(3),
EXP_PROCESS_342_.tb11_2(3),
EXP_PROCESS_342_.tb11_3(3),
null,
'N'
,
1,
...
EXP_PROCESS_342_.tb11_0(4):=134077;
EXP_PROCESS_342_.tb11_1(4):=EXP_PROCESS_342_.tb5_0(3);
EXP_PROCESS_342_.tb11_2(4):=EXP_PROCESS_342_.tb9_0(2);
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT_ATTRIBUTE fila (4)',1);
UPDATE WF_UNIT_ATTRIBUTE SET UNIT_ATTRIBUTE_ID=EXP_PROCESS_342_.tb11_0(4),
UNIT_ID=EXP_PROCESS_342_.tb11_1(4),
ATTRIBUTE_ID=EXP_PROCESS_342_.tb11_2(4),
STATEMENT_ID=null,
VALUE='0'
,
IS_DUPLICABLE='N'
...
MANDATORY='Y'
 ...
 WHERE UNIT_ATTRIBUTE_ID = EXP_PROCESS_342_.tb11_0(4);
if not (sql%found) then
INSERT INTO WF_UNIT_ATTRIBUTE(UNIT_ATTRIBUTE_ID,UNIT_ID,ATTRIBUTE_ID,STATEMENT_ID,VALUE,IS_DUPLICABLE,IN_OUT,MANDATORY) 
VALUES (EXP_PROCESS_342_.tb11_0(4),
EXP_PROCESS_342_.tb11_1(4),
EXP_PROCESS_342_.tb11_2(4),
null,
'0'
,
'N'
...
EXP_PROCESS_342_.tb11_0(5):=134078;
EXP_PROCESS_342_.tb11_1(5):=EXP_PROCESS_342_.tb5_0(3);
EXP_PROCESS_342_.tb11_2(5):=EXP_PROCESS_342_.tb9_0(1);
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT_ATTRIBUTE fila (5)',1);
UPDATE WF_UNIT_ATTRIBUTE SET UNIT_ATTRIBUTE_ID=EXP_PROCESS_342_.tb11_0(5),
UNIT_ID=EXP_PROCESS_342_.tb11_1(5),
ATTRIBUTE_ID=EXP_PROCESS_342_.tb11_2(5),
STATEMENT_ID=null,
VALUE=null,
IS_DUPLICABLE='N'
,
...
MANDATORY='N'
 ...
 WHERE UNIT_ATTRIBUTE_ID = EXP_PROCESS_342_.tb11_0(5);
if not (sql%found) then
INSERT INTO WF_UNIT_ATTRIBUTE(UNIT_ATTRIBUTE_ID,UNIT_ID,ATTRIBUTE_ID,STATEMENT_ID,VALUE,IS_DUPLICABLE,IN_OUT,MANDATORY) 
VALUES (EXP_PROCESS_342_.tb11_0(5),
EXP_PROCESS_342_.tb11_1(5),
EXP_PROCESS_342_.tb11_2(5),
null,
null,
'N'
,
...
EXP_PROCESS_342_.tb6_0(9):=124099;
EXP_PROCESS_342_.tb6_1(9):=EXP_PROCESS_342_.tb5_0(4);
EXP_PROCESS_342_.tb6_2(9):=EXP_PROCESS_342_.tb5_0(5);
ut_trace.trace('Actualizar o insertar tabla: WF_TRANSITION fila (9)',1);
UPDATE WF_TRANSITION SET TRANS_ID=EXP_PROCESS_342_.tb6_0(9),
ORIGIN_ID=EXP_PROCESS_342_.tb6_1(9),
TARGET_ID=EXP_PROCESS_342_.tb6_2(9),
GEOMETRY=null,
GROUP_ID=0,
...
,
TRANSITION_TYPE_ID=1
 WHERE TRANS_ID = EXP_PROCESS_342_.tb6_0(9);
if not (sql%found) then
INSERT INTO WF_TRANSITION(TRANS_ID,ORIGIN_ID,TARGET_ID,GEOMETRY,GROUP_ID,EXPRESSION,EXPRESSION_TYPE,DESCRIPTION,TRANSITION_TYPE_ID) 
VALUES (EXP_PROCESS_342_.tb6_0(9),
EXP_PROCESS_342_.tb6_1(9),
EXP_PROCESS_342_.tb6_2(9),
null,
0,
'FINANCING_PLAN_ == 1'
,
...
EXP_PROCESS_342_.old_tb10_0(3):=120196718;
EXP_PROCESS_342_.tb10_0(3):=GE_BOSEQUENCE.NEXTGE_STATEMENT;
EXP_PROCESS_342_.tb10_0(3):=EXP_PROCESS_342_.tb10_0(3);
ut_trace.trace('insertando tabla: GE_STATEMENT fila (3)',1);
INSERT INTO GE_STATEMENT(STATEMENT_ID,MODULE_ID,DESCRIPTION,STATEMENT,NAME) 
VALUES (EXP_PROCESS_342_.tb10_0(3),
16,
'Valida si la financiación requiere el pago de una cuota inicial'
,
'SELECT decode(WAIT_BY_PAYMENT,'|| chr(39) ||'Y'|| chr(39) ||',1,0) FINANCING_PLAN_
FROM   cc_financing_request
WHERE financing_request_id = :INST.EXTERNAL_ID:
...
EXP_PROCESS_342_.tb11_0(6):=133868;
EXP_PROCESS_342_.tb11_1(6):=EXP_PROCESS_342_.tb5_0(4);
EXP_PROCESS_342_.tb11_2(6):=EXP_PROCESS_342_.tb9_0(0);
EXP_PROCESS_342_.tb11_3(6):=EXP_PROCESS_342_.tb10_0(3);
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT_ATTRIBUTE fila (6)',1);
UPDATE WF_UNIT_ATTRIBUTE SET UNIT_ATTRIBUTE_ID=EXP_PROCESS_342_.tb11_0(6),
UNIT_ID=EXP_PROCESS_342_.tb11_1(6),
ATTRIBUTE_ID=EXP_PROCESS_342_.tb11_2(6),
STATEMENT_ID=EXP_PROCESS_342_.tb11_3(6),
VALUE=null,
IS_DUPLICABLE='N'
,
IN_OUT=2,
MANDATORY='Y'
 ...
 WHERE UNIT_ATTRIBUTE_ID = EXP_PROCESS_342_.tb11_0(6);
if not (sql%found) then
INSERT INTO WF_UNIT_ATTRIBUTE(UNIT_ATTRIBUTE_ID,UNIT_ID,ATTRIBUTE_ID,STATEMENT_ID,VALUE,IS_DUPLICABLE,IN_OUT,MANDATORY) 
VALUES (EXP_PROCESS_342_.tb11_0(6),
EXP_PROCESS_342_.tb11_1(6),
EXP_PROCESS_342_.tb11_2(6),
EXP_PROCESS_342_.tb11_3(6),
null,
'N'
,
2,
...
EXP_PROCESS_342_.tb11_0(7):=133711;
EXP_PROCESS_342_.tb11_1(7):=EXP_PROCESS_342_.tb5_0(6);
EXP_PROCESS_342_.tb11_2(7):=EXP_PROCESS_342_.tb9_0(1);
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT_ATTRIBUTE fila (7)',1);
UPDATE WF_UNIT_ATTRIBUTE SET UNIT_ATTRIBUTE_ID=EXP_PROCESS_342_.tb11_0(7),
UNIT_ID=EXP_PROCESS_342_.tb11_1(7),
ATTRIBUTE_ID=EXP_PROCESS_342_.tb11_2(7),
STATEMENT_ID=null,
VALUE=null,
IS_DUPLICABLE='N'
,
...
MANDATORY='N'
 ...
 WHERE UNIT_ATTRIBUTE_ID = EXP_PROCESS_342_.tb11_0(7);
if not (sql%found) then
INSERT INTO WF_UNIT_ATTRIBUTE(UNIT_ATTRIBUTE_ID,UNIT_ID,ATTRIBUTE_ID,STATEMENT_ID,VALUE,IS_DUPLICABLE,IN_OUT,MANDATORY) 
VALUES (EXP_PROCESS_342_.tb11_0(7),
EXP_PROCESS_342_.tb11_1(7),
EXP_PROCESS_342_.tb11_2(7),
null,
null,
'N'
,
...
EXP_PROCESS_342_.tb11_0(8):=133712;
EXP_PROCESS_342_.tb11_1(8):=EXP_PROCESS_342_.tb5_0(6);
EXP_PROCESS_342_.tb11_2(8):=EXP_PROCESS_342_.tb9_0(1);
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT_ATTRIBUTE fila (8)',1);
UPDATE WF_UNIT_ATTRIBUTE SET UNIT_ATTRIBUTE_ID=EXP_PROCESS_342_.tb11_0(8),
UNIT_ID=EXP_PROCESS_342_.tb11_1(8),
ATTRIBUTE_ID=EXP_PROCESS_342_.tb11_2(8),
STATEMENT_ID=null,
VALUE=null,
IS_DUPLICABLE='N'
,
...
MANDATORY='N'
 ...
 WHERE UNIT_ATTRIBUTE_ID = EXP_PROCESS_342_.tb11_0(8);
if not (sql%found) then
INSERT INTO WF_UNIT_ATTRIBUTE(UNIT_ATTRIBUTE_ID,UNIT_ID,ATTRIBUTE_ID,STATEMENT_ID,VALUE,IS_DUPLICABLE,IN_OUT,MANDATORY) 
VALUES (EXP_PROCESS_342_.tb11_0(8),
EXP_PROCESS_342_.tb11_1(8),
EXP_PROCESS_342_.tb11_2(8),
null,
null,
'N'
,
...
EXP_PROCESS_342_.tb11_0(9):=133782;
EXP_PROCESS_342_.tb11_1(9):=EXP_PROCESS_342_.tb5_0(7);
EXP_PROCESS_342_.tb11_2(9):=EXP_PROCESS_342_.tb9_0(1);
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT_ATTRIBUTE fila (9)',1);
UPDATE WF_UNIT_ATTRIBUTE SET UNIT_ATTRIBUTE_ID=EXP_PROCESS_342_.tb11_0(9),
UNIT_ID=EXP_PROCESS_342_.tb11_1(9),
ATTRIBUTE_ID=EXP_PROCESS_342_.tb11_2(9),
STATEMENT_ID=null,
VALUE=null,
IS_DUPLICABLE='N'
,
...
MANDATORY='N'
 ...
 WHERE UNIT_ATTRIBUTE_ID = EXP_PROCESS_342_.tb11_0(9);
if not (sql%found) then
INSERT INTO WF_UNIT_ATTRIBUTE(UNIT_ATTRIBUTE_ID,UNIT_ID,ATTRIBUTE_ID,STATEMENT_ID,VALUE,IS_DUPLICABLE,IN_OUT,MANDATORY) 
VALUES (EXP_PROCESS_342_.tb11_0(9),
EXP_PROCESS_342_.tb11_1(9),
EXP_PROCESS_342_.tb11_2(9),
null,
null,
'N'
,
...
BEGIN
sa_bocreatePackages.CreatePackage('EXP_UNITTYPE_342_',
'CREATE OR REPLACE PACKAGE EXP_UNITTYPE_342_ IS ' || chr(10) ||
'blProcessStatus boolean := true; ' || chr(10) ||
'type tyWF_UNIT_TYPERowId is table of rowid index by varchar2(100); ' || chr(10) ||
...
'tbObjectToDelete tyObjectToDelete; ' || chr(10) ||
' ' || chr(10) ||
'END EXP_UNITTYPE_342_;');
END;
/


BEGIN ut_trace.trace('********************Comenzar proceso de objeto:EXP_UNITTYPE_342_******************************'); END;
/

BEGIN
ut_trace.trace('Actualizar Variable de Proceso: EXP_UNITTYPE_342',1);
EXP_UNITTYPE_342_.blProcessStatus := EXP_PROCESS_342_.blProcessStatus ; 
exception when others then
rollback;
ut_trace.trace('**ERROR:'||sqlerrm,1);
raise;
...
if (not EXP_UNITTYPE_342_.blProcessStatus) then
 return;
end if;
 nuIndex := 1;
 FOR crObjectsName IN cuObjectsName LOOP
  EXP_UNITTYPE_342_.tbObjectToDelete(nuIndex) := crObjectsName.OBJECT_NAME;
  nuIndex := nuIndex + 1;
 END LOOP;
EXCEPTION 
 when ex.controlled_error then
...
CURSOR cuRowIdToDelete is
SELECT rowid
FROM WF_UNIT_TYPE_ATTRIB WHERE (UNIT_TYPE_ID) in (SELECT UNIT_TYPE_ID FROM WF_UNIT_TYPE WHERE UNIT_TYPE_ID=342);
nuIndex binary_integer;
BEGIN
...
exception when others then
EXP_UNITTYPE_342_.blProcessStatus := false;
rollback;
ut_trace.trace('**ERROR:'||sqlerrm);
raise;
END; 
...
CURSOR cuRowIdToDelete is
SELECT rowid
FROM WF_CAUSAL_UNIT_TYPE WHERE (UNIT_TYPE_ID) in (SELECT UNIT_TYPE_ID FROM WF_UNIT_TYPE WHERE UNIT_TYPE_ID=342);
nuIndex binary_integer;
BEGIN
...
CURSOR cuRowIdToDelete is
SELECT rowid
FROM MO_TIME_UNI_TYP_PRIO WHERE (UNIT_TYPE_ID) in (SELECT UNIT_TYPE_ID FROM WF_UNIT_TYPE WHERE UNIT_TYPE_ID=342);
nuIndex binary_integer;
BEGIN
...
CURSOR cuRowIdToDelete is
SELECT rowid
FROM OR_ACT_BY_TASK_MOD WHERE (TASK_CODE) in (SELECT UNIT_TYPE_ID FROM WF_UNIT_TYPE WHERE UNIT_TYPE_ID=342);
nuIndex binary_integer;
BEGIN
...
BEGIN
ut_trace.trace('Actualizar Variable de Proceso: EXP_PROCESS_342',1);
EXP_PROCESS_342_.blProcessStatus := EXP_UNITTYPE_342_.blProcessStatus ; 
exception when others then
rollback;
ut_trace.trace('**ERROR:'||sqlerrm,1);
raise;
...
BEGIN
 ut_trace.trace('Inicia borrado de objetos de reglas',1);
 nuIndex := EXP_UNITTYPE_342_.tbObjectToDelete.first;
 while (nuIndex is not null) LOOP
  BEGIN
   ut_trace.trace('Objeto a borrar: ' || EXP_UNITTYPE_342_.tbObjectToDelete(nuIndex),1);
   IF(ut_object.fboExistObject(EXP_UNITTYPE_342_.tbObjectToDelete(nuIndex)) and not fblIsObjectUseByRule(EXP_UNITTYPE_342_.tbObjectToDelete(nuIndex))) THEN
    blObjectDeleted := ut_object.fboDeleteObject(EXP_UNITTYPE_342_.tbObjectToDelete(nuIndex));
    ut_trace.trace('  * Borrado ' || EXP_UNITTYPE_342_.tbObjectToDelete(nuIndex), 1);
   END IF;
  EXCEPTION when others then
   ut_trace.trace('- No se borra el objeto (' || EXP_UNITTYPE_342_.tbObjectToDelete(nuIndex) || '): ' || sqlerrm,1);
  END;
  nuIndex := EXP_UNITTYPE_342_.tbObjectToDelete.next(nuIndex);
 END loop;
 ut_trace.trace('Finaliza borrado de objetos de reglas',1);
EXCEPTION 
 when ex.controlled_error then
...
begin
SA_BOCreatePackages.DropPackage('EXP_UNITTYPE_342_');
end;
/

BEGIN ut_trace.trace('********************FIN  proceso de objeto:EXP_UNITTYPE_342_******************************'); end;
/
...
EXP_UNITTYPE_283_.blProcessStatus := EXP_PROCESS_342_.blProcessStatus ; 
exception when others then
rollback;
ut_trace.trace('**ERROR:'||sqlerrm,1);
raise;
...
BEGIN
ut_trace.trace('Actualizar Variable de Proceso: EXP_PROCESS_342',1);
EXP_PROCESS_342_.blProcessStatus := EXP_UNITTYPE_283_.blProcessStatus ; 
exception when others then
rollback;
ut_trace.trace('**ERROR:'||sqlerrm,1);
raise;
...
EXP_UNITTYPE_347_.blProcessStatus := EXP_PROCESS_342_.blProcessStatus ; 
exception when others then
rollback;
ut_trace.trace('**ERROR:'||sqlerrm,1);
raise;
...
BEGIN
ut_trace.trace('Actualizar Variable de Proceso: EXP_PROCESS_342',1);
EXP_PROCESS_342_.blProcessStatus := EXP_UNITTYPE_347_.blProcessStatus ; 
exception when others then
rollback;
ut_trace.trace('**ERROR:'||sqlerrm,1);
raise;
...
EXP_UNITTYPE_252_.blProcessStatus := EXP_PROCESS_342_.blProcessStatus ; 
exception when others then
rollback;
ut_trace.trace('**ERROR:'||sqlerrm,1);
raise;
...
BEGIN
ut_trace.trace('Actualizar Variable de Proceso: EXP_PROCESS_342',1);
EXP_PROCESS_342_.blProcessStatus := EXP_UNITTYPE_252_.blProcessStatus ; 
exception when others then
rollback;
ut_trace.trace('**ERROR:'||sqlerrm,1);
raise;
...
EXP_UNITTYPE_348_.blProcessStatus := EXP_PROCESS_342_.blProcessStatus ; 
exception when others then
rollback;
ut_trace.trace('**ERROR:'||sqlerrm,1);
raise;
...
BEGIN
ut_trace.trace('Actualizar Variable de Proceso: EXP_PROCESS_342',1);
EXP_PROCESS_342_.blProcessStatus := EXP_UNITTYPE_348_.blProcessStatus ; 
exception when others then
rollback;
ut_trace.trace('**ERROR:'||sqlerrm,1);
raise;
...
BEGIN
sa_bocreatePackages.CreatePackage('EXP_UNITTYPE_345_',
'CREATE OR REPLACE PACKAGE EXP_UNITTYPE_345_ IS ' || chr(10) ||
'blProcessStatus boolean := true; ' || chr(10) ||
'type tyWF_UNIT_TYPERowId is table of rowid index by varchar2(100); ' || chr(10) ||
'tbWF_UNIT_TYPERowId tyWF_UNIT_TYPERowId;type tyWF_CAUSAL_UNIT_TYPERowId is table of rowid index by varchar2(100); ' || chr(10) ||
'tbWF_CAUSAL_UNIT_TYPERowId tyWF_CAUSAL_UNIT_TYPERowId;type tyGR_CONFIGURA_TYPERowId is table of rowid index by varchar2(100); ' || chr(10) ||
...
'tbObjectToDelete tyObjectToDelete; ' || chr(10) ||
' ' || chr(10) ||
'END EXP_UNITTYPE_345_;');
END;
/


BEGIN ut_trace.trace('********************Comenzar proceso de objeto:EXP_UNITTYPE_345_******************************'); END;
/

BEGIN
ut_trace.trace('Actualizar Variable de Proceso: EXP_UNITTYPE_345',1);
EXP_UNITTYPE_345_.blProcessStatus := EXP_PROCESS_342_.blProcessStatus ; 
exception when others then
rollback;
ut_trace.trace('**ERROR:'||sqlerrm,1);
raise;
...
if (not EXP_UNITTYPE_345_.blProcessStatus) then
 return;
end if;
 nuIndex := 1;
 FOR crObjectsName IN cuObjectsName LOOP
  EXP_UNITTYPE_345_.tbObjectToDelete(nuIndex) := crObjectsName.OBJECT_NAME;
  nuIndex := nuIndex + 1;
 END LOOP;
EXCEPTION 
 when ex.controlled_error then
...
CURSOR cuRowIdToDelete is
SELECT rowid
FROM WF_UNIT_TYPE_ATTRIB WHERE (UNIT_TYPE_ID) in (SELECT UNIT_TYPE_ID FROM WF_UNIT_TYPE WHERE UNIT_TYPE_ID=345);
nuIndex binary_integer;
BEGIN
...
exception when others then
EXP_UNITTYPE_345_.blProcessStatus := false;
rollback;
ut_trace.trace('**ERROR:'||sqlerrm);
raise;
END; 
...
CURSOR cuRowIdToDelete is
SELECT rowid
FROM WF_CAUSAL_UNIT_TYPE WHERE (UNIT_TYPE_ID) in (SELECT UNIT_TYPE_ID FROM WF_UNIT_TYPE WHERE UNIT_TYPE_ID=345);
nuIndex binary_integer;
BEGIN
...
CURSOR cuRowIdToDelete is
SELECT rowid
FROM MO_TIME_UNI_TYP_PRIO WHERE (UNIT_TYPE_ID) in (SELECT UNIT_TYPE_ID FROM WF_UNIT_TYPE WHERE UNIT_TYPE_ID=345);
nuIndex binary_integer;
BEGIN
...
CURSOR cuRowIdToDelete is
SELECT rowid
FROM OR_ACT_BY_TASK_MOD WHERE (TASK_CODE) in (SELECT UNIT_TYPE_ID FROM WF_UNIT_TYPE WHERE UNIT_TYPE_ID=345);
nuIndex binary_integer;
BEGIN
...
CURSOR cuRowIdToDelete is
SELECT rowid
FROM WF_UNIT_TYPE WHERE UNIT_TYPE_ID=345;
nuIndex binary_integer;
BEGIN
...
EXP_UNITTYPE_345_.tb0_0(0):=345;
ut_trace.trace('insertando tabla sin fallo: WF_UNIT_TYPE fila (0)',1);
INSERT INTO WF_UNIT_TYPE(UNIT_TYPE_ID,CATEGORY_ID,MODULE_ID,ACTION_ID,PARENT_ID,INIT_AREA_EXPRESSION_ID,ASSIGN_COMMENT_CLASS,ATTEND_COMMENT_CLASS,UNASSIGN_COMMENT_CLASS,TAG_NAME,DESCRIPTION,DISPLAY,ICON,IS_STAGE_PROCESS,USABLE_IN_FLOW_STAGE,ENTITY_ID,MULTI_INSTANCE,IS_COUNTABLE,NOTIFICATION_ID,VIEWABLE,DEFAULT_PRIORITY_ID,INITIAL_NOTIFY_TIME,IS_ADMIN_PROCESS,ASSIGN_NOTIFICATION_ID,ASSIGNED_USER_ONLY) 
VALUES (EXP_UNITTYPE_345_.tb0_0(0),
1,
9,
null,
983,
...
BEGIN
ut_trace.trace('Actualizar Variable de Proceso: EXP_PROCESS_342',1);
EXP_PROCESS_342_.blProcessStatus := EXP_UNITTYPE_345_.blProcessStatus ; 
exception when others then
rollback;
ut_trace.trace('**ERROR:'||sqlerrm,1);
raise;
...
BEGIN
 ut_trace.trace('Inicia borrado de objetos de reglas',1);
 nuIndex := EXP_UNITTYPE_345_.tbObjectToDelete.first;
 while (nuIndex is not null) LOOP
  BEGIN
   ut_trace.trace('Objeto a borrar: ' || EXP_UNITTYPE_345_.tbObjectToDelete(nuIndex),1);
   IF(ut_object.fboExistObject(EXP_UNITTYPE_345_.tbObjectToDelete(nuIndex)) and not fblIsObjectUseByRule(EXP_UNITTYPE_345_.tbObjectToDelete(nuIndex))) THEN
    blObjectDeleted := ut_object.fboDeleteObject(EXP_UNITTYPE_345_.tbObjectToDelete(nuIndex));
    ut_trace.trace('  * Borrado ' || EXP_UNITTYPE_345_.tbObjectToDelete(nuIndex), 1);
   END IF;
  EXCEPTION when others then
   ut_trace.trace('- No se borra el objeto (' || EXP_UNITTYPE_345_.tbObjectToDelete(nuIndex) || '): ' || sqlerrm,1);
  END;
  nuIndex := EXP_UNITTYPE_345_.tbObjectToDelete.next(nuIndex);
 END loop;
 ut_trace.trace('Finaliza borrado de objetos de reglas',1);
EXCEPTION 
 when ex.controlled_error then
...
begin
SA_BOCreatePackages.DropPackage('EXP_UNITTYPE_345_');
end;
/

BEGIN ut_trace.trace('********************FIN  proceso de objeto:EXP_UNITTYPE_345_******************************'); end;
/
...
EXP_UNITTYPE_393_.blProcessStatus := EXP_PROCESS_342_.blProcessStatus ; 
exception when others then
rollback;
ut_trace.trace('**ERROR:'||sqlerrm,1);
raise;
...
BEGIN
ut_trace.trace('Actualizar Variable de Proceso: EXP_PROCESS_342',1);
EXP_PROCESS_342_.blProcessStatus := EXP_UNITTYPE_393_.blProcessStatus ; 
exception when others then
rollback;
ut_trace.trace('**ERROR:'||sqlerrm,1);
raise;
...
EXP_UNITTYPE_394_.blProcessStatus := EXP_PROCESS_342_.blProcessStatus ; 
exception when others then
rollback;
ut_trace.trace('**ERROR:'||sqlerrm,1);
raise;
...
BEGIN
ut_trace.trace('Actualizar Variable de Proceso: EXP_PROCESS_342',1);
EXP_PROCESS_342_.blProcessStatus := EXP_UNITTYPE_394_.blProcessStatus ; 
exception when others then
rollback;
ut_trace.trace('**ERROR:'||sqlerrm,1);
raise;
...
BEGIN
sa_bocreatePackages.CreatePackage('EXP_PROCESS_345_',
'CREATE OR REPLACE PACKAGE EXP_PROCESS_345_ IS ' || chr(10) ||
'blProcessStatus boolean := true; ' || chr(10) ||
'type tyWF_UNIT_TYPERowId is table of rowid index by varchar2(100); ' || chr(10) ||
'tbWF_UNIT_TYPERowId tyWF_UNIT_TYPERowId;type tyWF_UNIT_CATEGORYRowId is table of rowid index by varchar2(100); ' || chr(10) ||
'tbWF_UNIT_CATEGORYRowId tyWF_UNIT_CATEGORYRowId;type tyWF_ATTRIBUTES_EQUIVRowId is table of rowid index by varchar2(100); ' || chr(10) ||
...
'tbObjectToDelete tyObjectToDelete; ' || chr(10) ||
' ' || chr(10) ||
'END EXP_PROCESS_345_;');
END;
/


BEGIN ut_trace.trace('********************Comenzar proceso de objeto:EXP_PROCESS_345_******************************'); END;
/


BEGIN
   ut_trace.trace('Cargando tabla en memoria para posterior borrado GR_CONFIG_EXPRESSION',1);
   open EXP_PROCESS_345_.cuExpression;
   fetch EXP_PROCESS_345_.cuExpression bulk collect INTO EXP_PROCESS_345_.tbGR_CONFIG_EXPRESSIONId;
   close EXP_PROCESS_345_.cuExpression;
END;
/ 
BEGIN
ut_trace.trace('Actualizar Variable de Proceso: EXP_PROCESS_345',1);
EXP_PROCESS_345_.blProcessStatus := EXP_PROCESS_342_.blProcessStatus ; 
exception when others then
rollback;
ut_trace.trace('**ERROR:'||sqlerrm,1);
raise;
...
BEGIN
sa_bocreatePackages.CreatePackage('DEL_ROOT_673_',
'CREATE OR REPLACE PACKAGE DEL_ROOT_673_ IS ' || chr(10) ||
'blProcessStatus boolean := true; ' || chr(10) ||
'type tyWF_UNIT_TYPERowId is table of rowid index by varchar2(100); ' || chr(10) ||
'tbWF_UNIT_TYPERowId tyWF_UNIT_TYPERowId;type tyWF_UNIT_CATEGORYRowId is table of rowid index by varchar2(100); ' || chr(10) ||
'tbWF_UNIT_CATEGORYRowId tyWF_UNIT_CATEGORYRowId;type tyWF_ATTRIBUTES_EQUIVRowId is table of rowid index by varchar2(100); ' || chr(10) ||
...
'WHERE   UNIT_ID IN ( ' || chr(10) ||
'        SELECT UNIT_ID         FROM WF_UNIT  ' || chr(10) ||
'        WHERE UNIT_ID = 673 ' || chr(10) ||
'        UNION ALL         SELECT UNIT_ID         FROM WF_UNIT  ' || chr(10) ||
'        WHERE PROCESS_ID = 673  ' || chr(10) ||
'       ) ' || chr(10) ||
'AND   PRE_EXPRESSION_ID IS NOT NULL ' || chr(10) ||
'UNION ALL  ' || chr(10) ||
'SELECT POS_EXPRESSION_ID ' || chr(10) ||
...
BEGIN ut_trace.trace('********************Comenzar proceso de objeto:DEL_ROOT_673_******************************'); END;
/


BEGIN
   ut_trace.trace('Cargando tabla en memoria para posterior borrado GR_CONFIG_EXPRESSION',1);
   open DEL_ROOT_673_.cuExpression;
   fetch DEL_ROOT_673_.cuExpression bulk collect INTO DEL_ROOT_673_.tbGR_CONFIG_EXPRESSIONId;
   close DEL_ROOT_673_.cuExpression;
END;
/ 
...
AND     A.UNIT_ID IN (
        ...
        WHERE UNIT_ID = 673
        UNION ALL         SELECT UNIT_ID         FROM WF_UNIT 
        WHERE PROCESS_ID = 673 
       )
;
BEGIN

if (not DEL_ROOT_673_.blProcessStatus) then
 return;
end if;
 nuIndex := 1;
 FOR crObjectsName IN cuObjectsName LOOP
  DEL_ROOT_673_.tbObjectToDelete(nuIndex) := crObjectsName.OBJECT_NAME;
  nuIndex := nuIndex + 1;
 END LOOP;
EXCEPTION 
 when ex.controlled_error then
...
DECLARE 
 CURSOR cuLoadTemporaryTable IS 
 SELECT rowid FROM GR_CONFIG_EXPRESSION WHERE (CONFIG_EXPRESSION_ID) in (SELECT PRE_EXPRESSION_ID FROM WF_UNIT WHERE PROCESS_ID = 673);
BEGIN 
...
ut_trace.trace('Cargando a memoria tabla GR_CONFIG_EXPRESSION',1);
for rcData in cuLoadTemporaryTable loop
DEL_ROOT_673_.tbGR_CONFIG_EXPRESSIONRowId(rcData.rowid):=rcData.rowid; 
END loop; 

exception when others then
DEL_ROOT_673_.blProcessStatus := false;
rollback;
ut_trace.trace('**ERROR:'||sqlerrm);
raise;
END; 
...
DECLARE 
 CURSOR cuLoadTemporaryTable IS 
 SELECT rowid FROM GR_CONFIG_EXPRESSION WHERE (CONFIG_EXPRESSION_ID) in (SELECT POS_EXPRESSION_ID FROM WF_UNIT WHERE PROCESS_ID = 673);
BEGIN 
...
DECLARE 
 CURSOR cuLoadTemporaryTable IS 
 SELECT rowid FROM GR_CONFIG_EXPRESSION WHERE (CONFIG_EXPRESSION_ID) in (SELECT PRE_EXPRESSION_ID FROM WF_UNIT WHERE (UNIT_ID) in (SELECT TARGET_ID FROM WF_TRANSITION WHERE (TARGET_ID) in (SELECT UNIT_ID FROM WF_UNIT WHERE PROCESS_ID = 673)));
BEGIN 
...
DECLARE 
 CURSOR cuLoadTemporaryTable IS 
 SELECT rowid FROM GR_CONFIG_EXPRESSION WHERE (CONFIG_EXPRESSION_ID) in (SELECT POS_EXPRESSION_ID FROM WF_UNIT WHERE (UNIT_ID) in (SELECT TARGET_ID FROM WF_TRANSITION WHERE (TARGET_ID) in (SELECT UNIT_ID FROM WF_UNIT WHERE PROCESS_ID = 673)));
BEGIN 
...
DECLARE 
 CURSOR cuLoadTemporaryTable IS 
 SELECT rowid FROM WF_UNIT WHERE (UNIT_ID) in (SELECT TARGET_ID FROM WF_TRANSITION WHERE (TARGET_ID) in (SELECT UNIT_ID FROM WF_UNIT WHERE PROCESS_ID = 673));
BEGIN 
...
ut_trace.trace('Cargando a memoria tabla WF_UNIT',1);
for rcData in cuLoadTemporaryTable loop
DEL_ROOT_673_.tbWF_UNITRowId(rcData.rowid):=rcData.rowid; 
END loop; 
...
CURSOR cuRowIdToDelete is
SELECT rowid
FROM WF_TRANSITION WHERE (TARGET_ID) in (SELECT UNIT_ID FROM WF_UNIT WHERE PROCESS_ID = 673);
nuIndex binary_integer;
BEGIN
...
ut_trace.trace('Borrando tabla cargada en memoria GR_CONFIG_EXPRESSION',1);
nuVarcharIndex:=DEL_ROOT_673_.tbGR_CONFIG_EXPRESSIONRowId.first;  
while (nuVarcharIndex is not null) loop 
begin
ut_trace.trace('Row id a borrar:'||nuVarcharIndex,1);
delete from GR_CONFIG_EXPRESSION where rowid = DEL_ROOT_673_.tbGR_CONFIG_EXPRESSIONRowId(nuVarcharIndex); 
nuOldVarcharIndex:=nuVarcharIndex;
EXCEPTION
when ex.RECORD_HAVE_CHILDREN then
ut_trace.trace('No se pudo borrar el registro '||nuVarcharIndex,1);
...
END;
if nuOldVarcharIndex is not null then
DEL_ROOT_673_.tbGR_CONFIG_EXPRESSIONRowId.delete(nuOldVarcharIndex);
end if;
nuVarcharIndex := DEL_ROOT_673_.tbGR_CONFIG_EXPRESSIONRowId.next(nuVarcharIndex); 
END loop; 
...
ut_trace.trace('Borrando tabla cargada en memoria WF_UNIT',1);
nuVarcharIndex:=DEL_ROOT_673_.tbWF_UNITRowId.first;  
while (nuVarcharIndex is not null) loop 
begin
ut_trace.trace('Row id a borrar:'||nuVarcharIndex,1);
delete from WF_UNIT where rowid = DEL_ROOT_673_.tbWF_UNITRowId(nuVarcharIndex); 
nuOldVarcharIndex:=nuVarcharIndex;
EXCEPTION
when ex.RECORD_HAVE_CHILDREN then
ut_trace.trace('No se pudo borrar el registro '||nuVarcharIndex,1);
...
END;
if nuOldVarcharIndex is not null then
DEL_ROOT_673_.tbWF_UNITRowId.delete(nuOldVarcharIndex);
end if;
nuVarcharIndex := DEL_ROOT_673_.tbWF_UNITRowId.next(nuVarcharIndex); 
END loop; 
...
CURSOR cuRowIdToDelete is
SELECT rowid
FROM WF_UNIT_DATA_MAP WHERE (SOURCE_ID) in (SELECT UNIT_ATTRIBUTE_ID FROM WF_UNIT_ATTRIBUTE WHERE (UNIT_ID) in (SELECT UNIT_ID FROM WF_UNIT WHERE PROCESS_ID = 673));
nuIndex binary_integer;
BEGIN
...
CURSOR cuRowIdToDelete is
SELECT rowid
FROM WF_UNIT_DATA_MAP WHERE (TARGET_ID) in (SELECT UNIT_ATTRIBUTE_ID FROM WF_UNIT_ATTRIBUTE WHERE (UNIT_ID) in (SELECT UNIT_ID FROM WF_UNIT WHERE PROCESS_ID = 673));
nuIndex binary_integer;
BEGIN
...
CURSOR cuRowIdToDelete is
SELECT rowid
FROM WF_UNIT_ATTRIBUTE WHERE (UNIT_ID) in (SELECT UNIT_ID FROM WF_UNIT WHERE PROCESS_ID = 673);
nuIndex binary_integer;
BEGIN
...
CURSOR cuRowIdToDelete is
SELECT rowid
FROM WF_UNIT WHERE PROCESS_ID = 673;
nuIndex binary_integer;
BEGIN
...
DECLARE 
    nuBinaryIndex Binary_Integer;
BEGIN 
    nuBinaryIndex := DEL_ROOT_673_.tbGR_CONFIG_EXPRESSIONId.first;
    while (nuBinaryIndex IS not null) loop 
       BEGIN
           delete from GR_CONFIG_EXPRESSION where config_expression_id = DEL_ROOT_673_.tbGR_CONFIG_EXPRESSIONId(nuBinaryIndex);
           ut_trace.trace('Delete config_expression_id = : '||DEL_ROOT_673_.tbGR_CONFIG_EXPRESSIONId(nuBinaryIndex) || ' OK', 1);
       EXCEPTION
           when ex.RECORD_HAVE_CHILDREN then
               ut_trace.trace('No se pudo borrar la regla config_expression_id '||DEL_ROOT_673_.tbGR_CONFIG_EXPRESSIONId(nuBinaryIndex),1);
       END;
       nuBinaryIndex := DEL_ROOT_673_.tbGR_CONFIG_EXPRESSIONId.next(nuBinaryIndex);
    END loop;
...
BEGIN
ut_trace.trace('Actualizar Variable de Proceso: EXP_PROCESS_345',1);
EXP_PROCESS_345_.blProcessStatus := DEL_ROOT_673_.blProcessStatus ; 
exception when others then
rollback;
ut_trace.trace('**ERROR:'||sqlerrm,1);
raise;
...
BEGIN
 ut_trace.trace('Inicia borrado de objetos de reglas',1);
 nuIndex := DEL_ROOT_673_.tbObjectToDelete.first;
 while (nuIndex is not null) LOOP
  BEGIN
   ut_trace.trace('Objeto a borrar: ' || DEL_ROOT_673_.tbObjectToDelete(nuIndex),1);
   IF(ut_object.fboExistObject(DEL_ROOT_673_.tbObjectToDelete(nuIndex)) and not fblIsObjectUseByRule(DEL_ROOT_673_.tbObjectToDelete(nuIndex))) THEN
    blObjectDeleted := ut_object.fboDeleteObject(DEL_ROOT_673_.tbObjectToDelete(nuIndex));
    ut_trace.trace('  * Borrado ' || DEL_ROOT_673_.tbObjectToDelete(nuIndex), 1);
   END IF;
  EXCEPTION when others then
   ut_trace.trace('- No se borra el objeto (' || DEL_ROOT_673_.tbObjectToDelete(nuIndex) || '): ' || sqlerrm,1);
  END;
  nuIndex := DEL_ROOT_673_.tbObjectToDelete.next(nuIndex);
 END loop;
 ut_trace.trace('Finaliza borrado de objetos de reglas',1);
EXCEPTION 
 when ex.controlled_error then
...
begin
SA_BOCreatePackages.DropPackage('DEL_ROOT_673_');
end;
/

BEGIN ut_trace.trace('********************FIN  proceso de objeto:DEL_ROOT_673_******************************'); end;
/
...
DECLARE 
 CURSOR cuLoadTemporaryTable IS 
 SELECT rowid FROM GR_CONFIG_EXPRESSION WHERE (CONFIG_EXPRESSION_ID) in (SELECT PRE_EXPRESSION_ID FROM WF_UNIT WHERE UNIT_ID = 673);
BEGIN 
...
DECLARE 
 CURSOR cuLoadTemporaryTable IS 
 SELECT rowid FROM GR_CONFIG_EXPRESSION WHERE (CONFIG_EXPRESSION_ID) in (SELECT POS_EXPRESSION_ID FROM WF_UNIT WHERE UNIT_ID = 673);
BEGIN 
...
DECLARE 
 CURSOR cuLoadTemporaryTable IS 
 SELECT rowid FROM GR_CONFIG_EXPRESSION WHERE (CONFIG_EXPRESSION_ID) in (SELECT PRE_EXPRESSION_ID FROM WF_UNIT WHERE (UNIT_ID) in (SELECT TARGET_ID FROM WF_TRANSITION WHERE (TARGET_ID) in (SELECT UNIT_ID FROM WF_UNIT WHERE UNIT_ID = 673)));
BEGIN 
...
DECLARE 
 CURSOR cuLoadTemporaryTable IS 
 SELECT rowid FROM GR_CONFIG_EXPRESSION WHERE (CONFIG_EXPRESSION_ID) in (SELECT POS_EXPRESSION_ID FROM WF_UNIT WHERE (UNIT_ID) in (SELECT TARGET_ID FROM WF_TRANSITION WHERE (TARGET_ID) in (SELECT UNIT_ID FROM WF_UNIT WHERE UNIT_ID = 673)));
BEGIN 
...
DECLARE 
 CURSOR cuLoadTemporaryTable IS 
 SELECT rowid FROM WF_UNIT WHERE (UNIT_ID) in (SELECT TARGET_ID FROM WF_TRANSITION WHERE (TARGET_ID) in (SELECT UNIT_ID FROM WF_UNIT WHERE UNIT_ID = 673));
BEGIN 
...
CURSOR cuRowIdToDelete is
SELECT rowid
FROM WF_TRANSITION WHERE (TARGET_ID) in (SELECT UNIT_ID FROM WF_UNIT WHERE UNIT_ID = 673);
nuIndex binary_integer;
BEGIN
...
CURSOR cuRowIdToDelete is
SELECT rowid
FROM WF_UNIT_DATA_MAP WHERE (SOURCE_ID) in (SELECT UNIT_ATTRIBUTE_ID FROM WF_UNIT_ATTRIBUTE WHERE (UNIT_ID) in (SELECT UNIT_ID FROM WF_UNIT WHERE UNIT_ID = 673));
nuIndex binary_integer;
BEGIN
...
CURSOR cuRowIdToDelete is
SELECT rowid
FROM WF_UNIT_DATA_MAP WHERE (TARGET_ID) in (SELECT UNIT_ATTRIBUTE_ID FROM WF_UNIT_ATTRIBUTE WHERE (UNIT_ID) in (SELECT UNIT_ID FROM WF_UNIT WHERE UNIT_ID = 673));
nuIndex binary_integer;
BEGIN
...
CURSOR cuRowIdToDelete is
SELECT rowid
FROM WF_UNIT_ATTRIBUTE WHERE (UNIT_ID) in (SELECT UNIT_ID FROM WF_UNIT WHERE UNIT_ID = 673);
nuIndex binary_integer;
BEGIN
...
CURSOR cuRowIdToDelete is
SELECT rowid
FROM WF_UNIT WHERE UNIT_ID = 673;
nuIndex binary_integer;
BEGIN
...
BEGIN
sa_bocreatePackages.CreatePackage('EXP_ACTION_268_',
'CREATE OR REPLACE PACKAGE EXP_ACTION_268_ IS ' || chr(10) ||
'blProcessStatus boolean := true; ' || chr(10) ||
'type tyGR_CONFIGURA_TYPERowId is table of rowid index by varchar2(100); ' || chr(10) ||
'tbGR_CONFIGURA_TYPERowId tyGR_CONFIGURA_TYPERowId;type tyGR_CONFIG_EXPRESSIONRowId is table of rowid index by varchar2(100); ' || chr(10) ||
'tbGR_CONFIG_EXPRESSIONRowId tyGR_CONFIG_EXPRESSIONRowId;type tyGE_ACTION_MODULERowId is table of rowid index by varchar2(100); ' || chr(10) ||
...
BEGIN ut_trace.trace('********************Comenzar proceso de objeto:EXP_ACTION_268_******************************'); END;
/


BEGIN
   ut_trace.trace('Cargando tabla en memoria para posterior borrado GR_CONFIG_EXPRESSION',1);
   open EXP_ACTION_268_.cuExpression;
   fetch EXP_ACTION_268_.cuExpression bulk collect INTO EXP_ACTION_268_.tbGR_CONFIG_EXPRESSIONId;
   close EXP_ACTION_268_.cuExpression;
END;
/ 
BEGIN
ut_trace.trace('Actualizar Variable de Proceso: EXP_ACTION_268',1);
EXP_ACTION_268_.blProcessStatus := EXP_PROCESS_345_.blProcessStatus ; 
exception when others then
rollback;
ut_trace.trace('**ERROR:'||sqlerrm,1);
raise;
...
if (not EXP_ACTION_268_.blProcessStatus) then
 return;
end if;
 nuIndex := 1;
 FOR crObjectsName IN cuObjectsName LOOP
  EXP_ACTION_268_.tbObjectToDelete(nuIndex) := crObjectsName.OBJECT_NAME;
  nuIndex := nuIndex + 1;
 END LOOP;
EXCEPTION 
 when ex.controlled_error then
...
DECLARE 
 CURSOR cuLoadTemporaryTable IS 
 SELECT rowid FROM GR_CONFIG_EXPRESSION WHERE (CONFIG_EXPRESSION_ID) in (SELECT CONFIG_EXPRESSION_ID FROM GE_ACTION_MODULE WHERE ACTION_ID=268);
BEGIN 
...
ut_trace.trace('Cargando a memoria tabla GR_CONFIG_EXPRESSION',1);
for rcData in cuLoadTemporaryTable loop
EXP_ACTION_268_.tbGR_CONFIG_EXPRESSIONRowId(rcData.rowid):=rcData.rowid; 
END loop; 

exception when others then
EXP_ACTION_268_.blProcessStatus := false;
rollback;
ut_trace.trace('**ERROR:'||sqlerrm);
raise;
END; 
...
CURSOR cuRowIdToDelete is
SELECT rowid
FROM GE_ACTION_MODULE WHERE ACTION_ID=268;
nuIndex binary_integer;
BEGIN
...
ut_trace.trace('Borrando tabla cargada en memoria GR_CONFIG_EXPRESSION',1);
nuVarcharIndex:=EXP_ACTION_268_.tbGR_CONFIG_EXPRESSIONRowId.first;  
while (nuVarcharIndex is not null) loop 
begin
ut_trace.trace('Row id a borrar:'||nuVarcharIndex,1);
delete from GR_CONFIG_EXPRESSION where rowid = EXP_ACTION_268_.tbGR_CONFIG_EXPRESSIONRowId(nuVarcharIndex); 
nuOldVarcharIndex:=nuVarcharIndex;
EXCEPTION
when ex.RECORD_HAVE_CHILDREN then
ut_trace.trace('No se pudo borrar el registro '||nuVarcharIndex,1);
...
END;
if nuOldVarcharIndex is not null then
EXP_ACTION_268_.tbGR_CONFIG_EXPRESSIONRowId.delete(nuOldVarcharIndex);
end if;
nuVarcharIndex := EXP_ACTION_268_.tbGR_CONFIG_EXPRESSIONRowId.next(nuVarcharIndex); 
END loop; 
...
EXP_ACTION_268_.tb0_0(0):=1;
ut_trace.trace('Actualizar o insertar tabla: GR_CONFIGURA_TYPE fila (0)',1);
UPDATE GR_CONFIGURA_TYPE SET CONFIGURA_TYPE_ID=EXP_ACTION_268_.tb0_0(0),
MODULE_ID=1,
...
GENERATION_MASK='_EXEACTION_'
 ...
 WHERE CONFIGURA_TYPE_ID = EXP_ACTION_268_.tb0_0(0);
if not (sql%found) then
INSERT INTO GR_CONFIGURA_TYPE(CONFIGURA_TYPE_ID,MODULE_ID,DESCRIPTION,GENERATION_TYPE,EXECUTION_MODE,CALL_TYPE,GENERATION_MASK) 
VALUES (EXP_ACTION_268_.tb0_0(0),
1,
...
EXP_ACTION_268_.old_tb1_0(0):=121400397;
EXP_ACTION_268_.tb1_0(0):=GR_BOSEQUENCE.NEXTGR_CONFIG_EXPRESSION;
EXP_ACTION_268_.tb1_0(0):=EXP_ACTION_268_.tb1_0(0);
EXP_ACTION_268_.old_tb1_1(0):='GE_EXEACTION_CT1E121400397'
;
EXP_ACTION_268_.tb1_1(0):=TO_CHAR(EXP_ACTION_268_.tb1_0(0));
EXP_ACTION_268_.tb1_2(0):=EXP_ACTION_268_.tb0_0(0);
ut_trace.trace('insertando tabla: GR_CONFIG_EXPRESSION fila (0)',1);
INSERT INTO GR_CONFIG_EXPRESSION(CONFIG_EXPRESSION_ID,OBJECT_NAME,CONFIGURA_TYPE_ID,EXPRESSION,AUTHOR,CREATION_DATE,GENERATION_DATE,LAST_MODIFI_DATE,STATUS,USED_OTHER_EXPRESION,MODIFICATION_TYPE,PASSWORD,EXECUTION_TYPE,DESCRIPTION,OBJECT_TYPE,CODE) 
VALUES (EXP_ACTION_268_.tb1_0(0),
EXP_ACTION_268_.tb1_1(0),
EXP_ACTION_268_.tb1_2(0),
'nuPackageId = MO_BOINSTANCE_DB.FNUGETPACKIDINSTANCE();FI_BOFINANDEBTPROCESS.EXECDEBTFINANC(nuPackageId);nuActionId = GE_BOPARAMETER.FNUGET("ACTION_ATTEND", "N");MO_BOMOTIVEACTIONUTIL.EXECTRANSTATUSFORREQU(nuPackageId,nuActionId)'
,
'LBTEST'
,
to_date('31-08-2012 17:25:26','DD-MM-YYYY HH24:MI:SS'),
to_date('20-12-2023 15:52:05','DD-MM-YYYY HH24:MI:SS'),
to_date('20-12-2023 15:52:05','DD-MM-YYYY HH24:MI:SS'),
'G'
,
'N'
...
'DS'
,
'Atención Financiación de  Deuda'
,
'PP'
,
null);
...
EXP_ACTION_268_.tb2_0(0):=268;
EXP_ACTION_268_.tb2_2(0):=EXP_ACTION_268_.tb1_0(0);
ut_trace.trace('Actualizar o insertar tabla: GE_ACTION_MODULE fila (0)',1);
UPDATE GE_ACTION_MODULE SET ACTION_ID=EXP_ACTION_268_.tb2_0(0),
MODULE_ID=5,
CONFIG_EXPRESSION_ID=EXP_ACTION_268_.tb2_2(0),
DESCRIPTION='Atención Financiación de Deuda'
,
EXE_AT_STATUS_CHANGE='N'
,
EXE_TRANSITION_STATE='N'

 WHERE ACTION_ID = EXP_ACTION_268_.tb2_0(0);
if not (sql%found) then
INSERT INTO GE_ACTION_MODULE(ACTION_ID,MODULE_ID,CONFIG_EXPRESSION_ID,DESCRIPTION,EXE_AT_STATUS_CHANGE,EXE_TRANSITION_STATE) 
VALUES (EXP_ACTION_268_.tb2_0(0),
5,
EXP_ACTION_268_.tb2_2(0),
'Atención Financiación de Deuda'
,
'N'
,
...
EXP_ACTION_268_.tb3_0(0):=EXP_ACTION_268_.tb2_0(0);
EXP_ACTION_268_.tb3_1(0):=5;
ut_trace.trace('insertando tabla sin fallo: GE_VALID_ACTION_MODU fila (0)',1);
INSERT INTO GE_VALID_ACTION_MODU(ACTION_ID,VALID_MODULE_ID) 
VALUES (EXP_ACTION_268_.tb3_0(0),
EXP_ACTION_268_.tb3_1(0));
...
EXP_ACTION_268_.tb3_0(1):=EXP_ACTION_268_.tb2_0(0);
EXP_ACTION_268_.tb3_1(1):=9;
ut_trace.trace('insertando tabla sin fallo: GE_VALID_ACTION_MODU fila (1)',1);
INSERT INTO GE_VALID_ACTION_MODU(ACTION_ID,VALID_MODULE_ID) 
VALUES (EXP_ACTION_268_.tb3_0(1),
EXP_ACTION_268_.tb3_1(1));
...
EXP_ACTION_268_.tb3_0(2):=EXP_ACTION_268_.tb2_0(0);
EXP_ACTION_268_.tb3_1(2):=16;
ut_trace.trace('insertando tabla sin fallo: GE_VALID_ACTION_MODU fila (2)',1);
INSERT INTO GE_VALID_ACTION_MODU(ACTION_ID,VALID_MODULE_ID) 
VALUES (EXP_ACTION_268_.tb3_0(2),
EXP_ACTION_268_.tb3_1(2));
...
DECLARE 
    nuBinaryIndex Binary_Integer;
BEGIN 
    nuBinaryIndex := EXP_ACTION_268_.tbGR_CONFIG_EXPRESSIONId.first;
    while (nuBinaryIndex IS not null) loop 
       BEGIN
           delete from GR_CONFIG_EXPRESSION where config_expression_id = EXP_ACTION_268_.tbGR_CONFIG_EXPRESSIONId(nuBinaryIndex);
           ut_trace.trace('Delete config_expression_id = : '||EXP_ACTION_268_.tbGR_CONFIG_EXPRESSIONId(nuBinaryIndex) || ' OK', 1);
       EXCEPTION
           when ex.RECORD_HAVE_CHILDREN then
               ut_trace.trace('No se pudo borrar la regla config_expression_id '||EXP_ACTION_268_.tbGR_CONFIG_EXPRESSIONId(nuBinaryIndex),1);
       END;
       nuBinaryIndex := EXP_ACTION_268_.tbGR_CONFIG_EXPRESSIONId.next(nuBinaryIndex);
    END loop;
...
BEGIN
ut_trace.trace('Actualizar Variable de Proceso: EXP_PROCESS_345',1);
EXP_PROCESS_345_.blProcessStatus := EXP_ACTION_268_.blProcessStatus ; 
exception when others then
rollback;
ut_trace.trace('**ERROR:'||sqlerrm,1);
raise;
...
nuRowProcess:=EXP_ACTION_268_.tb1_0.first;
while (nuRowProcess is not null) loop 
ut_trace.trace('Genera expresión regla:'|| EXP_ACTION_268_.tb1_0(nuRowProcess),1); 
begin
GR_BOINTERFACE_BODY.MakeExpression(EXP_ACTION_268_.tb1_0(nuRowProcess)); 
exception when others then
ut_trace.trace('Fallo la generacion de la regla:'|| EXP_ACTION_268_.tb1_0(nuRowProcess),1);
end;
nuRowProcess := EXP_ACTION_268_.tb1_0.next(nuRowProcess);
END loop;
COMMIT;
...
BEGIN
 ut_trace.trace('Inicia borrado de objetos de reglas',1);
 nuIndex := EXP_ACTION_268_.tbObjectToDelete.first;
 while (nuIndex is not null) LOOP
  BEGIN
   ut_trace.trace('Objeto a borrar: ' || EXP_ACTION_268_.tbObjectToDelete(nuIndex),1);
   IF(ut_object.fboExistObject(EXP_ACTION_268_.tbObjectToDelete(nuIndex)) and not fblIsObjectUseByRule(EXP_ACTION_268_.tbObjectToDelete(nuIndex))) THEN
    blObjectDeleted := ut_object.fboDeleteObject(EXP_ACTION_268_.tbObjectToDelete(nuIndex));
    ut_trace.trace('  * Borrado ' || EXP_ACTION_268_.tbObjectToDelete(nuIndex), 1);
   END IF;
  EXCEPTION when others then
   ut_trace.trace('- No se borra el objeto (' || EXP_ACTION_268_.tbObjectToDelete(nuIndex) || '): ' || sqlerrm,1);
  END;
  nuIndex := EXP_ACTION_268_.tbObjectToDelete.next(nuIndex);
 END loop;
 ut_trace.trace('Finaliza borrado de objetos de reglas',1);
EXCEPTION 
 when ex.controlled_error then
...
begin
SA_BOCreatePackages.DropPackage('EXP_ACTION_268_');
end;
/

BEGIN ut_trace.trace('********************FIN  proceso de objeto:EXP_ACTION_268_******************************'); end;
/
...
if (not EXP_PROCESS_345_.blProcessStatus) then
 return;
end if;
 nuIndex := 1;
 FOR crObjectsName IN cuObjectsName LOOP
  EXP_PROCESS_345_.tbObjectToDelete(nuIndex) := crObjectsName.OBJECT_NAME;
  nuIndex := nuIndex + 1;
 END LOOP;
EXCEPTION 
 when ex.controlled_error then
...
DECLARE 
 CURSOR cuLoadTemporaryTable IS 
 SELECT rowid FROM GR_CONFIG_EXPRESSION WHERE (CONFIG_EXPRESSION_ID) in (SELECT PRE_EXPRESSION_ID FROM WF_UNIT WHERE (UNIT_TYPE_ID) in (SELECT UNIT_TYPE_ID FROM WF_UNIT_TYPE WHERE UNIT_TYPE_ID=345) AND NODE_TYPE_ID=0);
BEGIN 
...
ut_trace.trace('Cargando a memoria tabla GR_CONFIG_EXPRESSION',1);
for rcData in cuLoadTemporaryTable loop
EXP_PROCESS_345_.tbGR_CONFIG_EXPRESSIONRowId(rcData.rowid):=rcData.rowid; 
END loop; 

exception when others then
EXP_PROCESS_345_.blProcessStatus := false;
rollback;
ut_trace.trace('**ERROR:'||sqlerrm);
raise;
END; 
...
DECLARE 
 CURSOR cuLoadTemporaryTable IS 
 SELECT rowid FROM GR_CONFIG_EXPRESSION WHERE (CONFIG_EXPRESSION_ID) in (SELECT POS_EXPRESSION_ID FROM WF_UNIT WHERE (UNIT_TYPE_ID) in (SELECT UNIT_TYPE_ID FROM WF_UNIT_TYPE WHERE UNIT_TYPE_ID=345) AND NODE_TYPE_ID=0);
BEGIN 
...
CURSOR cuRowIdToDelete is
SELECT rowid
FROM WF_UNIT WHERE (UNIT_TYPE_ID) in (SELECT UNIT_TYPE_ID FROM WF_UNIT_TYPE WHERE UNIT_TYPE_ID=345) AND NODE_TYPE_ID=0;
nuIndex binary_integer;
BEGIN
...
ut_trace.trace('Borrando tabla cargada en memoria GR_CONFIG_EXPRESSION',1);
nuVarcharIndex:=EXP_PROCESS_345_.tbGR_CONFIG_EXPRESSIONRowId.first;  
while (nuVarcharIndex is not null) loop 
begin
ut_trace.trace('Row id a borrar:'||nuVarcharIndex,1);
delete from GR_CONFIG_EXPRESSION where rowid = EXP_PROCESS_345_.tbGR_CONFIG_EXPRESSIONRowId(nuVarcharIndex); 
nuOldVarcharIndex:=nuVarcharIndex;
EXCEPTION
when ex.RECORD_HAVE_CHILDREN then
ut_trace.trace('No se pudo borrar el registro '||nuVarcharIndex,1);
...
END;
if nuOldVarcharIndex is not null then
EXP_PROCESS_345_.tbGR_CONFIG_EXPRESSIONRowId.delete(nuOldVarcharIndex);
end if;
nuVarcharIndex := EXP_PROCESS_345_.tbGR_CONFIG_EXPRESSIONRowId.next(nuVarcharIndex); 
END loop; 
...
EXP_PROCESS_345_.tb0_0(0):=1;
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT_CATEGORY fila (0)',1);
UPDATE WF_UNIT_CATEGORY SET CATEGORY_ID=EXP_PROCESS_345_.tb0_0(0),
DISPLAY_NUMBER='Proceso B¿sico'

 WHERE CATEGORY_ID = EXP_PROCESS_345_.tb0_0(0);
if not (sql%found) then
INSERT INTO WF_UNIT_CATEGORY(CATEGORY_ID,DISPLAY_NUMBER) 
VALUES (EXP_PROCESS_345_.tb0_0(0),
'Proceso B¿sico'
);
end if;
...
EXP_PROCESS_345_.tb1_0(0):=9;
ut_trace.trace('Actualizar o insertar tabla: GE_MODULE fila (0)',1);
UPDATE GE_MODULE SET MODULE_ID=EXP_PROCESS_345_.tb1_0(0),
DESCRIPTION='WorkFlow'
,
MNEMONIC='WF'
,
...
LOCALIZATION='IN'
 ...
 WHERE MODULE_ID = EXP_PROCESS_345_.tb1_0(0);
if not (sql%found) then
INSERT INTO GE_MODULE(MODULE_ID,DESCRIPTION,MNEMONIC,LAST_MESSAGE,PATH_MODULE,ICON_NAME,LOCALIZATION) 
VALUES (EXP_PROCESS_345_.tb1_0(0),
'WorkFlow'
,
'WF'
,
...
EXP_PROCESS_345_.tb2_0(0):=345;
EXP_PROCESS_345_.tb2_1(0):=EXP_PROCESS_345_.tb0_0(0);
EXP_PROCESS_345_.tb2_2(0):=EXP_PROCESS_345_.tb1_0(0);
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT_TYPE fila (0)',1);
UPDATE WF_UNIT_TYPE SET UNIT_TYPE_ID=EXP_PROCESS_345_.tb2_0(0),
CATEGORY_ID=EXP_PROCESS_345_.tb2_1(0),
MODULE_ID=EXP_PROCESS_345_.tb2_2(0),
ACTION_ID=null,
PARENT_ID=983,
INIT_AREA_EXPRESSION_ID=null,
ASSIGN_COMMENT_CLASS=null,
...
ASSIGN_NOTIFICATION_ID=null,
ASSIGNED_USER_ONLY=null
 WHERE UNIT_TYPE_ID = EXP_PROCESS_345_.tb2_0(0);
if not (sql%found) then
INSERT INTO WF_UNIT_TYPE(UNIT_TYPE_ID,CATEGORY_ID,MODULE_ID,ACTION_ID,PARENT_ID,INIT_AREA_EXPRESSION_ID,ASSIGN_COMMENT_CLASS,ATTEND_COMMENT_CLASS,UNASSIGN_COMMENT_CLASS,TAG_NAME,DESCRIPTION,DISPLAY,ICON,IS_STAGE_PROCESS,USABLE_IN_FLOW_STAGE,ENTITY_ID,MULTI_INSTANCE,IS_COUNTABLE,NOTIFICATION_ID,VIEWABLE,DEFAULT_PRIORITY_ID,INITIAL_NOTIFY_TIME,IS_ADMIN_PROCESS,ASSIGN_NOTIFICATION_ID,ASSIGNED_USER_ONLY) 
VALUES (EXP_PROCESS_345_.tb2_0(0),
EXP_PROCESS_345_.tb2_1(0),
EXP_PROCESS_345_.tb2_2(0),
null,
983,
null,
null,
...
EXP_PROCESS_345_.tb3_0(0):=0;
ut_trace.trace('Actualizar o insertar tabla: WF_NODE_TYPE fila (0)',1);
UPDATE WF_NODE_TYPE SET NODE_TYPE_ID=EXP_PROCESS_345_.tb3_0(0),
DESCRIPTION='Ra¿z'

 WHERE NODE_TYPE_ID = EXP_PROCESS_345_.tb3_0(0);
if not (sql%found) then
INSERT INTO WF_NODE_TYPE(NODE_TYPE_ID,DESCRIPTION) 
VALUES (EXP_PROCESS_345_.tb3_0(0),
'Ra¿z'
);
end if;
...
EXP_PROCESS_345_.tb4_0(0):=673;
EXP_PROCESS_345_.tb4_2(0):=EXP_PROCESS_345_.tb2_0(0);
EXP_PROCESS_345_.tb4_3(0):=EXP_PROCESS_345_.tb3_0(0);
EXP_PROCESS_345_.tb4_4(0):=EXP_PROCESS_345_.tb1_0(0);
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT fila (0)',1);
UPDATE WF_UNIT SET UNIT_ID=EXP_PROCESS_345_.tb4_0(0),
PROCESS_ID=null,
UNIT_TYPE_ID=EXP_PROCESS_345_.tb4_2(0),
NODE_TYPE_ID=EXP_PROCESS_345_.tb4_3(0),
MODULE_ID=EXP_PROCESS_345_.tb4_4(0),
ACTION_ID=null,
PRE_EXPRESSION_ID=null,
POS_EXPRESSION_ID=null,
NOTIFICATION_ID=null,
...
VIEWABLE='N'
 ...
 WHERE UNIT_ID = EXP_PROCESS_345_.tb4_0(0);
if not (sql%found) then
INSERT INTO WF_UNIT(UNIT_ID,PROCESS_ID,UNIT_TYPE_ID,NODE_TYPE_ID,MODULE_ID,ACTION_ID,PRE_EXPRESSION_ID,POS_EXPRESSION_ID,NOTIFICATION_ID,GEOMETRY,DESCRIPTION,ONLINE_EXEC_ID,MULTI_INSTANCE,SINCRONIC_TIMEOUT,ASINCRONIC_TIMEOUT,FUNCTION_TYPE,IS_COUNTABLE,MIN_GROUP_SIZE,EXECUTION_ORDER,ANNULATION_ORDER,ENTITY_ID,VIEWABLE) 
VALUES (EXP_PROCESS_345_.tb4_0(0),
null,
EXP_PROCESS_345_.tb4_2(0),
EXP_PROCESS_345_.tb4_3(0),
EXP_PROCESS_345_.tb4_4(0),
null,
null,
null,
null,
...
EXP_PROCESS_345_.tb0_0(1):=2;
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT_CATEGORY fila (1)',1);
UPDATE WF_UNIT_CATEGORY SET CATEGORY_ID=EXP_PROCESS_345_.tb0_0(1),
DISPLAY_NUMBER='Actividad'

 WHERE CATEGORY_ID = EXP_PROCESS_345_.tb0_0(1);
if not (sql%found) then
INSERT INTO WF_UNIT_CATEGORY(CATEGORY_ID,DISPLAY_NUMBER) 
VALUES (EXP_PROCESS_345_.tb0_0(1),
'Actividad'
);
end if;
...
EXP_PROCESS_345_.tb2_0(1):=283;
EXP_PROCESS_345_.tb2_1(1):=EXP_PROCESS_345_.tb0_0(1);
EXP_PROCESS_345_.tb2_2(1):=EXP_PROCESS_345_.tb1_0(0);
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT_TYPE fila (1)',1);
UPDATE WF_UNIT_TYPE SET UNIT_TYPE_ID=EXP_PROCESS_345_.tb2_0(1),
CATEGORY_ID=EXP_PROCESS_345_.tb2_1(1),
MODULE_ID=EXP_PROCESS_345_.tb2_2(1),
ACTION_ID=null,
PARENT_ID=983,
INIT_AREA_EXPRESSION_ID=null,
ASSIGN_COMMENT_CLASS=null,
...
ASSIGN_NOTIFICATION_ID=null,
ASSIGNED_USER_ONLY=null
 WHERE UNIT_TYPE_ID = EXP_PROCESS_345_.tb2_0(1);
if not (sql%found) then
INSERT INTO WF_UNIT_TYPE(UNIT_TYPE_ID,CATEGORY_ID,MODULE_ID,ACTION_ID,PARENT_ID,INIT_AREA_EXPRESSION_ID,ASSIGN_COMMENT_CLASS,ATTEND_COMMENT_CLASS,UNASSIGN_COMMENT_CLASS,TAG_NAME,DESCRIPTION,DISPLAY,ICON,IS_STAGE_PROCESS,USABLE_IN_FLOW_STAGE,ENTITY_ID,MULTI_INSTANCE,IS_COUNTABLE,NOTIFICATION_ID,VIEWABLE,DEFAULT_PRIORITY_ID,INITIAL_NOTIFY_TIME,IS_ADMIN_PROCESS,ASSIGN_NOTIFICATION_ID,ASSIGNED_USER_ONLY) 
VALUES (EXP_PROCESS_345_.tb2_0(1),
EXP_PROCESS_345_.tb2_1(1),
EXP_PROCESS_345_.tb2_2(1),
null,
983,
null,
null,
...
EXP_PROCESS_345_.tb3_0(1):=1;
ut_trace.trace('Actualizar o insertar tabla: WF_NODE_TYPE fila (1)',1);
UPDATE WF_NODE_TYPE SET NODE_TYPE_ID=EXP_PROCESS_345_.tb3_0(1),
DESCRIPTION='Arranque'

 WHERE NODE_TYPE_ID = EXP_PROCESS_345_.tb3_0(1);
if not (sql%found) then
INSERT INTO WF_NODE_TYPE(NODE_TYPE_ID,DESCRIPTION) 
VALUES (EXP_PROCESS_345_.tb3_0(1),
'Arranque'
);
end if;
...
EXP_PROCESS_345_.tb4_0(1):=675;
EXP_PROCESS_345_.tb4_1(1):=EXP_PROCESS_345_.tb4_0(0);
EXP_PROCESS_345_.tb4_2(1):=EXP_PROCESS_345_.tb2_0(1);
EXP_PROCESS_345_.tb4_3(1):=EXP_PROCESS_345_.tb3_0(1);
EXP_PROCESS_345_.tb4_4(1):=EXP_PROCESS_345_.tb1_0(0);
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT fila (1)',1);
UPDATE WF_UNIT SET UNIT_ID=EXP_PROCESS_345_.tb4_0(1),
PROCESS_ID=EXP_PROCESS_345_.tb4_1(1),
UNIT_TYPE_ID=EXP_PROCESS_345_.tb4_2(1),
NODE_TYPE_ID=EXP_PROCESS_345_.tb4_3(1),
MODULE_ID=EXP_PROCESS_345_.tb4_4(1),
ACTION_ID=null,
PRE_EXPRESSION_ID=null,
POS_EXPRESSION_ID=null,
NOTIFICATION_ID=null,
...
VIEWABLE='N'
 ...
 WHERE UNIT_ID = EXP_PROCESS_345_.tb4_0(1);
if not (sql%found) then
INSERT INTO WF_UNIT(UNIT_ID,PROCESS_ID,UNIT_TYPE_ID,NODE_TYPE_ID,MODULE_ID,ACTION_ID,PRE_EXPRESSION_ID,POS_EXPRESSION_ID,NOTIFICATION_ID,GEOMETRY,DESCRIPTION,ONLINE_EXEC_ID,MULTI_INSTANCE,SINCRONIC_TIMEOUT,ASINCRONIC_TIMEOUT,FUNCTION_TYPE,IS_COUNTABLE,MIN_GROUP_SIZE,EXECUTION_ORDER,ANNULATION_ORDER,ENTITY_ID,VIEWABLE) 
VALUES (EXP_PROCESS_345_.tb4_0(1),
EXP_PROCESS_345_.tb4_1(1),
EXP_PROCESS_345_.tb4_2(1),
EXP_PROCESS_345_.tb4_3(1),
EXP_PROCESS_345_.tb4_4(1),
null,
null,
null,
null,
...
EXP_PROCESS_345_.tb1_0(1):=5;
ut_trace.trace('Actualizar o insertar tabla: GE_MODULE fila (1)',1);
UPDATE GE_MODULE SET MODULE_ID=EXP_PROCESS_345_.tb1_0(1),
DESCRIPTION='GESTI¿N DE MOTIVOS'
,
MNEMONIC='MO'
,
...
LOCALIZATION='IN'
 ...
 WHERE MODULE_ID = EXP_PROCESS_345_.tb1_0(1);
if not (sql%found) then
INSERT INTO GE_MODULE(MODULE_ID,DESCRIPTION,MNEMONIC,LAST_MESSAGE,PATH_MODULE,ICON_NAME,LOCALIZATION) 
VALUES (EXP_PROCESS_345_.tb1_0(1),
'GESTI¿N DE MOTIVOS'
,
'MO'
,
...
EXP_PROCESS_345_.tb2_0(2):=346;
EXP_PROCESS_345_.tb2_1(2):=EXP_PROCESS_345_.tb0_0(1);
EXP_PROCESS_345_.tb2_2(2):=EXP_PROCESS_345_.tb1_0(1);
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT_TYPE fila (2)',1);
UPDATE WF_UNIT_TYPE SET UNIT_TYPE_ID=EXP_PROCESS_345_.tb2_0(2),
CATEGORY_ID=EXP_PROCESS_345_.tb2_1(2),
MODULE_ID=EXP_PROCESS_345_.tb2_2(2),
ACTION_ID=null,
PARENT_ID=983,
INIT_AREA_EXPRESSION_ID=null,
ASSIGN_COMMENT_CLASS=null,
...
ASSIGN_NOTIFICATION_ID=null,
ASSIGNED_USER_ONLY=null
 WHERE UNIT_TYPE_ID = EXP_PROCESS_345_.tb2_0(2);
if not (sql%found) then
INSERT INTO WF_UNIT_TYPE(UNIT_TYPE_ID,CATEGORY_ID,MODULE_ID,ACTION_ID,PARENT_ID,INIT_AREA_EXPRESSION_ID,ASSIGN_COMMENT_CLASS,ATTEND_COMMENT_CLASS,UNASSIGN_COMMENT_CLASS,TAG_NAME,DESCRIPTION,DISPLAY,ICON,IS_STAGE_PROCESS,USABLE_IN_FLOW_STAGE,ENTITY_ID,MULTI_INSTANCE,IS_COUNTABLE,NOTIFICATION_ID,VIEWABLE,DEFAULT_PRIORITY_ID,INITIAL_NOTIFY_TIME,IS_ADMIN_PROCESS,ASSIGN_NOTIFICATION_ID,ASSIGNED_USER_ONLY) 
VALUES (EXP_PROCESS_345_.tb2_0(2),
EXP_PROCESS_345_.tb2_1(2),
EXP_PROCESS_345_.tb2_2(2),
null,
983,
null,
null,
...
EXP_PROCESS_345_.tb3_0(2):=2;
ut_trace.trace('Actualizar o insertar tabla: WF_NODE_TYPE fila (2)',1);
UPDATE WF_NODE_TYPE SET NODE_TYPE_ID=EXP_PROCESS_345_.tb3_0(2),
DESCRIPTION='Normal'

 WHERE NODE_TYPE_ID = EXP_PROCESS_345_.tb3_0(2);
if not (sql%found) then
INSERT INTO WF_NODE_TYPE(NODE_TYPE_ID,DESCRIPTION) 
VALUES (EXP_PROCESS_345_.tb3_0(2),
'Normal'
);
end if;
...
EXP_PROCESS_345_.tb4_0(2):=680;
EXP_PROCESS_345_.tb4_1(2):=EXP_PROCESS_345_.tb4_0(0);
EXP_PROCESS_345_.tb4_2(2):=EXP_PROCESS_345_.tb2_0(2);
EXP_PROCESS_345_.tb4_3(2):=EXP_PROCESS_345_.tb3_0(2);
EXP_PROCESS_345_.tb4_4(2):=EXP_PROCESS_345_.tb1_0(1);
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT fila (2)',1);
UPDATE WF_UNIT SET UNIT_ID=EXP_PROCESS_345_.tb4_0(2),
PROCESS_ID=EXP_PROCESS_345_.tb4_1(2),
UNIT_TYPE_ID=EXP_PROCESS_345_.tb4_2(2),
NODE_TYPE_ID=EXP_PROCESS_345_.tb4_3(2),
MODULE_ID=EXP_PROCESS_345_.tb4_4(2),
ACTION_ID=268,
PRE_EXPRESSION_ID=null,
POS_EXPRESSION_ID=null,
NOTIFICATION_ID=null,
...
VIEWABLE='Y'
 ...
 WHERE UNIT_ID = EXP_PROCESS_345_.tb4_0(2);
if not (sql%found) then
INSERT INTO WF_UNIT(UNIT_ID,PROCESS_ID,UNIT_TYPE_ID,NODE_TYPE_ID,MODULE_ID,ACTION_ID,PRE_EXPRESSION_ID,POS_EXPRESSION_ID,NOTIFICATION_ID,GEOMETRY,DESCRIPTION,ONLINE_EXEC_ID,MULTI_INSTANCE,SINCRONIC_TIMEOUT,ASINCRONIC_TIMEOUT,FUNCTION_TYPE,IS_COUNTABLE,MIN_GROUP_SIZE,EXECUTION_ORDER,ANNULATION_ORDER,ENTITY_ID,VIEWABLE) 
VALUES (EXP_PROCESS_345_.tb4_0(2),
EXP_PROCESS_345_.tb4_1(2),
EXP_PROCESS_345_.tb4_2(2),
EXP_PROCESS_345_.tb4_3(2),
EXP_PROCESS_345_.tb4_4(2),
268,
null,
null,
null,
...
EXP_PROCESS_345_.tb5_0(0):=123949;
EXP_PROCESS_345_.tb5_1(0):=EXP_PROCESS_345_.tb4_0(1);
EXP_PROCESS_345_.tb5_2(0):=EXP_PROCESS_345_.tb4_0(2);
ut_trace.trace('Actualizar o insertar tabla: WF_TRANSITION fila (0)',1);
UPDATE WF_TRANSITION SET TRANS_ID=EXP_PROCESS_345_.tb5_0(0),
ORIGIN_ID=EXP_PROCESS_345_.tb5_1(0),
TARGET_ID=EXP_PROCESS_345_.tb5_2(0),
GEOMETRY=null,
GROUP_ID=0,
EXPRESSION=null,
EXPRESSION_TYPE=0,
DESCRIPTION=null,
TRANSITION_TYPE_ID=1
 WHERE TRANS_ID = EXP_PROCESS_345_.tb5_0(0);
if not (sql%found) then
INSERT INTO WF_TRANSITION(TRANS_ID,ORIGIN_ID,TARGET_ID,GEOMETRY,GROUP_ID,EXPRESSION,EXPRESSION_TYPE,DESCRIPTION,TRANSITION_TYPE_ID) 
VALUES (EXP_PROCESS_345_.tb5_0(0),
EXP_PROCESS_345_.tb5_1(0),
EXP_PROCESS_345_.tb5_2(0),
null,
0,
null,
0,
...
EXP_PROCESS_345_.tb6_0(0):=1;
ut_trace.trace('Actualizar o insertar tabla: GE_ATTRIBUTE_CLASS fila (0)',1);
UPDATE GE_ATTRIBUTE_CLASS SET ATTRIBUTE_CLASS_ID=EXP_PROCESS_345_.tb6_0(0),
NAME='De Entitdad'
,
DESCRIPTION='Valores que ser¿n tra¿dos de la tabla base usando old record'

 WHERE ATTRIBUTE_CLASS_ID = EXP_PROCESS_345_.tb6_0(0);
if not (sql%found) then
INSERT INTO GE_ATTRIBUTE_CLASS(ATTRIBUTE_CLASS_ID,NAME,DESCRIPTION) 
VALUES (EXP_PROCESS_345_.tb6_0(0),
'De Entitdad'
,
'Valores que ser¿n tra¿dos de la tabla base usando old record'
);
...
EXP_PROCESS_345_.tb7_0(0):=1;
ut_trace.trace('Actualizar o insertar tabla: GE_ATTRIBUTES_TYPE fila (0)',1);
UPDATE GE_ATTRIBUTES_TYPE SET ATTRIBUTE_TYPE_ID=EXP_PROCESS_345_.tb7_0(0),
DESCRIPTION='NUMBER'
,
INTERNAL_TYPE=2,
INTERNAL_JAVA_TYPE=2
 WHERE ATTRIBUTE_TYPE_ID = EXP_PROCESS_345_.tb7_0(0);
if not (sql%found) then
INSERT INTO GE_ATTRIBUTES_TYPE(ATTRIBUTE_TYPE_ID,DESCRIPTION,INTERNAL_TYPE,INTERNAL_JAVA_TYPE) 
VALUES (EXP_PROCESS_345_.tb7_0(0),
'NUMBER'
,
2,
2);
...
EXP_PROCESS_345_.tb8_0(0):=400;
EXP_PROCESS_345_.tb8_1(0):=EXP_PROCESS_345_.tb6_0(0);
EXP_PROCESS_345_.tb8_2(0):=EXP_PROCESS_345_.tb7_0(0);
ut_trace.trace('Actualizar o insertar tabla: GE_ATTRIBUTES fila (0)',1);
UPDATE GE_ATTRIBUTES SET ATTRIBUTE_ID=EXP_PROCESS_345_.tb8_0(0),
ATTRIBUTE_CLASS_ID=EXP_PROCESS_345_.tb8_1(0),
ATTRIBUTE_TYPE_ID=EXP_PROCESS_345_.tb8_2(0),
VALID_EXPRESSION=null,
FATHER_ID=null,
MODULE_ID=9,
NAME_ATTRIBUTE='CAUSAL'
...
DISPLAY_NAME='Causal'
 ...
 WHERE ATTRIBUTE_ID = EXP_PROCESS_345_.tb8_0(0);
if not (sql%found) then
INSERT INTO GE_ATTRIBUTES(ATTRIBUTE_ID,ATTRIBUTE_CLASS_ID,ATTRIBUTE_TYPE_ID,VALID_EXPRESSION,FATHER_ID,MODULE_ID,NAME_ATTRIBUTE,LENGTH,PRECISION,SCALE,DEFAULT_VALUE,IS_FIX_OR_VARIABLE,COMMENT_,DISPLAY_NAME) 
VALUES (EXP_PROCESS_345_.tb8_0(0),
EXP_PROCESS_345_.tb8_1(0),
EXP_PROCESS_345_.tb8_2(0),
null,
null,
9,
'CAUSAL'
...
EXP_PROCESS_345_.tb9_0(0):=133716;
EXP_PROCESS_345_.tb9_1(0):=EXP_PROCESS_345_.tb4_0(1);
EXP_PROCESS_345_.tb9_2(0):=EXP_PROCESS_345_.tb8_0(0);
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT_ATTRIBUTE fila (0)',1);
UPDATE WF_UNIT_ATTRIBUTE SET UNIT_ATTRIBUTE_ID=EXP_PROCESS_345_.tb9_0(0),
UNIT_ID=EXP_PROCESS_345_.tb9_1(0),
ATTRIBUTE_ID=EXP_PROCESS_345_.tb9_2(0),
STATEMENT_ID=null,
VALUE=null,
IS_DUPLICABLE='N'
,
...
MANDATORY='N'
 ...
 WHERE UNIT_ATTRIBUTE_ID = EXP_PROCESS_345_.tb9_0(0);
if not (sql%found) then
INSERT INTO WF_UNIT_ATTRIBUTE(UNIT_ATTRIBUTE_ID,UNIT_ID,ATTRIBUTE_ID,STATEMENT_ID,VALUE,IS_DUPLICABLE,IN_OUT,MANDATORY) 
VALUES (EXP_PROCESS_345_.tb9_0(0),
EXP_PROCESS_345_.tb9_1(0),
EXP_PROCESS_345_.tb9_2(0),
null,
null,
'N'
,
...
EXP_PROCESS_345_.tb2_0(3):=252;
EXP_PROCESS_345_.tb2_1(3):=EXP_PROCESS_345_.tb0_0(1);
EXP_PROCESS_345_.tb2_2(3):=EXP_PROCESS_345_.tb1_0(0);
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT_TYPE fila (3)',1);
UPDATE WF_UNIT_TYPE SET UNIT_TYPE_ID=EXP_PROCESS_345_.tb2_0(3),
CATEGORY_ID=EXP_PROCESS_345_.tb2_1(3),
MODULE_ID=EXP_PROCESS_345_.tb2_2(3),
ACTION_ID=null,
PARENT_ID=983,
INIT_AREA_EXPRESSION_ID=null,
ASSIGN_COMMENT_CLASS=null,
...
ASSIGN_NOTIFICATION_ID=null,
ASSIGNED_USER_ONLY=null
 WHERE UNIT_TYPE_ID = EXP_PROCESS_345_.tb2_0(3);
if not (sql%found) then
INSERT INTO WF_UNIT_TYPE(UNIT_TYPE_ID,CATEGORY_ID,MODULE_ID,ACTION_ID,PARENT_ID,INIT_AREA_EXPRESSION_ID,ASSIGN_COMMENT_CLASS,ATTEND_COMMENT_CLASS,UNASSIGN_COMMENT_CLASS,TAG_NAME,DESCRIPTION,DISPLAY,ICON,IS_STAGE_PROCESS,USABLE_IN_FLOW_STAGE,ENTITY_ID,MULTI_INSTANCE,IS_COUNTABLE,NOTIFICATION_ID,VIEWABLE,DEFAULT_PRIORITY_ID,INITIAL_NOTIFY_TIME,IS_ADMIN_PROCESS,ASSIGN_NOTIFICATION_ID,ASSIGNED_USER_ONLY) 
VALUES (EXP_PROCESS_345_.tb2_0(3),
EXP_PROCESS_345_.tb2_1(3),
EXP_PROCESS_345_.tb2_2(3),
null,
983,
null,
null,
...
EXP_PROCESS_345_.tb3_0(3):=3;
ut_trace.trace('Actualizar o insertar tabla: WF_NODE_TYPE fila (3)',1);
UPDATE WF_NODE_TYPE SET NODE_TYPE_ID=EXP_PROCESS_345_.tb3_0(3),
DESCRIPTION='Final'

 WHERE NODE_TYPE_ID = EXP_PROCESS_345_.tb3_0(3);
if not (sql%found) then
INSERT INTO WF_NODE_TYPE(NODE_TYPE_ID,DESCRIPTION) 
VALUES (EXP_PROCESS_345_.tb3_0(3),
'Final'
);
end if;
...
EXP_PROCESS_345_.tb4_0(3):=678;
EXP_PROCESS_345_.tb4_1(3):=EXP_PROCESS_345_.tb4_0(0);
EXP_PROCESS_345_.tb4_2(3):=EXP_PROCESS_345_.tb2_0(3);
EXP_PROCESS_345_.tb4_3(3):=EXP_PROCESS_345_.tb3_0(3);
EXP_PROCESS_345_.tb4_4(3):=EXP_PROCESS_345_.tb1_0(0);
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT fila (3)',1);
UPDATE WF_UNIT SET UNIT_ID=EXP_PROCESS_345_.tb4_0(3),
PROCESS_ID=EXP_PROCESS_345_.tb4_1(3),
UNIT_TYPE_ID=EXP_PROCESS_345_.tb4_2(3),
NODE_TYPE_ID=EXP_PROCESS_345_.tb4_3(3),
MODULE_ID=EXP_PROCESS_345_.tb4_4(3),
ACTION_ID=null,
PRE_EXPRESSION_ID=null,
POS_EXPRESSION_ID=null,
NOTIFICATION_ID=null,
...
VIEWABLE='N'
 ...
 WHERE UNIT_ID = EXP_PROCESS_345_.tb4_0(3);
if not (sql%found) then
INSERT INTO WF_UNIT(UNIT_ID,PROCESS_ID,UNIT_TYPE_ID,NODE_TYPE_ID,MODULE_ID,ACTION_ID,PRE_EXPRESSION_ID,POS_EXPRESSION_ID,NOTIFICATION_ID,GEOMETRY,DESCRIPTION,ONLINE_EXEC_ID,MULTI_INSTANCE,SINCRONIC_TIMEOUT,ASINCRONIC_TIMEOUT,FUNCTION_TYPE,IS_COUNTABLE,MIN_GROUP_SIZE,EXECUTION_ORDER,ANNULATION_ORDER,ENTITY_ID,VIEWABLE) 
VALUES (EXP_PROCESS_345_.tb4_0(3),
EXP_PROCESS_345_.tb4_1(3),
EXP_PROCESS_345_.tb4_2(3),
EXP_PROCESS_345_.tb4_3(3),
EXP_PROCESS_345_.tb4_4(3),
null,
null,
null,
null,
...
EXP_PROCESS_345_.tb5_0(1):=123950;
EXP_PROCESS_345_.tb5_1(1):=EXP_PROCESS_345_.tb4_0(2);
EXP_PROCESS_345_.tb5_2(1):=EXP_PROCESS_345_.tb4_0(3);
ut_trace.trace('Actualizar o insertar tabla: WF_TRANSITION fila (1)',1);
UPDATE WF_TRANSITION SET TRANS_ID=EXP_PROCESS_345_.tb5_0(1),
ORIGIN_ID=EXP_PROCESS_345_.tb5_1(1),
TARGET_ID=EXP_PROCESS_345_.tb5_2(1),
GEOMETRY=null,
GROUP_ID=0,
EXPRESSION=null,
EXPRESSION_TYPE=0,
DESCRIPTION=null,
TRANSITION_TYPE_ID=1
 WHERE TRANS_ID = EXP_PROCESS_345_.tb5_0(1);
if not (sql%found) then
INSERT INTO WF_TRANSITION(TRANS_ID,ORIGIN_ID,TARGET_ID,GEOMETRY,GROUP_ID,EXPRESSION,EXPRESSION_TYPE,DESCRIPTION,TRANSITION_TYPE_ID) 
VALUES (EXP_PROCESS_345_.tb5_0(1),
EXP_PROCESS_345_.tb5_1(1),
EXP_PROCESS_345_.tb5_2(1),
null,
0,
null,
0,
...
EXP_PROCESS_345_.tb9_0(1):=133890;
EXP_PROCESS_345_.tb9_1(1):=EXP_PROCESS_345_.tb4_0(2);
EXP_PROCESS_345_.tb9_2(1):=EXP_PROCESS_345_.tb8_0(0);
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT_ATTRIBUTE fila (1)',1);
UPDATE WF_UNIT_ATTRIBUTE SET UNIT_ATTRIBUTE_ID=EXP_PROCESS_345_.tb9_0(1),
UNIT_ID=EXP_PROCESS_345_.tb9_1(1),
ATTRIBUTE_ID=EXP_PROCESS_345_.tb9_2(1),
STATEMENT_ID=null,
VALUE=null,
IS_DUPLICABLE='N'
,
...
MANDATORY='N'
 ...
 WHERE UNIT_ATTRIBUTE_ID = EXP_PROCESS_345_.tb9_0(1);
if not (sql%found) then
INSERT INTO WF_UNIT_ATTRIBUTE(UNIT_ATTRIBUTE_ID,UNIT_ID,ATTRIBUTE_ID,STATEMENT_ID,VALUE,IS_DUPLICABLE,IN_OUT,MANDATORY) 
VALUES (EXP_PROCESS_345_.tb9_0(1),
EXP_PROCESS_345_.tb9_1(1),
EXP_PROCESS_345_.tb9_2(1),
null,
null,
'N'
,
...
EXP_PROCESS_345_.tb9_0(2):=133717;
EXP_PROCESS_345_.tb9_1(2):=EXP_PROCESS_345_.tb4_0(3);
EXP_PROCESS_345_.tb9_2(2):=EXP_PROCESS_345_.tb8_0(0);
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT_ATTRIBUTE fila (2)',1);
UPDATE WF_UNIT_ATTRIBUTE SET UNIT_ATTRIBUTE_ID=EXP_PROCESS_345_.tb9_0(2),
UNIT_ID=EXP_PROCESS_345_.tb9_1(2),
ATTRIBUTE_ID=EXP_PROCESS_345_.tb9_2(2),
STATEMENT_ID=null,
VALUE=null,
IS_DUPLICABLE='N'
,
...
MANDATORY='N'
 ...
 WHERE UNIT_ATTRIBUTE_ID = EXP_PROCESS_345_.tb9_0(2);
if not (sql%found) then
INSERT INTO WF_UNIT_ATTRIBUTE(UNIT_ATTRIBUTE_ID,UNIT_ID,ATTRIBUTE_ID,STATEMENT_ID,VALUE,IS_DUPLICABLE,IN_OUT,MANDATORY) 
VALUES (EXP_PROCESS_345_.tb9_0(2),
EXP_PROCESS_345_.tb9_1(2),
EXP_PROCESS_345_.tb9_2(2),
null,
null,
'N'
,
...
EXP_PROCESS_345_.tb9_0(3):=133718;
EXP_PROCESS_345_.tb9_1(3):=EXP_PROCESS_345_.tb4_0(3);
EXP_PROCESS_345_.tb9_2(3):=EXP_PROCESS_345_.tb8_0(0);
ut_trace.trace('Actualizar o insertar tabla: WF_UNIT_ATTRIBUTE fila (3)',1);
UPDATE WF_UNIT_ATTRIBUTE SET UNIT_ATTRIBUTE_ID=EXP_PROCESS_345_.tb9_0(3),
UNIT_ID=EXP_PROCESS_345_.tb9_1(3),
ATTRIBUTE_ID=EXP_PROCESS_345_.tb9_2(3),
STATEMENT_ID=null,
VALUE=null,
IS_DUPLICABLE='N'
,
...
MANDATORY='N'
 ...
 WHERE UNIT_ATTRIBUTE_ID = EXP_PROCESS_345_.tb9_0(3);
if not (sql%found) then
INSERT INTO WF_UNIT_ATTRIBUTE(UNIT_ATTRIBUTE_ID,UNIT_ID,ATTRIBUTE_ID,STATEMENT_ID,VALUE,IS_DUPLICABLE,IN_OUT,MANDATORY) 
VALUES (EXP_PROCESS_345_.tb9_0(3),
EXP_PROCESS_345_.tb9_1(3),
EXP_PROCESS_345_.tb9_2(3),
null,
null,
'N'
,
...
BEGIN
sa_bocreatePackages.CreatePackage('EXP_UNITTYPE_346_',
'CREATE OR REPLACE PACKAGE EXP_UNITTYPE_346_ IS ' || chr(10) ||
'blProcessStatus boolean := true; ' || chr(10) ||
'type tyWF_UNIT_TYPERowId is table of rowid index by varchar2(100); ' || chr(10) ||
'tbWF_UNIT_TYPERowId tyWF_UNIT_TYPERowId;type tyWF_CAUSAL_UNIT_TYPERowId is table of rowid index by varchar2(100); ' || chr(10) ||
'tbWF_CAUSAL_UNIT_TYPERowId tyWF_CAUSAL_UNIT_TYPERowId;type tyGR_CONFIGURA_TYPERowId is table of rowid index by varchar2(100); ' || chr(10) ||
...
'tbObjectToDelete tyObjectToDelete; ' || chr(10) ||
' ' || chr(10) ||
'END EXP_UNITTYPE_346_;');
END;
/


BEGIN ut_trace.trace('********************Comenzar proceso de objeto:EXP_UNITTYPE_346_******************************'); END;
/

BEGIN
ut_trace.trace('Actualizar Variable de Proceso: EXP_UNITTYPE_346',1);
EXP_UNITTYPE_346_.blProcessStatus := EXP_PROCESS_345_.blProcessStatus ; 
exception when others then
rollback;
ut_trace.trace('**ERROR:'||sqlerrm,1);
raise;
...
if (not EXP_UNITTYPE_346_.blProcessStatus) then
 return;
end if;
 nuIndex := 1;
 FOR crObjectsName IN cuObjectsName LOOP
  EXP_UNITTYPE_346_.tbObjectToDelete(nuIndex) := crObjectsName.OBJECT_NAME;
  nuIndex := nuIndex + 1;
 END LOOP;
EXCEPTION 
 when ex.controlled_error then
...
CURSOR cuRowIdToDelete is
SELECT rowid
FROM WF_UNIT_TYPE_ATTRIB WHERE (UNIT_TYPE_ID) in (SELECT UNIT_TYPE_ID FROM WF_UNIT_TYPE WHERE UNIT_TYPE_ID=346);
nuIndex binary_integer;
BEGIN
...
exception when others then
EXP_UNITTYPE_346_.blProcessStatus := false;
rollback;
ut_trace.trace('**ERROR:'||sqlerrm);
raise;
END; 
...
CURSOR cuRowIdToDelete is
SELECT rowid
FROM WF_CAUSAL_UNIT_TYPE WHERE (UNIT_TYPE_ID) in (SELECT UNIT_TYPE_ID FROM WF_UNIT_TYPE WHERE UNIT_TYPE_ID=346);
nuIndex binary_integer;
BEGIN
...
CURSOR cuRowIdToDelete is
SELECT rowid
FROM MO_TIME_UNI_TYP_PRIO WHERE (UNIT_TYPE_ID) in (SELECT UNIT_TYPE_ID FROM WF_UNIT_TYPE WHERE UNIT_TYPE_ID=346);
nuIndex binary_integer;
BEGIN
...
CURSOR cuRowIdToDelete is
SELECT rowid
FROM OR_ACT_BY_TASK_MOD WHERE (TASK_CODE) in (SELECT UNIT_TYPE_ID FROM WF_UNIT_TYPE WHERE UNIT_TYPE_ID=346);
nuIndex binary_integer;
BEGIN
...
CURSOR cuRowIdToDelete is
SELECT rowid
FROM WF_UNIT_TYPE WHERE UNIT_TYPE_ID=346;
nuIndex binary_integer;
BEGIN
...
EXP_UNITTYPE_346_.tb0_0(0):=346;
ut_trace.trace('insertando tabla sin fallo: WF_UNIT_TYPE fila (0)',1);
INSERT INTO WF_UNIT_TYPE(UNIT_TYPE_ID,CATEGORY_ID,MODULE_ID,ACTION_ID,PARENT_ID,INIT_AREA_EXPRESSION_ID,ASSIGN_COMMENT_CLASS,ATTEND_COMMENT_CLASS,UNASSIGN_COMMENT_CLASS,TAG_NAME,DESCRIPTION,DISPLAY,ICON,IS_STAGE_PROCESS,USABLE_IN_FLOW_STAGE,ENTITY_ID,MULTI_INSTANCE,IS_COUNTABLE,NOTIFICATION_ID,VIEWABLE,DEFAULT_PRIORITY_ID,INITIAL_NOTIFY_TIME,IS_ADMIN_PROCESS,ASSIGN_NOTIFICATION_ID,ASSIGNED_USER_ONLY) 
VALUES (EXP_UNITTYPE_346_.tb0_0(0),
2,
5,
null,
983,
...
BEGIN
ut_trace.trace('Actualizar Variable de Proceso: EXP_PROCESS_345',1);
EXP_PROCESS_345_.blProcessStatus := EXP_UNITTYPE_346_.blProcessStatus ; 
exception when others then
rollback;
ut_trace.trace('**ERROR:'||sqlerrm,1);
raise;
...
BEGIN
 ut_trace.trace('Inicia borrado de objetos de reglas',1);
 nuIndex := EXP_UNITTYPE_346_.tbObjectToDelete.first;
 while (nuIndex is not null) LOOP
  BEGIN
   ut_trace.trace('Objeto a borrar: ' || EXP_UNITTYPE_346_.tbObjectToDelete(nuIndex),1);
   IF(ut_object.fboExistObject(EXP_UNITTYPE_346_.tbObjectToDelete(nuIndex)) and not fblIsObjectUseByRule(EXP_UNITTYPE_346_.tbObjectToDelete(nuIndex))) THEN
    blObjectDeleted := ut_object.fboDeleteObject(EXP_UNITTYPE_346_.tbObjectToDelete(nuIndex));
    ut_trace.trace('  * Borrado ' || EXP_UNITTYPE_346_.tbObjectToDelete(nuIndex), 1);
   END IF;
  EXCEPTION when others then
   ut_trace.trace('- No se borra el objeto (' || EXP_UNITTYPE_346_.tbObjectToDelete(nuIndex) || '): ' || sqlerrm,1);
  END;
  nuIndex := EXP_UNITTYPE_346_.tbObjectToDelete.next(nuIndex);
 END loop;
 ut_trace.trace('Finaliza borrado de objetos de reglas',1);
EXCEPTION 
 when ex.controlled_error then
...
begin
SA_BOCreatePackages.DropPackage('EXP_UNITTYPE_346_');
end;
/

BEGIN ut_trace.trace('********************FIN  proceso de objeto:EXP_UNITTYPE_346_******************************'); end;
/

BEGIN
ut_trace.trace('Actualizar Variable de Proceso: EXP_PROCESS_342',1);
EXP_PROCESS_342_.blProcessStatus := EXP_PROCESS_345_.blProcessStatus ; 
exception when others then
rollback;
ut_trace.trace('**ERROR:'||sqlerrm,1);
raise;
...
DECLARE 
    nuBinaryIndex Binary_Integer;
BEGIN 
    nuBinaryIndex := EXP_PROCESS_345_.tbGR_CONFIG_EXPRESSIONId.first;
    while (nuBinaryIndex IS not null) loop 
       BEGIN
           delete from GR_CONFIG_EXPRESSION where config_expression_id = EXP_PROCESS_345_.tbGR_CONFIG_EXPRESSIONId(nuBinaryIndex);
           ut_trace.trace('Delete config_expression_id = : '||EXP_PROCESS_345_.tbGR_CONFIG_EXPRESSIONId(nuBinaryIndex) || ' OK', 1);
       EXCEPTION
           when ex.RECORD_HAVE_CHILDREN then
               ut_trace.trace('No se pudo borrar la regla config_expression_id '||EXP_PROCESS_345_.tbGR_CONFIG_EXPRESSIONId(nuBinaryIndex),1);
       END;
       nuBinaryIndex := EXP_PROCESS_345_.tbGR_CONFIG_EXPRESSIONId.next(nuBinaryIndex);
    END loop;
...
BEGIN
 ut_trace.trace('Inicia borrado de objetos de reglas',1);
 nuIndex := EXP_PROCESS_345_.tbObjectToDelete.first;
 while (nuIndex is not null) LOOP
  BEGIN
   ut_trace.trace('Objeto a borrar: ' || EXP_PROCESS_345_.tbObjectToDelete(nuIndex),1);
   IF(ut_object.fboExistObject(EXP_PROCESS_345_.tbObjectToDelete(nuIndex)) and not fblIsObjectUseByRule(EXP_PROCESS_345_.tbObjectToDelete(nuIndex))) THEN
    blObjectDeleted := ut_object.fboDeleteObject(EXP_PROCESS_345_.tbObjectToDelete(nuIndex));
    ut_trace.trace('  * Borrado ' || EXP_PROCESS_345_.tbObjectToDelete(nuIndex), 1);
   END IF;
  EXCEPTION when others then
   ut_trace.trace('- No se borra el objeto (' || EXP_PROCESS_345_.tbObjectToDelete(nuIndex) || '): ' || sqlerrm,1);
  END;
  nuIndex := EXP_PROCESS_345_.tbObjectToDelete.next(nuIndex);
 END loop;
 ut_trace.trace('Finaliza borrado de objetos de reglas',1);
EXCEPTION 
 when ex.controlled_error then
...
begin
SA_BOCreatePackages.DropPackage('EXP_PROCESS_345_');
end;
/

BEGIN ut_trace.trace('********************FIN  proceso de objeto:EXP_PROCESS_345_******************************'); end;
/
...
EXP_PROCESS_393_.blProcessStatus := EXP_PROCESS_342_.blProcessStatus ; 
exception when others then
rollback;
ut_trace.trace('**ERROR:'||sqlerrm,1);
raise;
...
BEGIN
ut_trace.trace('Actualizar Variable de Proceso: EXP_PROCESS_342',1);
EXP_PROCESS_342_.blProcessStatus := EXP_PROCESS_393_.blProcessStatus ; 
exception when others then
rollback;
ut_trace.trace('**ERROR:'||sqlerrm,1);
raise;
...
EXP_PROCESS_394_.blProcessStatus := EXP_PROCESS_342_.blProcessStatus ; 
exception when others then
rollback;
ut_trace.trace('**ERROR:'||sqlerrm,1);
raise;
...
BEGIN
ut_trace.trace('Actualizar Variable de Proceso: EXP_PROCESS_342',1);
EXP_PROCESS_342_.blProcessStatus := EXP_PROCESS_394_.blProcessStatus ; 
exception when others then
rollback;
ut_trace.trace('**ERROR:'||sqlerrm,1);
raise;
...
ut_trace.trace('Realizar Commit de EXP_PROCESS_342 ',1);
COMMIT;
exception when others then
rollback;
ut_trace.trace('**ERROR:'||sqlerrm,1);
...
DECLARE 
    nuBinaryIndex Binary_Integer;
BEGIN 
    nuBinaryIndex := EXP_PROCESS_342_.tbGR_CONFIG_EXPRESSIONId.first;
    while (nuBinaryIndex IS not null) loop 
       BEGIN
           delete from GR_CONFIG_EXPRESSION where config_expression_id = EXP_PROCESS_342_.tbGR_CONFIG_EXPRESSIONId(nuBinaryIndex);
           ut_trace.trace('Delete config_expression_id = : '||EXP_PROCESS_342_.tbGR_CONFIG_EXPRESSIONId(nuBinaryIndex) || ' OK', 1);
       EXCEPTION
           when ex.RECORD_HAVE_CHILDREN then
               ut_trace.trace('No se pudo borrar la regla config_expression_id '||EXP_PROCESS_342_.tbGR_CONFIG_EXPRESSIONId(nuBinaryIndex),1);
       END;
       nuBinaryIndex := EXP_PROCESS_342_.tbGR_CONFIG_EXPRESSIONId.next(nuBinaryIndex);
    END loop;
...
nuRowProcess:=EXP_PROCESS_342_.tb13_0.first;
while (nuRowProcess is not null) loop 
ut_trace.trace('Genera expresión regla:'|| EXP_PROCESS_342_.tb13_0(nuRowProcess),1); 
begin
GR_BOINTERFACE_BODY.MakeExpression(EXP_PROCESS_342_.tb13_0(nuRowProcess)); 
exception when others then
ut_trace.trace('Fallo la generacion de la regla:'|| EXP_PROCESS_342_.tb13_0(nuRowProcess),1);
end;
nuRowProcess := EXP_PROCESS_342_.tb13_0.next(nuRowProcess);
END loop;
COMMIT;
...
BEGIN
 ut_trace.trace('Inicia borrado de objetos de reglas',1);
 nuIndex := EXP_PROCESS_342_.tbObjectToDelete.first;
 while (nuIndex is not null) LOOP
  BEGIN
   ut_trace.trace('Objeto a borrar: ' || EXP_PROCESS_342_.tbObjectToDelete(nuIndex),1);
   IF(ut_object.fboExistObject(EXP_PROCESS_342_.tbObjectToDelete(nuIndex)) and not fblIsObjectUseByRule(EXP_PROCESS_342_.tbObjectToDelete(nuIndex))) THEN
    blObjectDeleted := ut_object.fboDeleteObject(EXP_PROCESS_342_.tbObjectToDelete(nuIndex));
    ut_trace.trace('  * Borrado ' || EXP_PROCESS_342_.tbObjectToDelete(nuIndex), 1);
   END IF;
  EXCEPTION when others then
   ut_trace.trace('- No se borra el objeto (' || EXP_PROCESS_342_.tbObjectToDelete(nuIndex) || '): ' || sqlerrm,1);
  END;
  nuIndex := EXP_PROCESS_342_.tbObjectToDelete.next(nuIndex);
 END loop;
 ut_trace.trace('Finaliza borrado de objetos de reglas',1);
EXCEPTION 
 when ex.controlled_error then
...
begin
SA_BOCreatePackages.DropPackage('EXP_PROCESS_342_');
end;
/

BEGIN ut_trace.trace('********************FIN  proceso de objeto:EXP_PROCESS_342_******************************'); end;
/
