set serveroutput on;
PROMPT BORRAR LDC_CREATRAMITEREPARACIONRP  
DECLARE
  nuConta       NUMBER;
  sbDueno	    VARCHAR2(30);
...
BEGIN
    BEGIN
    dbms_output.put_Line('Preparando borrado de synonym ADM_PERSON.LDC_CREATRAMITEREPARACIONRP');
...
--
    BEGIN
    dbms_output.put_Line('Preparando borrado de procedimiento OPEN.LDC_CREATRAMITEREPARACIONRP');
        SELECT COUNT(*), OWNER, OBJECT_NAME, OBJECT_TYPE INTO nuConta, sbDueno, sbobject_name, sbobject_type
          FROM dba_objects
         WHERE object_name = 'LDC_CREATRAMITEREPARACIONRP'  
           AND OWNER = 'OPEN'
