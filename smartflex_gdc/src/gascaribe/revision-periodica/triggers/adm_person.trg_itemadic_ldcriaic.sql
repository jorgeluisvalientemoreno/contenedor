CREATE OR REPLACE trigger ADM_PERSON.TRG_ITEMADIC_LDCRIAIC
  before insert or update or delete on LDC_ITEMADIC_LDCRIAIC
  for each row
...
declare
  ...
  cursor cuordencerradoanulado is
    select count(oo.order_id) CANTIDAD,
      ...
      from or_order oo
     where oo.order_id in (select LIL.ORDER_ID
                             FROM LDC_ITEMCOTI_LDCRIAIC LIL
