...
PROMPT Borrar Trigger LDC_PER_COMERCIAL_TRG01
PROMPT 
DECLARE
  nuConta       NUMBER;
  sbDueno	      VARCHAR2(30);
...
BEGIN
    BEGIN
        SELECT COUNT(*), OWNER, OBJECT_NAME, OBJECT_TYPE INTO nuConta, sbDueno, sbobject_name, sbobject_type
          FROM dba_objects
         WHERE object_name = 'LDC_PER_COMERCIAL_TRG01'  
           AND OWNER = 'OPEN'
  ...
  IF nuConta > 0 then
    ...
    dbms_output.put_Line('Trigger LDC_PER_COMERCIAL_TRG01 borrado con exito');
  END IF;
EXCEPTION
    WHEN OTHERS THEN 
