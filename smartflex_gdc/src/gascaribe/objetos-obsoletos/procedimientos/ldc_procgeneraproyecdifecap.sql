CREATE OR REPLACE Procedure ldc_procgeneraproyecdifecap(
                                                       inano      In Number,
                                                       inmes      In Number,
  ...
  /****************************************************************************************************************************
   ...
   20/OCT/2020    JJJM           CASO 533 Se agregan los parametros de entrada : itipodif y iamormesmes.
                                          Se agregan los campos tipo_calculo,amort_mes_mes al insert de la tabla :
                                          open.ldc_osf_proyrecar cuyos valores se recuperan de los parametros de entrada :
                                          itipodif y iamormesmes.
                                          Se hace el calculo de amortizaci?n del diferido mes a mes en caso de que el parametro
                                          amort_mes_mes tenga valor = 'S'.
                                          Se valida el tipo de diferido para el calculo del capital para saber si toma el
                                          totaldiferido, diferido corriente o diferido no-corriente
  *************************************************************************************************************************************/
...
Begin
  ...
  Delete open.ldc_osf_proyrecar rc
   Where rc.anogene = inano
     ...
     and rc.tipo_calculo = itipodif
     and rc.amort_mes_mes = iamormesmes;
  Commit;
  ...
  For indife In curdiferido Loop
    ...
    -- Inicio CA533
    if itipodif = 'DTO' THEN
     saldo := indife.difesape;
    ELSIF itipodif = 'DCO' THEN
     saldo := indife.difecorr;
    elsif itipodif = 'DNC' THEN
     saldo := indife.difencorr;
    else
     saldo := 0;
    end if;
    -- Fin CA533
  ...
  Insert Into open.ldc_osf_proyrecar
    (anogene, mesgene, servgene, periodo, valor,tipo_calculo,amort_mes_mes)
  Values
    (inano, inmes, inservicio, sbperi, tvcap0,itipodif,iamormesmes);
  ...
  Insert Into open.ldc_osf_proyrecar
    (anogene, mesgene, servgene, periodo, valor,tipo_calculo,amort_mes_mes)
  Values
    (inano, inmes, inservicio, sbperi, tvcap1,itipodif,iamormesmes);
  ...
  Insert Into open.ldc_osf_proyrecar
    (anogene, mesgene, servgene, periodo, valor,tipo_calculo,amort_mes_mes)
  Values
    (inano, inmes, inservicio, sbperi, tvcap2,itipodif,iamormesmes);
  ...
  Insert Into open.ldc_osf_proyrecar
    (anogene, mesgene, servgene, periodo, valor,tipo_calculo,amort_mes_mes)
  Values
    (inano, inmes, inservicio, sbperi, tvcap3,itipodif,iamormesmes);
  ...
  Insert Into open.ldc_osf_proyrecar
    (anogene, mesgene, servgene, periodo, valor,tipo_calculo,amort_mes_mes)
  Values
    (inano, inmes, inservicio, sbperi, tvcap4,itipodif,iamormesmes);
  ...
  Insert Into open.ldc_osf_proyrecar
    (anogene, mesgene, servgene, periodo, valor,tipo_calculo,amort_mes_mes)
  Values
    (inano, inmes, inservicio, sbperi, tvcap5,itipodif,iamormesmes);

  sbperi := 'De ' || mes || ' de ' || (varano + 6) || ' en adelante : ';
  Insert Into open.ldc_osf_proyrecar
    (anogene, mesgene, servgene, periodo, valor,tipo_calculo,amort_mes_mes)
  Values
    (inano, inmes, inservicio, sbperi, tvcap6,itipodif,iamormesmes);

  Commit;

  merro := 'Proceso termino Ok (Servicio ' || inservicio || ')';
  ldc_proactualizaestaprog(nusession,merro,'LDRGPRCB-CAPITAL','Termin? Ok.');

Exception
  When Others Then
    merro := 'Error en LDRGPRCB (Servicio ' || inservicio || ')' ||
             Sqlerrm;
    ldc_proactualizaestaprog(nusession,merro,'LDRGPRCB-CAPITAL','Termin? con errores.');

End ldc_procgeneraproyecdifecap;
/
PROMPT OTORGA PERMISOS a REXEREPORTES sobre procedimiento LDC_PROCGENERAPROYECDIFECAP
GRANT EXECUTE ON LDC_PROCGENERAPROYECDIFECAP TO REXEREPORTES;
/