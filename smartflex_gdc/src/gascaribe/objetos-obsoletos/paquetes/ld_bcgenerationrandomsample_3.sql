CREATE OR REPLACE PACKAGE ld_bcgenerationrandomsample_3 IS
  ...
  ****************************************************************************************/
 TYPE rcsesu IS RECORD(
    sesunuse NUMBER);

  TYPE tbsesu IS TABLE OF rcsesu INDEX BY pls_integer;
  tsesu tbsesu;
  
cursor cudatos is
select account_number 
  from (select d1.account_number
          from open.ld_sample_Detai d1
         where d1.sample_id = 60
         minus
         select d1.account_number
           from open.ld_sample_Detai d1
          where d1.sample_id = 68);

nucontinsert number := 0;
       
function fnubuscsesu (inunuse number) return number;
            ...
            SELECT subscriber_id,
              ...
              FROM (SELECT subscriber_id,
                      ...
                      FROM (SELECT 1 subscriber_id,
                              ...
                              FROM servsusc s, ldc_osf_sesucier_T o
                             WHERE s.sesunuse = o.producto
                               --and sesunuse in (51530723, 51532432, 51532442)
                               AND ld_bcgenerationrandomsample_3.fnubuscsesu(SESUNUSE) = 1
                               AND o.estado_corte NOT IN
                                   (SELECT numercial_value
                                      FROM ld_general_parameters
                                     WHERE parameter_desc LIKE
...
END ld_bcgenerationrandomsample_3;
/
CREATE OR REPLACE PACKAGE BODY ld_bcgenerationrandomsample_3 IS
  ...
  -- Constantes
  csbversion   CONSTANT VARCHAR2(250) := '227685';
  ...
  nucount                     ld_sample_detai.detail_sample_id%TYPE := 126550; --------------------------------- consecut de ld_Sample_detai
  ...
  nuerrorcount                ld_error_account.error_id%TYPE := 134920; --Number := 0;
...
/*
  ...
  BEGIN
    ...
    xinsertfgrcr('Voy a crear tabla');
   
    tsesu.delete;
    for rg in cudatos loop
      nuidx := rg.account_number;
      tsesu(nuidx).sesunuse := rg.account_number;
      nucont := nucont + 1;
    end loop;
    xinsertfgrcr('Termine de crear tabla ... No Filas: '||TO_CHAR(nucont));
    nuRegTabla := tsesu.count;
    xinsertfgrcr('No Reg Tabla: '||TO_CHAR(nuRegTabla));
    
    if tsesu.exists(51721956) then
      xinsertfgrcr('Reg 51721956: '||TO_CHAR(tsesu(51721956).sesunuse));
    else
       xinsertfgrcr('NO EXISTE Reg 51721956');
    end if;
    
    if tsesu.exists(51694520) then
      xinsertfgrcr('Reg 51694520: '||TO_CHAR(tsesu(51694520).sesunuse));
    else
       xinsertfgrcr('NO EXISTE Reg 51694520');
    end if; 
   
     
  /*  dbms_output.put_line('termina tabla ' ||' - ' || sysdate || ' - ' || nucont);*/
    ...
    insbtypegen         := p_insbtypegen;
    ionutotalrec        := p_ionutotalrec;
    nuerrorcount        := 134920/*p_ionutotalerr*/;
    ionutotalnov        := p_ionutotalnov;
    idtfechgen          := p_idtfechgen;
    onuerrorcode        := p_onuerrorcode;
    osberrormessage     := p_osberrormessage;
   ...
   /* prosample;*/
    ...
    IF isbrepomuestra = 'R' THEN
      ...
      xinsertfgrcr('voy a recorrer cu_servicios ahora');
      xinsertfgrcr('inucreditbereauid ' || inucreditbereauid);
      xinsertfgrcr('inutypesector ' || inutypesector);
      xinsertfgrcr('inutypeproductid ' || inutypeproductid);
      xinsertfgrcr('nucategory ' || nucategory);
      xinsertfgrcr('nuoverduebills ' || nuoverduebills);
      xinsertfgrcr('isbrepomuestra ' || isbrepomuestra);
      xinsertfgrcr('sbsubscriber_number ' || sbsubscriber_number);
      xinsertfgrcr('nuanogen ' || nuanogen);
      xinsertfgrcr('numesgen ' || numesgen);
      ...
      END LOOP;
      xinsertfgrcr('Sali de cu_Servicio');
      /* For regselcrit In cuselcrit(inutypesector, inucreditbereauid) Loop
...
--               WHERE l.parameter_desc = 'EDAD_MORA_NOTI')
  ...
  END PRO_MARCARUSUARIOS;
   ...
   function fnubuscsesu (inunuse number) return number is
  onuexiste number := 0;
begin
   nucontinsert := nucontinsert + 1;
   if mod(nucontinsert,1000) = 0 then
      xinsertfgrcr('en fnubuscsesu ' || nucontinsert);
   end if;
   
   if tsesu.EXISTS(inunuse) THEN
     --xinsertfgrcr('en fnubuscsesu ... Encontro ' || inunuse);
     onuexiste := 1;
   end if;
   return onuexiste;
end fnubuscsesu;
...
END ld_bcgenerationrandomsample_3;
/
GRANT EXECUTE on LD_BCGENERATIONRANDOMSAMPLE_3 to SYSTEM_OBJ_PRIVS_ROLE;
/
