...
BEGIN
sa_bocreatePackages.CreatePackage('CONRT_',
'CREATE OR REPLACE PACKAGE CONRT_ IS ' || chr(10) ||
...
'tb2_0 ty2_0;  executableName ge_catalog.tag_name%type := ''CONRT''; ' || chr(10) ||
...
'( ' || chr(10) ||
'SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME=''CONRT'' ' || chr(10) ||
'); ' || chr(10) ||
...
BEGIN ut_trace.trace('********************Comenzar proceso de objeto:CONRT_******************************'); END;
/
...
if (not CONRT_.blProcessStatus) then
 return;
end if;
Open CONRT_.cuRoleExecutables;
loop
 fetch CONRT_.cuRoleExecutables INTO CONRT_.rcRoleExecutables;
 exit when  CONRT_.cuRoleExecutables%notfound;
 CONRT_.tbRoleExecutables(nuIndex) := CONRT_.rcRoleExecutables;
 nuIndex := nuIndex + 1;
END loop;
close CONRT_.cuRoleExecutables;
nuIndex := 0;
Open CONRT_.cuUserExceptions ;
loop
 fetch CONRT_.cuUserExceptions INTO  CONRT_.rcUserExceptions;
 exit when CONRT_.cuUserExceptions%notfound;
 CONRT_.tbUserException(nuIndex):=CONRT_.rcUserExceptions;
 nuIndex := nuIndex + 1;
END loop;
close CONRT_.cuUserExceptions;
nuIndex := 0;
Open CONRT_.cuExecEntities ;
loop
 fetch CONRT_.cuExecEntities INTO  CONRT_.rcExecEntities;
 exit when CONRT_.cuExecEntities%notfound;
 CONRT_.tbExecEntities(nuIndex):=CONRT_.rcExecEntities;
 nuIndex := nuIndex + 1;
END loop;
close CONRT_.cuExecEntities;

exception when others then
CONRT_.blProcessStatus := false;
rollback;
...
(
 SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='CONRT'
);
    ...
    BEGIN
      ...
      IF (nuNumRecords > 0) THEN
        ...
        IF ( sbObjectName IS NOT null ) THEN
          ...
          if ( CONRT_.tbObjectToDelete.last IS null ) then
            nuNumObjects := 1;
          else
            nuNumObjects := CONRT_.tbObjectToDelete.last + 1;
          end if;
          CONRT_.tbObjectToDelete(nuNumObjects) := sbObjectName;
        END IF;
...
if (not CONRT_.blProcessStatus) then
 return;
  ...
  WHERE GE_DISTRIBUTION_FILE.DISTRIBUTION_FILE_ID = 'CONRT';
  IF (nuNumRecords > 0) THEN
...
WHERE   sa_executable.name = 'CONRT'
...
if (not CONRT_.blProcessStatus) then
 ...
 FOR crObjectsName IN cuObjectsName LOOP
  CONRT_.tbObjectToDelete(nuIndex) := crObjectsName.OBJECT_NAME;
  nuIndex := nuIndex + 1;
 END LOOP;
EXCEPTION 
...
if (not CONRT_.blProcessStatus) then
 return;
  ...
  DELETE FROM SA_ROLE_EXECUTABLES WHERE (EXECUTABLE_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='CONRT') AND ROLE_ID=1;
...
if (not CONRT_.blProcessStatus) then
 return;
  ...
  DELETE FROM SA_USER_EXCEPTIONS WHERE (EXECUTABLE_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='CONRT') AND 1=2;
...
SELECT rowid
FROM SA_EXECUTABLE_SYNON WHERE (SYNONYMOUS_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='CONRT');
nuIndex binary_integer;
BEGIN
...
if (not CONRT_.blProcessStatus) then
 return;
  ...
  DELETE FROM SA_EXEC_ENTITIES WHERE (EXECUTABLE_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='CONRT');
...
if (not CONRT_.blProcessStatus) then
 return;
  ...
  DELETE FROM SA_MENU_OPTION WHERE (EXECUTABLE_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='CONRT');
...
if (not CONRT_.blProcessStatus) then
 return;
  ...
  DELETE FROM GI_CONFIG_COMP WHERE (CONFIG_ID) in (SELECT CONFIG_ID FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='CONRT') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339);
...
if (not CONRT_.blProcessStatus) then
 return;
  ...
  ut_trace.trace('Borrando tabla GI_CONFIG_COMP',1);
  DELETE FROM GI_CONFIG_COMP WHERE (COMPOSITION_ID) in (SELECT COMPOSITION_ID FROM GI_COMPOSITION WHERE (CONFIG_ID) in (SELECT CONFIG_ID FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='CONRT') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339));
...
DECLARE 
 ...
 SELECT rowid FROM GE_STATEMENT WHERE (STATEMENT_ID) in (SELECT SELECT_STATEMENT_ID FROM GI_COMP_ATTRIBS WHERE (COMPOSITION_ID) in (SELECT COMPOSITION_ID FROM GI_COMPOSITION WHERE (CONFIG_ID) in (SELECT CONFIG_ID FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='CONRT') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339)));
BEGIN 
...
CONRT_.tbGE_STATEMENTRowId(rcData.rowid):=rcData.rowid; 
END loop; 
...
for rcData in cuLoadTemporaryTable loop
CONRT_.tbGR_CONFIG_EXPRESSIONRowId(rcData.rowid):=rcData.rowid; 
END loop; 
...
if (not CONRT_.blProcessStatus) then
 return;
  ...
  DELETE FROM GI_COMP_ATTRIBS WHERE (COMPOSITION_ID) in (SELECT COMPOSITION_ID FROM GI_COMPOSITION WHERE (CONFIG_ID) in (SELECT CONFIG_ID FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='CONRT') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339));
...
nuVarcharIndex:=CONRT_.tbGE_STATEMENTRowId.first;  
while (nuVarcharIndex is not null) loop 
begin
ut_trace.trace('Row id a borrar:'||nuVarcharIndex,1);
delete from GE_STATEMENT where rowid = CONRT_.tbGE_STATEMENTRowId(nuVarcharIndex); 
...
CONRT_.tbGE_STATEMENTRowId.delete(nuOldVarcharIndex);
end if;
nuVarcharIndex := CONRT_.tbGE_STATEMENTRowId.next(nuVarcharIndex); 
END loop; 
...
nuVarcharIndex:=CONRT_.tbGR_CONFIG_EXPRESSIONRowId.first;  
while (nuVarcharIndex is not null) loop 
ut_trace.trace('Row id a borrar:'||nuVarcharIndex,1);
delete from GR_CONFIG_EXPRESSION where rowid = CONRT_.tbGR_CONFIG_EXPRESSIONRowId(nuVarcharIndex); 
nuOldVarcharIndex:=nuVarcharIndex;
nuVarcharIndex := CONRT_.tbGR_CONFIG_EXPRESSIONRowId.next(nuVarcharIndex); 
CONRT_.tbGR_CONFIG_EXPRESSIONRowId.delete(nuOldVarcharIndex); 
END loop; 
...
for rcData in cuLoadTemporaryTable loop
CONRT_.tbGI_COMPOSITIONRowId(rcData.rowid):=rcData.rowid; 
END loop; 
...
if (not CONRT_.blProcessStatus) then
 return;
  ...
  DELETE FROM GI_FRAME WHERE (COMPOSITION_ID) in (SELECT COMPOSITION_ID FROM GI_COMPOSITION WHERE (CONFIG_ID) in (SELECT CONFIG_ID FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='CONRT') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339));
...
nuVarcharIndex:=CONRT_.tbGI_COMPOSITIONRowId.first;  
while (nuVarcharIndex is not null) loop 
ut_trace.trace('Row id a borrar:'||nuVarcharIndex,1);
delete from GI_COMPOSITION where rowid = CONRT_.tbGI_COMPOSITIONRowId(nuVarcharIndex); 
nuOldVarcharIndex:=nuVarcharIndex;
nuVarcharIndex := CONRT_.tbGI_COMPOSITIONRowId.next(nuVarcharIndex); 
CONRT_.tbGI_COMPOSITIONRowId.delete(nuOldVarcharIndex); 
END loop; 
...
if (not CONRT_.blProcessStatus) then
 return;
  ...
  DELETE FROM GI_COMPOSITION WHERE (CONFIG_ID) in (SELECT CONFIG_ID FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='CONRT') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339);
...
if (not CONRT_.blProcessStatus) then
 return;
  ...
  DELETE FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='CONRT') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339;
...
if (not CONRT_.blProcessStatus) then
 return;
  ...
  DELETE FROM GE_DISTRIBUTION_FILE WHERE (DISTRIBUTION_FILE_ID) in (SELECT NAME FROM SA_EXECUTABLE WHERE NAME='CONRT');
...
if (not CONRT_.blProcessStatus) then
 return;
  ...
  DELETE FROM SA_EXECUTABLE WHERE NAME='CONRT';
...
CONRT_.old_tb0_0(0):='CONRT'
;
CONRT_.tb0_0(0):=UPPER(CONRT_.old_tb0_0(0));
CONRT_.tb0_0(0):=CONRT_.tb0_0(0);
CONRT_.old_tb0_1(0):=500000000000960;
CONRT_.tb0_1(0):=GE_BOCATALOG.FNUGETIDSEQFROMCATALOG(CONRT_.tb0_0(0), 'EXECUTABLE', 'SA_BOEXECUTABLE.GETNEXTID');
CONRT_.tb0_1(0):=CONRT_.tb0_1(0);
ut_trace.trace('insertando tabla: SA_EXECUTABLE fila (0)',1);
INSERT INTO SA_EXECUTABLE(NAME,EXECUTABLE_ID,DESCRIPTION,PATH,VERSION,EXECUTABLE_TYPE_ID,EXEC_OPER_TYPE_ID,MODULE_ID,SUBSYSTEM_ID,PARENT_EXECUTABLE_ID,LAST_RECORD_ALLOWED,PATH_FILE_HELP,WITHOUT_RESTR_POLICY,DIRECT_EXECUTION,TIMES_EXECUTED,EXEC_OWNER,LAST_DATE_EXECUTED,CLASS_ID) 
VALUES (CONRT_.tb0_0(0),
CONRT_.tb0_1(0),
...
1446,
null,
to_date('23-08-2016 11:58:25','DD-MM-YYYY HH24:MI:SS'),
null);
...
CONRT_.tb1_0(0):=1;
CONRT_.tb1_1(0):=CONRT_.tb0_1(0);
ut_trace.trace('insertando tabla: SA_ROLE_EXECUTABLES fila (0)',1);
INSERT INTO SA_ROLE_EXECUTABLES(ROLE_ID,EXECUTABLE_ID) 
VALUES (CONRT_.tb1_0(0),
CONRT_.tb1_1(0));
...
<PB xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema">
  <APPLICATION>
    <EXECUTABLE_ID>500000000000960</EXECUTABLE_ID>
    <NAME>CONRT</NAME>
  ...
  <COMPOSITION>
    ...
    <EXTERNAL_TYPE_ID>500000000000960</EXTERNAL_TYPE_ID>
    <ENTITY_ID>9623</ENTITY_ID>
    <EXTERNAL_ID>500000000000960</EXTERNAL_ID>
...
'C_IMCORETE</ENTITY_NAME>
      ...
      <INIT_BASIC_EXP_ID/>
      <VALID_BASIC_EXP_ID/>
      <INIT_EXPRESSION_ID>121054813</INIT_EXPRESSION_ID>
      <VALID_EXPRESSION_ID>121054814</VALID_EXPRESSION_ID>
      <DEFAULT_VALUE/>
      <SELECT_STATEMENT/>
      <SELECT_STATEMENT_ID/>
      <TAG_NAME/>
...
'E_CASE>U</STYLE_CASE>
    ...
    <ATTRIBUTE>
      ...
      <DISPLAY_NAME>Atencion ' ||
'Actividad</DISPLAY_NAME>
      <INIT_BASIC_EXP_ID/>
      <VALID_BASIC_EXP_ID/>
      <INIT_EXPRESSION_ID/>
      <VALID_EXPRESSION_ID/>
...
'||chr(64)||'active = '||chr(38)||'apos;Y'||chr(38)||'apos; '||chr(64)||'</SELECT_STATEMENT>
      <SELECT_STATEMENT_ID>120027769</SELECT_STATEMENT_ID>
      <TAG_NAME' ||
'/>
      <PROCESS_SEQUENCE/>
      <STATEMENT_CACHE>N</STATEMENT_CACHE>
...
'||chr(64)||'address_id = :address_id'||chr(64)||'</SELECT_STATEMENT>
      <SELECT_STATEMENT_ID>120027770</SELECT_STATEMENT_ID>
      <TAG_NAME/>
      <PROCESS_SEQUENCE/>
      <STATEMENT_CACHE>N</STATEMENT_CACHE>
      <USER_DEFAULT_ALLOWED>N</USER_DEFAULT_ALLOWED>
    ...
    <ATTRIBUTE>
      <ENTITY_ATTRIBU' ||
'TE_ID>90018313</ENTITY_ATTRIBUTE_ID>
      ...
      <VALID_EXPRESSION_ID/>
      <DEFAULT_VALUE/>
      <SELECT_STATEMENT>SELECT in_persist ID, DECODE(in_persist,'||chr(38)||'apos;N'||chr(38)||'apos;,'||chr(38)||'apo' ||
's;No'||chr(38)||'apos;,'||chr(38)||'apos;Si'||chr(38)||'apos;) DESCRIPTION
FROM ge_entity
WHERE entity_id IN (68,3203 )</SELECT_STATEMENT>
      <SELECT_STATEMENT_ID>120027771</SELECT_STATEMENT_ID>
      <TAG_NAME/>
      <PROCESS_SEQUENCE/>
      <STATEMENT_CACHE>N</STATEMENT_CACHE>
      <USER_DEFAULT_ALLOWED>N</USER_DEFAULT_ALLOWED>
    ...
    <ATTRIBUTE>
      ...
      <TYPE_ATTRIB_ID>2</TYPE_ATTRIB_ID>
      <ENTITY_TYPE_ID>3339</ENTITY_TYPE_ID>
      <EXTERNAL_TYPE_ID>500000000000960</EXTERNAL_TY' ||
'PE_ID>
      <FRAME_ID>94764</FRAME_ID>
      <DATA_LENGTH>1</DATA_LENGTH>
      <DATA_PRECISION/>
...
'||chr(64)||'operating_sector_id = :operating_sector_id '||chr(64)||'
      ...
      <SELECT_STATEMENT_ID>120027772</SELECT_STATEMENT_ID>
      <TAG_NAME/>
      <PROCESS_SEQUENCE/>
      <STATEMENT_CACHE>N</STATEMENT_CACHE>' ||
'
...
'||chr(64)||'task_type_id = :task_type_id'||chr(64)||'</SELECT_STATEMENT>
      <SELECT_STATEMENT_ID>120027773</SELECT_STATEMENT_ID>
      <TAG_NAME/>
      <PROCESS_SEQUENCE/>
      <STATEMENT_CACHE>N</STATEMENT_CACHE>
      <USER_DEFAULT_ALLOWED>N</USER_DEFAULT_ALLOWED>
...
'||chr(64)||'WHERE'||chr(64)||'
'||chr(64)||'tas' ||
'k_type_id = :task_type_id'||chr(64)||'</SELECT_STATEMENT>
      <SELECT_STATEMENT_ID>120027774</SELECT_STATEMENT_ID>
      <TAG_NAME/>
      <PROCESS_SEQUENCE/>
      <STATEMENT_CACHE>N</STATEMENT_CACHE>
      <USER_DEFAULT_ALLOWED>N</USER_DEFAULT_ALLOWED>
...
'||chr(64)||'operating_unit_id = :operating_unit_id'||chr(64)||'</SELECT_STATEMENT>
      <SELECT_STATEMENT_ID>120027775</SELECT_STATEMENT_ID>
      <TAG_NAME/>
      <PROCESS_SEQUENCE/>
      <STATEMENT_CACHE>N</STATEMENT_CACHE>
      <USER_DEFAULT_ALLOWED>N</USER_DEFAULT_ALLOWED>
      ...
      <EXTERNAL_ROOT_ID>6121</EXTERNAL_ROOT_ID>
      <PARENT_ATTRIBUTE_NAME>ICRTOR' ||
'DE</PARENT_ATTRIBUTE_NAME>
      <PARENT_ENTITY_NAME>LDC_IMCORETE</PARENT_ENTITY_NAME>
    </ATTRIBUTE>
    <ATTRIBUTE>
...
'998</PARENT_CONF_ID>
      ...
      <EXTERNAL_ROOT_ID>6121</EXTERNAL_ROOT_ID>
      <PARENT_ATTRIBU' ||
'TE_NAME>ICRTORDE</PARENT_ATTRIBUTE_NAME>
      <PARENT_ENTITY_NAME>LDC_IMCORETE</PARENT_ENTITY_NAME>
    </ATTRIBUTE>
    <ATTRIBUTE>
...
FROM LDC_IMCOTIEV
      ...
      <SELECT_STATEMENT_ID>120027776</SELECT_STATEMENT_ID>
      <T' ||
'AG_NAME/>
      <PROCESS_SEQUENCE/>
      <STATEMENT_CACHE>N</STATEMENT_CACHE>
...
FROM ge_entity
WHERE entity_id IN (68,3203 )</SELECT_STATEMENT>
      <SELECT_STATEMENT_ID>120027777</SELECT_STATEMENT_ID>
      <TAG_NAME/>
      <PROCESS_SEQUENCE/>
      <STATEMENT_CACHE>N</STATEMENT_CACHE>
      <USER_DEFAULT_ALLOWED>N</USER_DEFAULT_ALLOWED>
...
'
      ...
      <VALID_EXPRESSION_ID/>
      <DEFAULT_VALUE/>
      <SELECT_STATEMENT>SELECT in_persist ID, ' ||
'DECODE(in_persist,'||chr(38)||'apos;N'||chr(38)||'apos;,'||chr(38)||'apos;No'||chr(38)||'apos;,'||chr(38)||'apos;Si'||chr(38)||'apos;) DESCRIPTION
FROM ge_entity
WHERE entity_id IN (68,3203 )</SELECT_STATEMENT>
      <SELECT_STATEMENT_ID>120027778</SELECT_STATEMENT_ID>
      <TAG_NAME/>
      <PROCESS_SEQUENCE/>
      <STATEMENT_CACHE>N</STATEMENT_CACHE>
      <USER_DEFAULT_ALLOWED>N</USER_DEFAULT_ALLOWED>
    ...
    <ATTRIBUTE>
      ...
      <ENTITY_TYPE_ID>3339</ENTITY_TYPE_ID>
      <EXTERNAL' ||
'_TYPE_ID>500000000000960</EXTERNAL_TYPE_ID>
      <FRAME_ID>94764</FRAME_ID>
      <DATA_LENGTH>15</DATA_LENGTH>
      <DATA_PRECISION>15</DATA_PRECISION>
      <DATA_SCALE>0</DATA_SCALE>
      ...
      <EXTERNAL_ROOT_ID>6121</EXTERNAL_ROOT_ID>
      <PARENT_ATTRIBUTE_NAME>ICRTORDE</PARENT_ATTRIBUTE_NAME>
      <PARENT_ENTITY_NAME>LDC_IMCORETE</PARENT_EN' ||
'TITY_NAME>
    </ATTRIBUTE>
    <ATTRIBUTE>
      <ENTITY_ATTRIBUTE_ID>-1</ENTITY_ATTRIBUTE_ID>
...
'SIBLE>
    ...
    <ATTRIBUTE>
      ...
      <ENTITY_ID>8363</ENTITY_ID>
      <ENTITY_NAME>LDC_IMCORETE</ENTITY_NAME>
      <PARENT_CONF_ID>-999999998</PAREN' ||
'T_CONF_ID>
      <TYPE_ATTRIB_ID>2</TYPE_ATTRIB_ID>
      <ENTITY_TYPE_ID>3339</ENTITY_TYPE_ID>
      <EXTERNAL_TYPE_ID>500000000000960</EXTERNAL_TYPE_ID>
...
FROM LDC_IMCOREPO</SELECT_STATEMENT>
      <SELECT_STATEMENT_ID>120027779</SELECT_STATEMENT_ID>
      <TAG_NAME/>
      <PROCESS_SEQUENCE/>
      <STATEMENT_CACHE>N</STATEMENT_CACHE>
      <USER_DEFAULT_ALLOWED>N</USER_DEFAULT_ALLOWED>
...
FROM LDC_IMCOLORE
      ...
      <SELECT_STATEMENT_ID>120027780</SELECT_STATEMENT_ID>
      <TAG_NAME/>
      <PROCESS_SEQUENCE/>
      <STATEMENT_CACHE>N</STATEMENT_CACHE>
      <USER_DEFAULT_ALLOWED>N</USER_DEFAULT_ALLOWED>
...
'_CASE>
    ...
    <ATTRIBUTE>
      ...
      <DISPLAY_NAME>IDENTIFIC' ||
'ACION ELEMENTO QUE FALLO</DISPLAY_NAME>
      <INIT_BASIC_EXP_ID/>
      <VALID_BASIC_EXP_ID/>
      <INIT_EXPRESSION_ID/>
      <VALID_EXPRESSION_ID/>
    ...
    <ATTRIBUTE>
      ...
      <TYPE_ATTRIB_ID>2</TYPE_ATTRIB_ID>
      <ENTITY_TYPE_ID>3339</ENTITY_TYPE_ID>
      <EXTERNAL_TYPE_ID>500000000000960</EXTER' ||
'NAL_TYPE_ID>
      <FRAME_ID>94764</FRAME_ID>
      <DATA_LENGTH>4</DATA_LENGTH>
      <DATA_PRECISION/>
      ...
      <SELECT_STATEMENT_ID>120027781</SELECT_STATEMENT_ID>
      <TAG_NAME/>
      <PROCESS_SEQUENCE/>
      <STATEMENT_CACHE>N</STATEMENT_CACHE>
      <USER_DEFAULT_ALLOWED>N</USER_DEFAULT_ALLOWED>
...
'ESSION_ID/>
      ...
      <SELECT_STATEMENT_ID>120027782</SELECT_STATEMENT_ID>
      <TAG_NAME/>
      <PROCESS_SEQUENCE/>
      <STATEMENT_CACHE>N</STATEMENT_CACHE>
      <USER_DEFAULT_ALLOWED>N</USER_DEFAULT_ALLOWED>
    ...
    <ATTRIBUTE>
      ...
      <ENTITY_TYPE_ID>3339</ENTITY_TYPE_ID>
      <EXTERN' ||
'AL_TYPE_ID>500000000000960</EXTERNAL_TYPE_ID>
      <FRAME_ID>94764</FRAME_ID>
      <DATA_LENGTH>4</DATA_LENGTH>
      <DATA_PRECISION>4</DATA_PRECISION>
      <DATA_SCALE>0</DATA_SCALE>
      ...
      <SELECT_STATEMENT_ID>120027783</SELECT_STATEMENT_ID>
      <TAG_NAME/>
      <PROCESS_SEQUENCE/>
      <STATEMENT_CACHE>N</STATEMENT_CACHE>
      <USER_DEFAULT_ALLOWED>N</USER_DEFAULT_ALLOWED>
...
FROM LDC_IMCOCODI
      ...
      <SELECT_STATEMENT_ID>120027784</SELECT_STATEMENT_ID>
      <TAG_NAME/>
      <PROCESS_SEQUENCE/>
      <STATEMENT_CACHE>N</STATEMENT_CACHE>
      <USER_DEFAULT_ALLOWED>N</USER_DEFAULT_ALLOWED>
...
'   <ATTRIBUTE>
      ...
      <DEFAULT_VALUE/>
      <SEL' ||
'ECT_STATEMENT>SELECT ORIGIN_VALUE ID, TARGET_VALUE DESCRIPTION FROM ge_equivalenc_values WHERE equivalence_set_id = 1208  </SELECT_STATEMENT>
      <SELECT_STATEMENT_ID>120027785</SELECT_STATEMENT_ID>
      <TAG_NAME/>
      <PROCESS_SEQUENCE/>
      <STATEMENT_CACHE>N</STATEMENT_CACHE>
      <USER_DEFAULT_ALLOWED>N</USER_DEFAULT_ALLOWED>
    ...
    <ATTRIBUTE>
      ...
      <ENTITY_TYPE_ID>3339</ENTITY_TYPE_ID>
      <EXTER' ||
'NAL_TYPE_ID>500000000000960</EXTERNAL_TYPE_ID>
      <FRAME_ID>94764</FRAME_ID>
      <DATA_LENGTH>4</DATA_LENGTH>
      <DATA_PRECISION>4</DATA_PRECISION>
      <DATA_SCALE>0</DATA_SCALE>
      ...
      <SELECT_STATEMENT_ID>120027786</SELECT_STATEMENT_ID>
      <TAG_NAME/>
      <PROCESS_SEQUENCE/>
      <STATEMENT_CACHE>N</STATEMENT_CACHE>
      <USER_DEFAULT_ALLOWED>N</USER_DEFAULT_ALLOWED>
      <STYLE_CASE>U</STYLE_CASE>
      <SEQUENCE_ID>38</SEQUE' ||
'NCE_ID>
      <EXTERNAL_ROOT_ID>6121</EXTERNAL_ROOT_ID>
      <PARENT_ATTRIBUTE_NAME>ICRTORDE</PARENT_ATTRIBUTE_NAME>
      <PARENT_ENTITY_NAME>LDC_IMCORETE</PARENT_ENTITY_NAME>
    ...
    <ATTRIBUTE>
      ...
      <DISPLAY_NAME>IDENTIFICACION DE LA CAUSA Y CONSECUENCIAS</' ||
'DISPLAY_NAME>
      <INIT_BASIC_EXP_ID/>
      <VALID_BASIC_EXP_ID/>
      <INIT_EXPRESSION_ID/>
...
'ID>94764</FRAME_ID>
      ...
      <SELECT_STATEMENT_ID>120027787</SELECT_STATEMENT_ID>
      <TAG_NAME/>
      <PROCESS_SEQUENCE/>
      <STATEMENT_CACHE>N</STATEMENT_CACHE>
      <USER_DEFAULT_ALLOWED>N</USER_DEFAULT_ALLOWED>
...
'NTITY_ID>3294</ENTITY_ID>
      ...
      <SELECT_STATEMENT_ID>120027788</SELECT_STATEMENT_ID>
      <TAG_NAME/>
      <PROCESS_SEQUENCE' ||
'/>
      <STATEMENT_CACHE>N</STATEMENT_CACHE>
...
'LAY_VIEW>4</DISPLAY_VIEW>
      ...
      <SELECT_STATEMENT_ID>120027789</SELECT_STATEMENT_ID>
      <TAG_NAME/>
      <PROCESS_SEQUENCE/>
      <STATEMENT_CACHE>N</STATEMENT_CACHE>
      <USER_DEFAULT_ALLOWED>N</USER_DEFAULT_ALLOWED>
...
'99999954</CONFIGURATION_ID>
      ...
      <STATEMENT_CACHE>N</STATEMENT_CACHE>
      <USER_DEF' ||
'AULT_ALLOWED>N</USER_DEFAULT_ALLOWED>
      <STYLE_CASE>U</STYLE_CASE>
    ...
    <ATTRIBUTE>
      ...
      <CONFIGURATION_ID>-999999953</CONFIGURATION_ID>
...
';
CONRT_.clColumn_0 := clTemp0 || clTemp1 || clTemp2
;
...
VALUES (CONRT_.tb2_0(0),
XMLType(CONRT_.clColumn_0),
...
,
null,
'a06758013b2403fa9a17d660bbe83d61'
,
3);
...
if (not CONRT_.blProcessStatus) then
 ...
 ut_trace.trace('Borrando tabla GR_CONFIG_EXPRESSION['||'121054813'||']',1);
delete from GR_CONFIG_EXPRESSION where 
  OBJECT_NAME = 'FWFW_PROCESS_CT54E121054813';
...
'OPEN', to_date('26-10-2013 14:03:46','DD-MM-YYYY HH24:MI:SS'), 
to_date('28-10-2013 17:12:53','DD-MM-YYYY HH24:MI:SS'), to_date('28-10-2013 17:12:53','DD-MM-YYYY HH24:MI:SS'), 'G', 
'N', 'PU', null, 
'DS', 'INI - DATOS ENTRADA PARA CONRT', 'PP', 
'CREATE OR REPLACE PROCEDURE FWFW_PROCESS_CT54E121054813(errorNumber OUT NUMBER, errorMessage OUT VARCHAR2)
IS
...
if (not CONRT_.blProcessStatus) then
 ...
 ut_trace.trace('Borrando tabla GR_CONFIG_EXPRESSION['||'121054814'||']',1);
delete from GR_CONFIG_EXPRESSION where 
  OBJECT_NAME = 'FWFW_PROCESS_CT54E121054814';
...
'CREATE OR REPLACE PROCEDURE FWFW_PROCESS_CT54E121054814(errorNumber OUT NUMBER, errorMessage OUT VARCHAR2)
IS
-- Generated by Code Generator (PVCS Version 1.5)
 -- Open Systems Ltd, Copyright 2003.
V0 NUMBER;
...
if (not CONRT_.blProcessStatus) then
 return;
  ...
  BEGIN
   delete from GE_STATEMENT where STATEMENT_ID = 120027769 and MODULE_ID = 16 and NAME = 'product Suscription';
  EXCEPTION
  when ex.RECORD_HAVE_CHILDREN then
    ut_trace.trace('No se pudo borrar la sentencia: [120027769]',1);
   null;
  END;
...
if (not CONRT_.blProcessStatus) then
 return;
  ...
  ut_trace.trace('Borrando tabla GE_STATEMENT',1);
  BEGIN
   delete from GE_STATEMENT where STATEMENT_ID = 120027770 and MODULE_ID = 16 and NAME = 'Direccion IM';
  EXCEPTION
  when ex.RECORD_HAVE_CHILDREN then
    ut_trace.trace('No se pudo borrar la sentencia: [120027770]',1);
   null;
  END;
ut_trace.trace('Obtiene nueva secuencia de GE_STATEMENT',1);
nuNewStatement := GE_BOSEQUENCE.NEXTGE_STATEMENT();
...
if (not CONRT_.blProcessStatus) then
 return;
  ...
  ut_trace.trace('Borrando tabla GE_STATEMENT',1);
  BEGIN
   delete from GE_STATEMENT where STATEMENT_ID = 120027771 and MODULE_ID = 16 and NAME = 'Controlado  S/N';
  EXCEPTION
  when ex.RECORD_HAVE_CHILDREN then
    ut_trace.trace('No se pudo borrar la sentencia: [120027771]',1);
   null;
  END;
ut_trace.trace('Obtiene nueva secuencia de GE_STATEMENT',1);
nuNewStatement := GE_BOSEQUENCE.NEXTGE_STATEMENT();
...
if (not CONRT_.blProcessStatus) then
 return;
  ...
  ut_trace.trace('Borrando tabla GE_STATEMENT',1);
  BEGIN
   delete from GE_STATEMENT where STATEMENT_ID = 120027772 and MODULE_ID = 16 and NAME = 'Sector IM';
  EXCEPTION
  when ex.RECORD_HAVE_CHILDREN then
    ut_trace.trace('No se pudo borrar la sentencia: [120027772]',1);
   null;
  END;
ut_trace.trace('Obtiene nueva secuencia de GE_STATEMENT',1);
nuNewStatement := GE_BOSEQUENCE.NEXTGE_STATEMENT();
...
if (not CONRT_.blProcessStatus) then
 return;
  ...
  ut_trace.trace('Borrando tabla GE_STATEMENT',1);
  BEGIN
   delete from GE_STATEMENT where STATEMENT_ID = 120027773 and MODULE_ID = 16 and NAME = 'Tipo de Trabajo IM';
  EXCEPTION
  when ex.RECORD_HAVE_CHILDREN then
    ut_trace.trace('No se pudo borrar la sentencia: [120027773]',1);
   null;
  END;
ut_trace.trace('Obtiene nueva secuencia de GE_STATEMENT',1);
nuNewStatement := GE_BOSEQUENCE.NEXTGE_STATEMENT();
...
if (not CONRT_.blProcessStatus) then
 return;
  ...
  ut_trace.trace('Borrando tabla GE_STATEMENT',1);
  BEGIN
   delete from GE_STATEMENT where STATEMENT_ID = 120027774 and MODULE_ID = 16 and NAME = 'Tipo de trabajo IM';
  EXCEPTION
  when ex.RECORD_HAVE_CHILDREN then
    ut_trace.trace('No se pudo borrar la sentencia: [120027774]',1);
   null;
  END;
ut_trace.trace('Obtiene nueva secuencia de GE_STATEMENT',1);
nuNewStatement := GE_BOSEQUENCE.NEXTGE_STATEMENT();
...
if (not CONRT_.blProcessStatus) then
 return;
  ...
  ut_trace.trace('Borrando tabla GE_STATEMENT',1);
  BEGIN
   delete from GE_STATEMENT where STATEMENT_ID = 120027775 and MODULE_ID = 16 and NAME = 'Unidad Operativa IMCO';
  EXCEPTION
  when ex.RECORD_HAVE_CHILDREN then
    ut_trace.trace('No se pudo borrar la sentencia: [120027775]',1);
   null;
  END;
ut_trace.trace('Obtiene nueva secuencia de GE_STATEMENT',1);
nuNewStatement := GE_BOSEQUENCE.NEXTGE_STATEMENT();
...
if (not CONRT_.blProcessStatus) then
 return;
  ...
  ut_trace.trace('Borrando tabla GE_STATEMENT',1);
  BEGIN
   delete from GE_STATEMENT where STATEMENT_ID = 120027776 and MODULE_ID = 16 and NAME = 'Tipo de Eventos IM';
  EXCEPTION
  when ex.RECORD_HAVE_CHILDREN then
    ut_trace.trace('No se pudo borrar la sentencia: [120027776]',1);
   null;
  END;
ut_trace.trace('Obtiene nueva secuencia de GE_STATEMENT',1);
nuNewStatement := GE_BOSEQUENCE.NEXTGE_STATEMENT();
...
if (not CONRT_.blProcessStatus) then
 return;
  ...
  ut_trace.trace('Borrando tabla GE_STATEMENT',1);
  BEGIN
   delete from GE_STATEMENT where STATEMENT_ID = 120027777 and MODULE_ID = 16 and NAME = 'Controlado IM';
  EXCEPTION
  when ex.RECORD_HAVE_CHILDREN then
    ut_trace.trace('No se pudo borrar la sentencia: [120027777]',1);
   null;
  END;
ut_trace.trace('Obtiene nueva secuencia de GE_STATEMENT',1);
nuNewStatement := GE_BOSEQUENCE.NEXTGE_STATEMENT();
...
if (not CONRT_.blProcessStatus) then
 return;
  ...
  ut_trace.trace('Borrando tabla GE_STATEMENT',1);
  BEGIN
   delete from GE_STATEMENT where STATEMENT_ID = 120027778 and MODULE_ID = 16 and NAME = 'Suspencio del Servicio IM';
  EXCEPTION
  when ex.RECORD_HAVE_CHILDREN then
    ut_trace.trace('No se pudo borrar la sentencia: [120027778]',1);
   null;
  END;
ut_trace.trace('Obtiene nueva secuencia de GE_STATEMENT',1);
nuNewStatement := GE_BOSEQUENCE.NEXTGE_STATEMENT();
...
if (not CONRT_.blProcessStatus) then
 return;
  ...
  ut_trace.trace('Borrando tabla GE_STATEMENT',1);
  BEGIN
   delete from GE_STATEMENT where STATEMENT_ID = 120027779 and MODULE_ID = 16 and NAME = 'Reportado por IM';
  EXCEPTION
  when ex.RECORD_HAVE_CHILDREN then
    ut_trace.trace('No se pudo borrar la sentencia: [120027779]',1);
   null;
  END;
ut_trace.trace('Obtiene nueva secuencia de GE_STATEMENT',1);
nuNewStatement := GE_BOSEQUENCE.NEXTGE_STATEMENT();
...
if (not CONRT_.blProcessStatus) then
 return;
  ...
  ut_trace.trace('Borrando tabla GE_STATEMENT',1);
  BEGIN
   delete from GE_STATEMENT where STATEMENT_ID = 120027780 and MODULE_ID = 16 and NAME = 'Localizacion en la Red IMCO';
  EXCEPTION
  when ex.RECORD_HAVE_CHILDREN then
    ut_trace.trace('No se pudo borrar la sentencia: [120027780]',1);
   null;
  END;
ut_trace.trace('Obtiene nueva secuencia de GE_STATEMENT',1);
nuNewStatement := GE_BOSEQUENCE.NEXTGE_STATEMENT();
...
if (not CONRT_.blProcessStatus) then
 return;
  ...
  ut_trace.trace('Borrando tabla GE_STATEMENT',1);
  BEGIN
   delete from GE_STATEMENT where STATEMENT_ID = 120027781 and MODULE_ID = 14 and NAME = 'Elementos IMCO';
  EXCEPTION
  when ex.RECORD_HAVE_CHILDREN then
    ut_trace.trace('No se pudo borrar la sentencia: [120027781]',1);
   null;
  END;
ut_trace.trace('Obtiene nueva secuencia de GE_STATEMENT',1);
nuNewStatement := GE_BOSEQUENCE.NEXTGE_STATEMENT();
...
if (not CONRT_.blProcessStatus) then
 return;
  ...
  ut_trace.trace('Borrando tabla GE_STATEMENT',1);
  BEGIN
   delete from GE_STATEMENT where STATEMENT_ID = 120027782 and MODULE_ID = 16 and NAME = 'SubElementos IM';
  EXCEPTION
  when ex.RECORD_HAVE_CHILDREN then
    ut_trace.trace('No se pudo borrar la sentencia: [120027782]',1);
   null;
  END;
ut_trace.trace('Obtiene nueva secuencia de GE_STATEMENT',1);
nuNewStatement := GE_BOSEQUENCE.NEXTGE_STATEMENT();
...
if (not CONRT_.blProcessStatus) then
 return;
  ...
  ut_trace.trace('Borrando tabla GE_STATEMENT',1);
  BEGIN
   delete from GE_STATEMENT where STATEMENT_ID = 120027783 and MODULE_ID = 16 and NAME = 'Marcas Elementos IM';
  EXCEPTION
  when ex.RECORD_HAVE_CHILDREN then
    ut_trace.trace('No se pudo borrar la sentencia: [120027783]',1);
   null;
  END;
ut_trace.trace('Obtiene nueva secuencia de GE_STATEMENT',1);
nuNewStatement := GE_BOSEQUENCE.NEXTGE_STATEMENT();
...
if (not CONRT_.blProcessStatus) then
 return;
  ...
  ut_trace.trace('Borrando tabla GE_STATEMENT',1);
  BEGIN
   delete from GE_STATEMENT where STATEMENT_ID = 120027784 and MODULE_ID = 16 and NAME = 'Diametros de Rotuta IM';
  EXCEPTION
  when ex.RECORD_HAVE_CHILDREN then
    ut_trace.trace('No se pudo borrar la sentencia: [120027784]',1);
   null;
  END;
ut_trace.trace('Obtiene nueva secuencia de GE_STATEMENT',1);
nuNewStatement := GE_BOSEQUENCE.NEXTGE_STATEMENT();
...
if (not CONRT_.blProcessStatus) then
 return;
  ...
  ut_trace.trace('Borrando tabla GE_STATEMENT',1);
  BEGIN
   delete from GE_STATEMENT where STATEMENT_ID = 120027785 and MODULE_ID = 16 and NAME = 'Dimension de rotura IM';
  EXCEPTION
  when ex.RECORD_HAVE_CHILDREN then
    ut_trace.trace('No se pudo borrar la sentencia: [120027785]',1);
   null;
  END;
ut_trace.trace('Obtiene nueva secuencia de GE_STATEMENT',1);
nuNewStatement := GE_BOSEQUENCE.NEXTGE_STATEMENT();
...
if (not CONRT_.blProcessStatus) then
 return;
  ...
  ut_trace.trace('Borrando tabla GE_STATEMENT',1);
  BEGIN
   delete from GE_STATEMENT where STATEMENT_ID = 120027786 and MODULE_ID = 16 and NAME = 'Nivel de Presion IM';
  EXCEPTION
  when ex.RECORD_HAVE_CHILDREN then
    ut_trace.trace('No se pudo borrar la sentencia: [120027786]',1);
   null;
  END;
ut_trace.trace('Obtiene nueva secuencia de GE_STATEMENT',1);
nuNewStatement := GE_BOSEQUENCE.NEXTGE_STATEMENT();
...
if (not CONRT_.blProcessStatus) then
 return;
  ...
  ut_trace.trace('Borrando tabla GE_STATEMENT',1);
  BEGIN
   delete from GE_STATEMENT where STATEMENT_ID = 120027787 and MODULE_ID = 16 and NAME = 'Causas IM';
  EXCEPTION
  when ex.RECORD_HAVE_CHILDREN then
    ut_trace.trace('No se pudo borrar la sentencia: [120027787]',1);
   null;
  END;
ut_trace.trace('Obtiene nueva secuencia de GE_STATEMENT',1);
nuNewStatement := GE_BOSEQUENCE.NEXTGE_STATEMENT();
...
if (not CONRT_.blProcessStatus) then
 return;
  ...
  ut_trace.trace('Borrando tabla GE_STATEMENT',1);
  BEGIN
   delete from GE_STATEMENT where STATEMENT_ID = 120027788 and MODULE_ID = 16 and NAME = 'Resultado - Causal _ IM';
  EXCEPTION
  when ex.RECORD_HAVE_CHILDREN then
    ut_trace.trace('No se pudo borrar la sentencia: [120027788]',1);
   null;
  END;
ut_trace.trace('Obtiene nueva secuencia de GE_STATEMENT',1);
nuNewStatement := GE_BOSEQUENCE.NEXTGE_STATEMENT();
...
if (not CONRT_.blProcessStatus) then
 return;
  ...
  ut_trace.trace('Borrando tabla GE_STATEMENT',1);
  BEGIN
   delete from GE_STATEMENT where STATEMENT_ID = 120027789 and MODULE_ID = 16 and NAME = 'Responsable IM';
  EXCEPTION
  when ex.RECORD_HAVE_CHILDREN then
    ut_trace.trace('No se pudo borrar la sentencia: [120027789]',1);
   null;
  END;
ut_trace.trace('Obtiene nueva secuencia de GE_STATEMENT',1);
nuNewStatement := GE_BOSEQUENCE.NEXTGE_STATEMENT();
...
BEGIN
 ...
 nuIndexInternal := CONRT_.tb0_0.first;
 while nuIndexInternal IS NOT null loop
  IF (CONRT_.tb0_0(nuIndexInternal)=isbExecutableName ) THEN
   IF (CONRT_.tb0_1.exists(nuIndexInternal)) then
    tbExecutableEquivalence(isbExecutableName) := CONRT_.tb0_1(nuIndexInternal);
   END IF;
   exit;
  END IF;
  nuIndexInternal := CONRT_.tb0_0.next(nuIndexInternal);
 END loop; 
 return tbExecutableEquivalence(isbExecutableName);
END;
BEGIN
...
nuIndex :=  CONRT_.tbRoleExecutables.first;
while nuIndex IS NOT null loop
 if (CONRT_.tbRoleExecutables(nuIndex).role_id!=1) then
  nuNewExecutableId := fnuGetNewExecutableId(CONRT_.tbRoleExecutables(nuIndex).name);
  IF nuNewExecutableId is not null then
    INSERT INTO sa_role_executables (role_id, executable_id)
    VALUES (CONRT_.tbRoleExecutables(nuIndex).role_id, nuNewExecutableId);
  END IF;
  end if;
 nuIndex := CONRT_.tbRoleExecutables.next(nuIndex);
END loop;
nuIndex :=  CONRT_.tbUserException.first;
while nuIndex IS NOT null loop
 nuNewExecutableId := fnuGetNewExecutableId(CONRT_.tbUserException(nuIndex).name);
 IF nuNewExecutableId is not null then
  insert INTO sa_user_exceptions (executable_id, user_id, status, usr_exc_type_id, comment_)
  VALUES (nuNewExecutableId ,CONRT_.tbUserException(nuIndex).user_id, CONRT_.tbUserException(nuIndex).status , CONRT_.tbUserException(nuIndex).usr_exc_type_id, CONRT_.tbUserException(nuIndex).comment_);
 END IF;
 nuIndex := CONRT_.tbUserException.next(nuIndex);
END loop;
nuIndex :=  CONRT_.tbExecEntities.first;
while nuIndex IS NOT null loop
 nuNewExecutableId := fnuGetNewExecutableId(CONRT_.tbExecEntities(nuIndex).name);
 IF nuNewExecutableId is not null then
  select count(1) into nuRecCount
 from sa_exec_entities
  where executable_id = nuNewExecutableId and entity_id = CONRT_.tbExecEntities(nuIndex).entity_id;
  IF nuRecCount = 0 then
    insert INTO sa_exec_entities (executable_id, entity_id )
    VALUES (nuNewExecutableId ,CONRT_.tbExecEntities(nuIndex).entity_id );
  END IF;
 END IF;
 nuIndex := CONRT_.tbExecEntities.next(nuIndex);
END loop;
...
BEGIN
 ...
 nuIndex := CONRT_.tbObjectToDelete.first;
 while (nuIndex is not null) LOOP
  BEGIN
   ut_trace.trace('Objeto a borrar: ' || CONRT_.tbObjectToDelete(nuIndex),1);
   IF(ut_object.fboExistObject(CONRT_.tbObjectToDelete(nuIndex)) and not fblIsObjectUseByRule(CONRT_.tbObjectToDelete(nuIndex))) THEN
    blObjectDeleted := ut_object.fboDeleteObject(CONRT_.tbObjectToDelete(nuIndex));
    ut_trace.trace('  * Borrado ' || CONRT_.tbObjectToDelete(nuIndex), 1);
   END IF;
  EXCEPTION when others then
   ut_trace.trace('- No se borra el objeto (' || CONRT_.tbObjectToDelete(nuIndex) || '): ' || sqlerrm,1);
  END;
  nuIndex := CONRT_.tbObjectToDelete.next(nuIndex);
 END loop;
...
begin
SA_BOCreatePackages.DropPackage('CONRT_');
end;
/

BEGIN ut_trace.trace('********************FIN  proceso de objeto:CONRT_******************************'); end;
/
