...
BEGIN
sa_bocreatePackages.CreatePackage('LDCCUS_',
'CREATE OR REPLACE PACKAGE LDCCUS_ IS ' || chr(10) ||
...
'tb2_0 ty2_0;  executableName ge_catalog.tag_name%type := ''LDCCUS''; ' || chr(10) ||
...
'( ' || chr(10) ||
'SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME=''LDCCUS'' ' || chr(10) ||
'); ' || chr(10) ||
...
BEGIN ut_trace.trace('********************Comenzar proceso de objeto:LDCCUS_******************************'); END;
/
...
if (not LDCCUS_.blProcessStatus) then
 return;
end if;
Open LDCCUS_.cuRoleExecutables;
loop
 fetch LDCCUS_.cuRoleExecutables INTO LDCCUS_.rcRoleExecutables;
 exit when  LDCCUS_.cuRoleExecutables%notfound;
 LDCCUS_.tbRoleExecutables(nuIndex) := LDCCUS_.rcRoleExecutables;
 nuIndex := nuIndex + 1;
END loop;
close LDCCUS_.cuRoleExecutables;
nuIndex := 0;
Open LDCCUS_.cuUserExceptions ;
loop
 fetch LDCCUS_.cuUserExceptions INTO  LDCCUS_.rcUserExceptions;
 exit when LDCCUS_.cuUserExceptions%notfound;
 LDCCUS_.tbUserException(nuIndex):=LDCCUS_.rcUserExceptions;
 nuIndex := nuIndex + 1;
END loop;
close LDCCUS_.cuUserExceptions;
nuIndex := 0;
Open LDCCUS_.cuExecEntities ;
loop
 fetch LDCCUS_.cuExecEntities INTO  LDCCUS_.rcExecEntities;
 exit when LDCCUS_.cuExecEntities%notfound;
 LDCCUS_.tbExecEntities(nuIndex):=LDCCUS_.rcExecEntities;
 nuIndex := nuIndex + 1;
END loop;
close LDCCUS_.cuExecEntities;

exception when others then
LDCCUS_.blProcessStatus := false;
rollback;
...
(
 SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDCCUS'
);
    ...
    BEGIN
      ...
      IF (nuNumRecords > 0) THEN
        ...
        IF ( sbObjectName IS NOT null ) THEN
          ...
          if ( LDCCUS_.tbObjectToDelete.last IS null ) then
            nuNumObjects := 1;
          else
            nuNumObjects := LDCCUS_.tbObjectToDelete.last + 1;
          end if;
          LDCCUS_.tbObjectToDelete(nuNumObjects) := sbObjectName;
        END IF;
...
if (not LDCCUS_.blProcessStatus) then
 return;
  ...
  WHERE GE_DISTRIBUTION_FILE.DISTRIBUTION_FILE_ID = 'LDCCUS';
  IF (nuNumRecords > 0) THEN
...
WHERE   sa_executable.name = 'LDCCUS'
...
if (not LDCCUS_.blProcessStatus) then
 ...
 FOR crObjectsName IN cuObjectsName LOOP
  LDCCUS_.tbObjectToDelete(nuIndex) := crObjectsName.OBJECT_NAME;
  nuIndex := nuIndex + 1;
 END LOOP;
EXCEPTION 
...
if (not LDCCUS_.blProcessStatus) then
 return;
  ...
  DELETE FROM SA_ROLE_EXECUTABLES WHERE (EXECUTABLE_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDCCUS') AND ROLE_ID=1;
...
if (not LDCCUS_.blProcessStatus) then
 return;
  ...
  DELETE FROM SA_USER_EXCEPTIONS WHERE (EXECUTABLE_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDCCUS') AND 1=2;
...
SELECT rowid
FROM SA_EXECUTABLE_SYNON WHERE (SYNONYMOUS_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDCCUS');
nuIndex binary_integer;
BEGIN
...
if (not LDCCUS_.blProcessStatus) then
 return;
  ...
  DELETE FROM SA_EXEC_ENTITIES WHERE (EXECUTABLE_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDCCUS');
...
if (not LDCCUS_.blProcessStatus) then
 return;
  ...
  DELETE FROM SA_MENU_OPTION WHERE (EXECUTABLE_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDCCUS');
...
if (not LDCCUS_.blProcessStatus) then
 return;
  ...
  DELETE FROM GI_CONFIG_COMP WHERE (CONFIG_ID) in (SELECT CONFIG_ID FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDCCUS') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339);
...
if (not LDCCUS_.blProcessStatus) then
 return;
  ...
  ut_trace.trace('Borrando tabla GI_CONFIG_COMP',1);
  DELETE FROM GI_CONFIG_COMP WHERE (COMPOSITION_ID) in (SELECT COMPOSITION_ID FROM GI_COMPOSITION WHERE (CONFIG_ID) in (SELECT CONFIG_ID FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDCCUS') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339));
...
DECLARE 
 ...
 SELECT rowid FROM GE_STATEMENT WHERE (STATEMENT_ID) in (SELECT SELECT_STATEMENT_ID FROM GI_COMP_ATTRIBS WHERE (COMPOSITION_ID) in (SELECT COMPOSITION_ID FROM GI_COMPOSITION WHERE (CONFIG_ID) in (SELECT CONFIG_ID FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDCCUS') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339)));
BEGIN 
...
LDCCUS_.tbGE_STATEMENTRowId(rcData.rowid):=rcData.rowid; 
END loop; 
...
for rcData in cuLoadTemporaryTable loop
LDCCUS_.tbGR_CONFIG_EXPRESSIONRowId(rcData.rowid):=rcData.rowid; 
END loop; 
...
if (not LDCCUS_.blProcessStatus) then
 return;
  ...
  DELETE FROM GI_COMP_ATTRIBS WHERE (COMPOSITION_ID) in (SELECT COMPOSITION_ID FROM GI_COMPOSITION WHERE (CONFIG_ID) in (SELECT CONFIG_ID FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDCCUS') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339));
...
nuVarcharIndex:=LDCCUS_.tbGE_STATEMENTRowId.first;  
while (nuVarcharIndex is not null) loop 
begin
ut_trace.trace('Row id a borrar:'||nuVarcharIndex,1);
delete from GE_STATEMENT where rowid = LDCCUS_.tbGE_STATEMENTRowId(nuVarcharIndex); 
...
LDCCUS_.tbGE_STATEMENTRowId.delete(nuOldVarcharIndex);
end if;
nuVarcharIndex := LDCCUS_.tbGE_STATEMENTRowId.next(nuVarcharIndex); 
END loop; 
...
nuVarcharIndex:=LDCCUS_.tbGR_CONFIG_EXPRESSIONRowId.first;  
while (nuVarcharIndex is not null) loop 
ut_trace.trace('Row id a borrar:'||nuVarcharIndex,1);
delete from GR_CONFIG_EXPRESSION where rowid = LDCCUS_.tbGR_CONFIG_EXPRESSIONRowId(nuVarcharIndex); 
nuOldVarcharIndex:=nuVarcharIndex;
nuVarcharIndex := LDCCUS_.tbGR_CONFIG_EXPRESSIONRowId.next(nuVarcharIndex); 
LDCCUS_.tbGR_CONFIG_EXPRESSIONRowId.delete(nuOldVarcharIndex); 
END loop; 
...
for rcData in cuLoadTemporaryTable loop
LDCCUS_.tbGI_COMPOSITIONRowId(rcData.rowid):=rcData.rowid; 
END loop; 
...
if (not LDCCUS_.blProcessStatus) then
 return;
  ...
  DELETE FROM GI_FRAME WHERE (COMPOSITION_ID) in (SELECT COMPOSITION_ID FROM GI_COMPOSITION WHERE (CONFIG_ID) in (SELECT CONFIG_ID FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDCCUS') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339));
...
nuVarcharIndex:=LDCCUS_.tbGI_COMPOSITIONRowId.first;  
while (nuVarcharIndex is not null) loop 
ut_trace.trace('Row id a borrar:'||nuVarcharIndex,1);
delete from GI_COMPOSITION where rowid = LDCCUS_.tbGI_COMPOSITIONRowId(nuVarcharIndex); 
nuOldVarcharIndex:=nuVarcharIndex;
nuVarcharIndex := LDCCUS_.tbGI_COMPOSITIONRowId.next(nuVarcharIndex); 
LDCCUS_.tbGI_COMPOSITIONRowId.delete(nuOldVarcharIndex); 
END loop; 
...
if (not LDCCUS_.blProcessStatus) then
 return;
  ...
  DELETE FROM GI_COMPOSITION WHERE (CONFIG_ID) in (SELECT CONFIG_ID FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDCCUS') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339);
...
if (not LDCCUS_.blProcessStatus) then
 return;
  ...
  DELETE FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDCCUS') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339;
...
if (not LDCCUS_.blProcessStatus) then
 return;
  ...
  DELETE FROM GE_DISTRIBUTION_FILE WHERE (DISTRIBUTION_FILE_ID) in (SELECT NAME FROM SA_EXECUTABLE WHERE NAME='LDCCUS');
...
if (not LDCCUS_.blProcessStatus) then
 return;
  ...
  DELETE FROM SA_EXECUTABLE WHERE NAME='LDCCUS';
...
LDCCUS_.old_tb0_0(0):='LDCCUS'
;
LDCCUS_.tb0_0(0):=UPPER(LDCCUS_.old_tb0_0(0));
LDCCUS_.tb0_0(0):=LDCCUS_.tb0_0(0);
LDCCUS_.old_tb0_1(0):=500000000011030;
LDCCUS_.tb0_1(0):=GE_BOCATALOG.FNUGETIDSEQFROMCATALOG(LDCCUS_.tb0_0(0), 'EXECUTABLE', 'SA_BOEXECUTABLE.GETNEXTID');
LDCCUS_.tb0_1(0):=LDCCUS_.tb0_1(0);
ut_trace.trace('insertando tabla: SA_EXECUTABLE fila (0)',1);
INSERT INTO SA_EXECUTABLE(NAME,EXECUTABLE_ID,DESCRIPTION,PATH,VERSION,EXECUTABLE_TYPE_ID,EXEC_OPER_TYPE_ID,MODULE_ID,SUBSYSTEM_ID,PARENT_EXECUTABLE_ID,LAST_RECORD_ALLOWED,PATH_FILE_HELP,WITHOUT_RESTR_POLICY,DIRECT_EXECUTION,TIMES_EXECUTED,EXEC_OWNER,LAST_DATE_EXECUTED,CLASS_ID) 
VALUES (LDCCUS_.tb0_0(0),
LDCCUS_.tb0_1(0),
'Conciliaci¿n de Usuarios Facturados'
,
null,
'5'
,
...
7,
null,
to_date('28-09-2016 15:00:16','DD-MM-YYYY HH24:MI:SS'),
null);
...
LDCCUS_.tb1_0(0):=1;
LDCCUS_.tb1_1(0):=LDCCUS_.tb0_1(0);
ut_trace.trace('insertando tabla: SA_ROLE_EXECUTABLES fila (0)',1);
INSERT INTO SA_ROLE_EXECUTABLES(ROLE_ID,EXECUTABLE_ID) 
VALUES (LDCCUS_.tb1_0(0),
LDCCUS_.tb1_1(0));
...
LDCCUS_.tb2_0(0):=LDCCUS_.tb0_0(0);
LDCCUS_.clColumn_0 := '<?xml version="1.0" encoding="UTF-8"?>
<PB xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema">
  <APPLICATION>
    <EXECUTABLE_ID>500000000011030</EXECUTABLE_ID>
    <NAME>LDCCUS</NAME>
    <DESCRIPTION>Conciliaci¿n de Usuarios Facturados</DESCRIPTION>
    <VERSION>5</VERSION>
    <MODULE>21</MODULE>
    <DIRECT_EXECUTION>Y</DIRECT_EXECUTION>
    <PATH_FILE_HELP/>
    <OBJECT_NAME>LDC_BOCONCIL_USU_FACT.PROVALIDALDCCUS</OBJECT_NAME>
    <QUERY_NAME/>
  ...
  <COMPOSITION>
    ...
    <EXTERNAL_TYPE_ID>500000000011030</EXTERNAL_TYPE_ID>
    <ENTITY_ID>9623</ENTITY_ID>
    <EXTERNAL_ID>500000000011030</EXTERNAL_ID>
    <FRAME_ID>-6393</FRAME_ID>
' ||
...
union all
SELECT 4 id, '||chr(38)||'apos;A' ||
'BRIL'||chr(38)||'apos; description  FROM dual
union all
...
SELECT 12 id, '||chr(38)||'apos;DICIEMBRE'||chr(38)||'apos; description  FROM dual</SELECT_STATEMENT>
      <SELECT_STATEMENT_ID>120116148</SELECT_STATEMENT_ID>
      <TAG_NAME/>
      <PROCESS_SEQUENCE/>
      <STATEMENT_CACHE>N</STATEMENT_CACHE>
      <USER_DEFAULT_ALLOWED>N</USER_DEFAULT_ALLOWED>
    ...
    <ATTRIBUTE>
      <ENTIT' ||
'Y_ATTRIBUTE_ID>3236</ENTITY_ATTRIBUTE_ID>
      ...
      <VALID_EXPRESSION_ID/>
      <DEFAULT_VALUE/>
      <SELECT_STATEMENT>selec' ||
't geograp_location_id ID, description DESCRIPTION
from ge_geogra_location
WHERE geog_loca_area_type = 2</SELECT_STATEMENT>
      <SELECT_STATEMENT_ID>120116149</SELECT_STATEMENT_ID>
      <TAG_NAME/>
      <PROCESS_SEQUENCE/>
      <STATEMENT_CACHE>N</STATEMENT_CACHE>
      <USER_DEFAULT_ALLOWED>N</USER_DEFAULT_ALLOWED>
...
'    <DATA_PRECISION/>
      ...
      <DISPLAY_NAME>Email para Notificaci¿n de Resultado</DISPLAY_NAME>
      <INIT_BASIC_EXP_ID/>
      <VALID_BASIC_EXP_ID/>
      <INIT_EXPRESSION_ID/>
      <VALID_EXPRESSION_ID/>
...
VALUES (LDCCUS_.tb2_0(0),
XMLType(LDCCUS_.clColumn_0),
'Conciliaci¿n de Usuarios Facturados'
,
'1.0.0.0'
,
...
,
null,
'6ee9fa3b686801bcdf1d07403cfb57bf'
,
3);
...
if (not LDCCUS_.blProcessStatus) then
 return;
  ...
  BEGIN
   delete from GE_STATEMENT where STATEMENT_ID = 120116148 and MODULE_ID = 14 and NAME = 'LDC - SENT - Meses';
  EXCEPTION
  when ex.RECORD_HAVE_CHILDREN then
    ut_trace.trace('No se pudo borrar la sentencia: [120116148]',1);
   null;
  END;
...
VALUES (nuNewStatement, 14, 'LDC - SENT - Meses', 
...
if (not LDCCUS_.blProcessStatus) then
 return;
  ...
  ut_trace.trace('Borrando tabla GE_STATEMENT',1);
  BEGIN
   delete from GE_STATEMENT where STATEMENT_ID = 120116149 and MODULE_ID = 21 and NAME = 'LDC - SENT - Departamentos';
  EXCEPTION
  when ex.RECORD_HAVE_CHILDREN then
    ut_trace.trace('No se pudo borrar la sentencia: [120116149]',1);
   null;
  END;
ut_trace.trace('Obtiene nueva secuencia de GE_STATEMENT',1);
nuNewStatement := GE_BOSEQUENCE.NEXTGE_STATEMENT();
...
ut_trace.trace('insertando tabla: GE_STATEMENT ['||nuNewStatement||']',1);
INSERT INTO GE_STATEMENT(STATEMENT_ID, MODULE_ID, DESCRIPTION, STATEMENT, NAME)  
VALUES (nuNewStatement, 21, 'LDC - SENT - Departamentos', 
'select geograp_location_id ID, description DESCRIPTION
from ge_geogra_location
WHERE geog_loca_area_type = 2', 'LDC - SENT - Departamentos'); 
...
BEGIN
 ...
 nuIndexInternal := LDCCUS_.tb0_0.first;
 while nuIndexInternal IS NOT null loop
  IF (LDCCUS_.tb0_0(nuIndexInternal)=isbExecutableName ) THEN
   IF (LDCCUS_.tb0_1.exists(nuIndexInternal)) then
    tbExecutableEquivalence(isbExecutableName) := LDCCUS_.tb0_1(nuIndexInternal);
   END IF;
   exit;
  END IF;
  nuIndexInternal := LDCCUS_.tb0_0.next(nuIndexInternal);
 END loop; 
 return tbExecutableEquivalence(isbExecutableName);
END;
BEGIN
...
nuIndex :=  LDCCUS_.tbRoleExecutables.first;
while nuIndex IS NOT null loop
 if (LDCCUS_.tbRoleExecutables(nuIndex).role_id!=1) then
  nuNewExecutableId := fnuGetNewExecutableId(LDCCUS_.tbRoleExecutables(nuIndex).name);
  IF nuNewExecutableId is not null then
    INSERT INTO sa_role_executables (role_id, executable_id)
    VALUES (LDCCUS_.tbRoleExecutables(nuIndex).role_id, nuNewExecutableId);
  END IF;
  end if;
 nuIndex := LDCCUS_.tbRoleExecutables.next(nuIndex);
END loop;
nuIndex :=  LDCCUS_.tbUserException.first;
while nuIndex IS NOT null loop
 nuNewExecutableId := fnuGetNewExecutableId(LDCCUS_.tbUserException(nuIndex).name);
 IF nuNewExecutableId is not null then
  insert INTO sa_user_exceptions (executable_id, user_id, status, usr_exc_type_id, comment_)
  VALUES (nuNewExecutableId ,LDCCUS_.tbUserException(nuIndex).user_id, LDCCUS_.tbUserException(nuIndex).status , LDCCUS_.tbUserException(nuIndex).usr_exc_type_id, LDCCUS_.tbUserException(nuIndex).comment_);
 END IF;
 nuIndex := LDCCUS_.tbUserException.next(nuIndex);
END loop;
nuIndex :=  LDCCUS_.tbExecEntities.first;
while nuIndex IS NOT null loop
 nuNewExecutableId := fnuGetNewExecutableId(LDCCUS_.tbExecEntities(nuIndex).name);
 IF nuNewExecutableId is not null then
  select count(1) into nuRecCount
 from sa_exec_entities
  where executable_id = nuNewExecutableId and entity_id = LDCCUS_.tbExecEntities(nuIndex).entity_id;
  IF nuRecCount = 0 then
    insert INTO sa_exec_entities (executable_id, entity_id )
    VALUES (nuNewExecutableId ,LDCCUS_.tbExecEntities(nuIndex).entity_id );
  END IF;
 END IF;
 nuIndex := LDCCUS_.tbExecEntities.next(nuIndex);
END loop;
...
BEGIN
 ...
 nuIndex := LDCCUS_.tbObjectToDelete.first;
 while (nuIndex is not null) LOOP
  BEGIN
   ut_trace.trace('Objeto a borrar: ' || LDCCUS_.tbObjectToDelete(nuIndex),1);
   IF(ut_object.fboExistObject(LDCCUS_.tbObjectToDelete(nuIndex)) and not fblIsObjectUseByRule(LDCCUS_.tbObjectToDelete(nuIndex))) THEN
    blObjectDeleted := ut_object.fboDeleteObject(LDCCUS_.tbObjectToDelete(nuIndex));
    ut_trace.trace('  * Borrado ' || LDCCUS_.tbObjectToDelete(nuIndex), 1);
   END IF;
  EXCEPTION when others then
   ut_trace.trace('- No se borra el objeto (' || LDCCUS_.tbObjectToDelete(nuIndex) || '): ' || sqlerrm,1);
  END;
  nuIndex := LDCCUS_.tbObjectToDelete.next(nuIndex);
 END loop;
...
begin
SA_BOCreatePackages.DropPackage('LDCCUS_');
end;
/

BEGIN ut_trace.trace('********************FIN  proceso de objeto:LDCCUS_******************************'); end;
/
