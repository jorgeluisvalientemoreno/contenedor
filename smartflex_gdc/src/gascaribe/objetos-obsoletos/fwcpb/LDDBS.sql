...
BEGIN
sa_bocreatePackages.CreatePackage('LDDBS_',
'CREATE OR REPLACE PACKAGE LDDBS_ IS ' || chr(10) ||
...
'tb3_0 ty3_0;  executableName ge_catalog.tag_name%type := ''LDDBS''; ' || chr(10) ||
...
'( ' || chr(10) ||
'SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME=''LDDBS'' ' || chr(10) ||
'); ' || chr(10) ||
...
BEGIN ut_trace.trace('********************Comenzar proceso de objeto:LDDBS_******************************'); END;
/
...
if (not LDDBS_.blProcessStatus) then
 return;
end if;
Open LDDBS_.cuRoleExecutables;
loop
 fetch LDDBS_.cuRoleExecutables INTO LDDBS_.rcRoleExecutables;
 exit when  LDDBS_.cuRoleExecutables%notfound;
 LDDBS_.tbRoleExecutables(nuIndex) := LDDBS_.rcRoleExecutables;
 nuIndex := nuIndex + 1;
END loop;
close LDDBS_.cuRoleExecutables;
nuIndex := 0;
Open LDDBS_.cuUserExceptions ;
loop
 fetch LDDBS_.cuUserExceptions INTO  LDDBS_.rcUserExceptions;
 exit when LDDBS_.cuUserExceptions%notfound;
 LDDBS_.tbUserException(nuIndex):=LDDBS_.rcUserExceptions;
 nuIndex := nuIndex + 1;
END loop;
close LDDBS_.cuUserExceptions;
nuIndex := 0;
Open LDDBS_.cuExecEntities ;
loop
 fetch LDDBS_.cuExecEntities INTO  LDDBS_.rcExecEntities;
 exit when LDDBS_.cuExecEntities%notfound;
 LDDBS_.tbExecEntities(nuIndex):=LDDBS_.rcExecEntities;
 nuIndex := nuIndex + 1;
END loop;
close LDDBS_.cuExecEntities;

exception when others then
LDDBS_.blProcessStatus := false;
rollback;
...
(
 SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDDBS'
);
    ...
    BEGIN
      ...
      IF (nuNumRecords > 0) THEN
        ...
        IF ( sbObjectName IS NOT null ) THEN
          ...
          if ( LDDBS_.tbObjectToDelete.last IS null ) then
            nuNumObjects := 1;
          else
            nuNumObjects := LDDBS_.tbObjectToDelete.last + 1;
          end if;
          LDDBS_.tbObjectToDelete(nuNumObjects) := sbObjectName;
        END IF;
...
if (not LDDBS_.blProcessStatus) then
 return;
  ...
  WHERE GE_DISTRIBUTION_FILE.DISTRIBUTION_FILE_ID = 'LDDBS';
  IF (nuNumRecords > 0) THEN
...
WHERE   sa_executable.name = 'LDDBS'
...
if (not LDDBS_.blProcessStatus) then
 ...
 FOR crObjectsName IN cuObjectsName LOOP
  LDDBS_.tbObjectToDelete(nuIndex) := crObjectsName.OBJECT_NAME;
  nuIndex := nuIndex + 1;
 END LOOP;
EXCEPTION 
...
if (not LDDBS_.blProcessStatus) then
 return;
  ...
  DELETE FROM SA_ROLE_EXECUTABLES WHERE (EXECUTABLE_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDDBS') AND ROLE_ID=1;
...
if (not LDDBS_.blProcessStatus) then
 return;
  ...
  DELETE FROM SA_USER_EXCEPTIONS WHERE (EXECUTABLE_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDDBS') AND 1=2;
...
SELECT rowid
FROM SA_EXECUTABLE_SYNON WHERE (SYNONYMOUS_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDDBS');
nuIndex binary_integer;
BEGIN
...
if (not LDDBS_.blProcessStatus) then
 return;
  ...
  DELETE FROM SA_EXEC_ENTITIES WHERE (EXECUTABLE_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDDBS');
...
if (not LDDBS_.blProcessStatus) then
 return;
  ...
  DELETE FROM SA_MENU_OPTION WHERE (EXECUTABLE_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDDBS');
...
if (not LDDBS_.blProcessStatus) then
 return;
  ...
  DELETE FROM GI_CONFIG_COMP WHERE (CONFIG_ID) in (SELECT CONFIG_ID FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDDBS') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339);
...
if (not LDDBS_.blProcessStatus) then
 return;
  ...
  ut_trace.trace('Borrando tabla GI_CONFIG_COMP',1);
  DELETE FROM GI_CONFIG_COMP WHERE (COMPOSITION_ID) in (SELECT COMPOSITION_ID FROM GI_COMPOSITION WHERE (CONFIG_ID) in (SELECT CONFIG_ID FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDDBS') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339));
...
DECLARE 
 ...
 SELECT rowid FROM GE_STATEMENT WHERE (STATEMENT_ID) in (SELECT SELECT_STATEMENT_ID FROM GI_COMP_ATTRIBS WHERE (COMPOSITION_ID) in (SELECT COMPOSITION_ID FROM GI_COMPOSITION WHERE (CONFIG_ID) in (SELECT CONFIG_ID FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDDBS') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339)));
BEGIN 
...
LDDBS_.tbGE_STATEMENTRowId(rcData.rowid):=rcData.rowid; 
END loop; 
...
for rcData in cuLoadTemporaryTable loop
LDDBS_.tbGR_CONFIG_EXPRESSIONRowId(rcData.rowid):=rcData.rowid; 
END loop; 
...
if (not LDDBS_.blProcessStatus) then
 return;
  ...
  DELETE FROM GI_COMP_ATTRIBS WHERE (COMPOSITION_ID) in (SELECT COMPOSITION_ID FROM GI_COMPOSITION WHERE (CONFIG_ID) in (SELECT CONFIG_ID FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDDBS') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339));
...
nuVarcharIndex:=LDDBS_.tbGE_STATEMENTRowId.first;  
while (nuVarcharIndex is not null) loop 
begin
ut_trace.trace('Row id a borrar:'||nuVarcharIndex,1);
delete from GE_STATEMENT where rowid = LDDBS_.tbGE_STATEMENTRowId(nuVarcharIndex); 
...
LDDBS_.tbGE_STATEMENTRowId.delete(nuOldVarcharIndex);
end if;
nuVarcharIndex := LDDBS_.tbGE_STATEMENTRowId.next(nuVarcharIndex); 
END loop; 
...
nuVarcharIndex:=LDDBS_.tbGR_CONFIG_EXPRESSIONRowId.first;  
while (nuVarcharIndex is not null) loop 
ut_trace.trace('Row id a borrar:'||nuVarcharIndex,1);
delete from GR_CONFIG_EXPRESSION where rowid = LDDBS_.tbGR_CONFIG_EXPRESSIONRowId(nuVarcharIndex); 
nuOldVarcharIndex:=nuVarcharIndex;
nuVarcharIndex := LDDBS_.tbGR_CONFIG_EXPRESSIONRowId.next(nuVarcharIndex); 
LDDBS_.tbGR_CONFIG_EXPRESSIONRowId.delete(nuOldVarcharIndex); 
END loop; 
...
for rcData in cuLoadTemporaryTable loop
LDDBS_.tbGI_COMPOSITIONRowId(rcData.rowid):=rcData.rowid; 
END loop; 
...
if (not LDDBS_.blProcessStatus) then
 return;
  ...
  DELETE FROM GI_FRAME WHERE (COMPOSITION_ID) in (SELECT COMPOSITION_ID FROM GI_COMPOSITION WHERE (CONFIG_ID) in (SELECT CONFIG_ID FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDDBS') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339));
...
nuVarcharIndex:=LDDBS_.tbGI_COMPOSITIONRowId.first;  
while (nuVarcharIndex is not null) loop 
ut_trace.trace('Row id a borrar:'||nuVarcharIndex,1);
delete from GI_COMPOSITION where rowid = LDDBS_.tbGI_COMPOSITIONRowId(nuVarcharIndex); 
nuOldVarcharIndex:=nuVarcharIndex;
nuVarcharIndex := LDDBS_.tbGI_COMPOSITIONRowId.next(nuVarcharIndex); 
LDDBS_.tbGI_COMPOSITIONRowId.delete(nuOldVarcharIndex); 
END loop; 
...
if (not LDDBS_.blProcessStatus) then
 return;
  ...
  DELETE FROM GI_COMPOSITION WHERE (CONFIG_ID) in (SELECT CONFIG_ID FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDDBS') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339);
...
if (not LDDBS_.blProcessStatus) then
 return;
  ...
  DELETE FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDDBS') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339;
...
if (not LDDBS_.blProcessStatus) then
 return;
  ...
  DELETE FROM GE_DISTRIBUTION_FILE WHERE (DISTRIBUTION_FILE_ID) in (SELECT NAME FROM SA_EXECUTABLE WHERE NAME='LDDBS');
...
if (not LDDBS_.blProcessStatus) then
 return;
  ...
  DELETE FROM SA_EXECUTABLE WHERE NAME='LDDBS';
...
LDDBS_.old_tb0_0(0):='LDDBS'
;
LDDBS_.tb0_0(0):=UPPER(LDDBS_.old_tb0_0(0));
LDDBS_.tb0_0(0):=LDDBS_.tb0_0(0);
LDDBS_.old_tb0_1(0):=500000000000047;
LDDBS_.tb0_1(0):=GE_BOCATALOG.FNUGETIDSEQFROMCATALOG(LDDBS_.tb0_0(0), 'EXECUTABLE', 'SA_BOEXECUTABLE.GETNEXTID');
LDDBS_.tb0_1(0):=LDDBS_.tb0_1(0);
ut_trace.trace('insertando tabla: SA_EXECUTABLE fila (0)',1);
INSERT INTO SA_EXECUTABLE(NAME,EXECUTABLE_ID,DESCRIPTION,PATH,VERSION,EXECUTABLE_TYPE_ID,EXEC_OPER_TYPE_ID,MODULE_ID,SUBSYSTEM_ID,PARENT_EXECUTABLE_ID,LAST_RECORD_ALLOWED,PATH_FILE_HELP,WITHOUT_RESTR_POLICY,DIRECT_EXECUTION,TIMES_EXECUTED,EXEC_OWNER,LAST_DATE_EXECUTED,CLASS_ID) 
VALUES (LDDBS_.tb0_0(0),
LDDBS_.tb0_1(0),
...
null,
null,
to_date('04-12-2013 19:47:32','DD-MM-YYYY HH24:MI:SS'),
null);
...
LDDBS_.tb1_0(0):=1;
LDDBS_.tb1_1(0):=LDDBS_.tb0_1(0);
ut_trace.trace('insertando tabla: SA_ROLE_EXECUTABLES fila (0)',1);
INSERT INTO SA_ROLE_EXECUTABLES(ROLE_ID,EXECUTABLE_ID) 
VALUES (LDDBS_.tb1_0(0),
LDDBS_.tb1_1(0));
...
LDDBS_.old_tb2_0(0):=40005959;
LDDBS_.tb2_0(0):=SA_BOSEQUENCES.FNUNEXTSA_MENU_OPTION;
LDDBS_.tb2_0(0):=LDDBS_.tb2_0(0);
LDDBS_.tb2_1(0):=LDDBS_.tb0_1(0);
...
LDDBS_.tb3_0(0):=LDDBS_.tb0_0(0);
LDDBS_.clColumn_0 := '<?xml version="1.0" encoding="UTF-8"?>
<PB xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema">
  <APPLICATION>
    <EXECUTABLE_ID>500000000000047</EXECUTABLE_ID>
    <NAME>LDDBS</NAME>
    <DESCRIPTION>Duplicado de factura de venta</DESCRIPTION>
    <VERSION>23</VERSION>
    <MODULE>16</MODULE>
    <DIRECT_EXECUTION>Y</DIRECT_EXECUTION>
    <PATH_FILE_HELP/>
    <OBJECT_NAME>LD_BOSUBSIDY.INSERTBILLCLOB</OBJECT_NAME>
  ...
  <COMPOSITION>
    ...
    <EXTERNAL_TYPE_ID>500000000000047</EXTERNAL_TYPE_ID>
    <ENTITY_ID>9623</ENTITY_ID>
    <EXTERNAL_ID>500000000000047</EXTERNAL_ID>
   ' ||
' <FRAME_ID>96363</FRAME_ID>
    ...
    <TAG_NAME/>
    <AFTER_EXPRESSION_ID/>
    <BEFORE_EXPRESSION_ID>121057705</BEFORE_EXPRESSION_ID>
...
ORDER BY DEAL_ID ASC
      ...
      <SELECT_STATEMENT_ID>120030105</SELECT_STATEMENT_ID>
      <TAG_NAME/>
      <PROCESS_SEQUENCE/>
      <STATEMENT_CACHE>N</STATEMENT_CACHE>
      <USER_DEFAULT_ALLOWED>N</USER_DEFAULT_ALLOWED>
...
'7</DISPLAY_VIEW>
    ...
    <ATTRIBUTE>
      ...
      <ENTITY_ID>8014</ENTITY_ID>
      <ENTITY_NAME>LD_SUBSIDY</ENTITY_NA' ||
'ME>
      <PARENT_CONF_ID>-999999998</PARENT_CONF_ID>
      <TYPE_ATTRIB_ID>2</TYPE_ATTRIB_ID>
      <ENTITY_TYPE_ID>3339</ENTITY_TYPE_ID>
...
ORDER BY SUBSIDY_ID
      ...
      <SELECT_STATEMENT_ID>120030106</SELECT_STATEMENT_ID>
      <TAG_NAME/>
      <PROCESS_SEQUENCE/>
      <STATEMENT_CACHE>N</STATEMENT_CACHE>
      <USER_DEFAULT_ALLOWED>N</USER_DEFAULT_ALLOWED>
 ...
 WHERE ASSIGN_LEVEL = '||chr(38)||'apos;Y'||chr(38)||'apos;
   AND EXISTS (SELECT 1
         ...
         WHERE GE_GEOGRA_LOCATION.GEOG_LOCA_AREA_TYPE = GE_GEOGRA_LOCA_TYPE.GEOG_LOCA_AREA_TYPE
           AN' ||
'D (GE_GEOGRA_LOCA_TYPE.GEOG_LOCA_AREA_TYPE = AB_BOCONSTANTS.FNUOBTTIPOUBICACIONLOC OR
               GE_GEOGRA_LOCA_TYPE.GEOG_LOCA_AREA_TYPE =  AB_BOCONSTANTS.FNUOBTTIPOUBICACIONDPTO)
              )
 ORDER BY GEOGRAP_LOCATION_ID, DESCRIPTION
</SELECT_STATEMENT>
      <SELECT_STATEMENT_ID>120030107</SELECT_STATEMENT_ID>
      <TAG_NAME/>
      <PROCESS_SEQUENCE/>
      <STATEMENT_CACHE>N</STATEMENT_CACHE>
      <USER_DEFAULT_ALLOWED>N</USER_DEFAULT_ALLOWED>
...
where  CATECODI = 1</SELECT_STATEMENT>
      <SELECT_STATEMENT_ID>120030108</SELECT_STATEMENT_ID>
      <TAG_NAME/>
      <PROCESS_SEQUENCE/>
      <STATEMENT_CACHE>N</STATEMENT_CACHE>
      <USER_DEFAULT_ALLOWED>N</USER_DEFAULT_ALLOWED>
...
and    SUCACATE '||chr(38)||'lt;'||chr(38)||'gt; -1</SELECT_STATEMENT>
      <SELECT_STATEMENT_ID>120030109</SELECT_STATEMENT_ID>
      <TAG_NAME/>
      <PROCESS_SEQUENCE/>
      <STATEMENT_CACHE>N</STATEMENT_CACHE>
      <USER_DEFAULT_ALLOWED>N</USER_DEFAULT_ALLOWED>
    ...
    <ATTRIBUTE>
      ...
      <ENTITY_NAME>LD_SALES_WI' ||
'THOUTSUBSIDY</ENTITY_NAME>
      <PARENT_CONF_ID>-999999998</PARENT_CONF_ID>
      <TYPE_ATTRIB_ID>2</TYPE_ATTRIB_ID>
      <ENTITY_TYPE_ID>3339</ENTITY_TYPE_ID>
      <EXTERNAL_TYPE_ID>500000000000047</EXTERNAL_TYPE_ID>
...
FROM   MO_PACKAGES
      ...
      <SELECT_STATEMENT_ID>120030110</SELECT_STATEMENT_ID>
      <TAG_NAME/>
      <PROCESS_SEQUENCE/>
      <STATEMENT_CACHE>N</STATEMENT_CACH' ||
'E>
      <USER_DEFAULT_ALLOWED>N</USER_DEFAULT_ALLOWED>
      <STYLE_CASE>U</STYLE_CASE>
...
VALUES (LDDBS_.tb3_0(0),
XMLType(LDDBS_.clColumn_0),
'Duplicado de factura de venta'
,
'1.0.0.0'
,
...
,
null,
'1eeea8133f117b1c235ed189e456706b'
,
3);
...
if (not LDDBS_.blProcessStatus) then
 ...
 ut_trace.trace('Borrando tabla GR_CONFIG_EXPRESSION['||'121057705'||']',1);
delete from GR_CONFIG_EXPRESSION where 
  OBJECT_NAME = 'FWFW_PROCESS_CT54E121057705';
...
to_date('05-12-2013 11:21:59','DD-MM-YYYY HH24:MI:SS'), to_date('05-12-2013 11:21:59','DD-MM-YYYY HH24:MI:SS'), 'G', 
'N', 'PU', null, 
'DS', 'Eliminar Botón Limpiar', 'PP', 
'CREATE OR REPLACE PROCEDURE FWFW_PROCESS_CT54E121057705(errorNumber OUT NUMBER, errorMessage OUT VARCHAR2)
IS
...
if (not LDDBS_.blProcessStatus) then
 return;
  ...
  BEGIN
   delete from GE_STATEMENT where STATEMENT_ID = 120030105 and MODULE_ID = 16 and NAME = 'convenios parametrizados';
  EXCEPTION
  when ex.RECORD_HAVE_CHILDREN then
    ut_trace.trace('No se pudo borrar la sentencia: [120030105]',1);
   null;
  END;
...
if (not LDDBS_.blProcessStatus) then
 return;
  ...
  ut_trace.trace('Borrando tabla GE_STATEMENT',1);
  BEGIN
   delete from GE_STATEMENT where STATEMENT_ID = 120030106 and MODULE_ID = 16 and NAME = 'Subsidios de un convenio';
  EXCEPTION
  when ex.RECORD_HAVE_CHILDREN then
    ut_trace.trace('No se pudo borrar la sentencia: [120030106]',1);
   null;
  END;
ut_trace.trace('Obtiene nueva secuencia de GE_STATEMENT',1);
nuNewStatement := GE_BOSEQUENCE.NEXTGE_STATEMENT();
...
if (not LDDBS_.blProcessStatus) then
 return;
  ...
  ut_trace.trace('Borrando tabla GE_STATEMENT',1);
  BEGIN
   delete from GE_STATEMENT where STATEMENT_ID = 120030107 and MODULE_ID = 16 and NAME = 'Ubicación geográfica subsidios';
  EXCEPTION
  when ex.RECORD_HAVE_CHILDREN then
    ut_trace.trace('No se pudo borrar la sentencia: [120030107]',1);
   null;
  END;
ut_trace.trace('Obtiene nueva secuencia de GE_STATEMENT',1);
nuNewStatement := GE_BOSEQUENCE.NEXTGE_STATEMENT();
...
if (not LDDBS_.blProcessStatus) then
 return;
  ...
  ut_trace.trace('Borrando tabla GE_STATEMENT',1);
  BEGIN
   delete from GE_STATEMENT where STATEMENT_ID = 120030108 and MODULE_ID = 16 and NAME = 'Categoria residencial';
  EXCEPTION
  when ex.RECORD_HAVE_CHILDREN then
    ut_trace.trace('No se pudo borrar la sentencia: [120030108]',1);
   null;
  END;
ut_trace.trace('Obtiene nueva secuencia de GE_STATEMENT',1);
nuNewStatement := GE_BOSEQUENCE.NEXTGE_STATEMENT();
...
if (not LDDBS_.blProcessStatus) then
 return;
  ...
  ut_trace.trace('Borrando tabla GE_STATEMENT',1);
  BEGIN
   delete from GE_STATEMENT where STATEMENT_ID = 120030109 and MODULE_ID = 16 and NAME = 'Subcategoria subsidio';
  EXCEPTION
  when ex.RECORD_HAVE_CHILDREN then
    ut_trace.trace('No se pudo borrar la sentencia: [120030109]',1);
   null;
  END;
ut_trace.trace('Obtiene nueva secuencia de GE_STATEMENT',1);
nuNewStatement := GE_BOSEQUENCE.NEXTGE_STATEMENT();
...
if (not LDDBS_.blProcessStatus) then
 return;
  ...
  ut_trace.trace('Borrando tabla GE_STATEMENT',1);
  BEGIN
   delete from GE_STATEMENT where STATEMENT_ID = 120030110 and MODULE_ID = 16 and NAME = 'Solicitudes subsidios';
  EXCEPTION
  when ex.RECORD_HAVE_CHILDREN then
    ut_trace.trace('No se pudo borrar la sentencia: [120030110]',1);
   null;
  END;
ut_trace.trace('Obtiene nueva secuencia de GE_STATEMENT',1);
nuNewStatement := GE_BOSEQUENCE.NEXTGE_STATEMENT();
...
BEGIN
 ...
 nuIndexInternal := LDDBS_.tb0_0.first;
 while nuIndexInternal IS NOT null loop
  IF (LDDBS_.tb0_0(nuIndexInternal)=isbExecutableName ) THEN
   IF (LDDBS_.tb0_1.exists(nuIndexInternal)) then
    tbExecutableEquivalence(isbExecutableName) := LDDBS_.tb0_1(nuIndexInternal);
   END IF;
   exit;
  END IF;
  nuIndexInternal := LDDBS_.tb0_0.next(nuIndexInternal);
 END loop; 
 return tbExecutableEquivalence(isbExecutableName);
END;
BEGIN
...
nuIndex :=  LDDBS_.tbRoleExecutables.first;
while nuIndex IS NOT null loop
 if (LDDBS_.tbRoleExecutables(nuIndex).role_id!=1) then
  nuNewExecutableId := fnuGetNewExecutableId(LDDBS_.tbRoleExecutables(nuIndex).name);
  IF nuNewExecutableId is not null then
    INSERT INTO sa_role_executables (role_id, executable_id)
    VALUES (LDDBS_.tbRoleExecutables(nuIndex).role_id, nuNewExecutableId);
  END IF;
  end if;
 nuIndex := LDDBS_.tbRoleExecutables.next(nuIndex);
END loop;
nuIndex :=  LDDBS_.tbUserException.first;
while nuIndex IS NOT null loop
 nuNewExecutableId := fnuGetNewExecutableId(LDDBS_.tbUserException(nuIndex).name);
 IF nuNewExecutableId is not null then
  insert INTO sa_user_exceptions (executable_id, user_id, status, usr_exc_type_id, comment_)
  VALUES (nuNewExecutableId ,LDDBS_.tbUserException(nuIndex).user_id, LDDBS_.tbUserException(nuIndex).status , LDDBS_.tbUserException(nuIndex).usr_exc_type_id, LDDBS_.tbUserException(nuIndex).comment_);
 END IF;
 nuIndex := LDDBS_.tbUserException.next(nuIndex);
END loop;
nuIndex :=  LDDBS_.tbExecEntities.first;
while nuIndex IS NOT null loop
 nuNewExecutableId := fnuGetNewExecutableId(LDDBS_.tbExecEntities(nuIndex).name);
 IF nuNewExecutableId is not null then
  select count(1) into nuRecCount
 from sa_exec_entities
  where executable_id = nuNewExecutableId and entity_id = LDDBS_.tbExecEntities(nuIndex).entity_id;
  IF nuRecCount = 0 then
    insert INTO sa_exec_entities (executable_id, entity_id )
    VALUES (nuNewExecutableId ,LDDBS_.tbExecEntities(nuIndex).entity_id );
  END IF;
 END IF;
 nuIndex := LDDBS_.tbExecEntities.next(nuIndex);
END loop;
...
BEGIN
 ...
 nuIndex := LDDBS_.tbObjectToDelete.first;
 while (nuIndex is not null) LOOP
  BEGIN
   ut_trace.trace('Objeto a borrar: ' || LDDBS_.tbObjectToDelete(nuIndex),1);
   IF(ut_object.fboExistObject(LDDBS_.tbObjectToDelete(nuIndex)) and not fblIsObjectUseByRule(LDDBS_.tbObjectToDelete(nuIndex))) THEN
    blObjectDeleted := ut_object.fboDeleteObject(LDDBS_.tbObjectToDelete(nuIndex));
    ut_trace.trace('  * Borrado ' || LDDBS_.tbObjectToDelete(nuIndex), 1);
   END IF;
  EXCEPTION when others then
   ut_trace.trace('- No se borra el objeto (' || LDDBS_.tbObjectToDelete(nuIndex) || '): ' || sqlerrm,1);
  END;
  nuIndex := LDDBS_.tbObjectToDelete.next(nuIndex);
 END loop;
...
begin
SA_BOCreatePackages.DropPackage('LDDBS_');
end;
/

BEGIN ut_trace.trace('********************FIN  proceso de objeto:LDDBS_******************************'); end;
/
