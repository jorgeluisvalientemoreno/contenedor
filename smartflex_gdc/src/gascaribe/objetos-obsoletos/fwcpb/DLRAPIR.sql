...
BEGIN
sa_bocreatePackages.CreatePackage('DLRAPIR_',
'CREATE OR REPLACE PACKAGE DLRAPIR_ IS ' || chr(10) ||
...
'tb3_0 ty3_0;  executableName ge_catalog.tag_name%type := ''DLRAPIR''; ' || chr(10) ||
...
'( ' || chr(10) ||
'SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME=''DLRAPIR'' ' || chr(10) ||
'); ' || chr(10) ||
...
BEGIN ut_trace.trace('********************Comenzar proceso de objeto:DLRAPIR_******************************'); END;
/
...
if (not DLRAPIR_.blProcessStatus) then
 return;
end if;
Open DLRAPIR_.cuRoleExecutables;
loop
 fetch DLRAPIR_.cuRoleExecutables INTO DLRAPIR_.rcRoleExecutables;
 exit when  DLRAPIR_.cuRoleExecutables%notfound;
 DLRAPIR_.tbRoleExecutables(nuIndex) := DLRAPIR_.rcRoleExecutables;
 nuIndex := nuIndex + 1;
END loop;
close DLRAPIR_.cuRoleExecutables;
nuIndex := 0;
Open DLRAPIR_.cuUserExceptions ;
loop
 fetch DLRAPIR_.cuUserExceptions INTO  DLRAPIR_.rcUserExceptions;
 exit when DLRAPIR_.cuUserExceptions%notfound;
 DLRAPIR_.tbUserException(nuIndex):=DLRAPIR_.rcUserExceptions;
 nuIndex := nuIndex + 1;
END loop;
close DLRAPIR_.cuUserExceptions;
nuIndex := 0;
Open DLRAPIR_.cuExecEntities ;
loop
 fetch DLRAPIR_.cuExecEntities INTO  DLRAPIR_.rcExecEntities;
 exit when DLRAPIR_.cuExecEntities%notfound;
 DLRAPIR_.tbExecEntities(nuIndex):=DLRAPIR_.rcExecEntities;
 nuIndex := nuIndex + 1;
END loop;
close DLRAPIR_.cuExecEntities;

exception when others then
DLRAPIR_.blProcessStatus := false;
rollback;
...
(
 SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='DLRAPIR'
);
    ...
    BEGIN
      ...
      IF (nuNumRecords > 0) THEN
        ...
        IF ( sbObjectName IS NOT null ) THEN
          ...
          if ( DLRAPIR_.tbObjectToDelete.last IS null ) then
            nuNumObjects := 1;
          else
            nuNumObjects := DLRAPIR_.tbObjectToDelete.last + 1;
          end if;
          DLRAPIR_.tbObjectToDelete(nuNumObjects) := sbObjectName;
        END IF;
...
if (not DLRAPIR_.blProcessStatus) then
 return;
  ...
  WHERE GE_DISTRIBUTION_FILE.DISTRIBUTION_FILE_ID = 'DLRAPIR';
  IF (nuNumRecords > 0) THEN
...
WHERE   sa_executable.name = 'DLRAPIR'
...
if (not DLRAPIR_.blProcessStatus) then
 ...
 FOR crObjectsName IN cuObjectsName LOOP
  DLRAPIR_.tbObjectToDelete(nuIndex) := crObjectsName.OBJECT_NAME;
  nuIndex := nuIndex + 1;
 END LOOP;
EXCEPTION 
...
if (not DLRAPIR_.blProcessStatus) then
 return;
  ...
  DELETE FROM SA_ROLE_EXECUTABLES WHERE (EXECUTABLE_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='DLRAPIR') AND ROLE_ID=1;
...
if (not DLRAPIR_.blProcessStatus) then
 return;
  ...
  DELETE FROM SA_USER_EXCEPTIONS WHERE (EXECUTABLE_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='DLRAPIR') AND 1=2;
...
SELECT rowid
FROM SA_EXECUTABLE_SYNON WHERE (SYNONYMOUS_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='DLRAPIR');
nuIndex binary_integer;
BEGIN
...
if (not DLRAPIR_.blProcessStatus) then
 return;
  ...
  DELETE FROM SA_EXEC_ENTITIES WHERE (EXECUTABLE_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='DLRAPIR');
...
if (not DLRAPIR_.blProcessStatus) then
 return;
  ...
  DELETE FROM SA_MENU_OPTION WHERE (EXECUTABLE_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='DLRAPIR');
...
if (not DLRAPIR_.blProcessStatus) then
 return;
  ...
  DELETE FROM GI_CONFIG_COMP WHERE (CONFIG_ID) in (SELECT CONFIG_ID FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='DLRAPIR') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339);
...
if (not DLRAPIR_.blProcessStatus) then
 return;
  ...
  ut_trace.trace('Borrando tabla GI_CONFIG_COMP',1);
  DELETE FROM GI_CONFIG_COMP WHERE (COMPOSITION_ID) in (SELECT COMPOSITION_ID FROM GI_COMPOSITION WHERE (CONFIG_ID) in (SELECT CONFIG_ID FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='DLRAPIR') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339));
...
DECLARE 
 ...
 SELECT rowid FROM GE_STATEMENT WHERE (STATEMENT_ID) in (SELECT SELECT_STATEMENT_ID FROM GI_COMP_ATTRIBS WHERE (COMPOSITION_ID) in (SELECT COMPOSITION_ID FROM GI_COMPOSITION WHERE (CONFIG_ID) in (SELECT CONFIG_ID FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='DLRAPIR') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339)));
BEGIN 
...
DLRAPIR_.tbGE_STATEMENTRowId(rcData.rowid):=rcData.rowid; 
END loop; 
...
for rcData in cuLoadTemporaryTable loop
DLRAPIR_.tbGR_CONFIG_EXPRESSIONRowId(rcData.rowid):=rcData.rowid; 
END loop; 
...
if (not DLRAPIR_.blProcessStatus) then
 return;
  ...
  DELETE FROM GI_COMP_ATTRIBS WHERE (COMPOSITION_ID) in (SELECT COMPOSITION_ID FROM GI_COMPOSITION WHERE (CONFIG_ID) in (SELECT CONFIG_ID FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='DLRAPIR') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339));
...
nuVarcharIndex:=DLRAPIR_.tbGE_STATEMENTRowId.first;  
while (nuVarcharIndex is not null) loop 
begin
ut_trace.trace('Row id a borrar:'||nuVarcharIndex,1);
delete from GE_STATEMENT where rowid = DLRAPIR_.tbGE_STATEMENTRowId(nuVarcharIndex); 
...
DLRAPIR_.tbGE_STATEMENTRowId.delete(nuOldVarcharIndex);
end if;
nuVarcharIndex := DLRAPIR_.tbGE_STATEMENTRowId.next(nuVarcharIndex); 
END loop; 
...
nuVarcharIndex:=DLRAPIR_.tbGR_CONFIG_EXPRESSIONRowId.first;  
while (nuVarcharIndex is not null) loop 
ut_trace.trace('Row id a borrar:'||nuVarcharIndex,1);
delete from GR_CONFIG_EXPRESSION where rowid = DLRAPIR_.tbGR_CONFIG_EXPRESSIONRowId(nuVarcharIndex); 
nuOldVarcharIndex:=nuVarcharIndex;
nuVarcharIndex := DLRAPIR_.tbGR_CONFIG_EXPRESSIONRowId.next(nuVarcharIndex); 
DLRAPIR_.tbGR_CONFIG_EXPRESSIONRowId.delete(nuOldVarcharIndex); 
END loop; 
...
for rcData in cuLoadTemporaryTable loop
DLRAPIR_.tbGI_COMPOSITIONRowId(rcData.rowid):=rcData.rowid; 
END loop; 
...
if (not DLRAPIR_.blProcessStatus) then
 return;
  ...
  DELETE FROM GI_FRAME WHERE (COMPOSITION_ID) in (SELECT COMPOSITION_ID FROM GI_COMPOSITION WHERE (CONFIG_ID) in (SELECT CONFIG_ID FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='DLRAPIR') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339));
...
nuVarcharIndex:=DLRAPIR_.tbGI_COMPOSITIONRowId.first;  
while (nuVarcharIndex is not null) loop 
ut_trace.trace('Row id a borrar:'||nuVarcharIndex,1);
delete from GI_COMPOSITION where rowid = DLRAPIR_.tbGI_COMPOSITIONRowId(nuVarcharIndex); 
nuOldVarcharIndex:=nuVarcharIndex;
nuVarcharIndex := DLRAPIR_.tbGI_COMPOSITIONRowId.next(nuVarcharIndex); 
DLRAPIR_.tbGI_COMPOSITIONRowId.delete(nuOldVarcharIndex); 
END loop; 
...
if (not DLRAPIR_.blProcessStatus) then
 return;
  ...
  DELETE FROM GI_COMPOSITION WHERE (CONFIG_ID) in (SELECT CONFIG_ID FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='DLRAPIR') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339);
...
if (not DLRAPIR_.blProcessStatus) then
 return;
  ...
  DELETE FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='DLRAPIR') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339;
...
if (not DLRAPIR_.blProcessStatus) then
 return;
  ...
  DELETE FROM GE_DISTRIBUTION_FILE WHERE (DISTRIBUTION_FILE_ID) in (SELECT NAME FROM SA_EXECUTABLE WHERE NAME='DLRAPIR');
...
if (not DLRAPIR_.blProcessStatus) then
 return;
  ...
  DELETE FROM SA_EXECUTABLE WHERE NAME='DLRAPIR';
...
DLRAPIR_.old_tb0_0(0):='DLRAPIR'
;
DLRAPIR_.tb0_0(0):=UPPER(DLRAPIR_.old_tb0_0(0));
DLRAPIR_.tb0_0(0):=DLRAPIR_.tb0_0(0);
DLRAPIR_.old_tb0_1(0):=500000000001895;
DLRAPIR_.tb0_1(0):=GE_BOCATALOG.FNUGETIDSEQFROMCATALOG(DLRAPIR_.tb0_0(0), 'EXECUTABLE', 'SA_BOEXECUTABLE.GETNEXTID');
DLRAPIR_.tb0_1(0):=DLRAPIR_.tb0_1(0);
ut_trace.trace('insertando tabla: SA_EXECUTABLE fila (0)',1);
INSERT INTO SA_EXECUTABLE(NAME,EXECUTABLE_ID,DESCRIPTION,PATH,VERSION,EXECUTABLE_TYPE_ID,EXEC_OPER_TYPE_ID,MODULE_ID,SUBSYSTEM_ID,PARENT_EXECUTABLE_ID,LAST_RECORD_ALLOWED,PATH_FILE_HELP,WITHOUT_RESTR_POLICY,DIRECT_EXECUTION,TIMES_EXECUTED,EXEC_OWNER,LAST_DATE_EXECUTED,CLASS_ID) 
VALUES (DLRAPIR_.tb0_0(0),
DLRAPIR_.tb0_1(0),
'Archivo Plano con información de Revisiones Periódicas ITANSUCA'
,
null,
'49'
,
...
2,
null,
to_date('28-04-2016 09:02:50','DD-MM-YYYY HH24:MI:SS'),
null);
...
DLRAPIR_.tb1_0(0):=1;
DLRAPIR_.tb1_1(0):=DLRAPIR_.tb0_1(0);
ut_trace.trace('insertando tabla: SA_ROLE_EXECUTABLES fila (0)',1);
INSERT INTO SA_ROLE_EXECUTABLES(ROLE_ID,EXECUTABLE_ID) 
VALUES (DLRAPIR_.tb1_0(0),
DLRAPIR_.tb1_1(0));
...
DLRAPIR_.old_tb2_0(0):=40007090;
DLRAPIR_.tb2_0(0):=SA_BOSEQUENCES.FNUNEXTSA_MENU_OPTION;
DLRAPIR_.tb2_0(0):=DLRAPIR_.tb2_0(0);
DLRAPIR_.tb2_1(0):=DLRAPIR_.tb0_1(0);
...
DLRAPIR_.tb3_0(0):=DLRAPIR_.tb0_0(0);
DLRAPIR_.clColumn_0 := '<?xml version="1.0" encoding="UTF-8"?>
<PB xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema">
  <APPLICATION>
    <EXECUTABLE_ID>500000000001895</EXECUTABLE_ID>
    <NAME>DLRAPIR</NAME>
    <DESCRIPTION>Archivo Plano con información de Revisiones Periódicas ITANSUCA</DESCRIPTION>
    <VERSION>49</VERSION>
    <MODULE>61</MODULE>
    <DIRECT_EXECUTION>Y</DIRECT_EXECUTION>
    <PATH_FILE_HELP/>
    <OBJECT_NAME>LDC_REPORTE_LEY.PROCESSDLRAPIR</OBJECT_NAME>
    <QUERY_NAME/>
  ...
  <COMPOSITION>
    ...
    <EXTERNAL_TYPE_ID>500000000001895</EXTERNAL_TYPE_ID>
    <ENTITY_ID>9623</ENTITY_ID>
    <EXTERNAL_ID>500000000001895</EXTERNAL_ID>
    <FRA' ||
'ME_ID>96080</FRAME_ID>
...
'CALE/>
    ...
    <ATTRIBUTE>
      <ENTITY_ATTRIBUTE_ID>90042529</ENTITY_ATTRIBUTE_ID>
      <CONFIGURATION_ID>-999999996</CONFIGURATION_ID>
      <ATTRIBUTE_NAME>SERVICIOEXCLU</ATTRIBUTE_NAME>
      <ENTITY_ID>8717</ENT' ||
'ITY_ID>
      <ENTITY_NAME>LDC_LOCALIDAD_ASE_ASNE</ENTITY_NAME>
      <PARENT_CONF_ID>-999999998</PARENT_CONF_ID>
      <TYPE_ATTRIB_ID>2</TYPE_ATTRIB_ID>
      <ENTITY_TYPE_ID>3339</ENTITY_TYPE_ID>
      <EXTERNAL_TYPE_ID>500000000001895</EXTERNAL_TYPE_ID>
      ...
      <DISPLAY_NAME>Agencia Servicio Exclusivo</DISPLAY_NAME>
      <INIT_BASIC_EXP_ID/>
      <VALID_BASIC_EXP_ID/>
      <INIT_EXPRESSION_ID/>
      <VALID_EXPRESSION_ID/>
      <DEFAULT_VALUE/>
      <SELECT_STATEMENT>select '||chr(38)||'apos;-1'||chr(38)||'apos; id, '||chr(38)||'apos;Todos'||chr(38)||'apos; description FROM dual
union
select '||chr(38)||'apos;S'||chr(38)||'apos; id, '||chr(38)||'apos;ASE'||chr(38)||'apos; description FROM dual
union
select '||chr(38)||'apos;N'||chr(38)||'apos; id, '||chr(38)||'apos;ASNE'||chr(38)||'apos; description F' ||
'ROM dual</SELECT_STATEMENT>
      <SELECT_STATEMENT_ID>120037640</SELECT_STATEMENT_ID>
      <TAG_NAME/>
      <PROCESS_SEQUENCE/>
      <STATEMENT_CACHE>N</STATEMENT_CACHE>
      <USER_DEFAULT_ALLOWED>N</USER_DEFAULT_ALLOWED>
...
where geog_loca_area_type = 2</SELECT_STATEMENT>
      <SELECT_STATEMENT_ID>120037641</SELECT_STATEMENT_ID>
      <TAG_NAME/>
      <PROCESS_SEQUENCE/>
      <STATEMENT_CACHE>N</STATEMENT_CACHE>
      <USER_DEFAULT_ALLOWED>N</USER_DEFAULT_ALLOWED>
...
and geo_loca_father_id = GE_BOInstanceControl.fsbGetFieldValue('||chr(38)||'apos;AB_ADDRESS'||chr(38)||'apos;,'||chr(38)||'apos;FATHER_ADDRESS_ID'||chr(38)||'apos;)
      ...
      <SELECT_STATEMENT_ID>120037642</SELECT_STATEMENT_ID>
      <TAG_NAME/>
      <PROCESS_SEQUENCE/>
      <STATEMENT_CACHE>N</STATEMENT_CACHE>
      <USER_DEFAULT_ALLOWED>N</USER_DEFAULT_ALLOWED>
    ...
    <ATTRIBUTE>
      <ENTITY_ATTRIBUTE_ID>97997</ENTITY_ATTRIBUTE_ID>
      <CONFIGURATION_ID>-999999993</CONFIGURATION_ID>
      <ATTRIBUTE_NAME>FECHA_FINAL</ATTRIBUTE_NAME>
      <ENTITY_ID>6181</ENTITY_ID>
      <ENTITY_NAME>GE_PERIODO_CERT</ENTITY_NAME>
      <PARENT_CONF_ID>-99' ||
...
ORDER BY directory_id</SELECT_STATEMENT>
      <SELECT_STATEMENT_ID>120037643</SELECT_STATEMENT_ID>
      <TAG_NAME/>
      <PROCESS_SEQUENCE/>
      <STATEMENT_CACHE>N</STATEMENT_CACHE>
      <USER_DEFAULT_ALLOWED>N</USER_DEFAULT_ALLOWED>
    ...
    <ATTRIBUTE>
      ...
      <CO' ||
'NFIGURATION_ID>-999999990</CONFIGURATION_ID>
      ...
      <DISPLAY_NAME>Dirección Electrónica</DISPLAY_NAME>
      <INIT_BASIC_EXP_ID/>
      <VALID_BASIC_EXP_ID/>
      <INIT_EXPRESSION_ID/>
      <VALID_EXPRESSION_ID/>
...
VALUES (DLRAPIR_.tb3_0(0),
XMLType(DLRAPIR_.clColumn_0),
...
,
null,
'145e4123133d345e32acbfe36870f8ff'
,
3);
...
if (not DLRAPIR_.blProcessStatus) then
 return;
  ...
  BEGIN
   delete from GE_STATEMENT where STATEMENT_ID = 120037640 and MODULE_ID = 61 and NAME = 'Agencia Servicio Exclusivo_1';
  EXCEPTION
  when ex.RECORD_HAVE_CHILDREN then
    ut_trace.trace('No se pudo borrar la sentencia: [120037640]',1);
   null;
  END;
...
VALUES (nuNewStatement, 61, 'Agencia Servicio Exclusivo', 
'select '|| chr(39) ||'-1'|| chr(39) ||' id, '|| chr(39) ||'Todos'|| chr(39) ||' description FROM dual
union
select '|| chr(39) ||'S'|| chr(39) ||' id, '|| chr(39) ||'ASE'|| chr(39) ||' description FROM dual
union
select '|| chr(39) ||'N'|| chr(39) ||' id, '|| chr(39) ||'ASNE'|| chr(39) ||' description FROM dual', 'Agencia Servicio Exclusivo_1'); 
...
UPDATE GE_DISTRIBUTION_FILE SET  GE_DISTRIBUTION_FILE.APP_XML = 
  ...
  '//COMPOSITION/ATTRIBUTE[ENTITY_ATTRIBUTE_ID = 90042529 and ENTITY_ID = 8717]/SELECT_STATEMENT_ID/text()', 
...
if (not DLRAPIR_.blProcessStatus) then
 return;
  ...
  ut_trace.trace('Borrando tabla GE_STATEMENT',1);
  BEGIN
   delete from GE_STATEMENT where STATEMENT_ID = 120037641 and MODULE_ID = 12 and NAME = 'Lista de Departamentos';
  EXCEPTION
  when ex.RECORD_HAVE_CHILDREN then
    ut_trace.trace('No se pudo borrar la sentencia: [120037641]',1);
   null;
  END;
ut_trace.trace('Obtiene nueva secuencia de GE_STATEMENT',1);
nuNewStatement := GE_BOSEQUENCE.NEXTGE_STATEMENT();
...
if (not DLRAPIR_.blProcessStatus) then
 return;
  ...
  ut_trace.trace('Borrando tabla GE_STATEMENT',1);
  BEGIN
   delete from GE_STATEMENT where STATEMENT_ID = 120037642 and MODULE_ID = 12 and NAME = 'Localidades';
  EXCEPTION
  when ex.RECORD_HAVE_CHILDREN then
    ut_trace.trace('No se pudo borrar la sentencia: [120037642]',1);
   null;
  END;
ut_trace.trace('Obtiene nueva secuencia de GE_STATEMENT',1);
nuNewStatement := GE_BOSEQUENCE.NEXTGE_STATEMENT();
...
if (not DLRAPIR_.blProcessStatus) then
 return;
  ...
  ut_trace.trace('Borrando tabla GE_STATEMENT',1);
  BEGIN
   delete from GE_STATEMENT where STATEMENT_ID = 120037643 and MODULE_ID = 1 and NAME = 'Directorio';
  EXCEPTION
  when ex.RECORD_HAVE_CHILDREN then
    ut_trace.trace('No se pudo borrar la sentencia: [120037643]',1);
   null;
  END;
ut_trace.trace('Obtiene nueva secuencia de GE_STATEMENT',1);
nuNewStatement := GE_BOSEQUENCE.NEXTGE_STATEMENT();
...
ut_trace.trace('insertando tabla: GE_STATEMENT ['||nuNewStatement||']',1);
INSERT INTO GE_STATEMENT(STATEMENT_ID, MODULE_ID, DESCRIPTION, STATEMENT, NAME)  
VALUES (nuNewStatement, 1, 'Directorio de Trabajo', 
'SELECT directory_id id, description description
FROM ge_directory
ORDER BY directory_id', 'Directorio'); 
...
BEGIN
 ...
 nuIndexInternal := DLRAPIR_.tb0_0.first;
 while nuIndexInternal IS NOT null loop
  IF (DLRAPIR_.tb0_0(nuIndexInternal)=isbExecutableName ) THEN
   IF (DLRAPIR_.tb0_1.exists(nuIndexInternal)) then
    tbExecutableEquivalence(isbExecutableName) := DLRAPIR_.tb0_1(nuIndexInternal);
   END IF;
   exit;
  END IF;
  nuIndexInternal := DLRAPIR_.tb0_0.next(nuIndexInternal);
 END loop; 
 return tbExecutableEquivalence(isbExecutableName);
END;
BEGIN
...
nuIndex :=  DLRAPIR_.tbRoleExecutables.first;
while nuIndex IS NOT null loop
 if (DLRAPIR_.tbRoleExecutables(nuIndex).role_id!=1) then
  nuNewExecutableId := fnuGetNewExecutableId(DLRAPIR_.tbRoleExecutables(nuIndex).name);
  IF nuNewExecutableId is not null then
    INSERT INTO sa_role_executables (role_id, executable_id)
    VALUES (DLRAPIR_.tbRoleExecutables(nuIndex).role_id, nuNewExecutableId);
  END IF;
  end if;
 nuIndex := DLRAPIR_.tbRoleExecutables.next(nuIndex);
END loop;
nuIndex :=  DLRAPIR_.tbUserException.first;
while nuIndex IS NOT null loop
 nuNewExecutableId := fnuGetNewExecutableId(DLRAPIR_.tbUserException(nuIndex).name);
 IF nuNewExecutableId is not null then
  insert INTO sa_user_exceptions (executable_id, user_id, status, usr_exc_type_id, comment_)
  VALUES (nuNewExecutableId ,DLRAPIR_.tbUserException(nuIndex).user_id, DLRAPIR_.tbUserException(nuIndex).status , DLRAPIR_.tbUserException(nuIndex).usr_exc_type_id, DLRAPIR_.tbUserException(nuIndex).comment_);
 END IF;
 nuIndex := DLRAPIR_.tbUserException.next(nuIndex);
END loop;
nuIndex :=  DLRAPIR_.tbExecEntities.first;
while nuIndex IS NOT null loop
 nuNewExecutableId := fnuGetNewExecutableId(DLRAPIR_.tbExecEntities(nuIndex).name);
 IF nuNewExecutableId is not null then
  select count(1) into nuRecCount
 from sa_exec_entities
  where executable_id = nuNewExecutableId and entity_id = DLRAPIR_.tbExecEntities(nuIndex).entity_id;
  IF nuRecCount = 0 then
    insert INTO sa_exec_entities (executable_id, entity_id )
    VALUES (nuNewExecutableId ,DLRAPIR_.tbExecEntities(nuIndex).entity_id );
  END IF;
 END IF;
 nuIndex := DLRAPIR_.tbExecEntities.next(nuIndex);
END loop;
...
BEGIN
 ...
 nuIndex := DLRAPIR_.tbObjectToDelete.first;
 while (nuIndex is not null) LOOP
  BEGIN
   ut_trace.trace('Objeto a borrar: ' || DLRAPIR_.tbObjectToDelete(nuIndex),1);
   IF(ut_object.fboExistObject(DLRAPIR_.tbObjectToDelete(nuIndex)) and not fblIsObjectUseByRule(DLRAPIR_.tbObjectToDelete(nuIndex))) THEN
    blObjectDeleted := ut_object.fboDeleteObject(DLRAPIR_.tbObjectToDelete(nuIndex));
    ut_trace.trace('  * Borrado ' || DLRAPIR_.tbObjectToDelete(nuIndex), 1);
   END IF;
  EXCEPTION when others then
   ut_trace.trace('- No se borra el objeto (' || DLRAPIR_.tbObjectToDelete(nuIndex) || '): ' || sqlerrm,1);
  END;
  nuIndex := DLRAPIR_.tbObjectToDelete.next(nuIndex);
 END loop;
...
begin
SA_BOCreatePackages.DropPackage('DLRAPIR_');
end;
/

BEGIN ut_trace.trace('********************FIN  proceso de objeto:DLRAPIR_******************************'); end;
/
