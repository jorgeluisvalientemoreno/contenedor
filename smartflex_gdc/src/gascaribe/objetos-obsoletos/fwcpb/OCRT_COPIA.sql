...
BEGIN
sa_bocreatePackages.CreatePackage('OCRT_COPIA_',
'CREATE OR REPLACE PACKAGE OCRT_COPIA_ IS ' || chr(10) ||
...
'tb2_0 ty2_0;  executableName ge_catalog.tag_name%type := ''OCRT_COPIA''; ' || chr(10) ||
...
'( ' || chr(10) ||
'SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME=''OCRT_COPIA'' ' || chr(10) ||
'); ' || chr(10) ||
...
BEGIN ut_trace.trace('********************Comenzar proceso de objeto:OCRT_COPIA_******************************'); END;
/
...
if (not OCRT_COPIA_.blProcessStatus) then
 return;
end if;
Open OCRT_COPIA_.cuRoleExecutables;
loop
 fetch OCRT_COPIA_.cuRoleExecutables INTO OCRT_COPIA_.rcRoleExecutables;
 exit when  OCRT_COPIA_.cuRoleExecutables%notfound;
 OCRT_COPIA_.tbRoleExecutables(nuIndex) := OCRT_COPIA_.rcRoleExecutables;
 nuIndex := nuIndex + 1;
END loop;
close OCRT_COPIA_.cuRoleExecutables;
nuIndex := 0;
Open OCRT_COPIA_.cuUserExceptions ;
loop
 fetch OCRT_COPIA_.cuUserExceptions INTO  OCRT_COPIA_.rcUserExceptions;
 exit when OCRT_COPIA_.cuUserExceptions%notfound;
 OCRT_COPIA_.tbUserException(nuIndex):=OCRT_COPIA_.rcUserExceptions;
 nuIndex := nuIndex + 1;
END loop;
close OCRT_COPIA_.cuUserExceptions;
nuIndex := 0;
Open OCRT_COPIA_.cuExecEntities ;
loop
 fetch OCRT_COPIA_.cuExecEntities INTO  OCRT_COPIA_.rcExecEntities;
 exit when OCRT_COPIA_.cuExecEntities%notfound;
 OCRT_COPIA_.tbExecEntities(nuIndex):=OCRT_COPIA_.rcExecEntities;
 nuIndex := nuIndex + 1;
END loop;
close OCRT_COPIA_.cuExecEntities;

exception when others then
OCRT_COPIA_.blProcessStatus := false;
rollback;
...
(
 SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='OCRT_COPIA'
);
    ...
    BEGIN
      ...
      IF (nuNumRecords > 0) THEN
        ...
        IF ( sbObjectName IS NOT null ) THEN
          ...
          if ( OCRT_COPIA_.tbObjectToDelete.last IS null ) then
            nuNumObjects := 1;
          else
            nuNumObjects := OCRT_COPIA_.tbObjectToDelete.last + 1;
          end if;
          OCRT_COPIA_.tbObjectToDelete(nuNumObjects) := sbObjectName;
        END IF;
...
if (not OCRT_COPIA_.blProcessStatus) then
 return;
  ...
  WHERE GE_DISTRIBUTION_FILE.DISTRIBUTION_FILE_ID = 'OCRT_COPIA';
  IF (nuNumRecords > 0) THEN
...
WHERE   sa_executable.name = 'OCRT_COPIA'
...
if (not OCRT_COPIA_.blProcessStatus) then
 ...
 FOR crObjectsName IN cuObjectsName LOOP
  OCRT_COPIA_.tbObjectToDelete(nuIndex) := crObjectsName.OBJECT_NAME;
  nuIndex := nuIndex + 1;
 END LOOP;
EXCEPTION 
...
if (not OCRT_COPIA_.blProcessStatus) then
 return;
  ...
  DELETE FROM SA_ROLE_EXECUTABLES WHERE (EXECUTABLE_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='OCRT_COPIA') AND ROLE_ID=1;
...
if (not OCRT_COPIA_.blProcessStatus) then
 return;
  ...
  DELETE FROM SA_USER_EXCEPTIONS WHERE (EXECUTABLE_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='OCRT_COPIA') AND 1=2;
...
SELECT rowid
FROM SA_EXECUTABLE_SYNON WHERE (SYNONYMOUS_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='OCRT_COPIA');
nuIndex binary_integer;
BEGIN
...
if (not OCRT_COPIA_.blProcessStatus) then
 return;
  ...
  DELETE FROM SA_EXEC_ENTITIES WHERE (EXECUTABLE_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='OCRT_COPIA');
...
if (not OCRT_COPIA_.blProcessStatus) then
 return;
  ...
  DELETE FROM SA_MENU_OPTION WHERE (EXECUTABLE_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='OCRT_COPIA');
...
if (not OCRT_COPIA_.blProcessStatus) then
 return;
  ...
  DELETE FROM GI_CONFIG_COMP WHERE (CONFIG_ID) in (SELECT CONFIG_ID FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='OCRT_COPIA') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339);
...
if (not OCRT_COPIA_.blProcessStatus) then
 return;
  ...
  ut_trace.trace('Borrando tabla GI_CONFIG_COMP',1);
  DELETE FROM GI_CONFIG_COMP WHERE (COMPOSITION_ID) in (SELECT COMPOSITION_ID FROM GI_COMPOSITION WHERE (CONFIG_ID) in (SELECT CONFIG_ID FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='OCRT_COPIA') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339));
...
DECLARE 
 ...
 SELECT rowid FROM GE_STATEMENT WHERE (STATEMENT_ID) in (SELECT SELECT_STATEMENT_ID FROM GI_COMP_ATTRIBS WHERE (COMPOSITION_ID) in (SELECT COMPOSITION_ID FROM GI_COMPOSITION WHERE (CONFIG_ID) in (SELECT CONFIG_ID FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='OCRT_COPIA') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339)));
BEGIN 
...
OCRT_COPIA_.tbGE_STATEMENTRowId(rcData.rowid):=rcData.rowid; 
END loop; 
...
for rcData in cuLoadTemporaryTable loop
OCRT_COPIA_.tbGR_CONFIG_EXPRESSIONRowId(rcData.rowid):=rcData.rowid; 
END loop; 
...
if (not OCRT_COPIA_.blProcessStatus) then
 return;
  ...
  DELETE FROM GI_COMP_ATTRIBS WHERE (COMPOSITION_ID) in (SELECT COMPOSITION_ID FROM GI_COMPOSITION WHERE (CONFIG_ID) in (SELECT CONFIG_ID FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='OCRT_COPIA') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339));
...
nuVarcharIndex:=OCRT_COPIA_.tbGE_STATEMENTRowId.first;  
while (nuVarcharIndex is not null) loop 
begin
ut_trace.trace('Row id a borrar:'||nuVarcharIndex,1);
delete from GE_STATEMENT where rowid = OCRT_COPIA_.tbGE_STATEMENTRowId(nuVarcharIndex); 
...
OCRT_COPIA_.tbGE_STATEMENTRowId.delete(nuOldVarcharIndex);
end if;
nuVarcharIndex := OCRT_COPIA_.tbGE_STATEMENTRowId.next(nuVarcharIndex); 
END loop; 
...
nuVarcharIndex:=OCRT_COPIA_.tbGR_CONFIG_EXPRESSIONRowId.first;  
while (nuVarcharIndex is not null) loop 
ut_trace.trace('Row id a borrar:'||nuVarcharIndex,1);
delete from GR_CONFIG_EXPRESSION where rowid = OCRT_COPIA_.tbGR_CONFIG_EXPRESSIONRowId(nuVarcharIndex); 
nuOldVarcharIndex:=nuVarcharIndex;
nuVarcharIndex := OCRT_COPIA_.tbGR_CONFIG_EXPRESSIONRowId.next(nuVarcharIndex); 
OCRT_COPIA_.tbGR_CONFIG_EXPRESSIONRowId.delete(nuOldVarcharIndex); 
END loop; 
...
for rcData in cuLoadTemporaryTable loop
OCRT_COPIA_.tbGI_COMPOSITIONRowId(rcData.rowid):=rcData.rowid; 
END loop; 
...
if (not OCRT_COPIA_.blProcessStatus) then
 return;
  ...
  DELETE FROM GI_FRAME WHERE (COMPOSITION_ID) in (SELECT COMPOSITION_ID FROM GI_COMPOSITION WHERE (CONFIG_ID) in (SELECT CONFIG_ID FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='OCRT_COPIA') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339));
...
nuVarcharIndex:=OCRT_COPIA_.tbGI_COMPOSITIONRowId.first;  
while (nuVarcharIndex is not null) loop 
ut_trace.trace('Row id a borrar:'||nuVarcharIndex,1);
delete from GI_COMPOSITION where rowid = OCRT_COPIA_.tbGI_COMPOSITIONRowId(nuVarcharIndex); 
nuOldVarcharIndex:=nuVarcharIndex;
nuVarcharIndex := OCRT_COPIA_.tbGI_COMPOSITIONRowId.next(nuVarcharIndex); 
OCRT_COPIA_.tbGI_COMPOSITIONRowId.delete(nuOldVarcharIndex); 
END loop; 
...
if (not OCRT_COPIA_.blProcessStatus) then
 return;
  ...
  DELETE FROM GI_COMPOSITION WHERE (CONFIG_ID) in (SELECT CONFIG_ID FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='OCRT_COPIA') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339);
...
if (not OCRT_COPIA_.blProcessStatus) then
 return;
  ...
  DELETE FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='OCRT_COPIA') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339;
...
if (not OCRT_COPIA_.blProcessStatus) then
 return;
  ...
  DELETE FROM GE_DISTRIBUTION_FILE WHERE (DISTRIBUTION_FILE_ID) in (SELECT NAME FROM SA_EXECUTABLE WHERE NAME='OCRT_COPIA');
...
if (not OCRT_COPIA_.blProcessStatus) then
 return;
  ...
  DELETE FROM SA_EXECUTABLE WHERE NAME='OCRT_COPIA';
...
OCRT_COPIA_.old_tb0_0(0):='OCRT_COPIA'
;
OCRT_COPIA_.tb0_0(0):=UPPER(OCRT_COPIA_.old_tb0_0(0));
OCRT_COPIA_.tb0_0(0):=OCRT_COPIA_.tb0_0(0);
OCRT_COPIA_.old_tb0_1(0):=500000000000961;
OCRT_COPIA_.tb0_1(0):=GE_BOCATALOG.FNUGETIDSEQFROMCATALOG(OCRT_COPIA_.tb0_0(0), 'EXECUTABLE', 'SA_BOEXECUTABLE.GETNEXTID');
OCRT_COPIA_.tb0_1(0):=OCRT_COPIA_.tb0_1(0);
ut_trace.trace('insertando tabla: SA_EXECUTABLE fila (0)',1);
INSERT INTO SA_EXECUTABLE(NAME,EXECUTABLE_ID,DESCRIPTION,PATH,VERSION,EXECUTABLE_TYPE_ID,EXEC_OPER_TYPE_ID,MODULE_ID,SUBSYSTEM_ID,PARENT_EXECUTABLE_ID,LAST_RECORD_ALLOWED,PATH_FILE_HELP,WITHOUT_RESTR_POLICY,DIRECT_EXECUTION,TIMES_EXECUTED,EXEC_OWNER,LAST_DATE_EXECUTED,CLASS_ID) 
VALUES (OCRT_COPIA_.tb0_0(0),
OCRT_COPIA_.tb0_1(0),
...
null,
null,
to_date('31-05-2013 08:34:04','DD-MM-YYYY HH24:MI:SS'),
null);
...
OCRT_COPIA_.tb1_0(0):=1;
OCRT_COPIA_.tb1_1(0):=OCRT_COPIA_.tb0_1(0);
ut_trace.trace('insertando tabla: SA_ROLE_EXECUTABLES fila (0)',1);
INSERT INTO SA_ROLE_EXECUTABLES(ROLE_ID,EXECUTABLE_ID) 
VALUES (OCRT_COPIA_.tb1_0(0),
OCRT_COPIA_.tb1_1(0));
...
OCRT_COPIA_.tb2_0(0):=OCRT_COPIA_.tb0_0(0);
...
<PB xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema">
  <APPLICATION>
    <EXECUTABLE_ID>500000000000961</EXECUTABLE_ID>
    <NAME>OCRT_COPIA</NAME>
  ...
  <COMPOSITION>
    ...
    <EXTERNAL_TYPE_ID>500000000000961</EXTERNAL_TYPE_ID>
    <ENTITY_ID>9623</ENTITY_ID>
    <EXTERNAL_ID>500000000000961</EXTERNAL_ID>
...
'_IMCORETE</ENTITY_NAME>
      ...
      <VALID_BASIC_EXP_ID/>
      <INIT_EXPRESSION_ID/>
      <VALID_EXPRESSION_ID>121037765</VALID_EXPRESSION_ID>
      <DEFAULT_VALUE/>
      <SELECT_STATEMENT/>
      <SELECT_STATEMENT_ID/>
      <TAG_NAME/>
...
' <INIT_BASIC_EXP_ID/>
    ...
    <ATTRIBUTE>
      ...
      <TYPE_ATTRIB_ID>2</TYPE_ATTRIB_ID>
      <ENTITY_TYPE_ID>3339</ENTITY_TYPE_ID>
      <EXTERNAL_TYPE_ID>50000' ||
'0000000961</EXTERNAL_TYPE_ID>
      <FRAME_ID>99005</FRAME_ID>
      <DATA_LENGTH>15</DATA_LENGTH>
      <DATA_PRECISION>15</DATA_PRECISION>
      <DATA_SCALE>0</DATA_SCALE>
...
'||chr(64)||'active = '||chr(38)||'apos;Y'||chr(38)||'apos; '||chr(64)||'</SELECT_STATEMENT>
      <SELECT_STATEMENT_ID>120018050</SELECT_STATEMENT_ID>
      <TAG_NAME/>
      <PROCESS_SEQUENCE/>
 ' ||
'     <STATEMENT_CACHE>N</STATEMENT_CACHE>
...
'||chr(64)||'address_id = :address_id'||chr(64)||'</SELECT_STATEMENT>
      <SELECT_STATEMENT_ID>120018051</SELECT_STATEMENT_ID>
      <TAG_NAME/>
      <PROCESS_SEQUENCE/>
      <STATEMENT_CACHE>N</STATEMENT_CACHE>
      <USER_DEFAULT_ALLOWED>N</USER_DEFAULT_ALLOWED>
    ...
    <ATTRIBUTE>
      <ENTITY_ATTRIBUTE_ID>90018313</ENTITY_ATTRIBU' ||
'TE_ID>
      ...
      <VALID_EXPRESSION_ID/>
      <DEFAULT_VALUE/>
      <SELECT_STATEMENT>SELECT in_persist ID, DECODE(in_persist,'||chr(38)||'apos;N'||chr(38)||'apos;,'||chr(38)||'apos;No'||chr(38)||'apos;,'||chr(38)||'apos;Si'||chr(38)||'apos;) DES' ||
'CRIPTION
FROM ge_entity
WHERE entity_id IN (68,3203 )</SELECT_STATEMENT>
      <SELECT_STATEMENT_ID>120018052</SELECT_STATEMENT_ID>
      <TAG_NAME/>
      <PROCESS_SEQUENCE/>
      <STATEMENT_CACHE>N</STATEMENT_CACHE>
      <USER_DEFAULT_ALLOWED>N</USER_DEFAULT_ALLOWED>
...
'||chr(64)||'operating_sector_id = :operating_sector_id '||chr(64)||'
      ...
      <SELECT_STATEMENT_ID>120018053</SELECT_STATEMENT_ID>
      <TAG_NAME/>
      <PROCESS_SEQUENCE/>
      <STATEMENT_CACHE>N</STATEMENT_CACHE>
      <USER_DEFAULT_ALLOWED>N</USER_DEFAULT_ALLOWED>
    ...
    <ATTRIBUTE>
      ...
      <ATTRIB' ||
'UTE_NAME>ICRTCARE</ATTRIBUTE_NAME>
      <ENTITY_ID>8363</ENTITY_ID>
      <ENTITY_NAME>LDC_IMCORETE</ENTITY_NAME>
      <PARENT_CONF_ID>-999999998</PARENT_CONF_ID>
      <TYPE_ATTRIB_ID>2</TYPE_ATTRIB_ID>
...
'||chr(64)||'task_type_id = :task_type_id'||chr(64)||'</SELECT_STATEMENT>
  ' ||
'    <SELECT_STATEMENT_ID>120018054</SELECT_STATEMENT_ID>
      <TAG_NAME/>
      <PROCESS_SEQUENCE/>
      <STATEMENT_CACHE>N</STATEMENT_CACHE>
      <USER_DEFAULT_ALLOWED>N</USER_DEFAULT_ALLOWED>
...
'||chr(64)||'WHERE'||chr(64)||'
'||chr(64)||'task_type_id = :task_type_id'||chr(64)||'</SELECT_STATEMENT>
      <SELECT_STATEMENT_ID>120018055</SELECT_STATEMENT_ID>
      <TAG_NAME/>
      <PROCESS_SEQUENCE/>
      <STATEMENT_CACHE>N</STATEMENT_CACHE>
      <USER_DEFAULT_ALLOWED>N</USER_DEFAULT_ALLOWED>
      ...
      <EXTERNAL_ROOT_ID>6121</EXTERNAL_ROOT_ID>
      <PARENT_ATTRIBUTE_NAME>ICRTORDE</PARENT_ATTRIBUTE_NAME>
      <PARENT_ENTITY_NAME>LD' ||
'C_IMCORETE</PARENT_ENTITY_NAME>
    </ATTRIBUTE>
    <ATTRIBUTE>
...
'||chr(64)||'operating_unit_id = :operating_unit_id'||chr(64)||'</SELECT_STATEMENT>
      <SELECT_STATEMENT_ID>120018056</SELECT_STATEMENT_ID>
      <TAG_NAME/>
      <PROCESS_SEQUENCE/>
      <STATEMENT_CACHE>N</STATEMENT_CACHE>
      <USER_DEFAULT_ALLOWED>N</USER_DEFAULT_ALLOWED>
...
'     <ENTITY_TYPE_ID>3339</ENTITY_TYPE_ID>
      ...
      <PARENT_ATTRIBUTE_NAME>ICRTORDE</PARENT_ATTRIBUTE_NAME>
      <PARENT_ENTITY_NAME>' ||
'LDC_IMCORETE</PARENT_ENTITY_NAME>
    </ATTRIBUTE>
    <ATTRIBUTE>
...
'/>
    ...
    <ATTRIBUTE>
      ...
      <ATT' ||
'RIBUTE_NAME>ICRTCTRL</ATTRIBUTE_NAME>
      <ENTITY_ID>8363</ENTITY_ID>
      <ENTITY_NAME>LDC_IMCORETE</ENTITY_NAME>
      <PARENT_CONF_ID>-999999998</PARENT_CONF_ID>
      <TYPE_ATTRIB_ID>2</TYPE_ATTRIB_ID>
      ...
      <PROCESS_SEQUENCE/>
      <STATEMENT_CACHE>N</STATEMENT_CACHE>
      <USER_DEFAULT_ALLOWED>N</USER_DEFAULT_AL' ||
'LOWED>
      <STYLE_CASE>U</STYLE_CASE>
...
FROM LDC_IMCOTIEV
      ...
      <SELECT_STATEMENT_ID>120018057</SELECT_STATEMENT_ID>
      <TAG_NAME/>
      <PROCESS_SEQUENCE/>
      <STATEMENT_CACHE>N</STATEMENT_CACHE>
      <USER_DEFAULT_ALLOWED>N</USER_DEFAULT_ALLOWED>
    ...
    <ATTRIBUTE>
      <ENTITY_ATTRIBUTE_ID>90018326</ENTITY_ATTR' ||
'IBUTE_ID>
      ...
      <VALID_EXPRESSION_ID/>
      <DEFAULT_VALUE/>
      <SELECT_STATEMENT>SELECT in_persist ID, DECODE(in_persist,'||chr(38)||'apos;N'||chr(38)||'apos;,'||chr(38)||'apos;No'||chr(38)||'apos;,'||chr(38)||'apos;Si'||chr(38)||'apos;' ||
') DESCRIPTION
FROM ge_entity
WHERE entity_id IN (68,3203 )</SELECT_STATEMENT>
      <SELECT_STATEMENT_ID>120018058</SELECT_STATEMENT_ID>
      <TAG_NAME/>
      <PROCESS_SEQUENCE/>
      <STATEMENT_CACHE>N</STATEMENT_CACHE>
      <USER_DEFAULT_ALLOWED>N</USER_DEFAULT_ALLOWED>
...
FROM ge_entity
WHERE entity_id IN (68,3203 )</SELECT_STATEMENT>
      <SELECT_STATEMENT_ID>120018059</SELECT_STATEMENT_ID>
      <TAG_NAME/>
      <PROCESS_SEQUENCE/>
      <STATEMENT_CACHE>N</STATEMENT_CACHE>
      <USER_DEFAULT_ALLOWED>N</USER_DEFAULT_ALLOWED>
      ...
      <EXTERNAL_ROOT_ID>6121</EXTERNAL_ROOT_I' ||
'D>
      <PARENT_ATTRIBUTE_NAME>ICRTORDE</PARENT_ATTRIBUTE_NAME>
      <PARENT_ENTITY_NAME>LDC_IMCORETE</PARENT_ENTITY_NAME>
    </ATTRIBUTE>
...
'T_BASIC_EXP_ID/>
    ...
    <ATTRIBUTE>
      ...
      <TYPE_ATTRIB_ID>2</TYPE_ATTRIB_ID>
      <ENTITY_TYPE_ID>3339</ENTITY_TYPE_ID>
      <EXTERNAL_TYPE_ID>500000000' ||
'000961</EXTERNAL_TYPE_ID>
      <FRAME_ID>99005</FRAME_ID>
    ...
    <ATTRIBUTE>
      <ENTITY_ATT' ||
'RIBUTE_ID>90018336</ENTITY_ATTRIBUTE_ID>
      ...
      <VALID_EXPRESSION_ID/>
      <DEFAULT_VALUE/>
      <SELECT_STATEMENT>SELECT ICRPCODI ID,IC' ||
'RPDESC DESCRIPTION
FROM LDC_IMCOREPO</SELECT_STATEMENT>
      <SELECT_STATEMENT_ID>120018060</SELECT_STATEMENT_ID>
      <TAG_NAME/>
      <PROCESS_SEQUENCE/>
      <STATEMENT_CACHE>N</STATEMENT_CACHE>
      <USER_DEFAULT_ALLOWED>N</USER_DEFAULT_ALLOWED>
...
FROM LDC_IMCOLORE
      ...
      <SELECT_STATEMENT_ID>120018061</SELECT_STATEMENT_ID>
      <TAG_NAME/>
      <PROCESS_SEQUENCE/>
      <STATEMENT_CACHE>N</STATEMENT_CACHE>
      <USER_DEFAULT_ALLOWED>N</USER_DEFAULT_ALLOWED>
      ...
      <PARENT_ATTRIBUTE_NAME>ICRTORDE</PARENT_ATTRIBUTE_NAME>
      <PAR' ||
'ENT_ENTITY_NAME>LDC_IMCORETE</PARENT_ENTITY_NAME>
    </ATTRIBUTE>
    <ATTRIBUTE>
...
'
      ...
      <SELECT_STATEMENT_ID>120018062</SELECT_STATEMENT_ID>
      <TAG_NAME/>
      <PROCESS_SEQUENCE/>
      <STATEM' ||
'ENT_CACHE>N</STATEMENT_CACHE>
      <USER_DEFAULT_ALLOWED>N</USER_DEFAULT_ALLOWED>
      <STYLE_CASE>U</STYLE_CASE>
...
'S_UPDATABLE>
      ...
      <SELECT_STATEMENT_ID>120018063</SELECT_STATEMENT_ID>
      <TAG_NAME/>
      <PROCESS_SEQUENCE/>
      <STATEMENT_CACHE>N</STATEMENT_CACHE>
      <USER_DEFAULT_ALLOWED>N</USER_DEFAULT_ALLOWED>
    ...
    <ATTRIBUTE>
      ...
      <ATTRIBU' ||
'TE_NAME>ICTRMARC</ATTRIBUTE_NAME>
      <ENTITY_ID>8363</ENTITY_ID>
      <ENTITY_NAME>LDC_IMCORETE</ENTITY_NAME>
      <PARENT_CONF_ID>-999999998</PARENT_CONF_ID>
      <TYPE_ATTRIB_ID>2</TYPE_ATTRIB_ID>
      ...
      <SELECT_STATEMENT_ID>120018064</SELECT_STATEMENT_ID>
  ' ||
'    <TAG_NAME/>
      <PROCESS_SEQUENCE/>
      <STATEMENT_CACHE>N</STATEMENT_CACHE>
...
'  <IS_UPDATABLE>Y</IS_UPDATABLE>
    ...
    <ATTRIBUTE>
      ...
      <ENTITY_NAME>LDC_IMCODIAM</ENTITY_NAM' ||
'E>
      <PARENT_CONF_ID>-999999998</PARENT_CONF_ID>
      <TYPE_ATTRIB_ID>2</TYPE_ATTRIB_ID>
      <ENTITY_TYPE_ID>3339</ENTITY_TYPE_ID>
...
'ERNAL_ROOT_ID>
    ...
    <ATTRIBUTE>
      ...
      <DISPLAY_NAME>Dimension Rotura</DISP' ||
'LAY_NAME>
      <INIT_BASIC_EXP_ID/>
      <VALID_BASIC_EXP_ID/>
      <INIT_EXPRESSION_ID/>
      ...
      <SELECT_STATEMENT_ID>120018065</SELECT_STATEMENT_ID>
      <TAG_NAME/>
      <PROCESS_SEQUENCE/>
      <STATEMENT_CACHE>N</STATEMENT_CACHE>
      <USER_DEFAULT_ALLOWED>N</USER_DEFAULT_ALLOWED>
    ...
    <ATTRIBUTE>
      ...
      <ENTITY_ID>8353</ENTITY_ID>
      <ENTITY_NAME>LDC_IMCODIAM</ENTIT' ||
'Y_NAME>
      <PARENT_CONF_ID>-999999998</PARENT_CONF_ID>
      <TYPE_ATTRIB_ID>2</TYPE_ATTRIB_ID>
      <ENTITY_TYPE_ID>3339</ENTITY_TYPE_ID>
      ...
      <SELECT_STATEMENT_ID>120018066</SELECT_STATEMENT_ID>
      <TAG_NAME/>
      <PROCESS_SEQUENCE/>
  ' ||
'    <STATEMENT_CACHE>N</STATEMENT_CACHE>
...
'    <TYPE_ATTRIB_ID>2</TYPE_ATTRIB_ID>
      ...
      <SELECT_STATEMENT_ID>120018067</SELECT_STATEMENT_ID>
      <TAG_NAME/>
      <PROCESS_SEQUENCE/>
      <STATEMENT_CACHE>N</STATEMENT_CACHE>
      <USER_DEFAULT_ALLOWED>N</USER_DEFAULT_ALLOWED>
...
';
OCRT_COPIA_.clColumn_0 := clTemp0 || clTemp1
;
...
VALUES (OCRT_COPIA_.tb2_0(0),
XMLType(OCRT_COPIA_.clColumn_0),
...
,
null,
'd49c5f4f36f404c6410e940c24f7edf9'
,
3);
...
if (not OCRT_COPIA_.blProcessStatus) then
 ...
 ut_trace.trace('Borrando tabla GR_CONFIG_EXPRESSION['||'121037765'||']',1);
delete from GR_CONFIG_EXPRESSION where 
  OBJECT_NAME = 'FWFW_PROCESS_CT54E121037765';
...
to_date('31-05-2013 08:26:27','DD-MM-YYYY HH24:MI:SS'), to_date('31-05-2013 08:26:27','DD-MM-YYYY HH24:MI:SS'), 'G', 
'N', 'PU', null, 
'DS', 'VAL - INI - CODIGO ORDEN', 'PP', 
'CREATE OR REPLACE PROCEDURE FWFW_PROCESS_CT54E121037765(errorNumber OUT NUMBER, errorMessage OUT VARCHAR2)
IS
...
if (not OCRT_COPIA_.blProcessStatus) then
 return;
  ...
  BEGIN
   delete from GE_STATEMENT where STATEMENT_ID = 120018050 and MODULE_ID = 5 and NAME = 'Producto subscriptor';
  EXCEPTION
  when ex.RECORD_HAVE_CHILDREN then
    ut_trace.trace('No se pudo borrar la sentencia: [120018050]',1);
   null;
  END;
...
if (not OCRT_COPIA_.blProcessStatus) then
 return;
  ...
  ut_trace.trace('Borrando tabla GE_STATEMENT',1);
  BEGIN
   delete from GE_STATEMENT where STATEMENT_ID = 120018051 and MODULE_ID = 16 and NAME = 'Direción imagen Colombia';
  EXCEPTION
  when ex.RECORD_HAVE_CHILDREN then
    ut_trace.trace('No se pudo borrar la sentencia: [120018051]',1);
   null;
  END;
ut_trace.trace('Obtiene nueva secuencia de GE_STATEMENT',1);
nuNewStatement := GE_BOSEQUENCE.NEXTGE_STATEMENT();
...
if (not OCRT_COPIA_.blProcessStatus) then
 return;
  ...
  ut_trace.trace('Borrando tabla GE_STATEMENT',1);
  BEGIN
   delete from GE_STATEMENT where STATEMENT_ID = 120018052 and MODULE_ID = 16 and NAME = 'Controlado ';
  EXCEPTION
  when ex.RECORD_HAVE_CHILDREN then
    ut_trace.trace('No se pudo borrar la sentencia: [120018052]',1);
   null;
  END;
ut_trace.trace('Obtiene nueva secuencia de GE_STATEMENT',1);
nuNewStatement := GE_BOSEQUENCE.NEXTGE_STATEMENT();
...
if (not OCRT_COPIA_.blProcessStatus) then
 return;
  ...
  ut_trace.trace('Borrando tabla GE_STATEMENT',1);
  BEGIN
   delete from GE_STATEMENT where STATEMENT_ID = 120018053 and MODULE_ID = 16 and NAME = 'Sector';
  EXCEPTION
  when ex.RECORD_HAVE_CHILDREN then
    ut_trace.trace('No se pudo borrar la sentencia: [120018053]',1);
   null;
  END;
ut_trace.trace('Obtiene nueva secuencia de GE_STATEMENT',1);
nuNewStatement := GE_BOSEQUENCE.NEXTGE_STATEMENT();
...
if (not OCRT_COPIA_.blProcessStatus) then
 return;
  ...
  ut_trace.trace('Borrando tabla GE_STATEMENT',1);
  BEGIN
   delete from GE_STATEMENT where STATEMENT_ID = 120018054 and MODULE_ID = 16 and NAME = 'Tipo_trabajo_IMC';
  EXCEPTION
  when ex.RECORD_HAVE_CHILDREN then
    ut_trace.trace('No se pudo borrar la sentencia: [120018054]',1);
   null;
  END;
ut_trace.trace('Obtiene nueva secuencia de GE_STATEMENT',1);
nuNewStatement := GE_BOSEQUENCE.NEXTGE_STATEMENT();
...
if (not OCRT_COPIA_.blProcessStatus) then
 return;
  ...
  ut_trace.trace('Borrando tabla GE_STATEMENT',1);
  BEGIN
   delete from GE_STATEMENT where STATEMENT_ID = 120018055 and MODULE_ID = 16 and NAME = 'Tipo_trabajo_IMC';
  EXCEPTION
  when ex.RECORD_HAVE_CHILDREN then
    ut_trace.trace('No se pudo borrar la sentencia: [120018055]',1);
   null;
  END;
ut_trace.trace('Obtiene nueva secuencia de GE_STATEMENT',1);
nuNewStatement := GE_BOSEQUENCE.NEXTGE_STATEMENT();
...
if (not OCRT_COPIA_.blProcessStatus) then
 return;
  ...
  ut_trace.trace('Borrando tabla GE_STATEMENT',1);
  BEGIN
   delete from GE_STATEMENT where STATEMENT_ID = 120018056 and MODULE_ID = 16 and NAME = 'Unidad_operativa_IMC';
  EXCEPTION
  when ex.RECORD_HAVE_CHILDREN then
    ut_trace.trace('No se pudo borrar la sentencia: [120018056]',1);
   null;
  END;
ut_trace.trace('Obtiene nueva secuencia de GE_STATEMENT',1);
nuNewStatement := GE_BOSEQUENCE.NEXTGE_STATEMENT();
...
if (not OCRT_COPIA_.blProcessStatus) then
 return;
  ...
  ut_trace.trace('Borrando tabla GE_STATEMENT',1);
  BEGIN
   delete from GE_STATEMENT where STATEMENT_ID = 120018057 and MODULE_ID = 16 and NAME = 'TIPOS DE EVENTOS';
  EXCEPTION
  when ex.RECORD_HAVE_CHILDREN then
    ut_trace.trace('No se pudo borrar la sentencia: [120018057]',1);
   null;
  END;
ut_trace.trace('Obtiene nueva secuencia de GE_STATEMENT',1);
nuNewStatement := GE_BOSEQUENCE.NEXTGE_STATEMENT();
...
if (not OCRT_COPIA_.blProcessStatus) then
 return;
  ...
  ut_trace.trace('Borrando tabla GE_STATEMENT',1);
  BEGIN
   delete from GE_STATEMENT where STATEMENT_ID = 120018058 and MODULE_ID = 16 and NAME = 'Controlado ';
  EXCEPTION
  when ex.RECORD_HAVE_CHILDREN then
    ut_trace.trace('No se pudo borrar la sentencia: [120018058]',1);
   null;
  END;
ut_trace.trace('Obtiene nueva secuencia de GE_STATEMENT',1);
nuNewStatement := GE_BOSEQUENCE.NEXTGE_STATEMENT();
...
if (not OCRT_COPIA_.blProcessStatus) then
 return;
  ...
  ut_trace.trace('Borrando tabla GE_STATEMENT',1);
  BEGIN
   delete from GE_STATEMENT where STATEMENT_ID = 120018059 and MODULE_ID = 16 and NAME = 'Suspension del Servicio';
  EXCEPTION
  when ex.RECORD_HAVE_CHILDREN then
    ut_trace.trace('No se pudo borrar la sentencia: [120018059]',1);
   null;
  END;
ut_trace.trace('Obtiene nueva secuencia de GE_STATEMENT',1);
nuNewStatement := GE_BOSEQUENCE.NEXTGE_STATEMENT();
...
if (not OCRT_COPIA_.blProcessStatus) then
 return;
  ...
  ut_trace.trace('Borrando tabla GE_STATEMENT',1);
  BEGIN
   delete from GE_STATEMENT where STATEMENT_ID = 120018060 and MODULE_ID = 16 and NAME = 'REPORTADO POR IMC';
  EXCEPTION
  when ex.RECORD_HAVE_CHILDREN then
    ut_trace.trace('No se pudo borrar la sentencia: [120018060]',1);
   null;
  END;
ut_trace.trace('Obtiene nueva secuencia de GE_STATEMENT',1);
nuNewStatement := GE_BOSEQUENCE.NEXTGE_STATEMENT();
...
if (not OCRT_COPIA_.blProcessStatus) then
 return;
  ...
  ut_trace.trace('Borrando tabla GE_STATEMENT',1);
  BEGIN
   delete from GE_STATEMENT where STATEMENT_ID = 120018061 and MODULE_ID = 16 and NAME = 'localizacion_red';
  EXCEPTION
  when ex.RECORD_HAVE_CHILDREN then
    ut_trace.trace('No se pudo borrar la sentencia: [120018061]',1);
   null;
  END;
ut_trace.trace('Obtiene nueva secuencia de GE_STATEMENT',1);
nuNewStatement := GE_BOSEQUENCE.NEXTGE_STATEMENT();
...
if (not OCRT_COPIA_.blProcessStatus) then
 return;
  ...
  ut_trace.trace('Borrando tabla GE_STATEMENT',1);
  BEGIN
   delete from GE_STATEMENT where STATEMENT_ID = 120018062 and MODULE_ID = 16 and NAME = 'ElementosIMC';
  EXCEPTION
  when ex.RECORD_HAVE_CHILDREN then
    ut_trace.trace('No se pudo borrar la sentencia: [120018062]',1);
   null;
  END;
ut_trace.trace('Obtiene nueva secuencia de GE_STATEMENT',1);
nuNewStatement := GE_BOSEQUENCE.NEXTGE_STATEMENT();
...
if (not OCRT_COPIA_.blProcessStatus) then
 return;
  ...
  ut_trace.trace('Borrando tabla GE_STATEMENT',1);
  BEGIN
   delete from GE_STATEMENT where STATEMENT_ID = 120018063 and MODULE_ID = 16 and NAME = 'SubElementoIMG';
  EXCEPTION
  when ex.RECORD_HAVE_CHILDREN then
    ut_trace.trace('No se pudo borrar la sentencia: [120018063]',1);
   null;
  END;
ut_trace.trace('Obtiene nueva secuencia de GE_STATEMENT',1);
nuNewStatement := GE_BOSEQUENCE.NEXTGE_STATEMENT();
...
if (not OCRT_COPIA_.blProcessStatus) then
 return;
  ...
  ut_trace.trace('Borrando tabla GE_STATEMENT',1);
  BEGIN
   delete from GE_STATEMENT where STATEMENT_ID = 120018064 and MODULE_ID = 16 and NAME = 'Marcas Elementos IMC';
  EXCEPTION
  when ex.RECORD_HAVE_CHILDREN then
    ut_trace.trace('No se pudo borrar la sentencia: [120018064]',1);
   null;
  END;
ut_trace.trace('Obtiene nueva secuencia de GE_STATEMENT',1);
nuNewStatement := GE_BOSEQUENCE.NEXTGE_STATEMENT();
...
if (not OCRT_COPIA_.blProcessStatus) then
 return;
  ...
  ut_trace.trace('Borrando tabla GE_STATEMENT',1);
  BEGIN
   delete from GE_STATEMENT where STATEMENT_ID = 120018065 and MODULE_ID = 16 and NAME = 'DimensionRotura';
  EXCEPTION
  when ex.RECORD_HAVE_CHILDREN then
    ut_trace.trace('No se pudo borrar la sentencia: [120018065]',1);
   null;
  END;
ut_trace.trace('Obtiene nueva secuencia de GE_STATEMENT',1);
nuNewStatement := GE_BOSEQUENCE.NEXTGE_STATEMENT();
...
if (not OCRT_COPIA_.blProcessStatus) then
 return;
  ...
  ut_trace.trace('Borrando tabla GE_STATEMENT',1);
  BEGIN
   delete from GE_STATEMENT where STATEMENT_ID = 120018066 and MODULE_ID = 16 and NAME = 'NivelPresionIMC';
  EXCEPTION
  when ex.RECORD_HAVE_CHILDREN then
    ut_trace.trace('No se pudo borrar la sentencia: [120018066]',1);
   null;
  END;
ut_trace.trace('Obtiene nueva secuencia de GE_STATEMENT',1);
nuNewStatement := GE_BOSEQUENCE.NEXTGE_STATEMENT();
...
if (not OCRT_COPIA_.blProcessStatus) then
 return;
  ...
  ut_trace.trace('Borrando tabla GE_STATEMENT',1);
  BEGIN
   delete from GE_STATEMENT where STATEMENT_ID = 120018067 and MODULE_ID = 16 and NAME = 'CAUSAS IMG';
  EXCEPTION
  when ex.RECORD_HAVE_CHILDREN then
    ut_trace.trace('No se pudo borrar la sentencia: [120018067]',1);
   null;
  END;
ut_trace.trace('Obtiene nueva secuencia de GE_STATEMENT',1);
nuNewStatement := GE_BOSEQUENCE.NEXTGE_STATEMENT();
...
BEGIN
 ...
 nuIndexInternal := OCRT_COPIA_.tb0_0.first;
 while nuIndexInternal IS NOT null loop
  IF (OCRT_COPIA_.tb0_0(nuIndexInternal)=isbExecutableName ) THEN
   IF (OCRT_COPIA_.tb0_1.exists(nuIndexInternal)) then
    tbExecutableEquivalence(isbExecutableName) := OCRT_COPIA_.tb0_1(nuIndexInternal);
   END IF;
   exit;
  END IF;
  nuIndexInternal := OCRT_COPIA_.tb0_0.next(nuIndexInternal);
 END loop; 
 return tbExecutableEquivalence(isbExecutableName);
END;
BEGIN
...
nuIndex :=  OCRT_COPIA_.tbRoleExecutables.first;
while nuIndex IS NOT null loop
 if (OCRT_COPIA_.tbRoleExecutables(nuIndex).role_id!=1) then
  nuNewExecutableId := fnuGetNewExecutableId(OCRT_COPIA_.tbRoleExecutables(nuIndex).name);
  IF nuNewExecutableId is not null then
    INSERT INTO sa_role_executables (role_id, executable_id)
    VALUES (OCRT_COPIA_.tbRoleExecutables(nuIndex).role_id, nuNewExecutableId);
  END IF;
  end if;
 nuIndex := OCRT_COPIA_.tbRoleExecutables.next(nuIndex);
END loop;
nuIndex :=  OCRT_COPIA_.tbUserException.first;
while nuIndex IS NOT null loop
 nuNewExecutableId := fnuGetNewExecutableId(OCRT_COPIA_.tbUserException(nuIndex).name);
 IF nuNewExecutableId is not null then
  insert INTO sa_user_exceptions (executable_id, user_id, status, usr_exc_type_id, comment_)
  VALUES (nuNewExecutableId ,OCRT_COPIA_.tbUserException(nuIndex).user_id, OCRT_COPIA_.tbUserException(nuIndex).status , OCRT_COPIA_.tbUserException(nuIndex).usr_exc_type_id, OCRT_COPIA_.tbUserException(nuIndex).comment_);
 END IF;
 nuIndex := OCRT_COPIA_.tbUserException.next(nuIndex);
END loop;
nuIndex :=  OCRT_COPIA_.tbExecEntities.first;
while nuIndex IS NOT null loop
 nuNewExecutableId := fnuGetNewExecutableId(OCRT_COPIA_.tbExecEntities(nuIndex).name);
 IF nuNewExecutableId is not null then
  select count(1) into nuRecCount
 from sa_exec_entities
  where executable_id = nuNewExecutableId and entity_id = OCRT_COPIA_.tbExecEntities(nuIndex).entity_id;
  IF nuRecCount = 0 then
    insert INTO sa_exec_entities (executable_id, entity_id )
    VALUES (nuNewExecutableId ,OCRT_COPIA_.tbExecEntities(nuIndex).entity_id );
  END IF;
 END IF;
 nuIndex := OCRT_COPIA_.tbExecEntities.next(nuIndex);
END loop;
...
BEGIN
 ...
 nuIndex := OCRT_COPIA_.tbObjectToDelete.first;
 while (nuIndex is not null) LOOP
  BEGIN
   ut_trace.trace('Objeto a borrar: ' || OCRT_COPIA_.tbObjectToDelete(nuIndex),1);
   IF(ut_object.fboExistObject(OCRT_COPIA_.tbObjectToDelete(nuIndex)) and not fblIsObjectUseByRule(OCRT_COPIA_.tbObjectToDelete(nuIndex))) THEN
    blObjectDeleted := ut_object.fboDeleteObject(OCRT_COPIA_.tbObjectToDelete(nuIndex));
    ut_trace.trace('  * Borrado ' || OCRT_COPIA_.tbObjectToDelete(nuIndex), 1);
   END IF;
  EXCEPTION when others then
   ut_trace.trace('- No se borra el objeto (' || OCRT_COPIA_.tbObjectToDelete(nuIndex) || '): ' || sqlerrm,1);
  END;
  nuIndex := OCRT_COPIA_.tbObjectToDelete.next(nuIndex);
 END loop;
...
begin
SA_BOCreatePackages.DropPackage('OCRT_COPIA_');
end;
/

BEGIN ut_trace.trace('********************FIN  proceso de objeto:OCRT_COPIA_******************************'); end;
/
