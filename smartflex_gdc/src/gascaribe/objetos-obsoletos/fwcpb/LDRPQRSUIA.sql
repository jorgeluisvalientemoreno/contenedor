...
BEGIN
sa_bocreatePackages.CreatePackage('LDRPQRSUIA_',
'CREATE OR REPLACE PACKAGE LDRPQRSUIA_ IS ' || chr(10) ||
...
'tb3_0 ty3_0;  executableName ge_catalog.tag_name%type := ''LDRPQRSUIA''; ' || chr(10) ||
...
'( ' || chr(10) ||
'SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME=''LDRPQRSUIA'' ' || chr(10) ||
'); ' || chr(10) ||
...
BEGIN ut_trace.trace('********************Comenzar proceso de objeto:LDRPQRSUIA_******************************'); END;
/
...
if (not LDRPQRSUIA_.blProcessStatus) then
 return;
end if;
Open LDRPQRSUIA_.cuRoleExecutables;
loop
 fetch LDRPQRSUIA_.cuRoleExecutables INTO LDRPQRSUIA_.rcRoleExecutables;
 exit when  LDRPQRSUIA_.cuRoleExecutables%notfound;
 LDRPQRSUIA_.tbRoleExecutables(nuIndex) := LDRPQRSUIA_.rcRoleExecutables;
 nuIndex := nuIndex + 1;
END loop;
close LDRPQRSUIA_.cuRoleExecutables;
nuIndex := 0;
Open LDRPQRSUIA_.cuUserExceptions ;
loop
 fetch LDRPQRSUIA_.cuUserExceptions INTO  LDRPQRSUIA_.rcUserExceptions;
 exit when LDRPQRSUIA_.cuUserExceptions%notfound;
 LDRPQRSUIA_.tbUserException(nuIndex):=LDRPQRSUIA_.rcUserExceptions;
 nuIndex := nuIndex + 1;
END loop;
close LDRPQRSUIA_.cuUserExceptions;
nuIndex := 0;
Open LDRPQRSUIA_.cuExecEntities ;
loop
 fetch LDRPQRSUIA_.cuExecEntities INTO  LDRPQRSUIA_.rcExecEntities;
 exit when LDRPQRSUIA_.cuExecEntities%notfound;
 LDRPQRSUIA_.tbExecEntities(nuIndex):=LDRPQRSUIA_.rcExecEntities;
 nuIndex := nuIndex + 1;
END loop;
close LDRPQRSUIA_.cuExecEntities;

exception when others then
LDRPQRSUIA_.blProcessStatus := false;
rollback;
...
(
 SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDRPQRSUIA'
);
    ...
    BEGIN
      ...
      IF (nuNumRecords > 0) THEN
        ...
        IF ( sbObjectName IS NOT null ) THEN
          ...
          if ( LDRPQRSUIA_.tbObjectToDelete.last IS null ) then
            nuNumObjects := 1;
          else
            nuNumObjects := LDRPQRSUIA_.tbObjectToDelete.last + 1;
          end if;
          LDRPQRSUIA_.tbObjectToDelete(nuNumObjects) := sbObjectName;
        END IF;
...
if (not LDRPQRSUIA_.blProcessStatus) then
 return;
  ...
  WHERE GE_DISTRIBUTION_FILE.DISTRIBUTION_FILE_ID = 'LDRPQRSUIA';
  IF (nuNumRecords > 0) THEN
...
WHERE   sa_executable.name = 'LDRPQRSUIA'
...
if (not LDRPQRSUIA_.blProcessStatus) then
 ...
 FOR crObjectsName IN cuObjectsName LOOP
  LDRPQRSUIA_.tbObjectToDelete(nuIndex) := crObjectsName.OBJECT_NAME;
  nuIndex := nuIndex + 1;
 END LOOP;
EXCEPTION 
...
if (not LDRPQRSUIA_.blProcessStatus) then
 return;
  ...
  DELETE FROM SA_ROLE_EXECUTABLES WHERE (EXECUTABLE_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDRPQRSUIA') AND ROLE_ID=1;
...
if (not LDRPQRSUIA_.blProcessStatus) then
 return;
  ...
  DELETE FROM SA_USER_EXCEPTIONS WHERE (EXECUTABLE_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDRPQRSUIA') AND 1=2;
...
SELECT rowid
FROM SA_EXECUTABLE_SYNON WHERE (SYNONYMOUS_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDRPQRSUIA');
nuIndex binary_integer;
BEGIN
...
if (not LDRPQRSUIA_.blProcessStatus) then
 return;
  ...
  DELETE FROM SA_EXEC_ENTITIES WHERE (EXECUTABLE_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDRPQRSUIA');
...
if (not LDRPQRSUIA_.blProcessStatus) then
 return;
  ...
  DELETE FROM SA_MENU_OPTION WHERE (EXECUTABLE_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDRPQRSUIA');
...
if (not LDRPQRSUIA_.blProcessStatus) then
 return;
  ...
  DELETE FROM GI_CONFIG_COMP WHERE (CONFIG_ID) in (SELECT CONFIG_ID FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDRPQRSUIA') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339);
...
if (not LDRPQRSUIA_.blProcessStatus) then
 return;
  ...
  ut_trace.trace('Borrando tabla GI_CONFIG_COMP',1);
  DELETE FROM GI_CONFIG_COMP WHERE (COMPOSITION_ID) in (SELECT COMPOSITION_ID FROM GI_COMPOSITION WHERE (CONFIG_ID) in (SELECT CONFIG_ID FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDRPQRSUIA') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339));
...
DECLARE 
 ...
 SELECT rowid FROM GE_STATEMENT WHERE (STATEMENT_ID) in (SELECT SELECT_STATEMENT_ID FROM GI_COMP_ATTRIBS WHERE (COMPOSITION_ID) in (SELECT COMPOSITION_ID FROM GI_COMPOSITION WHERE (CONFIG_ID) in (SELECT CONFIG_ID FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDRPQRSUIA') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339)));
BEGIN 
...
LDRPQRSUIA_.tbGE_STATEMENTRowId(rcData.rowid):=rcData.rowid; 
END loop; 
...
for rcData in cuLoadTemporaryTable loop
LDRPQRSUIA_.tbGR_CONFIG_EXPRESSIONRowId(rcData.rowid):=rcData.rowid; 
END loop; 
...
if (not LDRPQRSUIA_.blProcessStatus) then
 return;
  ...
  DELETE FROM GI_COMP_ATTRIBS WHERE (COMPOSITION_ID) in (SELECT COMPOSITION_ID FROM GI_COMPOSITION WHERE (CONFIG_ID) in (SELECT CONFIG_ID FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDRPQRSUIA') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339));
...
nuVarcharIndex:=LDRPQRSUIA_.tbGE_STATEMENTRowId.first;  
while (nuVarcharIndex is not null) loop 
begin
ut_trace.trace('Row id a borrar:'||nuVarcharIndex,1);
delete from GE_STATEMENT where rowid = LDRPQRSUIA_.tbGE_STATEMENTRowId(nuVarcharIndex); 
...
LDRPQRSUIA_.tbGE_STATEMENTRowId.delete(nuOldVarcharIndex);
end if;
nuVarcharIndex := LDRPQRSUIA_.tbGE_STATEMENTRowId.next(nuVarcharIndex); 
END loop; 
...
nuVarcharIndex:=LDRPQRSUIA_.tbGR_CONFIG_EXPRESSIONRowId.first;  
while (nuVarcharIndex is not null) loop 
ut_trace.trace('Row id a borrar:'||nuVarcharIndex,1);
delete from GR_CONFIG_EXPRESSION where rowid = LDRPQRSUIA_.tbGR_CONFIG_EXPRESSIONRowId(nuVarcharIndex); 
nuOldVarcharIndex:=nuVarcharIndex;
nuVarcharIndex := LDRPQRSUIA_.tbGR_CONFIG_EXPRESSIONRowId.next(nuVarcharIndex); 
LDRPQRSUIA_.tbGR_CONFIG_EXPRESSIONRowId.delete(nuOldVarcharIndex); 
END loop; 
...
for rcData in cuLoadTemporaryTable loop
LDRPQRSUIA_.tbGI_COMPOSITIONRowId(rcData.rowid):=rcData.rowid; 
END loop; 
...
if (not LDRPQRSUIA_.blProcessStatus) then
 return;
  ...
  DELETE FROM GI_FRAME WHERE (COMPOSITION_ID) in (SELECT COMPOSITION_ID FROM GI_COMPOSITION WHERE (CONFIG_ID) in (SELECT CONFIG_ID FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDRPQRSUIA') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339));
...
nuVarcharIndex:=LDRPQRSUIA_.tbGI_COMPOSITIONRowId.first;  
while (nuVarcharIndex is not null) loop 
ut_trace.trace('Row id a borrar:'||nuVarcharIndex,1);
delete from GI_COMPOSITION where rowid = LDRPQRSUIA_.tbGI_COMPOSITIONRowId(nuVarcharIndex); 
nuOldVarcharIndex:=nuVarcharIndex;
nuVarcharIndex := LDRPQRSUIA_.tbGI_COMPOSITIONRowId.next(nuVarcharIndex); 
LDRPQRSUIA_.tbGI_COMPOSITIONRowId.delete(nuOldVarcharIndex); 
END loop; 
...
if (not LDRPQRSUIA_.blProcessStatus) then
 return;
  ...
  DELETE FROM GI_COMPOSITION WHERE (CONFIG_ID) in (SELECT CONFIG_ID FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDRPQRSUIA') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339);
...
if (not LDRPQRSUIA_.blProcessStatus) then
 return;
  ...
  DELETE FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDRPQRSUIA') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339;
...
if (not LDRPQRSUIA_.blProcessStatus) then
 return;
  ...
  DELETE FROM GE_DISTRIBUTION_FILE WHERE (DISTRIBUTION_FILE_ID) in (SELECT NAME FROM SA_EXECUTABLE WHERE NAME='LDRPQRSUIA');
...
if (not LDRPQRSUIA_.blProcessStatus) then
 return;
  ...
  DELETE FROM SA_EXECUTABLE WHERE NAME='LDRPQRSUIA';
...
LDRPQRSUIA_.old_tb0_0(0):='LDRPQRSUIA'
;
LDRPQRSUIA_.tb0_0(0):=UPPER(LDRPQRSUIA_.old_tb0_0(0));
LDRPQRSUIA_.tb0_0(0):=LDRPQRSUIA_.tb0_0(0);
LDRPQRSUIA_.old_tb0_1(0):=500000000001907;
LDRPQRSUIA_.tb0_1(0):=GE_BOCATALOG.FNUGETIDSEQFROMCATALOG(LDRPQRSUIA_.tb0_0(0), 'EXECUTABLE', 'SA_BOEXECUTABLE.GETNEXTID');
LDRPQRSUIA_.tb0_1(0):=LDRPQRSUIA_.tb0_1(0);
ut_trace.trace('insertando tabla: SA_EXECUTABLE fila (0)',1);
INSERT INTO SA_EXECUTABLE(NAME,EXECUTABLE_ID,DESCRIPTION,PATH,VERSION,EXECUTABLE_TYPE_ID,EXEC_OPER_TYPE_ID,MODULE_ID,SUBSYSTEM_ID,PARENT_EXECUTABLE_ID,LAST_RECORD_ALLOWED,PATH_FILE_HELP,WITHOUT_RESTR_POLICY,DIRECT_EXECUTION,TIMES_EXECUTED,EXEC_OWNER,LAST_DATE_EXECUTED,CLASS_ID) 
VALUES (LDRPQRSUIA_.tb0_0(0),
LDRPQRSUIA_.tb0_1(0),
'Reporte PQR SUI Anexo A'
,
null,
'23'
,
...
99,
null,
to_date('03-08-2016 16:50:33','DD-MM-YYYY HH24:MI:SS'),
null);
...
LDRPQRSUIA_.tb1_0(0):=1;
LDRPQRSUIA_.tb1_1(0):=LDRPQRSUIA_.tb0_1(0);
ut_trace.trace('insertando tabla: SA_ROLE_EXECUTABLES fila (0)',1);
INSERT INTO SA_ROLE_EXECUTABLES(ROLE_ID,EXECUTABLE_ID) 
VALUES (LDRPQRSUIA_.tb1_0(0),
LDRPQRSUIA_.tb1_1(0));
...
LDRPQRSUIA_.old_tb2_0(0):=40007329;
LDRPQRSUIA_.tb2_0(0):=SA_BOSEQUENCES.FNUNEXTSA_MENU_OPTION;
LDRPQRSUIA_.tb2_0(0):=LDRPQRSUIA_.tb2_0(0);
LDRPQRSUIA_.tb2_1(0):=LDRPQRSUIA_.tb0_1(0);
...
LDRPQRSUIA_.tb3_0(0):=LDRPQRSUIA_.tb0_0(0);
LDRPQRSUIA_.clColumn_0 := '<?xml version="1.0" encoding="UTF-8"?>
<PB xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema">
  <APPLICATION>
    <EXECUTABLE_ID>500000000001907</EXECUTABLE_ID>
    <NAME>LDRPQRSUIA</NAME>
    <DESCRIPTION>Reporte PQR SUI Anexo A</DESCRIPTION>
    <VERSION>23</VERSION>
    <MODULE>61</MODULE>
    <DIRECT_EXECUTION>Y</DIRECT_EXECUTION>
    <PATH_FILE_HELP/>
    <OBJECT_NAME>LDC_REPLEYVENTAATECLIE.FORMATO_A_RECLAMACIONES</OBJECT_NAME>
    <QUERY_NAME/>
  ...
  <COMPOSITION>
    ...
    <EXTERNAL_TYPE_ID>500000000001907</EXTERNAL_TYPE_ID>
    <ENTITY_ID>9623</ENTITY_ID>
    <EXTERNAL_ID>500000000001907</EXTERNAL_ID>
    <FRAME_ID>96304</FRAME_ID' ||
'>
...
'ION>4</DATA_PRECISION>
    ...
    <ATTRIBUTE>
      ...
      <ATTRIBUTE_NAME>NUMBER_OF_PROD</ATTRIBUTE_NA' ||
'ME>
      <ENTITY_ID>17</ENTITY_ID>
      <ENTITY_NAME>MO_PACKAGES</ENTITY_NAME>
      <PARENT_CONF_ID>-999999998</PARENT_CONF_ID>
      ...
      <DISPLAY_NAME>Mes de generaci¿n</DISPLAY_NAME>
      <INIT_BASIC_EXP_ID/>
      <VALID_BASIC_EXP_ID/>
      <INIT_EXPRESSION_ID/>
      <VALID_EXPRESSION_ID/>
...
union
select 3 ID, '||chr(38)||'apos;Marzo'||chr(38)||'apos; descripti' ||
'on from dual
union
...
select 12 ID, '||chr(38)||'apos;Diciembre'||chr(38)||'apos; description from dual</SELECT_STATEMENT>
      <SELECT_STATEMENT_ID>120038145</SELECT_STATEMENT_ID>
      <TAG_NAME/>
      <PROCESS_SEQUENCE/>
      <STATEMENT_CACHE>N</STATEMENT_CACHE>
      <USER_DEFAULT_ALLOWED>N</USER_DEFAULT_ALLOWED>
    ...
    <ATTRIBUTE>
      <ENTIT' ||
'Y_ATTRIBUTE_ID>3146</ENTITY_ATTRIBUTE_ID>
      ...
      <VALID_EXPRESSION_ID/>
      <DEFAULT_VALUE/>
      <SELECT_STATEMENT>select director' ||
'y_id id, path description
from  GE_DIRECTORY
</SELECT_STATEMENT>
      <SELECT_STATEMENT_ID>120038146</SELECT_STATEMENT_ID>
      <TAG_NAME/>
      <PROCESS_SEQUENCE/>
      <STATEMENT_CACHE>N</STATEMENT_CACHE>
      <USER_DEFAULT_ALLOWED>N</USER_DEFAULT_ALLOWED>
...
VALUES (LDRPQRSUIA_.tb3_0(0),
XMLType(LDRPQRSUIA_.clColumn_0),
'Reporte PQR SUI Anexo A'
,
'1.0.0.0'
,
...
,
null,
'09ae816ef4bc4fa097ae5e954f0d2c21'
,
3);
...
if (not LDRPQRSUIA_.blProcessStatus) then
 return;
  ...
  BEGIN
   delete from GE_STATEMENT where STATEMENT_ID = 120038145 and MODULE_ID = 61 and NAME = 'meses';
  EXCEPTION
  when ex.RECORD_HAVE_CHILDREN then
    ut_trace.trace('No se pudo borrar la sentencia: [120038145]',1);
   null;
  END;
...
if (not LDRPQRSUIA_.blProcessStatus) then
 return;
  ...
  ut_trace.trace('Borrando tabla GE_STATEMENT',1);
  BEGIN
   delete from GE_STATEMENT where STATEMENT_ID = 120038146 and MODULE_ID = 1 and NAME = 'Directorios';
  EXCEPTION
  when ex.RECORD_HAVE_CHILDREN then
    ut_trace.trace('No se pudo borrar la sentencia: [120038146]',1);
   null;
  END;
ut_trace.trace('Obtiene nueva secuencia de GE_STATEMENT',1);
nuNewStatement := GE_BOSEQUENCE.NEXTGE_STATEMENT();
...
BEGIN
 ...
 nuIndexInternal := LDRPQRSUIA_.tb0_0.first;
 while nuIndexInternal IS NOT null loop
  IF (LDRPQRSUIA_.tb0_0(nuIndexInternal)=isbExecutableName ) THEN
   IF (LDRPQRSUIA_.tb0_1.exists(nuIndexInternal)) then
    tbExecutableEquivalence(isbExecutableName) := LDRPQRSUIA_.tb0_1(nuIndexInternal);
   END IF;
   exit;
  END IF;
  nuIndexInternal := LDRPQRSUIA_.tb0_0.next(nuIndexInternal);
 END loop; 
 return tbExecutableEquivalence(isbExecutableName);
END;
BEGIN
...
nuIndex :=  LDRPQRSUIA_.tbRoleExecutables.first;
while nuIndex IS NOT null loop
 if (LDRPQRSUIA_.tbRoleExecutables(nuIndex).role_id!=1) then
  nuNewExecutableId := fnuGetNewExecutableId(LDRPQRSUIA_.tbRoleExecutables(nuIndex).name);
  IF nuNewExecutableId is not null then
    INSERT INTO sa_role_executables (role_id, executable_id)
    VALUES (LDRPQRSUIA_.tbRoleExecutables(nuIndex).role_id, nuNewExecutableId);
  END IF;
  end if;
 nuIndex := LDRPQRSUIA_.tbRoleExecutables.next(nuIndex);
END loop;
nuIndex :=  LDRPQRSUIA_.tbUserException.first;
while nuIndex IS NOT null loop
 nuNewExecutableId := fnuGetNewExecutableId(LDRPQRSUIA_.tbUserException(nuIndex).name);
 IF nuNewExecutableId is not null then
  insert INTO sa_user_exceptions (executable_id, user_id, status, usr_exc_type_id, comment_)
  VALUES (nuNewExecutableId ,LDRPQRSUIA_.tbUserException(nuIndex).user_id, LDRPQRSUIA_.tbUserException(nuIndex).status , LDRPQRSUIA_.tbUserException(nuIndex).usr_exc_type_id, LDRPQRSUIA_.tbUserException(nuIndex).comment_);
 END IF;
 nuIndex := LDRPQRSUIA_.tbUserException.next(nuIndex);
END loop;
nuIndex :=  LDRPQRSUIA_.tbExecEntities.first;
while nuIndex IS NOT null loop
 nuNewExecutableId := fnuGetNewExecutableId(LDRPQRSUIA_.tbExecEntities(nuIndex).name);
 IF nuNewExecutableId is not null then
  select count(1) into nuRecCount
 from sa_exec_entities
  where executable_id = nuNewExecutableId and entity_id = LDRPQRSUIA_.tbExecEntities(nuIndex).entity_id;
  IF nuRecCount = 0 then
    insert INTO sa_exec_entities (executable_id, entity_id )
    VALUES (nuNewExecutableId ,LDRPQRSUIA_.tbExecEntities(nuIndex).entity_id );
  END IF;
 END IF;
 nuIndex := LDRPQRSUIA_.tbExecEntities.next(nuIndex);
END loop;
...
BEGIN
 ...
 nuIndex := LDRPQRSUIA_.tbObjectToDelete.first;
 while (nuIndex is not null) LOOP
  BEGIN
   ut_trace.trace('Objeto a borrar: ' || LDRPQRSUIA_.tbObjectToDelete(nuIndex),1);
   IF(ut_object.fboExistObject(LDRPQRSUIA_.tbObjectToDelete(nuIndex)) and not fblIsObjectUseByRule(LDRPQRSUIA_.tbObjectToDelete(nuIndex))) THEN
    blObjectDeleted := ut_object.fboDeleteObject(LDRPQRSUIA_.tbObjectToDelete(nuIndex));
    ut_trace.trace('  * Borrado ' || LDRPQRSUIA_.tbObjectToDelete(nuIndex), 1);
   END IF;
  EXCEPTION when others then
   ut_trace.trace('- No se borra el objeto (' || LDRPQRSUIA_.tbObjectToDelete(nuIndex) || '): ' || sqlerrm,1);
  END;
  nuIndex := LDRPQRSUIA_.tbObjectToDelete.next(nuIndex);
 END loop;
...
begin
SA_BOCreatePackages.DropPackage('LDRPQRSUIA_');
end;
/

BEGIN ut_trace.trace('********************FIN  proceso de objeto:LDRPQRSUIA_******************************'); end;
/
