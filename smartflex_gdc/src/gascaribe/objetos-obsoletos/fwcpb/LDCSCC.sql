...
BEGIN
sa_bocreatePackages.CreatePackage('LDCSCC_',
'CREATE OR REPLACE PACKAGE LDCSCC_ IS ' || chr(10) ||
...
'tb2_0 ty2_0;  executableName ge_catalog.tag_name%type := ''LDCSCC''; ' || chr(10) ||
...
'( ' || chr(10) ||
'SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME=''LDCSCC'' ' || chr(10) ||
'); ' || chr(10) ||
...
BEGIN ut_trace.trace('********************Comenzar proceso de objeto:LDCSCC_******************************'); END;
/
...
if (not LDCSCC_.blProcessStatus) then
 return;
end if;
Open LDCSCC_.cuRoleExecutables;
loop
 fetch LDCSCC_.cuRoleExecutables INTO LDCSCC_.rcRoleExecutables;
 exit when  LDCSCC_.cuRoleExecutables%notfound;
 LDCSCC_.tbRoleExecutables(nuIndex) := LDCSCC_.rcRoleExecutables;
 nuIndex := nuIndex + 1;
END loop;
close LDCSCC_.cuRoleExecutables;
nuIndex := 0;
Open LDCSCC_.cuUserExceptions ;
loop
 fetch LDCSCC_.cuUserExceptions INTO  LDCSCC_.rcUserExceptions;
 exit when LDCSCC_.cuUserExceptions%notfound;
 LDCSCC_.tbUserException(nuIndex):=LDCSCC_.rcUserExceptions;
 nuIndex := nuIndex + 1;
END loop;
close LDCSCC_.cuUserExceptions;
nuIndex := 0;
Open LDCSCC_.cuExecEntities ;
loop
 fetch LDCSCC_.cuExecEntities INTO  LDCSCC_.rcExecEntities;
 exit when LDCSCC_.cuExecEntities%notfound;
 LDCSCC_.tbExecEntities(nuIndex):=LDCSCC_.rcExecEntities;
 nuIndex := nuIndex + 1;
END loop;
close LDCSCC_.cuExecEntities;

exception when others then
LDCSCC_.blProcessStatus := false;
rollback;
...
(
 SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDCSCC'
);
    ...
    BEGIN
      ...
      IF (nuNumRecords > 0) THEN
        ...
        IF ( sbObjectName IS NOT null ) THEN
          ...
          if ( LDCSCC_.tbObjectToDelete.last IS null ) then
            nuNumObjects := 1;
          else
            nuNumObjects := LDCSCC_.tbObjectToDelete.last + 1;
          end if;
          LDCSCC_.tbObjectToDelete(nuNumObjects) := sbObjectName;
        END IF;
...
if (not LDCSCC_.blProcessStatus) then
 return;
  ...
  WHERE GE_DISTRIBUTION_FILE.DISTRIBUTION_FILE_ID = 'LDCSCC';
  IF (nuNumRecords > 0) THEN
...
WHERE   sa_executable.name = 'LDCSCC'
...
if (not LDCSCC_.blProcessStatus) then
 ...
 FOR crObjectsName IN cuObjectsName LOOP
  LDCSCC_.tbObjectToDelete(nuIndex) := crObjectsName.OBJECT_NAME;
  nuIndex := nuIndex + 1;
 END LOOP;
EXCEPTION 
...
if (not LDCSCC_.blProcessStatus) then
 return;
  ...
  DELETE FROM SA_ROLE_EXECUTABLES WHERE (EXECUTABLE_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDCSCC') AND ROLE_ID=1;
...
if (not LDCSCC_.blProcessStatus) then
 return;
  ...
  DELETE FROM SA_USER_EXCEPTIONS WHERE (EXECUTABLE_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDCSCC') AND 1=2;
...
SELECT rowid
FROM SA_EXECUTABLE_SYNON WHERE (SYNONYMOUS_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDCSCC');
nuIndex binary_integer;
BEGIN
...
if (not LDCSCC_.blProcessStatus) then
 return;
  ...
  DELETE FROM SA_EXEC_ENTITIES WHERE (EXECUTABLE_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDCSCC');
...
if (not LDCSCC_.blProcessStatus) then
 return;
  ...
  DELETE FROM SA_MENU_OPTION WHERE (EXECUTABLE_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDCSCC');
...
if (not LDCSCC_.blProcessStatus) then
 return;
  ...
  DELETE FROM GI_CONFIG_COMP WHERE (CONFIG_ID) in (SELECT CONFIG_ID FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDCSCC') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339);
...
if (not LDCSCC_.blProcessStatus) then
 return;
  ...
  ut_trace.trace('Borrando tabla GI_CONFIG_COMP',1);
  DELETE FROM GI_CONFIG_COMP WHERE (COMPOSITION_ID) in (SELECT COMPOSITION_ID FROM GI_COMPOSITION WHERE (CONFIG_ID) in (SELECT CONFIG_ID FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDCSCC') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339));
...
DECLARE 
 ...
 SELECT rowid FROM GE_STATEMENT WHERE (STATEMENT_ID) in (SELECT SELECT_STATEMENT_ID FROM GI_COMP_ATTRIBS WHERE (COMPOSITION_ID) in (SELECT COMPOSITION_ID FROM GI_COMPOSITION WHERE (CONFIG_ID) in (SELECT CONFIG_ID FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDCSCC') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339)));
BEGIN 
...
LDCSCC_.tbGE_STATEMENTRowId(rcData.rowid):=rcData.rowid; 
END loop; 
...
for rcData in cuLoadTemporaryTable loop
LDCSCC_.tbGR_CONFIG_EXPRESSIONRowId(rcData.rowid):=rcData.rowid; 
END loop; 
...
if (not LDCSCC_.blProcessStatus) then
 return;
  ...
  DELETE FROM GI_COMP_ATTRIBS WHERE (COMPOSITION_ID) in (SELECT COMPOSITION_ID FROM GI_COMPOSITION WHERE (CONFIG_ID) in (SELECT CONFIG_ID FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDCSCC') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339));
...
nuVarcharIndex:=LDCSCC_.tbGE_STATEMENTRowId.first;  
while (nuVarcharIndex is not null) loop 
begin
ut_trace.trace('Row id a borrar:'||nuVarcharIndex,1);
delete from GE_STATEMENT where rowid = LDCSCC_.tbGE_STATEMENTRowId(nuVarcharIndex); 
...
LDCSCC_.tbGE_STATEMENTRowId.delete(nuOldVarcharIndex);
end if;
nuVarcharIndex := LDCSCC_.tbGE_STATEMENTRowId.next(nuVarcharIndex); 
END loop; 
...
nuVarcharIndex:=LDCSCC_.tbGR_CONFIG_EXPRESSIONRowId.first;  
while (nuVarcharIndex is not null) loop 
ut_trace.trace('Row id a borrar:'||nuVarcharIndex,1);
delete from GR_CONFIG_EXPRESSION where rowid = LDCSCC_.tbGR_CONFIG_EXPRESSIONRowId(nuVarcharIndex); 
nuOldVarcharIndex:=nuVarcharIndex;
nuVarcharIndex := LDCSCC_.tbGR_CONFIG_EXPRESSIONRowId.next(nuVarcharIndex); 
LDCSCC_.tbGR_CONFIG_EXPRESSIONRowId.delete(nuOldVarcharIndex); 
END loop; 
...
for rcData in cuLoadTemporaryTable loop
LDCSCC_.tbGI_COMPOSITIONRowId(rcData.rowid):=rcData.rowid; 
END loop; 
...
if (not LDCSCC_.blProcessStatus) then
 return;
  ...
  DELETE FROM GI_FRAME WHERE (COMPOSITION_ID) in (SELECT COMPOSITION_ID FROM GI_COMPOSITION WHERE (CONFIG_ID) in (SELECT CONFIG_ID FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDCSCC') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339));
...
nuVarcharIndex:=LDCSCC_.tbGI_COMPOSITIONRowId.first;  
while (nuVarcharIndex is not null) loop 
ut_trace.trace('Row id a borrar:'||nuVarcharIndex,1);
delete from GI_COMPOSITION where rowid = LDCSCC_.tbGI_COMPOSITIONRowId(nuVarcharIndex); 
nuOldVarcharIndex:=nuVarcharIndex;
nuVarcharIndex := LDCSCC_.tbGI_COMPOSITIONRowId.next(nuVarcharIndex); 
LDCSCC_.tbGI_COMPOSITIONRowId.delete(nuOldVarcharIndex); 
END loop; 
...
if (not LDCSCC_.blProcessStatus) then
 return;
  ...
  DELETE FROM GI_COMPOSITION WHERE (CONFIG_ID) in (SELECT CONFIG_ID FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDCSCC') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339);
...
if (not LDCSCC_.blProcessStatus) then
 return;
  ...
  DELETE FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDCSCC') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339;
...
if (not LDCSCC_.blProcessStatus) then
 return;
  ...
  DELETE FROM GE_DISTRIBUTION_FILE WHERE (DISTRIBUTION_FILE_ID) in (SELECT NAME FROM SA_EXECUTABLE WHERE NAME='LDCSCC');
...
if (not LDCSCC_.blProcessStatus) then
 return;
  ...
  DELETE FROM SA_EXECUTABLE WHERE NAME='LDCSCC';
...
LDCSCC_.old_tb0_0(0):='LDCSCC'
;
LDCSCC_.tb0_0(0):=UPPER(LDCSCC_.old_tb0_0(0));
LDCSCC_.tb0_0(0):=LDCSCC_.tb0_0(0);
LDCSCC_.old_tb0_1(0):=500000000010603;
LDCSCC_.tb0_1(0):=GE_BOCATALOG.FNUGETIDSEQFROMCATALOG(LDCSCC_.tb0_0(0), 'EXECUTABLE', 'SA_BOEXECUTABLE.GETNEXTID');
LDCSCC_.tb0_1(0):=LDCSCC_.tb0_1(0);
ut_trace.trace('insertando tabla: SA_EXECUTABLE fila (0)',1);
INSERT INTO SA_EXECUTABLE(NAME,EXECUTABLE_ID,DESCRIPTION,PATH,VERSION,EXECUTABLE_TYPE_ID,EXEC_OPER_TYPE_ID,MODULE_ID,SUBSYSTEM_ID,PARENT_EXECUTABLE_ID,LAST_RECORD_ALLOWED,PATH_FILE_HELP,WITHOUT_RESTR_POLICY,DIRECT_EXECUTION,TIMES_EXECUTED,EXEC_OWNER,LAST_DATE_EXECUTED,CLASS_ID) 
VALUES (LDCSCC_.tb0_0(0),
LDCSCC_.tb0_1(0),
'Forma de Seguimiento de Cartera Brilla a Cierre'
,
null,
'9'
,
...
4,
null,
to_date('29-09-2016 14:26:09','DD-MM-YYYY HH24:MI:SS'),
null);
...
LDCSCC_.tb1_0(0):=1;
LDCSCC_.tb1_1(0):=LDCSCC_.tb0_1(0);
ut_trace.trace('insertando tabla: SA_ROLE_EXECUTABLES fila (0)',1);
INSERT INTO SA_ROLE_EXECUTABLES(ROLE_ID,EXECUTABLE_ID) 
VALUES (LDCSCC_.tb1_0(0),
LDCSCC_.tb1_1(0));
...
LDCSCC_.tb2_0(0):=LDCSCC_.tb0_0(0);
LDCSCC_.clColumn_0 := '<?xml version="1.0" encoding="UTF-8"?>
<PB xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema">
  <APPLICATION>
    <EXECUTABLE_ID>500000000010603</EXECUTABLE_ID>
    <NAME>LDCSCC</NAME>
    <DESCRIPTION>Forma de Seguimiento de Cartera Brilla a Cierre</DESCRIPTION>
    <VERSION>9</VERSION>
    <MODULE>47</MODULE>
    <DIRECT_EXECUTION>Y</DIRECT_EXECUTION>
    <PATH_FILE_HELP/>
    <OBJECT_NAME>LDC_BOREPORTECARTBRILLA.PROVALIDALDCSCC</OBJECT_NAME>
    <QUERY_NAME/>
  ...
  <COMPOSITION>
    ...
    <EXTERNAL_TYPE_ID>500000000010603</EXTERNAL_TYPE_ID>
    <ENTITY_ID>9623</ENTITY_ID>
    <EXTERNAL_ID>500000000010603</EXTERNAL_ID>
    <FRAME_ID>-66' ||
'73</FRAME_ID>
...
union all
SELECT' ||
' 4 id, '||chr(38)||'apos;ABRIL'||chr(38)||'apos; description  FROM dual
union all
...
SELECT 12 id, '||chr(38)||'apos;DICIEMBRE'||chr(38)||'apos; description  FROM dual</SELECT_STATEMENT>
      <SELECT_STATEMENT_ID>120110359</SELECT_STATEMENT_ID>
      <TAG_NAME/>
      <PROCESS_SEQUENCE/>
      <STATEMENT_CACHE>N</STATEMENT_CACHE>
      <USER_DEFAULT_ALLOWED>N</USER_DEFAULT_ALLOWED>
...
'>
      ...
      <DEFAULT_VALUE/>
      <SELECT_S' ||
'TATEMENT>SELECT -1 ID, '||chr(38)||'apos;TODOS'||chr(38)||'apos; DESCRIPTION
FROM DUAL
UNION
...
WHERE geog_loca_area_type = 2
      ...
      <SELECT_STATEMENT_ID>120110360</SELECT_STATEMENT_ID>
      <TAG_NAME/>
      <PROCESS_SEQUENCE/>
      <STATEMENT_CACHE>N</STATEMENT_CACHE>
      <USER_DEFAULT_ALLOWED>N</USER_DEFAULT_ALLOWED>
    ...
    <ATTRIBUTE>
      ...
      <ENTITY_TYPE_ID>3339</ENTITY_TYPE_ID>
      <EXTERNAL_TYPE' ||
'_ID>500000000010603</EXTERNAL_TYPE_ID>
      <FRAME_ID>-6673</FRAME_ID>
      <DATA_LENGTH>0</DATA_LENGTH>
...
FROM DUAL
UNION
select t.task_type_id ID, t.description DESCRIPTION
  from or_task_type t
 where t.task_type_id IN
       (SELECT TO_NUMBER(COLUMN_VALUE)
          FROM TABLE(LDC_BOUTILITIES.SPLITSTRINGS(DALD_PARAMETER.fsbGetValue_Chain('||chr(38)||'apos;TIPO_TRAB_SEGCART_BRILLA'||chr(38)||'apos;,
                                                                              ' ||
'     0),
                                                  '||chr(38)||'apos;|'||chr(38)||'apos;)))</SELECT_STATEMENT>
      <SELECT_STATEMENT_ID>120110361</SELECT_STATEMENT_ID>
      <TAG_NAME/>
      <PROCESS_SEQUENCE/>
      <STATEMENT_CACHE>N</STATEMENT_CACHE>
      <USER_DEFAULT_ALLOWED>N</USER_DEFAULT_ALLOWED>
...
VALUES (LDCSCC_.tb2_0(0),
XMLType(LDCSCC_.clColumn_0),
'Forma de Seguimiento de Cartera Brilla a Cierre'
,
'1.0.0.0'
,
...
,
null,
'948d973ba70dbe3493a62941ccb9cd1e'
,
3);
...
if (not LDCSCC_.blProcessStatus) then
 return;
  ...
  BEGIN
   delete from GE_STATEMENT where STATEMENT_ID = 120110359 and MODULE_ID = 47 and NAME = 'LDC - SENT - Meses';
  EXCEPTION
  when ex.RECORD_HAVE_CHILDREN then
    ut_trace.trace('No se pudo borrar la sentencia: [120110359]',1);
   null;
  END;
...
VALUES (nuNewStatement, 47, 'LDC - SENT - Meses', 
...
if (not LDCSCC_.blProcessStatus) then
 return;
  ...
  ut_trace.trace('Borrando tabla GE_STATEMENT',1);
  BEGIN
   delete from GE_STATEMENT where STATEMENT_ID = 120110360 and MODULE_ID = 47 and NAME = 'LDC - SENT - Departamentos';
  EXCEPTION
  when ex.RECORD_HAVE_CHILDREN then
    ut_trace.trace('No se pudo borrar la sentencia: [120110360]',1);
   null;
  END;
ut_trace.trace('Obtiene nueva secuencia de GE_STATEMENT',1);
nuNewStatement := GE_BOSEQUENCE.NEXTGE_STATEMENT();
...
ut_trace.trace('insertando tabla: GE_STATEMENT ['||nuNewStatement||']',1);
INSERT INTO GE_STATEMENT(STATEMENT_ID, MODULE_ID, DESCRIPTION, STATEMENT, NAME)  
VALUES (nuNewStatement, 47, 'LDC - SENT - Departamentos', 
...
if (not LDCSCC_.blProcessStatus) then
 return;
  ...
  ut_trace.trace('Borrando tabla GE_STATEMENT',1);
  BEGIN
   delete from GE_STATEMENT where STATEMENT_ID = 120110361 and MODULE_ID = 47 and NAME = 'LDC - SENT - Tipos de Trabajo de Seguimiento de Cartera Brilla';
  EXCEPTION
  when ex.RECORD_HAVE_CHILDREN then
    ut_trace.trace('No se pudo borrar la sentencia: [120110361]',1);
   null;
  END;
ut_trace.trace('Obtiene nueva secuencia de GE_STATEMENT',1);
nuNewStatement := GE_BOSEQUENCE.NEXTGE_STATEMENT();
...
ut_trace.trace('insertando tabla: GE_STATEMENT ['||nuNewStatement||']',1);
INSERT INTO GE_STATEMENT(STATEMENT_ID, MODULE_ID, DESCRIPTION, STATEMENT, NAME)  
VALUES (nuNewStatement, 47, 'LDC - SENT - Tipos de Trabajo de Seguimiento de Cartera Brilla', 
'select -1 ID, '|| chr(39) ||'TODOS'|| chr(39) ||' DESCRIPTION
FROM DUAL
UNION
select t.task_type_id ID, t.description DESCRIPTION
 ...
 where t.task_type_id IN
       (SELECT TO_NUMBER(COLUMN_VALUE)
          FROM TABLE(LDC_BOUTILITIES.SPLITSTRINGS(DALD_PARAMETER.fsbGetValue_Chain('|| chr(39) ||'TIPO_TRAB_SEGCART_BRILLA'|| chr(39) ||',
                                                                                   0),
                                                  '|| chr(39) ||'|'|| chr(39) ||')))', 'LDC - SENT - Tipos de Trabajo de Seguimiento de Cartera Brilla'); 
...
BEGIN
 ...
 nuIndexInternal := LDCSCC_.tb0_0.first;
 while nuIndexInternal IS NOT null loop
  IF (LDCSCC_.tb0_0(nuIndexInternal)=isbExecutableName ) THEN
   IF (LDCSCC_.tb0_1.exists(nuIndexInternal)) then
    tbExecutableEquivalence(isbExecutableName) := LDCSCC_.tb0_1(nuIndexInternal);
   END IF;
   exit;
  END IF;
  nuIndexInternal := LDCSCC_.tb0_0.next(nuIndexInternal);
 END loop; 
 return tbExecutableEquivalence(isbExecutableName);
END;
BEGIN
...
nuIndex :=  LDCSCC_.tbRoleExecutables.first;
while nuIndex IS NOT null loop
 if (LDCSCC_.tbRoleExecutables(nuIndex).role_id!=1) then
  nuNewExecutableId := fnuGetNewExecutableId(LDCSCC_.tbRoleExecutables(nuIndex).name);
  IF nuNewExecutableId is not null then
    INSERT INTO sa_role_executables (role_id, executable_id)
    VALUES (LDCSCC_.tbRoleExecutables(nuIndex).role_id, nuNewExecutableId);
  END IF;
  end if;
 nuIndex := LDCSCC_.tbRoleExecutables.next(nuIndex);
END loop;
nuIndex :=  LDCSCC_.tbUserException.first;
while nuIndex IS NOT null loop
 nuNewExecutableId := fnuGetNewExecutableId(LDCSCC_.tbUserException(nuIndex).name);
 IF nuNewExecutableId is not null then
  insert INTO sa_user_exceptions (executable_id, user_id, status, usr_exc_type_id, comment_)
  VALUES (nuNewExecutableId ,LDCSCC_.tbUserException(nuIndex).user_id, LDCSCC_.tbUserException(nuIndex).status , LDCSCC_.tbUserException(nuIndex).usr_exc_type_id, LDCSCC_.tbUserException(nuIndex).comment_);
 END IF;
 nuIndex := LDCSCC_.tbUserException.next(nuIndex);
END loop;
nuIndex :=  LDCSCC_.tbExecEntities.first;
while nuIndex IS NOT null loop
 nuNewExecutableId := fnuGetNewExecutableId(LDCSCC_.tbExecEntities(nuIndex).name);
 IF nuNewExecutableId is not null then
  select count(1) into nuRecCount
 from sa_exec_entities
  where executable_id = nuNewExecutableId and entity_id = LDCSCC_.tbExecEntities(nuIndex).entity_id;
  IF nuRecCount = 0 then
    insert INTO sa_exec_entities (executable_id, entity_id )
    VALUES (nuNewExecutableId ,LDCSCC_.tbExecEntities(nuIndex).entity_id );
  END IF;
 END IF;
 nuIndex := LDCSCC_.tbExecEntities.next(nuIndex);
END loop;
...
BEGIN
 ...
 nuIndex := LDCSCC_.tbObjectToDelete.first;
 while (nuIndex is not null) LOOP
  BEGIN
   ut_trace.trace('Objeto a borrar: ' || LDCSCC_.tbObjectToDelete(nuIndex),1);
   IF(ut_object.fboExistObject(LDCSCC_.tbObjectToDelete(nuIndex)) and not fblIsObjectUseByRule(LDCSCC_.tbObjectToDelete(nuIndex))) THEN
    blObjectDeleted := ut_object.fboDeleteObject(LDCSCC_.tbObjectToDelete(nuIndex));
    ut_trace.trace('  * Borrado ' || LDCSCC_.tbObjectToDelete(nuIndex), 1);
   END IF;
  EXCEPTION when others then
   ut_trace.trace('- No se borra el objeto (' || LDCSCC_.tbObjectToDelete(nuIndex) || '): ' || sqlerrm,1);
  END;
  nuIndex := LDCSCC_.tbObjectToDelete.next(nuIndex);
 END loop;
...
begin
SA_BOCreatePackages.DropPackage('LDCSCC_');
end;
/

BEGIN ut_trace.trace('********************FIN  proceso de objeto:LDCSCC_******************************'); end;
/
