...
BEGIN
sa_bocreatePackages.CreatePackage('LDRITAN_',
'CREATE OR REPLACE PACKAGE LDRITAN_ IS ' || chr(10) ||
...
'tb3_0 ty3_0;  executableName ge_catalog.tag_name%type := ''LDRITAN''; ' || chr(10) ||
...
'( ' || chr(10) ||
'SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME=''LDRITAN'' ' || chr(10) ||
'); ' || chr(10) ||
...
BEGIN ut_trace.trace('********************Comenzar proceso de objeto:LDRITAN_******************************'); END;
/
...
if (not LDRITAN_.blProcessStatus) then
 return;
end if;
Open LDRITAN_.cuRoleExecutables;
loop
 fetch LDRITAN_.cuRoleExecutables INTO LDRITAN_.rcRoleExecutables;
 exit when  LDRITAN_.cuRoleExecutables%notfound;
 LDRITAN_.tbRoleExecutables(nuIndex) := LDRITAN_.rcRoleExecutables;
 nuIndex := nuIndex + 1;
END loop;
close LDRITAN_.cuRoleExecutables;
nuIndex := 0;
Open LDRITAN_.cuUserExceptions ;
loop
 fetch LDRITAN_.cuUserExceptions INTO  LDRITAN_.rcUserExceptions;
 exit when LDRITAN_.cuUserExceptions%notfound;
 LDRITAN_.tbUserException(nuIndex):=LDRITAN_.rcUserExceptions;
 nuIndex := nuIndex + 1;
END loop;
close LDRITAN_.cuUserExceptions;
nuIndex := 0;
Open LDRITAN_.cuExecEntities ;
loop
 fetch LDRITAN_.cuExecEntities INTO  LDRITAN_.rcExecEntities;
 exit when LDRITAN_.cuExecEntities%notfound;
 LDRITAN_.tbExecEntities(nuIndex):=LDRITAN_.rcExecEntities;
 nuIndex := nuIndex + 1;
END loop;
close LDRITAN_.cuExecEntities;

exception when others then
LDRITAN_.blProcessStatus := false;
rollback;
...
(
 SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDRITAN'
);
    ...
    BEGIN
      ...
      IF (nuNumRecords > 0) THEN
        ...
        IF ( sbObjectName IS NOT null ) THEN
          ...
          if ( LDRITAN_.tbObjectToDelete.last IS null ) then
            nuNumObjects := 1;
          else
            nuNumObjects := LDRITAN_.tbObjectToDelete.last + 1;
          end if;
          LDRITAN_.tbObjectToDelete(nuNumObjects) := sbObjectName;
        END IF;
...
if (not LDRITAN_.blProcessStatus) then
 return;
  ...
  WHERE GE_DISTRIBUTION_FILE.DISTRIBUTION_FILE_ID = 'LDRITAN';
  IF (nuNumRecords > 0) THEN
...
WHERE   sa_executable.name = 'LDRITAN'
...
if (not LDRITAN_.blProcessStatus) then
 ...
 FOR crObjectsName IN cuObjectsName LOOP
  LDRITAN_.tbObjectToDelete(nuIndex) := crObjectsName.OBJECT_NAME;
  nuIndex := nuIndex + 1;
 END LOOP;
EXCEPTION 
...
if (not LDRITAN_.blProcessStatus) then
 return;
  ...
  DELETE FROM SA_ROLE_EXECUTABLES WHERE (EXECUTABLE_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDRITAN') AND ROLE_ID=1;
...
if (not LDRITAN_.blProcessStatus) then
 return;
  ...
  DELETE FROM SA_USER_EXCEPTIONS WHERE (EXECUTABLE_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDRITAN') AND 1=2;
...
SELECT rowid
FROM SA_EXECUTABLE_SYNON WHERE (SYNONYMOUS_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDRITAN');
nuIndex binary_integer;
BEGIN
...
if (not LDRITAN_.blProcessStatus) then
 return;
  ...
  DELETE FROM SA_EXEC_ENTITIES WHERE (EXECUTABLE_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDRITAN');
...
if (not LDRITAN_.blProcessStatus) then
 return;
  ...
  DELETE FROM SA_MENU_OPTION WHERE (EXECUTABLE_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDRITAN');
...
if (not LDRITAN_.blProcessStatus) then
 return;
  ...
  DELETE FROM GI_CONFIG_COMP WHERE (CONFIG_ID) in (SELECT CONFIG_ID FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDRITAN') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339);
...
if (not LDRITAN_.blProcessStatus) then
 return;
  ...
  ut_trace.trace('Borrando tabla GI_CONFIG_COMP',1);
  DELETE FROM GI_CONFIG_COMP WHERE (COMPOSITION_ID) in (SELECT COMPOSITION_ID FROM GI_COMPOSITION WHERE (CONFIG_ID) in (SELECT CONFIG_ID FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDRITAN') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339));
...
DECLARE 
 ...
 SELECT rowid FROM GE_STATEMENT WHERE (STATEMENT_ID) in (SELECT SELECT_STATEMENT_ID FROM GI_COMP_ATTRIBS WHERE (COMPOSITION_ID) in (SELECT COMPOSITION_ID FROM GI_COMPOSITION WHERE (CONFIG_ID) in (SELECT CONFIG_ID FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDRITAN') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339)));
BEGIN 
...
LDRITAN_.tbGE_STATEMENTRowId(rcData.rowid):=rcData.rowid; 
END loop; 
...
for rcData in cuLoadTemporaryTable loop
LDRITAN_.tbGR_CONFIG_EXPRESSIONRowId(rcData.rowid):=rcData.rowid; 
END loop; 
...
if (not LDRITAN_.blProcessStatus) then
 return;
  ...
  DELETE FROM GI_COMP_ATTRIBS WHERE (COMPOSITION_ID) in (SELECT COMPOSITION_ID FROM GI_COMPOSITION WHERE (CONFIG_ID) in (SELECT CONFIG_ID FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDRITAN') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339));
...
nuVarcharIndex:=LDRITAN_.tbGE_STATEMENTRowId.first;  
while (nuVarcharIndex is not null) loop 
begin
ut_trace.trace('Row id a borrar:'||nuVarcharIndex,1);
delete from GE_STATEMENT where rowid = LDRITAN_.tbGE_STATEMENTRowId(nuVarcharIndex); 
...
LDRITAN_.tbGE_STATEMENTRowId.delete(nuOldVarcharIndex);
end if;
nuVarcharIndex := LDRITAN_.tbGE_STATEMENTRowId.next(nuVarcharIndex); 
END loop; 
...
nuVarcharIndex:=LDRITAN_.tbGR_CONFIG_EXPRESSIONRowId.first;  
while (nuVarcharIndex is not null) loop 
ut_trace.trace('Row id a borrar:'||nuVarcharIndex,1);
delete from GR_CONFIG_EXPRESSION where rowid = LDRITAN_.tbGR_CONFIG_EXPRESSIONRowId(nuVarcharIndex); 
nuOldVarcharIndex:=nuVarcharIndex;
nuVarcharIndex := LDRITAN_.tbGR_CONFIG_EXPRESSIONRowId.next(nuVarcharIndex); 
LDRITAN_.tbGR_CONFIG_EXPRESSIONRowId.delete(nuOldVarcharIndex); 
END loop; 
...
for rcData in cuLoadTemporaryTable loop
LDRITAN_.tbGI_COMPOSITIONRowId(rcData.rowid):=rcData.rowid; 
END loop; 
...
if (not LDRITAN_.blProcessStatus) then
 return;
  ...
  DELETE FROM GI_FRAME WHERE (COMPOSITION_ID) in (SELECT COMPOSITION_ID FROM GI_COMPOSITION WHERE (CONFIG_ID) in (SELECT CONFIG_ID FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDRITAN') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339));
...
nuVarcharIndex:=LDRITAN_.tbGI_COMPOSITIONRowId.first;  
while (nuVarcharIndex is not null) loop 
ut_trace.trace('Row id a borrar:'||nuVarcharIndex,1);
delete from GI_COMPOSITION where rowid = LDRITAN_.tbGI_COMPOSITIONRowId(nuVarcharIndex); 
nuOldVarcharIndex:=nuVarcharIndex;
nuVarcharIndex := LDRITAN_.tbGI_COMPOSITIONRowId.next(nuVarcharIndex); 
LDRITAN_.tbGI_COMPOSITIONRowId.delete(nuOldVarcharIndex); 
END loop; 
...
if (not LDRITAN_.blProcessStatus) then
 return;
  ...
  DELETE FROM GI_COMPOSITION WHERE (CONFIG_ID) in (SELECT CONFIG_ID FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDRITAN') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339);
...
if (not LDRITAN_.blProcessStatus) then
 return;
  ...
  DELETE FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDRITAN') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339;
...
if (not LDRITAN_.blProcessStatus) then
 return;
  ...
  DELETE FROM GE_DISTRIBUTION_FILE WHERE (DISTRIBUTION_FILE_ID) in (SELECT NAME FROM SA_EXECUTABLE WHERE NAME='LDRITAN');
...
if (not LDRITAN_.blProcessStatus) then
 return;
  ...
  DELETE FROM SA_EXECUTABLE WHERE NAME='LDRITAN';
...
LDRITAN_.old_tb0_0(0):='LDRITAN'
;
LDRITAN_.tb0_0(0):=UPPER(LDRITAN_.old_tb0_0(0));
LDRITAN_.tb0_0(0):=LDRITAN_.tb0_0(0);
LDRITAN_.old_tb0_1(0):=500000000002255;
LDRITAN_.tb0_1(0):=GE_BOCATALOG.FNUGETIDSEQFROMCATALOG(LDRITAN_.tb0_0(0), 'EXECUTABLE', 'SA_BOEXECUTABLE.GETNEXTID');
LDRITAN_.tb0_1(0):=LDRITAN_.tb0_1(0);
ut_trace.trace('insertando tabla: SA_EXECUTABLE fila (0)',1);
INSERT INTO SA_EXECUTABLE(NAME,EXECUTABLE_ID,DESCRIPTION,PATH,VERSION,EXECUTABLE_TYPE_ID,EXEC_OPER_TYPE_ID,MODULE_ID,SUBSYSTEM_ID,PARENT_EXECUTABLE_ID,LAST_RECORD_ALLOWED,PATH_FILE_HELP,WITHOUT_RESTR_POLICY,DIRECT_EXECUTION,TIMES_EXECUTED,EXEC_OWNER,LAST_DATE_EXECUTED,CLASS_ID) 
VALUES (LDRITAN_.tb0_0(0),
LDRITAN_.tb0_1(0),
'Reporte atención al cliente Itansuca'
,
null,
'12'
,
...
2,
null,
to_date('23-05-2016 07:28:10','DD-MM-YYYY HH24:MI:SS'),
null);
...
LDRITAN_.tb1_0(0):=1;
LDRITAN_.tb1_1(0):=LDRITAN_.tb0_1(0);
ut_trace.trace('insertando tabla: SA_ROLE_EXECUTABLES fila (0)',1);
INSERT INTO SA_ROLE_EXECUTABLES(ROLE_ID,EXECUTABLE_ID) 
VALUES (LDRITAN_.tb1_0(0),
LDRITAN_.tb1_1(0));
...
LDRITAN_.old_tb2_0(0):=40008129;
LDRITAN_.tb2_0(0):=SA_BOSEQUENCES.FNUNEXTSA_MENU_OPTION;
LDRITAN_.tb2_0(0):=LDRITAN_.tb2_0(0);
LDRITAN_.tb2_1(0):=LDRITAN_.tb0_1(0);
...
LDRITAN_.tb3_0(0):=LDRITAN_.tb0_0(0);
LDRITAN_.clColumn_0 := '<?xml version="1.0" encoding="UTF-8"?>
<PB xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema">
  <APPLICATION>
    <EXECUTABLE_ID>500000000002255</EXECUTABLE_ID>
    <NAME>LDRITAN</NAME>
    <DESCRIPTION>Reporte atención al cliente Itansuca</DESCRIPTION>
    <VERSION>12</VERSION>
    <MODULE>61</MODULE>
    <DIRECT_EXECUTION>Y</DIRECT_EXECUTION>
    <PATH_FILE_HELP/>
    <OBJECT_NAME>LDC_REPLEYVENTAATECLIE.FORMATO_I_ITANSUCA</OBJECT_NAME>
    <QUERY_NAME/>
  ...
  <COMPOSITION>
    ...
    <EXTERNAL_TYPE_ID>500000000002255</EXTERNAL_TYPE_ID>
    <ENTITY_ID>9623</ENTITY_ID>
    <EXTERNAL_ID>500000000002255</EXTERNAL_ID>
    <FRAME_ID>93805</FRA' ||
'ME_ID>
    ...
    <ATTRIBUTE>
      <ENTITY_ATTRIBUTE_ID>175490</ENTITY_ATTRIBUTE_ID>
      <CONFIGURATION_ID>-999999997</CONFIGURATION_ID>
      <ATTRIBUTE_NAME>INSISTENTLY_COUNTER</ATTRIBUTE_NAME>
      ...
      <ENTITY_TYPE_ID>3339</ENTITY_TYPE_ID>
      <EXTERNAL_TYPE_ID>500000000002255</EXTERNAL_TYPE_ID>
      <FRAME_ID>93805</FRAME_ID>
      <DATA_LENGTH>0</DATA_LENGTH>
      <DATA_P' ||
'RECISION>4</DATA_PRECISION>
      ...
      <DISPLAY_NAME>Año de generación</DISPLAY_NAME>
      <INIT_BASIC_EXP_ID/>
      <VALID_BASIC_EXP_ID/>
      <INIT_EXPRESSION_ID/>
      <VALID_EXPRESSION_ID/>
    ...
    <ATTRIBUTE>
      <ENTITY_ATTRIBUTE_ID>566</ENTITY_ATTRIBUTE_ID>
      <CONFIGURATION_ID>-999999996</CONFIGURATION_ID>
      <ATTRIBUTE_NAME>NUMBER_OF_PROD</ATTRIBU' ||
'TE_NAME>
      <ENTITY_ID>17</ENTITY_ID>
      <ENTITY_NAME>MO_PACKAGES</ENTITY_NAME>
      <PARENT_CONF_ID>-999999998</PARENT_CONF_ID>
      ...
      <DISPLAY_NAME>Mes de generación</DISPLAY_NAME>
      <INIT_BASIC_EXP_ID/>
      <VALID_BASIC_EXP_ID/>
      <INIT_EXPRESSION_ID/>
      <VALID_EXPRESSION_ID/>
...
union
select 3 ID, '||chr(38)||'apos;Marzo'||chr(38)||'apos; desc' ||
'ription from dual
union
...
select 12 ID, '||chr(38)||'apos;Diciembre'||chr(38)||'apos; description from dual</SELECT_STATEMENT>
      <SELECT_STATEMENT_ID>120073590</SELECT_STATEMENT_ID>
      <TAG_NAME/>
      <PROCESS_SEQUENCE/>
      <STATEMENT_CACHE>N</STATEMENT_CACHE>
      <USER_DEFAULT_ALLOWED>N</USER_DEFAULT_ALLOWED>
...
'ENTITY_ATTRIBUTE_ID>3146</ENTITY_ATTRIBUTE_ID>
      ...
      <VALID_EXPRESSION_ID/>
      <DEFAULT_VALUE/>
      <SELECT_STATEMENT>select dir' ||
'ectory_id id, path description
from  GE_DIRECTORY
</SELECT_STATEMENT>
      <SELECT_STATEMENT_ID>120073591</SELECT_STATEMENT_ID>
      <TAG_NAME/>
      <PROCESS_SEQUENCE/>
      <STATEMENT_CACHE>N</STATEMENT_CACHE>
      <USER_DEFAULT_ALLOWED>N</USER_DEFAULT_ALLOWED>
...
'DATA_PRECISION>
    ...
    <ATTRIBUTE>
      <ENTITY_ATTRIBUTE_ID>90043582</ENTITY_ATTRIBUTE_ID>
      <CONFIGURATION_ID>-999999993</CONFIGURATION_ID>
      <ATTRIBUTE_NAME>SERVICIOEXCLU</ATTRIBUTE_NAME>
      <EN' ||
'TITY_ID>8783</ENTITY_ID>
      <ENTITY_NAME>LDC_EQUIVA_LOCALIDAD</ENTITY_NAME>
      <PARENT_CONF_ID>-999999998</PARENT_CONF_ID>
      <TYPE_ATTRIB_ID>2</TYPE_ATTRIB_ID>
      <ENTITY_TYPE_ID>3339</ENTITY_TYPE_ID>
      <EXTERNAL_TYPE_ID>500000000002255</EXTERNAL_TYPE_ID>
      ...
      <DISPLAY_NAME>Área</DISPLAY_NAME>
      <INIT_BASIC_EXP_ID/>
      <VALID_BASIC_EXP_ID/>
      <INIT_EXPRESSION_ID/>
      <VALID_EXPRESSION_ID/>
      <DEFAULT_VALUE/>
      <SELECT_STATEMENT>select '||chr(38)||'apos;S'||chr(38)||'apos; id, '||chr(38)||'apos;ASE'||chr(38)||'apos; description FROM dual
union
select '||chr(38)||'apos;N'||chr(38)||'apos; id, '||chr(38)||'apos;ASNE'||chr(38)||'apos; description FROM dual</SELECT_STATEMENT>
      <SELECT_STATEMENT_ID>120073592</SELECT_STATEM' ||
'ENT_ID>
      <TAG_NAME/>
      <PROCESS_SEQUENCE/>
      <STATEMENT_CACHE>N</STATEMENT_CACHE>
...
VALUES (LDRITAN_.tb3_0(0),
XMLType(LDRITAN_.clColumn_0),
'Reporte atenci¿n al cliente Itansuca'
,
'1.0.0.0'
,
12,
...
,
null,
'149d6a2cd53cc4498d62f13514a94207'
,
3);
...
if (not LDRITAN_.blProcessStatus) then
 return;
  ...
  BEGIN
   delete from GE_STATEMENT where STATEMENT_ID = 120073590 and MODULE_ID = 61 and NAME = 'Meses';
  EXCEPTION
  when ex.RECORD_HAVE_CHILDREN then
    ut_trace.trace('No se pudo borrar la sentencia: [120073590]',1);
   null;
  END;
...
VALUES (nuNewStatement, 61, 'Meses del a¿o', 
...
union
select 12 ID, '|| chr(39) ||'Diciembre'|| chr(39) ||' description from dual', 'Meses'); 
...
UPDATE GE_DISTRIBUTION_FILE SET  GE_DISTRIBUTION_FILE.APP_XML = 
  ...
  '//COMPOSITION/ATTRIBUTE[ENTITY_ATTRIBUTE_ID = 566 and ENTITY_ID = 17]/SELECT_STATEMENT_ID/text()', 
...
if (not LDRITAN_.blProcessStatus) then
 return;
  ...
  ut_trace.trace('Borrando tabla GE_STATEMENT',1);
  BEGIN
   delete from GE_STATEMENT where STATEMENT_ID = 120073591 and MODULE_ID = 1 and NAME = 'Directorios';
  EXCEPTION
  when ex.RECORD_HAVE_CHILDREN then
    ut_trace.trace('No se pudo borrar la sentencia: [120073591]',1);
   null;
  END;
ut_trace.trace('Obtiene nueva secuencia de GE_STATEMENT',1);
nuNewStatement := GE_BOSEQUENCE.NEXTGE_STATEMENT();
...
if (not LDRITAN_.blProcessStatus) then
 return;
  ...
  ut_trace.trace('Borrando tabla GE_STATEMENT',1);
  BEGIN
   delete from GE_STATEMENT where STATEMENT_ID = 120073592 and MODULE_ID = 61 and NAME = 'agencia servicio exclusivo ASE[S] ASNE[N]';
  EXCEPTION
  when ex.RECORD_HAVE_CHILDREN then
    ut_trace.trace('No se pudo borrar la sentencia: [120073592]',1);
   null;
  END;
ut_trace.trace('Obtiene nueva secuencia de GE_STATEMENT',1);
nuNewStatement := GE_BOSEQUENCE.NEXTGE_STATEMENT();
...
ut_trace.trace('insertando tabla: GE_STATEMENT ['||nuNewStatement||']',1);
INSERT INTO GE_STATEMENT(STATEMENT_ID, MODULE_ID, DESCRIPTION, STATEMENT, NAME)  
VALUES (nuNewStatement, 61, 'agencia servicio exclusivo ASE[S] ASNE[N]', 
'select '|| chr(39) ||'S'|| chr(39) ||' id, '|| chr(39) ||'ASE'|| chr(39) ||' description FROM dual
union
select '|| chr(39) ||'N'|| chr(39) ||' id, '|| chr(39) ||'ASNE'|| chr(39) ||' description FROM dual', 'agencia servicio exclusivo ASE[S] ASNE[N]'); 

ut_trace.trace('Modificando GE_DISTRIBUTION_FILE',1);
UPDATE GE_DISTRIBUTION_FILE SET  GE_DISTRIBUTION_FILE.APP_XML = 
  UPDATEXML(GE_DISTRIBUTION_FILE.APP_XML, 
  '//COMPOSITION/ATTRIBUTE[ENTITY_ATTRIBUTE_ID = 90043582 and ENTITY_ID = 8783]/SELECT_STATEMENT_ID/text()', 
  to_char(nuNewStatement)) 
WHERE GE_DISTRIBUTION_FILE.DISTRIBUTION_FILE_ID = 'LDRITAN';
...
BEGIN
 ...
 nuIndexInternal := LDRITAN_.tb0_0.first;
 while nuIndexInternal IS NOT null loop
  IF (LDRITAN_.tb0_0(nuIndexInternal)=isbExecutableName ) THEN
   IF (LDRITAN_.tb0_1.exists(nuIndexInternal)) then
    tbExecutableEquivalence(isbExecutableName) := LDRITAN_.tb0_1(nuIndexInternal);
   END IF;
   exit;
  END IF;
  nuIndexInternal := LDRITAN_.tb0_0.next(nuIndexInternal);
 END loop; 
 return tbExecutableEquivalence(isbExecutableName);
END;
BEGIN
...
nuIndex :=  LDRITAN_.tbRoleExecutables.first;
while nuIndex IS NOT null loop
 if (LDRITAN_.tbRoleExecutables(nuIndex).role_id!=1) then
  nuNewExecutableId := fnuGetNewExecutableId(LDRITAN_.tbRoleExecutables(nuIndex).name);
  IF nuNewExecutableId is not null then
    INSERT INTO sa_role_executables (role_id, executable_id)
    VALUES (LDRITAN_.tbRoleExecutables(nuIndex).role_id, nuNewExecutableId);
  END IF;
  end if;
 nuIndex := LDRITAN_.tbRoleExecutables.next(nuIndex);
END loop;
nuIndex :=  LDRITAN_.tbUserException.first;
while nuIndex IS NOT null loop
 nuNewExecutableId := fnuGetNewExecutableId(LDRITAN_.tbUserException(nuIndex).name);
 IF nuNewExecutableId is not null then
  insert INTO sa_user_exceptions (executable_id, user_id, status, usr_exc_type_id, comment_)
  VALUES (nuNewExecutableId ,LDRITAN_.tbUserException(nuIndex).user_id, LDRITAN_.tbUserException(nuIndex).status , LDRITAN_.tbUserException(nuIndex).usr_exc_type_id, LDRITAN_.tbUserException(nuIndex).comment_);
 END IF;
 nuIndex := LDRITAN_.tbUserException.next(nuIndex);
END loop;
nuIndex :=  LDRITAN_.tbExecEntities.first;
while nuIndex IS NOT null loop
 nuNewExecutableId := fnuGetNewExecutableId(LDRITAN_.tbExecEntities(nuIndex).name);
 IF nuNewExecutableId is not null then
  select count(1) into nuRecCount
 from sa_exec_entities
  where executable_id = nuNewExecutableId and entity_id = LDRITAN_.tbExecEntities(nuIndex).entity_id;
  IF nuRecCount = 0 then
    insert INTO sa_exec_entities (executable_id, entity_id )
    VALUES (nuNewExecutableId ,LDRITAN_.tbExecEntities(nuIndex).entity_id );
  END IF;
 END IF;
 nuIndex := LDRITAN_.tbExecEntities.next(nuIndex);
END loop;
...
BEGIN
 ...
 nuIndex := LDRITAN_.tbObjectToDelete.first;
 while (nuIndex is not null) LOOP
  BEGIN
   ut_trace.trace('Objeto a borrar: ' || LDRITAN_.tbObjectToDelete(nuIndex),1);
   IF(ut_object.fboExistObject(LDRITAN_.tbObjectToDelete(nuIndex)) and not fblIsObjectUseByRule(LDRITAN_.tbObjectToDelete(nuIndex))) THEN
    blObjectDeleted := ut_object.fboDeleteObject(LDRITAN_.tbObjectToDelete(nuIndex));
    ut_trace.trace('  * Borrado ' || LDRITAN_.tbObjectToDelete(nuIndex), 1);
   END IF;
  EXCEPTION when others then
   ut_trace.trace('- No se borra el objeto (' || LDRITAN_.tbObjectToDelete(nuIndex) || '): ' || sqlerrm,1);
  END;
  nuIndex := LDRITAN_.tbObjectToDelete.next(nuIndex);
 END loop;
...
begin
SA_BOCreatePackages.DropPackage('LDRITAN_');
end;
/

BEGIN ut_trace.trace('********************FIN  proceso de objeto:LDRITAN_******************************'); end;
/
