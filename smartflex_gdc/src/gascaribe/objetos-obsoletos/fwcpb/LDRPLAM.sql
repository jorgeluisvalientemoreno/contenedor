...
BEGIN
sa_bocreatePackages.CreatePackage('LDRPLAM_',
'CREATE OR REPLACE PACKAGE LDRPLAM_ IS ' || chr(10) ||
...
'tb2_0 ty2_0;  executableName ge_catalog.tag_name%type := ''LDRPLAM''; ' || chr(10) ||
...
'( ' || chr(10) ||
'SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME=''LDRPLAM'' ' || chr(10) ||
'); ' || chr(10) ||
...
BEGIN ut_trace.trace('********************Comenzar proceso de objeto:LDRPLAM_******************************'); END;
/
...
if (not LDRPLAM_.blProcessStatus) then
 return;
end if;
Open LDRPLAM_.cuRoleExecutables;
loop
 fetch LDRPLAM_.cuRoleExecutables INTO LDRPLAM_.rcRoleExecutables;
 exit when  LDRPLAM_.cuRoleExecutables%notfound;
 LDRPLAM_.tbRoleExecutables(nuIndex) := LDRPLAM_.rcRoleExecutables;
 nuIndex := nuIndex + 1;
END loop;
close LDRPLAM_.cuRoleExecutables;
nuIndex := 0;
Open LDRPLAM_.cuUserExceptions ;
loop
 fetch LDRPLAM_.cuUserExceptions INTO  LDRPLAM_.rcUserExceptions;
 exit when LDRPLAM_.cuUserExceptions%notfound;
 LDRPLAM_.tbUserException(nuIndex):=LDRPLAM_.rcUserExceptions;
 nuIndex := nuIndex + 1;
END loop;
close LDRPLAM_.cuUserExceptions;
nuIndex := 0;
Open LDRPLAM_.cuExecEntities ;
loop
 fetch LDRPLAM_.cuExecEntities INTO  LDRPLAM_.rcExecEntities;
 exit when LDRPLAM_.cuExecEntities%notfound;
 LDRPLAM_.tbExecEntities(nuIndex):=LDRPLAM_.rcExecEntities;
 nuIndex := nuIndex + 1;
END loop;
close LDRPLAM_.cuExecEntities;

exception when others then
LDRPLAM_.blProcessStatus := false;
rollback;
...
(
 SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDRPLAM'
);
    ...
    BEGIN
      ...
      IF (nuNumRecords > 0) THEN
        ...
        IF ( sbObjectName IS NOT null ) THEN
          ...
          if ( LDRPLAM_.tbObjectToDelete.last IS null ) then
            nuNumObjects := 1;
          else
            nuNumObjects := LDRPLAM_.tbObjectToDelete.last + 1;
          end if;
          LDRPLAM_.tbObjectToDelete(nuNumObjects) := sbObjectName;
        END IF;
...
if (not LDRPLAM_.blProcessStatus) then
 return;
  ...
  WHERE GE_DISTRIBUTION_FILE.DISTRIBUTION_FILE_ID = 'LDRPLAM';
  IF (nuNumRecords > 0) THEN
...
WHERE   sa_executable.name = 'LDRPLAM'
...
if (not LDRPLAM_.blProcessStatus) then
 ...
 FOR crObjectsName IN cuObjectsName LOOP
  LDRPLAM_.tbObjectToDelete(nuIndex) := crObjectsName.OBJECT_NAME;
  nuIndex := nuIndex + 1;
 END LOOP;
EXCEPTION 
...
if (not LDRPLAM_.blProcessStatus) then
 return;
  ...
  DELETE FROM SA_ROLE_EXECUTABLES WHERE (EXECUTABLE_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDRPLAM') AND ROLE_ID=1;
...
if (not LDRPLAM_.blProcessStatus) then
 return;
  ...
  DELETE FROM SA_USER_EXCEPTIONS WHERE (EXECUTABLE_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDRPLAM') AND 1=2;
...
SELECT rowid
FROM SA_EXECUTABLE_SYNON WHERE (SYNONYMOUS_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDRPLAM');
nuIndex binary_integer;
BEGIN
...
if (not LDRPLAM_.blProcessStatus) then
 return;
  ...
  DELETE FROM SA_EXEC_ENTITIES WHERE (EXECUTABLE_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDRPLAM');
...
if (not LDRPLAM_.blProcessStatus) then
 return;
  ...
  DELETE FROM SA_MENU_OPTION WHERE (EXECUTABLE_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDRPLAM');
...
if (not LDRPLAM_.blProcessStatus) then
 return;
  ...
  DELETE FROM GI_CONFIG_COMP WHERE (CONFIG_ID) in (SELECT CONFIG_ID FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDRPLAM') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339);
...
if (not LDRPLAM_.blProcessStatus) then
 return;
  ...
  ut_trace.trace('Borrando tabla GI_CONFIG_COMP',1);
  DELETE FROM GI_CONFIG_COMP WHERE (COMPOSITION_ID) in (SELECT COMPOSITION_ID FROM GI_COMPOSITION WHERE (CONFIG_ID) in (SELECT CONFIG_ID FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDRPLAM') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339));
...
DECLARE 
 ...
 SELECT rowid FROM GE_STATEMENT WHERE (STATEMENT_ID) in (SELECT SELECT_STATEMENT_ID FROM GI_COMP_ATTRIBS WHERE (COMPOSITION_ID) in (SELECT COMPOSITION_ID FROM GI_COMPOSITION WHERE (CONFIG_ID) in (SELECT CONFIG_ID FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDRPLAM') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339)));
BEGIN 
...
LDRPLAM_.tbGE_STATEMENTRowId(rcData.rowid):=rcData.rowid; 
END loop; 
...
for rcData in cuLoadTemporaryTable loop
LDRPLAM_.tbGR_CONFIG_EXPRESSIONRowId(rcData.rowid):=rcData.rowid; 
END loop; 
...
if (not LDRPLAM_.blProcessStatus) then
 return;
  ...
  DELETE FROM GI_COMP_ATTRIBS WHERE (COMPOSITION_ID) in (SELECT COMPOSITION_ID FROM GI_COMPOSITION WHERE (CONFIG_ID) in (SELECT CONFIG_ID FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDRPLAM') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339));
...
nuVarcharIndex:=LDRPLAM_.tbGE_STATEMENTRowId.first;  
while (nuVarcharIndex is not null) loop 
begin
ut_trace.trace('Row id a borrar:'||nuVarcharIndex,1);
delete from GE_STATEMENT where rowid = LDRPLAM_.tbGE_STATEMENTRowId(nuVarcharIndex); 
...
LDRPLAM_.tbGE_STATEMENTRowId.delete(nuOldVarcharIndex);
end if;
nuVarcharIndex := LDRPLAM_.tbGE_STATEMENTRowId.next(nuVarcharIndex); 
END loop; 
...
nuVarcharIndex:=LDRPLAM_.tbGR_CONFIG_EXPRESSIONRowId.first;  
while (nuVarcharIndex is not null) loop 
ut_trace.trace('Row id a borrar:'||nuVarcharIndex,1);
delete from GR_CONFIG_EXPRESSION where rowid = LDRPLAM_.tbGR_CONFIG_EXPRESSIONRowId(nuVarcharIndex); 
nuOldVarcharIndex:=nuVarcharIndex;
nuVarcharIndex := LDRPLAM_.tbGR_CONFIG_EXPRESSIONRowId.next(nuVarcharIndex); 
LDRPLAM_.tbGR_CONFIG_EXPRESSIONRowId.delete(nuOldVarcharIndex); 
END loop; 
...
for rcData in cuLoadTemporaryTable loop
LDRPLAM_.tbGI_COMPOSITIONRowId(rcData.rowid):=rcData.rowid; 
END loop; 
...
if (not LDRPLAM_.blProcessStatus) then
 return;
  ...
  DELETE FROM GI_FRAME WHERE (COMPOSITION_ID) in (SELECT COMPOSITION_ID FROM GI_COMPOSITION WHERE (CONFIG_ID) in (SELECT CONFIG_ID FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDRPLAM') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339));
...
nuVarcharIndex:=LDRPLAM_.tbGI_COMPOSITIONRowId.first;  
while (nuVarcharIndex is not null) loop 
ut_trace.trace('Row id a borrar:'||nuVarcharIndex,1);
delete from GI_COMPOSITION where rowid = LDRPLAM_.tbGI_COMPOSITIONRowId(nuVarcharIndex); 
nuOldVarcharIndex:=nuVarcharIndex;
nuVarcharIndex := LDRPLAM_.tbGI_COMPOSITIONRowId.next(nuVarcharIndex); 
LDRPLAM_.tbGI_COMPOSITIONRowId.delete(nuOldVarcharIndex); 
END loop; 
...
if (not LDRPLAM_.blProcessStatus) then
 return;
  ...
  DELETE FROM GI_COMPOSITION WHERE (CONFIG_ID) in (SELECT CONFIG_ID FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDRPLAM') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339);
...
if (not LDRPLAM_.blProcessStatus) then
 return;
  ...
  DELETE FROM GI_CONFIG WHERE (EXTERNAL_ROOT_ID) in (SELECT EXECUTABLE_ID FROM SA_EXECUTABLE WHERE NAME='LDRPLAM') AND CONFIG_TYPE_ID=4 AND ENTITY_ROOT_ID=3339;
...
if (not LDRPLAM_.blProcessStatus) then
 return;
  ...
  DELETE FROM GE_DISTRIBUTION_FILE WHERE (DISTRIBUTION_FILE_ID) in (SELECT NAME FROM SA_EXECUTABLE WHERE NAME='LDRPLAM');
...
if (not LDRPLAM_.blProcessStatus) then
 return;
  ...
  DELETE FROM SA_EXECUTABLE WHERE NAME='LDRPLAM';
...
LDRPLAM_.old_tb0_0(0):='LDRPLAM'
;
LDRPLAM_.tb0_0(0):=UPPER(LDRPLAM_.old_tb0_0(0));
LDRPLAM_.tb0_0(0):=LDRPLAM_.tb0_0(0);
LDRPLAM_.old_tb0_1(0):=500000000005026;
LDRPLAM_.tb0_1(0):=GE_BOCATALOG.FNUGETIDSEQFROMCATALOG(LDRPLAM_.tb0_0(0), 'EXECUTABLE', 'SA_BOEXECUTABLE.GETNEXTID');
LDRPLAM_.tb0_1(0):=LDRPLAM_.tb0_1(0);
ut_trace.trace('insertando tabla: SA_EXECUTABLE fila (0)',1);
INSERT INTO SA_EXECUTABLE(NAME,EXECUTABLE_ID,DESCRIPTION,PATH,VERSION,EXECUTABLE_TYPE_ID,EXEC_OPER_TYPE_ID,MODULE_ID,SUBSYSTEM_ID,PARENT_EXECUTABLE_ID,LAST_RECORD_ALLOWED,PATH_FILE_HELP,WITHOUT_RESTR_POLICY,DIRECT_EXECUTION,TIMES_EXECUTED,EXEC_OWNER,LAST_DATE_EXECUTED,CLASS_ID) 
VALUES (LDRPLAM_.tb0_0(0),
LDRPLAM_.tb0_1(0),
'Reporte de Amortización Mensual'
,
null,
'6'
,
...
null,
null,
to_date('17-09-2014 17:48:32','DD-MM-YYYY HH24:MI:SS'),
null);
...
LDRPLAM_.tb1_0(0):=1;
LDRPLAM_.tb1_1(0):=LDRPLAM_.tb0_1(0);
ut_trace.trace('insertando tabla: SA_ROLE_EXECUTABLES fila (0)',1);
INSERT INTO SA_ROLE_EXECUTABLES(ROLE_ID,EXECUTABLE_ID) 
VALUES (LDRPLAM_.tb1_0(0),
LDRPLAM_.tb1_1(0));
...
LDRPLAM_.tb2_0(0):=LDRPLAM_.tb0_0(0);
LDRPLAM_.clColumn_0 := '<?xml version="1.0" encoding="UTF-8"?>
<PB xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema">
  <APPLICATION>
    <EXECUTABLE_ID>500000000004977</EXECUTABLE_ID>
    <NAME>LDRPLAM</NAME>
    <DESCRIPTION>Reporte de Amortización Mensual</DESCRIPTION>
    <VERSION>6</VERSION>
    <MODULE>61</MODULE>
    <DIRECT_EXECUTION>Y</DIRECT_EXECUTION>
    <PATH_FILE_HELP/>
    <OBJECT_NAME>LDC_PROCESSLDRPLAM</OBJECT_NAME>
    <QUERY_NAME/>
  ...
  <COMPOSITION>
    ...
    <EXTERNAL_TYPE_ID>500000000004977</EXTERNAL_TYPE_ID>
    <ENTITY_ID>9623</ENTITY_ID>
    <EXTERNAL_ID>500000000004977</EXTERNAL_ID>
    <FRAME_ID>91896</FRAME_ID>
    <IS_UPDATABLE>N</I' ||
'S_UPDATABLE>
...
'DATA_PRECISION>
      ...
      <VALID_EXPRESSION_ID/>
      <DEFAULT_VALUE/>
      <SELECT_STATEMENT>SELECT directory_id id, description FROM ge_directory
order by directory_id</SELECT_STATEMENT>
      <SELECT_STATEMENT_ID>120043687</SELECT_STATEMENT_ID>
      <TAG_NAME/>
      <PROCESS_SEQUENCE/>
      <STATEMENT_CACHE>N</STATEMENT_CACHE>
      <USER_DEFAULT_ALLOWED>N</USER_DEFAULT_ALLOWED>
...
VALUES (LDRPLAM_.tb2_0(0),
XMLType(LDRPLAM_.clColumn_0),
'Reporte de Amortización Mensual'
,
'1.0.0.0'
,
...
,
null,
'e261ae45a46b37c30895c13fe0c5374f'
,
3);
...
if (not LDRPLAM_.blProcessStatus) then
 return;
  ...
  BEGIN
   delete from GE_STATEMENT where STATEMENT_ID = 120043687 and MODULE_ID = 1 and NAME = 'Lista de Directorios';
  EXCEPTION
  when ex.RECORD_HAVE_CHILDREN then
    ut_trace.trace('No se pudo borrar la sentencia: [120043687]',1);
   null;
  END;
...
VALUES (nuNewStatement, 1, 'Carga los directorios de GE_DIRECTORY', 
'SELECT directory_id id, description FROM ge_directory
order by directory_id', 'Lista de Directorios'); 
...
BEGIN
 ...
 nuIndexInternal := LDRPLAM_.tb0_0.first;
 while nuIndexInternal IS NOT null loop
  IF (LDRPLAM_.tb0_0(nuIndexInternal)=isbExecutableName ) THEN
   IF (LDRPLAM_.tb0_1.exists(nuIndexInternal)) then
    tbExecutableEquivalence(isbExecutableName) := LDRPLAM_.tb0_1(nuIndexInternal);
   END IF;
   exit;
  END IF;
  nuIndexInternal := LDRPLAM_.tb0_0.next(nuIndexInternal);
 END loop; 
 return tbExecutableEquivalence(isbExecutableName);
END;
BEGIN
...
nuIndex :=  LDRPLAM_.tbRoleExecutables.first;
while nuIndex IS NOT null loop
 if (LDRPLAM_.tbRoleExecutables(nuIndex).role_id!=1) then
  nuNewExecutableId := fnuGetNewExecutableId(LDRPLAM_.tbRoleExecutables(nuIndex).name);
  IF nuNewExecutableId is not null then
    INSERT INTO sa_role_executables (role_id, executable_id)
    VALUES (LDRPLAM_.tbRoleExecutables(nuIndex).role_id, nuNewExecutableId);
  END IF;
  end if;
 nuIndex := LDRPLAM_.tbRoleExecutables.next(nuIndex);
END loop;
nuIndex :=  LDRPLAM_.tbUserException.first;
while nuIndex IS NOT null loop
 nuNewExecutableId := fnuGetNewExecutableId(LDRPLAM_.tbUserException(nuIndex).name);
 IF nuNewExecutableId is not null then
  insert INTO sa_user_exceptions (executable_id, user_id, status, usr_exc_type_id, comment_)
  VALUES (nuNewExecutableId ,LDRPLAM_.tbUserException(nuIndex).user_id, LDRPLAM_.tbUserException(nuIndex).status , LDRPLAM_.tbUserException(nuIndex).usr_exc_type_id, LDRPLAM_.tbUserException(nuIndex).comment_);
 END IF;
 nuIndex := LDRPLAM_.tbUserException.next(nuIndex);
END loop;
nuIndex :=  LDRPLAM_.tbExecEntities.first;
while nuIndex IS NOT null loop
 nuNewExecutableId := fnuGetNewExecutableId(LDRPLAM_.tbExecEntities(nuIndex).name);
 IF nuNewExecutableId is not null then
  select count(1) into nuRecCount
 from sa_exec_entities
  where executable_id = nuNewExecutableId and entity_id = LDRPLAM_.tbExecEntities(nuIndex).entity_id;
  IF nuRecCount = 0 then
    insert INTO sa_exec_entities (executable_id, entity_id )
    VALUES (nuNewExecutableId ,LDRPLAM_.tbExecEntities(nuIndex).entity_id );
  END IF;
 END IF;
 nuIndex := LDRPLAM_.tbExecEntities.next(nuIndex);
END loop;
...
BEGIN
 ...
 nuIndex := LDRPLAM_.tbObjectToDelete.first;
 while (nuIndex is not null) LOOP
  BEGIN
   ut_trace.trace('Objeto a borrar: ' || LDRPLAM_.tbObjectToDelete(nuIndex),1);
   IF(ut_object.fboExistObject(LDRPLAM_.tbObjectToDelete(nuIndex)) and not fblIsObjectUseByRule(LDRPLAM_.tbObjectToDelete(nuIndex))) THEN
    blObjectDeleted := ut_object.fboDeleteObject(LDRPLAM_.tbObjectToDelete(nuIndex));
    ut_trace.trace('  * Borrado ' || LDRPLAM_.tbObjectToDelete(nuIndex), 1);
   END IF;
  EXCEPTION when others then
   ut_trace.trace('- No se borra el objeto (' || LDRPLAM_.tbObjectToDelete(nuIndex) || '): ' || sqlerrm,1);
  END;
  nuIndex := LDRPLAM_.tbObjectToDelete.next(nuIndex);
 END loop;
...
begin
SA_BOCreatePackages.DropPackage('LDRPLAM_');
end;
/

BEGIN ut_trace.trace('********************FIN  proceso de objeto:LDRPLAM_******************************'); end;
/
