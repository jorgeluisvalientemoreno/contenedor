...
cursor cuUnidades is
 select a.operating_unit_id,
       ...
       (select count(1) from open.ge_items_seriado s where s.operating_unit_id=a.operating_unit_id and s.items_id=a.items_id and id_items_estado_inv in (1,12)) cant_med
...
begin
  dmbs_output.put_line('UNIDAD_OPER|NOMBRE|ITEM|DESCRIPCION|CANTIDAD_BODEGA|CANTIDAD_ACTIVO|CANTIDAD_INVENTARIO');
  for reg in cuUnidades loop
    for reg2 in cuCuadre(reg.operating_unit_id) loop
      dmbs_output.put_line(reg2.operating_unit_id||'|'||reg2.nombre||'|'||reg2.items_id||'|'||reg.descripcion||'|'||reg2.balance||'|'||reg2.cantidad_activo||'|'||reg2.cantidad_inv);
    end loop;
  end loop;
end;
/
