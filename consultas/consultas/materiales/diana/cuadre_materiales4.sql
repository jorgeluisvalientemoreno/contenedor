...
where (a.balance!=nvl((select balance from open.ldc_inv_ouib b where a.items_id=b.items_id and a.operating_unit_id=b.operating_unit_id), 0)+
      ...
      and a.operating_unit_id IN (1885 )
      and a.items_id=10000905
...
where mmitcodi=dmitmmit
 ...
 union all*/
   SELECT  null tipo_documento, null description,  B.CUADHOMO operating_unit_id, nvl(migra.FNUGETITEMOSF_ROLLOUT(eixbitem, a.BASEDATO),0) item, eixbtipo tipo, sum(eixbdisu) cantidad
...
from open.or_uni_item_bala_mov m left join open.ge_items_documento d on d.id_items_Documento=m.id_items_documento, open.ge_document_type t
