...
declare
    cursor cuSolicitudes is
        ...
        WHERE p.package_id IN (190465427)
        AND p2.motive_status_id = 13
        ORDER BY  p2.package_id;
        
	 
    sbComment 		VARCHAR2(4000):='Se cambia estado a anulado por SOSF-607';
			...
			FOR regOt IN cuOrdenes(reg.package_id) LOOP
				BEGIN
					...
					if nuErrorCode=0 then
						...
						rollback;
						dbms_output.put_line('Error anulando orden: '||regOt.order_id||' : '||sbErrorMesse);
						nuError := 1;
					end IF; 
...
declare    nuPackage_id number:=99163;
BEGIN
