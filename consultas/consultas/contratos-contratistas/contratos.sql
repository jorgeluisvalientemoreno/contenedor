...
WHERE (CONTRACT_TYPE_ID=co.id_tipo_contrato or co.id_contrato=c.contract_id)
...
SELECT *
FROM OPEN.ldc_antic_contr
WHERE IDCONTRATO=1441;
OPEN.CT_PROCESS_LOG
...
select *
from open.ldc_tasktype_contype_hist
where tipo_trabajo=12587

select id_contrato,co.status, co.descripcion, id_tipo_contrato,co.fecha_inicial, co.fecha_final,(select c.fecha_maxasig from open.ldc_contfema c where c.id_contrato=co.id_contrato) fecha_max_asig,  id_contratista, co.valor_asignado, co.valor_liquidado, co.valor_no_liquidado, co.valor_total_contrato, co.valor_total_pagado,
...
select *
from open.ge_contrato
where id_contrato in (3941,4061,4861);
select *
from  OPEN.ct_conpla_con_type c
where c.contract_id in (3941,4061,4861);
select *
from  OPEN.CT_CONDITIONS_BY_PLAN
WHERE CONDITIONS_PLAN_ID=324;
select *
...
--area organizacional con contrato
select *
from open.LDC_CONTRATO_AREA