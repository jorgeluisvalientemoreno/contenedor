...
nuacta Number:= 86793;
  ...
  --                que no encuentre el dato adicional
    ...
    sbdatoadicerr    open.ldci_carasewe.casevalo%Type;
  ...
  Begin
    ...
    If nucantacta > 0 Then
      For rgcuordenes In cuordenes(nucontratista) Loop
        ...
        If (upper(sbttactivo) = 'Y' /*Or rgcuordenes.defined_contract_id Is Not Null*/) Then
          ...
          --200-1957
                  dbms_output.put_line('1nudatoadic:'||nudatoadic||'orden ;'||nuorden_id);
                  Exception
                    When no_data_found Then
                      nudatoadic := Null;
                    When Others Then
          ...
          --200-1957
                  ...
                  If nudatoadic Is Null or nudatoadic > 1 or nudatoadic = 0 Then
                    ...
                    if nvl(instr(sbMensaje,rgcuordenes.task_type_id),0) = 0 then
                      ...
                      dbms_output.put_line(nuorden_id);
                      dbms_output.put_line('sin dato adicional o mas de un registro');
                      dbms_output.put_line('2 sbdatoadicerr:'||sbdatoadicerr||'orden ;'||nuorden_id);
                     dbms_output.put_line('2 nudatoadic:'||nudatoadic||'orden ;'||nuorden_id);
              ...
              Else
                ...
                If nucentrocosto Is Null Then
                  ...
                  if nvl(instr(sbMensaje,rgcuordenes.task_type_id),0) = 0 then
                    sbMensaje:=sbMensaje||','||rgcuordenes.titr_validar;
                    dbms_output.put_line(nuorden_id);
                    dbms_output.put_line('sin centro de costo');
                  end if;
                Else
    ...
    If sbordenes > 0 Then
      sbMensaje:='Existen ordenes de los siguientes tipos de trabajo sin configuracion contable: '||sbMensaje;
      null;
    Else
      null;
    End If;
