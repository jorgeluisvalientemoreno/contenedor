 SELECT (  SELECT nvl( SUM( CASE ca.cargsign WHEN 'DB' THEN cargunid WHEN 'CR' then -cargunid END ),0) voltotal_ult_3meses
  ...
  base.*  from ( SELECT 
               c1.cargnuse,sesucico ,trunc(o.execution_final_date) Fecha_suspension, nvl( SUM( CASE c1.cargsign WHEN 'DB' THEN cargunid WHEN 'CR' then -cargunid END ),0) voltotal_dsp_susp
