...
select base4.*,
       ...
       (select max(c.cosspefa) from conssesu c where c.cosssesu=base4.product_id and c.cosspefa<base4.periodo4) periodo5,
        (select l.leemoble from open.lectelme l where l.leemsesu=base4.product_id and l.leempefa=base4.periodo1 and l.leemclec='F' and rownum < 2 ) lectura1,
       (select l.leemoble from open.lectelme l where l.leemsesu=base4.product_id and l.leempefa=base4.periodo2 and l.leemclec='F' and rownum < 2  ) lectura2,
       (select l.leemoble from open.lectelme l where l.leemsesu=base4.product_id and l.leempefa=base4.periodo3 and l.leemclec='F' and rownum < 2  ) lectura3
from base4
...
select *
from base5 
where califica3 = 2015
and rownum <= 15 
and lectura1 IN (59,80,71,79,12)
and lectura2 IN (59,80,71,79,12)
and lectura3 IN (59,80,71,79,12)
and (select count(1)
     from or_order oo
     inner join or_order_Activity aa on aa.order_id=oo.order_id
     left join or_requ_Data_value da on da.order_id = oo.order_id
     where  base5.product_id = aa.product_id
     and oo.task_type_id in (12620)
     and oo.order_status_id in (8)
     and da.name_1 = 'Lectura'
     and da.value_1 is not null) >= 1;
