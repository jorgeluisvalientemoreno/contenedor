select notas.notasusc  "Contrato",cargcodo "Cons documento",  
       ...
       cargos.cargsign "Signo",notaprog "Programa notas" ,
...
where servsusc.sesunuse in (14518870) 
and notas.notafecr >= trunc(sysdate)
...
cargos.cargcaca ||'- '|| initcap(cacadesc)  ,cargos.cargsign,cargcodo,notaprog
order by notas.notasusc
;
/*
select * from notas n where n.notasusc = 48229624 order by notafecr desc ;
select * from cargos where cargdoso like '%113259244%'; 
select * from cargos where cargcodo = 113259244; 
*/
