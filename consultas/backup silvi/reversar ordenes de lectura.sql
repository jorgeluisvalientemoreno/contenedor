
update or_order
   set causal_id=null,
       legalization_date=null,
       order_status_id=5,
       saved_data_values=null,
       exec_initial_date=null,
       execution_final_date=null
where order_id=336440228;


update or_order_activity
   set status='R',
       value1=null,
       value2=null
where order_id=336440228;

update or_order_items
   set legal_item_amount=0
 where order_id=336440228;
 

delete  or_order_stat_change
   where order_id=336440228
     and initial_status_id=5
     and final_status_id=8;


delete or_order_person
where order_id=336440228;

delete from or_order_comment
where order_id = 336440228; 

/*UPDATE LDC_OTLEGALIZAR
    SET LEGALIZADO='N'
WHERE ORDER_ID=323605029;

delete ldc_docuorder where order_id=323599275;
delete or_requ_data_value where order_id=323599275;*/

update lectelme
set leemdocu = 330066925,
leemleto = null, 
leemoble = null,
 leemfele = null
where leemcons = 148437210;

select * from hileelme
where hlemelme    = 148437210 --[lectelme.leemcons]

delete from hileelme
where hlemelme = 148437210;

select *  from open.conssesu 
where conssesu.cosssesu = 2077347
order by cossfere desc ;

delete from open.conssesu 
where conssesu.cosssesu = 2062426
and cosspecs  =113329
