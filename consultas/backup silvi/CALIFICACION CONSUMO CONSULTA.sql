with  Consumo_actual as (select   (29+36.03)  Consumo_desv
                        from dual
) , max_perifact as  ( select max(l2.leempefa) periodo_fact  from open.lectelme l2 where l2.leemsesu = 51530383)
, Consumo_anterior as  (select lectelme.leemleto Lectura_ant
                        from open.lectelme
                        where leemsesu = 51530383
...
where caravaco.crvccate = (select category_id  
                           ...
                           where product_id = 51530383)
and caravaco.crvcsuca = (select subcategory_id   
                         from open.pr_product
                         where product_id = 51530383)
and  35.333  between caravaco.crvcraci and caravaco.crvcracf
and (select round(( Consumo_desv - 35.333) /  35.333 * 100)  
