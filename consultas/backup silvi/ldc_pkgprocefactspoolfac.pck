...
(
 blNRegulado      BOOLEAN
,orfcursordatconc OUT constants.tyRefCursor
);
-----------------------------------------------------------------------------------------------------------------------------------------
...
,orfcursorimpdig OUT constants.tyRefCursor
);
-----------------------------------------------------------------------------------------------------------------------------------------
...
BEGIN
 ...
 ELSIF sbconsumo = 'LEC.MEDIDOR' THEN
   nuPerFactPrev   := open.pkbillingperiodmgr.fnugetperiodprevious(nufactpefa);
...
BEGIN
 ...
 IF NOT blNoRegulada AND nuProducto > 0 THEN
  ...
  FOR i IN tbCompCost.first .. tbCompCost.last LOOP
   BEGIN
    nuVitcons := open.FA_BOPrintCostCompRules.fnuGetCostCompValid(
 ...
 IF sbTipoMet = 'T' THEN
  OPEN orfcursor FOR
   SELECT nvl(abs(SUM(decode(dpttsign, 'CR', -nvl(dpttvano,0), nvl(dpttvano,0)))),0) acumu
     FROM open.ldc_deprtatt, ldc_prodtatt
...
PROCEDURE RfConcepParcial(
 ...
 sbcod_con_iva_gdc     ld_parameter.value_chain%type := open.dald_parameter.fsbgetvalue_chain('COD_CON_IVA_GDC',NULL);
    ...
    IF NOT blNRegulado THEN
        ...
        LOOP
            ...
            IF nuLastSesu <> tbCargos(i).cargnuse THEN
               ...
               nuSaldoAnterior := ldc_detallefact_gascaribe.FnuGetSaldoAnterior(
                ...
                IF (tbCargos(i).cargpefa = -9999 AND nuSaldoProd > 0) OR tbCargos(i).cargpefa <> -9999 THEN
                    ...
                    tbCargosOrdered(-1).car_doso := tbCargos(i).servcodi;
            ...
            CASE -- Acumulado de IVA
                ...
                '|' || tbCargos(i).cargconc || '|') > 0 and substr(tbCargos(i).cargdoso, 0, 3)  NOT IN ('ID-', 'DF-')  THEN
                    ...
                    IF tbCargosOrdered(-3).conceptos = 'x' THEN
                        tbCargosOrdered(-3).conceptos := 'INTERES DE MORA(Tasa ' || tbCargos(i).cargunid || '%)';
                    END IF;
                -- Arma el detalle para los dem?s cargos diferentes de diferidos
                WHEN substr(tbCargos(i).cargdoso, 0, 3) NOT IN ('ID-', 'DF-') AND tbCargos(i).cargsign IN ('DB', 'CR', 'PA', 'AS') THEN
                    tbCargosOrdered(inx).etiqueta := '32';
            ...
            IF tbCargos.next(i) IS NULL THEN
                ut_trace.Trace('LDC_DetalleFact_GasCaribe.RfConcepParcial Inicio Almacenar diferidos restantes',10);
                IF (tbCargosOrdered.Count() > 0) THEN
                    ut_trace.Trace(' LDC_DetalleFact_GasCaribe.RfConcepParcial Inicio Almacenar diferidos restantes 1',10);
                ...
                IF nuLastSesu <> -1 THEN
                      ...
                      LOOP
                          ...
                          IF (NOT gtbFinancion.exists(tdiferidos(k).difecodi)) THEN
                              ...
                              inxConc := RTrim(LTrim(inx));
                               ut_trace.trace('   tdiferidos (k).difecodi[' || tdiferidos(k).difecodi || '] inx [' || inx || ']',15);
                              ...
                              IF (gtbConceptos.exists(inxConc) AND (nuServicio != 7053 or ( (nuServicio = 7053  or nuExiste > 0) AND instr(upper(tdiferidos(k).concdefa), 'RESCREG-059') <> 0)) ) THEN
                                  ut_trace.trace('tdiferidos (k).difecodi[' || tdiferidos(k).difecodi || '] EXISTE ',15);
                                  inx := gtbConceptos(inxConc);
                                  tbcargosordered(inx).saldo_dif := tbcargosordered(inx).saldo_dif + tdiferidos(k).difesape;
                              ELSE
                                  ut_trace.trace('tdiferidos (k).difecodi[' || tdiferidos(k).difecodi || '] NO EXISTE ',15);
                                  tbCargosOrdered(inx).etiqueta := '32';
                                  ut_trace.trace('tdiferidos (k).difecodi[' || tdiferidos(k).difecodi || '] NO EXISTE ',15);
                      ...
                      END LOOP;
                   END IF;
                ut_trace.trace('RfConcepParcial Termina  loop de difereidos',15);
        ...
        ut_trace.TRACE('   tbCargosOrdered.count[' || tbCargosOrdered.Count || '] ',15);
        nuSaldoDife := 0;
        tblProdDife.delete;
        nuIndeBri := 1;
  ...
  SELECT etiqueta
   ...
   FROM (
         SELECT id etiqueta
          ...
          GROUP BY id, concepto, cuotas_pendientes, doc_soporte --, cau_cargo
          UNION ALL
         SELECT id etiqueta
               ,concepto desc_concep
               ,SUM(vencido) saldo_ant
...
/******************************************************************************************
  ...
  =========       =========           ====================
******************************************************************************************/
  sbflgimpre ld_parameter.value_chain%TYPE;
  nmcontaenv  NUMBER(4);
  nmcontarev  NUMBER(4);
  nmcontacart NUMBER(4);
  sbRespuesta VARCHAR2(2) := 'S';
BEGIN
  sbflgimpre := dald_parameter.fsbGetValue_Chain('VALIDAIMPRFACTFISICASPOOL',NULL);
  -- Validamos si el flag de impresion esta en S
  IF sbflgimpre = 'S' THEN
   -- Validamos si el usuario esta marcado para envio de factura digital
   SELECT COUNT(1) INTO nmcontaenv
     FROM suscripc s
    WHERE s.susccodi = nmsusccodi
      AND suscefce   = 'S'
      AND suscdeco IS NOT NULL;

   -- Validamos si el usuario tiene financiaciones activas
   SELECT COUNT(1) INTO nmcontarev
     FROM diferido d
    WHERE d.difesusc = nmsusccodi
      AND d.difeprog = 'GCNED'
      AND d.difesape >= 1;

   -- Validamos si el usuario esta mayor a 90 dias
   SELECT COUNT(1) INTO nmcontacart
     FROM ldc_cartdiaria cd
    WHERE cd.contrato = nmsusccodi
      AND cd.edad_mora >= 90;

   IF nmcontaenv >= 1 AND nmcontarev = 0 AND nmcontacart = 0 THEN
     sbRespuesta := 'N';
   ELSE
     sbRespuesta := 'S';
   END IF;
   -- Validamos si el flag de impresion esta en N para no realizar validacion
  ELSE
    sbRespuesta := 'S';
  END IF;
...
BEGIN
 ...
 END;
 OPEN orfcursor for
 SELECT  to_char(nuPayment,'FM999,999,999,990.00') valor_ult_pago,to_char(dtFechPago,'DD/MM/YYYY') fecha_ult_pago
   FROM dual ;
EXCEPTION
 WHEN ex.controlled_error THEN
  RAISE ex.controlled_error;
...
--------------------------------------------------------------------------------------------------------------------------------------
  ...
  ******************************************************************/
    ...
    sbPrograma  diferido.difeprog%TYPE;
