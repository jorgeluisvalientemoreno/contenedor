...
(select count ( distinct (c1.cosspecs)) from open.conssesu c1 
           ...
           and c1.cosscoca = 0 
           and c1.cosspecs between '102385' and '103221' 
...
(select count ( distinct (o2.order_id)) from open.or_order o2, open.or_order_activity a2
           ...
           and o2.legalization_date >'23/02/2023' )as ot_leg_12,
(select count ( distinct (o3.order_id)) from open.or_order o3, open.or_order_activity a3
           ...
           and o3.created_date<'23/03/2023')  as ot_pend_onl_12  ,
(select count ( distinct (o4.order_id)) from open.or_order o4, open.or_order_activity a4
           ...
           and o4.legalization_date >'08/03/2023' )as ot_leg_38,
(select count ( distinct (o5.order_id)) from open.or_order o5, open.or_order_activity a5
           ...
           and o5.created_date<'23/03/2023')  as ot_pend_onl_38  ,
(select count ( distinct (o6.order_id)) from open.or_order o6, open.or_order_activity a6
           ...
           and o6.legalization_date >'23/02/2023' )as ot_leg_82,
(select count ( distinct (o7.order_id)) from open.or_order o7, open.or_order_activity a7
           ...
           and o7.created_date<'23/03/2023')  as ot_pend_onl_82  ,
(select count ( distinct (o8.order_id)) from open.or_order o8, open.or_order_activity a8
           ...
           and o8.legalization_date >'23/02/2023' )as ot_leg_64,
(select count ( distinct (o9.order_id)) from open.or_order o9, open.or_order_activity a9
           ...
           and o9.created_date<'23/03/2023')  as ot_pend_onl_64  ,
(select count ( distinct (o10.order_id)) from open.or_order o10, open.or_order_activity a10
           ...
           and o10.created_date <'23/03/2023') as ot_onl_8_pend , 
 (select count ( distinct (o11.order_id)) from open.or_order o11, open.or_order_activity a11
           ...
           and o11.legalization_date >'08/03/2023' ) as ot_onl_8_liq,
...
case when (select count (hcppcons)
            ...
            and hcpppeco =  103221 ) > 0 then 'si_cpp' 
    when   (select count (hcppcons)
            from hicoprpm h1
            where h1.hcppsesu= sesunuse 
            and hcpppeco =  103221 ) = 0 then 'No_cpp' end as cons_pp ,
...
(select count ( distinct (o12.order_id)) from open.or_order o12, open.or_order_activity a12
           ...
           and o12.created_date>'22/03/2023 17:00:00')  as ot_creadas_hoy  ,  
 (select o13.task_type_id from open.or_order o13, open.or_order_activity a13
           ...
           and o13.created_date>'22/03/2023 17:00:00')  as ot_task_type  ,          
 (select c2.cosscoca  from open.conssesu c2
           where c2.cosssesu = a.product_id 
           and c2.cosssesu = sesunuse 
           and c2.cosspecs = 103625
           and c2.cossfere = (select max(c6.cossfere) from conssesu c6 where c2.cosssesu= c6.cosssesu )
...
(select c3.cosscavc  from open.conssesu c3
           ...
           and c3.cosspecs = 103625
           and c3.cossfere = (select max(c6.cossfere) from conssesu c6 where c3.cosssesu= c6.cosssesu )
...
(select c4.cossfufa   from open.conssesu c4
           ...
           and c4.cosspecs = 103625
           and c4.cossfere = (select max(c6.cossfere) from conssesu c6 where c4.cosssesu= c6.cosssesu )
           and c4.cossmecc in (1,3)) funcion_cons,substrc( a.value2,10,2) ONL,      
a.order_id||'|'||8017||'|'||'38963||'||a.order_activity_id||'>'||'1'||';'||  regexp_substr( a.value2,'COMMENT1>[^>]*')||'>>'||';;;'||'|||'||'1277'||';OSF-654|'||
substrc ( r.exec_initial_date,1,19) ||';'|| substrc ( r.execution_final_date,1,19) as cadena_legalizacion
...
and r.legalization_date >= '23/03/2023 16:40:00'
