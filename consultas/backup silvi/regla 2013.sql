...
case when (select count (hcppcons)
    ...
    when   (select count (hcppcons)
            from hicoprpm h1
            where h1.hcppsesu= sesunuse 
            and hcpppeco =  103221 ) = 0 then 'No_cpp' end as cons_pp , i.value1,i.value2,substrc( i.value2,10,2) ONL,      
...
left join open.infopuerto i on i.cosssesu = a.product_id and i.task_type_id = 12617
left join open.ab_address ab on  pr.address_id = ab.address_id 
where sesucicl  in (1501)
and pecscons =103654
