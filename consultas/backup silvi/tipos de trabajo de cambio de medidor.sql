select or_order_activity.subscription_id  "Contrato", or_order_activity.product_id  "Producto", 
       or_order_activity.package_id  "Solicitud", mo_packages.package_type_id || ' :  ' || ps_package_type.description as "Tipo de solicitud", 
       mo_packages.motive_status_id || ' :  ' || ps_motive_status.description as "Estado solicitud", 
       or_order.order_id  "Orden", or_order.task_type_id ||' : '|| initcap(or_task_type.description)  "Tipo de trabajo",
...
inner join open.mo_packages on mo_packages.package_id = or_order_activity.package_id
inner join open.ps_package_type on ps_package_type.package_type_id = mo_packages.package_type_id
inner join open.ps_motive_status on ps_motive_status.motive_status_id = mo_packages.motive_status_id
where or_order.order_status_id in (5)
and or_order.task_type_id in (select task_type_id
