...
base2 as(
select base.*,
      (select max(c.cossfere) from conssesu c where c.cosssesu=base.product_id and c.cosspefa=base.periodo1 and c.cossmecc in (3) ) fecha1,
...
base3 as(
select base2.*,
       (select c.cossmecc from conssesu c where c.cosssesu=base2.product_id and c.cosspefa=base2.periodo1 and c.cossmecc in (3) and c.cossfere=base2.fecha1 and rownum=1) metodo1,
       (select c.cosscoca from conssesu c where c.cosssesu=base2.product_id and c.cosspefa=base2.periodo1 and c.cossmecc in (3) and c.cossfere=base2.fecha1 and rownum=1) consumo1,
       (select c.COSSCAVC from conssesu c where c.cosssesu=base2.product_id and c.cosspefa=base2.periodo1 and c.cossmecc in (3) and c.cossfere=base2.fecha1 and rownum=1) califica1,
       
       (select max(c.cossfere) from conssesu c where c.cosssesu=base2.product_id and c.cosspefa=base2.periodo2 and c.cossmecc in (3) ) fecha2,
