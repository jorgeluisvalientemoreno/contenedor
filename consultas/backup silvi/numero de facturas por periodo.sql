...
where  pefacodi in (select esprpefa
                   ...
                   and   esprpefa in (select regexp_substr(102103,'[^,]+', 1, level) from dual
                    connect by regexp_substr(102103, '[^,]+', 1, level) is not null))
and  pefacodi in (select esprpefa
                   from open.estaprog
                   where esprprog like 'FIDF%'
                   and   esprfein >= trunc(sysdate-60)
                   and   esprpefa in (select regexp_substr(99413,'[^,]+', 1, level) from dual
                    connect by regexp_substr(99413, '[^,]+', 1, level) is not null))

(select count(distinct(factsusc))
        from open.factura 
        where factprog = 6
        and   factpefa = pefacodi) BETWEEN 50 and 100 ; 
