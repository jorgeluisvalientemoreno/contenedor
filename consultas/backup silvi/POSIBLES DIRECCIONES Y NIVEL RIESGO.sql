select pr_product.product_id  "Producto",ab_address.address_id,
       ...
       ab_premise.subcategory_ ||'- '|| initcap(open.subcateg.sucadesc)  "Subcategoria",
              ab_segments.ciclcodi,empresa,ab_info_premise.premise_id
...
left join ciclo_facturacion on ciclo = ab_segments.ciclcodi
...
and not exists(select null from open.pr_product
...
and ab_premise.category_ = 1 and   ab_address.geograp_location_id = 5
