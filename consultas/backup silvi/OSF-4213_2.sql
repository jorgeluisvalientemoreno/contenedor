...
--select * from ldci_claveconta where clavcodi in ('40','50','01','31')  --31,50 credito 40 y 01 debito

with base as (
select cod_interfazldc cod_interfazldc_old, ASIGNACN  ASIGNACN_old , sum ( impomsoc ) sum_impomsoc_old
from ldci_detaintesap
where cod_interfazldc in  ( 57294) 
group by cod_interfazldc , ASIGNACN),
base_2 as (
select cod_interfazldc cod_interfazldc_new, ASIGNACN  ASIGNACN_new , sum ( impomsoc ) sum_impomsoc_new
from ldci_detaintesap
where cod_interfazldc in  ( 57303 ) 
group by cod_interfazldc , ASIGNACN)
select cod_interfazldc_old, ASIGNACN_old , sum_impomsoc_old , cod_interfazldc_new, ASIGNACN_new , sum_impomsoc_new ,
case when sum_impomsoc_old = sum_impomsoc_new then 'COINCIDE'  end as validacion 
from base_2 
full outer join base on ASIGNACN_new=ASIGNACN_old  ;
