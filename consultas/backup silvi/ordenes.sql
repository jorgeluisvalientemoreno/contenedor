...
where  or_order_activity.product_id = 1046276 --and or_order.task_type_id in (11056, 11230, 11231, 11232, 11177 , 11178)
and or_order.order_status_id in (0,5,11); --cert. interna . cxc
 
--sesunuse= 1186962
--and or_order.task_type_id in (10043)-- and legalization_date >= sysdate -30 
--select * from or_task_type t where t.task_type_id= 11056 for update 

select  m.package_id,package_type_id ,m.request_date, m.motive_status_id , pr.subscription_id, pr.product_id , pr.product_status_id , ps.description , sesucicl 
...
and  pr.product_id  = 1046224
 order by m.request_date desc ;

select * from cupon where cuposusc = 67504981
