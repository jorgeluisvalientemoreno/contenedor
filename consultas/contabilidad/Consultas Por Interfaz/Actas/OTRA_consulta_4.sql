select t.idacta acta, dt.cod_interfazldc, dt.clavcont, dt.clasecta, dt.impomtrx, /*id_orden,*/ activo, total
  from open.ldci_detaintesap dt, open.ldci_encaintesap c, open.ldci_actacont t,
      (select id_acta, activo, sum(total) total
         from 
              ...
              group by d.id_acta, d.id_orden, u.value_1)
        group by id_acta, activo)  u
 where c.cod_interfazldc = t.codocont
   ...
   and (substr(dt.clasecta,1,12) = activo or dt.impomtrx = total)
