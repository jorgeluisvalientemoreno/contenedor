select count(*),COD_TIPCOMPROBANTE,
       ...
       DESC_CLASCONT,
       DOMINIO,
       VALOR_REPORTAR, CTA_CONTABLE_DB, (select CUCTDESC from OPEN.LDCI_CUENTACONTABLE where cuctcodi = CTA_CONTABLE_DB) desc_cuenta,
       CTA_CONTABLE_CR,
       CRITERIO,
       (ltrim(rtrim(substr(CRITERIO,(instr(CRITERIO, '=')+1), 30)))) item,
       (select description from OPEN.GE_ITEMS where items_id = (ltrim(rtrim(substr(CRITERIO,(instr(CRITERIO, '=')+1), 30))))) desc_item,
       porcentaje,
