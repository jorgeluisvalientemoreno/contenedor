...
sum(gd.valor_total) total, gd.id_acta acta, gd.geograp_location_id cod_localidad, (select g.description from ge_geogra_location g where g.geograp_location_id = gd.geograp_location_id) localidad,GD.ID_ORDEN, nvl(gcon.valor_aui_admin,0) valor_aui_admin, nvl(gcon.valor_aui_util,0) valor_aui_util, nvl(gcon.valor_aui_imprev,0) valor_aui_imprev, ldci_pkinterfazactas.fvaGetCuGaCoClasi(GD.ID_ORDEN) cuenta
