      ...
      select (select l.celocebe
             ...
             decode((SELECT 1
                      ...
                      WHERE OR_related_order.rela_order_type_id in (4, 13) -- Tipo de Orden, de Apoyo o Relacionada
                        ...
                        AND OR_order.legalization_date <= '&2FECHA_FINAL 23:59:59'
       ...
       where m.invmsesu  not in (select h.hcecnuse from open.hicaesco h
                                 where h.hcececan =  96
                                   ...
                                   and h.hcecfech <= '&2FECHA_FINAL 23:59:59' and h.hcecnuse = m.invmsesu)
               ...
               (select cargnuse, cargconc, cargcaca, cargvalo, substr(cargdoso, 4, 8) SOLICITUD, trunc(cargfecr) fec_cargo,
                       (select count(*) from open.or_order_activity act, open.or_order oo
                         where act.package_id = substr(cargdoso, 4, 8)
                           ...
                           and oo.created_date  <= '&2FECHA_FINAL 23:59:59'
                 ...
                 where cargcuco != -1
                   ...
                   and FACTFEGE BETWEEN to_date('09/02/2015 00:00:00','dd/mm/yyyy hh24:mi:ss')     
                                    and to_date('&2FECHA_FINAL 23:59:59','dd/mm/yyyy hh24:mi:ss')                     
         ...
         where VENTAS > 0
           ...
           and ort.product_id not in (select h.hcecnuse from open.hicaesco h
                                       where h.hcecnuse = ort.product_id
                                         --and h.hcececan = 96
                                         and h.hcececac = 1 and h.hcecfech <= '&2FECHA_FINAL 23:59:59')
       ) xx
...
where nuse not in (select h.hcecnuse from open.hicaesco h
                    where h.hcecfech <= '&2FECHA_FINAL 23:59:59' and h.hcecnuse = nuse and h.hcececac  in (1,95,110))
/*UNION
-- NOTAS A CONSTRUCTORAS
-- NOTAS A CONSTRUCTORAS
select 'NOTAS_CONSTRU' TIPO , NULL, NULL, NULL, NULL, NUSE, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, 
       NULL, vr_notas NOTAS, NULL
from (
(select cargnuse NUSE,  sum(decode(cargsign, 'DB', cargvalo, -cargvalo)) vr_notas
  from open.cargos ca, open.servsusc sc
 where cargnuse = sesunuse
   and sesuserv = 6121
   and cargfecr BETWEEN '01-07-2015' --to_date('&1FECHA_INICIAL 00:00:00','dd/mm/yyyy hh24:mi:ss')     
                    and to_date('&2FECHA_FINAL 23:59:59','dd/mm/yyyy hh24:mi:ss')    
   and cargtipr = 'P'    
...
where nuse not in (select h.hcecnuse from open.hicaesco h
                    where h.hcecfech <= '&2FECHA_FINAL 23:59:59' and h.hcecnuse = nuse and h.hcececac  in (\*5,94,*\1,95,110))*/
)
)
PIVOT
(
