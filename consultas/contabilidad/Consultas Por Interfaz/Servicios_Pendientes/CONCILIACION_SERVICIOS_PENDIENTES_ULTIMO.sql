...
SELECT SALDO, TIPO, DEPA, LOCA, CEBE, NUSE, CONCEPTO, CONCDESC, SOLICITUD, TIPO_SOLICITUD, FEC_VENTA, FEC_CARGO, 
       TOTAL TOTAL_ANT, NOTAS_ANT, -- TOTAL MES ANTERIOR
...
select SALDO, TIPO, DEPA, LOCA, NUSE, CONCEPTO, CONCDESC,  SOLICITUD, TIPO_SOLICITUD, 
       FEC_VENTA, FEC_CARGO, TOTAL, NOTAS_ANT, -- TOTAL MES ANTERIOR NOTAS
       (select cb.celocebe from open.ldci_centbenelocal cb where cb.celoloca = loca) CEBE,
...
SELECT 'SALDO_INICIAL' SALDO, TIPO, DEPA, LOCA, NUSE, CONCEPTO, CONCDESC,  SOLICITUD, TIPO_SOLICITUD, 
       ...
       (NVL(VR_INGRESO,0)+NVL(ING_REPORTADO,0)/*+NVL(NOTAS,0)*/) TOTAL, NVL(NOTAS,0) NOTAS_ANT -- NOTAS MES ANTERIOR
  from
(
...
select 'PROD_MIGRADOS' TIPO,
       ...
       100271 TIPO_SOLICITUD, null fec_cargo, ING_REPORTADO, 0 TOTAL_ANT
from (
...
select TIPO, CEBE, DEPA, LOCA, DESCRIPCION, NUSE, CATE, EST_TECNICO, DES_TECNICO, CONCEPTO, CONCDESC, VR_INGRESO,
       SOLICITUD, TIPO_SOLICITUD, FEC_CARGO, ING_REPORTADO, 0 TOTAL_ANT
 from
(
        ...
        (decode((SELECT 'X'
                  ...
                  WHERE OR_related_order.rela_order_type_id in (4, 13) -- Tipo de Orden, de Apoyo o Relacionada
                    ...
                    AND OR_order.CAUSAL_ID IN (select c.causal_id from open.ge_causal c
                                                where c.class_causal_id = 1)), 'X', ((cargvalo/ventas)*-1), 0)) ING_REPORTADO
  from
       (
...
select 'SIN_PRODUCTOS' TIPO,
       ...
       decode(cargconc,19, 'CAR_X_CON.',
                       ...
                       291,'INT_INDUS.',
                       674,'REV_PREVIA','NO_EXISTE') concdesc,
       cargvalo VR_INGRESO, SOLICITUD, TIPO_SOLICITUD, fec_cargo, 0 ING_REPORTADO, 0 NOTAS_ANT
...
select 'VENTAS_MES' SALDO, TIPO, DEPA, LOCA, NUSE, CONCEPTO, CONCDESC, SOLICITUD, TIPO_SOLICITUD,
       FEC_CARGO FEC_VENTA, FEC_CARGO, VR_INGRESO TOTAL, 0 NOTAS_ANT
 from
(
...
--WHERE  NUSE IN (6641255,50665459,50672780,50723252)
