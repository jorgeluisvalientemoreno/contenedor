...
--
-- ------   
-- INTERNAS MIGRADAS REPORTADAS CON ORDEN DE APOYO HASTA EL 31-05-2018
-- INTERNAS MIGRADAS REPORTADAS CON ORDEN DE INTERNA APARTIR DEL '01-06-2019'
-- 
SELECT 'Int_Mig' Tipo, m.invmsesu, sesucate, invmconc, o.concdesc, sum(invmvain), 0 reportada, sum(invmvain) contabilizar, l.celocebe  
  from open.Ldci_Ingrevemi m, open.servsusc s, open.ab_address, open.suscripc, open.ge_subscriber g, open.ldci_centbenelocal l, open.concepto o,
       (SELECT DISTINCT OR_order_activity.product_id
         ...
         WHERE OR_order_activity.package_id = mo_packages.package_id
           ...
           AND OR_order.legalization_date >= to_date(:Fecha_Inicial, 'dd-mm-yyyy')
           ...
           AND OR_order_activity.product_id not in (select act.product_id 
                                                     ...
                                                     where act.product_id = OR_order_activity.product_id
                                                       ...
                                                       and oo.legalization_date < to_date('01/06/2018', 'dd-mm-yyyy')
                                                   )
           AND OR_order_activity.product_id not in (SELECT distinct hcecnuse
                                                     ...
                                                     WHERE hcececan = 96
                                                      ...
                                                      --AND hcecfech < to_date(:Fecha_Inicial, 'dd-mm-yyyy')
                                                       AND hcecfech < to_date('01'||to_char(OR_order.legalization_date, 'mmyyyy'), 'dd-mm-yyyy') 
                                                   )
...
where m.invmsesu = xx.product_id
  AND m.invmsesu = s.sesunuse
  AND sesususc = susccodi
  AND suscclie = g.subscriber_id
  AND g.address_id = ab_address.address_id
                      ...
                      and x.invmsesu in (SELECT DISTINCT OR_order_activity.product_id
                                             ...
                                             WHERE OR_related_order.related_order_id = OR_order_activity.order_id
                                               ...
                                               AND (
                                                    ...
                                                    OR_order.legalization_date >= to_date('09/02/2015') AND 
                                                    OR_order.legalization_date <= to_date('31/05/2018 23:59:59') -- Fecha Fija
                                                  OR
                                                    -- Nuevo esquema de reportar Ingreso de Interna
                                                    or_order.task_type_id in (12149, 12151)    AND                   
                                                    OR_order.legalization_date >= to_date('01/06/2018') AND    -- Fecha Fija - Nuevo esquema de reportar Interna  
                                                    OR_order.legalization_date <  to_date(:Fecha_Final, 'dd-mm-yyyy') + 1 -- Fecha cierre mes anterior
                                                    --OR_order.legalization_date <= to_date(to_char(to_char(last_day(trunc(hcecfech)), 'dd-mm-yyyy') || ' 23:59:59'), 'dd-mm-yyyy hh24:mi:ss')
                                                   )                                                   
       ...
       (select cargconc, o.concdesc, product_id, m.package_type_id, u.order_id,
          ...
          from open.cargos c, open.concepto o, open.mo_packages m,
               (SELECT DISTINCT to_char(mo_packages.package_id) package_id, OR_order_activity.product_id, OR_order.order_id
                  FROM open.OR_order_activity, open.or_order, open.mo_packages
                 WHERE OR_order.legalization_date >= to_date(:Fecha_Inicial, 'dd-mm-yyyy')                         -- FECHA INICIAL
                   ...
                   AND OR_order_activity.product_id not in (select act.product_id
                                                             ...
                                                             where act.product_id = OR_order_activity.product_id
                                                               ...
                                                               and oo.legalization_date < to_date('01/06/2018') -- Fecha Fija, comienza nuevo proceso interna
...
--           and cargDOSO =  'PP-84114953'
 ...
 UNION ALL
 -- INGRESOS RED MATRIZ
select 'Ing_Red_Matriz' Tipo, producto, SESUCATE, CARGCONC, CONCDESC, SUM(VALOR) TOTAL, 0 Reportada,  SUM(VALOR) TOTAL,
       (select db.celocebe from open.ldci_centbenelocal db where db.celoloca = loca) cebe
  FROM (
          ...
          from open.cargos,
               (SELECT distinct to_char(m.package_id) package_id, m.package_type_id, a.product_id producto
                 ...
                 where a.order_id = o.order_id
                   ...
                   and o.legalization_date >= to_date(:Fecha_Inicial, 'dd-mm-yyyy')
                   and o.legalization_date <  to_date(:Fecha_Final, 'dd-mm-yyyy') + 1 
               ) u
