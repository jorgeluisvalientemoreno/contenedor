...
select CEBE, LOCA, DESCRIPCION, package_type_id, SESUCATE, SESUSUCA, CARGCONC, CONCDESC, SUM(VALOR) TOTAL
FROM (
select (select l.celocebe 
 ...
 from  open.servsusc, open.suscripc,
       (select cargconc, o.concdesc, product_id, m.package_type_id, 
               (cargvalo/(select count(*) from open.or_order_activity
...
GROUP BY CEBE, LOCA, DESCRIPCION, SESUCATE, SESUSUCA, CARGCONC, package_type_id, CONCDESC    
