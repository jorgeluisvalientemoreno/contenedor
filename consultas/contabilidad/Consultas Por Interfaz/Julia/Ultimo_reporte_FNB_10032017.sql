-- Reporte JULIA con Fra
select * from
(
Select 'PAGOS' Tipo, contrato, producto, sol_venta, /*ot_entrega,*/ fecha, difecodi, /*valor_item,*/ val_diferido, Sal_diferido,
  ...
  from (
select contrato, producto, sol_venta, ot_entrega, fecha, /*valor_cont,*/ valor_sol, /*ot_pago,*/ tipo_trabajo, difecodi, /*valor_item,*/ val_diferido,
       ...
       (case when acta is not null then
         (select ac.extern_invoice_num from open.ge_Acta ac
           where ac.id_acta = acta
             and ac.extern_pay_date <= '28-02-2017 23:59:59')
...
select contrato, producto, sol_venta, ot_entrega, fecha, valor_sol, /*ot_pago, */difecodi, /*valor_item,*/ val_diferido,
       ...
       --  where cargdoso = 'PP-'||sol_venta and cargvalo = val_diferido and rownum = 1) valor_cont,
       (case when ot_pago is not null then
         (select distinct ac.id_acta from open.ge_detalle_Acta ac where ac.id_orden = ot_pago)
         end) Acta,
      --(SELECT max(package_id) FROM open.ld_return_item lri
      --  WHERE lri.package_sale = sol_venta and lri.approved = 'Y') Sol_Causal_46,
      ot_venta,
      (select oi.article_id
         from open.ld_item_work_order oi
        where oi.order_id = ot_venta and rownum = 1) Articulo
from (
select a.subscription_id contrato, difenuse Producto, a.package_id sol_venta, a.order_id ot_entrega, trunc(o.legalization_date) fecha,
       ot_pago, iwo.difecodi, --(iwo.value + nvl(iwo.iva,0)) Valor_item, 
       dif.difevatd Val_diferido, dif.difesape Sal_diferido,
  ...
  From open.or_order_activity a, open.or_order o, open.ld_item_work_order iwo, open.diferido dif,
       (select aa.order_id ot_pago, aa.package_id, aa.motive_id, aa.origin_activity_id
         ...
         where aa.task_type_id = 10144
           and aa.register_date >= '01/02/2017 00:00:00' -- Fija
 ...
 Where o.legalization_date >= '01-02-2017' -- Fija
   and o.legalization_date <= '28-02-2017 23:59:59'
...
Group by a.subscription_id, difenuse, a.package_id, a.order_id, trunc(o.legalization_date), a.motive_id, ot_pago, iwo.difecodi, value,
...
select 'OTROS' Tipo, subscription_id, product_id, sol_venta,/* null id_order,*/ register_date fecha_ot_pago,
       ...
       /*valor_pago,*/ 0 valor_sol, /*orden,*/ Tipo_Trabajo, Acta, fra, valor_pago, contratista, 
       --0 Sol_Causal_46, null Est_Ot,
       Articulo, Des_artic, linea, (select ll.description from open.ld_line ll where ll.line_id = linea) desc_linea
  from (
select register_date, subscription_id, product_id, id_order, valor_pago, orden, acta,
       (select act.extern_invoice_num from open.ge_Acta act
         where act.id_acta = acta
           and act.extern_pay_date <= '28-02-2017 23:59:59') fra,
       estado,
       (select oat.task_type_id || ' - ' || ott.description from open.or_order_activity oat, open.or_task_type ott
...
select register_date, subscription_id, product_id, id_order,
  ...
  from (
          select
                 ...
                 trunc(register_date) register_date, oac.subscription_id, oac.product_id, oac.order_id id_order,
                 ...
                 (select lt.actcontabiliza from open.ldci_actacont lt
                   where idacta = (select a.id_acta from open.ge_detalle_acta a, open.ge_acta gaa
                                    where a.id_acta = gaa.id_acta
                                      and gaa.extern_pay_date <= '28-02-2017 23:59:59'
           ...
           where oac.task_type_id in (10140, 10220, 10398)
             and register_date Between '01/02/2017 00:00:00' -- Fija
