-- PROVISION COSTOS LECTURAS
SELECT tipo, product_id, titr, Desc_Titr,
       sum(Total) Total, sum(Total_iva) total_iva, Clasificador, clcodesc, contratista, nombre, localidad,
       cuenta, nom_cuenta, ceco, O_i
from (       
SELECT tipo, product_id, titr, (select tt.description from open.or_task_type tt where tt.task_type_id = titr) Desc_Titr,
       concept, Total, Total_iva, clctclco Clasificador, clcodesc, contratista, nombre, 
...
from (       
select tipo, product_id, titr, concept, sum(decode(Nombre, null, 0, total)) Total, 
       ...
       orden, open.LDC_BOORDENES.FNUGETIDLOCALIDAD(orden) Localidad 
  from (
        select tipo, product_id, orden, titr, concept, sum(total) total, 
               sum(total_iva) total_iva, contratista, nombre, fec_lega, clctclco
        from (
          SELECT 'ACTA_S_F' TIPO, null product_id, a.id_acta acta, a.id_orden orden, 
                 decode(oa.task_type_id, 10336, ro.real_task_type_id, oa.task_type_id) titr, ot.concept,
          ...
          Group by 'ACTA_S_F', null, a.id_acta, a.id_orden, decode(oa.task_type_id, 10336, ro.real_task_type_id, oa.task_type_id), 
                   ot.concept, co.id_contratista, ta.nombre_contratista, trunc(ro.legalization_date), tt.clctclco
          ...
          where (ac.extern_pay_date is null or ac.extern_pay_date > '30-09-2015 23:59:59') 
           ...
           and co.id_contratista = ta.id_contratista  and a.id_items = it.items_id 
           and (it.item_classif_id != 23 or it.items_id = 4001293)
          Group by 'ACTA_S_F', null, a.id_acta, a.id_orden, 
                   decode(oa.task_type_id, 10336, ro.real_task_type_id, oa.task_type_id), ot.concept, co.id_contratista, 
                 ta.nombre_contratista, trunc(ro.legalization_date), tt.clctclco
        )
        Group by tipo, product_id, orden, titr, concept, contratista, nombre, fec_lega, clctclco
        UNION
        select tipo, product_id, orden, titr, concept, sum(total) total, sum(total_iva) total_iva,
               contratista, nombre, fec_lega, clctclco
        from (
        SELECT 'SIN_ACTA' TIPO, NULL product_id, oa.order_id orden, 
         ...
         where r.legalization_date >= '09-02-2015 00:00:00' and r.legalization_date <= '30-09-2015 23:59:59'
        ...
        Group by 'SIN_ACTA', NULL, oa.order_id, decode(oa.task_type_id, 10336, r.real_task_type_id, oa.task_type_id),
...
group by tipo, product_id, orden, titr, concept, clctclco,icc.clcodesc, contratista, nombre, fec_lega
)
where total != 0
)
group by tipo, product_id, titr, Desc_Titr, clasificador, clcodesc, contratista, nombre, localidad, 
         cuenta, nom_cuenta, ceco, O_i
