-- CARTERA CORRIENTE
-- SALDO INICIAL
select servicio, cargnuse, diferencia, concclco, t.clcodesc, --cargconc, cargcaca, cargdoso, cargfecr, 
       --(decode(cargsign, 'PA', -1, 'AP', 1, 'SA', 1, 'AS', -1, 'CR', -1, 'DB', 1, -1) * cargvalo) tot_cargos
  ...
  from (
        ...
        having sum(VALOR) != 0
        order by caccnuse
     ) u, open.cargos ca, open.perifact f, open.cuencobr c, open.factura fa, open.concepto, open.ic_clascont t
 where caccnuse = cargnuse
   ...
   and cargconc = conccodi and concclco = t.clcocodi
group by servicio, cargnuse, diferencia, concclco, t.clcodesc
