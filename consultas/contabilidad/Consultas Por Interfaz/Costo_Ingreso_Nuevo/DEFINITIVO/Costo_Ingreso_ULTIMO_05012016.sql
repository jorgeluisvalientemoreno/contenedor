-- INGRESO vs COSTO - SERVICIOS VARIOS - 05-01-2016
SELECT estado, product_id, Nom_Producto, Cate, Nom_cate, tipo, acta, factura, fecha, contratista, nombre, titr, desc_titr,
       (select lg.cuencosto from open.ldci_cugacoclasi lg where lg.cuenclasifi = clasificador) Cuenta,
       ...
       Clasificador, Desc_Clasificador, actividad, Des_actividad, concept, desc_conc, costo, iva, ing_otro,
       (select sum(decode(cargsign, 'DB',cargvalo,'AS', cargvalo, cargvalo*-1)) valor
          from open.cargos c
         where c.cargnuse = product_id
           and c.cargconc = concept
           and c.cargfecr between '&Fecha_Inicial'  --'&FECHA_INICIAL' --,'dd/mm/yyyy'
                              and '&Fecha_Final 23:59:59' --'&FECHA_FINAL 23:59:59' --,'dd/mm/yyyy hh24:mi:ss')
           and CARGCUCO > 0
           and cargtipr = 'P'
           and cargsign NOT IN ('PA','AP')
           and substr(cargdoso,1,1) IN ('N')
           and cargcaca not in (20,23,46,50,51,56,73)
        group by cargnuse) NOTAS,  Ing_Int_Mig,
       Ing_CxC_Mig, Ing_RP_Mig, ING_INT_OSF, ING_CXC_OSF, ING_RP_OSF, ING_INT_CON, ING_CXC_CON, ING_RP_CON, TOTAL_INGRESO,
...
FROM
(
select estado, product_id, Nom_Producto,
       Cate, (select ct.catedesc from open.categori ct where ct.catecodi = cate) Nom_cate,
       tipo, acta, factura, fecha, contratista, nombre, titr, desc_titr, Clasificador, Desc_Clasificador, actividad,
       Des_actividad, concept, desc_conc, costo, iva, ing_otro, Ing_Int_Mig, Ing_CxC_Mig, Ing_RP_Mig, ING_INT_OSF, ING_CXC_OSF,
  ...
  from (
select estado, Product_id, (select s.subscriber_name|| ' ' || s.subs_last_name
       ...
       clctclco Clasificador, clcodesc Desc_Clasificador,
...
select tipo, product_id, acta, titr, actividad, total, total_iva, concept, estado, contratista, nombre,
       ...
       (case when concept is not null and
             ...
             (select sum(cargvalo) from open.cargos
              where cargnuse = product_id and cargconc = concept and cargcaca in (41,53) and cargsign = 'DB' -- OTROS_ING_OSF
                and cargfecr >= '&Fecha_Inicial' and cargfecr <= '&Fecha_Final 23:59:59'
             ...
             (case when concept in (30) and
                   ...
                   then
                       (select sum(invmvain) from open.ldci_ingrevemi i
                         where i.invmsesu = x.product_id and i.invmconc = 30)
              else
                (0)
              end)
        else
             (case when concept is null and titr in (10622, 10624) and fec_lega between '&Fecha_Inicial' and '&Fecha_Final' then
               (select sum(invmvain) from open.ldci_ingrevemi i
          ...
          (case when concept in (30) AND
                  nvl((select 'Y'
                       ...
                       where a.product_id = x.product_id and a.task_type_id in (10622, 10624)
                         ...
                         and o.causal_id in (select gc.causal_id from open.ge_causal gc
                                              where gc.causal_id = o.causal_id and gc.class_causal_id = 1)
                         and rownum = 1),'N') = 'N'  then
              (select sum(decode(cargsign, 'DB',cargvalo,'AS', cargvalo, cargvalo*-1)) --valor
                from open.cargos c, OPEN.CUENCOBR, OPEN.FACTURA
               where cargnuse =  x.product_id
                 and cargconc =  concept
                 and CARGCUCO =  CUCOCODI
                 and factcodi =  CUCOfact
                 and FACTFEGE BETWEEN to_date('&Fecha_Inicial 00:00:00','dd/mm/yyyy hh24:mi:ss')
                                  and to_date('&Fecha_Final 23:59:59','dd/mm/yyyy hh24:mi:ss')
                 and cargtipr =  'A'
                      ...
                      from open.cargos c, open.concepto o,
                           (SELECT DISTINCT to_char(mo_packages.package_id) package_id, OR_order_activity.product_id
                             ...
                             WHERE OR_related_order.rela_order_type_id in (4, 13) -- Tipo de Orden, de Apoyo o Relacionada
                               ...
                               AND OR_order.legalization_date >= '&Fecha_Inicial'
       ...
       (case when estado is not null
              AND nvl((select sum(cargvalo) from open.cargos
                ...
                and cargfecr >= '&Fecha_Inicial' and cargfecr <= '&Fecha_Final 23:59:59' and cargdoso like 'PP%'),0) = 0 then -- CXC OSF
          (case when concept in (19) then
           (select sum(Valor)
              from (select product_id, (cargvalo/ventas) Valor
                      from (select cargconc, cargvalo, package_id, u.product_id,
...
from (
      select tipo, product_id, acta, titr, concept, sum(decode(Nombre, null, 0, total)) Total,
             ...
             (select 'CONECTADO' from open.hicaesco h
               where h.hcecfech >= '&Fecha_Inicial' and h.hcecfech <= '&Fecha_Final 23:59:59' and h.hcecserv = 7014
        ...
        from (
              ...
              from  (
                ...
                where ac.extern_pay_date >= '&Fecha_Inicial'
                 ...
                 and ro.legalization_date <  '&Fecha_Inicial'
                 and ro.legalization_date >= '&Fecha_Inicial' and ro.legalization_date <= '&Fecha_Final 23:59:59'
              ...
              SELECT 'ACTA_S_F' TIPO, oa.product_id, a.id_acta acta, null /*ac.extern_invoice_num*/ factura,
                ...
                FROM OPEN.ge_detalle_acta a, open.or_order_activity oa, open.mo_packages m, open.ge_acta ac, open.or_order ro,
                     open.ic_clascott tt, open.or_task_type ot, open.ge_contratista ta, open.ge_contrato co, open.ge_items it
                where (ac.extern_pay_date is null or ac.extern_pay_date > '&Fecha_Final 23:59:59')
                 and ac.id_acta   = a.id_acta
                 and a.id_orden   = oa.order_id
                 and oa.package_id = m.package_id(+)
                 and a.valor_total != 0
              ...
              SELECT 'SIN_ACTA' TIPO, oa.product_id, null acta, null factura, null fecha, oa.order_id orden,
               ...
               where r.legalization_date >= '&Fecha_Inicial' and r.legalization_date <= '&Fecha_Final 23:59:59'
...
Group by estado, Product_id, tipo, acta, factura, fecha, contratista, nombre, Titr, clctclco, clcodesc, actividad, concept
)
)
