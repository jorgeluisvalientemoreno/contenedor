-- INGRESO_COSTO_CERTIFICACION_PREVIA
select cargnuse, 19 concepto, sum(decode(cargsign, 'DB',cargvalo,'AS', cargvalo, cargvalo*-1)) valor
...
where cargnuse = sesunuse 
  ...
  and cargsign = 'DB'
group by cargnuse, 19
MINUS
(select product_id, 19 concepto, 68600
   from (
         -- Acta con Factura
