...
/*  where (nvl(Ing_Int_Mig,0) != 0 OR nvl(Ing_CxC_Mig,0) != 0 OR nvl(Ing_RP_Mig,0)  != 0 OR 
         nvl(ING_INT_OSF,0) != 0 OR nvl(ING_CXC_OSF,0) != 0 OR nvl(ING_RP_OSF,0)  != 0 OR
         nvl(ING_INT_CON,0) != 0 OR nvl(ING_CXC_CON,0) != 0 OR nvl(ING_RP_CON,0)  != 0)*/Group by Product_id, tipo, acta, factura, fecha, contratista, nombre, Titr, clctclco, clcodesc, actividad, concept
