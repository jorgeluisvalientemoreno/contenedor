       ...
       (case when concept is not null and
             ...
             (select sum(cargvalo) from open.cargos
              where cargnuse = product_id and cargconc = concept and cargcaca in (41,53) and cargsign = 'DB' -- ING_INT_OSF
                and cargfecr >= '&FECHA_INICIAL' and cargfecr <= '&FECHA_FINAL 23:59:59') end) Ing_Otro,
          ...
          (case when concept in (19) then       
            (select sum(cargvalo) from open.cargos
              where cargnuse = product_id and cargconc in (674) and cargcaca in (41,53) and cargsign = 'DB' -- ING_RP_OSF
                and cargfecr >= '&FECHA_INICIAL' and cargfecr <= '&FECHA_FINAL 23:59:59' and cargdoso like 'PP%') end) end) ING_RP_OSF, 
...
from ( 
      select tipo, product_id, acta, titr, concept, sum(decode(Nombre, null, 0, total)) Total, clctclco, 
        ...
        from (
              Select  tipo, product_id, acta, factura, fecha, orden, titr, concept, sum(total) total, contratista, nombre, 
                      fec_lega, clctclco
              from  (
              ...
              from (
                ...
                where (ac.extern_pay_date is null or ac.extern_pay_date > '&FECHA_FINAL 23:59:59') 
                 ...
                 and /*oa.task_type_id =*/ decode(oa.task_type_id, 10336, ro.real_task_type_id, oa.task_type_id) = tt.clcttitr
                 and oa.order_id = ro.order_id
                 and ro.legalization_date <= '&FECHA_FINAL 23:59:59'
                 and oa.task_type_id = ro.task_type_id
                 and tt.clctclco not in (311,246,303,252,253)
                 and oa.task_type_id = ot.task_type_id
                 and ac.id_contrato = co.id_contrato
                           ...
                           from (
                                ...
                                SELECT distinct o.product_id
                                 ...
                                 where r.legalization_date <= '&FECHA_FINAL 23:59:59'
                                   ...
                                   --and r.legalization_date <= '&FECHA_FINAL 23:59:59'
                                   and o.package_id = m.package_id(+)
                                   ...
                                   and o.order_id in (select a.id_orden from OPEN.ge_detalle_Acta a, open.ge_acta ac
                                                       where a.id_orden = o.order_id and a.id_acta = ac.id_acta 
                                                         and (ac.extern_pay_date is null or ac.extern_pay_date > '&FECHA_FINAL 23:59:59'))
                                   and o.task_type_id = ot.task_type_id
                                UNION
                                SELECT distinct o.product_id
                                  FROM open.or_order_activity o, open.mo_packages m, open.or_order_items oi, 
  ...
  where (nvl(total, 0)      != 0 OR nvl(Ing_otro, 0)   != 0 OR nvl(Ing_Int_Mig, 0) != 0 OR nvl(Ing_CxC_Mig, 0) != 0 OR 
         nvl(Ing_RP_Mig,0)  != 0 OR nvl(ING_INT_OSF,0) != 0 OR nvl(ING_CXC_OSF, 0) != 0 OR nvl(ING_RP_OSF, 0)  != 0 OR
         nvl(ING_INT_CON,0) != 0 OR nvl(ING_CXC_CON,0) != 0 OR nvl(ING_RP_CON, 0)  != 0)
