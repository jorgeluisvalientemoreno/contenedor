-- INGRESOS REGISTROS CONTABLES
ALTER SESSION SET CURRENT_SCHEMA= "OPEN";
SELECT distinct clasificador, cuenta 
  FROM  (  
            ...
            FROM
                  (SELECT 
                         (
                            ...
                            AND    ecrcfech  BETWEEN  '01/08/2022' and '31/08/2022'
...
--where clasificador||cuenta not in (select ctaclco||ctcacodi from open.ldci_ctacadmi)
where clasificador not in (select g.id_clascont from open.ldci_clnitgen g where g.id_clascont = clasificador)
