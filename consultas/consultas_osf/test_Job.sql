...
begin
  ...
  --contador := contador + 1;
  BEGIN
    DBMS_SCHEDULER.CREATE_JOB(job_name   => 'JOB_EFIGAS_CREAOTAUTO',
                              job_type   => 'PLSQL_BLOCK',
                              job_action => 'BEGIN LDC_PROCESOCREAUTOTPNO.CREAOTAUTO; END;',
                              start_date => sysdate,
                              auto_drop  => TRUE,
                              enabled    => TRUE,
                              comments   => 'Job EFIGAS CREAOTAUTO');
  
    dbms_output.put_line('Job JOB_EFIGAS_CREAOTAUTO Ok.');
