SELECT D.ID_ITEMS_DOCUMENTO RESERVA,
       ...
       (select oou.name from open.or_operating_unit oou where oou.operating_unit_id =OPERATING_UNIT_ID and rownum = 1) NOMBRE_UNI,
       DESTINO_OPER_UNI_ID PROVEEDOR,
       (select oou.name from open.or_operating_unit oou where oou.operating_unit_id =DESTINO_OPER_UNI_ID and rownum = 1) NOMBRE_PROV,
       MMITDSAP DOCU_SAP,
       ITEMS_ID ITEM,
       ...
       NULL ORDEN,
       DECODE(MMITESTA,1,DECODE(MMITINTE,0,'CREADO','CON ERROR'),'PROCESADO') ESTADO_INTERFAZ,
       D.COMENTARIO
  ...
  LEFT JOIN OPEN.LDCI_DMITMMIT
    ON REQU.ITEMS_ID = DMITITEM and
       MMITCODI = DMITMMIT 
       LEFT JOIN open.ldci_seridmit on serinume='S-77132156-22'
and mmitcodi=serimmit
