...
select * from open.hicaespr where hcetnuse=52359077 order by hcetfech desc;


select s.sesususc, s.sesunuse, s.sesuesco, s.sesufein
  from servsusc s
 where s.sesunuse = (select ooa.product_id
                       from open.Or_Order_Activity ooa
                      where ooa.order_id = 194264736);
select a.subcategory_id, a.product_id, a.product_status_id
  from open.pr_product a
 where a.product_id = (select ooa.product_id
                         from open.Or_Order_Activity ooa
                        where ooa.order_id = 194264736);
select a.product_id, a.component_status_id
  from pr_component a
 where a.product_id = (select ooa.product_id
                         from open.Or_Order_Activity ooa
                        where ooa.order_id = 194264736);
