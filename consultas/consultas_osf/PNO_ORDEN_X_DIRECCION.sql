 ...
 (select oo.order_id Orden_Padre,
   ...
   where ooa.order_id = oo.order_id
     ...
     and oo.order_status_id in (8)
        --Valdiar que la orden relacionada sea del TT 10059 - REVISION INICIO ACTUACION ADMINISTRATIVA
     and (select count(1)
            from open.or_related_order oro
        ...
        --Valdiar que no sea acitivdad para pago por novedad
     and (select count(1)
...
select base.*,
  ...
  left join open.or_order_activity ooa
    on ooa.address_id = base.codigo_direccon
      --and ooa.product_id(+) = base.producto
