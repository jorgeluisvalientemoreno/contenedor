...
csbOSS_PNO_SMS_200398 CONSTANT VARCHAR2(100) := 'OSS_PNO_SMS_200398_1';
  ...
  NUORDEN open.OR_ORDER.ORDER_ID%TYPE := 128547047;--129426821;
  ...
  CURSOR CUSERVICIO IS
    SELECT S.*
     ...
     WHERE OOA.ORDER_ID = NUORDEN
       ...
       AND S.SESUSERV = 7014;
           --open.DALD_PARAMETER.fnuGetNumeric_Value('COD_SERV_GAS', NULL);
  ...
  CURSOR CUGRUPOPREGUNTAACTIVO IS
    SELECT LGP.*
      FROM open.LDC_PREGUNTA_GRUPO LPG, open.LDC_PREGUNTA LP, open.LDC_GRUPO_PREGUNTA LGP
  ...
  CNUTRAMITECAMBIOUSO       open.PS_PACKAGE_TYPE.PACKAGE_TYPE_ID%TYPE := 100225;--open.DALD_PARAMETER.fnuGetNumeric_Value('MOTIVO_CAMBIO_CATEGO',0);
  ...
  IF nvl(rfCURESPUESTAACTIVIDAD.ACTIVIDAD, 0) > 0 THEN
    ...
    IF CUGRUPOPREGUNTAACTIVO%FOUND THEN
      ...
      IF CUSERVICIO%FOUND THEN
        dbms_output.put_line('Categoria: '||TEMPCUSERVICIO.SESUCATE);
        ...
        IF CUDATORESPUESTA%FOUND THEN
          ...
          ELSIF TEMPCUDATORESPUESTA.ITEMS_ID IS NOT NULL THEN
            ...
            IF fblAplicaEntrega(csbOSS_PNO_SMS_200398) THEN
              --proLegalizaCierrePNO(inuOrden => nuOrden);
