...
select *
 ...
 where gs.subscriber_id = s.suscclie and gs.subscriber_id = 2392715;

select s.* from open.suscripc s where s.susccodi = 67562052       ;
select a.*, rowid from open.servsusc a where a.sesuSUSC=67562052        ;
select a.*, rowid from open.pr_product a where a.Subscription_Id = 67562052       ; 
---cliento por contrato
select *
  from open.GE_SUBSCRIBER gs
WHERE GS.PHONE = '3145681829';

select a.*, rowid from OPEN.GE_SUBS_PHONE a where a.subscriber_id =2392715; 
select a.*, rowid from OPEN.GE_SUBS_GENERAL_DATA a where a.subscriber_id =2392715; 

select a.*, rowid from OPEN.GE_SUBSCRIBER a where --A.PHONE = '3016927584'
...
select * from open.ab_address a where a.address_id in (868016, 1640017); --3572200,4250141)

--DATA GENERAL
SELECT datos.susccodi Contrato,
       a.address_id,
  ...
  FROM open.ge_subscriber a,
       ...
       (select s.* from open.suscripc s where s.susccodi = 67562052) datos
 WHERE a.subscriber_id = datos.suscclie
