...
where (
      ...
      and a.operating_unit_id in (3015, 2558)
...
where m.operating_unit_id=NUUDIDAD
  ...
  union all
    ...
    select   D.document_type_id, t.description, d.operating_unit_id, m.items_id,
      ...
      SUM(DECODE(D.OPERATING_UNIT_ID,NUUDIDAD,-1,DECODE(D.DESTINO_OPER_UNI_ID,NUUDIDAD,1,0))*AMOUNT) CANTIDAD

from open.ge_items_documento d, open.ge_document_type t, open.or_uni_item_bala_mov m

where (d.operating_unit_id=NUUDIDAD or d.destino_oper_uni_id=NUUDIDAD)
...
BEGIN
  ...
  if RGBODEGA.BALANCE != 0 AND REG.ITEMS_ID!=10004070 THEN
  ...
  ELSE
    ...
    NUCOSTPROM := round(RGBODEGA.TOTAL_COSTS/RGBODEGA.BALANCE,2);
    ...
    insert into ldc_act_ouib (ITEMS_ID,OPERATING_UNIT_ID,QUOTA,BALANCE,TOTAL_COSTS,OCCACIONAL_QUOTA,TRANSIT_IN,TRANSIT_OUT)
      ...
      UPDATE ldc_inv_ouib b SET B.BALANCE=REG.BALANCE, B.TOTAL_COSTS=REG.TOTAL_COSTS  where b.items_id=reg.items_id and b.operating_unit_id=reg.operating_unit_id;
    ...
    if sql%rowcount=0 then
      ...
      insert into ldc_inv_ouib (ITEMS_ID,OPERATING_UNIT_ID,QUOTA,BALANCE,TOTAL_COSTS,OCCACIONAL_QUOTA,TRANSIT_IN,TRANSIT_OUT)
        ...
        UPDATE ldc_act_ouib b SET B.BALANCE=REG.BALANCE, B.TOTAL_COSTS=REG.TOTAL_COSTS  where b.items_id=reg.items_id and b.operating_unit_id=reg.operating_unit_id;
    ...
    if sql%rowcount=0 then
      ...
      ELSE
        ...
        IF NUINVENTARIO>=NUACTIVO THEN
          ...
          NUCOSTACTI := ROUND(NUACTIVO*NUCOSTPROM,2);

          NUCOSTINVE := ROUND(NUINVENTARIO2*NUCOSTPROM,2);        

          UPDATE ldc_inv_ouib b SET B.BALANCE=(NUINVENTARIO+NUOTROS), b.total_costs=NUCOSTINVE  where b.items_id=reg.items_id and b.operating_unit_id=reg.operating_unit_id; 

      if sql%rowcount=0 then

      insert into ldc_inv_ouib (ITEMS_ID,OPERATING_UNIT_ID,QUOTA,BALANCE,TOTAL_COSTS,OCCACIONAL_QUOTA,TRANSIT_IN,TRANSIT_OUT)

              values(reg.items_id, reg.operating_unit_id, RGBODEGA.QUOTA,(NUINVENTARIO+NUOTROS),NUCOSTINVE, 0,null,null);

      end if;

          UPDATE ldc_act_ouib b SET B.BALANCE=(NUACTIVO), b.total_costs=NUCOSTACTI  where b.items_id=reg.items_id and b.operating_unit_id=reg.operating_unit_id; 
      ...
      if sql%rowcount=0 then
        ...
        ELSE
          ...
          UPDATE ldc_inv_ouib b SET B.BALANCE=(NUINVENTARIO), b.total_costs=NUCOSTINVE  where b.items_id=reg.items_id and b.operating_unit_id=reg.operating_unit_id; 
      ...
      if sql%rowcount=0 then
        ...
        insert into ldc_inv_ouib (ITEMS_ID,OPERATING_UNIT_ID,QUOTA,BALANCE,TOTAL_COSTS,OCCACIONAL_QUOTA,TRANSIT_IN,TRANSIT_OUT)
          ...
          UPDATE ldc_act_ouib b SET B.BALANCE=(NUACTIVO+NUOTROS), b.total_costs=NUCOSTACTI  where b.items_id=reg.items_id and b.operating_unit_id=reg.operating_unit_id; 

      if sql%rowcount=0 then

        insert into ldc_act_ouib (ITEMS_ID,OPERATING_UNIT_ID,QUOTA,BALANCE,TOTAL_COSTS,OCCACIONAL_QUOTA,TRANSIT_IN,TRANSIT_OUT)

                values(reg.items_id, reg.operating_unit_id, RGBODEGA.QUOTA,(NUACTIVO+NUOTROS),NUCOSTACTI, 0,null,null);
