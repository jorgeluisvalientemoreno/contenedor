...
where o.operating_unit_id=NUUDIDAD
 ...
 union all
  ...
  select  D.document_type_id, t.description, d.operating_unit_id, m.items_id,
      CASE WHEN D.DOCUMENT_TYPE_ID IN (105,103,115) AND D.CAUSAL_ID=3369 THEN 'I'
...
where m.operating_unit_id=NUUDIDAD
  ...
  and (move_date<'26/12/2017 11:17:33' or move_date>'29/12/2017 09:10:25')
  and movement_type!='N'
  and items_id=NUITEM
...
where (d.operating_unit_id=NUUDIDAD or d.destino_oper_uni_id=NUUDIDAD)
  ...
  and (move_date>='26/12/2017 11:17:33' and move_date<'29/12/2017 09:10:25')
  and movement_type='N'
...
where (d.operating_unit_id=NUUDIDAD or d.destino_oper_uni_id=NUUDIDAD)
 ...
 -- and not exists(select null from open.ldci_intemmit where documento_externo like '%'||mmitdsap)
  ...
  NUOTROS  NUMBER;
BEGIN
  DBMS_OUTPUT.PUT_LINE('CODIGO_ITEM'||'|'||'TOTAL_BODEGA'||'|' ||'INVENTARIO'||'|'||'ACTIVO'||'|'||'OTROS');
  FOR REG IN CUDATOS LOOP
    ...
    FOR REG2 IN CUVALIDACION(REG.OPERATING_UNIT_ID, REG.ITEMS_ID) LOOP
      ...
      END IF;
    END LOOP;
    DBMS_OUTPUT.PUT_LINE(REG.ITEMS_ID||'|'||REG.BALANCE||'|'||nvl(NUINVENTARIO,0)||'|'||nvl(NUACTIVO,0)||'|'||nvl(NUOTROS,0));
  END LOOP;
END;    
