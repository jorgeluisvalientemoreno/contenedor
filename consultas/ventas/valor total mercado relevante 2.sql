with info_dire as (
select category_ , subcategory_ subcate ,geograp_location_id , lomrmeco , address_parsed 
from open.ab_premise 
inner join  open.ab_address on premise_id =  estate_number 
inner join open.fa_locamere  fa on fa.lomrloid= geograp_location_id 
where address_id = &address_id --Ingresar address_id 
),
--1 -- 22 mercado relevante 
--2 -- 9 plan comercial 
--3-- 8 subcategoria
--4 --- categoria
conceptos_base as ( 
  select null as promotion_id,
   ...
   inner join open.ta_tariconc ta on cotccons = tacocotc
   inner join info_dire on tacocr01= info_dire.lomrmeco and tacocr04= info_dire.category_
   ...
   where cotcconc in (19, 30, 674)
     ...
     and tacocr02 = &plan_come
     and (tacocr03 = info_dire.subcate or tacocr03 = -1)
     and tacocr01 = info_dire.lomrmeco
     and tacocr04  = info_dire.category_ 
) 
,
conceptos_iva as ( 
  select null as promotion_id,
...
concepto_iva_subsidio as (
  select null as promotion_id,
         cb2.coblconc as concepto_genera,
         cb2.coblcoba as concepto_aplica,
         CASE  WHEN promociones.valor IS NOT NULL THEN promociones.valor * (r.ravtporc / 100) 
           ELSE conceptos_sub.valor * (r.ravtporc / 100)  END as valor 
   from open.concbali cb2
   left  join conceptos_sub on conceptos_sub.concepto_genera = cb2.coblcoba
   left  join promociones on promociones.concepto_genera = cb2.coblcoba
   left join open.concepto c on cb2.coblconc = c.conccodi and c.concticl = 4
   left join open.ta_conftaco t  on cb2.coblconc = t.cotcconc
   left join open.ta_tariconc ta  on cotccons = tacocotc  
   left join open.ta_vigetaco vi  on tacocons = vitctaco 
   left join open.ta_rangvitc r on ravtvitc = vitccons
   where cb2.coblcoba = promociones.concepto_genera
   or cb2.coblcoba = conceptos_sub.concepto_genera
) 
