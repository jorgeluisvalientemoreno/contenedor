select distinct ex.executable_id, 
                ...
                e.name_, 
                da.default_where,
                (SELECT COUNT(1) FROM OPEN.GE_ENTITY_ATTRIBUTES EA WHERE  EA.ENTITY_ID=E.ENTITY_ID)
...
FROM OPEN.GI_ATTRIB_DISP_DATA D,  OPEN.GE_ENTITY_ATTRIBUTES AT, OPEN.GE_ENTITY E,  OPEN.SA_EXECUTABLE EX, OPEN.GI_ENTITY_DISP_DATA DA
WHERE D.EXECUTABLE_ID=EX.EXECUTABLE_ID
 AND NAME=upper('LDCCOMBSOL')
  AND D.ENTITY_ATTRIBUTE_ID=AT.ENTITY_ATTRIBUTE_ID
  AND D.ENTITY_ID=E.ENTITY_ID
...
FROM OPEN.GI_ATTRIB_DISP_DATA D,  
     OPEN.GE_ENTITY_ATTRIBUTES AT
     LEFT JOIN OPEN.GE_ATTR_ALLOWED_VALUES VALPER ON VALPER.ENTITY_ATTRIBUTE_ID=AT.ENTITY_ATTRIBUTE_ID, 
     OPEN.GE_ENTITY E,  
     OPEN.SA_EXECUTABLE EX, 
     OPEN.GI_ENTITY_DISP_DATA DA
WHERE D.EXECUTABLE_ID=EX.EXECUTABLE_ID
