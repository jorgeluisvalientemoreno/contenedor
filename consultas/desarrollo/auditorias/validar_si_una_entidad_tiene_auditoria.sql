...
WHERE b.entity_attribute_id = c.entity_attribute_id
  ...
  and (select t.STATUS from dba_triggers t where t.TRIGGER_NAME=au.trigger_name)!='DISABLED'
;
SELECT B.AUDIT_LOG_ID,
       ...
       DECODE(AP.AUD_POL_STATUS_ID, 0,'JUST_CREATED_STATUS',3,'ENABLED_INVALID_POLICY_STATUS',4,'ENABLED_POLICY_STATUS',5,'DISABLED_POLICY_STATUS ',7,'NO_CONSISTENT_POLICY_STATUS',8,'DISABLED_INVALID_POLICY_STATUS',18,'DELETED_POLICY_STATUS') DESC_AUD_POL_STATUS,
       (select t.STATUS from dba_triggers t where t.TRIGGER_NAME=au.trigger_name) estado_trigger,
...
FROM open.au_aud_pol_attr b,
     ...
     open.ge_entity d,
     open.AU_LOG_POLICY au
     left join open.GE_EVENT ev on ev.event_id = au.event_id,
     open.AU_AUDIT_POLICY ap
