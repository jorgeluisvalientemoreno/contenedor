...
where
  parent.TABLE_NAME = upper('gr_config_expression')
...
select 'select '''||hija.table_name||''' as tabla,'||u.column_name||' from open.'||u.TABLE_NAME||' where '||u.column_name||' is not null union all'
...
inner join dba_constraints padre on hija.r_constraint_name = padre.constraint_name and hija.r_owner = padre.owner and padre.TABLE_NAME = upper('gr_config_expression')


select 'select '''||tc.table_name||''' as tabla,'||tc.COLUMN_NAME||' from open.'||tc.table_name||' where '||tc.COLUMN_NAME||' is not null union all'
