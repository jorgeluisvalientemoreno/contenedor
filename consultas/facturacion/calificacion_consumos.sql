with  Consumo_actual as (select   (:lectura_ingresar +:cons_cambiomedidor)  Consumo_desv
                        from dual
) , max_perifact as  ( select max(l2.leempefa) periodo_fact  from open.lectelme l2 where l2.leemsesu = :producto)
...
and caravaco.crvcsuca = (select subcategory_id   
                         from open.pr_product
                         where product_id = :producto)
and  :cp  between caravaco.crvcraci and caravaco.crvcracf
and (select round(( Consumo_desv - :cpp) /  :cp * 100)  
