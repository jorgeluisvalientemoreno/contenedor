CREATE OR REPLACE PACKAGE DAGE_PROCESS_SCHEDULE IS
   CURSOR CURECORD( INUPROCESS_SCHEDULE_ID IN GE_PROCESS_SCHEDULE.PROCESS_SCHEDULE_ID%TYPE ) IS
...
SELECT GE_process_schedule.*,GE_process_schedule.rowid
		...
		WHERE 
			rowId = irirowid;
   SUBTYPE STYGE_PROCESS_SCHEDULE IS CURECORD%ROWTYPE;
   TYPE TYREFCURSOR IS REF CURSOR;
   TYPE TYTBGE_PROCESS_SCHEDULE IS TABLE OF STYGE_PROCESS_SCHEDULE INDEX BY BINARY_INTEGER;
   TYPE TYRFRECORDS IS REF CURSOR
    RETURN STYGE_PROCESS_SCHEDULE;
   TYPE TYTBPROCESS_SCHEDULE_ID IS TABLE OF GE_PROCESS_SCHEDULE.PROCESS_SCHEDULE_ID%TYPE INDEX BY BINARY_INTEGER;
   TYPE TYTBEXECUTABLE_ID IS TABLE OF GE_PROCESS_SCHEDULE.EXECUTABLE_ID%TYPE INDEX BY BINARY_INTEGER;
   TYPE TYTBPARAMETERS_ IS TABLE OF GE_PROCESS_SCHEDULE.PARAMETERS_%TYPE INDEX BY BINARY_INTEGER;
   TYPE TYTBFREQUENCY IS TABLE OF GE_PROCESS_SCHEDULE.FREQUENCY%TYPE INDEX BY BINARY_INTEGER;
   TYPE TYTBSTATUS IS TABLE OF GE_PROCESS_SCHEDULE.STATUS%TYPE INDEX BY BINARY_INTEGER;
   TYPE TYTBJOB IS TABLE OF GE_PROCESS_SCHEDULE.JOB%TYPE INDEX BY BINARY_INTEGER;
   TYPE TYTBLOG_USER IS TABLE OF GE_PROCESS_SCHEDULE.LOG_USER%TYPE INDEX BY BINARY_INTEGER;
   TYPE TYTBSTART_DATE_ IS TABLE OF GE_PROCESS_SCHEDULE.START_DATE_%TYPE INDEX BY BINARY_INTEGER;
   TYPE TYTBWHAT IS TABLE OF GE_PROCESS_SCHEDULE.WHAT%TYPE INDEX BY BINARY_INTEGER;
   TYPE TYTBROWID IS TABLE OF ROWID INDEX BY BINARY_INTEGER;
   TYPE TYRCGE_PROCESS_SCHEDULE IS RECORD
    (
      PROCESS_SCHEDULE_ID TYTBPROCESS_SCHEDULE_ID,
      EXECUTABLE_ID TYTBEXECUTABLE_ID,
      PARAMETERS_ TYTBPARAMETERS_,
      FREQUENCY TYTBFREQUENCY,
      STATUS TYTBSTATUS,
      JOB TYTBJOB,
      LOG_USER TYTBLOG_USER,
      START_DATE_ TYTBSTART_DATE_,
      WHAT TYTBWHAT,
      ROW_ID TYTBROWID
    );
   FUNCTION FSBVERSION
